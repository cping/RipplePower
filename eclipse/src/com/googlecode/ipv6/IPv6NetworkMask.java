...
/**
 * Immutable representation of an IPv6 network mask. A network mask is nothing
 * more than an IPv6 address with a continuous range of 1 bits starting from the
 * most significant bit. A network mask can also be represented as a prefix
 * length, which is the count of these 1 bits.
 * 
...
public final class IPv6NetworkMask {
	...
	/**
	 * Construct an IPv6 network mask from a prefix length. The prefix length
	 * should be in the interval ]0, 128].
	 * 
	 * @param prefixLength
	 *            prefix length
	...
	IPv6NetworkMask(int prefixLength) {
		if (prefixLength < 0 || prefixLength > 128)
			throw new IllegalArgumentException(
					"prefix length should be in interval [0, 128]");

		this.prefixLength = prefixLength;
	}

	/**
	 * Construct an IPv6 network mask from an IPv6 address. The address should
	 * be a valid network mask.
	 * 
	 * @param iPv6Address
	...
	private static void validateNetworkMask(IPv6Address addressToValidate) {
		final BitSet addressAsBitSet = bitSetOf(addressToValidate.getLowBits(),
				addressToValidate.getHighBits());
		if (!addressAsBitSet.get(127)) {
			throw new IllegalArgumentException(addressToValidate
					+ " is not a valid network mask");
		} else {
	...
	public IPv6Address asAddress() {
		if (prefixLength == 128) {
			...
			return new IPv6Address(0xFFFFFFFFFFFFFFFFL,
					(0xFFFFFFFFFFFFFFFFL << (64 - remainingPrefixLength)));
		} else {
			return new IPv6Address(0xFFFFFFFFFFFFFFFFL << (64 - prefixLength),
					0);
		}
	}
