...
public final class IPv6AddressHelpers {
	...
	static void validateLongs(long[] longs) {
		...
		for (long l : longs) {
			...
			if (l > 0xFFFF)
				throw new IllegalArgumentException(
						"each element should be less than 0xFFFF ["
								+ Arrays.toString(longs) + "]");
		}
	}
	...
	/**
	 * Replaces a w.x.y.z substring at the end of the given string with
	 * corresponding hexadecimal notation. This is useful in case the string was
	 * using IPv4-Mapped address notation.
	 */
	...
	static byte[] prefixWithZeroBytes(byte[] original, int newSize) {
		...
		System.arraycopy(original, 0, target, newSize - original.length,
				original.length);
		return target;
	}
}
