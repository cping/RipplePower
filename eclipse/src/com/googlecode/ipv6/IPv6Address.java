...
public final class IPv6Address implements Comparable<IPv6Address> {
	...
	public static final IPv6Address MAX = IPv6Address
			.fromString("ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff");
	...
	/**
	 * Construct an IPv6Address from two longs representing the 64 highest and
	 * 64 lowest bits. It is usually easier to construct IPv6Addresses from a
	 * {@link String} or an {@link java.net.InetAddress}. The internal
	 * representation of an IPv6Address is exactly these two longs though, so if
	 * you already happen to have them, this provides a very efficient way to
	 * construct an IPv6Address.
	 * 
	 * @param highBits
	 *            highest order bits
	 * @param lowBits
	...
	public static IPv6Address fromString(final String string) {
		...
		final String withoutIPv4MappedNotation = IPv6AddressHelpers
				.rewriteIPv4MappedNotation(string);
		final String longNotation = IPv6AddressHelpers
				.expandShortNotation(withoutIPv4MappedNotation);

		final long[] longs = tryParseStringArrayIntoLongArray(string,
				longNotation);
	...
	private static long[] tryParseStringArrayIntoLongArray(String string,
			String longNotation) {
		try {
			return IPv6AddressHelpers
					.parseStringArrayIntoLongArray(longNotation.split(":"));
	...
	public byte[] toByteArray() {
		ByteBuffer byteBuffer = ByteBuffer.allocate(N_BYTES).putLong(highBits)
				.putLong(lowBits);
	 ...
	 * The magnitude of the {@link java.math.BigInteger} represents the IPv6
	 * address value. Or in other words, the {@link java.math.BigInteger} with
	...
	public static IPv6Address fromBigInteger(final BigInteger bigInteger) {
		...
		if (bigInteger.compareTo(BigInteger.ZERO) < 0)
			throw new IllegalArgumentException(
					"can not construct from negative value");
		if (bigInteger.compareTo(MAX.toBigInteger()) > 0)
			throw new IllegalArgumentException(
					"bigInteger represents a value bigger than 2^128 - 1");

		byte[] bytes = bigInteger.toByteArray();

		if (bytes[0] == 0) {
			// a zero byte was added to represent the (always positive, hence
			// zero) sign bit
			return fromByteArray(prefixWithZeroBytes(
	...
	/**
	 * @return {@link java.math.BigInteger} representation. The magnitude of the
	 *         {@link java.math.BigInteger} represents the IPv6 address value.
	 *         Or in other words, the {@link java.math.BigInteger} with value N
	 *         defines the Nth possible IPv6 address.
	 */
	...
	/**
	 * Subtraction. Will never underflow, but wraps around when the lowest ip
	 * address has been reached.
	 * 
	 * @param value
	 *            value to substract
	...
	public IPv6Address subtract(int value) {
		...
		if (value >= 0) {
			if (IPv6AddressHelpers.isLessThanUnsigned(lowBits, newLowBits)) {
				// oops, we subtracted something postive and the result is
				// bigger -> overflow detected (carry over one bit from high to
				// low)
				return new IPv6Address(highBits - 1, newLowBits);
			} else {
				// no overflow
			...
			if (IPv6AddressHelpers.isLessThanUnsigned(newLowBits, lowBits)) {
				// oops, we subtracted something negative and the result is
				// smaller -> overflow detected (carry over one bit from low to
				// high)
				return new IPv6Address(highBits + 1, newLowBits);
			} else {
				// no overflow
	 ...
	 * @param networkMask
	 *            network mask
	 * @return an address of which the last 128 - networkMask.asPrefixLength()
	 *         bits are zero
	 */
	public IPv6Address maskWithNetworkMask(final IPv6NetworkMask networkMask) {
		if (networkMask.asPrefixLength() == 128) {
			...
			return new IPv6Address(this.highBits, this.lowBits
					& (0xFFFFFFFFFFFFFFFFL << (64 - remainingPrefixLength)));
		} else {
			// apply mask on high bits, low bits completely 0
			return new IPv6Address(
					this.highBits
							& (0xFFFFFFFFFFFFFFFFL << (64 - networkMask
									.asPrefixLength())),
					0);
		}
	}
	 ...
	 *         bits are one
	 */
	public IPv6Address maximumAddressWithNetworkMask(
		...
		if (networkMask.asPrefixLength() == 128) {
			...
			final int remainingPrefixLength = networkMask.asPrefixLength() - 64;
			return new IPv6Address(this.highBits, this.lowBits
					| (0xFFFFFFFFFFFFFFFFL >>> remainingPrefixLength));
		} else {
			// apply mask on high bits, low bits completely 1
			return new IPv6Address(this.highBits
					| (0xFFFFFFFFFFFFFFFFL >>> networkMask.asPrefixLength()),
					0xFFFFFFFFFFFFFFFFL);
		}
	}
	...
	public IPv6Address setBit(final int bit) {
		if (bit < 0 || bit > 127)
			throw new IllegalArgumentException(
					"can only set bits in the interval [0, 127]");

		if (bit < 64) {
			return new IPv6Address(this.highBits, this.lowBits | (1 << bit));
		} else {
			return new IPv6Address(this.highBits | (1 << (bit - 64)),
					this.lowBits);
		}
	}
	 ...
	 * addresses, the first 80 bits are zero, the next 16 bits are one, and the
	 * remaining 32 bits are the IPv4 address.
	 * 
	 * @return true if the address is an IPv4-mapped IPv6 addresses.
	 */
	public boolean isIPv4Mapped() {
		return this.highBits == 0 // 64 zero bits
				&& (this.lowBits & 0xFFFF000000000000L) == 0 // 16 more zero
																// bits
				&& (this.lowBits & 0x0000FFFF00000000L) == 0x0000FFFF00000000L; // 16
																				// one
																				// bits
																				// and
																				// the
	...
	/**
	 * @return true if the address is an IPv6 multicast address (an address in
	 *         the network ff00::/8)
	 */
	...
	/**
	 * @return true if the address is an IPv6 site-local address (an address in
	 *         the network fec0::/48)
	 */
	...
	/**
	 * @return true if the address is an IPv6 link-local address (an address in
	 *         the network fe80::/64)
	 */
	 ...
	 * shorthand notation and special notation for IPv4-mapped IPv6 addresses
	 * whenever possible.
	 * 
	...
	private String toIPv4MappedAddressString() {
		...
		result.append(byteZero).append(".").append(byteOne).append(".")
				.append(byteTwo).append(".").append(byteThree);
	...
	private String toShortHandNotationString() {
		...
		boolean useShortHandNotation = shortHandNotationLength > 1; // RFC5952
																	// recommends
																	// not to
																	// use
																	// shorthand
		...
		for (int i = 0; i < strings.length; i++) {
			if (useShortHandNotation && i == shortHandNotationPosition) {
				if (i == 0)
					...
					result.append(":");
			} else if (!(i > shortHandNotationPosition && i < shortHandNotationPosition
	...
	/**
	 * @return String representation of the IPv6 address, never using shorthand
	 *         notation.
	 */
	...
	public int compareTo(IPv6Address that) {
		if (this.highBits == that.highBits)
			if (this.lowBits == that.lowBits)
				...
				return IPv6AddressHelpers.isLessThanUnsigned(this.lowBits,
						that.lowBits) ? -1 : 1;
		else if (this.highBits == that.highBits)
			return 0;
		else
			return IPv6AddressHelpers.isLessThanUnsigned(this.highBits,
					that.highBits) ? -1 : 1;
	}
	...
	public int numberOfTrailingZeroes() {
		return lowBits == 0 ? Long.numberOfTrailingZeros(highBits) + 64 : Long
				.numberOfTrailingZeros(lowBits);
	}

	public int numberOfTrailingOnes() {
		// count trailing ones in "value" by counting the trailing zeroes in
		// "value + 1"
		final IPv6Address plusOne = this.add(1);
		return plusOne.getLowBits() == 0 ? Long.numberOfTrailingZeros(plusOne
				.getHighBits()) + 64 : Long.numberOfTrailingZeros(plusOne
				.getLowBits());
	}

	public int numberOfLeadingZeroes() {
		return highBits == 0 ? Long.numberOfLeadingZeros(lowBits) + 64 : Long
				.numberOfLeadingZeros(highBits);
	}

	public int numberOfLeadingOnes() {
		// count leading ones in "value" by counting leading zeroes in "~ value"
		final IPv6Address flipped = new IPv6Address(~this.highBits,
				~this.lowBits);
