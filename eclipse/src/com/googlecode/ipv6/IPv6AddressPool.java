 ...
 * <p/>
 * An IPv6 address pool is like an IPv6 address range in which some addresses
 * are "free" and some are "allocated". Think "dhcp server". Addresses are
 * allocated in whole subnet blocks at once. These subnet blocks have a
 * predefined prefix length for the whole allocatable range.
 * 
...
public final class IPv6AddressPool {
	...
	/**
	 * Create a pool of the given range (boundaries inclusive) which is
	 * completely free. The given subnet size is the network mask (thus size) of
	 * the allocated subnets in this range. This constructor verifies that the
	 * whole range is "aligned" with subnets of this size (i.e. there should not
	 * be a waste of space in the beginning or end which is smaller than one
	 * subnet of the given subnet size).
	 * 
	 * @param range
	 *            range from within to allocate
	 * @param allocationSubnetSize
	 *            size of the subnets that will be allocated
	 */
	public static IPv6AddressPool fromRangeAndSubnet(
			final IPv6AddressRange range,
			final IPv6NetworkMask allocationSubnetSize) {
		// in the beginning, all is free
		return new IPv6AddressPool(range, allocationSubnetSize,
				new TreeSet<IPv6AddressRange>(Arrays.asList(range)), null);
	}

	/**
	 * Private constructor to construct a pool with a given set of free ranges
	 * and a network which was just allocated.
	 * 
	 * @param range
	 *            range from within to allocate
	 * @param allocationSubnetSize
	 *            size of the subnets that will be allocated
	 * @param freeRanges
	 *            free ranges in the allocatable IP address range
	 */
	private IPv6AddressPool(final IPv6AddressRange range,
			final IPv6NetworkMask allocationSubnetSize,
			final SortedSet<IPv6AddressRange> freeRanges,
			final IPv6Network lastAllocated) {
		...
		validateRangeIsMultipleOfSubnetsOfGivenSize(underlyingRange,
				allocationSubnetSize);
	}

	private void validateFreeRanges(IPv6AddressRange range,
			SortedSet<IPv6AddressRange> toValidate) {
		if (!toValidate.isEmpty() && !checkWithinBounds(range, toValidate))
			throw new IllegalArgumentException(
					"invalid free ranges: not all within bounds of overall range");

		// TODO: some more validations would be useful. For example the free
		// ranges should be defragmented and non overlapping etc
	}

	private boolean checkWithinBounds(IPv6AddressRange range,
			SortedSet<IPv6AddressRange> toValidate) {
		return (toValidate.first().getFirst().compareTo(range.getFirst()) >= 0 && toValidate
				.last().getLast().compareTo(range.getLast()) <= 0);
	}

	private void validateRangeIsMultipleOfSubnetsOfGivenSize(
			IPv6AddressRange range, IPv6NetworkMask allocationSubnetSize) {
		...
		if (range.getFirst().numberOfTrailingZeroes() < allocatableBits)
			throw new IllegalArgumentException("range [" + this
					+ "] is not aligned with prefix length ["
					+ allocationSubnetSize.asPrefixLength() + "], "
					+ "first address should end with " + allocatableBits
					+ " zero bits");
		...
		if (range.getLast().numberOfTrailingOnes() < allocatableBits)
			throw new IllegalArgumentException("range [" + this
					+ "] is not aligned with prefix length ["
					+ allocationSubnetSize.asPrefixLength()
					+ "], last address should end with " + allocatableBits
					+ " one bits");
	}

	/**
	 * @return the last IPv6Network which was allocated or null if none was
	 *         allocated yet
	 */
	...
	public IPv6AddressPool allocate() {
		if (!isExhausted()) {
			// get the first range of free subnets, and take the first subnet of
			// that range
			final IPv6AddressRange firstFreeRange = freeRanges.first();
			final IPv6Network allocated = IPv6Network.fromAddressAndMask(
					firstFreeRange.getFirst(), allocationSubnetSize);
	...
	public IPv6AddressPool allocate(IPv6Network toAllocate) {
		if (!contains(toAllocate))
			throw new IllegalArgumentException(
					"can not allocate network which is not contained in the pool to allocate from ["
							+ toAllocate + "]");

		if (!this.allocationSubnetSize.equals(toAllocate.getNetmask()))
			throw new IllegalArgumentException(
					"can not allocate network with prefix length /"
							+ toAllocate.getNetmask().asPrefixLength()
	...
	private IPv6AddressRange findFreeRangeContaining(IPv6Network toAllocate) {
		...
		// the range containing the network to allocate is either the first of
		// the tail, or the last of the head, or it doesn't exist
	...
	/**
	 * Private helper method to perform the allocation of a subnet within one of
	 * the free ranges.
	 * 
	 * @param toAllocate
	 *            subnet to allocate
	 * @param rangeToAllocateFrom
	 *            free range to allocate from
	 * @return resulting pool
	 */
	private IPv6AddressPool doAllocate(final IPv6Network toAllocate,
		...
		final TreeSet<IPv6AddressRange> newFreeRanges = new TreeSet<IPv6AddressRange>(
				this.freeRanges);
		...
		final List<IPv6AddressRange> newRanges = rangeToAllocateFrom
				.remove(toAllocate);

		// and add the resulting ranges as new free ranges
		newFreeRanges.addAll(newRanges);

		return new IPv6AddressPool(underlyingRange, allocationSubnetSize,
				newFreeRanges, toAllocate);
	}
	...
	public IPv6AddressPool deAllocate(final IPv6Network toDeAllocate) {
		if (!contains(toDeAllocate)) {
			throw new IllegalArgumentException("Network to de-allocate["
					+ toDeAllocate
					+ "] is not contained in this allocatable range [" + this
					+ "]");
		}

		// find ranges just in front or after the network to deallocate. These
		// are the ranges to merge with to prevent fragmentation.
		...
		if ((freeRangeBeforeNetwork == null) && (freeRangeAfterNetwork == null)) {
			...
			if ((freeRangeBeforeNetwork != null)
					&& (freeRangeAfterNetwork != null)) {
				// merge two existing ranges
				newFreeRanges.remove(freeRangeBeforeNetwork);
				newFreeRanges.remove(freeRangeAfterNetwork);
				newFreeRanges.add(IPv6AddressRange.fromFirstAndLast(
						freeRangeBeforeNetwork.getFirst(),
						freeRangeAfterNetwork.getLast()));
			} else if (freeRangeBeforeNetwork != null) {
				// append
				newFreeRanges.remove(freeRangeBeforeNetwork);
				newFreeRanges.add(IPv6AddressRange.fromFirstAndLast(
						freeRangeBeforeNetwork.getFirst(),
						toDeAllocate.getLast()));
			} else /* if (freeRangeAfterNetwork != null) */
			{
				// prepend
				newFreeRanges.remove(freeRangeAfterNetwork);
				newFreeRanges.add(IPv6AddressRange.fromFirstAndLast(
						toDeAllocate.getFirst(),
						freeRangeAfterNetwork.getLast()));
			}
		}

		return new IPv6AddressPool(underlyingRange, allocationSubnetSize,
				newFreeRanges, getLastAllocated());
	}

	/**
	 * Private helper method to find the free range just before the given
	 * network.
	 */
	...
	/**
	 * Private helper method to find the free range just after the given
	 * address.
	 */
	...
	public boolean isFree(final IPv6Network network) {
		...
		if (!this.allocationSubnetSize.equals(network.getNetmask()))
			throw new IllegalArgumentException("network of prefix length ["
	...
	/**
	 * @return all networks (all with the same fixed prefix length) which are
	 *         free in this pool
	 */
		...
		return new Iterable<IPv6Network>() {
			...
			public Iterator<IPv6Network> iterator() {
				return new Iterator<IPv6Network>() {
					/*
					 * Iteration is implemented by allocating from a separate
					 * pool.
					 */
					...
					public IPv6Network next() {
						if (hasNext()) {
							poolInstanceUsedForIteration = poolInstanceUsedForIteration
									.allocate();
	...
	public boolean equals(Object o) {
		...
		if (allocationSubnetSize != null ? !allocationSubnetSize
				.equals(that.allocationSubnetSize)
				: that.allocationSubnetSize != null)
			return false;
		if (freeRanges != null ? !freeRanges.equals(that.freeRanges)
				: that.freeRanges != null)
			return false;
		if (lastAllocated != null ? !lastAllocated.equals(that.lastAllocated)
				: that.lastAllocated != null)
			return false;
		if (underlyingRange != null ? !underlyingRange
				.equals(that.underlyingRange) : that.underlyingRange != null)
			return false;
	...
	public int hashCode() {
		...
		result = 31
				* result
				+ (allocationSubnetSize != null ? allocationSubnetSize
						.hashCode() : 0);
		result = 31 * result
				+ (lastAllocated != null ? lastAllocated.hashCode() : 0);
		return result;
	}
	...
	/**
	 * @return like <code>toString</code> but without using shorthand notations
	 *         for addresses
	 */
