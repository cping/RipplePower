...
/**
 * Immutable representation of an IPv6 network based on an address and a prefix
 * length. An IPv6 network is also an IPv6 address range (but not all ranges are
 * valid networks).
 * 
...
public final class IPv6Network extends IPv6AddressRange {
	...
	private IPv6Network(IPv6Address address, IPv6NetworkMask networkMask) {
		super(address.maskWithNetworkMask(networkMask), address
				.maximumAddressWithNetworkMask(networkMask));
	 ...
	 * 
	 * @param address
	 *            IPv6 address (the network address or any other address within
	 *            the network)
	 * @param networkMask
	 *            IPv6 network mask
	 * @return IPv6 network
	...
	/**
	 * Create an IPv6 network from the two addresses within the network. This
	 * will construct the smallest possible network ("longest prefix length")
	 * which contains both addresses.
	 * 
	 * @param one
	 *            address one
	 * @param two
	...
	public static IPv6Network fromTwoAddresses(IPv6Address one, IPv6Address two) {
		final IPv6NetworkMask longestPrefixLength = IPv6NetworkMask
				.fromPrefixLength(IPv6NetworkHelpers.longestPrefixLength(one,
						two));
		return new IPv6Network(one.maskWithNetworkMask(longestPrefixLength),
				longestPrefixLength);
	}
	...
	public static IPv6Network fromString(String string) {
		...
		final IPv6Address networkAddress = IPv6Address
				.fromString(networkAddressString);

		return fromAddressAndMask(networkAddress, new IPv6NetworkMask(
				prefixLength));
	}
	...
	private static int parsePrefixLength(String string) {
		try {
			...
			throw new IllegalArgumentException(
					"Prefix length should be a positive integer");
		}
	}
	 ...
	 * 
	 * @param size
	 *            size (expressed as {@link com.googlecode.ipv6.IPv6NetworkMask}
	 *            ) of the subnets
	...
	public Iterator<IPv6Network> split(IPv6NetworkMask size) {
		if (size.asPrefixLength() < this.getNetmask().asPrefixLength())
			throw new IllegalArgumentException(
					String.format(
							"Can not split a network of size %s in subnets of larger size %s",
							this.getNetmask().asPrefixLength(),
							size.asPrefixLength()));
	...
	public boolean equals(Object o) {
		...
		if (address != null ? !address.equals(that.address)
				: that.address != null)
			return false;
		if (networkMask != null ? !networkMask.equals(that.networkMask)
				: that.networkMask != null)
			return false;
	...
	public int hashCode() {
		...
		result = 31 * result
				+ (networkMask != null ? networkMask.hashCode() : 0);
		return result;
	}
	...
	private final class IPv6NetworkSplitsIterator implements
		...
		public IPv6NetworkSplitsIterator(IPv6NetworkMask size) {
			this.size = size;
			this.nbrAddressesPerSplit = BigInteger.ONE.shiftLeft(128 - size
					.asPrefixLength());
			this.current = IPv6Network.fromAddressAndMask(
					IPv6Network.this.address, size);
		}
		...
		private IPv6Network calculateNext(IPv6Network current) {
			BigInteger next = current.address.toBigInteger().add(
					nbrAddressesPerSplit);
			return IPv6Network.fromAddressAndMask(
					IPv6Address.fromBigInteger(next), size);
		}
