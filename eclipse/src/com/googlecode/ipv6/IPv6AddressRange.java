...
/**
 * Immutable representation of a continuous range of IPv6 addresses (bounds
 * included).
 * 
 * @author Jan Van Besien
 */
public class IPv6AddressRange implements Comparable<IPv6AddressRange>,
	...
	IPv6AddressRange(IPv6Address first, IPv6Address last) {
		if (first.compareTo(last) > 0)
			throw new IllegalArgumentException(
					"Cannot create ip address range with last address < first address");
	 ...
	 * with a single IPv6 network) into a minimal set of non overlapping
	 * consecutive subnets.
	 * 
	 ...
	 * ranges. If an address outside the range is removed, this has no effect.
	 * If the first or last address is removed, a single new range is returned
	 * (potentially empty if the range only contained a single address). If an
	 * address somewhere else in the range is removed, two new ranges are
	 * returned.
	 * 
	 * @param address
	...
	public List<IPv6AddressRange> remove(IPv6Address address) {
		...
		if (!contains(address))
			...
			return Collections.singletonList(fromFirstAndLast(first.add(1),
					last));
		else if (address.equals(last))
			return Collections.singletonList(fromFirstAndLast(first,
					last.subtract(1)));
		else
	...
	/**
	 * Extend the range just enough at its head or tail such that the given
	 * address is included.
	 * 
	 * @param address
	...
	/**
	 * Remove a network from the range, resulting in one, none or two new
	 * ranges. If a network outside (or partially outside) the range is removed,
	 * this has no effect. If the network which is removed is aligned with the
	 * beginning or end of the range, a single new ranges is returned
	 * (potentially empty if the range was equal to the network which is removed
	 * from it). If a network somewhere else in the range is removed, two new
	 * ranges are returned.
	 * 
	 * @param network
	...
	public List<IPv6AddressRange> remove(IPv6Network network) {
		...
		if (!contains(network))
			...
			return Collections.singletonList(fromFirstAndLast(network.getLast()
					.add(1), last));
		else if (last.equals(network.getLast()))
			return Collections.singletonList(fromFirstAndLast(first, network
					.getFirst().subtract(1)));
		else
			return Arrays.asList(
					fromFirstAndLast(first, network.getFirst().subtract(1)),
	...
	/**
	 * The natural order of {@link com.googlecode.ipv6.IPv6AddressRange}s orders
	 * them on increasing first addresses, and on increasing last address if the
	 * first address would be equal.
	 * <p/>
	 * Note that the natural order does thus not compare sizes of ranges.
	 * 
	 * @param that
	 *            range to compare with
	 * @return negative, zero or positive depending on whether this is smaller,
	 ...
	 */
	private final class IPv6AddressRangeIterator implements
		...
		public void remove() {
			throw new UnsupportedOperationException(
					"This iterator provides read only access");
		}
	}

	private class IPv6AddressRangeAsSubnetsIterator implements
		...
		public IPv6Network next() {
			...
			if (hasNext()) {
				...
				// try setting the step-th bit until we reach a bit that is
				// already set
				while (!(base.setBit(step)).equals(base)) {
					// if the max address in this subnet is beyond the end of
					// the range, we went too far
					if ((base.maximumAddressWithNetworkMask(
							IPv6NetworkMask.fromPrefixLength(127 - step))
							.compareTo(last) > 0)) {
						break;
					}
					step++;
				...
				next = IPv6Network.fromAddressAndMask(base,
						IPv6NetworkMask.fromPrefixLength(128 - step));
		...
		@Override
		public boolean hasNext() {
			// there is a next subnet as long as we didn't reach the end of the
			// range
