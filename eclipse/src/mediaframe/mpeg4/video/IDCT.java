...
public final class IDCT {
	 ...
	 * row (horizontal) IDCT
	 * 
	 * 7 pi 1 dst[k] = sum c[l] * src[l] * cos( -- * ( k + - ) * l ) l=0 8 2
	 * 
	...
	private void idctrow(int[] blk, int[] pixel_data, int block_pointer) {
		...
		if (((x1 = blk[4] << 11) | (x2 = blk[6]) | (x3 = blk[2])
			...
			pixel_data[block_pointer] = pixel_data[block_pointer + 1] = pixel_data[block_pointer + 2] = pixel_data[block_pointer + 3] = pixel_data[block_pointer + 4] = pixel_data[block_pointer + 5] = pixel_data[block_pointer + 6] = pixel_data[block_pointer + 7] = blk[0] << 3;
			return;
		}
	 ...
	 * 
	 * 7 pi 1 dst[8*k] = sum c[l] * src[8*l] * cos( -- * ( k + - ) * l ) l=0 8 2
	 * 
	 * where: c[0] = 1/1024 c[1..7] = (1/1024)*sqrt(2)
	 */
	private void idctcol(int[] pixel_data, int block_pointer) {
		/* shortcut */
		if (((x1 = (pixel_data[block_pointer + line_4_index] << 8))
				| (x2 = pixel_data[block_pointer + line_6_index])
				| (x3 = pixel_data[block_pointer + line_2_index])
				| (x4 = pixel_data[block_pointer + line_1_index])
				| (x5 = pixel_data[block_pointer + line_7_index])
				| (x6 = pixel_data[block_pointer + line_5_index]) | (x7 = pixel_data[block_pointer
				+ line_3_index])) == 0) {
			pixel_data[block_pointer] = pixel_data[block_pointer + line_1_index] = pixel_data[block_pointer
					+ line_2_index] = pixel_data[block_pointer + line_3_index] = pixel_data[block_pointer
					+ line_4_index] = pixel_data[block_pointer + line_5_index] = pixel_data[block_pointer
					+ line_6_index] = pixel_data[block_pointer + line_7_index] = ((pixel_data[block_pointer] + 32) >> 6);
			return;
		}
