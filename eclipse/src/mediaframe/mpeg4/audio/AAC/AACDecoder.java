...
public final class AACDecoder {
	...
	/*
	 * coupling data - skipped float[][] cc_coef = null; float[][][] cc_gain =
	 * null; float[][] cc_data = null; float[][] cc_state = null; byte[] cc_wnd
	 * = null; Wnd_Shape[] cc_wnd_shape = null;
	 */

	Dolby_Adapt dolby_Adapt = new Dolby_Adapt();

	/*
	 * int nbits; long cword; int inerror; int bufpool; Float
	 * iq_exp_tbl[MAX_IQ_TBL]; Float exptable[TEXP]; int maxfac; Hcb
	 * book[NSPECBOOKS+2]; Stab table[MAXFFT/2]; Stab table2[MAXFFT]; int
	 * adif_header_present; MC_Info mc_info; int debug[256]; Info
	 * *win_seq_info[NUM_WIN_SEQ]; Info only_long_info; Info eight_short_info;
	 * Info* winmap[NUM_WIN_SEQ]; short sfbwidth128[(1<<LEN_MAX_SFBS)];
	 */
	...
	public AACDecoder(BitStream audio_stream) throws IOException {
		...
		int j;
		/*
		 * for(i = 0; i < debug.length; i++) { debug[i] = false; }
		 */
		...
		/*
		 * for(i = 0; i < Constants.NUM_WIN_SEQ; i++){ win_seq_info[i] = new
		 * Info(); }
		 * 
		 * if(Constants.CChans > 0) { cc_coef = new
		 * float[Constants.CChans][Constants.LN2]; cc_gain = new
		 * float[Constants.CChans][Constants.Chans][Constants.MAXBANDS]; cc_wnd
		 * = new byte[Constants.CChans]; cc_wnd_shape = new
		 * Wnd_Shape[Constants.CChans]; if(Constants.ICChans > 0) { cc_data =
		 * new float[Constants.CChans][Constants.LN2]; cc_state = new
		 * float[Constants.CChans][Constants.LN4]; } 5 }
		 */
	...
	void infoinit(SR_Info sip) {
		...
		for (ws = 0; ws < Constants.NUM_WIN_SEQ; ws++) {
			 ...
			 * System.out.println("win_info\t" + ws + " has " + ip.nsbk +
			 * " windows"); System.out.println("\tbins_per_bk\t" +
			 * ip.bins_per_bk); System.out.println("\tsfb_per_bk\t" +
			 * ip.sfb_per_bk); for (i=0; i<ip.nsbk; i++) {
			 * System.out.println("window\t" + i);
			 * System.out.println("\tbins_per_sbk\t" + ip.bins_per_sbk[i]);
			 * System.out.println("\tsfb_per_sbk	" + ip.sfb_per_sbk[i]); } if
			 * (ip.sfb_width_128 != null) {
			 * System.out.println("sfb top and width"); for (i=0;
			 * i<ip.sfb_per_sbk[0]; i++) { System.out.println(i + " " +
			 * ip.sbk_sfb_top[0][i] + " " + ip.sfb_width_128[i]); } } }
			 */
		}
	}

	/**
	 * Read and decode the data for the next 1024 output samples return -1 if
	 * there was an error.
	 * 
	 * @throws IOException
	 *             raises if an I/O error occurs.
	 */
	int huffdecode(int id, MC_Info mip, byte[] win, Wnd_Shape[] wshape,
			byte[][] group, byte[] hasmask, byte[][] mask, byte[] max_sfb,
			int[][] lpflag, int[][] prstflag, TNS_frame_info[] tns,
			float[][] coef) throws IOException {
		...
		case Constants.ID_CPE:
			...
			if (common_window) {
				widx = mip.ch_info[ch].widx;
				get_ics_info(widx, win, wshape, group[widx], max_sfb,
						lpflag[widx], prstflag[widx]);
		...
		/*
		 * if(debug[Constants.DEBUG_V]) { System.out.println("tag " + tag +
		 * ", common window " + common_window); System.out.println("nch " +
		 * (last-first+1) + ", channels " + first + " " + last + ", widx " +
		...
		for (i = first; i <= last; i++) {
			...
			if (getics(widx, info, common_window, win, wshape, group[widx],
					max_sfb, lpflag[widx], prstflag[widx], sect, coef[i],
					factors[i - first], tns[i]) == 0)
				return -1;
		}
		...
		if ((id == Constants.ID_CPE) && common_window) {
			...
			for (i = 0, j = 0; i < nsect; i++) {
				...
				top = sect[j + 1];
				/*
				 * is IS used? (but no IS if factor[left] is zero!)
				 */
				is_cb = ((table == Constants.INTENSITY_HCB) || (table == Constants.INTENSITY_HCB2)) ? 1
						: 0;
				if (is_cb > 0) {
					/*
					 * if (debug[Constants.DEBUG_I]) {
					 * System.out.println("intensity cb " + table + " from " +
					 * bot + " to " + top); }
					 */
					iip.is_present = true;
					iip.bot[is_sect] = bot;
					iip.top[is_sect] = top;
					iip.sign[is_sect] = (table == Constants.INTENSITY_HCB) ? 1
							: -1;
					is_sect++;
				}
				for (k = bot; k < top; k++) {
					if (is_cb > 0) {
						...
						/*
						 * if (debug[Constants.DEBUG_I])
						 * System.out.println("IS factor " + iip.fac[is_sfb] +
						 * " at sfb " + is_sfb);
						 */
					}
					is_sfb++;
	...
	void get_ics_info(int widx, byte[] win, Wnd_Shape[] wshape, byte[] group,
		...
		wshape[widx].this_bk = (byte) audio_stream
				.next_bits(Constants.LEN_WIN_SH);
		...
		prstflag[0] = 0;
		if (info.islong) {
			max_sfb[widx] = (byte) audio_stream
					.next_bits(Constants.LEN_MAX_SFBL);
			group[0] = 1;
			if ((lpflag[0] = (int) audio_stream
					.next_bits(Constants.LEN_PRED_PRES)) > 0) {
				if ((prstflag[0] = (int) audio_stream
						.next_bits(Constants.LEN_PRED_RST)) > 0) {
					for (i = 1; i < Constants.LEN_PRED_RSTGRP + 1; i++) {
						prstflag[i] = (int) audio_stream
								.next_bits(Constants.LEN_PRED_RST);
					}
				}
				j = ((max_sfb[widx] < Constants.MAX_PRED_SFB) ? max_sfb[widx]
						: Constants.MAX_PRED_SFB) + 1;
				for (i = 1; i < j; i++)
					lpflag[i] = (int) audio_stream
							.next_bits(Constants.LEN_PRED_ENAB);
				for (; i < Constants.MAX_PRED_SFB + 1; i++)
					lpflag[i] = /* 0 */1 /* SRQ FIX!!!! */;
			}
		} else {
			max_sfb[widx] = (byte) audio_stream
					.next_bits(Constants.LEN_MAX_SFBS);
			getgroup(info, group);
			lpflag[0] = 0;
		}
		/*
		 * if(debug[Constants.DEBUG_V]) { System.out.println("window_sequence "
		 * + win[widx] + ", window_shape " + wshape[widx].this_bk);
		 * System.out.println("max_sf " + max_sfb[widx]); }
		 */

		/*
		 * if (debug[Constants.DEBUG_P]) { if (lpflag[0] > 0) {
		 * System.out.print("prediction enabled (" + max_sfb[widx] + "):  ");
		 * for (i = 1; i < Constants.MAX_PRED_SFB+1; i++) System.out.print(" " +
		 * lpflag[i]); System.out.println(); } }
		 */
	}
	...
	void getgroup(Info info, byte[] group) throws IOException {
		...
		/*
		 * if( debug[Constants.DEBUG_G] ) System.out.print("Grouping: 0");
		 */
		...
		for (i = 0; i < info.nsbk; i++) {
			if (info.bins_per_sbk[i] > Constants.SN2) {
				...
				if (first_short) {
					...
					/*
					 * if( debug[Constants.DEBUG_G] ) System.out.print(j);
					 */
				}
			}
		}
		group[group_index] = (byte) i;
		/*
		 * if( debug[Constants.DEBUG_G] ) System.out.println();
		 */
	}

	/*
	 * read a synthesis mask uses EXTENDED_MS_MASK and grouped mask
	 */
	int getmask(Info info, byte[] group, byte max_sfb, byte[] mask)
		...
		for (b = 0; b < info.nsbk; b = group[group_index++]) {
			/*
			 * if( debug[Constants.DEBUG_M] ) System.out.print(" gr" + b + ":");
			 */
			for (i = 0; i < max_sfb; i++) {
				mask[mask_index] = (byte) audio_stream
						.next_bits(Constants.LEN_MASK);
				/*
				 * if( debug[Constants.DEBUG_M] )
		...
		/*
		 * if( debug[Constants.DEBUG_M] ) System.out.println();
		 */
		return 1;
	}
	...
	int get_tns(Info info, TNS_frame_info tns_frame_info) throws IOException {
		...
		for (s = 0; s < tns_frame_info.n_subblocks; s++) {
			...
			if ((tns_info.n_filt = (int) audio_stream.next_bits(short_flag ? 1
					: 2)) == 0)
				continue;
			...
			for (f = tns_info.n_filt; f > 0; f--) {
				...
				top = tns_filt[tns_filt_index].start_band = top
						- (int) audio_stream.next_bits(short_flag ? 4 : 6);
				tns_filt[tns_filt_index].order = (int) audio_stream
						.next_bits(short_flag ? 3 : 5);

				if (tns_filt[tns_filt_index].order > 0) {
					tns_filt[tns_filt_index].direction = (int) audio_stream
							.next_bits(1);
					...
					for (t = tns_filt[tns_filt_index].order; t > 0; t--) {
						...
						tns_filt[tns_filt_index].coef[sp++] = (short) (((tmp & s_mask) > 0) ? (tmp | n_mask)
								: tmp);
					}
				}
				tns_filt_index++;
	...
	void get_nec_nc(Nec_Info nec_info) throws IOException {
		...
		nec_info.number_pulse = (int) audio_stream
				.next_bits(Constants.LEN_NEC_NPULSE);
		nec_info.pulse_start_sfb = (int) audio_stream
				.next_bits(Constants.LEN_NEC_ST_SFB);
		...
		for (i = 0; i < nec_info.number_pulse; i++) {
			nec_info.pulse_offset[i] = (int) audio_stream
					.next_bits(Constants.LEN_NEC_POFF);
			nec_info.pulse_amp[i] = (int) audio_stream
					.next_bits(Constants.LEN_NEC_PAMP);
			// System.out.println("nec_info.pulse_offset[" + i + "] = " +
			// nec_info.pulse_offset[i]);
			// System.out.println("nec_info.pulse_amp[" + i + "] = " +
			// nec_info.pulse_amp[i]);
		}
	}
	 ...
	 * group[widx], max_sfb[widx], lpflag[widx], prstflag[widx], nsect, sect,
	 * coef[i], factors[i-first], tns[i]))
	 */
	int getics(int widx, Info info, boolean common_window, byte[] win,
			Wnd_Shape[] wshape, byte[] group, byte[] max_sfb, int[] lpflag,
			int[] prstflag, byte[] sect, float[] coef, short[] factors,
			TNS_frame_info tns) throws IOException {
		int i, tot_sfb;
		...
		/*
		 * if (debug[Constants.DEBUG_F]) System.out.println("global gain: " +
		 * global_gain);
		 */

		if (!common_window) {
			/*
			 * void get_ics_info(int widx, byte[] win, Wnd_Shape[] wshape,
		...
		if (max_sfb[widx] == 0) {
			...
			/*
			 * if (debug[Constants.DEBUG_F]) System.out.println("tot sfb " + i +
			 * " " + tot_sfb);
			 */
			while (group[i++] < info.nsbk) {
				tot_sfb += info.sfb_per_sbk[0];
				/*
				 * if (debug[Constants.DEBUG_F]) System.out.println("tot sfb " +
				 * i + " " + tot_sfb);
				 */
			}
		}
		 ...
		 * section data
		 */
		nsect = huffcb(sect, info.sectbits, tot_sfb, info.sfb_per_sbk[0],
				max_sfb[widx]);
		if ((nsect == 0) && (max_sfb[widx] > 0))
			return 0;

		/*
		 * calculate band offsets (because of grouping and interleaving this
		 * cannot be a constant: store it in info.bk_sfb_top)
		 */
	 ...
	 */
	int huffcb(byte[] sect, int[] sectbits, int tot_sfb, int sfb_per_sbk,
			byte max_sfb) throws IOException {
		int nsect, n, base, bits, len;
		int sect_index = 0;
		/*
		 * if (debug[Constants.DEBUG_S]) { System.out.println("total sfb " +
		 * tot_sfb); System.out.println("sect, top, cb"); }
		 */
		bits = sectbits[0];
		len = (1 << bits) - 1;
		nsect = 0;
		for (base = 0; base < tot_sfb && nsect < tot_sfb;) {
			sect[sect_index++] = (byte) audio_stream
					.next_bits(Constants.LEN_CB);
			...
			nsect++;
			/*
			 * if (debug[Constants.DEBUG_S]) System.out.println(" " + nsect +
			 * " " + sect[sect_index - 1] + " " + sect[sect_index - 2]);
			 */
	 ...
	 * get scale factors
	 */
	int hufffac(Info info, byte[] group, byte[] sect, short[] factors)
			throws IOException {
		Hcb hcb;
		int[][] hcw;
		...
		/*
		 * scale factors are dpcm relative to global gain intensity positions
		 * are dpcm relative to zero
		 */
		fac = global_gain;
		is_pos = 0;
		...
		bb = 0;
		/*
		 * if (debug[Constants.DEBUG_F]) { System.out.println("scale factors");
		...
		for (b = 0; b < info.nsbk;) {
			...
			b = group[group_idx++];
			/*
			 * System.out.println("n : " + n); System.out.println("b : " + b);
			 */
			for (i = 0; i < n; i++) {
				// System.out.println("fac_trans[" + fac_trans_index + i +
				// "] = " + fac_trans[fac_trans_index + i]);
				if (fac_trans[fac_trans_index + i] > 0) {
					/* decode intensity positions */
					if ((info.nsbk == 1)
							&& /* No short blocks yet! */
							((fac_trans[fac_trans_index + i] == Constants.INTENSITY_HCB) || (fac_trans[fac_trans_index
									+ i] == Constants.INTENSITY_HCB2))) {
						...
						/*
						 * if (debug[Constants.DEBUG_F]) { System.out.println(i
						 * + " " + is_pos + " (is_pos)"); }
						 */
					...
					/*
					 * if (debug[Constants.DEBUG_F]) { System.out.print(i + ":"
					 * + fac); }
					 */
			...
			/*
			 * if (debug[Constants.DEBUG_F]) { System.out.println(); }
			 */
			/* expand short block grouping */
			if (!info.islong) {
				for (bb++; bb < b; bb++) {
					for (i = 0; i < n; i++) {
						factors[factors_index + i + n] = factors[factors_index
								+ i];
					}
					factors_index += n;
				}
	...
	/*
	 * rm2 inverse quantization escape books need ftn call other books done via
	 * macro
	 */
	float iquant(int q) {
		return (q >= 0) ? (float) huffman.iq_exp_tbl[q]
				: (float) (-huffman.iq_exp_tbl[-q]);
	}
	...
	int huffspec(Info info, byte[] sect, short[] factors, float[] coef)
			throws IOException {
		Hcb hcb;
		int[][] hcw;
		...
		for (i = nsect; i > 0; i--) {
			...
			sect_index += 2;
			if ((table == 0) || (table == Constants.INTENSITY_HCB)
			...
			for (j = bottom; j < top; j++) {
				...
				/*
				 * if(debug[Constants.DEBUG_Q]){ System.out.println("sect " +
				 * table + " " + kstart); for (idx=kstart ;idx<k;idx++) {
		...
		if (!info.islong) {
			deinterleave(quant, tmp_spec, info.num_groups, info.group_len,
					info.sfb_per_sbk, info.sfb_width_128);
	 ...
	 */
	void intensity(MC_Info mip, Info info, int widx, int[] lpflag, int ch,
			float[][] coef) {
	...
	void predinit() throws IOException {
		...
		for (ch = 0; ch < Constants.Chans; ch++) {
			for (i = 0; i < Constants.LN2; i++) {
				Monopred.init_pred_stat(sp_status[ch][i], Constants.PRED_ORDER,
						Constants.PRED_ALPHA, Constants.PRED_A,
						Constants.PRED_B);
				prev_quant[ch][i] = 0;
			}
		}
	...
	/*
	 * int getdata(int *tag, int *dt_cnt, byte *data_bytes) { int i, cnt;
	 * 
	 * tag = getbits(LEN_TAG); if ((cnt = getbits(LEN_D_CNT)) ==
	 * (1<<LEN_D_CNT)-1) cnt += getbits(LEN_D_ESC);dt_cnt = cnt; if (debug['x'])
	 * PRINT(SE, "data element %d has %d bytes\n", *tag, cnt);
	 * 
	 * for (i=0; i<cnt; i++) data_bytes[i] = getbits(LEN_BYTE);
	 * 
	 * return 0; }
	 */
	...
	void getfill() throws IOException {
		...
		/*
		 * if (debug[Constants.DEBUG_X]) { System.out.print("fill element has "
		 * + cnt + " bytes"); }
		 */
		for (i = 0; i < cnt; i++) {
	 ...
	 * 
	 * try { BitStream audio_stream = new BitStream(new
	 * FileInputStream(argv[0])); AACDecoder decoder = new
	 * AACDecoder(audio_stream); decoder.initio(argv); // parse command line
	 * decoder.init(); // initialize data structures } catch (Exception ex) {
	 * ex.printStackTrace(); } }
	 */
	...
	public int decodeFrame(byte[] buf) throws IOException {
		...
		/*
		 * AU_ptr = AUptr; AU_idx = 0; AU_len = AUlen;
		 */
		/* PRINT( SE, "\n==== AU_len = %d\n", AU_len ); */
		/*
		 * if(debug[Constants.DEBUG_N]) { System.out.print("\rblock " + bno); }
		 */
		...
		while ((ele_id = (int) audio_stream.next_bits(Constants.LEN_SE_ID)) != Constants.ID_END) {
			...
			/*
			 * if(debug[Constants.DEBUG_V]) System.out.println("\nele_id " +
			 * ele_id);
			 */
			switch (ele_id) {
			...
			case Constants.ID_LFE: /* low freq effects channel */
				int result_code = huffdecode(ele_id, mip, wnd, wnd_shape,
						group, hasmask, mask, max_sfb, lpflag, prstflag,
						tns_frame_info, coef);
				if (result_code < 0)
			...
			case Constants.ID_CCE: /* coupling channel */
				/*
				 * if(Constants.CChans > 0) { if (getcc(mip, cc_wnd,
				 * cc_wnd_shape, cc_coef, cc_gain) < 0) throw new
				 * IOException("getcc"); break; }
				 */
		...
		/*
		 * // call transport layer if(endblock() < 0) { throw new
		 * IOException("endblock"); }
		 */
		...
		for (ch = 0; ch < Constants.Chans; ch++) {
			...
			Monopred.predict(info, mip.profile, lpflag[wn], sp_status[ch],
					prev_quant[ch], coef[ch]);
		}
		...
		for (ch = 0; ch < Constants.Chans; ch++) {
			...
			// coupling(mip, coef, cc_coef, cc_gain, ch, Constants.CC_DOM,
			// Constants.CC_IND == 0);
			// }
			...
			for (i = j = 0; i < tns_frame_info[ch].n_subblocks; i++) {
				/*
				 * if (debug[Constants.DEBUG_T]) { System.out.println(bno + " "
				 * + ch + " " + i); tns.print_tns( tns_frame_info[ch].info[i]);
				 * }
				 */

				// @TODO check the following code
				tns.tns_decode_subblock(coef[ch], j, info.sfb_per_sbk[i],
			...
			if (Constants.DOLBY_MDCT) {
				/* inverse transform */
				dolby_Adapt.freq2time_adapt(wnd[wn], wnd_shape[wn], coef[ch],
						state[ch], data[ch]);
				// } else {
				// imdct(wnd[wn], wnd_shape[wn].this_bk, coef[ch], state[ch],
				// data[ch]);
				//
				/* scale imdct output */
			...
			/*
			 * System.out.print("coef[" + ch + "] = "); for(i = 0; i < 10; i++)
			 * { System.out.print(coef[ch][i] + " "); } System.out.println();
			 * 
			 * System.out.print("state[" + ch + "] = "); for(i = 0; i < 10; i++)
			 * { System.out.print(state[ch][i] + " "); } System.out.println();
			 * System.out.print("data[" + ch + "] = "); for(i = 0; i < 10; i++)
			 * { System.out.print(data[ch][i] + " "); } System.out.println();
			 */
			// if(Constants.CChans > 0) {
	...
	/*
	 * // source portio.c void initio(String[] argv) throws IOException { int i,
	 * j;
	 * 
	 * // set defaults adif_header_present = false; current_program = -1;
	 * default_config = true; mc_info.profile = Constants.Main_Profile;
	 * mc_info.sampling_rate_idx = Constants.Fs_48;
	 * 
	 * // save cmd // ARGBEGIN is so clever that it throws away argv[0] and //
	 * increments the argv pointer //
	 * 
	 * // // if(argv.length != 2) { // usage(cmd); // } // for(i = 0; i <
	 * argv.length; i++,i++) { if(argv[i].length() > 1) { usage(); throw new
	 * IOException("unknown option"); } switch(argv[i].charAt(0)) { case 'i':
	 * adif_header_present = true; current_program = (argv[i + 1] == null) ? -1
	 * : Integer.parseInt(argv[i + 1]); if (current_program >
	 * ((1<<Constants.LEN_NUM_PCE)-1)) { throw new
	 * IOException("Invalid program: " + current_program); } break; case 'p': if
	 * ("Main".equalsIgnoreCase(argv[i + 1])) { mc_info.profile =
	 * Constants.Main_Profile; break; } if ("LC".equalsIgnoreCase(argv[i + 1]))
	 * { mc_info.profile = Constants.LC_Profile; break; } throw new
	 * IOException("Unsupported profile " + argv[i + 1]); case 's': j =
	 * Integer.parseInt(argv[i + 1]); for (i=0; i<(1<<Constants.LEN_SAMP_IDX);
	 * i++) { if (j == Tables.samp_rate_info[i].samp_rate) break; } if (i ==
	 * (1<<Constants.LEN_SAMP_IDX)) { throw new
	 * IOException("Unsupported sampling frequency " + j); }
	 * mc_info.sampling_rate_idx = i; break; case 'D': String options = argv[i +
	 * 1]; for(j = 0; j < options.length(); j++) { char debug_option =
	 * options.charAt(j); for(int z = 0; z < Constants.debug_options.length;
	 * z++) { if(Constants.debug_options[z] == debug_option) { debug[z] = true;
	 * break; } } }
	 * 
	 * break;
	 * 
	 * default: usage(); throw new IOException("unknown option"); } } }
	 * 
	 * void usage() { System.out.println("usage: [options] chan_file pcm_file");
	 * System.out.println("	options are:");
	 * System.out.println("	-a create AIFF output files"); System.out.println(
	 ...
	 */
	/*
	 * long f2ir(float x) { if(x >= 0) { return (long) (x+.5); } return
	 * -(long)(-x + .5); }
	 * 
	 * void fltcpy(float[] dp1, float[] dp2, int cnt) { for(int i = 0;i < cnt;
	 * i++) { dp2[i] = dp1[i]; } }
	 * 
	 * void fltset(float[] dp1, float dval, int cnt) { for(int i = 0; i < cnt;
	 * i++) { dp1[i] = dval; } }
	 * 
	 * void fltclr(float[] dp1, int cnt) { for(int i = 0; i < cnt; i++) { dp1[i]
	 * = 0; } }
	 * 
	 * void intcpy(int[] ip1, int[] ip2, int cnt) { for(int i = 0;i < cnt; i++)
	 * { ip2[i] = ip1[i]; } }
	 * 
	 * void intclr(int[] ip1, int cnt) { for(int i = 0; i < cnt; i++) { ip1[i] =
	 * 0; } }
	 * 
	 * void byteclr(byte[] ip1, int cnt) { for(int i = 0; i < cnt; i++) { ip1[i]
	 * = 0; } }
	 */
