...
public class NTLMEngine {
	...
	static {
		...
		System.arraycopy(bytesWithoutNull, 0, SIGNATURE, 0,
				bytesWithoutNull.length);
	 ...
	 */
	final String getResponseFor(String message, String username,
		...
		if (message == null || message.trim().length() == 0) {
			...
			response = getType3Message(username, password, host, domain,
					t2m.getChallenge(), t2m.getFlags(), t2m.getTarget(),
					t2m.getTargetInfo());
		}
		return response;
	}
	 ...
	 */
	String getType3Message(String user, String password, String host,
			String domain, byte[] nonce, int type2Flags, String target,
			byte[] targetInformation) throws NTLMEngineException {
		try {
			return new Type3Message(domain, host, user, password, nonce,
					type2Flags, target, targetInformation).getResponse();
		} catch (UnsupportedEncodingException e) {
			throw new NTLMEngineException("Unsupported encoding", e);
		}
	}
	...
	private static int readULong(byte[] src, int index)
		...
		if (src.length < index + 4)
			throw new NTLMEngineException(
					"NTLM authentication - buffer too small for DWORD");
	...
	private static int readUShort(byte[] src, int index)
		...
		if (src.length < index + 2)
			throw new NTLMEngineException(
					"NTLM authentication - buffer too small for WORD");
		return (src[index] & 0xff) | ((src[index + 1] & 0xff) << 8);
	}
	 ...
	 */
	static byte[] getNTLMv2Response(String target, String user,
	...
	private static byte[] lmHash(String password) throws NTLMEngineException {
		try {
			byte[] oemPassword = password.toUpperCase(Locale.ENGLISH).getBytes(
					"US-ASCII");
	...
	private static byte[] ntlmv2Hash(String target, String user, String password)
		...
		try {
			...
			hmacMD5.update(user.toUpperCase(Locale.ENGLISH).getBytes(
	...
	private static byte[] lmv2Response(byte[] hash, byte[] challenge,
			byte[] clientData) throws NTLMEngineException {
	...
	private static byte[] createBlob(byte[] clientChallenge,
		...
		byte[] blob = new byte[blobSignature.length + reserved.length
				+ timestamp.length + 8 + unknown1.length
	...
	static class NTLMMessage {
		...
		NTLMMessage(String messageBody, int expectedType)
			...
			if (type != expectedType)
				throw new NTLMEngineException("NTLM type "
						+ Integer.toString(expectedType)
		...
		protected void readBytes(byte[] buffer, int position)
			...
			if (messageContents.length < position + buffer.length)
				throw new NTLMEngineException("NTLM: Message too short");
			System.arraycopy(messageContents, position, buffer, 0,
					buffer.length);
		}
	...
	static class Type1Message extends NTLMMessage {
		...
		Type1Message(String domain, String host) throws NTLMEngineException {
			...
			try {
				...
				domainBytes = domain.toUpperCase(Locale.ENGLISH).getBytes(
						"UnicodeLittleUnmarked");
			} catch (java.io.UnsupportedEncodingException e) {
		...
		String getResponse() throws UnsupportedEncodingException {
			...
			addULong(FLAG_NEGOTIATE_NTLM | FLAG_NEGOTIATE_NTLM2
					| FLAG_NEGOTIATE_SIGN | FLAG_NEGOTIATE_SEAL |
					/*
					 * FLAG_NEGOTIATE_ALWAYS_SIGN | FLAG_NEGOTIATE_KEY_EXCH |
					 */
					FLAG_UNICODE_ENCODING | FLAG_TARGET_DESIRED
					| FLAG_NEGOTIATE_128);
	...
	static class Type3Message extends NTLMMessage {
		 ...
		 */
		Type3Message(String domain, String host, String user, String password,
				byte[] nonce, int type2Flags, String target,
				byte[] targetInformation) throws NTLMEngineException {
			// Save the flags
			...
			try {
				if (targetInformation != null && target != null) {
					...
					ntResp = getNTLMv2Response(target, user, password, nonce,
							clientChallenge, targetInformation);
					lmResp = getLMv2Response(target, user, password, nonce,
							clientChallenge);
				} else {
					if ((type2Flags & FLAG_NEGOTIATE_NTLM2) != 0) {
						...
						ntResp = getNTLM2SessionResponse(password, nonce,
								clientChallenge);
		...
		String getResponse() throws UnsupportedEncodingException {
			...
			addULong(FLAG_NEGOTIATE_NTLM | FLAG_UNICODE_ENCODING
					| FLAG_TARGET_DESIRED | FLAG_NEGOTIATE_128
	...
	static class MD4 {
		...
		void update(byte[] input) {
			...
			while (input.length - inputIndex + curBufferPos >= dataBuffer.length) {
				...
				System.arraycopy(input, inputIndex, dataBuffer, curBufferPos,
						transferAmt);
				count += transferAmt;
				curBufferPos = 0;
		...
		byte[] getOutput() {
			...
			int padLen = (bufferIndex < 56) ? (56 - bufferIndex)
					: (120 - bufferIndex);
	...
	public String generateType3Msg(final String username,
		...
		return getType3Message(username, password, workstation, domain,
				t2m.getChallenge(), t2m.getFlags(), t2m.getTarget(),
				t2m.getTargetInfo());
	}
