...
/**
 * Because some implementation of the ThreadSchedulingService do not clean up
 * cancel task until they try to run them, we wrap the task with the future so
 * the when the NettyResponseFuture cancel the reaper future this wrapper will
 * release the references to the channel and the nettyResponseFuture
 * immediately. Otherwise, the memory referenced this way will only be released
 * after the request timeout period which can be arbitrary long.
 */
public final class FutureReaper implements Runnable {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(FutureReaper.class);
	...
	public FutureReaper(NettyResponseFuture<?> nettyResponseFuture,
			AsyncHttpClientConfig config, AtomicBoolean closed,
			Channels channels) {
	...
	public synchronized void run() {
		...
		if (nettyResponseFuture != null && !futureDone && !futureCanceled) {
			...
			if (nettyResponseFuture.hasRequestTimedOut(now)) {
				...
				expire("Request reached time out of "
						+ nettyResponseFuture.getRequestTimeoutInMs()
						+ " ms after " + age + " ms");
			} else if (nettyResponseFuture.hasConnectionIdleTimedOut(now)) {
				long age = now - nettyResponseFuture.getStart();
				expire("Request reached idle time out of "
						+ config.getIdleConnectionTimeoutInMs() + " ms after "
						+ age + " ms");
			}
