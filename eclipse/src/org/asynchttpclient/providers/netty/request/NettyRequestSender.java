...
public class NettyRequestSender {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(NettyRequestSender.class);
	...
	public NettyRequestSender(AtomicBoolean closed,
			AsyncHttpClientConfig config, Channels channels) {
		this.closed = closed;
		this.config = config;
		...
		if (!closed.get()) {
			...
			if (future != null && future.canBeReplayed()) {
				...
				if (future.getAsyncHandler() instanceof AsyncHandlerExtensions) {
					AsyncHandlerExtensions.class.cast(future.getAsyncHandler())
							.onRetry();
				}
	...
	public boolean applyIoExceptionFiltersAndReplayRequest(
			ChannelHandlerContext ctx, NettyResponseFuture<?> future,
	...
	public <T> void sendNextRequest(final Request request,
			final NettyResponseFuture<T> f) throws IOException {
		sendRequest(request, f.getAsyncHandler(), f, true);
	}

	// FIXME is this useful? Can't we do that when building the request?
	private final boolean validateWebSocketRequest(Request request,
			AsyncHandler<?> asyncHandler) {
		return request.getMethod().equals(HttpMethod.GET.name())
				&& asyncHandler instanceof WebSocketUpgradeHandler;
	}

	private Channel getCachedChannel(NettyResponseFuture<?> future, URI uri,
			ConnectionPoolKeyStrategy poolKeyGen, ProxyServer proxyServer) {

		if (future != null && future.reuseChannel() && future.channel() != null) {
			return future.channel();
		} else {
			URI connectionKeyUri = proxyServer != null ? proxyServer.getURI()
					: uri;
			return channels.lookupInCache(connectionKeyUri, poolKeyGen);
		}
	}

	private <T> ListenableFuture<T> sendRequestWithCachedChannel(
			Channel channel, Request request, URI uri, ProxyServer proxy,
			NettyResponseFuture<T> future, AsyncHandler<T> asyncHandler)
			throws IOException {
		HttpRequest nettyRequest = null;

		if (future == null) {
			nettyRequest = NettyRequests.newNettyRequest(config, request, uri,
					false, proxy);
			future = NettyResponseFutures.newNettyResponseFuture(uri, request,
					asyncHandler, nettyRequest, config, proxy);
		} else {
			nettyRequest = NettyRequests.newNettyRequest(config, request, uri,
					future.isConnectAllowed(), proxy);
	...
	private ChannelFuture connect(Request request, URI uri, ProxyServer proxy,
			Bootstrap bootstrap) {
		...
		if (request.getInetAddress() != null) {
			...
			remoteAddress = new InetSocketAddress(proxy.getHost(),
					proxy.getPort());
		}

		if (request.getLocalAddress() != null) {
			return bootstrap.connect(remoteAddress, new InetSocketAddress(
					request.getLocalAddress(), 0));
		} else {
	...
	private <T> ListenableFuture<T> sendRequestWithNewChannel(Request request,
			URI uri, ProxyServer proxy, NettyResponseFuture<T> future,
			AsyncHandler<T> asyncHandler, boolean reclaimCache)
			throws IOException {
		...
		boolean acquiredConnection = !reclaimCache
				&& channels.acquireConnection(asyncHandler);
		Bootstrap bootstrap = channels.getBootstrap(request.getUrl(), useSSl);

		NettyConnectListener<T> cl = new NettyConnectListener.Builder<T>(
				config, this, request, asyncHandler, future).build(uri);
		...
		LOGGER.debug("\nNon cached request \n{}\n\nusing Channel \n{}\n", cl
				.future().getNettyRequest(), channelFuture.channel());
	...
	public <T> ListenableFuture<T> sendRequest(final Request request,
			final AsyncHandler<T> asyncHandler, NettyResponseFuture<T> future,
		...
		Channel channel = getCachedChannel(future, uri,
				request.getConnectionPoolKeyStrategy(), proxy);

		if (channel != null && channel.isOpen() && channel.isActive()) {
			return sendRequestWithCachedChannel(channel, request, uri, proxy,
					future, asyncHandler);
		} else {
			return sendRequestWithNewChannel(request, uri, proxy, future,
					asyncHandler, reclaimCache);
		}
	}

	private void sendFileBody(Channel channel, File file,
			NettyResponseFuture<?> future) throws IOException {
		...
		try {
			...
			if (Channels.getSslHandler(channel) != null) {
				writeFuture = channel.write(new ChunkedFile(raf, 0, fileLength,
						Constants.MAX_BUFFERED_BYTES), channel
			...
			writeFuture.addListener(new ProgressListener(config, false, future
					.getAsyncHandler(), future) {
	...
	private boolean sendStreamAndExit(Channel channel, final InputStream is,
			NettyResponseFuture<?> future) throws IOException {
		...
		channel.write(new ChunkedStream(is), channel.newProgressivePromise())
				.addListener(
						new ProgressListener(config, false, future
								.getAsyncHandler(), future) {
							public void operationComplete(
									ChannelProgressiveFuture cf) {
								try {
									is.close();
								} catch (IOException e) {
									LOGGER.warn(
	...
	public void sendBody(final Channel channel, final Body body,
		...
		if (Channels.getSslHandler(channel) == null
			...
			if (bg instanceof FeedableBodyGenerator) {
				FeedableBodyGenerator.class.cast(bg).setListener(
						new FeedListener() {
							@Override
							public void onContentAdded() {
								channel.pipeline()
										.get(ChunkedWriteHandler.class)
										.resumeTransfer();
							}
						});
			}
	...
	private Body computeBody(HttpRequest nettyRequest,
			NettyResponseFuture<?> future) {
		...
		if (bg != null) {
			...
			if (contentLength != null) {
				...
				nettyRequest.headers().add(HttpHeaders.Names.TRANSFER_ENCODING,
						HttpHeaders.Values.CHUNKED);
			}
			body = new MultipartBody(future.getRequest().getParts(),
					contentType, length);
		}
	...
	private void configureTransferAdapter(AsyncHandler<?> handler,
	...
	private void scheduleReaper(NettyResponseFuture<?> future) {
		try {
			...
			int requestTimeout = AsyncHttpProviderUtils.requestTimeout(config,
					future.getRequest());
			int schedulePeriod = requestTimeout != -1 ? (config
					.getIdleConnectionTimeoutInMs() != -1 ? Math.min(
					requestTimeout, config.getIdleConnectionTimeoutInMs())
					: requestTimeout) : config.getIdleConnectionTimeoutInMs();

			if (schedulePeriod != -1 && !future.isDone()
					&& !future.isCancelled()) {
				FutureReaper reaperFuture = new FutureReaper(future, config,
						closed, channels);
				Future<?> scheduledFuture = config.reaper()
						.scheduleAtFixedRate(reaperFuture, 0, schedulePeriod,
	...
	public final <T> void writeRequest(final Channel channel,
		...
		try {
			...
			// Leave it to true.
			// FIXME That doesn't just leave to true, the set is always done?
			// and what's the point of not having a is/get?
			if (future.getAndSetWriteHeaders(true)) {
				try {
					if (future.getAsyncHandler() instanceof AsyncHandlerExtensions) {
						AsyncHandlerExtensions.class.cast(
								future.getAsyncHandler()).onRequestSent();
					}
					channel.writeAndFlush(nettyRequest,
							channel.newProgressivePromise()).addListener(
							new ProgressListener(config, true, future
									.getAsyncHandler(), future));
			...
			if (future.getAndSetWriteBody(true)) {
				if (!future.getNettyRequest().getMethod()
					...
					if (future.getRequest().getFile() != null) {
						sendFileBody(channel, future.getRequest().getFile(),
								future);

					} else if (future.getRequest().getStreamData() != null) {
						if (sendStreamAndExit(channel, future.getRequest()
								.getStreamData(), future))
							return;
					} else if (future.getRequest().getBodyGenerator() instanceof InputStreamBodyGenerator) {
						if (sendStreamAndExit(
								channel,
								InputStreamBodyGenerator.class.cast(
										future.getRequest().getBodyGenerator())
										.getInputStream(), future))
							return;
	...
	// WTF return???
	public void replayRequest(final NettyResponseFuture<?> future,
			FilterContext fc, ChannelHandlerContext ctx) throws IOException {
