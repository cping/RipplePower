...
public class NettyRequests {
	...
	public static final String GZIP_DEFLATE = HttpHeaders.Values.GZIP + ","
			+ HttpHeaders.Values.DEFLATE;

	public static HttpRequest newNettyRequest(AsyncHttpClientConfig config,
			Request request, URI uri, boolean allowConnect,
		...
		if (webSocket) {
			...
			headers.put(HttpHeaders.Names.CONNECTION,
					HttpHeaders.Values.UPGRADE);
			headers.put(HttpHeaders.Names.ORIGIN, "http://"
					+ uri.getHost()
					+ ":"
					+ (uri.getPort() == -1 ? isSecure(uri.getScheme()) ? 443
							: 80 : uri.getPort()));
			headers.put(HttpHeaders.Names.SEC_WEBSOCKET_KEY,
					WebSocketUtil.getKey());
		...
		if (method != HttpMethod.CONNECT) {
			...
			List<String> auth = request.getHeaders().get(
		...
		if (realm != null && realm.getUsePreemptiveAuth()) {
			...
			case BASIC:
				authorizationHeader = AuthenticatorUtils
			...
			case NTLM:
				try {
					String msg = NTLMEngine.INSTANCE.generateType1Msg("NTLM "
							+ domain, authHost);
			...
			case SPNEGO:
				...
				String server = proxyServer == null ? host : proxyServer
						.getHost();
				try {
					challengeHeader = SpnegoEngine.instance().generateToken(
							server);
				} catch (Throwable e) {
					throw new IOException(e);
				}
		...
		if (proxyServer != null) {
			// FIXME Wikipedia says that Proxy-Connection was a misunderstanding
			// of Connection
			// http://en.wikipedia.org/wiki/List_of_HTTP_header_fields
			if (!request.getHeaders().containsKey("Proxy-Connection")) {
				headers.put("Proxy-Connection",
						AsyncHttpProviderUtils.keepAliveHeaderValue(config));
			}
		...
		String userAgentHeader = request.getHeaders().getFirstValue(
		...
		if (userAgentHeader != null) {
			...
			headers.put(HttpHeaders.Names.USER_AGENT, AsyncHttpProviderUtils
					.constructUserAgent(NettyAsyncHttpProvider.class, config));
		}

		boolean hasDeferredContent = false;
		if (method != HttpMethod.CONNECT) {
			if (isNonEmpty(request.getCookies())) {
				headers.put(HttpHeaders.Names.COOKIE, CookieEncoder
						.encodeClientSide(request.getCookies(),
			...
			if (request.getByteData() != null) {
				headers.put(HttpHeaders.Names.CONTENT_LENGTH,
				...
				headers.put(HttpHeaders.Names.TRANSFER_ENCODING,
		...
		if (hasDeferredContent) {
			nettyRequest = new DefaultHttpRequest(httpVersion, method,
					requestUri);
		} else if (content != null) {
			nettyRequest = new DefaultFullHttpRequest(httpVersion, method,
					requestUri, content);
		} else {
			nettyRequest = new DefaultFullHttpRequest(httpVersion, method,
					requestUri);
		...
		if (authorizationHeader != null) {
			...
			nettyRequest.headers().add(HttpHeaders.Names.AUTHORIZATION,
					authorizationHeader);
		}
