...
final class HttpProtocol extends Protocol {
	...
	public HttpProtocol(Channels channels, AsyncHttpClientConfig config,
			NettyAsyncHttpProviderConfig nettyConfig,
			NettyRequestSender requestSender) {
		super(channels, config, nettyConfig, requestSender);
	}

	private Realm kerberosChallenge(List<String> proxyAuth, Request request,
			ProxyServer proxyServer, FluentCaseInsensitiveStringsMap headers,
			Realm realm, NettyResponseFuture<?> future)
			throws NTLMEngineException {

		URI uri = request.getURI();
		String host = request.getVirtualHost() == null ? AsyncHttpProviderUtils
				.getHost(uri) : request.getVirtualHost();
		String server = proxyServer == null ? host : proxyServer.getHost();
		try {
			String challengeHeader = SpnegoEngine.instance().generateToken(
					server);
	...
	private Realm ntlmChallenge(List<String> wwwAuth, Request request,
			ProxyServer proxyServer, FluentCaseInsensitiveStringsMap headers,
			Realm realm, NettyResponseFuture<?> future)
			throws NTLMEngineException {
		...
		String ntlmDomain = useRealm ? realm.getNtlmDomain() : proxyServer
				.getNtlmDomain();
		String ntlmHost = useRealm ? realm.getNtlmHost() : proxyServer
				.getHost();
		String principal = useRealm ? realm.getPrincipal() : proxyServer
				.getPrincipal();
		String password = useRealm ? realm.getPassword() : proxyServer
				.getPassword();
		...
		if (realm != null && !realm.isNtlmMessageType2Received()) {
			...
			newRealm = new Realm.RealmBuilder().clone(realm)
					.setScheme(realm.getAuthScheme()).setUri(uri.getRawPath())
					.setMethodName(request.getMethod())
					.setNtlmMessageType2Received(true).build();
			future.getAndSetAuth(false);
		} else {
			addType3NTLMAuthorizationHeader(wwwAuth, headers, principal,
					password, ntlmDomain, ntlmHost);
	...
	private Realm ntlmProxyChallenge(List<String> wwwAuth, Request request,
			ProxyServer proxyServer, FluentCaseInsensitiveStringsMap headers,
			Realm realm, NettyResponseFuture<?> future)
			throws NTLMEngineException {
		future.getAndSetAuth(false);
		...
		addType3NTLMAuthorizationHeader(wwwAuth, headers,
	...
	private void addType3NTLMAuthorizationHeader(List<String> auth,
			FluentCaseInsensitiveStringsMap headers, String username,
			String password, String domain, String workstation)
			throws NTLMEngineException {
		headers.remove(HttpHeaders.Names.AUTHORIZATION);

		if (isNTLM(auth)) {
			String serverChallenge = auth.get(0).trim()
	...
	private List<String> getAuthorizationToken(
			Iterable<Entry<String, String>> list, String headerAuth) {
	...
	private void finishUpdate(final NettyResponseFuture<?> future,
			ChannelHandlerContext ctx, boolean lastValidChunk)
			throws IOException {
		if (lastValidChunk && future.isKeepAlive()) {
			...
			if (future.isKeepAlive()
					...
					&& channels.offerToPool(channels.getPoolKey(future),
							ctx.channel())) {
	...
	private final boolean updateBodyAndInterrupt(NettyResponseFuture<?> future,
			AsyncHandler<?> handler, HttpResponseBodyPart bodyPart)
			throws Exception {
	...
	private void markAsDone(NettyResponseFuture<?> future,
	...
	private boolean applyResponseFiltersAndReplayRequest(
		...
		FilterContext fc = new FilterContext.FilterContextBuilder()
				.asyncHandler(handler).request(future.getRequest())
	...
	private boolean handleResponseAndExit(final ChannelHandlerContext ctx,
			final NettyResponseFuture<?> future, AsyncHandler<?> handler,
			HttpRequest nettyRequest, ProxyServer proxyServer,
			HttpResponse response) throws Exception {
		Request request = future.getRequest();
		int statusCode = response.getStatus().code();
		HttpResponseStatus status = new ResponseStatus(future.getURI(),
				response, config);
		HttpResponseHeaders responseHeaders = new ResponseHeaders(
				future.getURI(), response.headers());
		...
		future.setKeepAlive(!HttpHeaders.Values.CLOSE.equalsIgnoreCase(response
				.headers().get(HttpHeaders.Names.CONNECTION)));

		if (!config.getResponseFilters().isEmpty()
				&& applyResponseFiltersAndReplayRequest(ctx, future, status,
						responseHeaders)) {
			return true;
		}
		...
		if (statusCode == UNAUTHORIZED.code() && realm != null) {
			List<String> wwwAuth = getAuthorizationToken(response.headers(),
			...
			if (!wwwAuth.isEmpty() && !future.getAndSetAuth(true)) {
				...
				if (!wwwAuth.contains("Kerberos")
						&& (isNTLM(wwwAuth) || negociate)) {
					newRealm = ntlmChallenge(wwwAuth, request, proxyServer,
							headers, realm, future);
					// SPNEGO KERBEROS
				} else if (negociate) {
					newRealm = kerberosChallenge(wwwAuth, request, proxyServer,
							headers, realm, future);
					if (newRealm == null) {
						return true;
					}
				...
				NettyChannelHandler.LOGGER.debug(
						"Sending authentication to {}", request.getUrl());
				Callback callback = new Callback(future) {
					public void call() throws Exception {
						channels.drainChannel(ctx, future);
						requestSender.sendNextRequest(
								builder.setHeaders(headers).setRealm(nr)
										.build(), future);
					}
				};
			...
			List<String> proxyAuth = getAuthorizationToken(response.headers(),
					HttpHeaders.Names.PROXY_AUTHENTICATE);
			if (realm != null && !proxyAuth.isEmpty()
					&& !future.getAndSetAuth(true)) {
				NettyChannelHandler.LOGGER.debug(
						"Sending proxy authentication to {}", request.getUrl());
				...
				requestSender.sendNextRequest(builder.setHeaders(headers)
						.setRealm(newRealm).build(), future);
				return true;
			}

		} else if (statusCode == OK.code()
				&& nettyRequest.getMethod() == HttpMethod.CONNECT) {

			NettyChannelHandler.LOGGER.debug("Connected to {}:{}",
			...
			try {
				NettyChannelHandler.LOGGER.debug(
						"Connecting to proxy {} for scheme {}", proxyServer,
						request.getUrl());
				channels.upgradeProtocol(ctx.channel().pipeline(), request
						.getURI().getScheme());
			} catch (Throwable ex) {
		...
		if (!future.getAndSetStatusReceived(true)
				&& (handler.onStatusReceived(status) != STATE.CONTINUE || handler
						.onHeadersReceived(responseHeaders) != STATE.CONTINUE)) {
	...
	public void handle(final ChannelHandlerContext ctx,
			final NettyResponseFuture future, final Object e) throws Exception {
		future.touch();
		...
		try {
			...
			if (e instanceof HttpContent) {
				...
				if (handler != null) {
					if (response != null
							&& handleResponseAndExit(ctx, future, handler,
									nettyRequest, proxyServer, response)) {
						return;
					}
					...
					if (last) {
						...
						HttpHeaders trailingHeaders = lastChunk
								.trailingHeaders();
						if (!trailingHeaders.isEmpty()) {
							interrupt = handler
									.onHeadersReceived(new ResponseHeaders(
											future.getURI(), future
													.getHttpResponse()
													.headers(), trailingHeaders)) != STATE.CONTINUE;
						}
					}
					...
					try {
						if (!interrupt && buf.readableBytes() > 0) {
							interrupt = updateBodyAndInterrupt(future, handler,
									nettyConfig.getBodyPartFactory()
											.newResponseBodyPart(buf, last));
						}
					} finally {
						buf.release();
					}
			...
			if (t instanceof IOException
					...
					&& requestSender.applyIoExceptionFiltersAndReplayRequest(
							ctx, future, IOException.class.cast(t))) {
				return;
			}
