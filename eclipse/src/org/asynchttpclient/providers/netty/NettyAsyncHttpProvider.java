...
public class NettyAsyncHttpProvider implements AsyncHttpProvider {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(NettyAsyncHttpProvider.class);
	...
	public NettyAsyncHttpProvider(AsyncHttpClientConfig config) {
		...
		NettyAsyncHttpProviderConfig.class.cast(config
		...
		channelHandler = new NettyChannelHandler(config, nettyConfig,
				requestSender, channels, closed);
		channels.configure(channelHandler);
	}

	@Override
	public String toString() {
		int availablePermits = channels.freeConnections != null ? channels.freeConnections
				.availablePermits() : 0;
		return String
				.format("NettyAsyncHttpProvider4:\n\t- maxConnections: %d\n\t- openChannels: %s\n\t- connectionPools: %s",
						config.getMaxTotalConnections() - availablePermits,
						channels.openChannels.toString(),
						channels.connectionsPool.toString());
	}
