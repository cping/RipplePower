...
public abstract class HttpResponseBodyPart {
	...
	/**
	 * Return a {@link ByteBuffer} that wraps the actual bytes read from the
	 * response's chunk. The {@link ByteBuffer} capacity is equal to the number
	 * of bytes available.
	 * 
	 ...
	 * Invoking that method means the underlying TCP connection will be closed
	 * as soon as the processing of the response is completed. That means the
	 * underlying connection will never get pooled.
	 */
	public abstract void markUnderlyingConnectionAsToBeClosed();

	/**
	 * Return true of the underlying connection will be closed once the response
	 * has been fully processed.
	 * 
	 * @return true of the underlying connection will be closed once the
