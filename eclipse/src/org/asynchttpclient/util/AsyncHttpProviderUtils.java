...
public class AsyncHttpProviderUtils {
	...
	public static final IOException REMOTELY_CLOSED_EXCEPTION = new IOException(
			"Remotely Closed");
	...
	public final static String contentToString(
			List<HttpResponseBodyPart> bodyParts, String charset)
	...
	public final static byte[] contentToBytes(
			List<HttpResponseBodyPart> bodyParts, int maxLen)
			throws UnsupportedEncodingException {
		final int partCount = bodyParts.size();
		if (partCount == 0) {
			return NO_BYTES;
	...
	public final static InputStream contentAsStream(
			List<HttpResponseBodyPart> bodyParts) {
	...
	public final static URI getRedirectUri(URI uri, String location) {
		if (location == null)
			throw new IllegalArgumentException("URI " + uri
					+ " was redirected to null location");

		URI locationURI = null;
		try {
			locationURI = new URI(location);
		} catch (URISyntaxException e) {
			// rich, we have a badly encoded location, let's try to encode the
			// query params
			String[] parts = location.split("\\?");
			if (parts.length != 2) {
				throw new IllegalArgumentException(
						"Don't know how to turn this location into a proper URI:"
								+ location, e);
			} else {
				StringBuilder properUrl = new StringBuilder(location.length())
		...
		if (scheme == null || !scheme.equalsIgnoreCase("http")
				&& !scheme.equalsIgnoreCase("https") && !scheme.equals("ws")
				&& !scheme.equals("wss")) {
			throw new IllegalArgumentException(
					"The URI scheme, of the URI "
							+ redirectUri
	...
	public final static int getPort(URI uri) {
		int port = uri.getPort();
		if (port == -1)
			port = uri.getScheme().equals("http")
					|| uri.getScheme().equals("ws") ? 80 : 443;
		return port;
	}

	/**
	 * This is quite ugly as our internal names are duplicated, but we build on
	 * top of HTTP Client implementation.
	 * 
	 * @param params
	...
	public final static MultipartRequestEntity createMultipartRequestEntity(
			List<Part> params, FluentCaseInsensitiveStringsMap requestHeaders)
			throws FileNotFoundException {
		org.asynchttpclient.multipart.Part[] parts = new org.asynchttpclient.multipart.Part[params
				.size()];
		int i = 0;
		...
		for (Part part : params) {
			if (part instanceof org.asynchttpclient.multipart.Part) {
				...
				parts[i] = new org.asynchttpclient.multipart.StringPart(
						part.getName(), ((StringPart) part).getValue(),
						((StringPart) part).getCharset());
			} else if (part instanceof FilePart) {
				parts[i] = new org.asynchttpclient.multipart.FilePart(
						part.getName(), ((FilePart) part).getFile(),
						((FilePart) part).getMimeType(),
						((FilePart) part).getCharSet());

			} else if (part instanceof ByteArrayPart) {
				PartSource source = new ByteArrayPartSource(
						((ByteArrayPart) part).getFileName(),
				...
				throw new IllegalArgumentException(String.format(
						"Unsupported part type for multipart parameter %s",
						part.getName()));
			}
			++i;
		}
	...
	public static String constructUserAgent(
		...
		return new StringBuilder(config.getUserAgent()).append(' ').append('(')
				.append(httpProvider.getSimpleName()).append(" - ")
	...
	public static int convertExpireField(String timestring) {
		...
		for (SimpleDateFormat sdf : simpleDateFormat.get()) {
			...
			if (date != null) {
				...
				return (int) (maxAgeMillis / 1000)
						+ (maxAgeMillis % 1000 != 0 ? 1 : 0);
			}
		}

		throw new IllegalArgumentException("Not a valid expire field "
				+ trimmedTimeString);
	}
	...
	public static int requestTimeout(AsyncHttpClientConfig config,
			Request request) {
		return request.getRequestTimeoutInMs() != 0 ? request
