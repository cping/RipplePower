...
/**
 * Convenience class that encapsulates details of "percent encoding" (as per
 * RFC-3986, see [http://www.ietf.org/rfc/rfc3986.txt]).
 */
public class UTF8UrlEncoder {
	private static final boolean encodeSpaceUsingPlus = System
			.getProperty("com.UTF8UrlEncoder.encodeSpaceUsingPlus") != null;

	/**
	 * Encoding table used for figuring out ascii characters that must be
	 * escaped (all non-Ascii characters need to be encoded anyway)
	 */
	...
	public static StringBuilder appendEncoded(StringBuilder sb, String input) {
		...
		for (int c, i = 0, len = input.length(); i < len; i += Character
				.charCount(c)) {
