...
public class ProxyUtils {
	...
	/**
	 * A specification of non-proxy hosts. See
	 ...
	 * of it, or we want to go directly to target host. If <code>null</code>
	 * proxy is passed in, this method returns true -- since there is NO proxy,
	 * we should avoid to use it. Simple hostname pattern matching using "*" are
	 * supported, but only as prefixes. See
	 * http://download.oracle.com/javase/1.4.2/docs/guide/net/properties.html
	 * 
	 * @param proxyServer
	 * @param request
	 * @return true if we have to avoid proxy use (obeying non-proxy hosts
	 *         settings), false otherwise.
	 */
	public static boolean avoidProxy(final ProxyServer proxyServer,
			final Request request) {
		return avoidProxy(proxyServer,
				AsyncHttpProviderUtils.getHost(request.getOriginalURI()));
	}
			...
			if (isNonEmpty(nonProxyHosts)) {
				for (String nonProxyHost : nonProxyHosts) {
					if (nonProxyHost.startsWith("*")
							...
							&& targetHost.endsWith(nonProxyHost.substring(1)
									.toLowerCase(Locale.ENGLISH))) {
						return true;
					} else if (nonProxyHost.endsWith("*")
							&& nonProxyHost.length() > 1
							&& targetHost.startsWith(nonProxyHost.substring(0,
									nonProxyHost.length() - 1).toLowerCase(
									Locale.ENGLISH))) {
						return true;
	 ...
	 * <p/>
	 * Currently the default http.* proxy properties are supported as well as
	 * properties specific for AHC.
	 * 
	 * @param properties
	 ...
	 */
	public static ProxyServerSelector createProxyServerSelector(
			Properties properties) {
		String host = properties.getProperty(PROXY_HOST);

		if (host != null) {
			int port = Integer
					.valueOf(properties.getProperty(PROXY_PORT, "80"));

			Protocol protocol;
			try {
				protocol = Protocol.valueOf(properties.getProperty(
						PROXY_PROTOCOL, "HTTP"));
			} catch (IllegalArgumentException e) {
				protocol = Protocol.HTTP;
			}

			ProxyServer proxyServer = new ProxyServer(protocol, host, port,
					properties.getProperty(PROXY_USER),
	 ...
	 * 
	 * @param proxySelector
	 *            The proxy selector to use. Must not be null.
	 * @return The proxy server selector.
	 */
	public static ProxyServerSelector createProxyServerSelector(
		...
		return new ProxyServerSelector() {
			...
			public ProxyServer select(URI uri) {
				...
				if (proxies != null) {
					// Loop through them until we find one that we know how to
					// use
					for (Proxy proxy : proxies) {
						switch (proxy.type()) {
						case HTTP:
							if (!(proxy.address() instanceof InetSocketAddress)) {
								log.warn("Don't know how to connect to address "
										+ proxy.address());
							} else {
								InetSocketAddress address = (InetSocketAddress) proxy
										.address();
								return new ProxyServer(Protocol.HTTP,
										address.getHostName(),
										address.getPort());
							}
						case DIRECT:
							return null;
						default:
							log.warn("ProxySelector returned proxy type that we don't know how to use: "
									+ proxy.type());
							break;
						}
					}
