...
/**
 * An {@link AsyncHandler} which is able to execute WebSocket upgrade. Use the
 * Builder for configuring WebSocket options.
 */
public class WebSocketUpgradeHandler implements UpgradeHandler<WebSocket>,
	 ...
	 */
	@Override
	public final STATE onBodyPartReceived(HttpResponseBodyPart bodyPart)
			throws Exception {
	 ...
	 */
	@Override
	public final STATE onStatusReceived(HttpResponseStatus responseStatus)
			throws Exception {
	 ...
	 */
	@Override
	public final STATE onHeadersReceived(HttpResponseHeaders headers)
			throws Exception {
		return STATE.CONTINUE;
	}
	...
	public final void onClose(WebSocket webSocket, int status,
			String reasonPhrase) {
		// Connect failure
		...
		for (WebSocketListener w : l) {
			...
			if (w instanceof WebSocketCloseCodeReasonListener) {
				WebSocketCloseCodeReasonListener.class.cast(w).onClose(
						webSocket, status, reasonPhrase);
			}
		}
	}
	...
	public final static class Builder {
		...
		/**
		 * Add a {@link WebSocketListener} that will be added to the
		 * {@link WebSocket}
		 * 
		 * @param listener
		 *            a {@link WebSocketListener}
		 * @return this
		 */
		 ...
		 * 
		 * @param maxByteSize
		 *            max size of the WebSocket byte message
		 * @return this
		 */
