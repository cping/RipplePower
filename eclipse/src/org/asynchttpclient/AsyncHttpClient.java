...
public class AsyncHttpClient implements Closeable {
	...
	public AsyncHttpClient(String providerClass, AsyncHttpClientConfig config) {
		this(loadProvider(providerClass, config),
	...
	public class BoundRequestBuilder extends
			RequestBuilderBase<BoundRequestBuilder> {
		/**
		...
		public ListenableFuture<Response> execute() throws IOException {
			return AsyncHttpClient.this.executeRequest(build(),
		...
		public Request build() {
			...
			if (signatureCalculator != null) {
				...
				signatureCalculator
						.calculateAndAddSignature(url, request, this);
			}
			return super.build();
		}
		...
		public BoundRequestBuilder setSignatureCalculator(
	...
	public <T> ListenableFuture<T> executeRequest(Request request,
		...
		FilterContext<T> fc = new FilterContext.FilterContextBuilder<T>()
				.asyncHandler(handler).request(request).build();
	...
	public ListenableFuture<Response> executeRequest(Request request)
		...
		FilterContext<Response> fc = new FilterContext.FilterContextBuilder<Response>()
				.asyncHandler(new AsyncCompletionHandlerBase())
	 ...
	 */
	private <T> FilterContext<T> preProcessRequest(FilterContext<T> fc)
			throws IOException {
		if (config.hasRequestFilters()) {
			final List<RequestFilter> requestFilters = config
					.getRequestFilters();
		...
		if (fc.getAsyncHandler() instanceof ResumableAsyncHandler) {
			request = ResumableAsyncHandler.class.cast(fc.getAsyncHandler())
					.adjustRequestRange(request);
		}
		...
		fc = new FilterContext.FilterContextBuilder<T>(fc).request(request)
				.build();
		return fc;
	}
	...
	private static AsyncHttpProvider loadProvider(final String className,
		...
		try {
			Class<AsyncHttpProvider> providerClass = (Class<AsyncHttpProvider>) Thread
					.currentThread().getContextClassLoader()
					.loadClass(className);
			return providerClass.getDeclaredConstructor(
					new Class[] { AsyncHttpClientConfig.class }).newInstance(
					config);
		} catch (Throwable t) {
	...
	private static AsyncHttpProvider loadDefaultProvider(
			String[] providerClassNames, AsyncHttpClientConfig config) {
	...
	protected BoundRequestBuilder requestBuilder(String reqType, String url) {
		return new BoundRequestBuilder(reqType, config.isUseRawUrl()).setUrl(
				url).setSignatureCalculator(signatureCalculator);
	}
