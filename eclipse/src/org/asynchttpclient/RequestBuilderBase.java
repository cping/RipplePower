...
public abstract class RequestBuilderBase<T extends RequestBuilderBase<T>> {
	private final static Logger logger = LoggerFactory
			.getLogger(RequestBuilderBase.class);

	private static final URI DEFAULT_REQUEST_URL = URI
	...
	private static final class RequestImpl implements Request {
		...
		public RequestImpl(Request prototype) {
			if (prototype != null) {
				...
				this.connectionPoolKeyStrategy = prototype
		...
		private URI toURI(boolean encode) {
			...
			builder.append(originalUri.getScheme()).append("://")
					.append(originalUri.getAuthority());
			...
			if (isNonEmpty(queryParams)) {
				...
				for (Iterator<Entry<String, List<String>>> i = queryParams
						.iterator(); i.hasNext();) {
					Map.Entry<String, List<String>> param = i.next();
					String name = param.getKey();
					for (Iterator<String> j = param.getValue().iterator(); j
							.hasNext();) {
						String value = j.next();
						if (encode) {
		...
		public Collection<Cookie> getCookies() {
			if (cookies == null) {
				cookies = Collections.unmodifiableCollection(Collections
						.<Cookie> emptyList());
			}
			return cookies;
		}
	...
	private void checkIfBodyAllowed() {
		if ("HEAD".equals(request.method)) {
			throw new IllegalArgumentException(
					"Can NOT set Body on HTTP Request Method HEAD.");
		}
	}
	...
	public T setConnectionPoolKeyStrategy(
	...
	public Request build() {
		...
		if (request.cookies != null) {
			request.cookies = Collections
					.unmodifiableCollection(request.cookies);
		}
		return request;
	}
