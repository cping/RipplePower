...
/**
 * A {@link FilterContext} can be used to decorate {@link Request} and
 * {@link AsyncHandler} from a list of {@link RequestFilter}.
 * {@link RequestFilter} gets executed before the HTTP request is made to the
 * remote server. Once the response bytes are received, a {@link FilterContext}
 * is then passed to the list of {@link ResponseFilter}. {@link ResponseFilter}
 * gets invoked before the response gets processed, e.g. before authorization,
 * redirection and invocation of {@link AsyncHandler} gets processed.
 * <p/>
 * Invoking {@link FilterContext#getResponseStatus()} returns an instance of
 * {@link HttpResponseStatus} that can be used to decide if the response
 * processing should continue or not. You can stop the current response
 * processing and replay the request but creating a {@link FilterContext}. The
 * {@link org.asynchttpclient.AsyncHttpProvider} will interrupt the processing
 * and "replay" the associated {@link Request} instance.
 */
public class FilterContext<T> {
	...
	/**
	 * Return true if the current response's processing needs to be interrupted
	 * and a new {@link Request} be executed.
	 * 
	 * @return true if the current response's processing needs to be interrupted
	 *         and a new {@link Request} be executed.
	 */
	...
	public static class FilterContextBuilder<T> {
		...
		public FilterContextBuilder<T> responseStatus(
				HttpResponseStatus responseStatus) {
			this.responseStatus = responseStatus;
			return this;
		}

		public FilterContextBuilder<T> responseHeaders(
