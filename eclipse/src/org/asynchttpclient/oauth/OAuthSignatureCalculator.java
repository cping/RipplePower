...
/**
 * Simple OAuth signature calculator that can used for constructing client
 * signatures for accessing services that use OAuth for authorization.
 * <p/>
 * Supports most common signature inclusion and calculation methods: HMAC-SHA1
 * for calculation, and Header inclusion as inclusion method. Nonce generation
 * uses simple random numbers with base64 encoding.
 * 
...
public class OAuthSignatureCalculator implements SignatureCalculator {
	...
	/**
	 * To generate Nonce, need some (pseudo)randomness; no need for secure
	 * variant here.
	 */
	...
	/**
	 * @param consumerAuth
	 *            Consumer key to use for signature calculation
	 * @param userAuth
	...
	public void calculateAndAddSignature(String baseURL, Request request,
			RequestBuilderBase<?> requestBuilder) {
		String method = request.getMethod(); // POST etc
		String nonce = generateNonce();
		long timestamp = System.currentTimeMillis() / 1000L;
		String signature = calculateSignature(method, baseURL, timestamp,
				nonce, request.getParams(), request.getQueryParams());
	 ...
	 */
	public String calculateSignature(String method, String baseURL,
			long oauthTimestamp, String nonce, FluentStringsMap formParams,
		...
		/*
		 * 07-Oct-2010, tatu: URL may contain default port number; if so, need
		 * to extract from base URL.
		 */
	 ...
	 */
	public String constructAuthHeader(String signature, String nonce,
			long oauthTimestamp) {
		StringBuilder sb = new StringBuilder(200);
		sb.append("OAuth ");
		sb.append(KEY_OAUTH_CONSUMER_KEY).append("=\"")
				.append(consumerAuth.getKey()).append("\", ");
		sb.append(KEY_OAUTH_TOKEN).append("=\"").append(userAuth.getKey())
				.append("\", ");
		sb.append(KEY_OAUTH_SIGNATURE_METHOD).append("=\"")
		...
		sb.append(KEY_OAUTH_TIMESTAMP).append("=\"").append(oauthTimestamp)
				.append("\", ");

		// also: nonce may contain things that need URL encoding (esp. when
		// using base64):
		sb.append(KEY_OAUTH_NONCE).append("=\"");
		UTF8UrlEncoder.appendEncoded(sb, nonce);
		sb.append("\", ");

		sb.append(KEY_OAUTH_VERSION).append("=\"").append(OAUTH_VERSION_1_0)
				.append("\"");
		return sb.toString();
	}
	...
	private synchronized String generateNonce() {
		...
		// let's use base64 encoding over hex, slightly more compact than hex or
		// decimals
	...
	/**
	 * Container for parameters used for calculating OAuth signature. About the
	 * only confusing aspect is that of whether entries are to be sorted before
	 * encoded or vice versa: if my reading is correct, encoding is to occur
	 * first, then sorting; although this should rarely matter (since sorting is
	 * primary by key, which usually has nothing to encode)... of course, rarely
	 * means that when it would occur it'd be harder to track down.
	 */
	final static class OAuthParameterSet {
		...
		public OAuthParameterSet add(String key, String value) {
			Parameter p = new Parameter(UTF8UrlEncoder.encode(key),
					UTF8UrlEncoder.encode(value));
			allParameters.add(p);
			return this;
		}
		...
		public String sortAndConcat() {
			...
			Parameter[] params = allParameters
					.toArray(new Parameter[allParameters.size()]);
			Arrays.sort(params);
			...
			for (Parameter param : params) {
				...
				encodedParams.append(param.key()).append('=')
						.append(param.value());
			}
