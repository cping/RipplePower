...
public abstract class WebDavCompletionHandlerBase<T> implements AsyncHandler<T> {
	...
	private final List<HttpResponseBodyPart> bodies = Collections
			.synchronizedList(new ArrayList<HttpResponseBodyPart>());
	...
	public final T onCompleted() throws Exception {
		if (status != null) {
			...
			return onCompleted(new WebDavResponse(status.prepareResponse(
					headers, bodies), document));
		} else {
	 ...
	 * 
	 * @param response
	 *            The {@link org.asynchttpclient.Response}
	...
	private class HttpStatusWrapper extends HttpResponseStatus {
		...
		public HttpStatusWrapper(HttpResponseStatus wrapper, String statusText,
				int statusCode) {
		...
		public Response prepareResponse(HttpResponseHeaders headers,
			...
			final Response wrappedResponse = wrapped.prepareResponse(headers,
					bodyParts);
	...
	private void parse(Document document) {
		...
		for (int i = 0; i < statusNode.getLength(); i++) {
			...
			int statusCode = Integer.valueOf(value.substring(
