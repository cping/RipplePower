...
public class Codegen implements Evaluator {
	...
	private void generateFunctionInit(ClassFileWriter cfw, OptFunctionNode ofn) {
		...
		cfw.startMethod(getFunctionInitMethodName(ofn),
				FUNCTION_INIT_SIGNATURE, (short) (ACC_PRIVATE | ACC_FINAL));
		...
		case Token.ADD: {
			...
			default:
				if (child.getType() == Token.STRING) {
					addScriptRuntimeInvoke("add", "(Ljava/lang/CharSequence;"
							+ "Ljava/lang/Object;"
	...
	private void visitArrayLiteral(Node node, Node child, boolean topLevel) {
		...
		if (!topLevel && (count > 10 || cfw.getCurrentCodeOffset() > 30000)
			...
			cfw.addInvoke(ByteCode.INVOKEVIRTUAL, codegen.mainClassName,
					methodName, "(Lorg/mozilla/javascript/Context;"
							+ "Lorg/mozilla/javascript/Scriptable;"
							+ "Lorg/mozilla/javascript/Scriptable;"
							+ "[Ljava/lang/Object;"
							+ ")Lorg/mozilla/javascript/Scriptable;");
	...
	private void visitTryCatchFinally(Jump node, Node child) {
		...
		while (child != null) {
			if (child == catchTarget) {
				...
				exceptionManager
						.removeHandler(JAVASCRIPT_EXCEPTION, catchLabel);
