...
public class NativeRegExp extends IdScriptableObject implements Function {
	...
	static RECompiled compileRE(Context cx, String str, String global,
			boolean flat) {
	...
	private static boolean parseDisjunction(CompilerState state) {
		...
		if (index != source.length && source[index] == '|') {
			...
			if (result.kid.op == REOP_FLAT && result.kid2.op == REOP_FLAT) {
				result.op = (state.flags & JSREG_FOLD) == 0 ? REOP_ALTPREREQ
						: REOP_ALTPREREQi;
				...
				state.progLength += 13;
					...
					&& result.kid2.op == REOP_CLASS && result.kid2.index < 256
					&& (state.flags & JSREG_FOLD) == 0) {
				result.op = REOP_ALTPREREQ2;
				result.chr = result.kid.chr;
	...
	private static int emitREBytecode(CompilerState state, RECompiled re,
		...
		while (t != null) {
			...
			case REOP_ALTPREREQ2:
				...
				pc += INDEX_LEN;
				addIndex(program, pc, ignoreCase ? upcase((char) t.index)
						: t.index);
				pc += INDEX_LEN;
	...
	private static void pushProgState(REGlobalData gData, int min, int max,
			int cp, REBackTrackData backTrackLastToSave, int continuationOp,
			int continuationPc) {
		gData.stateStackTop = new REProgState(gData.stateStackTop, min, max,
				cp, backTrackLastToSave, continuationOp, continuationPc);
	}
			...
			if (reopIsSimple(op)) {
				...
				case REOP_ALTPREREQ2: {
					...
					if (op == REOP_ALTPREREQ2) {
						if (c != matchCh1
								&& !classMatcher(gData,
										gData.regexp.classList[matchCh2], c)) {
							result = false;
							break;
						}
					} else {
				...
				case REOP_ALT: {
					...
					int startcp = gData.cp;
					if (reopIsSimple(op)) {
						int match = simpleMatch(gData, input, op, program, pc,
								end, true);
						if (match < 0) {
							op = program[nextpc++];
							pc = nextpc;
				...
				case REOP_ASSERT: {
					...
					op = program[pc++];
					if (reopIsSimple(op)
							&& simpleMatch(gData, input, op, program, pc, end,
									false) < 0) {
						result = false;
						break;
					}
					pushProgState(gData, 0, 0, gData.cp,
							gData.backTrackStackTop, continuationOp,
							continuationPc);
				...
				case REOP_REPEAT: {
					...
					do {
						...
						if (reopIsSimple(nextop)) {
							nextpc++;
							int match = simpleMatch(gData, input, nextop,
									program, nextpc, end, true);
							if (match < 0) {
								result = (new_min == 0);
								continuationPc = state.continuationPc;
								continuationOp = state.continuationOp;
				...
				case REOP_MINIMALREPEAT: {
					...
					if (!result) {
						...
						if (state.max == -1 || state.max > 0) {
							pushProgState(gData, state.min, state.max,
									gData.cp, null, state.continuationOp,
...
class REProgState {
	REProgState(REProgState previous, int min, int max, int index,
			REBackTrackData backTrack, int continuationOp, int continuationPc) {
