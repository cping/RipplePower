...
import org.ripple.power.hft.bot.Extensions;
...
public class CrazyBuyerTrap extends TraderBase {
	...
	private final int ZOMBIE_CHECK = 10;
	...
	protected void check() {
		...
		this._volumeWall = Extensions.suggestWallVolume(coef, _minWallVolume,
				_maxWallVolume);
			...
			if (!sellOrder.Closed) {
				if (Extensions.eq(sellOrder.getAmountXrp(), _sellOrderAmount)) {
					...
					if (newAmount > _sellOrderAmount
						...
						this._sellOrderId = _rippleApi.updateSynXRPSellOrder(
					...
					log("SELL order ID=%s partially filled at price=%s %s. Remaining amount=%s XRP;",
							...
							sellOrder.getCurrency(), sellOrder.getAmountXrp());
					...
					if (sellOrder.getAmountXrp() < MIN_ORDER_AMOUNT) {
						...
						double price = suggestSellPrice(market);
						// The same price is totally unlikely, so we don't check
						// it here
				...
				if (Extensions.eq(balance, _xrpBalance, 0.1d)) {
					...
					log("SELL order ID=%s (amount=%s XRP) was closed at price=%s %s",
							_sellOrderId, _sellOrderAmount, _executedSellPrice,
							_currencyCode);
					_sellOrderId = -1;
					_sellOrderAmount = 0;
				}
		...
		if (_operativeAmount - _sellOrderAmount > NOT_SELL) {
			...
			if (-1 != _buyOrderId) {
				...
				if (!buyOrder.Closed) {
					...
					// Partially filled
					if (!Extensions
							.eq(buyOrder.getAmountXrp(), _buyOrderAmount)) {
						...
						_buyOrderId = _rippleApi.updateSynXRPBuyOrder(
								_buyOrderId, price, buyOrder.getAmountXrp(),
								_get);
