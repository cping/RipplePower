...
public class ForexData extends MarketMatrix {
	...
	public synchronized boolean update(long lastValue, long timestamp) {
		...
		if (get(indexof_lastRecord, HIGH) < lastValue
				|| get(indexof_lastRecord, 1) == UNSET) {
			set(indexof_lastRecord, 1, lastValue);
		}
		if (get(indexof_lastRecord, LOW) > lastValue
				|| get(indexof_lastRecord, 2) == UNSET) {
	...
	public synchronized void initRecord(int recordIndex, long open, long high,
			long low, long close, long volume, long timestamp) {
