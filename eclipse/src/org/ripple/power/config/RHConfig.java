...
public class RHConfig {
	...
	private final static HashMap<String, RHConfig> pConfigReaders = new HashMap<String, RHConfig>();

	public static RHConfig getInstance(String resName) {
		synchronized (pConfigReaders) {
			RHConfig reader = pConfigReaders.get(resName);
			if (reader == null || reader.isClose) {
				try {
					reader = new RHConfig(resName);
	...
	public static RHConfig getInstance(final InputStream in) {
		try {
			return new RHConfig(in);
		} catch (IOException ex) {
			throw new RuntimeException(ex.getMessage());
		}
	}
	...
	public RHConfig(final String resName) throws IOException {
