...
public class GraphicTool
{
    ...
    public void drawSelect(Graphics g, Container con, int x, int y, int width, int height, String strs[], 
            boolean oks[], int now)
    {
    ...
    public void drawSelect(Graphics g, Container con, int x, int y, int width, int height, String strs[], 
            boolean oks[], int now, Color cols[])
    {
    ...
    public void drawFrame(Graphics g, Container con, int x, int y, int width, int height)
    {
        ...
        for(int i = 0; i < corners.length; i++){
            corners[i] = Paramaters.Image_BOX.getBufferdImage((new StringBuilder("win")).append(i + 4).toString());
        }
            ...
            Color colors[])
    {
        ...
        {
            g.setFont(new Font("\u660E\u671D\u4F53", 1, getFontSize(bun - size, (int)Math.round((1.0D * (double)(width - 2 * size)) / (double)strs[i].length()))));
            g.setColor(colors[i]);
            g.drawString(strs[i], x + size, (int)Math.round((double)(y + bun * (i + 1)) - 0.10000000000000001D * (double)bun));
            if(!oks[i])
