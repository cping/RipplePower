...
public class AccountFind {
	...
	public final static DateTime RIPPLE_EPOC = new DateTime(2000, 1, 1, 8, 0, DateTimeZone.UTC);
	...
	public AccountInfo processTx(final String address, final long txPreLgrSeq, final int max, final AccountInfo accountinfo, final Updateable update) {
		...
		Updateable updateable = new Updateable() {
			...
			public void action() {
				...
				tx(address, txPreLgrSeq == -1 ? accountinfo.txPreLgrSeq : 0, max, new Rollback() {
						...
						if (result != null) {
							if (result.has("marker")) {
								try {
									...
									accountinfo.marker = accountinfo.marker == getLong(marker, "ledger") ? accountinfo.marker - 1 : getLong(marker, "ledger");
									AccountInfo newInfo = new AccountInfo();
									processTx(address, accountinfo.marker, max, newInfo, null);
									...
									try {
										...
										case "Payment":
											...
											if (meta.has("DeliveredAmount")) {
												currency = getAmount(getObject(meta, "DeliveredAmount"));
											} else {
												currency = getAmount(getObject(tx, "Amount"));
											}
											transactionTx.currency = currency;
											String flagType;
											if (address.equals(getStringObject(tx, "Account"))) {
												if (address.equals(getStringObject(tx, "Destination"))) {
													flagType = "Exchange";
												} else {
													flagType = "Send";
													counterparty = getStringObject(tx, "Destination");
											...
											} else if (address.equals(getStringObject(tx, "Destination"))) {
												flagType = "Receive";
												counterparty = getStringObject(tx, "Account");
											} else {
												flagType = "Convert";
											}
										...
										case "TrustSet":
											...
											break;
										case "OfferCreate":
											transactionTx.get = getAmount(getObject(tx, "TakerGets"));
											transactionTx.pay = getAmount(getObject(tx, "TakerPays"));
											break;
										case "OfferCancel":
												...
												if (obj.has("DeletedNode")) {
													...
													if ("Offer".equals(ledgerEntryType)) {
														...
														if (ffactount.equals(transactionTx.account)) {
															transactionTx.get = getAmount(getObject(ff, "TakerGets"));
															transactionTx.pay = getAmount(getObject(ff, "TakerPays"));
														}
													}
												}
					...
					if (offers.length() > 0) {
						for (int i = 0; i < offers.length(); i++) {
							try {
								...
								BookOffer offer = new BookOffer(getAmount(taker_gets), getAmount(taker_pays));
