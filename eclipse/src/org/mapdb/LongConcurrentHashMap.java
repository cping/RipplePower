 ...
 * @author Doug Lea
 */
public class LongConcurrentHashMap<V> extends LongMap<V> implements
		Serializable {
	...
	/**
	 * LongConcurrentHashMap list entry. Note that this is never exported out as
	 ...
	 * value {@code keys} such that {@code key.equals(k)}, then this method
	 * returns {@code keys}; otherwise it returns {@code null}. (There can be at
	...
	public boolean remove(long key, Object value) {
		final int hash = DataIO.longHash(key ^ hashSalt);
		return value != null
				&& segmentFor(hash).remove(key, hash, value) != null;
	}
