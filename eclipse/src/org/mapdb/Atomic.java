 ...
 * single records. In essence, the classes here provide provide an atomic
 ...
 * atomically sets a record to the {@code updateValue} if it currently holds the
 * {@code expectedValue}, reporting {@code true} on success. Classes jere also
 * contain methods to get and unconditionally set values.
 * 
 * <p>
 * The specifications of these methods enable to employ more efficient internal
 * DB locking. CompareAndSwap operation is typically faster than using
 * transactions, global lock or other concurrent protection.
 * 
 * <p>
 * Instances of classes {@link Atomic.Boolean}, {@link Atomic.Integer},
 * {@link Atomic.Long}, {@link Atomic.String} and {@link Atomic.Var} each
 * provide access and updates to a single record of the corresponding type. Each
 * class also provides appropriate utility methods for that type. For example,
 * classes {@code Atomic.Long} and {@code Atomic.Integer} provide atomic
 * increment methods. One application is to generate unique keys for Maps:
 * 
 * <pre>
 ...
 * methods such as {@code hashCode} and {@code compareTo}. (Because atomic
 * records are expected to be mutated, they are poor choices for hash table
 * keys.) Additionally, classes are provided only for those types that are
 * commonly useful in intended applications. Other types has to be wrapped into
 * general {@link Atomic.Var}
 * <p>
 * You can also hold floats using {@link java.lang.Float#floatToIntBits} and
...
final public class Atomic {
	 ...
	 * {@link java.lang.Integer}. However, this class does extend {@code Number}
	...
	public final static class Integer extends Number {
		...
		public final boolean compareAndSet(int expect, int update) {
			return engine.compareAndSwap(recid, expect, update,
					Serializer.INTEGER);
		}
	 ...
	 * incremented sequence numbers, and cannot be used as a replacement for a
	 * {@link java.lang.Long}. However, this class does extend {@code Number} to
	 * allow uniform access by tools and utilities that deal with
	 * numerically-based classes.
	 */
	public final static class Long extends Number {
		...
		public final boolean compareAndSet(long expect, long update) {
			return engine
					.compareAndSwap(recid, expect, update, Serializer.LONG);
		}
	...
	public static final class Var<E> {
		...
		protected Var(Engine engine, SerializerBase serializerBase,
