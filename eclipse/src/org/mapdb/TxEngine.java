...
/**
 * Naive implementation of Snapshots on top of StorageEngine. On update it takes
 * old value and stores it aside.
 * <p>
...
public class TxEngine extends EngineWrapper {
	...
	protected TxEngine(Engine engine, boolean fullTx) {
		super(engine);
		this.fullTx = fullTx;
		this.preallocRecids = fullTx ? new ArrayBlockingQueue<Long>(
				PREALLOC_RECID_SIZE) : null;
	}
		...
		try {
			...
			if (uncommitedData && canRollback())
				throw new IllegalAccessError(
						"Can not create snapshot with uncommited data");
			return new Tx();
		} finally {
	...
	protected void cleanTxQueue() {
		if (CC.PARANOID && !(commitLock.writeLock().isHeldByCurrentThread()))
			throw new AssertionError();
		for (Reference<? extends Tx> ref = txQueue.poll(); ref != null; ref = txQueue
				.poll()) {
			txs.remove(ref);
		}
	}
	...
	public class Tx implements Engine {
		...
		protected LongConcurrentHashMap<Fun.Pair> mod = fullTx ? new LongConcurrentHashMap<Fun.Pair>()
				: null;
		...
		public <A> boolean compareAndSwap(long recid, A expectedOldValue,
			...
			try {
				...
				try {
					...
					boolean ret = oldVal != null
							&& oldVal.equals(expectedOldValue);
					if (ret) {
		...
		SerializerPojo pojo = new SerializerPojo(
				(CopyOnWriteArrayList<SerializerPojo.ClassInfo>) TxEngine.this
						.getSerializerPojo().registered.clone());
