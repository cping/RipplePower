 ...
 * <p>
 * Implementations needs to be thread safe (especially 'ensureAvailable')
 * operation. However updates do not have to be atomic, it is clients
 * responsibility to ensure two threads are not writing/reading into the same
 * location.
 * 
 * @author Jan Kotek
 */
public abstract class Volume implements Closeable {
	...
	/**
	 * Check space allocated by Volume is bigger or equal to given offset. So it
	 * is safe to write into smaller offsets.
	 * 
	 * @param offset
	 */
	...
	abstract public void putData(final long offset, final byte[] src,
			int srcPos, int srcSize);
	...
	public int getUnsignedShort(long offset) {
		return (((getByte(offset) & 0xff) << 8) | ((getByte(offset + 1) & 0xff)));
	}
	...
	public long getSixLong(long pos) {
		return ((long) (getByte(pos + 0) & 0xff) << 40)
	...
	/**
	 * Transfers data from this Volume into target volume. If its possible, the
	 * implementation should override this method to enable direct memory
	 * transfer.
	 * 
	 * Caller must respect slice boundaries. ie it is not possible to transfer
	 * data which cross slice boundaries.
	 * 
	 * @param inputOffset
	 *            offset inside this Volume, ie data will be read from this
	 *            offset
	 * @param target
	 *            Volume to copy data into
	 * @param targetOffset
	 *            position in target volume where data will be copied into
	 * @param size
	 *            size of data to copy
	 */
	public void transferInto(long inputOffset, Volume target,
			long targetOffset, int size) {
	...
	public static Volume volumeForFile(File f, boolean useRandomAccessFile,
			boolean readOnly, int sliceShift, int sizeIncrement) {
		return useRandomAccessFile ? new FileChannelVol(f, readOnly,
				sliceShift, sizeIncrement) : new MappedFileVol(f, readOnly,
				sliceShift, sizeIncrement);
	}
	...
	public static Fun.Function1<Volume, String> memoryFactory(
		...
		return new Fun.Function1<Volume, String>() {
			...
			public Volume run(String s) {
				return useDirectBuffer ? new MemoryVol(true, sliceShift)
	...
	/**
	 * Abstract Volume over bunch of ByteBuffers It leaves ByteBufferVol details
	 * (allocation, disposal) on subclasses. Most methods are final for better
	 * performance (JIT compiler can inline those).
	 */
	abstract static public class ByteBufferVol extends Volume {
		...
		public final void ensureAvailable(long offset) {
			...
			try {
				...
				slices2 = Arrays.copyOf(
						slices2,
						Math.max(slicePos + 1, slices2.length + slices2.length
								/ 1000));
		...
		public final void putLong(final long offset, final long value) {
			slices[(int) (offset >>> sliceShift)].putLong(
					(int) (offset & sliceSizeModMask), value);
		}

		@Override
		public final void putInt(final long offset, final int value) {
			slices[(int) (offset >>> sliceShift)].putInt(
					(int) (offset & sliceSizeModMask), value);
		}

		@Override
		public final void putByte(final long offset, final byte value) {
			slices[(int) (offset >>> sliceShift)].put(
					(int) (offset & sliceSizeModMask), value);
		}

		@Override
		public void putData(final long offset, final byte[] src, int srcPos,
				int srcSize) {
			final ByteBuffer b1 = slices[(int) (offset >>> sliceShift)]
					.duplicate();
		...
		public void transferInto(long inputOffset, Volume target,
				long targetOffset, int size) {
			final ByteBuffer b1 = slices[(int) (inputOffset >>> sliceShift)]
					.duplicate();
		...
		final public long getLong(long offset) {
			return slices[(int) (offset >>> sliceShift)]
					.getLong((int) (offset & sliceSizeModMask));
		}

		@Override
		final public int getInt(long offset) {
			return slices[(int) (offset >>> sliceShift)]
					.getInt((int) (offset & sliceSizeModMask));
		}

		@Override
		public final byte getByte(long offset) {
			return slices[(int) (offset >>> sliceShift)]
					.get((int) (offset & sliceSizeModMask));
		}

		@Override
		public final DataIO.DataInputByteBuffer getDataInput(long offset,
				int size) {
			return new DataIO.DataInputByteBuffer(
					slices[(int) (offset >>> sliceShift)],
					(int) (offset & sliceSizeModMask));
		}
		...
		/**
		 * Hack to unmap MappedByteBuffer. Unmap is necessary on Windows,
		 * otherwise file is locked until JVM exits or BB is GCed. There is no
		 * public JVM API to unmap buffer, so this tries to use SUN proprietary
		 * API for unmap. Any error is silently ignored (for example SUN API
		...
		protected void unmap(MappedByteBuffer b) {
			try {
				if (unmapHackSupported) {
					...
					Method cleanerMethod = b.getClass().getMethod("cleaner",
							new Class[0]);
					if (cleanerMethod != null) {
						cleanerMethod.setAccessible(true);
						Object cleaner = cleanerMethod.invoke(b);
						if (cleaner != null) {
							Method clearMethod = cleaner.getClass().getMethod(
									"clean", new Class[0]);
							if (clearMethod != null)
				...
				// Utils.LOG.log(Level.WARNING, "ByteBufferVol Unmap failed",
				// e);
			}
		}
		...
		static {
			try {
				unmapHackSupported = SerializerPojo
		...
		private static boolean windowsWorkaround = System
				.getProperty("os.name").toLowerCase().startsWith("win");
	...
	public static final class MappedFileVol extends ByteBufferVol {
		...
		public MappedFileVol(File file, boolean readOnly, int sliceShift,
			...
			this.file = file;
			this.mapMode = readOnly ? FileChannel.MapMode.READ_ONLY
					: FileChannel.MapMode.READ_WRITE;
			try {
				FileChannelVol.checkFolder(file, readOnly);
				this.raf = new java.io.RandomAccessFile(file, readOnly ? "r"
						: "rw");
		...
		public void close() {
			...
			try {
				fileChannel.close();
				raf.close();
				// TODO not sure if no sync causes problems while unlocking
				// files
				// however if it is here, it causes slow commits, sync is called
				// on write-ahead-log just before it is deleted and closed
				// if(!readOnly)
				// sync();
	...
	public static final class MemoryVol extends ByteBufferVol {
		...
		@Override
		protected ByteBuffer makeNewBuffer(long offset) {
			return useDirectBuffer ? ByteBuffer.allocateDirect(sliceSize)
	...
	/**
	 * Volume which uses FileChannel. Uses global lock and does not use mapped
	 * memory.
	 */
	public static final class FileChannelVol extends Volume {
		...
		public void ensureAvailable(long offset) {
			if (offset % sliceSize != 0)
				offset += sliceSize - offset % sliceSize; // round up to
															// multiply of slice
															// size
	...
	protected static void handleIOException(IOException e) {
		if (e instanceof ClosedByInterruptException) {
			throw new DBException(DBException.Code.VOLUME_CLOSED_BY_INTERRUPT,
					e);
		}
	...
	/**
	 * transfer data from one volume to second. Second volume will be expanded
	 * if needed
	 */
