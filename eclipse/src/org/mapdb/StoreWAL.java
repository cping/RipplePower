...
public class StoreWAL extends StoreDirect {
	...
	public StoreWAL(String fileName, Fun.Function1<Volume, String> volFac,
			Fun.Function1<Volume, String> indexVolFac, boolean readOnly,
		...
		super(fileName, volFac, indexVolFac, readOnly, deleteFilesAfterClose,
				spaceReclaimMode, syncOnCommitDisabled, checksum, compress,
				password, sizeIncrement);
	...
	protected void reloadIndexFile() {
		...
		while (indexVals[((int) (maxUsedIoList / 8))] != 0
	...
	public <A> long put(A value, Serializer<A> serializer) {
		...
		try {
			...
			try {
				// write data into log
				walIndexVal((logPos[0] & LOG_MASK_OFFSET) - 1 - 8 - 8 - 1 - 8,
						ioRecid, physPos[0] | MASK_ARCHIVE);
	...
	protected void walPhysArray(DataIO.DataOutputByteArray out, long[] physPos,
			long[] logPos) {
		// write byte[] data
		int outPos = 0;
		int logC = 0;
		...
		for (int i = 0; i < logPos.length; i++) {
			...
			logC |= DataIO.longHash(pos | header | physPos[i]
					| (c > 0 ? physPos[i + 1] : 0) | crc32.getValue());
	...
	protected void walIndexVal(long logPos, long ioRecid, long indexVal) {
		...
		logChecksumAdd(DataIO.longHash(logPos | WAL_INDEX_LONG | ioRecid
				| indexVal));
	}
	...
	protected <A> A get2(long ioRecid, Serializer<A> serializer)
		...
		if (r.length == 1) {
			...
			for (int i = 0; i < r.length; i++) {
				...
				log.getDataInput((r[i] & LOG_MASK_OFFSET) + c, size).readFully(
						b, pos, size);
				pos += size;
			}
			if (pos != totalSize)
			...
			return deserialize(serializer, totalSize,
					new DataIO.DataInputByteArray(b));
		}
	}
	...
	protected void update2(DataIO.DataOutputByteArray out, long ioRecid) {
		...
		// write data into log
		walIndexVal((logPos[0] & LOG_MASK_OFFSET) - 1 - 8 - 8 - 1 - 8, ioRecid,
				physPos[0] | MASK_ARCHIVE);
		walPhysArray(out, physPos, logPos);
	...
	public void commit() {
		...
		try {
			...
			int crc = 0;
			LongMap.LongMapIterator<byte[]> iter = longStackPages
					.longMapIterator();
			while (iter.moveToNext()) {
				if (CC.PARANOID && !(iter.key() >>> 48 == 0))
					throw new AssertionError();
				final byte[] array = iter.value();
				final long pageSize = ((array[0] & 0xFF) << 8)
						| (array[1] & 0xFF);
			...
			crc |= DataIO.longHash(logSize | WAL_SEAL | indexSize | physSize
					| freeSize | indexChecksum);
	...
	protected boolean verifyLogFile() {
		...
		if (log.getUnsignedShort(6) != expectedMasks())
			throw new IllegalArgumentException(
					"Log file created with different features. Please check compression, checksum or encryption");
		...
		try {
			...
			while (ins != WAL_SEAL) {
				if (ins == WAL_INDEX_LONG) {
					...
					logSize += 8;
					crc |= DataIO.longHash((logSize - 1 - 8 - 8)
							| WAL_INDEX_LONG | ioRecid | indexVal);
					...
					crc |= DataIO.longHash(logSize | WAL_PHYS_ARRAY | offset2
							| crc32.getValue());
					...
					crc |= DataIO.longHash((logSize) | WAL_PHYS_ARRAY_ONE_LONG
							| offset2 | nextPageLink | crc32.getValue());
					...
					crc |= DataIO.longHash(origLogSize | WAL_LONGSTACK_PAGE
							| offset);
			...
			logSize += 8;
			crc |= DataIO.longHash((logSize - 1 - 3 * 6 - 8) | indexSize
					| physSize | freeSize | indexSum);
	...
	protected void replayLogFile() {
		...
		if (log.isEmpty() || log.getInt(0) != HEADER
				|| log.getUnsignedShort(4) > STORE_VERSION
				|| log.getLong(8) != LOG_SEAL
		...
		while (ins != WAL_SEAL) {
			if (ins == WAL_INDEX_LONG) {
			...
			} else if (ins == WAL_PHYS_ARRAY || ins == WAL_LONGSTACK_PAGE
					|| ins == WAL_PHYS_ARRAY_ONE_LONG) {
				long offset = log.getLong(logSize);
				logSize += 8;
				final int size = (int) (offset >>> 48);
				offset = offset & MASK_OFFSET;

				// transfer buffer directly from log file without copying into
				// memory
				...
				throw new AssertionError("unknown trans log instruction '"
						+ ins + "' at log offset: " + (logSize - 1));
			}
		...
		if (CC.LOG_STORE && LOG.isLoggable(Level.FINE))
			LOG.log(Level.FINE, "Replay WAL done at size {0,number,integer}",
					logSize);
	...
	protected long longStackTake(long ioList, boolean recursive) {
		...
		if (pos == 8) {
			...
			if (next != 0) {
				...
				byte[] nextPage = longStackGetPage(next); // TODO this page is
															// not modifed, but
															// is added to LOG
				long nextSize = ((nextPage[0] & 0xFF) << 8)
						| (nextPage[1] & 0xFF);
