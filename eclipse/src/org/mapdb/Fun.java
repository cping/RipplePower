...
public final class Fun {
	...
	/**
	 * empty iterator (note: Collections.EMPTY_ITERATOR is Java 7 specific and
	 * should not be used)
	 */
	...
	static public final class Pair<A, B> implements Comparable<Pair<A, B>>,
			Serializable {
		...
		protected Pair(SerializerBase serializer, DataInput in,
	...
	/**
	 * Used to run background threads. Unlike
	 * {@link java.util.concurrent.ThreadFactory} it does not give access to
	 * threads, so tasks can run inside {@link java.util.concurrent.Executor}.
	 * 
	 * There are some expectations from submitted tasks:
	 * 
	 * * Background tasks is started within reasonable delay. You can not block
	 * if thread pool is full. That could cause memory leak since queues are not
	 * flushed etc..
	 * 
	 * * Runnable code might pause and call {@link Thread#sleep(long)}.
	 * 
	 * * Threads must not be interrupted or terminated. Using daemon thread is
	 * forbidden. Runnable will exit itself, once db is closed.
	 * 
	 */
	public interface ThreadFactory {
		...
		/**
		 * execute new runnable. Optionally you can name thread using
		 * `threadName` argument
		 */
	...
	/**
	 * returns function which always returns the value itself without
	 * transformation
	 */
	...
	public static final class ArrayComparator implements Comparator<Object[]> {
		...
		/** constructor used for deserialization */
		protected ArrayComparator(SerializerBase serializer, DataInput in,
				SerializerBase.FastArrayList<Object> objectStack)
				throws IOException {
			objectStack.add(this);
			this.comparators = (Comparator[]) serializer.deserialize(in,
					objectStack);
		}
	 ...
	 * 
	 * Find all Primary Keys associated with Secondary Key. This is useful
	 * companion to
	 * {@link Bind#mapInverse(org.mapdb.Bind.MapWithModificationListener, java.util.Set)}
	 * and
	 * {@link Bind#secondaryKey(org.mapdb.Bind.MapWithModificationListener, java.util.Set, org.mapdb.Fun.Function2)}
	 * It can by also used to find values from 'MultiMap'.
	 * 
	 * @param set
	 *            Set or 'MultiMap' to find values in
	 * @param keys
	 *            key to look from
	 * @return all keys where primary value equals to `secondaryKey`
	 */
	public static Iterable<Object[]> filter(final NavigableSet<Object[]> set,
			final Object... keys) {
			...
			public Iterator<Object[]> iterator() {
				...
				return new Iterator<Object[]>() {
					...
					Object[] moveToNext() {
						...
						// TODO this does not work if byte[] etc is used in
						// array. Document or fail!
						// TODO add special check for Fun.ARRAY comparator and
						// use its sub-comparators
	...
	public interface RecordCondition<A> {
		boolean run(final long recid, final A value,
