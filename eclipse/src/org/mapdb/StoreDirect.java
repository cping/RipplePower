...
/**
 * Storage Engine which saves record directly into file. It has zero protection
 * from data corruption and must be closed properly after modifications. It is
 * used when Write-Ahead-Log transactions are disabled.
 * 
 * 
 * Storage format ---------------- `StoreDirect` is composed of two files: Index
 * file is sequence of 8-byte longs, it translates `recid` (offset in index
 * file) to record size and offset in physical file. Records position may
 * change, but it requires stable ID, so the index file is used for translation.
 * This store uses data structure called `Long Stack` to manage (and reuse) free
 * space, it is is linked LIFO queue of 8-byte longs.
 * 
 * Index file -------------- Index file is translation table between permanent
 * record ID (recid) and mutable location in physical file. Index file is
 * sequence of 8-byte longs, one for each record. It also has some extra longs
 * to manage free space and other metainfo. Index table and physical data could
 * be stored in single file, but keeping index table separate simplifies
 * compaction.
 * 
 * Basic **structure of index file** is bellow. Each slot is 8-bytes long so
 * `offset=slot*8`
 * 
 * <table summary="index">
 * <tr>
 ...
 * <tr>
 * <td>0</td>
 * <td>  {@link StoreDirect#HEADER}</td>
 * <td>File header, format version and flags</td>
 * </tr>
 * <tr>
 * <td>1</td>
 * <td>  {@link StoreDirect#IO_INDEX_SIZE}</td>
 * <td>Allocated file size of index file in bytes.</td>
 * </tr>
 * <tr>
 * <td>2</td>
 * <td>  {@link StoreDirect#IO_PHYS_SIZE}</td>
 * <td>Allocated file size of physical file in bytes.</td>
 * </tr>
 * <tr>
 * <td>3</td>
 * <td>  {@link StoreDirect#IO_FREE_SIZE}</td>
 * <td>Space occupied by free records in physical file in bytes.</td>
 * </tr>
 * <tr>
 * <td>4</td>
 * <td>  {@link StoreDirect#IO_INDEX_SUM}</td>
 * <td>Checksum of all Index file headers. Checks if store was closed correctly</td>
 * </tr>
 * <tr>
 * <td>5..9</td>
 * <td></td>
 ...
 * <td>10..14</td>
 * <td></td>
 * <td>For usage by user</td>
 * </tr>
 * <tr>
 * <td>15</td>
 * <td>  {@link StoreDirect#IO_FREE_RECID}</td>
 * <td>Long Stack of deleted recids, those will be reused and returned by
 * {@link Engine#put(Object, Serializer)}</td>
 * </tr>
 * <tr>
 * <td>16..4111</td>
 * <td></td>
 * <td>Long Stack of free physical records. This contains free space released by
 * record update or delete. Each slots corresponds to free record size. TODO
 * check 4111 is right</td>
 * </tr>
 * <tr>
 * <td>4112</td>
 * <td>  {@link StoreDirect#IO_USER_START}</td>
 * <td>Record size and offset in physical file for recid=1</td>
 * </tr>
 * <tr>
 * <td>4113</td>
 * <td></td>
 * <td>Record size and offset in physical file for recid=2</td>
 * </tr>
 * <tr>
 * <td>...</td>
 * <td>...</td>
 ...
 * <td>N+4111</td>
 * <td></td>
 * <td>Record size and offset in physical file for recid=N</td>
 * </tr>
 * </table>
 * 
 * Long Stack ------------ Long Stack is data structure used to store free
 * records. It is LIFO queue which uses linked records to store 8-byte longs.
 * Long Stack is identified by slot in Index File, which stores pointer to Long
 * Stack head. The structure of of index pointer is following:
 * 
 * <pre>
 * {@code
 *  byte    | description
 ...
 * </pre>
 * 
 * Each Long Stack Record is sequence of 8-byte longs, first slot is header.
 * Long Stack Record structure is following:
 * 
 * <pre>
 * {@code
 *  byte    | description
 ...
 * </pre>
 * 
 * Physical pointer ---------------- Index slot value typically contains
 * physical pointer (information about record location and size in physical
 * file). First 2 bytes are record size (max 65536). Then there is 6 byte offset
 * in physical file (max store size is 281 TB). Physical file offset must always
 * be multiple of 16, so last 4 bites are used to flag extra record information.
 ...
 * </pre>
 * 
 * Records in Physical File --------------------------- Records are stored in
 * physical file. Maximal record size size is 64KB, so larger records must be
 * stored in form of the linked list. Each record starts by Physical Pointer
 * from Index File. There is flag in Physical Pointer indicating if record is
 * linked. If record is not linked you may just read ByteBuffer from given size
 * and offset.
 * 
 * If record is linked, each record starts with Physical Pointer to next record.
 * So actual data payload is record size-8. The last linked record does not have
 * the Physical Pointer header to next record, there is MASK_LINKED flag which
...
public class StoreDirect extends Store {
	...
	/**
	 * checksum of all index file headers. Used to verify store was closed
	 * correctly
	 */
	...
	protected static final int IO_USER_START = IO_FREE_RECID
			+ PHYS_FREE_SLOTS_COUNT * 8 + 8;
	...
	/**
	 * maximal non zero slot in free phys record, access requires
	 * `structuralLock`
	 */
	...
	public StoreDirect(String fileName,
			...
			Fun.Function1<Volume, String> indexVolumeFactory, boolean readOnly,
			boolean deleteFilesAfterClose, int spaceReclaimMode,
			boolean syncOnCommitDisabled, boolean checksum, boolean compress,
			byte[] password, int sizeIncrement) {
		...
		try {
			...
			if (index.isEmpty()) {
				...
				while (index.getLong(maxUsedIoList) != 0
						&& maxUsedIoList > IO_FREE_RECID)
					maxUsedIoList -= 8;
			}
			allGood = true;
		} finally {
	...
	public StoreDirect(String fileName) {
		...
		this(fileName, fileName == null || fileName.isEmpty() ? Volume
				.memoryFactory() : Volume.fileFactory(), fileName == null
				|| fileName.isEmpty() ? Volume.memoryFactory() : Volume
				.fileFactory(), false, false, CC.DEFAULT_FREE_SPACE_RECLAIM_Q,
	...
	protected void checkHeaders() {
		...
		if (index.getUnsignedShort(4) > StoreDirect.STORE_VERSION
				|| phys.getUnsignedShort(4) > StoreDirect.STORE_VERSION)
			throw new IOError(new IOException(
					"New store format version, please use newer MapDB version"));

		final int masks = index.getUnsignedShort(6);
		if (masks != phys.getUnsignedShort(6))
			throw new IllegalArgumentException(
					"Index and Phys file have different feature masks");

		if (masks != expectedMasks())
			throw new IllegalArgumentException(
					"File created with different features. Please check compression, checksum or encryption");

		long checksum = index.getLong(IO_INDEX_SUM);
		if (checksum != indexHeaderChecksum())
			throw new IOError(
					new IOException(
							"Wrong index checksum, store was not closed properly and could be corrupted."));
	}
	...
	protected void createStructure() {
		...
		for (long recid = 1; recid < RECID_FIRST; recid++) {
			index.putLong(recid * 8 + IO_USER_START, MASK_DISCARD
					| MASK_ARCHIVE);
		}
	}
	...
	public <A> long put(A value, Serializer<A> serializer) {
		...
		if (CC.PARANOID && !(recid > 0))
			throw new AssertionError();
		if (CC.LOG_STORE && LOG.isLoggable(Level.FINEST))
			LOG.finest("Put recid=" + recid + ", " + " size=" + out.pos + ", "
					+ " val=" + value + " ser=" + serializer);
		recycledDataOuts.offer(out);
		return recid;
	}

	protected void put2(DataIO.DataOutputByteArray out, long ioRecid,
			long[] indexVals) {
		if (CC.PARANOID
				&& !(locks[Store.lockPos(ioRecid)].writeLock()
						.isHeldByCurrentThread()))
			throw new AssertionError();
		index.putLong(ioRecid, indexVals[0] | MASK_ARCHIVE);
		// write stuff
		if (indexVals.length == 1 || indexVals[1] == 0) { // is more then one?
															// ie linked
			// write single
	...
	public <A> A get(long recid, Serializer<A> serializer) {
		...
		try {
			...
			if (CC.LOG_STORE && LOG.isLoggable(Level.FINEST))
				LOG.finest("GET recid=" + recid + ", " + " ret=" + ret + ", "
						+ " ser=" + serializer);
			return ret;
		} catch (IOException e) {
			throw new IOError(e);
	...
	protected <A> A get2(long ioRecid, Serializer<A> serializer)
			throws IOException {
		if (CC.PARANOID
				&& !(locks[Store.lockPos(ioRecid)].getWriteHoldCount() == 0 || locks[Store
						.lockPos(ioRecid)].writeLock().isHeldByCurrentThread()))
			throw new AssertionError();
			...
			for (;;) {
				...
				if (buf.length < pos + size - c)
					buf = Arrays.copyOf(buf,
							Math.max(pos + size - c, buf.length * 2)); // buf to
																		// small,
																		// grow
	...
	public <A> void update(long recid, A value, Serializer<A> serializer) {
		...
		if (CC.LOG_STORE && LOG.isLoggable(Level.FINEST))
			LOG.finest("Update recid=" + recid + ", " + " size=" + out.pos
					+ ", " + " val=" + value + " ser=" + serializer);
	...
	protected void update2(DataIO.DataOutputByteArray out, long ioRecid) {
		...
		if (!linked && out.pos > 0 && size > 0
				&& size2ListIoRecid(size) == size2ListIoRecid(out.pos)) {
			// size did change, but still fits into this location
			final long offset = indexVal & MASK_OFFSET;

			// note: if size would not change, we still have to write
			// MASK_ARCHIVE bit
			index.putLong(ioRecid, (((long) out.pos) << 48) | offset
					| MASK_ARCHIVE);

			phys.putData(offset, out.buf, 0, out.pos);
		} else {
			long[] indexVals = spaceReclaimTrack ? getLinkedRecordsIndexVals(indexVal)
					: null;
			structuralLock.lock();
			try {
	...
	@Override
	public <A> boolean compareAndSwap(long recid, A expectedOldValue,
			A newValue, Serializer<A> serializer) {
		if (serializer == null)
			throw new NullPointerException();
		if (CC.PARANOID && !(recid > 0))
			throw new AssertionError();
	...
	protected void delete2(long ioRecid) {
		...
		try {
			...
			if (linkedRecords != null) {
				for (int i = 0; i < linkedRecords.length
						&& linkedRecords[i] != 0; i++) {
	...
	protected long[] getLinkedRecordsIndexVals(long indexVal) {
		...
		int linkedPos = 0;
		if ((indexVal & MASK_LINKED) != 0) {
			// record is composed of multiple linked records, so collect all of
			// them
			...
			for (;;) {
				if (linkedPos == linkedRecords.length) // grow if necessary
					linkedRecords = Arrays.copyOf(linkedRecords,
			...
			try {
				if (!readOnly) {
					if (serializerPojo != null
							&& serializerPojo.hasUnsavedChanges()) {
	...
	public void rollback() throws UnsupportedOperationException {
		throw new UnsupportedOperationException(
				"rollback not supported with journal disabled");
	}
	...
	public void compact() {
		...
		lockAllWrite();
		try {
			final File compactedFile = new File((indexFile != null ? indexFile
					: File.createTempFile("mapdb", "compact")) + ".compact");
			StoreDirect store2 = new StoreDirect(compactedFile.getPath(),
					volumeFactory, indexVolumeFactory, false, false, 5, false,
					checksum, compress, password, 0);
			...
			for (long ioRecid = longStackTake(IO_FREE_RECID, false); ioRecid != 0; ioRecid = longStackTake(
					IO_FREE_RECID, false)) {
			...
			for (long ioRecid = IO_USER_START; ioRecid < indexSize; ioRecid += 8) {
				...
				if (bb == null || bb.length == 0) {
					...
					DataIO.DataOutputByteArray out = serialize(bb,
							Serializer.BYTE_ARRAY_NOSIZE);
					long[] indexVals = store2
							.physAllocate(out.pos, true, false);
					store2.put2(out, ioRecid, indexVals); // TODO preserve
															// archiveFlag here
				}
			}
			...
			if (indexFile != null) {
				...
				final File indexFile_ = indexFile != null ? new File(
						indexFile.getPath() + "_" + time + "_orig") : null;
				final File physFile_ = physFile != null ? new File(
						physFile.getPath() + "_" + time + "_orig") : null;
				...
				if (!indexFile2.renameTo(indexFile))
					throw new AssertionError("could not rename file");
				// TODO process may fail in middle of rename, analyze sequence
				// and add recovery
				...
				Volume indexVol2 = new Volume.MemoryVol(useDirectBuffer,
						CC.VOLUME_SLICE_SHIFT);
				Volume.volumeTransfer(indexSize, store2.index, indexVol2);
				Volume physVol2 = new Volume.MemoryVol(useDirectBuffer,
						CC.VOLUME_SLICE_SHIFT);
	...
	protected long longStackTake(final long ioList, boolean recursive) {
		...
		if (pos == 8) {
			...
			// no, it was not last record at this page, so just decrement the
			// counter
			pos -= 6;
			index.putLong(ioList, (pos << 48) | dataOffset); // TODO update just
																// 2 bytes
		}
	...
	protected void longStackPut(final long ioList, long offset,
		...
		if (CC.PARANOID && !(offset >>> 48 == 0))
			throw new AssertionError();
		if (CC.PARANOID
				&& !(ioList >= IO_FREE_RECID && ioList <= IO_USER_START))
			throw new AssertionError("wrong ioList: " + ioList);
		...
		if (dataOffset == 0) { // empty list?
			...
			final long listPhysid = freePhysTake((int) LONG_STACK_PREF_SIZE,
					true, true) & MASK_OFFSET;
			if (listPhysid == 0)
				throw new AssertionError();
			...
			if (pos + 6 == size) { // is current page full?
				...
				// yes it is full, so we need to allocate new page and write our
				// number there
				final long listPhysid = freePhysTake((int) newPageSize, true,
						true) & MASK_OFFSET;
				if (listPhysid == 0)
					throw new AssertionError();

				// set location to previous page and set current page size
				phys.putLong(listPhysid, (newPageSize << 48)
						| (dataOffset & MASK_OFFSET));

				// set the value itself
				phys.putSixLong(listPhysid + 8, offset);

				// and update index file with new page location and number of
				// records
				index.putLong(ioList, (8L << 48) | listPhysid);
			} else {
				// there is space on page, so just write offset and increase the
				// counter
				pos += 6;
				phys.putSixLong(dataOffset + pos, offset);
				index.putLong(ioList, (pos << 48) | dataOffset); // TODO update
																	// just 2
																	// bytes
			}
		}
	...
	protected long freePhysTake(int size, boolean ensureAvail, boolean recursive) {
		...
		if (!recursive && spaceReclaimSplit) {
			for (long s = roundTo16(size) + 16; s < MAX_REC_SIZE; s += 16) {
				...
				if (ret != 0) {
					// found larger record, split in two slices, take first,
					// mark second free
					final long offset = ret & MASK_OFFSET;

					long remaining = s - roundTo16(size);
					long markFree = (remaining << 48)
							| (offset + s - remaining);
	...
	public Iterator<Long> getFreeRecids() {
		return Fun.EMPTY_ITERATOR; // TODO iterate over stack of free recids,
