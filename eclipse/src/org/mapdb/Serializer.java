...
public abstract class Serializer<A> {
	...
	public static final Serializer<Character> CHAR = new Serializer<Character>() {
		...
		@Override
		public Character deserialize(DataInput in, int available)
				throws IOException {
			return in.readChar();
		}
	...
	/**
	 * Serializes strings using UTF8 encoding. Stores string size so can be used
	 * as collection serializer. Does not handle null values
	 */
		...
		public String deserialize(DataInput in, int available)
			...
			if (available == -1)
				throw new IllegalArgumentException(
						"STRING_NOSIZE does not work with collections.");
	...
	public static final Serializer<Integer> INTEGER = new Serializer<Integer>() {
		...
		@Override
		public Integer deserialize(DataInput in, int available)
				throws IOException {
			return in.readInt();
		}
	...
	public static final Serializer<Boolean> BOOLEAN = new Serializer<Boolean>() {
		...
		@Override
		public Boolean deserialize(DataInput in, int available)
				throws IOException {
	...
	public static final Serializer<byte[]> BYTE_ARRAY = new Serializer<byte[]>() {
		...
		@Override
		public byte[] deserialize(DataInput in, int available)
				throws IOException {
			int size = DataIO.unpackInt(in);
	...
	/**
	 * Serializes `byte[]` directly into underlying store It does not store
	 * size, so it can not be used in Maps and other collections.
	 */
	...
	public static final Serializer<char[]> CHAR_ARRAY = new Serializer<char[]>() {
		...
		@Override
		public char[] deserialize(DataInput in, int available)
				throws IOException {
	...
	public static final Serializer<int[]> INT_ARRAY = new Serializer<int[]>() {
		...
		@Override
		public int[] deserialize(DataInput in, int available)
				throws IOException {
			final int size = DataIO.unpackInt(in);
	...
	public static final Serializer<double[]> DOUBLE_ARRAY = new Serializer<double[]>() {
		...
		@Override
		public double[] deserialize(DataInput in, int available)
				throws IOException {
			final int size = DataIO.unpackInt(in);
	...
	public static final Serializer<java.util.UUID> UUID = new Serializer<java.util.UUID>() {
		...
		public boolean equals(UUID a1, UUID a2) {
			...
			return a1 == a2
					|| (a1 != null
							&& a1.getLeastSignificantBits() == a2
									.getLeastSignificantBits() && a1
							.getMostSignificantBits() == a2
	...
	public static final Serializer<Float> FLOAT = new Serializer<Float>() {
		...
		@Override
		public Float deserialize(DataInput in, int available)
				throws IOException {
			return in.readFloat();
		}
	...
	public static final Serializer<Short> SHORT = new Serializer<Short>() {
		...
		@Override
		public Short deserialize(DataInput in, int available)
				throws IOException {
			return in.readShort();
		}
	...
	public static final Serializer<boolean[]> BOOLEAN_ARRAY = new Serializer<boolean[]>() {
		...
		public void serialize(DataOutput out, boolean[] value)
				throws IOException {
			DataIO.packInt(out, value.length);// write the number of booleans
	...
	public static final Serializer<BigInteger> BIG_INTEGER = new Serializer<BigInteger>() {
		@Override
		public void serialize(DataOutput out, BigInteger value)
				throws IOException {
			BYTE_ARRAY.serialize(out, value.toByteArray());
		}

		@Override
		public BigInteger deserialize(DataInput in, int available)
				throws IOException {
	...
	public static final Serializer<BigDecimal> BIG_DECIMAL = new Serializer<BigDecimal>() {
		@Override
		public void serialize(DataOutput out, BigDecimal value)
				throws IOException {
		...
		@Override
		public BigDecimal deserialize(DataInput in, int available)
				throws IOException {
			return new BigDecimal(
					new BigInteger(BYTE_ARRAY.deserialize(in, -1)),
					DataIO.unpackInt(in));
		}
	...
	public static final Serializer<Class> CLASS = new Serializer<Class>() {
		...
		@Override
		public Class deserialize(DataInput in, int available)
				throws IOException {
	...
	public final static class CompressionWrapper<E> extends Serializer<E>
		...
		protected CompressionWrapper(SerializerBase serializerBase,
				DataInput is, SerializerBase.FastArrayList<Object> objectStack)
				throws IOException {
			objectStack.add(this);
			this.serializer = (Serializer<E>) serializerBase.deserialize(is,
					objectStack);
		}
		...
		public void serialize(DataOutput out, E value) throws IOException {
			...
			DataIO.packInt(out, out2.pos + 1); // unpacked size, zero indicates
												// no compression
		...
		public E deserialize(DataInput in, int available) throws IOException {
			...
			if (unpackedSize == -1) {
				// was not compressed
				return serializer.deserialize(in, available > 0 ? available - 1
						: available);
			}

			byte[] unpacked = new byte[unpackedSize];
			LZF.get().expand(in, unpacked, 0, unpackedSize);
			DataIO.DataInputByteArray in2 = new DataIO.DataInputByteArray(
					unpacked);
	...
	public static final class Array extends Serializer<Object[]> implements
			Serializable {
		...
		/** used for deserialization */
		protected Array(SerializerBase serializerBase, DataInput is,
				SerializerBase.FastArrayList<Object> objectStack)
				throws IOException {
			objectStack.add(this);
			this.serializer = (Serializer) serializerBase.deserialize(is,
					objectStack);
		}
	...
	/**
	 * Basic serializer for most classes in 'java.lang' and 'java.util'
	 * packages. It does not handle custom POJO classes. It also does not handle
	 * classes which require access to `DB` itself.
	 */
	 ...
	 *            Object to serialize
	 */
	abstract public void serialize(DataOutput out, A value) throws IOException;
	 ...
	 * @param available
	 *            how many bytes are available in DataInput for reading, may be
	 *            -1 (in streams) or 0 (null).
