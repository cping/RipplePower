...
/**
 * Compiler Configuration. There are some static final boolean fields, which
 * describe features MapDB was compiled with.
 * <p>
 * MapDB can be compiled with/without some features. For example fine logging is
 * useful for debuging, but should not be present in production version. Java
 * does not have preprocessor so we use <a
 * href="http://en.wikipedia.org/wiki/Dead_code_elimination">Dead code
 * elimination</a> to achieve it.
 * <p>
 * Typical usage:
 * 
 * <pre>
...
public interface CC {
	...
	/**
	 * Compile with more assertions and verifications. For example HashMap may
	 * check if keys implements hash function correctly. This may slow down
	 * MapDB thousands times
	 */
	...
	//
	// /**
	// * If true MapDB will display warnings if user is using MapDB API wrong
	// way.
	// */
