...
public abstract class Store implements Engine {
	...
	/**
	 * default serializer used for persistence. Handles POJO and other stuff
	 * which requires write-able access to Engine
	 */
	...
	protected Store(String fileName,
			Fun.Function1<Volume, String> volumeFactory, boolean checksum,
	 ...
	 * It means file allocated by physical file
	 */
	...
	/**
	 * get some statistics about store. This may require traversing entire
	 * store, so it can take some time.
	 */
		...
		if (pojoLock != null) {
			...
			try {
				if (serializerPojo == null) {
					final CopyOnWriteArrayList<SerializerPojo.ClassInfo> classInfos = get(
							Engine.RECID_CLASS_CATALOG,
	...
	protected final Queue<DataIO.DataOutputByteArray> recycledDataOuts = new ArrayBlockingQueue<DataIO.DataOutputByteArray>(
			128);

	protected <A> DataIO.DataOutputByteArray serialize(A value,
			...
			if (out.pos > 0) {
				...
				if (compress) {
					...
					if (newLen == 0) {
						...
						// compression had no effect, so just write zero at
						// beginning and move array by 1
						out.ensureAvail(out.pos + 1);
						System.arraycopy(out.buf, 0, out.buf, 1, out.pos);
						out.pos += 1;
						out.buf[0] = 0;
					} else {
						// compression had effect, so write decompressed size
						// and compressed array
				...
				if (encrypt) {
					...
					if (size % EncryptionXTEA.ALIGN != 0)
						size += EncryptionXTEA.ALIGN - size
								% EncryptionXTEA.ALIGN;
				...
				if (CC.PARANOID)
					try {
						...
						DataInput inp = new DataIO.DataInputByteArray(
								Arrays.copyOf(out.buf, out.pos));
						byte[] decompress = deserialize(
								Serializer.BYTE_ARRAY_NOSIZE, out.pos, inp);

						DataIO.DataOutputByteArray expected = newDataOut2();
						serializer.serialize(expected, value);

						byte[] expected2 = Arrays.copyOf(expected.buf,
								expected.pos);
						// check arrays equals
						if (CC.PARANOID
								&& !(Arrays.equals(expected2, decompress)))
	...
	protected <A> A deserialize(Serializer<A> serializer, int size,
		...
		if (size > 0) {
			if (checksum) {
				...
				if (check != checkExpected)
					throw new IOException(
							"Checksum does not match, data broken");
			}
			...
			if (compress) {
				...
				if (decompSize == 0) {
					...
					// argument is not needed; unpackedSize=
					// size-(di.pos-origPos),
		...
		if (size + start > di.getPos())
			throw new AssertionError(
					"data were not fully read, check your serializer ");
		if (size + start < di.getPos())
			throw new AssertionError(
					"data were read beyond record size, check your serializer");
		return ret;
	}
