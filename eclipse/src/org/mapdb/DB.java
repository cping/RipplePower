 ...
 * @author Jan Kotek
 */
// TODO DB uses global lock, replace it with ReadWrite lock or fine grained
// locking.
...
public class DB implements Closeable {
	...
	/**
	 * already loaded named collections. It is important to keep collections as
	 * singletons, because of 'in-memory' locking
	 */
	protected Map<String, WeakReference<?>> namesInstanciated = new HashMap<String, WeakReference<?>>();

	protected Map<IdentityWrapper, String> namesLookup = Collections
			.synchronizedMap( // TODO remove synchronized map, after DB locking
								// is resolved
	...
	/**
	 * Construct new DB. It is just thin layer over {@link Engine} which does
	 * the real work.
	 * 
	 * @param engine
	 */
	...
	/**
	 * returns name for this object, if it has name and was instanciated by this
	 * DB
	 */
	...
	public class HTreeMapMaker {
		...
		/**
		 * by default collection does not have counter, without counter updates
		 * are faster, but entire collection needs to be traversed to count
		 * items.
		 */
		...
		/**
		 * maximal number of entries in this map. Less used entries will be
		 * expired and removed to make collection smaller
		 */
		 ...
		 * map once a fixed duration has elapsed after the entry's creation, or
		...
		/**
		 * Specifies that each entry should be automatically removed from the
		 * map once a fixed duration has elapsed after the entry's creation, the
		 * most recent replacement of its value, or its last access. Access time
		 * is reset by all map read and write operations
		 */
		 ...
		 * `valueCreator` is used to return new value. This way `HTreeMap.get()`
		 * never returns null
		 */
		...
		public <K, V> HTreeMapMaker pumpSource(Iterator<K> keysSource,
		...
		public <K, V> HTreeMapMaker pumpSource(
				Iterator<Fun.Pair<K, V>> entriesSource) {
		...
		public <K, V> HTreeMap<K, V> makeOrGet() {
			...
			synchronized (DB.this) {
				...
				// $DELAY$
				return (HTreeMap<K, V>) (catGet(name + ".type") == null ? make()
						: getHashMap(name));
			}
		}
	...
	public class HTreeSetMaker {
		...
		/**
		 * maximal size of store in GB, if store is larger entries will start
		 * expiring
		 */
		...
		public <K> Set<K> makeOrGet() {
			synchronized (DB.this) {
				// $DELAY$
				// TODO add parameter check
				return (Set<K>) (catGet(name + ".type") == null ? make()
						: getHashSet(name));
			}
		}
	...
	/**
	 * Opens existing or creates new Hash Tree Map. This collection perform well
	 * under concurrent access. Is best for large keys and large values.
	 * 
	 * @param name
	 *            of the map
	 * @return map
	 ...
	 *            of map
	 * @param valueCreator
	 *            if value is not found, new is created and placed into map.
	 * @return map
	 */
	synchronized public <K, V> HTreeMap<K, V> getHashMap(String name,
		...
		if (type == null) {
			...
			if (engine.isReadOnly()) {
				...
				return namedPut(name, new DB(
						new EngineWrapper.ReadOnlyEngine(e)).getHashMap("a"));
			}
		...
		// open existing map
		// $DELAY$
		ret = new HTreeMap<K, V>(engine, (Long) catGet(name + ".counterRecid"),
				(Integer) catGet(name + ".hashSalt"), (long[]) catGet(name
						+ ".segmentRecids"), catGet(name + ".keySerializer",
						getDefaultSerializer()), catGet(name
						+ ".valueSerializer", getDefaultSerializer()), catGet(
						name + ".expireTimeStart", 0L), catGet(
						name + ".expire", 0L), catGet(name + ".expireAccess",
						0L), catGet(name + ".expireMaxSize", 0L), catGet(name
						+ ".expireStoreSize", 0L), (long[]) catGet(name
						+ ".expireHeads", null), (long[]) catGet(name
						+ ".expireTails", null), valueCreator, threadFactory);
	...
	synchronized protected <K, V> HTreeMap<K, V> createHashMap(HTreeMapMaker m) {
		...
		if (m.expire != 0 || m.expireAccess != 0 || m.expireMaxSize != 0
				|| m.expireStoreSize != 0) {
			expireTimeStart = catPut(name + ".expireTimeStart",
			...
			expireStoreSize = catPut(name + ".expireStoreSize",
					m.expireStoreSize);
			// $DELAY$
		...
		HTreeMap<K, V> ret = new HTreeMap<K, V>(engine, catPut(name
				+ ".counterRecid",
				!m.counter ? 0L : engine.put(0L, Serializer.LONG)), catPut(name
				+ ".hashSalt", new Random().nextInt()), catPut(name
				+ ".segmentRecids", HTreeMap.preallocateSegments(engine)),
				catPut(name + ".keySerializer", m.keySerializer,
						getDefaultSerializer()), catPut(name
						+ ".valueSerializer", m.valueSerializer,
						getDefaultSerializer()), expireTimeStart, expire,
				expireAccess, expireMaxSize, expireStoreSize, expireHeads,
				expireTails, (Fun.Function1<V, K>) m.valueCreator,
				threadFactory
		...
		if (m.pumpSource != null) {
			Pump.fillHTreeMap(ret, m.pumpSource, m.pumpKeyExtractor,
					m.pumpValueExtractor, m.pumpPresortBatchSize,
					m.pumpIgnoreDuplicates, getDefaultSerializer());
		}
	...
	synchronized public <K> Set<K> getHashSet(String name) {
		...
		if (type == null) {
			...
			if (engine.isReadOnly()) {
				...
				return namedPut(name, new DB(
						new EngineWrapper.ReadOnlyEngine(e)).getHashSet("a"));
			}
		...
		// open existing map
		ret = new HTreeMap<K, Object>(engine, (Long) catGet(name
				+ ".counterRecid"), (Integer) catGet(name + ".hashSalt"),
				(long[]) catGet(name + ".segmentRecids"), catGet(name
						+ ".serializer", getDefaultSerializer()), null, 0L, 0L,
				0L, 0L, 0L, null, null, null, threadFactory).keySet();
	...
	synchronized protected <K> Set<K> createHashSet(HTreeSetMaker m) {
		...
		// $DELAY$
		HTreeMap<K, Object> ret = new HTreeMap<K, Object>(engine, catPut(name
				+ ".counterRecid",
				!m.counter ? 0L : engine.put(0L, Serializer.LONG)), catPut(name
				+ ".hashSalt", new Random().nextInt()), catPut(name
				+ ".segmentRecids", HTreeMap.preallocateSegments(engine)),
				catPut(name + ".serializer", m.serializer,
						getDefaultSerializer()), null, expireTimeStart, expire,
				expireAccess, expireMaxSize, expireStoreSize, expireHeads,
				expireTails, null, threadFactory);
	...
	public class BTreeMapMaker {
		...
		/**
		 * nodeSize maximal size of node, larger node causes overflow and
		 * creation of new BTree node. Use large number for small keys, use
		 * small number for large keys.
		 */
		...
		/**
		 * by default values are stored inside BTree Nodes. Large values should
		 * be stored outside of BTreeNodes
		 */
		...
		/**
		 * keySerializer used to convert keys into/from binary form. This wraps
		 * ordinary serializer, with no delta packing used
		 */
		public BTreeMapMaker keySerializer(Serializer serializer) {
			this.keySerializer = new BTreeKeySerializer.BasicKeySerializer(
		...
		public <K, V> BTreeMapMaker pumpSource(Iterator<K> keysSource,
				Fun.Function1<V, K> valueExtractor) {
			this.pumpSource = keysSource;
			this.pumpKeyExtractor = Fun.extractNoTransform();
			this.pumpValueExtractor = valueExtractor;
		...
		public <K, V> BTreeMapMaker pumpSource(
				Iterator<Fun.Pair<K, V>> entriesSource) {
			this.pumpSource = entriesSource;
			this.pumpKeyExtractor = Fun.extractKey();
			this.pumpValueExtractor = Fun.extractValue();
		...
		public <K, V> BTreeMap<K, V> makeOrGet() {
			synchronized (DB.this) {
				// TODO add parameter check
				return (BTreeMap<K, V>) (catGet(name + ".type") == null ? make()
						: getTreeMap(name));
			}
		}
	...
	public class BTreeSetMaker {
		...
		public <K> NavigableSet<K> makeOrGet() {
			synchronized (DB.this) {
				// TODO add parameter check
				return (NavigableSet<K>) (catGet(name + ".type") == null ? make()
						: getTreeSet(name));
			}
		}
	...
	/**
	 * Opens existing or creates new B-linked-tree Map. This collection performs
	 * well under concurrent access. Only trade-off are deletes, which causes
	 * tree fragmentation. It is ordered and best suited for small keys and
	 * values.
	 * 
	 * @param name
	 *            of map
	...
	synchronized public <K, V> BTreeMap<K, V> getTreeMap(String name) {
		...
		if (type == null) {
			...
			if (engine.isReadOnly()) {
				...
				// $DELAY$
				return namedPut(name, new DB(
						new EngineWrapper.ReadOnlyEngine(e)).getTreeMap("a"));
			}
		...
		ret = new BTreeMap<K, V>(engine, (Long) catGet(name + ".rootRecidRef"),
				catGet(name + ".maxNodeSize", 32), catGet(name
						+ ".valuesOutsideNodes", false), catGet(name
						+ ".counterRecid", 0L), catGet(name + ".keySerializer",
						new BTreeKeySerializer.BasicKeySerializer(
								getDefaultSerializer(), Fun.COMPARATOR)),
	...
	synchronized protected <K, V> BTreeMap<K, V> createTreeMap(
			final BTreeMapMaker m) {
		String name = m.name;
		checkNameNotExists(name);
		// $DELAY$
		...
		m.keySerializer = catPut(name + ".keySerializer", m.keySerializer,
				new BTreeKeySerializer.BasicKeySerializer(
						getDefaultSerializer(), m.comparator));
		m.valueSerializer = catPut(name + ".valueSerializer",
		...
		if (m.pumpPresortBatchSize != -1 && m.pumpSource != null) {
			Comparator presortComp = new Comparator() {
				...
				public int compare(Object o1, Object o2) {
					return -m.comparator.compare(m.pumpKeyExtractor.run(o1),
							m.pumpKeyExtractor.run(o2));
				}
			};

			m.pumpSource = Pump
					.sort(m.pumpSource, m.pumpIgnoreDuplicates,
							m.pumpPresortBatchSize, presortComp,
							getDefaultSerializer());
		}
		// $DELAY$
		...
		long rootRecidRef;
		if (m.pumpSource == null) {
			rootRecidRef = BTreeMap.createRootRef(engine, m.keySerializer,
					m.valueSerializer, 0);
		} else {
			rootRecidRef = Pump.buildTreeMap((Iterator<K>) m.pumpSource,
					engine, (Fun.Function1<K, K>) m.pumpKeyExtractor,
					(Fun.Function1<V, K>) m.pumpValueExtractor,
					m.pumpIgnoreDuplicates, m.nodeSize, m.valuesOutsideNodes,
					counterRecid, m.keySerializer,
					(Serializer<V>) m.valueSerializer);
		}
		// $DELAY$
		BTreeMap<K, V> ret = new BTreeMap<K, V>(engine, catPut(name
				+ ".rootRecidRef", rootRecidRef), catPut(name + ".maxNodeSize",
				m.nodeSize), catPut(name + ".valuesOutsideNodes",
				m.valuesOutsideNodes), catPut(name + ".counterRecid",
				counterRecid), m.keySerializer,
				(Serializer<V>) m.valueSerializer, catPut(m.name
						+ ".numberOfNodeMetas", 0));
		// $DELAY$
	...
	protected BTreeKeySerializer fillNulls(BTreeKeySerializer keySerializer) {
		...
		if (keySerializer instanceof BTreeKeySerializer.ArrayKeySerializer) {
			...
			for (int i = 0; i < k.tsize; i++) {
				serializers[i] = k.serializers[i] != null
						&& k.serializers[i] != Serializer.BASIC ? k.serializers[i]
						: getDefaultSerializer();
				comparators[i] = k.comparators[i] != null ? k.comparators[i]
						: Fun.COMPARATOR;
			}
			// $DELAY$
			return new BTreeKeySerializer.ArrayKeySerializer(comparators,
					serializers);
		}
		// $DELAY$
	...
	/**
	 * Get Name Catalog. It is metatable which contains information about named
	 * collections and records. Each collection constructor takes number of
	 * parameters, this map contains those parameters.
	 * 
	...
	synchronized public <K> NavigableSet<K> getTreeSet(String name) {
		...
		if (type == null) {
			...
			if (engine.isReadOnly()) {
				...
				return namedPut(name, new DB(
						new EngineWrapper.ReadOnlyEngine(e)).getTreeSet("a"));
			}
			// $DELAY$
		...
		// $DELAY$
		ret = new BTreeMap<K, Object>(engine, (Long) catGet(name
				+ ".rootRecidRef"), catGet(name + ".maxNodeSize", 32), false,
				catGet(name + ".counterRecid", 0L), catGet(name
						+ ".keySerializer",
						new BTreeKeySerializer.BasicKeySerializer(
								getDefaultSerializer(), Fun.COMPARATOR)), null,
				catGet(name + ".numberOfNodeMetas", 0)).keySet();
		// $DELAY$
		namedPut(name, ret);
		return ret;
	...
	synchronized public <K> NavigableSet<K> createTreeSet(BTreeSetMaker m) {
		...
		if (m.pumpPresortBatchSize != -1) {
			m.pumpSource = Pump.sort(m.pumpSource, m.pumpIgnoreDuplicates,
		...
		// $DELAY$
		if (m.pumpSource == null) {
			rootRecidRef = BTreeMap
					.createRootRef(engine, m.serializer, null, 0);
		} else {
			rootRecidRef = Pump.buildTreeMap((Iterator<Object>) m.pumpSource,
					engine, Fun.extractNoTransform(), null,
					m.pumpIgnoreDuplicates, m.nodeSize, false, counterRecid,
					m.serializer, null);
		}
		// $DELAY$
		NavigableSet<K> ret = new BTreeMap<K, Object>(engine, catPut(m.name
				+ ".rootRecidRef", rootRecidRef), catPut(m.name
				+ ".maxNodeSize", m.nodeSize), false, catPut(m.name
				+ ".counterRecid", counterRecid), m.serializer, null, catPut(
				m.name + ".numberOfNodeMetas", 0)).keySet();
		// $DELAY$
	...
	synchronized public <E> BlockingQueue<E> getQueue(String name) {
		...
		if (type == null) {
			...
			if (engine.isReadOnly()) {
				...
				new DB(e).getQueue("a");
				return namedPut(name, new DB(
						new EngineWrapper.ReadOnlyEngine(e)).getQueue("a"));
			}
			// $DELAY$
		...
		// $DELAY$
		ret = new Queues.Queue<E>(engine, (Serializer<E>) catGet(name
				+ ".serializer", getDefaultSerializer()), (Long) catGet(name
				+ ".headRecid"), (Long) catGet(name + ".tailRecid"),
	...
	synchronized public <E> BlockingQueue<E> createQueue(String name,
			Serializer<E> serializer, boolean useLocks) {
		checkNameNotExists(name);

		long node = engine.preallocate(); // serializer is new
											// Queues.SimpleQueue.NodeSerializer(serializer)
		long headRecid = engine.put(node, Serializer.LONG);
		long tailRecid = engine.put(node, Serializer.LONG);
		// $DELAY$
		Queues.Queue<E> ret = new Queues.Queue<E>(engine, catPut(name
				+ ".serializer", serializer, getDefaultSerializer()), catPut(
				name + ".headRecid", headRecid), catPut(name + ".tailRecid",
				tailRecid), catPut(name + ".useLocks", useLocks));
	...
	synchronized public <E> BlockingQueue<E> getStack(String name) {
		...
		if (type == null) {
			...
			if (engine.isReadOnly()) {
				...
				return namedPut(name, new DB(
						new EngineWrapper.ReadOnlyEngine(e)).getStack("a"));
			}
		...
		ret = new Queues.Stack<E>(engine, (Serializer<E>) catGet(name
				+ ".serializer", getDefaultSerializer()), (Long) catGet(name
				+ ".headRecid"));
		// $DELAY$
		namedPut(name, ret);
	...
	synchronized public <E> BlockingQueue<E> createStack(String name,
			Serializer<E> serializer, boolean useLocks) {
		checkNameNotExists(name);
		...
		long headRecid = engine.put(node, Serializer.LONG);
		// $DELAY$
		Queues.Stack<E> ret = new Queues.Stack<E>(engine, catPut(name
				+ ".serializer", serializer, getDefaultSerializer()), catPut(
				name + ".headRecid", headRecid));
		// $DELAY$
	...
	synchronized public <E> BlockingQueue<E> getCircularQueue(String name) {
		...
		if (type == null) {
			...
			if (engine.isReadOnly()) {
				...
				return namedPut(name,
						new DB(new EngineWrapper.ReadOnlyEngine(e))
								.getCircularQueue("a"));
			}
		...
		ret = new Queues.CircularQueue<E>(engine, (Serializer<E>) catGet(name
				+ ".serializer", getDefaultSerializer()), (Long) catGet(name
				+ ".headRecid"), (Long) catGet(name + ".headInsertRecid"),
	...
	synchronized public <E> BlockingQueue<E> createCircularQueue(String name,
			Serializer<E> serializer, long size) {
		checkNameNotExists(name);
		if (serializer == null)
		...
		long firstRecid = 0;
		// $DELAY$
		Serializer<Queues.SimpleQueue.Node<E>> nodeSer = new Queues.SimpleQueue.NodeSerializer<E>(
				serializer);
		for (long i = 0; i < size; i++) {
			Queues.SimpleQueue.Node<E> n = new Queues.SimpleQueue.Node<E>(
					prevRecid, null);
		...
		engine.update(firstRecid, new Queues.SimpleQueue.Node<E>(prevRecid,
				null), nodeSer);
		...
		Queues.CircularQueue<E> ret = new Queues.CircularQueue<E>(engine,
				catPut(name + ".serializer", serializer), catPut(name
						+ ".headRecid", headRecid), catPut(name
						+ ".headInsertRecid", headInsertRecid), catPut(name
						+ ".size", size));
		// $DELAY$
	...
	synchronized public Atomic.Long getAtomicLong(String name) {
		...
		if (type == null) {
			...
			if (engine.isReadOnly()) {
				...
				// $DELAY$
				return namedPut(name, new DB(
						new EngineWrapper.ReadOnlyEngine(e)).getAtomicLong("a"));
			}
	...
	synchronized public Atomic.Integer createAtomicInteger(String name,
			int initValue) {
		checkNameNotExists(name);
		long recid = engine.put(initValue, Serializer.INTEGER);
		Atomic.Integer ret = new Atomic.Integer(engine, catPut(name + ".recid",
				recid));
		// $DELAY$
	...
	synchronized public Atomic.Boolean createAtomicBoolean(String name,
			boolean initValue) {
		checkNameNotExists(name);
		long recid = engine.put(initValue, Serializer.BOOLEAN);
		// $DELAY$
		Atomic.Boolean ret = new Atomic.Boolean(engine, catPut(name + ".recid",
				recid));
	...
	public void checkShouldCreate(String name) {
		if (strictDBGet)
			throw new NoSuchElementException(
					"No record with this name was found: " + name);
	}

	synchronized public Atomic.String createAtomicString(String name,
			String initValue) {
		checkNameNotExists(name);
		if (initValue == null)
		...
		// $DELAY$
		Atomic.String ret = new Atomic.String(engine, catPut(name + ".recid",
				recid));
		// $DELAY$
	...
	synchronized public <E> Atomic.Var<E> createAtomicVar(String name,
			E initValue, Serializer<E> serializer) {
		checkNameNotExists(name);
		if (serializer == null)
			serializer = getDefaultSerializer();
		long recid = engine.put(initValue, serializer);
		// $DELAY$
		Atomic.Var ret = new Atomic.Var(engine, catPut(name + ".recid", recid),
	...
	synchronized public <E> Atomic.Var<E> getAtomicVar(String name) {
		...
		if (type == null) {
			...
			if (engine.isReadOnly()) {
				...
				return namedPut(name, new DB(
						new EngineWrapper.ReadOnlyEngine(e)).getAtomicVar("a"));
			}
			// $DELAY$
		...
		ret = new Atomic.Var(engine, (Long) catGet(name + ".recid"),
				(Serializer) catGet(name + ".serializer"));
		namedPut(name, ret);
		return ret;
	}
	...
	synchronized public void delete(String name) {
		...
		if (r instanceof Atomic.Boolean) {
			...
			HTreeMap m = (r instanceof HTreeMap) ? (HTreeMap) r
					: ((HTreeMap.KeySet) r).parent();
			m.clear();
			// $DELAY$
			// delete segments
			...
			BTreeMap m = (r instanceof BTreeMap) ? (BTreeMap) r
					: (BTreeMap) ((BTreeMap.KeySet) r).m;
			// $DELAY$
	 ...
	 * 
	 * @param oldName
	 *            current name of record/collection
	 * @param newName
	 *            new name of record/collection
	...
	synchronized public void rename(String oldName, String newName) {
		...
		if (toRemove.isEmpty())
			throw new NoSuchElementException(
					"Could not rename, name does not exist: " + oldName);
		// $DELAY$
	...
	/**
	 * Closes database. All other methods will throw 'IllegalAccessError' after
	...
	/**
	 * All collections are weakly referenced to prevent two instances of the
	 * same collection in memory. This is mainly for locking, two instances of
	 * the same lock would not simply work.
	 */
	...
	/**
	 * Perform storage maintenance. Typically compact underlying storage and
	 * reclaim unused space.
	 * <p>
	 * NOTE: MapDB does not have smart defragmentation algorithms. So compaction
	 * usually recreates entire store from scratch. This may require additional
	 * disk space.
	 */
	...
	/**
	 * Make readonly snapshot view of DB and all of its collection Collections
	 * loaded by this instance are not affected (are still mutable). You have to
	 * load new collections from DB returned by this method
	 * 
	...
	/**
	 * @return default serializer used in this DB, it handles POJO and other
	 *         stuff.
	 */
