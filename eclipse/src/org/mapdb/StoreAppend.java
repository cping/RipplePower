...
public class StoreAppend extends Store {
	...
	/**
	 * index value has two parts, first is file number, second is offset in
	 * file, this is how many bites file offset occupies
	 */
	...
	protected static final int MAX_FILE_SIZE_SHIFT = CC.VOLUME_SLICE_SHIFT + 6; // TODO
																				// shift
																				// +
																				// 6
																				// !!
	...
	/**
	 * at place of recid indicates commited transaction, just ignore this value
	 * and continue
	 */
	...
	protected Volume index = new Volume.MemoryVol(false, MAX_FILE_SIZE_SHIFT); // TODO
																				// option
																				// to
																				// keep
																				// index
																				...
																				// file
	/**
	 * same as `index`, but stores uncommited modifications made in this
	 * transaction
	 */
	protected final LongMap<Long> indexInTx;

	public StoreAppend(final String fileName,
			...
			final boolean syncOnCommitDisabled, boolean checksum,
			boolean compress, byte[] password) {
		...
		if (!parent.exists() || !parent.isDirectory())
			throw new IllegalArgumentException("Parent dir does not exist: "
					+ fileName);
		...
		if (sortedFiles.isEmpty()) {
			...
			Volume zero = Volume.volumeForFile(getFileFromNum(0),
					useRandomAccessFile, readOnly, MAX_FILE_SIZE_SHIFT, 0);
			...
			for (long recid = 1; recid <= RECID_LAST_RESERVED; recid++) {
				...
				pos += zero.putPackedLong(pos, 0 + SIZEP); // and mark it with
															// zero size
															// (0==tombstone)
			}
			...
			for (Fun.Pair<Long, File> t : sortedFiles) {
				Long num = t.a;
				File f = t.b;
				Volume vol = Volume.volumeForFile(f, useRandomAccessFile,
						readOnly, MAX_FILE_SIZE_SHIFT, 0);
				...
				while (pos <= FILE_MASK) {
					...
					if (recid == END) {
						...
						throw new IOError(new IOException("File corrupted: "
								+ f));
					}
					...
					if (size == 0) {
						...
						index.putLong(recid * 8, Long.MIN_VALUE); // TODO
																	// tombstone
					}
				}
			}
			...
			throw new IOError(new IOException(
					"File not sealed, data possibly corrupted"));
		}
	}

	public StoreAppend(String fileName) {
		this(fileName, fileName == null || fileName.isEmpty() ? Volume
				.memoryFactory() : Volume.fileFactory(), false, false, false,
	...
	protected void rollover() {
		...
		currVolume.sync();
		currFileNum++;
		currVolume = Volume.volumeForFile(getFileFromNum(currFileNum),
				useRandomAccessFile, readOnly, MAX_FILE_SIZE_SHIFT, 0);
	...
	public <A> long put(A value, Serializer<A> serializer) {
		...
		try {
			...
			try {
				...
				indexVal = (currFileNum << FILE_SHIFT) | currPos; // TODO file
																	// number
				// write size
	...
	protected <A> A getNoLock(long recid, Serializer<A> serializer)
			throws IOException {
		...
		if (size < 0)
			return null;
		if (size == 0)
			return serializer.deserialize(new DataIO.DataInputByteArray(
					new byte[0]), 0);
	...
	public void commit() {
		...
		try {
			...
			rollbackCurrVolume.putUnsignedByte(rollbackCurrPos,
					(int) (SKIP + RECIDP));
			if (syncOnCommit)
