...
public final class Caches {
	...
	/**
	 * Least Recently Used cache. If cache is full it removes less used items to
	 * make a space
	 */
	public static class LRU extends EngineWrapper {
		...
		public LRU(Engine engine, int cacheSize, Fun.RecordCondition condition) {
			this(engine, new LongConcurrentLRUMap<Object>(cacheSize,
					(int) (cacheSize * 0.8)), condition);
		}

		public LRU(Engine engine, LongMap<Object> cache,
				Fun.RecordCondition condition) {
			super(engine);
		...
		public <A> boolean compareAndSwap(long recid, A expectedOldValue,
			...
			if (!condition.run(recid, newValue, serializer)) {
				// $DELAY$
				return super.compareAndSwap(recid, expectedOldValue, newValue,
						serializer);
			}
			...
			try {
				...
				if (oldValue == expectedOldValue
						|| (oldValue != null && oldValue
								.equals(expectedOldValue))) {
					// found matching entry in cache, so just update and return
					// true
					...
					// $DELAY$
					boolean ret = engine.compareAndSwap(recid,
							expectedOldValue, newValue, serializer);
					// $DELAY$
					if (ret)
						cache2.put(recid, newValue);
					// $DELAY$
	...
	/**
	 * Fixed size cache which uses hash table. Is thread-safe and requires only
	 * minimal locking. Items are randomly removed and replaced by hash
	 * collisions.
	 * <p>
	...
	public static class HashTable extends EngineWrapper implements Engine {
		...
		public HashTable(Engine engine, int cacheMaxSize,
			...
			this.cacheMaxSize = 1 << (32 - Integer
					.numberOfLeadingZeros(cacheMaxSize - 1)); // next pow of two
		...
		public <A> A get(long recid, Serializer<A> serializer) {
			...
			HashItem[] items2 = checkClosed(items);
			HashItem item = items2[pos]; // TODO race condition? non volatile
											// access
		...
		public <A> boolean compareAndSwap(long recid, A expectedOldValue,
			...
			try {
				...
				if (item != null && item.key == recid) {
					...
					if (item.val == expectedOldValue
							|| item.val.equals(expectedOldValue)) {
						// $DELAY$
						// found matching entry in cache, so just update and
						// return true
	...
	/**
	 * Instance cache which uses <code>SoftReference</code> or
	 * <code>WeakReference</code> Items can be removed from cache by Garbage
	 * Collector if
	 * 
	 * @author Jan Kotek
	 */
	public static class WeakSoftRef extends EngineWrapper implements Engine {
		...
		protected static final class CacheWeakItem<A> extends WeakReference<A>
		...
		protected static final class CacheSoftItem<A> extends SoftReference<A>
				implements CacheItem {
		...
		public <A> long put(A value, Serializer<A> serializer) {
			...
			CacheItem item = useWeakRef ? new CacheWeakItem(value, q, recid)
		...
		public <A> A get(long recid, Serializer<A> serializer) {
			...
			try {
				...
				if (value != null && condition.run(recid, value, serializer)) {
					ReferenceQueue<A> q = (ReferenceQueue<A>) checkClosed(queue);
					// $DELAY$
					item = useWeakRef ? new CacheWeakItem(value, q, recid)
							: new CacheSoftItem(value, q, recid);
					CacheItem old = items2.put(recid, item);
					// $DELAY$
					if (old != null)
		...
		public <A> void update(long recid, A value, Serializer<A> serializer) {
			...
			LongConcurrentHashMap<CacheItem> items2 = checkClosed(items);
			// $DELAY$
			CacheItem item = useWeakRef ? new CacheWeakItem<A>(value, q, recid)
		...
		public <A> boolean compareAndSwap(long recid, A expectedOldValue,
			...
			try {
				...
				if (item != null
						&& item.getRecid() == recid
						&& (oldValue == expectedOldValue || (oldValue != null && oldValue
								.equals(expectedOldValue)))) {
					// found matching entry in cache, so just update and return
					// true
					// $DELAY$
					CacheItem old = items2.put(recid,
							useWeakRef ? new CacheWeakItem<A>(newValue, q,
									recid) : new CacheSoftItem<A>(newValue, q,
									recid));
					// $DELAY$
					if (old != null)
						old.clear();
					...
					if (ret) {
						...
						CacheItem old = items2.put(recid,
								useWeakRef ? new CacheWeakItem<A>(newValue, q,
										recid) : new CacheSoftItem<A>(newValue,
										q, recid));
						if (old != null)
							old.clear();
					}
		...
		public void close() {
			...
			try {
				...
				// TODO should we wait for cleaner threads to shutdown? I guess
	...
	/**
	 * Cache created objects using hard reference. It checks free memory every N
	 * operations (1024*10). If free memory is bellow 75% it clears the cache
	 * 
	 * @author Jan Kotek
	 */
	public static class HardRef extends LRU {
		...
		private void checkFreeMem() {
			...
			if (CC.LOG_EWRAP && LOG.isLoggable(Level.FINE))
				LOG.fine("HardRefCache: freemem = " + free + " = "
						+ (free / max) + "%");
			// $DELAY$
