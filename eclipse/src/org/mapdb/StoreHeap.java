...
public class StoreHeap extends Store implements Serializable {
	...
	protected final ConcurrentNavigableMap<Long, Fun.Pair> records = new ConcurrentSkipListMap<Long, Fun.Pair>();

	/** All not-yet commited records in store */
	protected final ConcurrentNavigableMap<Long, Fun.Pair> rollback = new ConcurrentSkipListMap<Long, Fun.Pair>();
	...
	public <A> long put(A value, Serializer<A> serializer) {
		...
		try {
			...
			if (recid == null)
				recid = maxRecid.incrementAndGet();
			records.put(recid, new Fun.Pair<Object, Serializer>(value,
					serializer));
	...
	public <A> void update(long recid, A value, Serializer<A> serializer) {
		...
		try {
			Fun.Pair old = records.put(recid, new Fun.Pair<Object, Serializer>(
					value, serializer));
	...
	public <A> boolean compareAndSwap(long recid, A expectedOldValue,
		...
		try {
			Fun.Pair old = expectedOldValue == null ? TOMBSTONE : new Fun.Pair(
					expectedOldValue, serializer);
			Fun.Pair newPair = newValue == null ? TOMBSTONE : new Fun.Pair(
					newValue, serializer);
		...
		try {
			if (!rollback.isEmpty()) {
				throw new DBException(
						DBException.Code.ENGINE_COMPACT_UNCOMMITED);
			}
			Iterator<Map.Entry<Long, Fun.Pair>> iter = records.entrySet()
					.iterator();
			while (iter.hasNext()) {
	...
	public ByteBuffer getRaw(long recid) {
		...
		if (t == null || t.a == null)
			return null;
		return ByteBuffer.wrap(serialize(t.a, (Serializer<Object>) t.b)
				.copyBytes());
	}
	...
	public void updateRaw(long recid, ByteBuffer data) {
		throw new UnsupportedOperationException(
				"can not put raw data into StoreHeap");
	}
