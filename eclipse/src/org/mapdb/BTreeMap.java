...
public class BTreeMap<K, V> extends AbstractMap<K, V> implements
	...
	protected static SortedMap<String, Object> preinitCatalog(DB db) {
		...
		Long rootRef = db.getEngine().get(Engine.RECID_NAME_CATALOG,
				Serializer.LONG);
		...
		if (rootRef == null) {
			...
			NodeSerializer rootSerializer = new NodeSerializer(false,
					BTreeKeySerializer.STRING, db.getDefaultSerializer(), 0);
			BNode root = new LeafNode(keyser.emptyKeys(), true, true, false,
					new Object[] {}, 0);
			rootRef = db.getEngine().put(root, rootSerializer);
			// $DELAY$
			db.getEngine().update(Engine.RECID_NAME_CATALOG, rootRef,
					Serializer.LONG);
			db.getEngine().commit();
		}
		return new BTreeMap<String, Object>(db.engine,
				Engine.RECID_NAME_CATALOG, 32, false, 0, keyser,
	...
	protected static final class ValRefSerializer extends Serializer<ValRef> {
		...
		@Override
		public ValRef deserialize(DataInput in, int available)
				throws IOException {
	...
	public abstract static class BNode {
		...
		public BNode(Object keys, boolean leftEdge, boolean rightEdge,
				boolean tooLarge) {
			this.keys = keys;

			this.flags = (byte) ((leftEdge ? LEFT_MASK : 0)
					| (rightEdge ? RIGHT_MASK : 0) | (tooLarge ? TOO_LARGE_MASK
					: 0));
		}
		...
		public final int compare(final BTreeKeySerializer keyser, int pos1,
				int pos2) {
			if (pos1 == pos2)
				return 0;
			// $DELAY$
		...
		public final int compare(final BTreeKeySerializer keyser, int pos,
				Object second) {
			if (isLeftEdge()) {
				// first position is negative infinity, so everything else is
				// bigger
			...
			if (end > 1) {
				for (int i = 1; i <= end; i++) {
					if (keyser.compare(keys, i - 1, i) >= 0)
						throw new AssertionError("keys are not sorted: "
								+ Arrays.toString(keyser.keysToArray(keys)));
				}
			}
			// check last key is sorted or null
			if (!isRightEdge() && keylen > 2) {
				if (keyser.compare(keys, keylen - 2, keylen - 1) > 0) {
					throw new AssertionError("Last key is not sorted: "
							+ Arrays.toString(keyser.keysToArray(keys)));
				}
			}
		}

		public abstract BNode copyAddKey(BTreeKeySerializer keyser, int pos,
				Object newKey, long newChild, Object newValue);

		public abstract BNode copySplitRight(BTreeKeySerializer keyser,
				int splitPos);

		public abstract BNode copySplitLeft(BTreeKeySerializer keyser,
				int splitPos, long newNext);
	}

	public final static class DirNode extends BNode {
		final long[] child;

		DirNode(Object keys, boolean leftEdge, boolean rightEdge,
				boolean tooLarge, long[] child) {
		...
		@Override
		public String toString() {
			return "Dir(" + leftEdgeInc() + "-" + rightEdgeInc() + "-K"
					+ Fun.toString(keys) + ", C" + Arrays.toString(child) + ")";
		}
		...
		@Override
		public DirNode copyAddKey(BTreeKeySerializer keyser, int pos,
				Object newKey, long newChild, Object newValue) {
			Object keys2 = keyser.putKey(keys, pos - leftEdgeInc(), newKey);

			long[] child2 = BTreeMap.arrayLongPut(child, pos, newChild);
			// $DELAY$
			return new DirNode(keys2, isLeftEdge(), isRightEdge(), false,
					child2);
		}
		...
		public DirNode copySplitRight(BTreeKeySerializer keyser, int splitPos) {
			int keylen = keyser.length(keys);
			Object keys2 = keyser.copyOfRange(keys, splitPos - leftEdgeInc(),
					keylen);
			// $DELAY$
		...
		@Override
		public DirNode copySplitLeft(BTreeKeySerializer keyser, int splitPos,
				long newNext) {
			Object keys2 = keyser.copyOfRange(keys, 0, splitPos + 1
					- leftEdgeInc());
			// $DELAY$
	...
	public final static class LeafNode extends BNode {
		...
		LeafNode(Object keys, boolean leftEdge, boolean rightEdge,
				boolean tooLarge, Object[] vals, long next) {
			super(keys, leftEdge, rightEdge, tooLarge);
			this.vals = vals;
			this.next = next;
		...
		@Override
		public String toString() {
			return "Leaf(" + leftEdgeInc() + "-" + rightEdgeInc() + "-" + "K"
					+ Fun.toString(keys) + ", V" + Arrays.toString(vals)
					+ ", L=" + next + ")";
		}
		...
		@Override
		public LeafNode copyAddKey(BTreeKeySerializer keyser, int pos,
				Object newKey, long newChild, Object newValue) {
			Object keys2 = keyser.putKey(keys, pos - leftEdgeInc(), newKey);
			// $DELAY$
			Object[] vals2 = arrayPut(vals, pos - 1, newValue);
			// $DELAY$
			return new LeafNode(keys2, isLeftEdge(), isRightEdge(), false,
					vals2, next);
		}
		...
		@Override
		public LeafNode copySplitLeft(BTreeKeySerializer keyser, int splitPos,
				long newNext) {
			...
			return new LeafNode(keys2, isLeftEdge(), false, false, vals2,
					newNext);
		}
		...
		public LeafNode copyChangeValue(int pos, Object value) {
			...
			vals2[pos - 1] = value;
			// $DELAY$
			return new LeafNode(keys, isLeftEdge(), isRightEdge(), false,
					vals2, next);
		}
		...
		public LeafNode copyRemoveKey(BTreeKeySerializer keyser, int pos) {
			...
			System.arraycopy(vals, pos, vals2, pos - 1, vals2.length
					- (pos - 1));
			// $DELAY$
			return new LeafNode(keys2, isLeftEdge(), isRightEdge(), false,
					vals2, next);
		...
		public LeafNode copyClear(BTreeKeySerializer keyser) {
			...
			// $DELAY$
			return new LeafNode(keyser.arrayToKeys(keys2), isLeftEdge(),
					isRightEdge(), false, new Object[] {}, next);
		}
	}
	...
	protected static final class NodeSerializer<A, B> extends Serializer<BNode> {
		...
		public NodeSerializer(boolean valsOutsideNodes,
			...
			this.valueSerializer = valsOutsideNodes ? new ValRefSerializer()
					: valueSerializer;
		...
		public void serialize(DataOutput out, BNode value) throws IOException {
			...
			final int header = (isLeaf ? LEAF_MASK : 0)
		...
		public void serializeSetFlags(DataOutput out, BNode value)
				throws IOException {
		...
		private BNode deserializeDir(final DataInput in, final int size,
				final int left, final int right) throws IOException {
			final long[] child = new long[size];
			for (int i = 0; i < size; i++)
				child[i] = DataIO.unpackLong(in);
			int keysize = size - left - right;
			// $DELAY$
			final Object keys = keysize == 0 ? keySerializer.emptyKeys()
					: keySerializer.deserialize(in, keysize);
			// $DELAY$
			return new DirNode(keys, left != 0, right != 0, false, child);
		}

		private BNode deserializeLeaf(final DataInput in, final int size,
				final int left, final int right) throws IOException {
		...
		private void deserSetVals(DataInput in, Object[] vals)
				throws IOException {
	...
	static protected long createRootRef(Engine engine,
			...
			int numberOfNodeMetas) {
		final LeafNode emptyRoot = new LeafNode(keySer.emptyKeys(), true, true,
				false, new Object[] {}, 0);
		...
		long rootRecidVal = engine.put(emptyRoot, new NodeSerializer(false,
				keySer, valueSer, numberOfNodeMetas));
			...
			while (true) {
				...
				do {
					...
					if (pos < A.keysLen(keySerializer) - 1 && v != null
							&& A.key(keySerializer, pos) != null && // TODO
																	// A.key(pos]!=null??
					...
					// if v > highvalue(a)
					if (!A.isRightEdge()
							&& A.compare(keySerializer,
									A.keysLen(keySerializer) - 1, v) < 0) {
						// $DELAY$
				...
				if (A.keysLen(keySerializer) - (A.isLeaf() ? 1 : 0) < maxNodeSize) {
					...
					if ((current != rootRecid)) { // is not root
						...
						BNode R = new DirNode(
								keySerializer.arrayToKeys(new Object[] { A
										.highKey(keySerializer) }), true, true,
								false, new long[] { current, q, 0 });
						// $DELAY$
						...
						// $DELAY$
						if (CC.PARANOID
								&& !(nodeLocks.get(rootRecidRef) == Thread
										.currentThread()))
							throw new AssertionError();
	...
	protected static class BTreeIterator {
		...
		BTreeIterator(BTreeMap m, Object lo, boolean loInclusive, Object hi,
			...
			if (lo == null) {
				...
				// $DELAY$
				Fun.Pair<Integer, LeafNode> l = m.findLargerNode(lo,
						loInclusive);
	...
	private V removeOrReplace(final Object key, final Object value,
			final Object putNewValue) {
		if (key == null)
			...
			for (;;) {
				...
				if (pos > 0 && pos != A.keysLen(keySerializer) - 1) {
					...
					if (value != null && valueSerializer != null
							&& !valueSerializer.equals((V) value, (V) oldVal)) {
						unlock(nodeLocks, current);
						// $DELAY$
						return null;
					}
					...
					if (putNewValue != null && valsOutsideNodes) {
						// $DELAY$
						long recid = engine.put((V) putNewValue,
								valueSerializer);
						// $DELAY$
						putNewValueOutside = new ValRef(recid);
					}

					A = putNewValue != null ? ((LeafNode) A).copyChangeValue(
							pos, putNewValueOutside) : ((LeafNode) A)
							.copyRemoveKey(keySerializer, pos);
					if (CC.PARANOID
							&& !(nodeLocks.get(current) == Thread
									.currentThread()))
						throw new AssertionError();
	...
	private Entry<K, V> findSmallerRecur(BNode n, K key, boolean inclusive) {
		...
		final int start = leaf ? n.keysLen(keySerializer) - 2 : n
				.keysLen(keySerializer) - 1;
		...
		for (int i = start; i >= end; i--) {
			...
			int comp = (key2 == null) ? -1 : keySerializer.comparator()
					.compare(key2, key);
			if (comp < res) {
				if (leaf) {
					// $DELAY$
		...
		while (true) {
			...
			for (int i = 1; i < leaf.keysLen(keySerializer) - 1; i++) {
				...
				if (-leaf.compare(keySerializer, i, key) < comp) {
					// $DELAY$
					return makeEntry(leaf.key(keySerializer, i),
							valExpand(leaf.vals[i - 1]));
				}
	...
	protected Fun.Pair<Integer, LeafNode> findLargerNode(final K key,
			boolean inclusive) {
		if (key == null)
			return null;
	...
	static protected class SubMap<K, V> extends AbstractMap<K, V> implements
		...
		public SubMap(BTreeMap<K, V> m, K lo, boolean loInclusive, K hi,
			...
			this.hiInclusive = hiInclusive;
			if (lo != null && hi != null
					&& m.keySerializer.comparator().compare(lo, hi) > 0) {
				throw new IllegalArgumentException();
			}
	...
	protected static void lock(LongConcurrentHashMap<Thread> locks, long recid) {
		...
		if (CC.PARANOID && !(locks.get(recid) != currentThread))
			throw new AssertionError("node already locked by current thread: "
					+ recid);
