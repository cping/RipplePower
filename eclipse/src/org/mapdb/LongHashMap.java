...
public class LongHashMap<V> extends LongMap<V> implements Serializable {
	...
	private static class EntryIterator<V> extends AbstractMapIterator<V>
			implements LongMapIterator<V> {
	...
	private static class ValueIterator<V> extends AbstractMapIterator<V>
	...
	final Entry<V> removeEntry(long key) {
		...
		while (entry != null
				&& !(entry.origKeyHash == hash && key == entry.key)) {
			last = entry;
			entry = entry.next;
		}
