...
/**
 * Data Pump moves data from one source to other. It can be used to import data
 * from text file, or copy store from memory to disk.
 */
public final class Pump {

	/** copies all data from first DB to second DB */
	// TODO Pump between stores is disabled for now, make this method public
	// once enabled
	...
	/**
	 * Sorts large data set by given `Comparator`. Data are sorted with
	 * in-memory cache and temporary files.
	 * 
	 * @param source
	 *            iterator over unsorted data
	 * @param mergeDuplicates
	 *            should be duplicate keys merged into single one?
	 * @param batchSize
	 *            how much items can fit into heap memory
	 * @param comparator
	 *            used to sort data
	 * @param serializer
	 *            used to store data in temporary files
	 * @return iterator over sorted data set
	 */
	public static <E> Iterator<E> sort(Iterator<E> source,
			boolean mergeDuplicates, final int batchSize,
		...
		try {
			...
			final DataInputStream[] ins = new DataInputStream[presortFiles
					.size()];
			for (int i = 0; i < presortFiles.size(); i++) {
				ins[i] = new DataInputStream(new BufferedInputStream(
						new FileInputStream(presortFiles.get(i))));
				final int pos = i;
			...
			// and finally sort presorted iterators and return iterators over
			// them
	 ...
	 * @param mergeDuplicates
	 *            if duplicate keys should be merged into single one
	 * @param iterators
	 *            array of already sorted iterators
	 * @return sorted iterator
	 */
	public static <E> Iterator<E> sort(Comparator comparator,
			final boolean mergeDuplicates, final Iterator... iterators) {
		final Comparator comparator2 = comparator == null ? Fun.COMPARATOR
				: comparator;
		return new Iterator<E>() {

			final NavigableSet<Object[]> items = new TreeSet<Object[]>(
					new Fun.ArrayComparator(new Comparator[] { comparator2,
							Fun.COMPARATOR }));

			Object next = this; // is initialized with this so first `next()`
			...
			public E next() {
				...
				if (oldNext != this && comparator2.compare(oldNext, next) > 0) {
					throw new IllegalArgumentException(
							"One of the iterators is not sorted");
				}
				...
				if (mergeDuplicates) {
					while (true) {
						Iterator<Object[]> subset = Fun.filter(items, next)
								.iterator();
						if (!subset.hasNext())
							break;
	...
	/**
	 * Merges multiple iterators into single iterator. Result iterator will
	 * return entries from all iterators. It does not do sorting or any other
	 * special functionality. Does not allow null elements.
	 * 
	 * @param iters
	 *            - iterators to be merged
	...
	/**
	 * Build BTreeMap (or TreeSet) from presorted data. This method is much
	 * faster than usual import using `Map.put(key,value)` method. It is because
	 * tree integrity does not have to be maintained and tree can be created in
	 * linear way with.
	 * 
	 * This method expect data to be presorted in **reverse order** (highest to
	 * lowest). There are technical reason for this requirement. To sort
	 * unordered data use
	 * {@link Pump#sort(java.util.Iterator, boolean, int, java.util.Comparator, Serializer)}
	 * 
	 * This method does not call commit. You should disable Write Ahead Log when
	 * this method is used {@link org.mapdb.DBMaker#transactionDisable()}
	 * 
	 * 
	 * @param source
	 *            iterator over source data, must be reverse sorted
	 * @param keyExtractor
	 *            transforms items from source iterator into keys. If null
	 *            source items will be used directly as keys.
	 * @param valueExtractor
	 *            transforms items from source iterator into values. If null
	 *            BTreeMap will be constructed without values (as Set)
	 * @param ignoreDuplicates
	 *            should be duplicate keys merged into single one?
	 * @param nodeSize
	 *            maximal BTree node size before it is splited.
	 * @param valuesStoredOutsideNodes
	 *            if true values will not be stored as part of BTree nodes
	 * @param counterRecid
	 *            TODO make size counter friendly to use
	 * @param keySerializer
	 *            serializer for keys, use null for default value
	 * @param valueSerializer
	 *            serializer for value, use null for default value
	 * @throws IllegalArgumentException
	 *             if source iterator is not reverse sorted
	 */
	public static <E, K, V> long buildTreeMap(Iterator<E> source,
			Engine engine, Fun.Function1<K, E> keyExtractor,
			Fun.Function1<V, E> valueExtractor, boolean ignoreDuplicates,
			int nodeSize, boolean valuesStoredOutsideNodes, long counterRecid,
			BTreeKeySerializer keySerializer, Serializer<V> valueSerializer) {

		final double NODE_LOAD = 0.75;

		Serializer<BTreeMap.BNode> nodeSerializer = new BTreeMap.NodeSerializer(
				valuesStoredOutsideNodes, keySerializer, valueSerializer, 0);
		...
		while (source.hasNext()) {
			...
			nodeLoop: for (int i = 0; i < nload && source.hasNext(); i++) {
				...
				if (next == null)
					throw new NullPointerException(
							"source returned null element");
				K key = keyExtractor == null ? (K) next : keyExtractor
						.run(next);
				int compared = oldKey == null ? -1 : keySerializer.comparator()
						.compare(key, oldKey);
				while (ignoreDuplicates && compared == 0) {
					...
					if (next == null)
						throw new NullPointerException(
								"source returned null element");
					key = keyExtractor == null ? (K) next : keyExtractor
							.run(next);
				...
				if (oldKey != null && compared >= 0)
					throw new IllegalArgumentException(
							"Keys in 'source' iterator are not reverse sorted");
				oldKey = key;
				keys.add(key);
				counter++;

				Object val = valueExtractor != null ? valueExtractor.run(next)
						: BTreeMap.EMPTY;
				if (val == null)
					throw new NullPointerException(
							"extractValue returned null value");
			...
			BTreeMap.LeafNode node = new BTreeMap.LeafNode(
					keySerializer.arrayToKeys(keys.toArray()), leftEdge,
					rightEdge, false, values.toArray(), nextNode);
			...
			for (int i = 0; i < dirKeys.size(); i++) {
				...
				// put node into store
				boolean rightEdge2 = dirKeys.get(i).get(
						dirKeys.get(i).size() - 1) == null;
				if (rightEdge2) {
		...
		for (int i = 0; i < dirKeys.size() - 1; i++) {
			...
			if (keys2.size() > 2 && keys2.get(0) == null
					&& keys2.get(1) == null) {
				keys2.remove(0);
			...
			BTreeMap.DirNode dir = new BTreeMap.DirNode(
					keySerializer.arrayToKeys(keys2.toArray()), leftEdge3,
					rightEdge3, false, toLongArray(dirRecids.get(i)));
			long dirRecid = engine.put(dir, nodeSerializer);
		...
		BTreeMap.DirNode dir = new BTreeMap.DirNode(
				...
				leftEdge4, rightEdge4, false, toLongArray(dirRecids.get(len)));
	...
	private static <E> Iterator<E> arrayIterator(final Object[] array,
	...
	public static <K, V, A> void fillHTreeMap(final HTreeMap<K, V> m,
			Iterator<A> pumpSource, final Fun.Function1<K, A> pumpKeyExtractor,
			Fun.Function1<V, A> pumpValueExtractor, int pumpPresortBatchSize,
			boolean pumpIgnoreDuplicates, Serializer<A> sortSerializer) {
		...
		pumpSource = sort(pumpSource, false, pumpPresortBatchSize,
				hashComparator, sortSerializer);

		// got sorted, now fill the map
		while (pumpSource.hasNext()) {
			A o = pumpSource.next();
			K key = pumpKeyExtractor.run(o);
			V val = pumpValueExtractor == null ? (V) BTreeMap.EMPTY
					: pumpValueExtractor.run(o);
			if (pumpIgnoreDuplicates) {
				m.put(key, val);
			} else {
				Object old = m.putIfAbsent(key, val);
				if (old != null)
					throw new IllegalArgumentException("Duplicate at: "
							+ o.toString());
			}
		}
