...
/**
 * Custom serializer for BTreeMap keys which enables [Delta
 * encoding](https://en.wikipedia.org/wiki/Delta_encoding).
 * 
 * Keys in BTree Nodes are sorted, this enables number of tricks to save disk
 * space. For example for numbers we may store only difference between
 * subsequent numbers, for string we can only take suffix, etc...
 * 
 * @param <KEY>
 *            type of key
 * @param <KEYS>
 *            type of object which holds multiple keys (
 */
public abstract class BTreeKeySerializer<KEY, KEYS> {
	 ...
	 * 
	 * @param out
	 *            output stream where to put ata
	 * @param keys
	 *            An object which represents keys
	 * 
	 * @throws IOException
	 */
	public abstract void serialize(DataOutput out, KEYS keys)
			throws IOException;
	 ...
	 * @throws IOException
	 */
	public abstract KEYS deserialize(DataInput in, int nodeSize)
			throws IOException;
	 ...
	 * key. If all items are smaller it returns {@code keyser.length(keys)}
	 */
	public int findChildren(final BTreeMap.BNode node, final Object key) {
		...
		for (;;) {
			...
			middle = (left + right) / 2;
			if (middle == keylen)
				return middle + node.leftEdgeInc(); // null is positive
													// infinitive
	...
	public int findChildren2(final BTreeMap.BNode node, final Object key) {
		...
		while (true) {
			...
			middle = (left + right) / 2;
			if (middle == keylen)
				return -1 - (middle + node.leftEdgeInc()); // null is positive
															// infinitive
			comp = compare(keys, middle, (KEY) key);
			if (comp == 0) {
				// try one before last, in some cases it might be duplicate of
				// last
	...
	/**
	 * Basic Key Serializer which just writes data without applying any
	 * compression. Is used by default if no other Key Serializer is specified.
	 */
	public static final class BasicKeySerializer extends
			BTreeKeySerializer<Object, Object[]> implements Serializable {
		...
		BasicKeySerializer(SerializerBase serializerBase, DataInput is,
			...
			serializer = (Serializer) serializerBase.deserialize(is,
					objectStack);
			comparator = (Comparator) serializerBase.deserialize(is,
					objectStack);
			if (serializer == null || comparator == null)
				throw new NullPointerException();
		}
		...
		@Override
		public Object[] deserialize(DataInput in, int nodeSize)
				throws IOException {
			// $DELAY$
	...
	public static final BTreeKeySerializer LONG = new BTreeKeySerializer<Long, long[]>() {
		...
		@Override
		public long[] deserialize(DataInput in, int nodeSize)
				throws IOException {
		...
		@Override
		public final int findChildren(final BTreeMap.BNode node,
				final Object key) {
		...
		@Override
		public final int findChildren2(final BTreeMap.BNode node,
				final Object key) {
			long[] keys = (long[]) node.keys;
			long key2 = (Long) key;
			...
			while (true) {
				...
				middle = (left + right) / 2;
				if (middle == keys.length)
					return -1 - (middle + node.leftEdgeInc()); // null is
																// positive
																// infinitive

				if (keys[middle] == key2) {
					// try one before last, in some cases it might be duplicate
					// of last
					if (!node.isRightEdge() && middle == keys.length - 1
							&& middle > 0 && keys[middle - 1] == key2) {
						middle--;
					}
					return middle + node.leftEdgeInc();
	...
	public static final BTreeKeySerializer ARRAY4 = new ArrayKeySerializer(
			new Comparator[] { Fun.COMPARATOR, Fun.COMPARATOR, Fun.COMPARATOR,
					Fun.COMPARATOR }, new Serializer[] { Serializer.BASIC,
					Serializer.BASIC, Serializer.BASIC, Serializer.BASIC });

	public final static class ArrayKeySerializer extends
			BTreeKeySerializer<Object[], Object[]> implements Serializable {
		...
		public ArrayKeySerializer(Comparator[] comparators,
		...
		public ArrayKeySerializer(SerializerBase serializerBase, DataInput is,
			...
			for (int i = 0; i < tsize; i++) {
				comparators[i] = (Comparator) serializerBase.deserialize(is,
						objectStack);
			}
			serializers = new Serializer[tsize];
			for (int i = 0; i < tsize; i++) {
				serializers[i] = (Serializer) serializerBase.deserialize(is,
						objectStack);
			}
		...
		public void serialize(DataOutput out, Object[] keys) throws IOException {
			...
			for (int i = 0; i < keys.length; i += tsize) {
				for (int j = 0; j < tsize - 1; j++) {
					...
					if (counts[j] == 0) {
						...
						while (i + j + counts[j] * tsize < keys.length
								&& comparators[j].compare(orig, keys[i + j
										+ counts[j] * tsize]) == 0) {
							counts[j]++;
						}
				...
				serializers[serializers.length - 1].serialize(out, keys[i
						+ tsize - 1]);
				// decrement all
				// $DELAY$
	...
	public static final BTreeKeySerializer<java.util.UUID, long[]> UUID = new BTreeKeySerializer<java.util.UUID, long[]>() {
		...
		public int compare(long[] longs, int pos, java.util.UUID uuid) {
			...
			// $DELAY$
			int r = Fun
					.compareLong(longs[pos++], uuid.getMostSignificantBits());
			if (r != 0)
				return r;
	...
	// TODO right now byte[] contains 7 bit characters, but it should be
	// expandable to 8bit.
	public static final class ByteArrayKeys implements StringArrayKeys {
		...
		ByteArrayKeys(int[] offset, byte[] array) {
			...
			if (CC.PARANOID
					&& !(array.length == 0 || array.length == offset[offset.length - 1]))
				throw new AssertionError();
		}

		ByteArrayKeys(DataInput in, int[] offsets, int prefixLen)
				throws IOException {
			this.offset = offsets;
		...
		public ByteArrayKeys putKey(int pos, byte[] newKey) {
			...
			System.arraycopy(array, split1, bb, split1 + newKey.length,
					array.length - split1);
	...
	public static final class CharArrayKeys implements StringArrayKeys {
		...
		public CharArrayKeys(DataInput in, int[] offsets, int prefixLen)
				throws IOException {
			this.offset = offsets;
		...
		private void inReadFully(DataInput in, int from, int to)
				throws IOException {
		...
		public CharArrayKeys putKey(int pos, String newKey) {
			...
			System.arraycopy(array, split1, bb, split1 + strLen, array.length
					- split1);
		...
		public static StringArrayKeys putKey(ByteArrayKeys kk, int pos,
				String newKey) {
			int strLen = newKey.length();
		...
		private void outWrite(DataOutput out, int from, int to)
				throws IOException {
			for (int i = from; i < to; i++) {
	...
	public static final BTreeKeySerializer<String, char[][]> STRING2 = new BTreeKeySerializer<String, char[][]>() {
		...
		@Override
		public void serialize(DataOutput out, char[][] chars)
				throws IOException {
		...
		@Override
		public char[][] deserialize(DataInput in, int nodeSize)
				throws IOException {
		...
		/**
		 * compares two char arrays, has same contract as
		...
		/**
		 * compares char array and string, has same contract as
		 * {@link String#compareTo(String)}
		 */
		...
		public char[][] putKey(char[][] keys, int pos, String newKey) {
			return (char[][]) BTreeMap
					.arrayPut(keys, pos, newKey.toCharArray());
		}
	...
	public static final BTreeKeySerializer<String, StringArrayKeys> STRING = new BTreeKeySerializer<String, StringArrayKeys>() {
		@Override
		public void serialize(DataOutput out, StringArrayKeys keys2)
				throws IOException {
		...
		@Override
		public StringArrayKeys deserialize(DataInput in, int nodeSize)
				throws IOException {
			// read data sizes
			...
			prefixLen >>>= 1;
			// $DELAY$
			return useUnicode ? new CharArrayKeys(in, offsets, prefixLen)
		...
		@Override
		public int compare(StringArrayKeys byteArrayKeys, int pos1,
				String string) {
		...
		@Override
		public StringArrayKeys putKey(StringArrayKeys byteArrayKeys, int pos,
				String string) {
		...
		public StringArrayKeys arrayToKeys(Object[] keys) {
			...
			return unicode ? new CharArrayKeys(offsets, keys)
					: new ByteArrayKeys(offsets, keys);
		}

		@Override
		public StringArrayKeys copyOfRange(StringArrayKeys byteArrayKeys,
				int from, int to) {
	...
	public static final BTreeKeySerializer<byte[], byte[][]> BYTE_ARRAY2 = new BTreeKeySerializer<byte[], byte[][]>() {
		...
		@Override
		public void serialize(DataOutput out, byte[][] chars)
				throws IOException {
			// write lengths
			for (byte[] b : chars) {
				DataIO.packInt(out, b.length);
		...
		@Override
		public byte[][] deserialize(DataInput in, int nodeSize)
				throws IOException {
	...
	public static final BTreeKeySerializer<byte[], ByteArrayKeys> BYTE_ARRAY = new BTreeKeySerializer<byte[], ByteArrayKeys>() {
		@Override
		public void serialize(DataOutput out, ByteArrayKeys keys)
				throws IOException {
			int offset = 0;
			// write sizes
			for (int o : keys.offset) {
		...
		@Override
		public ByteArrayKeys deserialize(DataInput in, int nodeSize)
				throws IOException {
			// read data sizes
			int[] offsets = new int[nodeSize];
			int old = 0;
		...
		@Override
		public ByteArrayKeys putKey(ByteArrayKeys byteArrayKeys, int pos,
				byte[] newKey) {
		...
		@Override
		public ByteArrayKeys copyOfRange(ByteArrayKeys byteArrayKeys, int from,
				int to) {
			return byteArrayKeys.copyOfRange(from, to);
		}
	...
	public static class Compress extends BTreeKeySerializer {
		...
		protected Compress(SerializerBase serializerBase, DataInput in,
				SerializerBase.FastArrayList objectStack) throws IOException {
			objectStack.add(this);
			wrapped = (BTreeKeySerializer) serializerBase.deserialize(in,
					objectStack);
			if (wrapped == null)
				throw new NullPointerException();
		}
		...
		public Object deserialize(DataInput in, int nodeSize)
			...
			return wrapped.deserialize(new DataIO.DataInputByteArray(in2),
					nodeSize);
		}
