 ...
 * <p>
 * This map uses full 32bit hash from beginning, There is no initial load factor
 * and rehash. Technically it is not hash table, but hash tree with nodes
 * expanding when they become full.
 * <p>
 * This map is suitable for number of records 1e9 and over. Larger number of
 * records will increase hash collisions and performance will degrade linearly
 * with number of records (separate chaining).
 * <p>
 * Concurrent scalability is achieved by splitting HashMap into 16 segments,
 * each with separate lock. Very similar to
 * {@link java.util.concurrent.ConcurrentHashMap}
 * 
 * @author Jan Kotek
 */
@SuppressWarnings({ "unchecked", "rawtypes" })
public class HTreeMap<K, V> extends AbstractMap<K, V> implements
		ConcurrentMap<K, V>, Bind.MapWithModificationListener<K, V>, Closeable {

	protected static final Logger LOG = Logger.getLogger(HTreeMap.class
			.getName());
	...
	/**
	 * Salt added to hash before rehashing, so it is harder to trigger hash
	 * collision attack.
	 */
	...
	protected static final class LinkedNode<K, V> {
		...
		public LinkedNode(final long next, long expireLinkNodeRecid,
	...
	protected final Serializer<LinkedNode<K, V>> LN_SERIALIZER = new Serializer<LinkedNode<K, V>>() {
		...
		/**
		 * used to check that every 64000 th element has consistent has befor
		...
		public LinkedNode<K, V> deserialize(DataInput in, int available)
			...
			return new LinkedNode<K, V>(DataIO.unpackLong(in),
					expireFlag ? DataIO.unpackLong(in) : 0L,
					keySerializer.deserialize(in, -1),
					hasValues ? valueSerializer.deserialize(in, -1)
							: (V) BTreeMap.EMPTY);
		}
	...
	private final void assertHashConsistent(K key) throws IOException {
		...
		if (out.pos != in.pos) {
			throw new IllegalArgumentException(
					"Key has inconsistent serialization length. Class: "
							+ key.getClass());
		}
	}
	 ...
	 * Opens HTreeMap
	 */
	public HTreeMap(Engine engine, long counterRecid, int hashSalt,
			long[] segmentRecids, Serializer<K> keySerializer,
			Serializer<V> valueSerializer, long expireTimeStart, long expire,
			long expireAccess, long expireMaxSize, long expireStoreSize,
			long[] expireHeads, long[] expireTails,
			Fun.Function1<V, K> valueCreator, Fun.ThreadFactory threadFactory) {
		if (counterRecid < 0)
			throw new IllegalArgumentException();
		if (engine == null)
		...
		if (expireMaxSize != 0 && counterRecid == 0) {
			throw new IllegalArgumentException(
					"expireMaxSize must have counter enabled");
		}

		this.expireFlag = expire != 0L || expireAccess != 0L
				|| expireMaxSize != 0 || expireStoreSize != 0;
		this.expire = expire;
		this.expireTimeStart = expireTimeStart;
		this.expireAccessFlag = expireAccess != 0L || expireMaxSize != 0
				|| expireStoreSize != 0;
		this.expireAccess = expireAccess;
		this.expireHeads = expireHeads == null ? null : Arrays.copyOf(
				expireHeads, 16);
		this.expireTails = expireTails == null ? null : Arrays.copyOf(
				expireTails, 16);
		...
		if (expireFlag) {
			...
			threadFactory.newThread("HTreeMap expirator", new ExpireRunnable(
					this));
		} else {
	...
	public V get(final Object o) {
		...
		final Lock lock = expireAccessFlag ? segmentLocks[segment].writeLock()
				: segmentLocks[segment].readLock();
		lock.lock();
		LinkedNode<K, V> ln;
		try {
		...
		// there is race condition, vc could be called twice. But map will be
		// updated only once
	...
	/**
	 * Return given value, without updating cache statistics if `expireAccess()`
	 * is true It also does not use `valueCreator` if value is not found (always
	...
	protected LinkedNode<K, V> getInner(Object o, int h, int segment) {
		...
		for (int level = 3; level >= 0; level--) {
			...
			if (recid == 0)
				return null;
			if ((recid & 1) != 0) { // last bite indicates if referenced record
									// is LinkedNode
				recid = recid >>> 1;
				while (true) {
			...
			if (recid != 0) {
				...
				while (ln != null) {
					if (keySerializer.equals(ln.key, key)) {
						...
						V oldVal = ln.value;
						ln = new LinkedNode<K, V>(ln.next,
								ln.expireLinkNodeRecid, ln.key, value);
						engine.update(recid, ln, LN_SERIALIZER);
						if (expireFlag)
							expireLinkBump(segment, ln.expireLinkNodeRecid,
									false);
				...
				// key was not found at linked list, so just append it to
				// beginning
			}

			// check if linked list has overflow and needs to be expanded to new
			// dir level
			if (counter >= BUCKET_OVERFLOW && level >= 1) {
				long[][] nextDir = new long[16][];

				{
					final long expireNodeRecid = expireFlag ? engine
							.preallocate() : 0L;
					final LinkedNode<K, V> node = new LinkedNode<K, V>(0,
							expireNodeRecid, key, value);
				...
				while (nodeRecid != 0) {
					...
					if (nextDir[posDiv8] == null)
						nextDir[posDiv8] = new long[8];
					n = new LinkedNode<K, V>(nextDir[posDiv8][posMod8] >>> 1,
							n.expireLinkNodeRecid, n.key, n.value);
				...
				final long expireNodeRecid = expireFlag ? engine.put(
						ExpireLinkNode.EMPTY, ExpireLinkNode.SERIALIZER) : 0L;

				final long newRecid = engine.put(new LinkedNode<K, V>(recid,
						expireNodeRecid, key, value), LN_SERIALIZER);
				dir = Arrays.copyOf(dir, 16);
	...
	protected V removeInternal(Object key, int segment, int h,
				...
				while (ln != null) {
					if (keySerializer.equals(ln.key, (K) key)) {
						...
						if (prevLn == null) {
							...
							if (ln.next == 0) {
								recursiveDirDelete(h, level, dirRecids, dir,
										slot);
								...
								engine.update(dirRecids[level], dir,
										DIR_SERIALIZER);
							}

						} else {
							// referenced from LinkedNode
							prevLn = new LinkedNode<K, V>(ln.next,
									prevLn.expireLinkNodeRecid, prevLn.key,
									prevLn.value);
	...
	private void recursiveDirDelete(int h, int level, long[] dirRecids,
			long[][] dir, int slot) {
		...
		if (allZero) {
			...
			if (level == 3) {
				// parent is segment, recid of this dir can not be modified, so
				// just update to null
				engine.update(dirRecids[level], new long[16][], DIR_SERIALIZER);
			} else {
				engine.delete(dirRecids[level], DIR_SERIALIZER);

				final long[][] parentDir = engine.get(dirRecids[level + 1],
						DIR_SERIALIZER);
				final int parentPos = (h >>> (7 * (level + 1))) & 0x7F;
				recursiveDirDelete(h, level + 1, dirRecids, parentDir,
						parentPos);
	...
	public boolean containsValue(Object value) {
		for (V v : values()) {
			if (valueSerializer.equals(v, (V) value))
				return true;
		}
		return false;
	}
	...
	protected class KeySet extends AbstractSet<K> {
		...
		public boolean remove(Object o) {
			...
			// Entry e = (Entry) o;
			// return HTreeMap.this.remove(((Entry) o).getKey(),((Entry)
			// o).getValue());
			// }
	...
	private final Set<Entry<K, V>> _entrySet = new AbstractSet<Entry<K, V>>() {
		...
		public boolean contains(Object o) {
			if (o instanceof Entry) {
				...
				return val != null
						&& valueSerializer.equals((V) val, (V) e.getValue());
			} else
				return false;
		}
	...
	abstract class HashIterator {
		...
		public boolean hasNext() {
			return currentLinkedList != null
					&& currentLinkedListPos < currentLinkedList.length;
		}
		...
		private LinkedNode[] advance(int lastHash) {
			...
			try {
				...
				while (true) {
					...
					final int posDiv8 = pos >>> DIV8;
					final int posMod8 = pos & MOD8;
					if (dir[posDiv8] == null || dir[posDiv8][posMod8] == 0
							|| (dir[posDiv8][posMod8] & 1) == 1) {
						// increase hash by 1
						if (level != 0) {
							lastHash = ((lastHash >>> (7 * level)) + 1) << (7 * level); // should
																						// use
																						// mask
																						// and
																						// XOR
		...
		private LinkedNode[] findNextLinkedNode(int hash) {
			...
			for (int segment = Math.max(hash >>> 28, lastSegment); segment < 16; segment++) {
				final Lock lock = expireAccessFlag ? segmentLocks[segment]
						.writeLock() : segmentLocks[segment].readLock();
				lock.lock();
				try {
					lastSegment = Math.max(segment, lastSegment);
					long dirRecid = segmentRecids[segment];
					LinkedNode ret[] = findNextLinkedNodeRecur(dirRecid, hash,
							3);
					if (CC.PARANOID && ret != null)
		...
		private LinkedNode[] findNextLinkedNodeRecur(long dirRecid,
				int newHash, int level) {
			long[][] dir = engine.get(dirRecid, DIR_SERIALIZER);
			if (dir == null)
				return null;
				...
				if (dir[pos >>> DIV8] != null) {
					...
					if (recid != 0) {
						if ((recid & 1) == 1) {
							...
							int arrayPos = 0;
							while (recid != 0) {
								LinkedNode ln = engine
										.get(recid, LN_SERIALIZER);
								if (ln == null) {
									recid = 0;
									continue;
								}
							...
							recid = recid >> 1;
							LinkedNode[] ret = findNextLinkedNodeRecur(recid,
									first ? newHash : 0, level - 1);
							if (ret != null)
								return ret;
						}
					}
	...
	class Entry2 implements Entry<K, V> {
		...
		public boolean equals(Object o) {
			return (o instanceof Entry)
					&& keySerializer.equals(key, (K) ((Entry) o).getKey());
		}
	...
	public boolean remove(Object key, Object value) {
		...
		try {
			...
			if (otherVal != null
					&& valueSerializer.equals((V) otherVal.value, (V) value)) {
	...
	protected static final class ExpireLinkNode {
		...
		public final static ExpireLinkNode EMPTY = new ExpireLinkNode(0, 0, 0,
				0, 0);
		...
		public static final Serializer<ExpireLinkNode> SERIALIZER = new Serializer<ExpireLinkNode>() {
			...
			public ExpireLinkNode deserialize(DataInput in, int available)
				...
				if (available == 0)
					return EMPTY;
				return new ExpireLinkNode(DataIO.unpackLong(in),
						DataIO.unpackLong(in), DataIO.unpackLong(in),
						DataIO.unpackLong(in), in.readInt());
			}
		...
		public ExpireLinkNode(long prev, long next, long keyRecid, long time,
				int hash) {
			this.prev = prev;
			this.next = next;
	...
	protected void expireLinkAdd(int segment, long expireNodeRecid,
			long keyRecid, int hash) {
		if (CC.PARANOID
				&& !(segmentLocks[segment].writeLock().isHeldByCurrentThread()))
			throw new AssertionError();
		...
		long time = expire == 0 ? 0 : expire + System.currentTimeMillis()
				- expireTimeStart;
		...
		if (head == 0) {
			...
			engine.update(expireHeads[segment], expireNodeRecid,
					Serializer.LONG);
			engine.update(expireTails[segment], expireNodeRecid,
					Serializer.LONG);
		} else {
			// insert new head
			...
			ExpireLinkNode oldHead = engine
					.get(head, ExpireLinkNode.SERIALIZER);
	...
	protected void expireLinkBump(int segment, long nodeRecid, boolean access) {
		...
		long newTime = access ? (expireAccess == 0 ? 0 : expireAccess
				+ System.currentTimeMillis() - expireTimeStart)
				: (expire == 0 ? 0 : expire + System.currentTimeMillis()
						- expireTimeStart);

		// TODO optimize bellow, but what if there is only size limit?
		// if(n.time>newTime) return; // older time greater than new one, do not
		// update
		...
		if (n.next == 0) {
			...
			if (n.prev != 0) {
				// not a tail
				ExpireLinkNode prev = engine.get(n.prev,
						ExpireLinkNode.SERIALIZER);
			...
			long oldHeadRecid = engine.get(expireHeads[segment],
					Serializer.LONG);
			ExpireLinkNode oldHead = engine.get(oldHeadRecid,
					ExpireLinkNode.SERIALIZER);
		...
		public void run() {
			...
			try {
				while (true) {
					...
					pause = ((!map.expireMaxSizeFlag || map.size() < map.expireMaxSize) && (map.expireStoreSize == 0L || Store
							.forEngine(map.engine).getCurrSize()
							- Store.forEngine(map.engine).getFreeSize() < map.expireStoreSize));
	...
	protected void expirePurge() {
		...
		if (expireStoreSize != 0 && removePerSegment == 0) {
			...
			if (expireStoreSize < storeSize) {
				...
				if (LOG.isLoggable(Level.FINE)) {
					LOG.log(Level.FINE,
							"HTreeMap store store size ({0,number,integer}) over limit,"
		...
		if (LOG.isLoggable(Level.FINE)) {
			LOG.log(Level.FINE,
					"HTreeMap expirator removed {0,number,integer}", counter);
		}
		...
		try {
			...
			while (recid != 0) {
				...
				final boolean remove = ++counter < removePerSegment
						|| ((expire != 0 || expireAccess != 0) && n.time
								+ expireTimeStart < System.currentTimeMillis());
	...
	protected void expireCheckSegment(int segment) {
		...
		long prev = 0;
		while (current != 0) {
			ExpireLinkNode curr = engine
					.get(current, ExpireLinkNode.SERIALIZER);
			if (CC.PARANOID && !(curr.prev == prev))
				throw new AssertionError("wrong prev " + curr.prev + " - "
						+ prev);
			prev = current;
			current = curr.next;
		}
	...
	public Map<K, V> snapshot() {
		...
		return new HTreeMap<K, V>(snapshot,
				counter == null ? 0 : counter.recid, hashSalt, segmentRecids,
				keySerializer, valueSerializer, 0L, 0L, 0L, 0L, 0L, null, null,
				null, null);
	}
