...
public final class Queues {
	...
	public static abstract class SimpleQueue<E> implements BlockingQueue<E> {
		...
		protected static class NodeSerializer<E> extends Serializer<Node<E>> {
			...
			@Override
			public Node<E> deserialize(DataInput in, int available)
					throws IOException {
				return new Node<E>(DataIO.unpackLong(in),
		...
		public SimpleQueue(Engine engine, Serializer<E> serializer,
				long headRecidRef) {
			this.engine = engine;
			this.serializer = serializer;
		...
		/**
		 * Closes underlying storage and releases all resources. Used mostly
		 * with temporary collections where engine is not accessible.
		 */
		public void close() {
			engine.close();
		}
	...
	public static class Stack<E> extends SimpleQueue<E> {
		...
		public Stack(Engine engine, Serializer<E> serializer,
				long headerRecidRef) {
	...
	public static class CircularQueue<E> extends SimpleQueue<E> {
		...
		public CircularQueue(Engine engine, Serializer<E> serializer,
				long headRecid, long headInsertRecid, long size) {
