...
public class EngineWrapper implements Engine {
	...
	protected static final Logger LOG = !CC.LOG_EWRAP ? null : Logger
			.getLogger(EngineWrapper.class.getName());
	...
	public <A> boolean compareAndSwap(long recid, A expectedOldValue,
		...
		return getWrappedEngine().compareAndSwap(recid, expectedOldValue,
				newValue, serializer);
	}
	 ...
	 * <code>UnsupportedOperationException("Read-only")</code> on any
	...
	/**
	 * check if Record Instances were not modified while in cache. Usuful to
	 * diagnose strange problems with Instance Cache.
	 */
	public static class ImmutabilityCheckEngine extends EngineWrapper {
		...
		public <A> boolean compareAndSwap(long recid, A expectedOldValue,
			...
			if (item != null)
				item.check();
			boolean ret = super.compareAndSwap(recid, expectedOldValue,
					newValue, serializer);
	...
	/**
	 * Engine wrapper with all methods synchronized on global lock, useful to
	...
	public static class SynchronizedEngineWrapper extends EngineWrapper {
		...
		@Override
		synchronized public <A> void update(long recid, A value,
		...
		synchronized public <A> boolean compareAndSwap(long recid,
				A expectedOldValue, A newValue, Serializer<A> serializer) {
	...
	/**
	 * Checks that Serializer used to serialize item is the same as Serializer
	...
	public static class SerializerCheckEngineWrapper extends EngineWrapper {
		...
		synchronized protected <A> void checkSerializer(long recid,
				Serializer<A> serializer) {
			Serializer other = recid2serializer.get(recid);
			if (other != null) {
				if (other != serializer
						&& other.getClass() != serializer.getClass())
					throw new IllegalArgumentException(
							"Serializer does not match. \n found: "
									+ serializer + " \n expected: " + other);
			} else
	 ...
	 * {@link Runtime#addShutdownHook(Thread)} If engine was closed by user
	 * before JVM shutdown, hook is removed to save memory.
	 */
	public static class CloseOnJVMShutdown extends EngineWrapper {

		final protected AtomicBoolean shutdownHappened = new AtomicBoolean(
				false);
