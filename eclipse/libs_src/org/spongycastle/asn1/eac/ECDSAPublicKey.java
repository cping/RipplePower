...
public class ECDSAPublicKey extends PublicKeyDataObject {
	...
	ECDSAPublicKey(ASN1Sequence seq) throws IllegalArgumentException {
		...
		while (en.hasMoreElements()) {
			...
			if (obj instanceof ASN1TaggedObject) {
				...
				case 0x4:
					...
					break;
				case 0x5:
					setOrderOfBasePointR(UnsignedInteger.getInstance(to)
							.getValue());
					break;
				case 0x6:
		...
		if (options != 0x20 && options != 0x7F) {
			throw new IllegalArgumentException(
					"All options must be either present or absent!");
		}
	}
	...
	public ECDSAPublicKey(ASN1ObjectIdentifier usage, BigInteger p,
			BigInteger a, BigInteger b, byte[] basePoint, BigInteger order,
	...
	private void setOrderOfBasePointR(BigInteger orderOfBasePointR)
		...
		if ((options & R) == 0) {
			...
			throw new IllegalArgumentException(
					"Order of base point R already set");
		}
	}
	...
	public ASN1EncodableVector getASN1EncodableVector(ASN1ObjectIdentifier oid,
		...
		if (!publicPointOnly) {
			...
			v.add(new DERTaggedObject(false, 0x04, new DEROctetString(
					getBasePointG())));
			v.add(new UnsignedInteger(0x05, getOrderOfBasePointR()));
		}
		v.add(new DERTaggedObject(false, 0x06, new DEROctetString(
				getPublicPointY())));
		if (!publicPointOnly) {
