...
public class DERApplicationSpecific extends ASN1Primitive {
	...
	public DERApplicationSpecific(boolean explicit, int tag,
			ASN1Encodable object) throws IOException {
		ASN1Primitive primitive = object.toASN1Primitive();

		byte[] data = primitive.getEncoded(ASN1Encoding.DER);

		this.isConstructed = explicit
				|| (primitive instanceof ASN1Set || primitive instanceof ASN1Sequence);
		this.tag = tag;
	...
	public DERApplicationSpecific(int tagNo, ASN1EncodableVector vec) {
		...
		for (int i = 0; i != vec.size(); i++) {
			try {
				bOut.write(((ASN1Object) vec.get(i))
	...
	public static DERApplicationSpecific getInstance(Object obj) {
		if (obj == null || obj instanceof DERApplicationSpecific) {
			...
			try {
				return DERApplicationSpecific.getInstance(ASN1Primitive
						.fromByteArray((byte[]) obj));
			} catch (IOException e) {
				throw new IllegalArgumentException(
						"failed to construct object from byte[]: "
								+ e.getMessage());
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.spongycastle.asn1.ASN1Primitive#encode(org.spongycastle.asn1.
	 * DEROutputStream)
	 */
