...
public class CertificatePolicies extends ASN1Object {
	...
	/**
	 * Retrieve a CertificatePolicies for a passed in Extensions object, if
	 * present.
	 * 
	 * @param extensions
	...
	public static CertificatePolicies fromExtensions(Extensions extensions) {
		return CertificatePolicies.getInstance(extensions
				.getExtensionParsedValue(Extension.certificatePolicies));
	}
	...
	private CertificatePolicies(ASN1Sequence seq) {
		...
		for (int i = 0; i != seq.size(); i++) {
			policyInformation[i] = PolicyInformation.getInstance(seq
					.getObjectAt(i));
		}
	}
	...
	public PolicyInformation getPolicyInformation(
		...
		for (int i = 0; i != policyInformation.length; i++) {
			if (policyIdentifier.equals(policyInformation[i]
					.getPolicyIdentifier())) {
