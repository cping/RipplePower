...
public class AttributeCertificateInfo extends ASN1Object {
	...
	private AttributeCertificateInfo(ASN1Sequence seq) {
		...
		int start;
		if (seq.getObjectAt(0) instanceof ASN1Integer) // in version 1 certs
														// version is DEFAULT
														// v1(0)
		{
		...
		this.signature = AlgorithmIdentifier.getInstance(seq
				.getObjectAt(start + 2));
		this.serialNumber = ASN1Integer.getInstance(seq.getObjectAt(start + 3));
		this.attrCertValidityPeriod = AttCertValidityPeriod.getInstance(seq
				.getObjectAt(start + 4));
		...
		for (int i = start + 6; i < seq.size(); i++) {
			...
			if (obj instanceof DERBitString) {
				this.issuerUniqueID = DERBitString.getInstance(seq
						.getObjectAt(i));
