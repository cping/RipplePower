...
public class DeclarationOfMajority extends ASN1Object implements ASN1Choice {
	...
	public DeclarationOfMajority(int notYoungerThan) {
		declaration = new DERTaggedObject(false, 0, new ASN1Integer(
				notYoungerThan));
	}
	...
	public DeclarationOfMajority(boolean fullAge, String country) {
		if (country.length() > 2) {
			throw new IllegalArgumentException(
					"country can only be 2 characters");
		}

		if (fullAge) {
			declaration = new DERTaggedObject(false, 1, new DERSequence(
					new DERPrintableString(country, true)));
		} else {
	...
	public int notYoungerThan() {
		...
		return ASN1Integer.getInstance(declaration, false).getValue()
				.intValue();
	}
