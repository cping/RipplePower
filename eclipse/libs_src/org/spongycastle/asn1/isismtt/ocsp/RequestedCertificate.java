...
public class RequestedCertificate extends ASN1Object implements ASN1Choice {
	...
	public static RequestedCertificate getInstance(ASN1TaggedObject obj,
		...
		if (!explicit) {
			throw new IllegalArgumentException(
					"choice item must be explicitly tagged");
		}
	...
	private RequestedCertificate(ASN1TaggedObject tagged) {
		if (tagged.getTagNo() == publicKeyCertificate) {
			publicKeyCert = ASN1OctetString.getInstance(tagged, true)
					.getOctets();
		} else if (tagged.getTagNo() == attributeCertificate) {
			attributeCert = ASN1OctetString.getInstance(tagged, true)
					.getOctets();
		} else {
	...
	public byte[] getCertificateBytes() {
		if (cert != null) {
			try {
				...
				throw new IllegalStateException("can't decode certificate: "
						+ e);
			}
		}
		if (publicKeyCert != null) {
