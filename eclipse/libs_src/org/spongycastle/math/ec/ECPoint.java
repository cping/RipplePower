...
public abstract class ECPoint {
	...
	protected static ECFieldElement[] getInitialZCoords(ECCurve curve) {
		...
		int coord = null == curve ? ECCurve.COORD_AFFINE : curve
				.getCoordinateSystem();
	...
	protected int getCurveCoordinateSystem() {
		// Cope with null curve, most commonly used by implicitlyCa
		return null == curve ? ECCurve.COORD_AFFINE : curve
				.getCoordinateSystem();
	}
	 ...
	 * getAffineXCoord if you expect the point to already have been normalized.
	 * 
	 ...
	 * same value as in an affine coordinate system; use normalize() to get a
	 * point where the coordinates have their affine values, or use
	 * getAffineYCoord if you expect the point to already have been normalized.
	 * 
	...
	public static class Fp extends ECPoint {
		...
		public ECFieldElement getZCoord(int index) {
			if (index == 1
					&& ECCurve.COORD_JACOBIAN_MODIFIED == this
		...
		public ECPoint add(ECPoint b) {
			...
			case ECCurve.COORD_AFFINE: {
				...
				ECFieldElement Y3 = gamma.multiply(X1.subtract(X3))
						.subtract(Y1);
			...
			case ECCurve.COORD_HOMOGENEOUS: {
				...
				ECFieldElement w = Z1IsOne ? Z2 : Z2IsOne ? Z1 : Z1
						.multiply(Z2);
				...
				ECFieldElement Y3 = vSquaredV2.subtract(A).multiply(u)
						.subtract(vCubed.multiply(u2));
		...
		public ECPoint twice() {
			...
			case ECCurve.COORD_HOMOGENEOUS: {
				...
				ECFieldElement Y3 = w.multiply(_4B.subtract(h)).subtract(
						two(two(t).square()));
			...
			case ECCurve.COORD_JACOBIAN: {
				...
				if (a4Neg.toBigInteger().equals(BigInteger.valueOf(3))) {
					M = three(X1.add(Z1Squared)
							.multiply(X1.subtract(Z1Squared)));
					...
					S = two(doubleProductFromSquares(X1, Y1Squared, X1Squared,
							T));
				}

				ECFieldElement X3 = M.square().subtract(two(S));
				ECFieldElement Y3 = S.subtract(X3).multiply(M)
						.subtract(eight(T));
		...
		public ECPoint twicePlus(ECPoint b) {
			...
			case ECCurve.COORD_AFFINE: {
				...
				ECFieldElement L2 = two(Y1).multiply(X).multiply(dx)
						.multiply(I).subtract(L1);
		...
		protected ECFieldElement doubleProductFromSquares(ECFieldElement a,
				ECFieldElement b, ECFieldElement aSquared,
				ECFieldElement bSquared) {
			/*
			 * NOTE: If squaring in the field is faster than multiplication,
			 * then this is a quicker way to calculate 2.A.B, if A^2 and B^2 are
			 * already known.
			 */
		...
		protected ECPoint.Fp twiceJacobianModified(boolean calculateW) {
			...
			ECFieldElement Y1Squared = Y1.square();
			ECFieldElement T = Y1Squared.square();
			ECFieldElement S = two(doubleProductFromSquares(X1, Y1Squared,
					X1Squared, T));
			ECFieldElement X3 = M.square().subtract(two(S));
	...
	public static class F2m extends ECPoint {
		 ...
		 * 
		 * @see
		 * org.spongycastle.math.ec.ECPoint#add(org.spongycastle.math.ec.ECPoint
		 * )
		 */
		public ECPoint add(ECPoint b) {
			checkPoints(this, b);
		...
		public ECPoint.F2m addSimple(ECPoint.F2m b) {
			...
			case ECCurve.COORD_AFFINE: {
				...
				ECFieldElement X3 = L.square().add(L).add(sumX)
						.add(curve.getA());
			...
			case ECCurve.COORD_HOMOGENEOUS: {
				...
				ECFieldElement A = U.square()
						.add(U.multiply(V).add(VSq.multiply(curve.getA())))
						.multiply(W).add(V.multiply(VSq));

				ECFieldElement X3 = V.multiply(A);
				ECFieldElement VSqZ2 = Z2IsOne ? VSq : VSq.multiply(Z2);
				ECFieldElement Y3 = VSqZ2.multiply(
						U.multiply(X1).add(Y1.multiply(V))).add(
						A.multiply(U.add(V)));
		 ...
		 * 
		 * @see
		 * org.spongycastle.math.ec.ECPoint#subtract(org.spongycastle.math.ec
		 * .ECPoint)
		 */
		public ECPoint subtract(ECPoint b) {
			checkPoints(this, b);
		...
		public ECPoint twice() {
			...
			case ECCurve.COORD_HOMOGENEOUS: {
				...
				ECFieldElement vSquared = V.square();
				ECFieldElement h = S.square().add(S.multiply(V))
						.add(curve.getA().multiply(vSquared));

				ECFieldElement X3 = V.multiply(h);
				ECFieldElement Y3 = h.multiply(S.add(V)).add(
						X1Sq.square().multiply(V));
			...
			case ECCurve.COORD_LAMBDA_PROJECTIVE: {
				...
				if (b.bitLength() < (curve.getFieldSize() >> 1)) {
					...
					L3 = t1.add(T).add(Z1Sq).multiply(t1).add(t2.add(t3))
							.add(X3).add(a.addOne().multiply(Z3));
				} else {
					ECFieldElement X1Z1 = Z1IsOne ? X1 : X1.multiply(Z1);
		...
		public ECPoint twicePlus(ECPoint b) {
			...
			case ECCurve.COORD_LAMBDA_PROJECTIVE: {
				...
				ECFieldElement T = curve.getA().multiply(Z1Sq).add(L1Sq)
						.add(L1Z1);
				ECFieldElement L2plus1 = L2.addOne();
				ECFieldElement A = curve.getA().add(L2plus1).multiply(Z1Sq)
						.add(L1Sq).multiply(T).add(X1Sq.multiply(Z1Sq));
				ECFieldElement X2Z1Sq = X2.multiply(Z1Sq);
				ECFieldElement B = X2Z1Sq.add(T).square();

				ECFieldElement X3 = A.square().multiply(X2Z1Sq);
				ECFieldElement Z3 = A.multiply(B).multiply(Z1Sq);
				ECFieldElement L3 = A.add(B).square().multiply(T)
						.add(L2plus1.multiply(Z3));
		...
		protected void checkCurveEquation() {
			...
			ECFieldElement lhs = L.square().add(L.multiply(Z))
					.add(this.getCurve().getA().multiply(ZSq)).multiply(XSq);
			ECFieldElement rhs = ZSq.square().multiply(this.getCurve().getB())
					.add(XSq.square());

			if (!lhs.equals(rhs)) {
				throw new IllegalStateException(
						"F2m Lambda-Projective invariant broken");
			}
		}
