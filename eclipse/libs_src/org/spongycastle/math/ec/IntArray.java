...
class IntArray {
	...
	/*
	 * This expands 8 bit indices into 16 bit contents, by inserting 0s between
	 * bits. In a binary field, this operation is the same as squaring an 8 bit
	 * number.
	 */
	private static final int[] INTERLEAVE_TABLE = new int[] { 0x0000, 0x0001,
	...
	private static int addShiftedByBits(int[] x, int xOff, int[] y, int yOff,
			int count, int shift) {
		int shiftInv = 32 - shift, prev = 0;
		for (int i = 0; i < count; ++i) {
			int next = y[yOff + i];
	...
	private static void distribute(int[] x, int dst1, int dst2, int src,
			int count) {
		for (int i = 0; i < count; ++i) {
			int v = x[src + i];
			x[dst1 + i] ^= v;
	...
	public IntArray multiply(IntArray other, int m) {
		...
		// TODO It'd be better to be able to tune the width directly (need
		// support for interleaving arbitrary widths)
	...
	// private static void deInterleave(int[] x, int xOff, int[] z, int zOff,
	...
	// while (--rounds >= 0)
	// {
	// x = deInterleave16(x & DEINTERLEAVE_MASK) | (deInterleave16((x >>> 1) &
	// DEINTERLEAVE_MASK) << 16);
	// }
	// return x;
	// }
	//
	...
	public void reduce(int m, int[] ks) {
		...
		// Instead of flipping the high bits in the loop, explicitly clear any
		// partial word above m bits
		int partial = m & 0x1F;
		if (partial != 0) {
	...
	private static void interleave(int[] x, int xOff, int[] z, int zOff,
			int count, int rounds) {
		for (int i = 0; i < count; ++i) {
