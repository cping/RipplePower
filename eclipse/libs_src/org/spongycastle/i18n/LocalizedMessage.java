...
public class LocalizedMessage {
	 ...
	 * name for the RessourceBundle and <code>id</code> as the message bundle id
	 * the resource file.
	 * 
	 * @param resource
	 ...
	 *             if <code>resource</code> or <code>id</code> is
	 *             <code>null</code>
	 */
	...
	public String getEntry(String key, Locale loc, TimeZone timezone)
		...
		try {
			...
			if (!arguments.isEmpty()) {
				result = formatWithTimeZone(result,
						arguments.getFilteredArgs(loc), loc, timezone);
			}
			result = addExtraArgs(result, loc);
			return result;
		} catch (MissingResourceException mre) {
			throw new MissingEntryException("Can't find entry " + entry
					+ " in resource file " + resource + ".", resource, entry,
					loc, loader != null ? loader : this.getClassLoader());
		} catch (UnsupportedEncodingException use) {
			// should never occur - cause we already test this in the
			// constructor
			throw new RuntimeException(use);
		}
	}

	protected String formatWithTimeZone(String template, Object[] arguments,
	...
	/**
	 * Sets the {@link Filter} that is used to filter the arguments of this
	 * message
	 * 
	 * @param filter
	 *            the {@link Filter} to use. <code>null</code> to disable
	 *            filtering.
	 */
	...
	/**
	 * Set the {@link ClassLoader} which loads the resource files. If it is set
	 * to <code>null</code> then the default {@link ClassLoader} is used.
	 * 
	 * @param loader
		...
		FilteredArguments(Object[] args) {
			...
			for (int i = 0; i < args.length; i++) {
				if (args[i] instanceof TrustedInput) {
					...
					this.unpackedArgs[i] = ((UntrustedInput) args[i])
							.getInput();
	...
	public String toString() {
		...
		sb.append(" Arguments: ").append(arguments.getArguments().length)
				.append(" normal");
		if (extraArgs != null && extraArgs.getArguments().length > 0) {
			sb.append(", ").append(extraArgs.getArguments().length)
					.append(" extra");
		}
