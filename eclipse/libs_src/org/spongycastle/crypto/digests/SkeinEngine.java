 ...
 * <p/>
 * This implementation is the basis for {@link SkeinDigest} and {@link SkeinMac}
 * , implementing the parameter based configuration system that allows Skein to
 * be adapted to multiple applications. <br>
...
public class SkeinEngine implements Memoable {
	...
	/**
	 * Precalculated UBI(CFG) states for common state/output combinations
	 * without key or other pre-message params.
	 */
	private static final Hashtable INITIAL_STATES = new Hashtable();

	static {
		// From Appendix C of the Skein 1.3 NIST submission
		initialState(SKEIN_256, 128, new long[] { 0xe1111906964d7260L,
				0x883daaa77c8d811cL, 0x10080df491960f7aL, 0xccf7dde5b45bc1c2L });

		initialState(SKEIN_256, 160, new long[] { 0x1420231472825e98L,
				0x2ac4e9a25a77e590L, 0xd47a58568838d63eL, 0x2dd2e4968586ab7dL });

		initialState(SKEIN_256, 224, new long[] { 0xc6098a8c9ae5ea0bL,
				0x876d568608c5191cL, 0x99cb88d7d7f53884L, 0x384bddb1aeddb5deL });

		initialState(SKEIN_256, 256, new long[] { 0xfc9da860d048b449L,
				0x2fca66479fa7d833L, 0xb33bc3896656840fL, 0x6a54e920fde8da69L });

		initialState(SKEIN_512, 128, new long[] { 0xa8bc7bf36fbf9f52L,
				0x1e9872cebd1af0aaL, 0x309b1790b32190d3L, 0xbcfbb8543f94805cL,
				0x0da61bcd6e31b11bL, 0x1a18ebead46a32e3L, 0xa2cc5b18ce84aa82L,
				0x6982ab289d46982dL });

		initialState(SKEIN_512, 160, new long[] { 0x28b81a2ae013bd91L,
				0xc2f11668b5bdf78fL, 0x1760d8f3f6a56f12L, 0x4fb747588239904fL,
				0x21ede07f7eaf5056L, 0xd908922e63ed70b8L, 0xb8ec76ffeccb52faL,
				0x01a47bb8a3f27a6eL });

		initialState(SKEIN_512, 224, new long[] { 0xccd0616248677224L,
				0xcba65cf3a92339efL, 0x8ccd69d652ff4b64L, 0x398aed7b3ab890b4L,
				0x0f59d1b1457d2bd0L, 0x6776fe6575d4eb3dL, 0x99fbc70e997413e9L,
				0x9e2cfccfe1c41ef7L });

		initialState(SKEIN_512, 384, new long[] { 0xa3f6c6bf3a75ef5fL,
				0xb0fef9ccfd84faa4L, 0x9d77dd663d770cfeL, 0xd798cbf3b468fddaL,
				0x1bc4a6668a0e4465L, 0x7ed7d434e5807407L, 0x548fc1acd4ec44d6L,
				0x266e17546aa18ff8L });

		initialState(SKEIN_512, 512, new long[] { 0x4903adff749c51ceL,
				0x0d95de399746df03L, 0x8fd1934127c79bceL, 0x9a255629ff352cb1L,
				0x5db62599df6ca7b0L, 0xeabe394ca9d5c3f4L, 0x991112c71a75b523L,
				0xae18a40b660fcc33L });
	}

	private static void initialState(int blockSize, int outputSize, long[] state) {
		INITIAL_STATES.put(variantIdentifier(blockSize / 8, outputSize / 8),
				state);
	}

	private static Integer variantIdentifier(int blockSizeBytes,
			int outputSizeBytes) {
	...
	private static class UbiTweak {
		/**
		 * Point at which position might overflow long, so switch to add with
		 * carry logic
		 */
		private static final long LOW_RANGE = Long.MAX_VALUE
				- Integer.MAX_VALUE;
		...
		public void setType(int type) {
			// Bits 120..125 = type
			tweak[1] = (tweak[1] & 0xFFFFFFC000000000L)
					| ((type & 0x3FL) << 56);
		}
		...
		public void advancePosition(int advance) {
			...
			if (extendedPosition) {
				...
				tweak[0] = ((parts[1] & 0xFFFFFFFFL) << 32)
						| (parts[0] & 0xFFFFFFFFL);
				tweak[1] = (tweak[1] & 0xFFFFFFFF00000000L)
						| (parts[2] & 0xFFFFFFFFL);
			} else {
	...
	private class UBI {
		...
		public void update(byte[] value, int offset, int len, long[] output) {
			/*
			 * Buffer complete blocks for the underlying Threefish cipher, only
			 * flushing when there are subsequent bytes (last block must be
			 * processed in doFinal() with final=true set).
			 */
			int copied = 0;
			while (len > copied) {
				...
				int toCopy = Math.min((len - copied), currentBlock.length
						- currentOffset);
				System.arraycopy(value, offset + copied, currentBlock,
	...
	private void copyIn(SkeinEngine engine) {
		...
		this.initialState = Arrays
				.clone(engine.initialState, this.initialState);
		this.key = Arrays.clone(engine.key, this.key);
		this.preMessageParameters = clone(engine.preMessageParameters,
				this.preMessageParameters);
		this.postMessageParameters = clone(engine.postMessageParameters,
	...
	public void reset(Memoable other) {
		...
		if ((getBlockSize() != s.getBlockSize())
	 ...
	 * {@link SkeinParameters} for details on the parameterisation of the Skein
	 * hash function.
	 * 
	 * @param params
	 *            the parameters to apply to this engine, or <code>null</code>
	...
	private void createInitialState() {
		long[] precalc = (long[]) INITIAL_STATES.get(variantIdentifier(
		...
		if ((key == null) && (precalc != null)) {
			...
			ubiComplete(PARAM_TYPE_CONFIG, new Configuration(
					outputSizeBytes * 8).getBytes());
		}
	...
	public int doFinal(byte[] out, int outOff) {
		...
		if (out.length < (outOff + outputSizeBytes)) {
			throw new DataLengthException(
					"Output buffer is too short to hold output of "
		...
		for (int i = 0; i < blocksRequired; i++) {
			final int toWrite = Math.min(blockSize, outputSizeBytes
					- (i * blockSize));
	...
	private void output(long outputSequence, byte[] out, int outOff,
			int outputBytes) {
		byte[] currentBytes = new byte[8];
		ThreefishEngine.wordToBytes(outputSequence, currentBytes, 0);

		// Output is a sequence of UBI invocations all of which use and preserve
		// the pre-output
		// state
		...
		for (int i = 0; i < wordsRequired; i++) {
			...
			if (toWrite == 8) {
				ThreefishEngine.wordToBytes(outputWords[i], out, outOff
						+ (i * 8));
			} else {
				ThreefishEngine.wordToBytes(outputWords[i], currentBytes, 0);
				System.arraycopy(currentBytes, 0, out, outOff + (i * 8),
						toWrite);
			}
		}
	}
