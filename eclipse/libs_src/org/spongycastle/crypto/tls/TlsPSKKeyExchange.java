...
public class TlsPSKKeyExchange extends AbstractTlsKeyExchange {
	...
	public TlsPSKKeyExchange(int keyExchange,
			Vector supportedSignatureAlgorithms, TlsPSKIdentity pskIdentity,
			DHParameters dhParameters, int[] namedCurves,
	...
	public byte[] generateServerKeyExchange() throws IOException {
		// TODO[RFC 4279] Need a server-side PSK API to determine hint and
		// resolve identities to keys
		...
		if (this.keyExchange == KeyExchangeAlgorithm.DHE_PSK) {
			...
			this.dhAgreePrivateKey = TlsDHUtils
					.generateEphemeralServerKeyExchange(
							context.getSecureRandom(), this.dhParameters, buf);
	...
	public void processServerCertificate(Certificate serverCertificate)
		...
		org.spongycastle.asn1.x509.Certificate x509Cert = serverCertificate
				.getCertificateAt(0);
	...
	public void processServerKeyExchange(InputStream input) throws IOException {
		...
		if (this.keyExchange == KeyExchangeAlgorithm.DHE_PSK) {
			...
			this.dhAgreePublicKey = TlsDHUtils
					.validateDHPublicKey(serverDHParams.getPublicKey());
		} else if (this.keyExchange == KeyExchangeAlgorithm.ECDHE_PSK) {
			// TODO[RFC 5489]
		}
	}
	...
	public void generateClientKeyExchange(OutputStream output)
		...
		if (this.keyExchange == KeyExchangeAlgorithm.DHE_PSK) {
			...
			this.premasterSecret = TlsRSAUtils
					.generateEncryptedPreMasterSecret(context,
	...
	public byte[] generatePremasterSecret() throws IOException {
		...
		ByteArrayOutputStream buf = new ByteArrayOutputStream(4
				+ other_secret.length + psk.length);
	...
	protected byte[] generateOtherSecret(int pskLength) throws IOException {
		if (this.keyExchange == KeyExchangeAlgorithm.DHE_PSK) {
			if (dhAgreePrivateKey != null) {
				return TlsDHUtils.calculateDHBasicAgreement(dhAgreePublicKey,
						dhAgreePrivateKey);
			}
