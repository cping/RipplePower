...
public class TlsECCUtils {
	...
	private static final String[] curveNames = new String[] { "sect163k1",
			...
			"sect409r1", "sect571k1", "sect571r1", "secp160k1", "secp160r1",
	...
	public static void addSupportedEllipticCurvesExtension(
		...
		extensions.put(EXT_elliptic_curves,
				createSupportedEllipticCurvesExtension(namedCurves));
	}
	...
	public static void addSupportedPointFormatsExtension(Hashtable extensions,
		...
		extensions.put(EXT_ec_point_formats,
				createSupportedPointFormatsExtension(ecPointFormats));
	}
	...
	public static int[] getSupportedEllipticCurvesExtension(Hashtable extensions)
		...
		return extensionData == null ? null
				: readSupportedEllipticCurvesExtension(extensionData);
	}
	...
	public static short[] getSupportedPointFormatsExtension(Hashtable extensions)
		...
		return extensionData == null ? null
				: readSupportedPointFormatsExtension(extensionData);
	}

	public static byte[] createSupportedEllipticCurvesExtension(
			int[] namedCurves) throws IOException {
	...
	public static byte[] createSupportedPointFormatsExtension(
		...
		if (ecPointFormats == null) {
			...
			/*
			 * RFC 4492 5.1. If the Supported Point Formats Extension is indeed
			 * sent, it MUST contain the value 0 (uncompressed) as one of the
			 * items in the list of point formats.
			 */
	...
	public static int[] readSupportedEllipticCurvesExtension(
	...
	public static short[] readSupportedPointFormatsExtension(
			byte[] extensionData) throws IOException {
		if (extensionData == null) {
			throw new IllegalArgumentException("'extensionData' cannot be null");
		}
	...
	public static String getNameOfNamedCurve(int namedCurve) {
		return isSupportedNamedCurve(namedCurve) ? curveNames[namedCurve - 1]
				: null;
	}
	...
	public static ECDomainParameters getParametersForNamedCurve(int namedCurve) {
		...
		return new ECDomainParameters(ecP.getCurve(), ecP.getG(), ecP.getN(),
	...
	public static byte[] serializeECPoint(short[] ecPointFormats, ECPoint point)
		...
		/*
		 * RFC 4492 5.7. ...an elliptic curve point in uncompressed or
		 * compressed format. Here, the format MUST conform to what the server
		 * has requested through a Supported Point Formats Extension if this
		 * extension was used, and MUST be uncompressed if this extension was
		 * not used.
		 */
	...
	public static BigInteger deserializeECFieldElement(int fieldSize,
	...
	public static ECPoint deserializeECPoint(short[] ecPointFormats,
			ECCurve curve, byte[] encoding) throws IOException {
		/*
		 * NOTE: Here we implicitly decode compressed or uncompressed encodings.
		 * DefaultTlsClient by default is set up to advertise that we can parse
		 * any encoding so this works fine, but extra checks might be needed
		 * here if that were changed.
		 */
	...
	public static ECPublicKeyParameters deserializeECPublicKey(
			short[] ecPointFormats, ECDomainParameters curve_params,
			byte[] encoding) throws IOException {
		try {
			ECPoint Y = deserializeECPoint(ecPointFormats,
					curve_params.getCurve(), encoding);
	...
	public static byte[] calculateECDHBasicAgreement(
			ECPublicKeyParameters publicKey, ECPrivateKeyParameters privateKey) {
		ECDHBasicAgreement basicAgreement = new ECDHBasicAgreement();
		basicAgreement.init(privateKey);
		BigInteger agreementValue = basicAgreement
				.calculateAgreement(publicKey);

		/*
		 * RFC 4492 5.10. Note that this octet string (Z in IEEE 1363
		 * terminology) as output by FE2OSP, the Field Element to Octet String
		 * Conversion Primitive, has constant length for any given field;
	...
	public static AsymmetricCipherKeyPair generateECKeyPair(
			SecureRandom random, ECDomainParameters ecParams) {
	...
	public static ECPrivateKeyParameters generateEphemeralClientKeyExchange(
			SecureRandom random, short[] ecPointFormats,
			ECDomainParameters ecParams, OutputStream output)
			throws IOException {
		AsymmetricCipherKeyPair kp = TlsECCUtils.generateECKeyPair(random,
				ecParams);

		ECPublicKeyParameters ecPublicKey = (ECPublicKeyParameters) kp
				.getPublic();
	...
	public static ECPublicKeyParameters validateECPublicKey(
			ECPublicKeyParameters key) throws IOException {
	...
	public static ECDomainParameters readECParameters(int[] namedCurves,
			short[] ecPointFormats, InputStream input) throws IOException {
		try {
			...
			case ECCurveType.explicit_prime: {
				...
				ECPoint base = deserializeECPoint(ecPointFormats, curve,
						TlsUtils.readOpaque8(input));
			...
			case ECCurveType.named_curve: {
				...
				if (!NamedCurve.refersToASpecificNamedCurve(namedCurve)) {
					/*
					 * RFC 4492 5.4. All those values of NamedCurve are allowed
					 * that refer to a specific curve. Values of NamedCurve that
					 * indicate support for a class of explicitly defined curves
	...
	private static void checkNamedCurve(int[] namedCurves, int namedCurve)
		...
		if (namedCurves != null && !Arrays.contains(namedCurves, namedCurve)) {
			/*
			 * RFC 4492 4. [...] servers MUST NOT negotiate the use of an ECC
			 * cipher suite unless they can complete the handshake while
			 * respecting the choice of curves and compression techniques
	...
	public static void writeECFieldElement(int fieldSize, BigInteger x,
	...
	public static void writeExplicitECParameters(short[] ecPointFormats,
			ECDomainParameters ecParameters, OutputStream output)
			throws IOException {
		...
		if (curve instanceof ECCurve.Fp) {
			...
			throw new IllegalArgumentException(
					"'ecParameters' not a known curve type");
		}

		writeECFieldElement(curve.getA(), output);
		writeECFieldElement(curve.getB(), output);
		TlsUtils.writeOpaque8(
				serializeECPoint(ecPointFormats, ecParameters.getG()), output);
		writeECParameter(ecParameters.getN(), output);
		writeECParameter(ecParameters.getH(), output);
	}

	public static void writeECPoint(short[] ecPointFormats, ECPoint point,
			OutputStream output) throws IOException {
		TlsUtils.writeOpaque8(
				TlsECCUtils.serializeECPoint(ecPointFormats, point), output);
	}
	...
	public static void writeNamedECParameters(int namedCurve,
		...
		if (!NamedCurve.refersToASpecificNamedCurve(namedCurve)) {
			/*
			 * RFC 4492 5.4. All those values of NamedCurve are allowed that
			 * refer to a specific curve. Values of NamedCurve that indicate
