...
public class TlsStreamCipher implements TlsCipher {
	...
	public byte[] encodePlaintext(long seqNo, short type, byte[] plaintext,
			int offset, int len) {
		/*
		 * TODO[draft-josefsson-salsa20-tls-02] Note that Salsa20 requires a
		 * 64-bit nonce. That nonce is updated on the encryption of every TLS
		 * record, and is set to be the 64-bit TLS record sequence number. In
	...
	public byte[] decodeCiphertext(long seqNo, short type, byte[] ciphertext,
		...
		if (encryptThenMAC) {
			...
			checkMAC(seqNo, type, ciphertext, ciphertextEnd - macSize,
					ciphertextEnd, ciphertext, offset, plaintextLength);
			byte[] deciphered = new byte[plaintextLength];
			decryptCipher.processBytes(ciphertext, offset, plaintextLength,
					deciphered, 0);
			return deciphered;
		} else {
			byte[] deciphered = new byte[len];
			decryptCipher.processBytes(ciphertext, offset, len, deciphered, 0);
			checkMAC(seqNo, type, deciphered, plaintextLength, len, deciphered,
					0, plaintextLength);
			return Arrays.copyOfRange(deciphered, 0, plaintextLength);
		}
	}

	private void checkMAC(long seqNo, short type, byte[] recBuf, int recStart,
			int recEnd, byte[] calcBuf, int calcOff, int calcLen)
			throws IOException {
		byte[] receivedMac = Arrays.copyOfRange(recBuf, recStart, recEnd);
		byte[] computedMac = readMac.calculateMac(seqNo, type, calcBuf,
				calcOff, calcLen);
