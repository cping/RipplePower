...
class DTLSReliableHandshake {
	...
	Message receiveMessage() throws IOException {
		...
		{
			DTLSReassembler next = (DTLSReassembler) currentInboundFlight
					.get(Integers.valueOf(next_receive_seq));
			if (next != null) {
				byte[] body = next.getBodyIfComplete();
				if (body != null) {
					previousInboundFlight = null;
					return updateHandshakeMessagesDigest(new Message(
							next_receive_seq++, next.getType(), body));
				}
			}
		}
		...
		for (;;) {
			...
			try {
				for (;;) {
					...
					if (seq < next_receive_seq) {
						/*
						 * NOTE: If we receive the previous flight of incoming
						 * messages in full again, retransmit our last flight
						 */
						if (previousInboundFlight != null) {
							...
							if (reassembler != null) {
								...
								reassembler.contributeFragment(msg_type,
										length, buf, 12, fragment_offset,
										fragment_length);
								...
								if (checkAll(previousInboundFlight)) {
									...
									/*
									 * TODO[DTLS] implementations SHOULD back
									 * off handshake packet size during the
									 * retransmit backoff.
									 */
									readTimeoutMillis = Math.min(
											readTimeoutMillis * 2, 60000);
						...
						if (reassembler == null) {
							...
							currentInboundFlight.put(Integers.valueOf(seq),
									reassembler);
						}

						reassembler.contributeFragment(msg_type, length, buf,
								12, fragment_offset, fragment_length);
			...
			/*
			 * TODO[DTLS] implementations SHOULD back off handshake packet size
	...
	void finish() {
		...
		if (!sending) {
			...
			/*
			 * RFC 6347 4.2.4. In addition, for at least twice the default MSL
			 * defined for [TCP], when in the FINISHED state, the node that
			 * transmits the last flight (the server in an ordinary handshake or
			 * the client in a resumed handshake) MUST respond to a retransmit
			 * of the peer's last flight with a retransmit of the last flight.
			 */
			retransmit = new DTLSHandshakeRetransmit() {
				public void receivedHandshakeRecord(int epoch, byte[] buf,
						int off, int len) throws IOException {
					/*
					 * TODO Need to handle the case where the previous inbound
					 * flight contains messages from two epochs.
					 */
					if (len < 12) {
						return;
					}
					...
					// TODO This is a hack that only works until we try to
					// support renegotiation
					int expectedEpoch = msg_type == HandshakeType.finished ? 1
							: 0;
					...
					if (reassembler != null) {
						reassembler.contributeFragment(msg_type, length, buf,
								off + 12, fragment_offset, fragment_length);
	...
	/**
	 * Check that there are no "extra" messages left in the current inbound
	 * flight
	 */
	...
	private void writeMessage(Message message) throws IOException {
		...
		do {
			int fragment_length = Math.min(length - fragment_offset,
					fragmentLimit);
	...
	private void writeHandshakeFragment(Message message, int fragment_offset,
			int fragment_length) throws IOException {
