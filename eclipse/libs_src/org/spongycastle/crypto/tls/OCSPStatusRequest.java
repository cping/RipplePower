...
public class OCSPStatusRequest {
	 ...
	 *            a {@link Vector} of {@link ResponderID}, specifying the list
	 *            of trusted OCSP responders. An empty list has the special
	 *            meaning that the responders are implicitly known to the server
	 *            - e.g., by prior arrangement.
	 * @param requestExtensions
	 *            OCSP request extensions. A null value means that there are no
	 *            extensions.
	 */
	public OCSPStatusRequest(Vector responderIDList,
	...
	public void encode(OutputStream output) throws IOException {
		if (responderIDList == null || responderIDList.isEmpty()) {
			...
			for (int i = 0; i < responderIDList.size(); ++i) {
				ResponderID responderID = (ResponderID) responderIDList
						.elementAt(i);
	...
	public static OCSPStatusRequest parse(InputStream input) throws IOException {
		Vector responderIDList = new Vector();
		{
			...
			if (length > 0) {
				...
				do {
					...
					ResponderID responderID = ResponderID.getInstance(TlsUtils
							.readDERObject(derEncoding));
		...
		Extensions requestExtensions = null;
		{
			...
			if (length > 0) {
				...
				requestExtensions = Extensions.getInstance(TlsUtils
						.readDERObject(derEncoding));
			}
		}
