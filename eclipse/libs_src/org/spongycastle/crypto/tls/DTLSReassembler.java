...
class DTLSReassembler {
	...
	void contributeFragment(short msg_type, int length, byte[] buf, int off,
			int fragment_offset, int fragment_length) {

		int fragment_end = fragment_offset + fragment_length;

		if (this.msg_type != msg_type || this.body.length != length
				|| fragment_end > length) {
			return;
		}

		if (fragment_length == 0) {
			// NOTE: Empty messages still require an empty fragment to complete
			// it
		...
		for (int i = 0; i < missing.size(); ++i) {
			...
			if (range.getEnd() > fragment_offset) {
				...
				System.arraycopy(buf, off + copyStart - fragment_offset, body,
				...
				if (copyStart == range.getStart()) {
					...
					if (copyEnd == range.getEnd()) {
						...
						missing.insertElementAt(
								new Range(copyEnd, range.getEnd()), ++i);
						range.setEnd(copyStart);
					}
				}
			}
