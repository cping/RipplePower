...
public abstract class AbstractTlsKeyExchange implements TlsKeyExchange {
	...
	public void init(TlsContext context) {
		...
		if (TlsUtils.isSignatureAlgorithmsExtensionAllowed(clientVersion)) {
			/*
			 * RFC 5264 7.4.1.4.1. If the client does not send the
			 * signature_algorithms extension, the server MUST do the following:
			 * 
			...
			throw new IllegalStateException(
					"supported_signature_algorithms not allowed for "
							+ clientVersion);
		}
	}
	...
	public void processServerCertificate(Certificate serverCertificate)
		...
		if (supportedSignatureAlgorithms == null) {
			/*
			 * TODO RFC 2264 7.4.2. Unless otherwise specified, the signing
			 * algorithm for the certificate must be the same as the algorithm
			 * for the certificate key.
			 */
		} else {
			/*
			 * TODO RFC 5264 7.4.2. If the client provided a
			 * "signature_algorithms" extension, then all certificates provided
			 * by the server MUST be signed by a hash/signature algorithm pair
			 * that appears in that extension.
			 */
		}
	}
