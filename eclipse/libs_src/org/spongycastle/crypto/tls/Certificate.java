...
public class Certificate {
	...
	/**
	 * @return an array of {@link org.spongycastle.asn1.x509.Certificate}
	...
	public void encode(OutputStream output) throws IOException {
		...
		for (int i = 0; i < this.certificateList.length; ++i) {
			byte[] derEncoding = certificateList[i]
	...
	public static Certificate parse(InputStream input) throws IOException {
		...
		while (buf.available() > 0) {
			...
			certificate_list.addElement(org.spongycastle.asn1.x509.Certificate
					.getInstance(asn1Cert));
		}

		org.spongycastle.asn1.x509.Certificate[] certificateList = new org.spongycastle.asn1.x509.Certificate[certificate_list
				.size()];
		for (int i = 0; i < certificate_list.size(); i++) {
			certificateList[i] = (org.spongycastle.asn1.x509.Certificate) certificate_list
					.elementAt(i);
		}
