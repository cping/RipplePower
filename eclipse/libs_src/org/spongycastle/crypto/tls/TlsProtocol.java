...
public abstract class TlsProtocol {
	...
	public TlsProtocol(InputStream input, OutputStream output,
	...
	protected void completeHandshake() throws IOException {
		try {
			...
			/*
			 * If this was an initial handshake, we are now ready to send and
				...
				if (this.sessionParameters == null) {
					this.sessionParameters = new SessionParameters.Builder()
							...
							.setCompressionAlgorithm(
									this.securityParameters.compressionAlgorithm)
							.setMasterSecret(
									this.securityParameters.masterSecret)
							.setPeerCertificate(this.peerCertificate)
							// TODO Consider filtering extensions that aren't
							// relevant to resumed sessions
		...
		do {
			...
			if (handshakeQueue.size() >= 4) {
				...
				if (handshakeQueue.size() >= (len + 4)) {
					 ...
					 * handshake messages starting at client hello up to, but
					 * not including, this finished message. [..] Note: [Also,]
					 * Hello Request messages are omitted from handshake hashes.
					 */
					switch (type) {
					...
					case HandshakeType.finished: {
						if (this.expected_verify_data == null) {
							this.expected_verify_data = createVerifyData(!getContext()
									.isServer());
						}
	...
	private void processApplicationData() {
		 ...
		 * 
		 * This function could be used for callbacks when application data
	...
	private void processAlert() throws IOException {
		while (alertQueue.size() >= 2) {
			...
			if (level == AlertLevel.fatal) {
				/*
				 * RFC 2246 7.2.1. The session becomes unresumable if any
				 * connection is terminated without proper close_notify messages
		...
		while (applicationDataQueue.size() == 0) {
			...
			if (this.closed) {
				if (this.failedWithError) {
					/*
					 * Something went terribly wrong, we should throw an
					 * IOException
					 */
					throw new IOException(TLS_ERROR_MESSAGE);
				}
	...
	protected void safeReadRecord() throws IOException {
		try {
			if (!recordStream.readRecord()) {
				// TODO It would be nicer to allow graceful connection close if
				// between records
				// this.failWithError(AlertLevel.warning,
	...
	protected void writeData(byte[] buf, int offset, int len)
		...
		while (len > 0) {
			/*
			 * RFC 5246 6.2.1. Zero-length fragments of Application data MAY be
			 * sent as they are potentially useful as a traffic analysis
			 * countermeasure.
			 * 
			 * NOTE: Actually, implementations appear to have settled on 1/n-1
			 * record splitting.
			 */
			...
			if (this.splitApplicationDataRecords) {
				 ...
				 * 
				 * DO NOT REMOVE THIS CODE, EXCEPT YOU KNOW EXACTLY WHAT YOU ARE
				 * DOING HERE.
				 */
			...
			if (len > 0) {
				...
				safeWriteRecord(ContentType.application_data, buf, offset,
						toWrite);
				offset += toWrite;
				len -= toWrite;
			}
	...
	/**
	 * Terminate this connection with an alert. Can be used for normal closure
	 * too.
	 * 
	 * @param alertLevel
	...
	protected void failWithError(short alertLevel, short alertDescription,
		...
		if (!closed) {
			...
			if (alertLevel == AlertLevel.fatal) {
				 ...
				 * with level equal to warning.
				 */
				// TODO This isn't quite in the right place. Also, as of TLS 1.1
	...
	protected void processFinishedMessage(ByteArrayInputStream buf)
			throws IOException {
		byte[] verify_data = TlsUtils.readFully(expected_verify_data.length,
				buf);
	...
	protected void raiseAlert(short alertLevel, short alertDescription,
			String message, Exception cause) throws IOException {
		getPeer().notifyAlertRaised(alertLevel, alertDescription, message,
				cause);
		...
		if (certificate.getLength() == 0) {
			...
			if (!context.isServer()) {
				...
				if (serverVersion.isSSL()) {
					String message = serverVersion.toString()
	...
	protected void sendChangeCipherSpecMessage() throws IOException {
		...
		safeWriteRecord(ContentType.change_cipher_spec, message, 0,
				message.length);
	...
	protected void sendFinishedMessage() throws IOException {
		...
		HandshakeMessage message = new HandshakeMessage(HandshakeType.finished,
				verify_data.length);
	...
	protected byte[] createVerifyData(boolean isServer) {
		...
		if (isServer) {
			return TlsUtils.calculateVerifyData(
					context,
					ExporterLabel.server_finished,
					getCurrentPRFHash(getContext(),
							recordStream.getHandshakeHash(),
							TlsUtils.SSL_SERVER));
		}
		...
		return TlsUtils.calculateVerifyData(
				...
				getCurrentPRFHash(getContext(),
						recordStream.getHandshakeHash(), TlsUtils.SSL_CLIENT));
	}
	...
	protected void handleClose(boolean user_canceled) throws IOException {
		if (!closed) {
			if (user_canceled && !appDataReady) {
				raiseWarning(AlertDescription.user_canceled,
						"User canceled handshake");
			}
			this.failWithError(AlertLevel.warning,
					AlertDescription.close_notify, "Connection closed", null);
		}
	}
	...
	protected short processMaxFragmentLengthExtension(
			Hashtable clientExtensions, Hashtable serverExtensions,
			short alertDescription) throws IOException {
		short maxFragmentLength = TlsExtensionsUtils
				.getMaxFragmentLengthExtension(serverExtensions);
		if (maxFragmentLength >= 0 && !this.resumedSession) {
			if (maxFragmentLength != TlsExtensionsUtils
	...
	protected static byte[] createRandomBlock(SecureRandom random) {
		...
		/*
		 * The consensus seems to be that using the time here is neither all
		 * that useful, nor secure. Perhaps there could be an option to
		 * (re-)enable it. Instead, we seed the random source with the current
		 * time to retain it's main benefit.
		 */
		// TlsUtils.writeGMTUnixTime(result, 0);
		return result;
	}

	protected static byte[] createRenegotiationInfo(
			byte[] renegotiated_connection) throws IOException {
		return TlsUtils.encodeOpaque8(renegotiated_connection);
	}

	protected static void establishMasterSecret(TlsContext context,
			TlsKeyExchange keyExchange) throws IOException {
		byte[] pre_master_secret = keyExchange.generatePremasterSecret();

		try {
			context.getSecurityParameters().masterSecret = TlsUtils
					.calculateMasterSecret(context, pre_master_secret);
		} finally {
			// TODO Is there a way to ensure the data is really overwritten?
			/*
			 * RFC 2246 8.1. The pre_master_secret should be deleted from memory
			 * once the master_secret has been computed.
			 */
	...
	protected static byte[] getCurrentPRFHash(TlsContext context,
			TlsHandshakeHash handshakeHash, byte[] sslSender) {
	...
	protected static Hashtable readExtensions(ByteArrayInputStream input)
		...
		while (buf.available() > 0) {
			...
			/*
			 * RFC 3546 2.3 There MUST NOT be more than one extension of the
			 * same type.
			 */
	...
	protected static Vector readSupplementalDataMessage(
			ByteArrayInputStream input) throws IOException {
		...
		while (buf.available() > 0) {
			...
			supplementalData.addElement(new SupplementalDataEntry(
					supp_data_type, data));
		}
	...
	protected static void writeExtensions(OutputStream output,
			Hashtable extensions) throws IOException {
	...
	protected static void writeSupplementalData(OutputStream output,
			Vector supplementalData) throws IOException {
		ByteArrayOutputStream buf = new ByteArrayOutputStream();

		for (int i = 0; i < supplementalData.size(); ++i) {
			SupplementalDataEntry entry = (SupplementalDataEntry) supplementalData
					.elementAt(i);
