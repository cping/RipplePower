...
public class CertificateRequest {
	...
	/*
	 * TODO RFC 5264 7.4.4 A list of the hash/signature algorithm pairs that the
	...
	public CertificateRequest(short[] certificateTypes,
			Vector supportedSignatureAlgorithms, Vector certificateAuthorities) {
	...
	/**
	 * @return a {@link Vector} of {@link SignatureAndHashAlgorithm} (or null
	 *         before TLS 1.2).
	 */
		...
		if (certificateAuthorities == null || certificateAuthorities.isEmpty()) {
			...
			for (int i = 0; i < certificateAuthorities.size(); ++i) {
				X500Name certificateAuthority = (X500Name) certificateAuthorities
						.elementAt(i);
				byte[] derEncoding = certificateAuthority
	...
	public static CertificateRequest parse(TlsContext context, InputStream input)
		...
		if (TlsUtils.isTLSv12(context)) {
			// TODO Check whether SignatureAlgorithm.anonymous is allowed here
			supportedSignatureAlgorithms = TlsUtils
					.parseSupportedSignatureAlgorithms(false, input);
		}
		...
		return new CertificateRequest(certificateTypes,
				supportedSignatureAlgorithms, certificateAuthorities);
	}
}
