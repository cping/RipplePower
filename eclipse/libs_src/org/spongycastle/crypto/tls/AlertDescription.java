...
public class AlertDescription {
	 ...
	 * more messages on this connection. Note that as of TLS 1.1, failure to
	 * properly close a connection no longer requires that a session not be
	 * resumed. This is a change from TLS 1.0 ("The session becomes unresumable
	 * if any connection is terminated without proper close_notify messages with
	 * level equal to warning.") to conform with widespread implementation
	 * practice.
	 */
	 ...
	 * This alert also MUST be returned if an alert is sent because a
	 * TLSCiphertext decrypted in an invalid way: either it wasn't an even
	 * multiple of the block length, or its padding values, when checked,
	 * weren't correct. This message is always fatal and should never be
	 ...
	 * permitted certain attacks against the CBC mode [CBCATT]. It MUST NOT be
	 ...
	 * 2^14+1024 bytes. This message is always fatal and should never be
	 * observed in communication between proper implementations (except when
	 * messages were corrupted in the network).
	 */
	 ...
	 * expand to excessive length). This message is always fatal and should
	 ...
	 * was unable to negotiate an acceptable set of security parameters given
	 * the options available. This is a fatal error.
	 */
	public static final short handshake_failure = 40;

	/**
	 * This alert was used in SSLv3 but not any version of TLS. It MUST NOT be
	 * sent by compliant implementations.
	 */
	 ...
	 * fields. This message is always fatal.
	 */
	 ...
	 * specified range or the length of the message was incorrect. This message
	 * is always fatal and should never be observed in communication between
	 ...
	 * correctly verify a signature or validate a Finished message. This message
	 * is always fatal.
	 */
	public static final short decrypt_error = 51;

	/**
	 * This alert was used in some earlier versions of TLS. It MUST NOT be sent
	 ...
	 * failure. If the user cancels an operation after the handshake is
	 * complete, just closing the connection by sending a close_notify is more
	 * appropriate. This alert should be followed by a close_notify. This
	 ...
	 * where this would be appropriate is where a server has spawned a process
	 * to satisfy a request; the process might receive security parameters (key
	 * length, authentication, etc.) at startup, and it might be difficult to
	 ...
	 * extension that they did not put in the corresponding client hello. This
	...
	/**
	 * This alert is sent by servers who are unable to retrieve a certificate
	 * chain from the URL supplied by the client (see Section 3.3). This message
	 * MAY be fatal - for example if client authentication is required by the
	 * server for the handshake to continue and the server is unable to retrieve
	 * the certificate chain, it may send a fatal alert.
	 */
	public static final short certificate_unobtainable = 111;

	/**
	 * This alert is sent by servers that receive a server_name extension
	 * request, but do not recognize the server name. This message MAY be fatal.
	 */
	...
	/**
	 * This alert is sent by servers when a certificate hash does not match a
	 * client provided certificate_hash. This message is always fatal.
	 */
