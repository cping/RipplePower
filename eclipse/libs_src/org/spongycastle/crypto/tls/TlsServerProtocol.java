...
public class TlsServerProtocol extends TlsProtocol {
	...
	public void accept(TlsServer tlsServer) throws IOException {
		...
		this.tlsServerContext = new TlsServerContextImpl(secureRandom,
				securityParameters);
	...
	protected void handleHandshakeMessage(short type, byte[] data)
		...
		case HandshakeType.client_hello: {
			...
			case CS_START: {
				...
				Vector serverSupplementalData = tlsServer
				...
				if (this.serverCredentials == null) {
					...
					this.keyExchange
							.processServerCredentials(this.serverCredentials);
				...
				if (this.allowCertificateStatus) {
					CertificateStatus certificateStatus = tlsServer
				...
				byte[] serverKeyExchange = this.keyExchange
		...
		case HandshakeType.supplemental_data: {
			...
			case CS_SERVER_HELLO_DONE: {
				tlsServer
						.processClientSupplementalData(readSupplementalDataMessage(buf));
		...
		case HandshakeType.certificate_verify: {
			...
			case CS_CLIENT_KEY_EXCHANGE: {
				 ...
				 * certificate that has signing capability (i.e., all
	...
	protected void handleWarningMessage(short description) throws IOException {
		...
		case AlertDescription.no_certificate: {
			/*
			 * SSL 3.0 If the server has sent a certificate request Message, the
			 * client must send either the certificate message or a
	...
	protected void notifyClientCertificate(Certificate clientCertificate)
		...
		if (clientCertificate.isEmpty()) {
			...
			this.clientCertificateType = TlsUtils.getClientCertificateType(
					clientCertificate, this.serverCredentials.getCertificate());
	...
	protected void receiveCertificateVerifyMessage(ByteArrayInputStream buf)
		...
		try {
			...
			// DigitallySigned
			byte[] certificateVerifyHash = getCurrentPRFHash(getContext(),
					prepareFinishHash, null);

			org.spongycastle.asn1.x509.Certificate x509Cert = this.peerCertificate
					.getCertificateAt(0);
			...
			TlsSigner tlsSigner = TlsUtils
					.createTlsSigner(this.clientCertificateType);
	...
	protected void receiveClientHelloMessage(ByteArrayInputStream buf)
		...
		/*
		 * TODO RFC 5077 3.4. If a ticket is presented by the client, the server
		 * MUST NOT attempt to use the Session ID in the ClientHello for
		...
		/*
		 * TODO RFC 5246 7.4.1.2. If the session_id field is not empty (implying
		 * a session resumption request), this vector MUST include at least the
		...
		/*
		 * TODO RFC 5246 7.4.1.2. If the session_id field is not empty (implying
		 * a session resumption request), it MUST include the compression_method
		 * from that session.
		 */
		...
		this.offeredCompressionMethods = TlsUtils.readUint8Array(
		 ...
		 */
		{
			 ...
			 */
			if (Arrays.contains(offeredCipherSuites,
	...
	protected void receiveClientKeyExchangeMessage(ByteArrayInputStream buf)
		...
		recordStream.setPendingConnectionState(getPeer().getCompression(),
				getPeer().getCipher());
	...
	protected void sendCertificateRequestMessage(
	...
	protected void sendCertificateStatusMessage(
	...
	protected void sendServerHelloMessage() throws IOException {
		...
		if (!server_version.isEqualOrEarlierVersionOf(getContext()
				.getClientVersion())) {
			throw new TlsFatalAlert(AlertDescription.internal_error);
		}
		...
		short selectedCompressionMethod = tlsServer
		...
		if (this.secure_renegotiation) {
			...
			if (noRenegExt) {
				 ...
				 * ServerHello message.
				 */
				this.serverExtensions = TlsExtensionsUtils
						.ensureExtensionsInitialised(this.serverExtensions);
		...
		if (this.serverExtensions != null) {
			this.securityParameters.maxFragmentLength = processMaxFragmentLengthExtension(
					clientExtensions, this.serverExtensions,
					AlertDescription.internal_error);

			this.securityParameters.truncatedHMac = TlsExtensionsUtils
					.hasTruncatedHMacExtension(this.serverExtensions);
		...
		securityParameters.prfAlgorithm = getPRFAlgorithm(getContext(),
	...
	protected void sendServerKeyExchangeMessage(byte[] serverKeyExchange)
			throws IOException {
		HandshakeMessage message = new HandshakeMessage(
				HandshakeType.server_key_exchange, serverKeyExchange.length);
	...
	protected boolean expectCertificateVerifyMessage() {
		return this.clientCertificateType >= 0
				&& TlsUtils.hasSigningCapability(this.clientCertificateType);
	}
}
