...
public class TlsSRPKeyExchange extends AbstractTlsKeyExchange {
	...
	public TlsSRPKeyExchange(int keyExchange,
			Vector supportedSignatureAlgorithms, byte[] identity,
			byte[] password) {
	...
	public void processServerKeyExchange(InputStream input) throws IOException {
		...
		if (buf != null) {
			DigitallySigned signed_params = DigitallySigned.parse(context,
					input);

			Signer signer = initVerifyer(tlsSigner,
					signed_params.getAlgorithm(), securityParameters);
	...
	public void generateClientKeyExchange(OutputStream output)
			throws IOException {
		BigInteger A = srpClient.generateClientCredentials(s, this.identity,
				this.password);
	...
	public byte[] generatePremasterSecret() throws IOException {
		try {
			...
			return BigIntegers
					.asUnsignedByteArray(srpClient.calculateSecret(B));
		} catch (CryptoException e) {
			throw new TlsFatalAlert(AlertDescription.illegal_parameter);
		}
	}
