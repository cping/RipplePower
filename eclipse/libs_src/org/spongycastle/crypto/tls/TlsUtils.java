...
public class TlsUtils {
	...
	public static final Integer EXT_signature_algorithms = Integers
			.valueOf(ExtensionType.signature_algorithms);
	...
	public static boolean isTLSv11(TlsContext context) {
		return ProtocolVersion.TLSv11.isEqualOrEarlierVersionOf(context
				.getServerVersion().getEquivalentTLSVersion());
	}

	public static boolean isTLSv12(TlsContext context) {
		return ProtocolVersion.TLSv12.isEqualOrEarlierVersionOf(context
				.getServerVersion().getEquivalentTLSVersion());
	}
	...
	public static long readUint32(InputStream input) throws IOException {
		...
		return (((long) i1) << 24) | (((long) i2) << 16) | (((long) i3) << 8)
				| ((long) i4);
	}
	...
	public static long readUint48(InputStream input) throws IOException {
		...
		return (((long) i1) << 40) | (((long) i2) << 32) | (((long) i3) << 24)
				| (((long) i4) << 16) | (((long) i5) << 8) | ((long) i6);
	}
	...
	public static ASN1Primitive readDERObject(byte[] encoding)
			throws IOException {
		/*
		 * NOTE: The current ASN.1 parsing code can't enforce DER-only parsing,
		 * but since DER is canonical, we can check it by re-encoding the result
	...
	public static void writeVersion(ProtocolVersion version, byte[] buf,
			int offset) {
	...
	public static Vector getDefaultDSSSignatureAlgorithms() {
		return vectorOfOne(new SignatureAndHashAlgorithm(HashAlgorithm.sha1,
	...
	public static byte[] getExtensionData(Hashtable extensions,
			Integer extensionType) {
		return extensions == null ? null : (byte[]) extensions
				.get(extensionType);
	}

	public static boolean hasExpectedEmptyExtensionData(Hashtable extensions,
			Integer extensionType, short alertDescription) throws IOException {
	...
	public static boolean isSignatureAlgorithmsExtensionAllowed(
	 ...
	 * @throws IOException
	 */
	public static void addSignatureAlgorithmsExtension(Hashtable extensions,
			Vector supportedSignatureAlgorithms) throws IOException {
		extensions
				.put(EXT_signature_algorithms,
	 ...
	 * 
	 * @param extensions
	 *            A {@link Hashtable} to get the extension from, if it is
	 *            present.
	...
	public static Vector getSignatureAlgorithmsExtension(Hashtable extensions)
			throws IOException {
		byte[] extensionData = getExtensionData(extensions,
	...
	public static byte[] createSignatureAlgorithmsExtension(
		...
		encodeSupportedSignatureAlgorithms(supportedSignatureAlgorithms, false,
				buf);
	...
	public static void encodeSupportedSignatureAlgorithms(
			Vector supportedSignatureAlgorithms, boolean allowAnonymous,
			OutputStream output) throws IOException {
		if (supportedSignatureAlgorithms == null
				|| supportedSignatureAlgorithms.size() < 1
		...
		for (int i = 0; i < supportedSignatureAlgorithms.size(); ++i) {
			SignatureAndHashAlgorithm entry = (SignatureAndHashAlgorithm) supportedSignatureAlgorithms
					.elementAt(i);
			if (!allowAnonymous
					&& entry.getSignature() == SignatureAlgorithm.anonymous) {
				/*
				 * RFC 5246 7.4.1.4.1 The "anonymous" value is meaningless in
				 * this context but used in Section 7.4.3. It MUST NOT appear in
				 * this extension.
				 */
				throw new IllegalArgumentException(
	...
	public static Vector parseSupportedSignatureAlgorithms(
			boolean allowAnonymous, InputStream input) throws IOException {
		// supported_signature_algorithms
		...
		for (int i = 0; i < count; ++i) {
			SignatureAndHashAlgorithm entry = SignatureAndHashAlgorithm
					.parse(input);
			if (!allowAnonymous
					&& entry.getSignature() == SignatureAlgorithm.anonymous) {
				/*
	...
	public static byte[] PRF(TlsContext context, byte[] secret,
			String asciiLabel, byte[] seed, int size) {
		ProtocolVersion version = context.getServerVersion();

		if (version.isSSL()) {
			throw new IllegalStateException(
					"No PRF available for SSLv3 session");
		}
	...
	static byte[] PRF_legacy(byte[] secret, byte[] label, byte[] labelSeed,
			int size) {
	...
	static void hmac_hash(Digest digest, byte[] secret, byte[] seed, byte[] out) {
		...
		for (int i = 0; i < iterations; i++) {
			...
			mac.doFinal(buf2, 0);
			System.arraycopy(buf2, 0, out, (size * i),
					Math.min(size, out.length - (size * i)));
		}
	}

	static void validateKeyUsage(org.spongycastle.asn1.x509.Certificate c,
			int keyUsageBits) throws IOException {
	...
	static byte[] calculateKeyBlock(TlsContext context, int size) {
		...
		return PRF(context, master_secret, ExporterLabel.key_expansion, seed,
				size);
	}

	static byte[] calculateKeyBlock_SSL(byte[] master_secret, byte[] random,
			int size) {
	...
	static byte[] calculateMasterSecret(TlsContext context,
			byte[] pre_master_secret) {
		SecurityParameters securityParameters = context.getSecurityParameters();
		byte[] seed = concat(securityParameters.getClientRandom(),
		...
		return PRF(context, pre_master_secret, ExporterLabel.master_secret,
				seed, 48);
	}

	static byte[] calculateMasterSecret_SSL(byte[] pre_master_secret,
			byte[] random) {
		Digest md5 = new MD5Digest();
		Digest sha1 = new SHA1Digest();
		int md5Size = md5.getDigestSize();
	...
	static byte[] calculateVerifyData(TlsContext context, String asciiLabel,
			byte[] handshakeHash) {
		if (isSSL(context)) {
		...
		return PRF(context, master_secret, asciiLabel, handshakeHash,
				verify_data_length);
	}
	...
	public static final Digest clonePRFHash(int prfAlgorithm, Digest hash) {
		...
		default:
			return cloneHash(getHashAlgorithmForPRFAlgorithm(prfAlgorithm),
					hash);
		}
	}
	...
	public static final short getHashAlgorithmForPRFAlgorithm(int prfAlgorithm) {
		...
		case PRFAlgorithm.tls_prf_legacy:
			throw new IllegalArgumentException(
					"legacy PRF not a valid algorithm");
	...
	public static ASN1ObjectIdentifier getOIDForHashAlgorithm(
			short hashAlgorithm) {
		switch (hashAlgorithm) {
		case HashAlgorithm.md5:
	...
	static short getClientCertificateType(Certificate clientCertificate,
			Certificate serverCertificate) throws IOException {
		if (clientCertificate.isEmpty()) {
			return -1;
		}

		org.spongycastle.asn1.x509.Certificate x509Cert = clientCertificate
				.getCertificateAt(0);
		...
		try {
			...
			/*
			 * TODO RFC 5246 7.4.6. The certificates MUST be signed using an
			 * acceptable hash/ signature algorithm pair, as described in
			 * Section 7.4.4. Note that this relaxes the constraints on
			 * certificate-signing algorithms found in prior versions of TLS.
			 */
			...
			/*
			 * RSA public key; the certificate MUST allow the key to be used for
			 * signing with the signature scheme and hash algorithm that will be
			...
			/*
			 * DSA public key; the certificate MUST allow the key to be used for
			 * signing with the hash algorithm that will be employed in the
			...
			/*
			 * ECDSA-capable public key; the certificate MUST allow the key to
			 * be used for signing with the hash algorithm that will be employed
			 * in the certificate verify message; the public key MUST use a
			 * curve and point format supported by the server.
			 */
	...
	static void trackHashAlgorithms(TlsHandshakeHash handshakeHash,
	...
	public static TlsSigner createTlsSigner(short clientCertificateType) {
		...
		default:
			throw new IllegalArgumentException(
					"'clientCertificateType' is not a type with signing capability");
		}
	}
