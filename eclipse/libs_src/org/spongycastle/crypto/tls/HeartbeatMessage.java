...
public class HeartbeatMessage {
	...
	public HeartbeatMessage(short type, byte[] payload, int paddingLength) {
		if (!HeartbeatMessageType.isValid(type)) {
			throw new IllegalArgumentException(
					"'type' is not a valid HeartbeatMessageType value");
		}
		if (payload == null || payload.length >= (1 << 16)) {
			throw new IllegalArgumentException(
					"'payload' must have length < 2^16");
		}
		if (paddingLength < 16) {
			throw new IllegalArgumentException(
					"'paddingLength' must be at least 16");
		}
	...
	public static HeartbeatMessage parse(InputStream input) throws IOException {
		...
		if (payload == null) {
			/*
			 * RFC 6520 4. If the payload_length of a received HeartbeatMessage
			 * is too large, the received HeartbeatMessage MUST be discarded
			 * silently.
			 */
			return null;
		}
