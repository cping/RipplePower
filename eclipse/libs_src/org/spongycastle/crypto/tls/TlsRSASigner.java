...
public class TlsRSASigner extends AbstractTlsSigner {
	...
	public boolean verifyRawSignature(SignatureAndHashAlgorithm algorithm,
			byte[] sigBytes, AsymmetricKeyParameter publicKey, byte[] hash)
			throws CryptoException {
	...
	public Signer createSigner(SignatureAndHashAlgorithm algorithm,
		...
		return makeSigner(algorithm, false, true, new ParametersWithRandom(
	...
	protected Signer makeSigner(SignatureAndHashAlgorithm algorithm,
			boolean raw, boolean forSigning, CipherParameters cp) {
		...
		if (algorithm != null) {
			/*
			 * RFC 5246 4.7. In RSA signing, the opaque vector contains the
			 * signature generated using the RSASSA-PKCS1-v1_5 signature scheme
			 * defined in [PKCS1].
			 */
			s = new RSADigestSigner(d,
					TlsUtils.getOIDForHashAlgorithm(algorithm.getHash()));
		} else {
			/*
			 * RFC 5246 4.7. Note that earlier versions of TLS used a different
			 * RSA signature scheme that did not include a DigestInfo encoding.
			 */
	...
	protected AsymmetricBlockCipher createRSAImpl() {
		/*
		 * RFC 5264 7.4.7.1. Implementation note: It is now known that remote
		 * timing-based attacks on TLS are possible, at least when the client
		 * and server are on the same LAN. Accordingly, implementations that use
		 * static RSA keys MUST use RSA blinding or some other anti-timing
