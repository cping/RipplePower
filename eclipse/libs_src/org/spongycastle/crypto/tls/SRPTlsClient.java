...
public abstract class SRPTlsClient extends AbstractTlsClient {
	...
	public SRPTlsClient(TlsCipherFactory cipherFactory, byte[] identity,
			byte[] password) {
		super(cipherFactory);
		this.identity = Arrays.clone(identity);
		this.password = Arrays.clone(password);
	...
	public Hashtable getClientExtensions() throws IOException {
		Hashtable clientExtensions = TlsExtensionsUtils
				.ensureExtensionsInitialised(super.getClientExtensions());
	...
	public void processServerExtensions(Hashtable serverExtensions)
		...
		if (!TlsUtils.hasExpectedEmptyExtensionData(serverExtensions,
				TlsSRPUtils.EXT_SRP, AlertDescription.illegal_parameter)) {
			// No explicit guidance in RFC 5054 here; we allow an optional empty
			// extension from server
		}
	}
	...
	protected TlsKeyExchange createSRPKeyExchange(int keyExchange) {
		return new TlsSRPKeyExchange(keyExchange, supportedSignatureAlgorithms,
				identity, password);
	}
}
