...
public class CipherInputStream extends FilterInputStream {
	...
	/**
	 * Read data from underlying stream and process with cipher until end of
	 * stream or some data is available after cipher processing.
	 * 
	 * @return -1 to indicate end of stream, or the number of bytes (> 0)
	 *         available.
	 */
		...
		while (maxBuf == 0) {
			...
			try {
				if (bufferedBlockCipher != null) {
					maxBuf = bufferedBlockCipher.processBytes(inBuf, 0, read,
							buf, 0);
				} else if (aeadBlockCipher != null) {
					maxBuf = aeadBlockCipher.processBytes(inBuf, 0, read, buf,
							0);
				} else {
	...
	/**
	 * Reads data from the underlying stream and processes it with the cipher
	 * until the cipher outputs data, and returns the next available byte.
	 * <p/>
	 ...
	 *             if the data read from the stream was invalid ciphertext (e.g.
	 *             the cipher is an AEAD cipher and the ciphertext tag check
	 *             fails).
	 */
	...
	/**
	 * Reads data from the underlying stream and processes it with the cipher
	 * until the cipher outputs data, and then returns up to
	 * <code>b.length</code> bytes in the provided array.
	 * <p/>
	 * If the underlying stream is exhausted by this call, the cipher will be
	 * finalised.
	 * 
	...
	/**
	 * Reads data from the underlying stream and processes it with the cipher
	 * until the cipher outputs data, and then returns up to <code>len</code>
	...
	/**
	 * Closes the underlying input stream and finalises the processing of the
	 * data by the cipher.
	 * 
	 * @throws IOException
	 *             if there was an error closing the input stream.
