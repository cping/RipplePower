...
/**
 * A basic hash-committer based on the one described in
...
public class GeneralHashCommitter implements Committer {
	...
	/**
	 * Base Constructor. The maximum message length that can be committed to is
	 * half the length of the internal block size for the digest
	...
	public Commitment commit(byte[] message) {
		if (message.length > byteLength / 2) {
			throw new DataLengthException(
					"Message to be committed to too large for digest.");
		}
	...
	/**
	 * Return true if the passed in commitment represents a commitment to the
	 * passed in message.
	 * 
	 * @param commitment
	...
	public boolean isRevealed(Commitment commitment, byte[] message) {
		if (message.length + commitment.getSecret().length != byteLength) {
			throw new DataLengthException(
					"Message and witness secret lengths do not match.");
		}

		byte[] calcCommitment = calculateCommitment(commitment.getSecret(),
				message);

		return Arrays.constantTimeAreEqual(commitment.getCommitment(),
				calcCommitment);
	}
