...
public class PublicKeyFactory {
	...
	public static AsymmetricKeyParameter createKey(byte[] keyInfoData)
			throws IOException {
		return createKey(SubjectPublicKeyInfo.getInstance(ASN1Primitive
	...
	public static AsymmetricKeyParameter createKey(SubjectPublicKeyInfo keyInfo)
		...
		if (algId.getAlgorithm().equals(PKCSObjectIdentifiers.rsaEncryption)
			...
			RSAPublicKey pubKey = RSAPublicKey.getInstance(keyInfo
					.parsePublicKey());

			return new RSAKeyParameters(false, pubKey.getModulus(),
					pubKey.getPublicExponent());
		} else if (algId.getAlgorithm().equals(
				X9ObjectIdentifiers.dhpublicnumber)) {
			DHPublicKey dhPublicKey = DHPublicKey.getInstance(keyInfo
					.parsePublicKey());

			BigInteger y = dhPublicKey.getY().getValue();

			DHDomainParameters dhParams = DHDomainParameters.getInstance(algId
					.getParameters());
			...
			if (dhValidationParms != null) {
				...
				validation = new DHValidationParameters(seed,
						pgenCounter.intValue());
			}

			return new DHPublicKeyParameters(y, new DHParameters(p, g, q, j,
					validation));
		} else if (algId.getAlgorithm().equals(
			...
			DHParameters dhParams = new DHParameters(params.getP(),
					params.getG(), null, l);
			...
			ElGamalParameter params = new ElGamalParameter(
					(ASN1Sequence) algId.getParameters());
			ASN1Integer derY = (ASN1Integer) keyInfo.parsePublicKey();
			...
			return new ElGamalPublicKeyParameters(derY.getValue(),
				...
				|| algId.getAlgorithm()
						.equals(OIWObjectIdentifiers.dsaWithSHA1)) {
			ASN1Integer derY = (ASN1Integer) keyInfo.parsePublicKey();
			...
			if (de != null) {
				DSAParameter params = DSAParameter.getInstance(de
						.toASN1Primitive());
				parameters = new DSAParameters(params.getP(), params.getQ(),
						params.getG());
			}
			...
			X962Parameters params = X962Parameters.getInstance(algId
					.getParameters());
			...
			ASN1OctetString key = new DEROctetString(keyInfo.getPublicKeyData()
					.getBytes());
			X9ECPoint derQ = new X9ECPoint(x9.getCurve(), key);

			// TODO We lose any named parameters here

			ECDomainParameters dParams = new ECDomainParameters(x9.getCurve(),
