...
public class CCMBlockCipher implements AEADBlockCipher {
	...
	public void init(boolean forEncryption, CipherParameters params)
		...
		// NOTE: Very basic support for key re-use, but no performance gain from
		// it
	 ...
	 */
	public int processPacket(byte[] in, int inOff, int inLen, byte[] output,
			int outOff) throws IllegalStateException,
			InvalidCipherTextException, DataLengthException {
		...
		if (forEncryption) {
			...
			System.arraycopy(block, 0, output, outIndex, inLen + inOff
					- inIndex);
			...
			System.arraycopy(in, inIndex, block, 0, outputLen
					- (inIndex - inOff));

			ctrCipher.processBlock(block, 0, block, 0);

			System.arraycopy(block, 0, output, outIndex, outputLen
					- (inIndex - inOff));
	...
	private int calculateMac(byte[] data, int dataOff, int dataLen,
		...
		if (hasAssociatedText()) {
			...
			if (initialAssociatedText != null) {
				cMac.update(initialAssociatedText, 0,
						initialAssociatedText.length);
			}
			if (associatedText.size() > 0) {
				cMac.update(associatedText.getBuffer(), 0,
						associatedText.size());
			}
	...
	private int getAssociatedTextLength() {
		return associatedText.size()
				+ ((initialAssociatedText == null) ? 0
						: initialAssociatedText.length);
	}
