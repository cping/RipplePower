...
public class CTSBlockCipher extends BufferedBlockCipher {
	...
	public CTSBlockCipher(BlockCipher cipher) {
		if ((cipher instanceof OFBBlockCipher)
			...
			// TODO: This is broken - need to introduce marker interface to
			// differentiate block cipher primitive from mode?
	 ...
	 *                if cipher text decrypts wrongly (in case the exception
		...
		if (forEncryption) {
			if (bufOff < blockSize) {
				throw new DataLengthException(
						"need at least one block of input for CTS");
			}
			...
			if (bufOff > blockSize) {
				...
				if (cipher instanceof CBCBlockCipher) {
					BlockCipher c = ((CBCBlockCipher) cipher)
							.getUnderlyingCipher();
