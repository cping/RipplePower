		...
		if (!this.forEncryption) {
			...
			for (int i = 0; i < primes.size(); i++) {
				...
				for (int j = 1; j < actualPrimeValue; j++) {
					...
					lookup[i].addElement(priv.getG().modPow(comp,
							priv.getModulus()));
				}
			}
		}
	...
	public byte[] processBlock(byte[] in, int inOff, int len)
		...
		if (key == null) {
			throw new IllegalStateException(
					"NaccacheStern engine not initialised");
		}
		...
		if (forEncryption) {
			...
			for (int i = 0; i < primes.size(); i++) {
				BigInteger exp = input.modPow(
						priv.getPhi_n()
								.divide((BigInteger) primes.elementAt(i)), priv
								.getModulus());
				Vector al = lookup[i];
				if (lookup[i].size() != ((BigInteger) primes.elementAt(i))
						.intValue()) {
					if (debug) {
						System.out.println("Prime is " + primes.elementAt(i)
								+ ", lookup table has size " + al.size());
					}
					throw new InvalidCipherTextException(
							"Error in lookup Array for "
									+ ((BigInteger) primes.elementAt(i))
											.intValue()
				...
				if (lookedup == -1) {
					if (debug) {
						System.out.println("Actual prime is "
								+ primes.elementAt(i));
						System.out.println("Decrypted value is " + exp);

						System.out.println("LookupList for "
								+ primes.elementAt(i) + " with size "
								+ lookup[i].size() + " is: ");
