...
public class IESEngine {
	 ...
	 */
	public void init(boolean forEncryption, CipherParameters privParam,
			CipherParameters pubParam, CipherParameters param) {
	...
	public void init(AsymmetricKeyParameter privateKey,
			CipherParameters params, KeyParser publicKeyParser) {
	...
	private byte[] decryptBlock(byte[] in_enc, int inOff, int inLen)
		...
		if (cipher == null) {
			...
			M = new byte[cipher.getOutputSize(inLen - V.length
					- mac.getMacSize())];
			len = cipher.processBytes(in_enc, inOff + V.length, inLen
					- V.length - mac.getMacSize(), M, 0);
	...
	public byte[] processBlock(byte[] in, int inOff, int inLen)
		...
		if (forEncryption) {
			...
			if (keyParser != null) {
				ByteArrayInputStream bIn = new ByteArrayInputStream(in, inOff,
						inLen);
				...
				try {
					...
					throw new InvalidCipherTextException(
							"unable to recover ephemeral public key: "
									+ e.getMessage(), e);
				}
		...
		return forEncryption ? encryptBlock(in, inOff, inLen) : decryptBlock(
				in, inOff, inLen);
	}
}
