...
public class Shacal2Engine implements BlockCipher {
	...
	private int[] workingKey = null; // expanded key: corresponds to the message
										// block W in FIPS PUB 180-2
	...
	public void setKey(byte[] kb) {
		if (kb.length == 0 || kb.length > 64 || kb.length < 16
				|| kb.length % 8 != 0) {
			throw new IllegalArgumentException(
					"Shacal2-key must be 16 - 64 bytes and multiple of 8");
		}

		bytes2ints(kb, workingKey, 0, 0);

		for (int i = 16; i < 64; i++) { // Key-Expansion, implicitly
										// Zero-Padding for 16 > i > kb.length/4
			workingKey[i] = ((workingKey[i - 2] >>> 17 | workingKey[i - 2] << -17) // corresponds
																					// to
																					// ROTL
																					// n(x)
																					// of
	...
	public void encryptBlock(byte[] in, int inOffset, byte[] out, int outOffset) {
		int[] block = new int[BLOCK_SIZE / 4];// corresponds to working
												// variables a,b,c,d,e,f,g,h of
												// FIPS PUB 180-2
		bytes2ints(in, block, inOffset, 0);

		for (int i = 0; i < ROUNDS; i++) {
			int tmp = (((block[4] >>> 6) | (block[4] << -6))
					^ ((block[4] >>> 11) | (block[4] << -11)) ^ ((block[4] >>> 25) | (block[4] << -25)))
					+ ((block[4] & block[5]) ^ ((~block[4]) & block[6]))
					+ block[7] + K[i] + workingKey[i]; // corresponds to T1 of
														// FIPS PUB 180-2
			block[7] = block[6];
			block[6] = block[5];
			block[5] = block[4];
			...
			block[0] = tmp
					+ (((block[0] >>> 2) | (block[0] << -2))
							^ ((block[0] >>> 13) | (block[0] << -13)) ^ ((block[0] >>> 22) | (block[0] << -22)))
					+ ((block[0] & block[2]) ^ (block[0] & block[3]) ^ (block[2] & block[3]));
			// corresponds to T2 of FIPS PUB 180-2, block[1] and block[2]
			// replaced
		}
	...
	public void decryptBlock(byte[] in, int inOffset, byte[] out, int outOffset) {
		...
		for (int i = ROUNDS - 1; i > -1; i--) {
			int tmp = block[0]
					- (((block[1] >>> 2) | (block[1] << -2))
							^ ((block[1] >>> 13) | (block[1] << -13)) ^ ((block[1] >>> 22) | (block[1] << -22)))
	...
	private void bytes2ints(byte[] bytes, int[] block, int bytesPos,
			int blockPos) {
