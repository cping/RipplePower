...
public class GOST3410ParametersGenerator {
	...
	private int procedure_A(int x0, int c, BigInteger[] pq, int size) {
		...
		for (int i = 0; i < s; i++) {
			...
			step6: for (;;) {
				...
				for (int j = 0; j < rm; j++) {
					y[j + 1] = (y[j].multiply(constA16).add(C))
							.mod(TWO.pow(16));
				}
				...
				BigInteger N = TWO
						.pow(t[m] - 1)
						.divide(p[m + 1])
						.add((TWO.pow(t[m] - 1).multiply(Ym)).divide(p[m + 1]
								.multiply(TWO.pow(16 * rm))));
				...
				step11: for (;;) {
					...
					if ((TWO.modPow(
							...
							p[m]).compareTo(ONE) == 0)
							&& (TWO.modPow(N.add(BigInteger.valueOf(k)), p[m])
									.compareTo(ONE) != 0)) {
						m -= 1;
						break;
					} else {
	...
	private long procedure_Aa(long x0, long c, BigInteger[] pq, int size) {
		...
		// step3
		BigInteger p[] = new BigInteger[s + 1];
		p[s] = new BigInteger("8000000B", 16); // set min prime number length 32
												// bit
		...
		for (int i = 0; i < s; i++) {
			...
			step6: for (;;) {
				...
				for (int j = 0; j < rm; j++) {
					y[j + 1] = (y[j].multiply(constA32).add(C))
							.mod(TWO.pow(32));
				}
	...
	private void procedure_B(int x0, int c, BigInteger[] pq) {
		...
		step3: for (;;) {
			...
			BigInteger N = TWO
					.pow(tp - 1)
					.divide(q.multiply(Q))
					.add((TWO.pow(tp - 1).multiply(Y)).divide(q.multiply(Q)
							.multiply(TWO.pow(1024))));
			...
			step8: for (;;) {
				// step 11
				p = q.multiply(Q).multiply(N.add(BigInteger.valueOf(k)))
						.add(ONE);
				...
				// step10
				if ((TWO.modPow(
						q.multiply(Q).multiply(N.add(BigInteger.valueOf(k))), p)
						.compareTo(ONE) == 0)
						&& (TWO.modPow(
		...
		if (typeproc == 1) {
			...
			default:
				throw new IllegalArgumentException(
						"Ooops! key size 512 or 1024 bit.");
			}
			p = pq[0];
			q = pq[1];
			a = procedure_C(p, q);
			...
			return new GOST3410Parameters(p, q, a,
					new GOST3410ValidationParameters(x0, c));
		} else {
			...
			// System.out.println("p:"+p+"\n"+"q:"+q+"\n"+"a:"+a);
			return new GOST3410Parameters(p, q, a,
					new GOST3410ValidationParameters(x0L, cL));
		}
	}
}
