 ...
 * specification.
 */
public class KDFDoublePipelineIterationBytesGenerator implements
	...
	public void init(DerivationParameters params) {
		...
		if (dpiParams.useCounter()) {
			...
			this.maxSizeExcl = maxSize.compareTo(INTEGER_MAX) == 1 ? Integer.MAX_VALUE
					: maxSize.intValue();
		} else {
	...
	public int generateBytes(byte[] out, int outOff, int len)
		...
		if (generatedBytesAfter < 0 || generatedBytesAfter >= maxSizeExcl) {
			throw new DataLengthException(
					"Current KDFCTR may only be used for " + maxSizeExcl
							+ " bytes");
		}
