 ...
 * domain <a
 * href="https://github.com/floodyberry/poly1305-donna">poly1305-donna-
 * unrolled</a> C implementation by Andrew M (@floodyberry).
 * 
...
public class Poly1305 implements Mac {
	 ...
	 * 
	 * @param a
	 *            {@link ParametersWithIV} containing a 128 bit nonce and a
	 *            {@link KeyParameter} with a 256 bit key complying to the
	 *            {@link Poly1305KeyGenerator Poly1305 key format}.
	 */
	public void init(final CipherParameters params)
		...
		if ((params instanceof ParametersWithIV)
				&& ((ParametersWithIV) params).getParameters() instanceof KeyParameter) {
			nonce = ((ParametersWithIV) params).getIV();
			key = ((KeyParameter) ((ParametersWithIV) params).getParameters())
					.getKey();
		} else {
			throw new IllegalArgumentException(
					"Poly1305 requires a key and and IV.");
		}
	...
	private void setKey(final byte[] key, final byte[] nonce) {
		if (nonce.length != BLOCK_SIZE) {
			throw new IllegalArgumentException(
					"Poly1305 requires a 128 bit IV.");
		}
	...
	public void update(final byte[] in, final int inOff, final int len)
		...
		while (len > copied) {
			...
			int toCopy = Math.min((len - copied), BLOCK_SIZE
					- currentBlockOffset);
			System.arraycopy(in, copied + inOff, currentBlock,
	...
	private void processBlock() {
		...
		long tp0 = mul32x32_64(h0, r0) + mul32x32_64(h1, s4)
				+ mul32x32_64(h2, s3) + mul32x32_64(h3, s2)
				+ mul32x32_64(h4, s1);
		long tp1 = mul32x32_64(h0, r1) + mul32x32_64(h1, r0)
				+ mul32x32_64(h2, s4) + mul32x32_64(h3, s3)
				+ mul32x32_64(h4, s2);
		long tp2 = mul32x32_64(h0, r2) + mul32x32_64(h1, r1)
				+ mul32x32_64(h2, r0) + mul32x32_64(h3, s4)
				+ mul32x32_64(h4, s3);
		long tp3 = mul32x32_64(h0, r3) + mul32x32_64(h1, r2)
				+ mul32x32_64(h2, r1) + mul32x32_64(h3, r0)
				+ mul32x32_64(h4, s4);
		long tp4 = mul32x32_64(h0, r4) + mul32x32_64(h1, r3)
