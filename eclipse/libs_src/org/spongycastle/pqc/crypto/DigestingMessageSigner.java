...
/**
 * Implements the sign and verify functions for a Signature Scheme which can use
 * a hash function.
 */
public class DigestingMessageSigner implements Signer {
	...
	public byte[] generateSignature() {
		if (!forSigning) {
			throw new IllegalStateException(
					"RainbowDigestSigner not initialised for signature generation.");
		}
	...
	public boolean verify(byte[] signature) {
		if (forSigning) {
			throw new IllegalStateException(
					"RainbowDigestSigner not initialised for verification");
		}
