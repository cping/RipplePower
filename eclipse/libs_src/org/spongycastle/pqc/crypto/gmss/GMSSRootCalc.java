...
public class GMSSRootCalc {
	...
	public GMSSRootCalc(Digest digest, byte[][] statByte, int[] statInt,
		...
		for (int i = 0; i < tailLength; i++) {
			this.heightOfNodes.addElement(Integers.valueOf(statInt[8
					+ heightOfTree + i]));
		}
	 ...
	 */
	public GMSSRootCalc(int heightOfTree, int K,
	...
	public void initialize(Vector sharedStack) {
		...
		for (int i = 0; i < this.heightOfTree - this.K; i++) {
			this.treehash[i] = new Treehash(sharedStack, i,
	...
	public byte[][] getStatByte() {
		...
		byte[][] statByte = new byte[1 + heightOfTree + tailLength][64]; // FIXME:
																			// messDigestTree.getByteLength()
		statByte[0] = root;
