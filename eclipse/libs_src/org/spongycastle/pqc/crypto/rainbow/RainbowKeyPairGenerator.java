 ...
 */
public class RainbowKeyPairGenerator implements
	...
	private short[][] A1; // matrix of the lin. affine map L1(n-v1 x n-v1
							// matrix)
	...
	/**
	 * The standard constructor tries to generate the Rainbow algorithm
	...
	public AsymmetricCipherKeyPair genKeyPair() {
		...
		privKey = new RainbowPrivateKeyParameters(A1inv, b1, A2inv, b2, vi,
				layers);

		/* marshall to PublicKey */
		pubKey = new RainbowPublicKeyParameters(vi[vi.length - 1] - vi[0],
				pub_quadratic, pub_singular, pub_scalar);
	...
	private void initializeDefault() {
		RainbowKeyGenerationParameters rbKGParams = new RainbowKeyGenerationParameters(
				new SecureRandom(), new RainbowParameters());
		initialize(rbKGParams);
	}

	/**
	 * This function calls the functions for the random generation of the
	 * coefficients and the matrices needed for the private key and the method
	 * for computing the public key.
	 */
	private void keygen() {
		generateL1();
		generateL2();
				...
				for (int x1 = 0; x1 < oils; x1++) {
					for (int x2 = 0; x2 < vins; x2++) {
						...
						coeff_quadratic_3dim[crnt_row + p] = c.addSquareMatrix(
								coeff_quadratic_3dim[crnt_row + p],
		...
		// Apply L1 = A1*x+b1 to composition of F and L2
		{
			...
			for (int r = 0; r < rows; r++) {
				for (int q = 0; q < A1.length; q++) {
					tmp_c_quad[r] = c.addSquareMatrix(tmp_c_quad[r],
							c.multMatrix(A1[r][q], coeff_quadratic_3dim[q]));
					tmp_c_sing[r] = c.addVect(tmp_c_sing[r],
							c.multVect(A1[r][q], this.pub_singular[q]));
					tmp_c_scal[r] = GF2Field.addElem(tmp_c_scal[r],
							GF2Field.multElem(A1[r][q], this.pub_scalar[q]));
				}
