...
public class NTRUEngine implements AsymmetricBlockCipher {
	...
	public void init(boolean forEncryption, CipherParameters parameters) {
		...
		if (forEncryption) {
			if (parameters instanceof ParametersWithRandom) {
				...
				this.pubKey = (NTRUEncryptionPublicKeyParameters) p
						.getParameters();
			} else {
	...
	private byte[] encrypt(byte[] m, NTRUEncryptionPublicKeyParameters pubKey) {
		...
		if (maxLenBytes > 255) {
			throw new IllegalArgumentException(
					"llen values bigger than 1 are not supported");
		}
		if (l > maxLenBytes) {
			throw new DataLengthException("Message too long: " + l + ">"
					+ maxLenBytes);
		}
	...
	private byte[] buildSData(byte[] oid, byte[] m, int l, byte[] b,
			byte[] hTrunc) {
		...
		System.arraycopy(hTrunc, 0, sData, oid.length + m.length + b.length,
				hTrunc.length);
		return sData;
	}

	protected IntegerPolynomial encrypt(IntegerPolynomial m,
			TernaryPolynomial r, IntegerPolynomial pubKey) {
	...
	/**
	 * Deterministically generates a blinding polynomial from a seed and a
	...
	/**
	 * Generates an <code>int</code> array containing <code>dr</code> elements
	 * equal to <code>1</code> and <code>dr</code> elements equal to
	 * <code>-1</code> using an index generator.
	 * 
	 * @param ig
	 *            an index generator
	 * @param dr
	 ...
	 * @return
	 */
	private IntegerPolynomial MGF(byte[] seed, int N, int minCallsR,
			boolean hashSeed) {
	 ...
	 * @throws InvalidCipherTextException
	 *             if the encrypted data is invalid, or <code>maxLenBytes</code>
	 *             is greater than 255
	 */
	private byte[] decrypt(byte[] data,
		...
		if (maxMsgLenBytes > 255) {
			throw new DataLengthException(
					"maxMsgLenBytes values bigger than 255 are not supported");
		}
		...
		if (ci.count(-1) < dm0) {
			throw new InvalidCipherTextException(
					"Less than dm0 coefficients equal -1");
		}
		if (ci.count(0) < dm0) {
			throw new InvalidCipherTextException(
					"Less than dm0 coefficients equal 0");
		}
		if (ci.count(1) < dm0) {
			throw new InvalidCipherTextException(
					"Less than dm0 coefficients equal 1");
		...
		if (cl > maxMsgLenBytes) {
			throw new InvalidCipherTextException("Message too long: " + cl
					+ ">" + maxMsgLenBytes);
		}
		byte[] cm = new byte[cl];
		...
		if (!Arrays.areEqual(p0, new byte[p0.length])) {
			throw new InvalidCipherTextException(
					"The message is not followed by zeroes");
		}
	...
	protected IntegerPolynomial decrypt(IntegerPolynomial e, Polynomial priv_t,
		...
		IntegerPolynomial c = params.fastFp ? a : new DenseTernaryPolynomial(a)
				.mult(priv_fp, 3);
		c.center0(3);
		return c;
	}
