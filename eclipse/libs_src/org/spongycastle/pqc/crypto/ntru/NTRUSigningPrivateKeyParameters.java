...
public class NTRUSigningPrivateKeyParameters extends AsymmetricKeyParameter {
	 ...
	 *            the NtruSign parameters to use
	 */
	public NTRUSigningPrivateKeyParameters(InputStream is,
			NTRUSigningKeyGenerationParameters params) throws IOException {
		super(true);
		...
		publicKey = new NTRUSigningPublicKeyParameters(is,
	...
	public boolean equals(Object obj) {
		...
		for (int i = 0; i < bases.size(); i++) {
			...
			if (i != 0 && !basis1.h.equals(basis2.h)) // don't compare h for the
														// 0th basis
			{
				return false;
			}
	...
	public static class Basis {
		 ...
		 *            NtruSign parameters
		 * @param include_h
		 *            whether to read the polynomial <code>h</code> (
		 *            <code>true</code>) or only <code>f</code> and
		 *            <code>f'</code> (<code>false</code>)
		 */
		Basis(InputStream is, NTRUSigningKeyGenerationParameters params,
				boolean include_h) throws IOException {
			int N = params.N;
			int q = params.q;
			int d1 = params.d1;
			int d2 = params.d2;
			...
			if (params.polyType == NTRUParameters.TERNARY_POLYNOMIAL_TYPE_PRODUCT) {
				...
				IntegerPolynomial fInt = IntegerPolynomial.fromBinary3Tight(is,
						N);
				f = sparse ? new SparseTernaryPolynomial(fInt)
						: new DenseTernaryPolynomial(fInt);
			}

			if (params.basisType == NTRUSigningKeyGenerationParameters.BASIS_TYPE_STANDARD) {
				IntegerPolynomial fPrimeInt = IntegerPolynomial.fromBinary(is,
						N, q);
				...
				fPrime = ProductFormPolynomial.fromBinary(is, N, d1, d2,
						d3 + 1, d3);
			} else {
		 ...
		 *            an output stream
		 * @param include_h
		 *            whether to write the polynomial <code>h</code> (
		 *            <code>true</code>) or only <code>f</code> and
		 *            <code>f'</code> (<code>false</code>)
		 * @throws IOException
		 */
