...
public class NTRUSigningKeyPairGenerator implements
	...
	public AsymmetricCipherKeyPair generateKeyPair() {
		...
		for (int k = params.B; k >= 0; k--) {
			...
			try {
				...
				if (k == params.B) {
					pub = new NTRUSigningPublicKeyParameters(basis.get().h,
							params.getSigningParameters());
				}
			} catch (Exception e) {
	...
	public AsymmetricCipherKeyPair generateKeyPairSingleThread() {
		...
		for (int k = params.B; k >= 0; k--) {
			...
			basises.add(basis);
			if (k == 0) {
				pub = new NTRUSigningPublicKeyParameters(basis.h,
						params.getSigningParameters());
			}
		}
		NTRUSigningPrivateKeyParameters priv = new NTRUSigningPrivateKeyParameters(
	...
	/**
	 * Implementation of the optional steps 20 through 26 in EESS1v2.pdf,
	 * section 3.5.1.1. This doesn't seem to have much of an effect and
	 * sometimes actually increases the norm of F, but on average it slightly
	 * reduces the norm.<br/>
	 ...
	 * @param N
	 */
	private void minimizeFG(IntegerPolynomial f, IntegerPolynomial g,
			IntegerPolynomial F, IntegerPolynomial G, int N) {
		int E = 0;
		for (int j = 0; j < N; j++) {
			E += 2 * N
					* (f.coeffs[j] * f.coeffs[j] + g.coeffs[j] * g.coeffs[j]);
		}
	 ...
	 * If <code>KeyGenAlg=FLOAT</code>, the basis may not be valid and this
	 * method must be rerun if that is the case.<br/>
	 * 
	...
	private FGBasis generateBasis() {
		...
		do {
			do {
				f = params.polyType == NTRUParameters.TERNARY_POLYNOMIAL_TYPE_SIMPLE ? DenseTernaryPolynomial
						.generateRandom(N, d + 1, d, new SecureRandom())
						: ProductFormPolynomial.generateRandom(N, d1, d2,
								d3 + 1, d3, new SecureRandom());
		...
		do {
			do {
				do {
					g = params.polyType == NTRUParameters.TERNARY_POLYNOMIAL_TYPE_SIMPLE ? DenseTernaryPolynomial
							.generateRandom(N, d + 1, d, new SecureRandom())
							: ProductFormPolynomial.generateRandom(N, d1, d2,
									d3 + 1, d3, new SecureRandom());
		...
		if (params.keyGenAlg == NTRUSigningKeyGenerationParameters.KEY_GEN_ALG_RESULTANT) {
			...
			C = fRev.mult(B); // fRev.mult(B) is actually faster than new
								// SparseTernaryPolynomial(fRev).mult(B),
								// possibly due to cache locality?
			C.add(gRev.mult(A));
			C = C.mult(rt.rho);
			C.div(rt.res);
			...
			// * Cdec needs to be accurate to 1 decimal place so it can be
			// correctly rounded;
			// * fInv loses up to (#digits of longest coeff of B) places in
			// fInv.mult(B);
			// * multiplying fInv by B also multiplies the rounding error by a
			// factor of N;
	...
	private class BasisGenerationTask implements
			Callable<NTRUSigningPrivateKeyParameters.Basis> {
	...
	public class FGBasis extends NTRUSigningPrivateKeyParameters.Basis {
		...
		FGBasis(Polynomial f, Polynomial fPrime, IntegerPolynomial h,
				IntegerPolynomial F, IntegerPolynomial G,
		...
		/**
		 * Returns <code>true</code> if the norms of the polynomials
		 * <code>F</code> and <code>G</code> are within
		 * {@link NTRUSigningKeyGenerationParameters#keyNormBound}.
		 * 
		 * @return
		 */
		boolean isNormOk() {
