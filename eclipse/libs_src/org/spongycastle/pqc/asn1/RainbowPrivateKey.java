...
public class RainbowPrivateKey extends ASN1Object {
	...
	private RainbowPrivateKey(ASN1Sequence seq) {
		...
		for (int l = 0; l < asnLayers.size(); l++) {
			...
			for (int m = 0; m < alphas3d.size(); m++) {
				...
				for (int n = 0; n < alphas2d.size(); n++) {
					alphas[l][m][n] = ((ASN1OctetString) alphas2d
							.getObjectAt(n)).getOctets();
				}
			}
			...
			for (int mb = 0; mb < betas3d.size(); mb++) {
				...
				for (int nb = 0; nb < betas2d.size(); nb++) {
					betas[l][mb][nb] = ((ASN1OctetString) betas2d
							.getObjectAt(nb)).getOctets();
				}
			}
			...
			for (int mg = 0; mg < gammas2d.size(); mg++) {
				gammas[l][mg] = ((ASN1OctetString) gammas2d.getObjectAt(mg))
						.getOctets();
			}
		...
		for (int i = 0; i < numOfLayers; i++) {
			Layer l = new Layer(vi[i], vi[i + 1],
					RainbowUtil.convertArray(alphas[i]),
					RainbowUtil.convertArray(betas[i]),
					RainbowUtil.convertArray(gammas[i]),
	...
	public ASN1Primitive toASN1Primitive() {
		...
		for (int l = 0; l < layers.length; l++) {
			...
			// eta
			aLayer.add(new DEROctetString(RainbowUtil.convertArray(layers[l]
					.getCoeffEta())));
