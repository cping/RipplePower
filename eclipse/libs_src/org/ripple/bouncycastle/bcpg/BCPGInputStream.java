	...
	public Packet readPacket() throws IOException {
		...
		if (newPacket) {
			...
			case 1:
				...
				break;
			case 2:
				bodyLen = (this.read() << 24) | (this.read() << 16)
		...
		if (bodyLen == 0 && partial) {
			...
			objStream = new BCPGInputStream(new PartialInputStream(this,
					partial, bodyLen));
		}
	...
	private static class PartialInputStream extends InputStream {
		...
		private int loadDataLength() throws IOException {
			...
			if (l < 192) {
				...
				dataLength = (in.read() << 24) | (in.read() << 16)
						| (in.read() << 8) | in.read();
			} else {
				partial = true;
				dataLength = 1 << (l & 0x1f);
		...
		public int read(byte[] buf, int offset, int len) throws IOException {
			do {
				if (dataLength != 0) {
					int readLen = (dataLength > len || dataLength < 0) ? len
							: dataLength;
