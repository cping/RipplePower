 ...
 * Basic output stream.
 */
public class BCPGOutputStream extends OutputStream implements PacketTags,
	...
	private static final int BUF_SIZE_POWER = 16; // 2^16 size buffer on long
													// files
	...
	public BCPGOutputStream(OutputStream out, int tag, long length,
			boolean oldFormat) throws IOException {
		this.out = out;
	...
	public BCPGOutputStream(OutputStream out, int tag, byte[] buffer)
		...
		if (partialPower > 30) {
			throw new IOException(
					"Buffer cannot be greater than 2^30 in length.");
		}
	...
	private void writeHeader(int tag, boolean oldPackets, boolean partial,
			long bodyLen) throws IOException {
		int hdr = 0x80;
	...
	private void writePartial(byte[] buf, int off, int len) throws IOException {
		...
		if (len <= (partialBufferLength - partialOffset)) {
			...
			System.arraycopy(buf, off, partialBuffer, partialOffset,
					partialBufferLength - partialOffset);
			off += partialBufferLength - partialOffset;
			len -= partialBufferLength - partialOffset;
			partialFlush(false);

			while (len > partialBufferLength) {
				System.arraycopy(buf, off, partialBuffer, 0,
						partialBufferLength);
