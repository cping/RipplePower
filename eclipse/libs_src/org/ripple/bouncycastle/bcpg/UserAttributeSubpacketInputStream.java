 ...
 */
public class UserAttributeSubpacketInputStream extends InputStream implements
	...
	public UserAttributeSubpacket readPacket() throws IOException {
		...
		if (l < 192) {
			...
			bodyLen = (in.read() << 24) | (in.read() << 16) | (in.read() << 8)
					| in.read();
		} else {
			// TODO Error?
		}
		...
		if (tag < 0) {
			throw new EOFException(
					"unexpected EOF reading user attribute sub packet");
		}
