 ...
 */
public class SecretKeyPacket extends ContainedPacket implements
	...
	SecretKeyPacket(BCPGInputStream in) throws IOException {
		...
		if (!(s2k != null && s2k.getType() == S2K.GNU_DUMMY_S2K && s2k
				.getProtectionMode() == 0x01)) {
			if (s2kUsage != 0) {
				if (encAlgorithm < 7) {
					iv = new byte[8];
				} else {
	 ...
	 * @param secKeyData
	 */
	public SecretKeyPacket(PublicKeyPacket pubKeyPacket, int encAlgorithm,
	...
	public SecretKeyPacket(PublicKeyPacket pubKeyPacket, int encAlgorithm,
			int s2kUsage, S2K s2k, byte[] iv, byte[] secKeyData) {
		this.pubKeyPacket = pubKeyPacket;
		this.encAlgorithm = encAlgorithm;
