 ...
 */
public class SignaturePacket extends ContainedPacket implements
	...
	SignaturePacket(BCPGInputStream in) throws IOException {
		...
		if (version == 3 || version == 2) {
			...
			creationTime = (((long) in.read() << 24) | (in.read() << 16)
					| (in.read() << 8) | in.read()) * 1000;
			...
			for (int i = 0; i != hashedData.length; i++) {
				...
				if (p instanceof IssuerKeyID) {
					...
					creationTime = ((SignatureCreationTime) p).getTime()
							.getTime();
				}
			...
			sIn = new SignatureSubpacketInputStream(new ByteArrayInputStream(
					unhashed));
		...
		case ELGAMAL_GENERAL:
			...
			break;
		default:
			if (keyAlgorithm >= PublicKeyAlgorithmTags.EXPERIMENTAL_1
					&& keyAlgorithm <= PublicKeyAlgorithmTags.EXPERIMENTAL_11) {
				signature = null;
	 ...
	 * @param signature
	 */
	public SignaturePacket(int signatureType, long keyID, int keyAlgorithm,
			int hashAlgorithm, SignatureSubpacket[] hashedData,
			SignatureSubpacket[] unhashedData, byte[] fingerPrint,
			MPInteger[] signature) {
		this(4, signatureType, keyID, keyAlgorithm, hashAlgorithm, hashedData,
	 ...
	 * @param signature
	 */
	public SignaturePacket(int version, int signatureType, long keyID,
			int keyAlgorithm, int hashAlgorithm, long creationTime,
			byte[] fingerPrint, MPInteger[] signature) {
		this(version, signatureType, keyID, keyAlgorithm, hashAlgorithm, null,
				null, fingerPrint, signature);
	...
	public SignaturePacket(int version, int signatureType, long keyID,
			...
			SignatureSubpacket[] hashedData, SignatureSubpacket[] unhashedData,
			byte[] fingerPrint, MPInteger[] signature) {
	...
	private void setCreationTime() {
		for (int i = 0; i != hashedData.length; i++) {
			if (hashedData[i] instanceof SignatureCreationTime) {
				creationTime = ((SignatureCreationTime) hashedData[i])
						.getTime().getTime();
				break;
			}
		}
