...
public class RSAKeyPairGenerator {
	private static void exportKeyPair(OutputStream secretOut,
		...
		PGPKeyPair keyPair = new PGPKeyPair(PGPPublicKey.RSA_GENERAL,
				publicKey, privateKey, new Date());
		PGPSecretKey secretKey = new PGPSecretKey(
				PGPSignature.DEFAULT_CERTIFICATION, keyPair, identity,
				sha1Calc, null, null,
	...
	public static void main(String[] args) throws Exception {
		...
		if (args[0].equals("-a")) {
			if (args.length < 3) {
				System.out
						.println("RSAKeyPairGenerator [-a] identity passPhrase");
				System.exit(0);
			}
			...
			exportKeyPair(out1, out2, kp.getPublic(), kp.getPrivate(), args[1],
			...
			exportKeyPair(out1, out2, kp.getPublic(), kp.getPrivate(), args[0],
