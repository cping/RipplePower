...
public class PGPSignature {
	...
	public void initVerify(PGPPublicKey pubKey, Provider provider)
			throws PGPException {
		init(new JcaPGPContentVerifierBuilderProvider().setProvider(provider),
				pubKey);
	}

	public void init(PGPContentVerifierBuilderProvider verifierBuilderProvider,
			PGPPublicKey pubKey) throws PGPException {
		PGPContentVerifierBuilder verifierBuilder = verifierBuilderProvider
				.get(sigPck.getKeyAlgorithm(), sigPck.getHashAlgorithm());
	...
	public boolean verifyCertification(
			PGPUserAttributeSubpacketVector userAttributes, PGPPublicKey key)
			throws PGPException, SignatureException {
		if (verifier == null) {
			throw new PGPException(
					"PGPSignature not initialised - call init().");
		}
	...
	/**
	 * Verify a certification for the passed in key against the passed in master
	 * key.
	 * 
	 * @param masterKey
	...
	private void addTrailer() throws SignatureException {
		try {
	 ...
	 * 
	 * @return true if either hashed or unhashed subpackets are present, false
	 *         otherwise.
	 */
	...
	private PGPSignatureSubpacketVector createSubpacketVector(
			SignatureSubpacket[] pcks) {
		if (pcks != null) {
	...
	public byte[] getSignature() throws PGPException {
		...
		if (sigValues != null) {
			...
			{
				signature = BigIntegers.asUnsignedByteArray(sigValues[0]
						.getValue());
			} else {
				try {
