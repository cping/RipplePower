...
public abstract class PGPKeyRing {
	...
	static TrustPacket readOptionalTrustPacket(BCPGInputStream pIn)
			throws IOException {
		return (pIn.nextPacketTag() == PacketTags.TRUST) ? (TrustPacket) pIn
				.readPacket() : null;
	}
	...
	static List readSignaturesAndTrust(BCPGInputStream pIn) throws IOException {
		try {
			...
			while (pIn.nextPacketTag() == PacketTags.SIGNATURE) {
				SignaturePacket signaturePacket = (SignaturePacket) pIn
						.readPacket();
			...
			throw new IOException("can't create signature object: "
					+ e.getMessage() + ", cause: "
					+ e.getUnderlyingException().toString());
		}
	}

	static void readUserIDs(BCPGInputStream pIn, List ids, List idTrusts,
			List idSigs) throws IOException {
		while (pIn.nextPacketTag() == PacketTags.USER_ID
			...
			if (obj instanceof UserIDPacket) {
				...
				ids.add(new PGPUserAttributeSubpacketVector(user
						.getSubpackets()));
			}
	...
	/**
	 * Return the first public key in the ring. In the case of a
	 * {@link PGPSecretKeyRing} this is also the public key of the master key
	 * pair.
	 * 
	 * @return PGPPublicKey
	 */
