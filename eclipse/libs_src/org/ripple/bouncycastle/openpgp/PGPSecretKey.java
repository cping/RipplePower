...
public class PGPSecretKey {
	...
	PGPSecretKey(PGPPrivateKey privKey, PGPPublicKey pubKey,
	...
	PGPSecretKey(PGPPrivateKey privKey, PGPPublicKey pubKey,
			PGPDigestCalculator checksumCalculator, boolean isMasterKey,
			PBESecretKeyEncryptor keyEncryptor) throws PGPException {
		this.pub = pubKey;
		...
		try {
			...
			if (encAlgorithm != SymmetricKeyAlgorithmTags.NULL) {
				...
				byte[] encData = keyEncryptor.encryptKeyData(keyData, 0,
						keyData.length);
				...
				if (isMasterKey) {
					this.secret = new SecretKeyPacket(pub.publicPk,
							encAlgorithm, s2kUsage, s2k, iv, encData);
				} else {
					this.secret = new SecretSubkeyPacket(pub.publicPk,
							encAlgorithm, s2kUsage, s2k, iv, encData);
				}
			} else {
				if (isMasterKey) {
					this.secret = new SecretKeyPacket(pub.publicPk,
							encAlgorithm, null, null, bOut.toByteArray());
				} else {
					this.secret = new SecretSubkeyPacket(pub.publicPk,
							encAlgorithm, null, null, bOut.toByteArray());
				}
	...
	public PGPSecretKey(int certificationLevel, PGPKeyPair keyPair, String id,
			int encAlgorithm, char[] passPhrase,
			PGPSignatureSubpacketVector hashedPcks,
			PGPSignatureSubpacketVector unhashedPcks, SecureRandom rand,
			String provider) throws PGPException, NoSuchProviderException {
		this(certificationLevel, keyPair, id, encAlgorithm, passPhrase, false,
				hashedPcks, unhashedPcks, rand, provider);
	}
	 ...
	 */
	public PGPSecretKey(int certificationLevel, PGPKeyPair keyPair, String id,
			int encAlgorithm, char[] passPhrase, boolean useSHA1,
			PGPSignatureSubpacketVector hashedPcks,
			PGPSignatureSubpacketVector unhashedPcks, SecureRandom rand,
			String provider) throws PGPException, NoSuchProviderException {
		this(certificationLevel, keyPair, id, encAlgorithm, passPhrase,
				useSHA1, hashedPcks, unhashedPcks, rand, PGPUtil
	...
	public PGPSecretKey(int certificationLevel, PGPKeyPair keyPair, String id,
			...
			PBESecretKeyEncryptor keyEncryptor) throws PGPException {
		this(certificationLevel, keyPair, id, null, hashedPcks, unhashedPcks,
	...
	public PGPSecretKey(int certificationLevel, PGPKeyPair keyPair, String id,
			...
			PGPSignatureSubpacketVector hashedPcks,
			PGPSignatureSubpacketVector unhashedPcks, SecureRandom rand,
			Provider provider) throws PGPException {
		this(keyPair.getPrivateKey(), certifiedPublicKey(certificationLevel,
				keyPair, id, hashedPcks, unhashedPcks,
				new JcaPGPContentSignerBuilder(keyPair.getPublicKey()
						.getAlgorithm(), HashAlgorithmTags.SHA1)
						.setProvider(provider)), convertSHA1Flag(useSHA1),
				true, new JcePBESecretKeyEncryptorBuilder(encAlgorithm,
						new JcaPGPDigestCalculatorProviderBuilder().build()
								.get(HashAlgorithmTags.SHA1))
						.setProvider(provider).setSecureRandom(rand)
						.build(passPhrase));
	}

	private static PGPDigestCalculator convertSHA1Flag(boolean useSHA1)
			throws PGPException {
		return useSHA1 ? new JcaPGPDigestCalculatorProviderBuilder().build()
				.get(HashAlgorithmTags.SHA1) : null;
	}
	...
	public PGPSecretKey(int certificationLevel, PGPKeyPair keyPair, String id,
		...
		this(keyPair.getPrivateKey(), certifiedPublicKey(certificationLevel,
				keyPair, id, hashedPcks, unhashedPcks,
				certificationSignerBuilder), checksumCalculator, true,
				keyEncryptor);
	}

	private static PGPPublicKey certifiedPublicKey(int certificationLevel,
			PGPKeyPair keyPair, String id,
			PGPSignatureSubpacketVector hashedPcks,
			PGPSignatureSubpacketVector unhashedPcks,
			PGPContentSignerBuilder certificationSignerBuilder)
			throws PGPException {
		...
		try {
			PGPSignature certification = sGen.generateCertification(id,
					keyPair.getPublicKey());

			return PGPPublicKey.addCertification(keyPair.getPublicKey(), id,
					certification);
		} catch (Exception e) {
	 ...
	 * @deprecated use method taking PBESecretKeyEncryptor
	 */
	public PGPSecretKey(int certificationLevel, int algorithm,
			PublicKey pubKey, PrivateKey privKey, Date time, String id,
			int encAlgorithm, char[] passPhrase,
			PGPSignatureSubpacketVector hashedPcks,
			PGPSignatureSubpacketVector unhashedPcks, SecureRandom rand,
			String provider) throws PGPException, NoSuchProviderException {
		this(certificationLevel, new PGPKeyPair(algorithm, pubKey, privKey,
				time), id, encAlgorithm, passPhrase, hashedPcks, unhashedPcks,
				rand, provider);
	}
	...
	public PGPSecretKey(int certificationLevel, int algorithm,
			...
			String provider) throws PGPException, NoSuchProviderException {
		this(certificationLevel, new PGPKeyPair(algorithm, pubKey, privKey,
				time), id, encAlgorithm, passPhrase, useSHA1, hashedPcks,
				unhashedPcks, rand, provider);
	}
	...
	public PGPSecretKey(int certificationLevel, int algorithm,
			...
			PBESecretKeyEncryptor keyEncryptor) throws PGPException {
		this(certificationLevel, new PGPKeyPair(algorithm, pubKey, privKey,
				time), id, checksumCalculator, hashedPcks, unhashedPcks,
				certificationSignerBuilder, keyEncryptor);
	}
	...
	public PGPSecretKey(int certificationLevel, int algorithm,
		...
		this(certificationLevel, new PGPKeyPair(algorithm, pubKey, privKey,
				time), id, null, hashedPcks, unhashedPcks,
				certificationSignerBuilder, keyEncryptor);
	}
	...
	public boolean isSigningKey() {
		...
		return ((algorithm == PGPPublicKey.RSA_GENERAL)
				...
				|| (algorithm == PGPPublicKey.ECDSA) || (algorithm == PGPPublicKey.ELGAMAL_GENERAL));
	}
		...
		if (secret.getEncAlgorithm() != SymmetricKeyAlgorithmTags.NULL) {
			try {
				if (secret.getPublicKeyPacket().getVersion() == 4) {
					byte[] key = decryptorFactory.makeKeyFromPassPhrase(
							secret.getEncAlgorithm(), secret.getS2K());

					data = decryptorFactory.recoverKeyData(
							secret.getEncAlgorithm(), key, secret.getIV(),
							encData, 0, encData.length);

					boolean useSHA1 = secret.getS2KUsage() == SecretKeyPacket.USAGE_SHA1;
					byte[] check = checksum(
							useSHA1 ? decryptorFactory.getChecksumCalculator(HashAlgorithmTags.SHA1)
									: null, data, (useSHA1) ? data.length - 20
									: data.length - 2);

					for (int i = 0; i != check.length; i++) {
						if (check[i] != data[data.length - check.length + i]) {
							throw new PGPException("checksum mismatch at " + i
									+ " of " + check.length);
						}
					}
				} else // version 2 or 3, RSA only.
				{
					...
					for (int i = 0; i != 4; i++) {
						...
						byte[] tmp = decryptorFactory.recoverKeyData(
								secret.getEncAlgorithm(), key, iv, encData,
								pos + 2, encLen);
						System.arraycopy(tmp, 0, data, pos + 2, tmp.length);
						pos += 2 + encLen;

						if (i != 3) {
							System.arraycopy(encData, pos - iv.length, iv, 0,
									iv.length);
						}
					}
	...
	public PGPPrivateKey extractPrivateKey(char[] passPhrase, Provider provider)
			throws PGPException {
		return extractPrivateKey(new JcePBESecretKeyDecryptorBuilder(
				new JcaPGPDigestCalculatorProviderBuilder().setProvider(
						provider).build()).setProvider(provider).build(
				passPhrase));
	}
	 ...
	 * Extract a PGPPrivate key from the SecretKey's encrypted contents.
	 * 
	 * @param decryptorFactory
	 *            factory to use to generate a decryptor for the passed in
	 *            secretKey.
	...
	public PGPPrivateKey extractPrivateKey(
			PBESecretKeyDecryptor decryptorFactory) throws PGPException {
		...
		try {
			...
			BCPGInputStream in = new BCPGInputStream(new ByteArrayInputStream(
					data));
	...
	private static byte[] checksum(PGPDigestCalculator digCalc, byte[] bytes,
			int length) throws PGPException {
		if (digCalc != null) {
			...
			try {
				...
				throw new PGPException("checksum digest calculation failed: "
						+ e.getMessage(), e);
			}
		...
		if (pub.subSigs == null) // is not a sub key
		{
			...
			for (int i = 0; i != pub.ids.size(); i++) {
				if (pub.ids.get(i) instanceof String) {
					...
					PGPUserAttributeSubpacketVector v = (PGPUserAttributeSubpacketVector) pub.ids
							.get(i);

					out.writePacket(new UserAttributePacket(v
							.toSubpacketArray()));
				}
	 ...
	 * @param newEncAlgorithm
	 *            the algorithm to be used for the encryption.
	 * @param rand
	 *            source of randomness.
	 * @param provider
	 *            name of the provider to use
	 * @deprecated use method taking PBESecretKeyDecryptor and
	 *             PBESecretKeyEncryptor
	 */
	public static PGPSecretKey copyWithNewPassword(PGPSecretKey key,
			char[] oldPassPhrase, char[] newPassPhrase, int newEncAlgorithm,
			SecureRandom rand, String provider) throws PGPException,
			NoSuchProviderException {
		return copyWithNewPassword(key, oldPassPhrase, newPassPhrase,
				newEncAlgorithm, rand, PGPUtil.getProvider(provider));
	}
	 ...
	 * @param key
	 *            the PGPSecretKey to be copied.
	 * @param oldKeyDecryptor
	 *            the current decryptor based on the current password for key.
	 * @param newKeyEncryptor
	 *            a new encryptor based on a new password for encrypting the
	 *            secret key material.
	 */
	public static PGPSecretKey copyWithNewPassword(PGPSecretKey key,
		...
		if (newKeyEncryptor == null
			...
			if (key.secret.getS2KUsage() == SecretKeyPacket.USAGE_SHA1) // SHA-1
																		// hash,
																		// need
																		// to
																		// rewrite
			...
			if (key.secret.getPublicKeyPacket().getVersion() < 4) {
				...
				if (newKeyEncryptor.getHashAlgorithm() != HashAlgorithmTags.MD5) {
					throw new PGPException(
							"MD5 Digest Calculator required for version 3 key encryptor.");
				}
				...
				for (int i = 0; i != 4; i++) {
					...
					byte[] tmp;
					if (i == 0) {
						tmp = newKeyEncryptor.encryptKeyData(encKey,
								rawKeyData, pos + 2, encLen);
						iv = newKeyEncryptor.getCipherIV();

					} else {
						byte[] tmpIv = new byte[iv.length];

						System.arraycopy(keyData, pos - iv.length, tmpIv, 0,
								tmpIv.length);
						tmp = newKeyEncryptor.encryptKeyData(encKey, tmpIv,
								rawKeyData, pos + 2, encLen);
					}
				...
				keyData = newKeyEncryptor.encryptKeyData(rawKeyData, 0,
						rawKeyData.length);
	...
	public static PGPSecretKey copyWithNewPassword(PGPSecretKey key,
			char[] oldPassPhrase, char[] newPassPhrase, int newEncAlgorithm,
			SecureRandom rand, Provider provider) throws PGPException {
		return copyWithNewPassword(
				key,
				new JcePBESecretKeyDecryptorBuilder(
						new JcaPGPDigestCalculatorProviderBuilder()
								.setProvider(provider).build()).setProvider(
						provider).build(oldPassPhrase),
				new JcePBESecretKeyEncryptorBuilder(newEncAlgorithm)
						.setProvider(provider).setSecureRandom(rand)
						.build(newPassPhrase));
	}
