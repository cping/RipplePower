 ...
 * <p>
 * Often PGP keyring files consist of multiple master keys, if you are trying to
 * process or construct one of these you should use the
 * PGPSecretKeyRingCollection class.
 */
public class PGPSecretKeyRing extends PGPKeyRing {
	...
	public PGPSecretKeyRing(byte[] encoding,
			KeyFingerPrintCalculator fingerPrintCalculator) throws IOException,
			PGPException {
	...
	public PGPSecretKeyRing(InputStream in,
		...
		if (initialTag != PacketTags.SECRET_KEY
		...
		keys.add(new PGPSecretKey(secret, new PGPPublicKey(secret
				.getPublicKeyPacket(), trust, keySigs, ids, idTrusts, idSigs,
		...
		while (pIn.nextPacketTag() == PacketTags.SECRET_SUBKEY
			...
			if (pIn.nextPacketTag() == PacketTags.SECRET_SUBKEY) {
				...
				keys.add(new PGPSecretKey(sub, new PGPPublicKey(sub
						.getPublicKeyPacket(), subTrust, sigList,
						fingerPrintCalculator)));
			} else {
	...
	/**
	 * Return the public key referred to by the passed in keyID if it is
	 * present.
	 * 
	 * @param keyID
	 * @return PGPPublicKey
	...
	/**
	 * Return an iterator of the public keys in the secret key ring that have no
	 * matching private key. At the moment only personal certificate data
	...
	/**
	 * Replace the public key set on the secret ring with the corresponding key
	 * off the public ring.
	 * 
	 * @param secretRing
	...
	public static PGPSecretKeyRing replacePublicKeys(
			PGPSecretKeyRing secretRing, PGPPublicKeyRing publicRing) {
	...
	/**
	 * Return a copy of the passed in secret key ring, with the master key and
	 * sub keys encrypted using a new password and the passed in algorithm.
	 * 
	 * @param ring
	 ...
	 * @deprecated use version taking
	 *             PBESecretKeyEncryptor/PBESecretKeyDecryptor
	 */
	public static PGPSecretKeyRing copyWithNewPassword(PGPSecretKeyRing ring,
		...
		return copyWithNewPassword(ring, oldPassPhrase, newPassPhrase,
	...
	public static PGPSecretKeyRing copyWithNewPassword(PGPSecretKeyRing ring,
		...
		for (Iterator keys = ring.getSecretKeys(); keys.hasNext();) {
			newKeys.add(PGPSecretKey.copyWithNewPassword(
					(PGPSecretKey) keys.next(), oldPassPhrase, newPassPhrase,
					newEncAlgorithm, rand, provider));
		}
	...
	/**
	 * Return a copy of the passed in secret key ring, with the private keys
	 * (where present) associated with the master key and sub keys are encrypted
	...
	public static PGPSecretKeyRing copyWithNewPassword(PGPSecretKeyRing ring,
		...
		for (Iterator keys = ring.getSecretKeys(); keys.hasNext();) {
			...
			if (key.isPrivateKeyEmpty()) {
				...
				newKeys.add(PGPSecretKey.copyWithNewPassword(key,
	...
	/**
	 * Returns a new key ring with the secret key passed in removed from the key
	 * ring.
	 * 
	 * @param secRing
	 *            the secret key ring to be modified.
