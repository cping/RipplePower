...
public class PGPPublicKey implements PublicKeyAlgorithmTags {
	...
	private void init(KeyFingerPrintCalculator fingerPrintCalculator)
		...
		if (publicPk.getVersion() <= 3) {
			...
			if (key instanceof RSAPublicBCPGKey) {
				this.keyStrength = ((RSAPublicBCPGKey) key).getModulus()
						.bitLength();
				...
				this.keyStrength = ((ElGamalPublicBCPGKey) key).getP()
						.bitLength();
			}
		}
	}
	 ...
	 * <p>
	 * Note: the time passed in affects the value of the key's keyID, so you
	 * probably only want to do this once for a JCA key, or make sure you keep
	 * track of the time you used.
	 * 
	 * @param algorithm
	 ...
	 *             if the specified provider is required and cannot be found.
	...
	public PGPPublicKey(int algorithm, PublicKey pubKey, Date time,
		...
		this(new JcaPGPKeyConverter().setProvider(provider).getPGPPublicKey(
				algorithm, pubKey, time));
	}
	...
	/**
	 * Create a PGP public key from a packet descriptor using the passed in
	 * fingerPrintCalculator to do calculate the fingerprint and keyID.
	 * 
	 * @param publicKeyPacket
	 *            packet describing the public key.
	 * @param fingerPrintCalculator
	 *            calculator providing the digest support ot create the key
	 *            fingerprint.
	 * @throws PGPException
	 *             if the packet is faulty, or the required calculations fail.
	 */
	...
	PGPPublicKey(PublicKeyPacket publicPk, TrustPacket trustPk, List keySigs,
			List ids, List idTrusts, List idSigs,
			KeyFingerPrintCalculator fingerPrintCalculator) throws PGPException {
		this.publicPk = publicPk;
		this.trustPk = trustPk;
	...
	/**
	 * @return number of valid days from creation time - zero means no expiry.
	 */
		...
		if (publicPk.getVersion() > 3) {
			if (this.isMasterKey()) {
				for (int i = 0; i != MASTER_KEY_CERTIFICATION_TYPES.length; i++) {
					long seconds = getExpirationTimeFromSig(true,
							MASTER_KEY_CERTIFICATION_TYPES[i]);
				...
				long seconds = getExpirationTimeFromSig(false,
	 ...
	 */
	public PublicKey getKey(String provider) throws PGPException,
			NoSuchProviderException {
		return new JcaPGPKeyConverter().setProvider(provider)
				.getPublicKey(this);
	}
	...
	/**
	 * Return an iterator of signatures associated with the passed in user
	 * attributes.
	 * 
		...
		if (subSigs == null) // not a sub-key
		{
			...
			for (int i = 0; i != ids.size(); i++) {
				if (ids.get(i) instanceof String) {
					...
					PGPUserAttributeSubpacketVector v = (PGPUserAttributeSubpacketVector) ids
							.get(i);
	 ...
	 */
	public static PGPPublicKey addCertification(PGPPublicKey key, String id,
			PGPSignature certification) {
		return addCert(key, id, certification);
	}

	/**
	 * Add a certification for the given UserAttributeSubpackets to the given
	 * public key.
	 * 
	 * @param key
	 *            the key the certification is to be added to.
	...
	private static PGPPublicKey addCert(PGPPublicKey key, Object id,
			PGPSignature certification) {
	...
	/**
	 * Remove any certifications associated with a given user attribute
	 * subpacket on a key.
	 * 
	 * @param key
	 ...
	 * @return the re-certified key, null if the user attribute subpacket was
	 *         not found on the key.
	 */
	 ...
	 *            the key the certifications are to be removed from.
	 * @param id
	 *            the id that the certification is to be removed from.
	 * @param certification
	 ...
	 *            the key the certifications are to be removed from.
	 * @param userAttributes
	 *            the user attributes that the certification is to be removed
	 *            from.
	 * @param certification
	 *            the certification to be removed.
	 * @return the re-certified key, null if the certification was not found.
	...
	public static PGPPublicKey addCertification(PGPPublicKey key,
		...
		if (key.isMasterKey()) {
			...
			if (certification.getSignatureType() == PGPSignature.KEY_REVOCATION) {
				throw new IllegalArgumentException(
						"signature type incorrect for sub-key revocation.");
			}
		}
		...
		if (!found) {
			for (Iterator it = key.getUserIDs(); it.hasNext();) {
				...
				for (Iterator sIt = key.getSignaturesForID(id); sIt.hasNext();) {
					if (certification == sIt.next()) {
						found = true;
						returnKey = PGPPublicKey.removeCertification(returnKey,
								id, certification);
					}
				}
			}
			...
			if (!found) {
				for (Iterator it = key.getUserAttributes(); it.hasNext();) {
					PGPUserAttributeSubpacketVector id = (PGPUserAttributeSubpacketVector) it
							.next();
					for (Iterator sIt = key.getSignaturesForUserAttribute(id); sIt
							.hasNext();) {
						if (certification == sIt.next()) {
							found = true;
							returnKey = PGPPublicKey.removeCertification(
									returnKey, id, certification);
						}
					}
				}
			}
