...
public class PGPSignatureSubpacketGenerator {
	...
	/**
	 * Add a TrustSignature packet to the signature. The values for depth and
	 * trust are largely installation dependent but there are some guidelines in
	 * RFC 4880 - 5.2.3.13.
	 * 
	 * @param isCritical
	...
	/**
	 * Set the number of seconds a key is valid for after the time of its
	 * creation. A value of zero means the key never expires.
	 * 
	 * @param isCritical
	...
	/**
	 * Set the number of seconds a signature is valid for after the time of its
	 * creation. A value of zero means the signature never expires.
	 * 
	 * @param isCritical
	 *            true if should be treated as critical, false otherwise.
	 * @param seconds
	...
	public void setPreferredHashAlgorithms(boolean isCritical, int[] algorithms) {
		list.add(new PreferredAlgorithms(
				SignatureSubpacketTags.PREFERRED_HASH_ALGS, isCritical,
				algorithms));
	}
	...
	public void setPreferredSymmetricAlgorithms(boolean isCritical,
			int[] algorithms) {
		list.add(new PreferredAlgorithms(
				SignatureSubpacketTags.PREFERRED_SYM_ALGS, isCritical,
				algorithms));
	}
	...
	public void setPreferredCompressionAlgorithms(boolean isCritical,
			int[] algorithms) {
		list.add(new PreferredAlgorithms(
				SignatureSubpacketTags.PREFERRED_COMP_ALGS, isCritical,
				algorithms));
	}
	...
	public void setSignerUserID(boolean isCritical, String userID) {
		if (userID == null) {
			throw new IllegalArgumentException(
					"attempt to set null SignerUserID");
		}
	...
	public void setEmbeddedSignature(boolean isCritical,
			PGPSignature pgpSignature) throws IOException {
	...
	public void setNotationData(boolean isCritical, boolean isHumanReadable,
			String notationName, String notationValue) {
		list.add(new NotationData(isCritical, isHumanReadable, notationName,
				notationValue));
	}
	 ...
	 */
	public void setRevocationReason(boolean isCritical, byte reason,
			String description) {
	 ...
	 */
	public void setRevocationKey(boolean isCritical, int keyAlgorithm,
			byte[] fingerprint) {
		list.add(new RevocationKey(isCritical, RevocationKeyTags.CLASS_DEFAULT,
				keyAlgorithm, fingerprint));
	}
	...
	public PGPSignatureSubpacketVector generate() {
		return new PGPSignatureSubpacketVector(
				(SignatureSubpacket[]) list.toArray(new SignatureSubpacket[list
						.size()]));
	}
}
