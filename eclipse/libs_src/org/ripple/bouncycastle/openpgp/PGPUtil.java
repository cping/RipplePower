...
public class PGPUtil implements HashAlgorithmTags {
	...
	/**
	 * Set the provider to be used by the package when it is necessary to find
	 * one on the fly.
	 * 
	 * @param provider
	 */
	...
	static String getDigestName(int hashAlgorithm) throws PGPException {
		...
		default:
			throw new PGPException(
					"unknown hash algorithm tag in getDigestName: "
							+ hashAlgorithm);
		}
	}
	 ...
	 * @param out
	 * @param fileType
	 *            the LiteralData type for the file.
	 * @param file
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * write out the passed in file as a literal data packet in partial packet
	 * format.
	 * 
	 * @param out
	 * @param fileType
	 ...
	 */
	public static void writeFileToLiteralData(OutputStream out, char fileType,
			File file, byte[] buffer) throws IOException {
		PGPLiteralDataGenerator lData = new PGPLiteralDataGenerator();
		OutputStream pOut = lData.open(out, fileType, file.getName(), new Date(
				file.lastModified()), buffer);
		pipeFileContents(file, pOut, buffer.length);
	}

	private static void pipeFileContents(File file, OutputStream pOut,
