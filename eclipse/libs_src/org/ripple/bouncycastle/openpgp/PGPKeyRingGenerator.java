...
/**
 * Generator for a PGP master and subkey ring. This class will generate both the
...
public class PGPKeyRingGenerator {
	...
	/**
	 * Create a new key ring generator using old style checksumming. It is
	 * recommended to use SHA1 checksumming where possible.
	 * 
	 ...
	 */
	public PGPKeyRingGenerator(int certificationLevel, PGPKeyPair masterKey,
			String id, int encAlgorithm, char[] passPhrase,
		...
		this(certificationLevel, masterKey, id, encAlgorithm, passPhrase,
				false, hashedPcks, unhashedPcks, rand, provider);
	}
	 ...
	 *            the passPhrase to be used to protect secret keys.
	 * @param useSHA1
	 *            checksum the secret keys with SHA1 rather than the older 16
	 *            bit checksum.
	 * @param hashedPcks
	 *            packets to be included in the certification hash.
	 * @param unhashedPcks
	 ...
	 */
	public PGPKeyRingGenerator(int certificationLevel, PGPKeyPair masterKey,
			String id, int encAlgorithm, char[] passPhrase, boolean useSHA1,
			PGPSignatureSubpacketVector hashedPcks,
			PGPSignatureSubpacketVector unhashedPcks, SecureRandom rand,
			String provider) throws PGPException, NoSuchProviderException {
		this(certificationLevel, masterKey, id, encAlgorithm, passPhrase,
	...
	public PGPKeyRingGenerator(int certificationLevel, PGPKeyPair masterKey,
			...
			PGPSignatureSubpacketVector hashedPcks,
			PGPSignatureSubpacketVector unhashedPcks, SecureRandom rand,
			Provider provider) throws PGPException, NoSuchProviderException {
		this.masterKey = masterKey;
		this.hashedPcks = hashedPcks;
		this.unhashedPcks = unhashedPcks;
		this.keyEncryptor = new JcePBESecretKeyEncryptorBuilder(encAlgorithm)
				.setProvider(provider).setSecureRandom(rand).build(passPhrase);
		this.checksumCalculator = convertSHA1Flag(useSHA1);
		this.keySignerBuilder = new JcaPGPContentSignerBuilder(masterKey
				.getPublicKey().getAlgorithm(), HashAlgorithmTags.SHA1);

		keys.add(new PGPSecretKey(certificationLevel, masterKey, id,
				checksumCalculator, hashedPcks, unhashedPcks, keySignerBuilder,
				keyEncryptor));
	}
	 ...
	 */
	public PGPKeyRingGenerator(int certificationLevel, PGPKeyPair masterKey,
			String id, PGPDigestCalculator checksumCalculator,
			PGPSignatureSubpacketVector hashedPcks,
	 ...
	 * and inheriting the hashed/unhashed packets of the master key.
	 * 
	 * @param keyPair
	 * @throws PGPException
	 */
	...
	/**
	 * Add a subkey with specific hashed and unhashed packets associated with it
	 * and default certification.
	 * 
	 * @param keyPair
	...
	public void addSubKey(PGPKeyPair keyPair,
			PGPSignatureSubpacketVector hashedPcks,
			PGPSignatureSubpacketVector unhashedPcks) throws PGPException {
		try {
			//
			...
			subSigs.add(sGen.generateCertification(masterKey.getPublicKey(),
					keyPair.getPublicKey()));

			keys.add(new PGPSecretKey(keyPair.getPrivateKey(),
					new PGPPublicKey(keyPair.getPublicKey(), null, subSigs),
					checksumCalculator, keyEncryptor));
	...
	public PGPPublicKeyRing generatePublicKeyRing() {
		...
		while (it.hasNext()) {
			PGPPublicKey k = new PGPPublicKey(
					((PGPSecretKey) it.next()).getPublicKey());

			k.publicPk = new PublicSubkeyPacket(k.getAlgorithm(),
					k.getCreationTime(), k.publicPk.getKey());
