...
public class PGPLiteralDataGenerator implements StreamGenerator {
	...
	/**
	 * Generates literal data objects in the old format, this is important if
	 * you need compatability with PGP 2.6.x.
	 * 
	 * @param oldFormat
	 */
	...
	private void writeHeader(OutputStream out, char format, byte[] encName,
	 ...
	 */
	public OutputStream open(OutputStream out, char format, String name,
		...
		pkOut = new BCPGOutputStream(out, PacketTags.LITERAL_DATA, length + 2
				+ encName.length + 4, oldFormat);
	 ...
	 */
	public OutputStream open(OutputStream out, char format, String name,
			Date modificationTime, byte[] buffer) throws IOException {
		if (pkOut != null) {
			throw new IllegalStateException("generator already in open state");
		}
	...
	/**
	 * Open a literal data packet for the passed in File object, returning an
	 * output stream for saving the file contents.
	 * <p>
	 * The stream created can be closed off by either calling close() on the
	 * stream or close() on the generator. Closing the returned stream does not
	 * close off the OutputStream parameter out.
	...
	public OutputStream open(OutputStream out, char format, File file)
		...
		pkOut = new BCPGOutputStream(out, PacketTags.LITERAL_DATA,
				file.length() + 2 + encName.length + 4, oldFormat);
	...
	/**
	 * Close the literal data packet - this is equivalent to calling close on
