...
public class PGPSignatureGenerator {
	...
	/**
	 * Create a generator for the passed in keyAlgorithm and hashAlgorithm
	 * codes.
	 * 
	 * @param keyAlgorithm
	 *            keyAlgorithm to use for signing
	 * @param hashAlgorithm
	 ...
	 */
	public PGPSignatureGenerator(int keyAlgorithm, int hashAlgorithm,
	 ...
	 */
	public PGPSignatureGenerator(int keyAlgorithm, int hashAlgorithm,
			Provider provider) throws NoSuchAlgorithmException, PGPException {
		this(keyAlgorithm, provider, hashAlgorithm, provider);
	}
	 ...
	 *            keyAlgorithm to use for signing
	 * @param sigProvider
	 *            provider to use for signature generation
	 * @param hashAlgorithm
	 *            algorithm to use for digest
	 * @param digProvider
	 *            provider to use for digest algorithm
	 ...
	 * @deprecated use method taking a PGPContentSignerBuilder
	 */
	public PGPSignatureGenerator(int keyAlgorithm, String sigProvider,
			int hashAlgorithm, String digProvider)
			throws NoSuchAlgorithmException, NoSuchProviderException,
			PGPException {
		this(keyAlgorithm, PGPUtil.getProvider(sigProvider), hashAlgorithm,
				PGPUtil.getProvider(digProvider));
	}
	 ...
	 */
	public PGPSignatureGenerator(int keyAlgorithm, Provider sigProvider,
			int hashAlgorithm, Provider digProvider)
			throws NoSuchAlgorithmException, PGPException {
		this.providedKeyAlgorithm = keyAlgorithm;
		this.contentSignerBuilder = new JcaPGPContentSignerBuilder(
				keyAlgorithm, hashAlgorithm).setProvider(sigProvider)
				.setDigestProvider(digProvider);
	}
	 ...
	 *            builder to produce PGPContentSigner objects for generating
	 *            signatures.
	 */
	...
	public void initSign(int signatureType, PGPPrivateKey key)
		...
		if (providedKeyAlgorithm >= 0
				&& providedKeyAlgorithm != contentSigner.getKeyAlgorithm()) {
	 ...
	 */
	public void initSign(int signatureType, PGPPrivateKey key,
			SecureRandom random) throws PGPException {
	...
	private void byteUpdate(byte b) throws SignatureException {
		try {
			sigOut.write(b);
		} catch (IOException e) { // TODO: we really should get rid of signature
									// exception next....
	...
	public PGPOnePassSignature generateOnePassVersion(boolean isNested)
			throws PGPException {
		return new PGPOnePassSignature(new OnePassSignaturePacket(sigType,
				contentSigner.getHashAlgorithm(),
				contentSigner.getKeyAlgorithm(), contentSigner.getKeyID(),
				isNested));
	}
	...
	public PGPSignature generate() throws PGPException, SignatureException {
		...
		if (!packetPresent(hashed, SignatureSubpacketTags.CREATION_TIME)) {
			hPkts = insertSubpacket(hashed, new SignatureCreationTime(false,
					new Date()));
		} else {
			hPkts = hashed;
		}
		...
		if (!packetPresent(hashed, SignatureSubpacketTags.ISSUER_KEY_ID)
				&& !packetPresent(unhashed,
						SignatureSubpacketTags.ISSUER_KEY_ID)) {
			unhPkts = insertSubpacket(unhashed, new IssuerKeyID(false,
					contentSigner.getKeyID()));
		} else {
			unhPkts = unhashed;
		}
		...
		if (contentSigner.getKeyAlgorithm() == PublicKeyAlgorithmTags.RSA_SIGN
				|| contentSigner.getKeyAlgorithm() == PublicKeyAlgorithmTags.RSA_GENERAL) // an
																							// RSA
																							// signature
		{
			sigValues = new MPInteger[1];
			sigValues[0] = new MPInteger(new BigInteger(1,
		...
		return new PGPSignature(new SignaturePacket(sigType,
				contentSigner.getKeyID(), contentSigner.getKeyAlgorithm(),
				contentSigner.getHashAlgorithm(), hPkts, unhPkts, fingerPrint,
				sigValues));
	}
	...
	public PGPSignature generateCertification(
			PGPUserAttributeSubpacketVector userAttributes, PGPPublicKey pubKey)
			throws SignatureException, PGPException {
		updateWithPublicKey(pubKey);
		...
		try {
			...
			UserAttributeSubpacket[] packets = userAttributes
					.toSubpacketArray();
	...
	public PGPSignature generateCertification(PGPPublicKey masterKey,
			PGPPublicKey pubKey) throws SignatureException, PGPException {
	...
	private SignatureSubpacket[] insertSubpacket(SignatureSubpacket[] packets,
