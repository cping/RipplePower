 ...
 */
public class PGPEncryptedDataGenerator implements SymmetricKeyAlgorithmTags,
		StreamGenerator {
	/**
	 ...
	 * @param provider
	 *            the provider name to use for encryption algorithms.
	 * @deprecated use constructor that takes a PGPDataEncryptor
	 */
	public PGPEncryptedDataGenerator(int encAlgorithm, SecureRandom rand,
			String provider) {
		this(new JcePGPDataEncryptorBuilder(encAlgorithm).setSecureRandom(rand)
	...
	/**
	 * Creates a cipher stream which will have an integrity packet associated
	 * with it.
	 * 
	 * @param encAlgorithm
	...
	public PGPEncryptedDataGenerator(int encAlgorithm,
			boolean withIntegrityPacket, SecureRandom rand, String provider) {
		this(new JcePGPDataEncryptorBuilder(encAlgorithm)
				.setWithIntegrityPacket(withIntegrityPacket)
				.setSecureRandom(rand).setProvider(provider));
	}
	 ...
	 */
	public PGPEncryptedDataGenerator(int encAlgorithm,
			boolean withIntegrityPacket, SecureRandom rand, Provider provider) {
		this(new JcePGPDataEncryptorBuilder(encAlgorithm)
				.setWithIntegrityPacket(withIntegrityPacket)
				.setSecureRandom(rand).setProvider(provider));
	}
	 ...
	 */
	public PGPEncryptedDataGenerator(int encAlgorithm, SecureRandom rand,
			boolean oldFormat, String provider) {
		this(new JcePGPDataEncryptorBuilder(encAlgorithm).setSecureRandom(rand)
				.setProvider(provider), oldFormat);
	}
	 ...
	 */
	public PGPEncryptedDataGenerator(int encAlgorithm, SecureRandom rand,
			boolean oldFormat, Provider provider) {
		this(new JcePGPDataEncryptorBuilder(encAlgorithm).setSecureRandom(rand)
				.setProvider(provider), oldFormat);
	}
	 ...
	 * Base constructor.
	 * 
	 * @param encryptorBuilder
	 *            builder to create actual data encryptor.
	 */
	...
	/**
	 * Base constructor with the option to turn on formatting for PGP 2.6.x
	 * compatibility.
	 * 
	 * @param encryptorBuilder
	 *            builder to create actual data encryptor.
	...
	/**
	 * Add a PBE encryption method to the encrypted object using the default
	 * algorithm (S2K_SHA1).
	 * 
	 * @param passPhrase
	 ...
	 */
	public void addMethod(char[] passPhrase) throws NoSuchProviderException,
			PGPException {
	 ...
	 * @param s2kDigest
	 *            digest algorithm to use for S2K calculation
	 * @throws NoSuchProviderException
	 * @throws PGPException
	 * @deprecated use addMethod that takes PGPKeyEncryptionMethodGenerator
	 */
	public void addMethod(char[] passPhrase, int s2kDigest)
		...
		addMethod(new JcePBEKeyEncryptionMethodGenerator(passPhrase,
				new JcaPGPDigestCalculatorProviderBuilder()
						.setProvider(defProvider).build().get(s2kDigest))
				.setProvider(defProvider).setSecureRandom(rand));
	}
	 ...
	 * @deprecated use addMethod that takes PGPKeyEncryptionMethodGenerator
	 */
	public void addMethod(PGPPublicKey key) throws NoSuchProviderException,
			PGPException {
		if (!key.isEncryptionKey()) {
			throw new IllegalArgumentException(
					"passed in key not an encryption key!");
		}
		...
		addMethod(new JcePublicKeyKeyEncryptionMethodGenerator(key)
				.setProvider(defProvider).setSecureRandom(rand));
	}

	/**
	 * Added a key encryption method to be used to encrypt the session data
	 * associated with this encrypted data.
	 * 
	 * @param method
	 *            key encryption method to use.
	 */
	...
	/**
	 * If buffer is non null stream assumed to be partial, otherwise the length
	 * will be used to output a fixed length packet.
	 * <p>
	 * The stream created can be closed off by either calling close() on the
	 * stream or close() on the generator. Closing the returned stream does not
	 * close off the OutputStream parameter out.
	 * 
	 * @param out
	 * @param length
	 * @param buffer
	...
	private OutputStream open(OutputStream out, long length, byte[] buffer)
		...
		if (methods.size() == 1) {
			...
			if (methods.get(0) instanceof PBEKeyEncryptionMethodGenerator) {
				PBEKeyEncryptionMethodGenerator m = (PBEKeyEncryptionMethodGenerator) methods
						.get(0);

				key = m.getKey(dataEncryptorBuilder.getAlgorithm());

				pOut.writePacket(((PGPKeyEncryptionMethodGenerator) methods
						.get(0)).generate(defAlgorithm, null));
			} else {
				key = PGPUtil.makeRandomKey(defAlgorithm, rand);
				byte[] sessionInfo = createSessionInfo(defAlgorithm, key);
				PGPKeyEncryptionMethodGenerator m = (PGPKeyEncryptionMethodGenerator) methods
						.get(0);
		...
		try {
			...
			if (buffer == null) {
				//
				// we have to add block size + 2 for the generated IV and + 1 +
				// 22 if integrity protected
				//
				if (digestCalc != null) {
					pOut = new ClosableBCPGOutputStream(out,
							PacketTags.SYM_ENC_INTEGRITY_PRO, length
									+ dataEncryptor.getBlockSize() + 2 + 1 + 22);

					pOut.write(1); // version number
				} else {
					pOut = new ClosableBCPGOutputStream(out,
							PacketTags.SYMMETRIC_KEY_ENC, length
									+ dataEncryptor.getBlockSize() + 2,
							oldFormat);
				}
			} else {
				if (digestCalc != null) {
					pOut = new ClosableBCPGOutputStream(out,
							PacketTags.SYM_ENC_INTEGRITY_PRO, buffer);
					pOut.write(1); // version number
				} else {
					pOut = new ClosableBCPGOutputStream(out,
							PacketTags.SYMMETRIC_KEY_ENC, buffer);
				}
			}
	...
	/**
	 * Return an outputstream which will encrypt the data as it is written to
	 * it.
	 * <p>
	 * The stream created can be closed off by either calling close() on the
	 * stream or close() on the generator. Closing the returned stream does not
	 ...
	 * @throws PGPException
	 */
	public OutputStream open(OutputStream out, long length) throws IOException,
			PGPException {
	...
	/**
	 * Return an outputstream which will encrypt the data as it is written to
	 * it. The stream will be written out in chunks according to the size of the
	 * passed in buffer.
	 * <p>
	 * The stream created can be closed off by either calling close() on the
	 * stream or close() on the generator. Closing the returned stream does not
	 * close off the OutputStream parameter out.
	 * <p>
	 * <b>Note</b>: if the buffer is not a power of 2 in length only the largest
	 * power of 2 bytes worth of the buffer will be used.
	 * 
	 * @param out
	 * @param buffer
	 *            the buffer to use.
	...
	/**
	 * Close off the encrypted object - this is equivalent to calling close on
	 * the stream returned by the open() method.
	 * <p>
	 * <b>Note</b>: This does not close the underlying output stream, only the
	 * stream on top of it created by the open() method.
	 * 
	 * @throws java.io.IOException
	 */
	public void close() throws IOException {
		if (cOut != null) {
			if (digestCalc != null) {
				...
				//
				BCPGOutputStream bOut = new BCPGOutputStream(genOut,
						PacketTags.MOD_DETECTION_CODE, 20);
	...
	private class ClosableBCPGOutputStream extends BCPGOutputStream {
		public ClosableBCPGOutputStream(OutputStream out, int symmetricKeyEnc,
				byte[] buffer) throws IOException {
			super(out, symmetricKeyEnc, buffer);
		}

		public ClosableBCPGOutputStream(OutputStream out, int symmetricKeyEnc,
				long length, boolean oldFormat) throws IOException {
			super(out, symmetricKeyEnc, length, oldFormat);
		}

		public ClosableBCPGOutputStream(OutputStream out,
				int symEncIntegrityPro, long length) throws IOException {
