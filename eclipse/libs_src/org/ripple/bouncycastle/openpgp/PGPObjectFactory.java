...
public class PGPObjectFactory {
	...
	/**
	 * Create an object factor suitable for reading keys, key rings and key ring
	 * collections.
	 * 
	 * @param in
	 *            stream to read from
			...
			while (in.nextPacketTag() == PacketTags.ONE_PASS_SIGNATURE) {
				try {
					...
					throw new IOException(
							"can't create one pass signature object: " + e);
				}
			}

			return new PGPOnePassSignatureList(
					(PGPOnePassSignature[]) l.toArray(new PGPOnePassSignature[l
							.size()]));
