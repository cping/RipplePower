...
public abstract class PublicKeyKeyEncryptionMethodGenerator extends
	...
	protected PublicKeyKeyEncryptionMethodGenerator(PGPPublicKey pubKey) {
		...
		default:
			throw new IllegalArgumentException("unknown asymmetric algorithm: "
					+ pubKey.getAlgorithm());
		}
	}
	...
	public BigInteger[] processSessionInfo(byte[] encryptedSessionInfo)
		...
		case PGPPublicKey.ELGAMAL_GENERAL:
			...
			break;
		default:
			throw new PGPException("unknown asymmetric algorithm: "
					+ pubKey.getAlgorithm());
		}
	...
	public ContainedPacket generate(int encAlgorithm, byte[] sessionInfo)
			throws PGPException {
		return new PublicKeyEncSessionPacket(pubKey.getKeyID(),
				pubKey.getAlgorithm(), processSessionInfo(encryptSessionInfo(
						pubKey, sessionInfo)));
	}

	abstract protected byte[] encryptSessionInfo(PGPPublicKey pubKey,
