...
public abstract class PBESecretKeyEncryptor {
	...
	protected PBESecretKeyEncryptor(int encAlgorithm,
			PGPDigestCalculator s2kDigestCalculator, SecureRandom random,
			char[] passPhrase) {
	...
	protected PBESecretKeyEncryptor(int encAlgorithm,
			...
			SecureRandom random, char[] passPhrase) {
	...
	public byte[] getKey() throws PGPException {
		return PGPUtil.makeKeyFromPassPhrase(s2kDigestCalculator, encAlgorithm,
				s2k, passPhrase);
	}
	 ...
	 *            raw key data
	 * @param keyOff
	 *            offset into rawe key data
	 * @param keyLen
	 *            length of key data to use.
	...
	public abstract byte[] encryptKeyData(byte[] key, byte[] keyData,
