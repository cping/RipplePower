...
public class BcPGPDataEncryptorBuilder implements PGPDataEncryptorBuilder {
	...
	/**
	 * Determine whether or not the resulting encrypted data will be protected
	 * using an integrity packet.
	 * 
	 * @param withIntegrityPacket
	 *            true if an integrity packet is to be included, false
	 *            otherwise.
	 * @return the current builder.
	 */
	public BcPGPDataEncryptorBuilder setWithIntegrityPacket(
	...
	private class MyPGPDataEncryptor implements PGPDataEncryptor {
		...
		MyPGPDataEncryptor(byte[] keyBytes) throws PGPException {
			...
			try {
				c = BcUtil.createStreamCipher(true, engine,
						withIntegrityPacket, keyBytes);
			} catch (IllegalArgumentException e) {
				throw new PGPException("invalid parameters: " + e.getMessage(),
						e);
			}
		}
