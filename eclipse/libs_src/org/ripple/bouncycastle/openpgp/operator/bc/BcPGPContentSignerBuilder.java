...
public class BcPGPContentSignerBuilder implements PGPContentSignerBuilder {
	...
	public PGPContentSigner build(final int signatureType,
			final PGPPrivateKey privateKey) throws PGPException {
		final PGPDigestCalculator digestCalculator = digestCalculatorProvider
				.get(hashAlgorithm);
		final Signer signer = BcImplProvider.createSigner(keyAlgorithm,
				hashAlgorithm);

		if (random != null) {
			signer.init(
					true,
					new ParametersWithRandom(keyConverter
							.getPrivateKey(privateKey), random));
		} else {
		...
		return new PGPContentSigner() {
			...
			public OutputStream getOutputStream() {
				return new TeeOutputStream(new SignerOutputStream(signer),
			...
			public byte[] getSignature() {
				...
				} catch (CryptoException e) { // TODO: need a specific runtime
												// exception for PGP operators.
