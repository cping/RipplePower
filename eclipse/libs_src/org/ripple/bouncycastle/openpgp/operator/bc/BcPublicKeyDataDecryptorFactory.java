 ...
 */
public class BcPublicKeyDataDecryptorFactory implements
	...
	public byte[] recoverSessionData(int keyAlgorithm, BigInteger[] secKeyData)
		...
		try {
			AsymmetricBlockCipher c = BcImplProvider
					.createPublicKeyCipher(keyAlgorithm);

			AsymmetricKeyParameter key = keyConverter.getPrivateKey(privKey);

			BufferedAsymmetricBlockCipher c1 = new BufferedAsymmetricBlockCipher(
					c);
			...
			if (keyAlgorithm == PGPPublicKey.RSA_ENCRYPT
				...
				ElGamalPrivateKeyParameters parms = (ElGamalPrivateKeyParameters) converter
				...
				if (bi.length > size) {
					...
					System.arraycopy(bi, 0, tmp, tmp.length - bi.length,
							bi.length);
