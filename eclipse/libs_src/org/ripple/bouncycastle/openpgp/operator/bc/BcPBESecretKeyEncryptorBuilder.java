...
public class BcPBESecretKeyEncryptorBuilder {
	...
	public PBESecretKeyEncryptor build(char[] passPhrase) {
		...
		return new PBESecretKeyEncryptor(encAlgorithm, s2kDigestCalculator,
				s2kCount, this.random, passPhrase) {
			private byte[] iv;
			...
			public byte[] encryptKeyData(byte[] key, byte[] iv, byte[] keyData,
					int keyOff, int keyLen) throws PGPException {
				try {
					BlockCipher engine = BcImplProvider
							.createBlockCipher(this.encAlgorithm);
					...
					BufferedBlockCipher c = BcUtil.createSymmetricKeyWrapper(
							true, engine, key, iv);

					byte[] out = new byte[keyLen];
					int outLen = c
							.processBytes(keyData, keyOff, keyLen, out, 0);
