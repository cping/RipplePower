...
public class BcPGPKeyConverter {
	...
	public PGPPrivateKey getPGPPrivateKey(PGPPublicKey pubKey,
			AsymmetricKeyParameter privKey) throws PGPException {
		BCPGKey privPk;
		...
		case PGPPublicKey.RSA_GENERAL:
			...
			privPk = new RSASecretBCPGKey(rsK.getExponent(), rsK.getP(),
					rsK.getQ());
			break;
		...
		return new PGPPrivateKey(pubKey.getKeyID(),
				pubKey.getPublicKeyPacket(), privPk);
	}
	...
	public AsymmetricKeyParameter getPublicKey(PGPPublicKey publicKey)
		...
		try {
			...
			case PublicKeyAlgorithmTags.RSA_SIGN:
				...
				return new RSAKeyParameters(false, rsaK.getModulus(),
						rsaK.getPublicExponent());
			case PublicKeyAlgorithmTags.DSA:
				DSAPublicBCPGKey dsaK = (DSAPublicBCPGKey) publicPk.getKey();

				return new DSAPublicKeyParameters(
						dsaK.getY(),
						new DSAParameters(dsaK.getP(), dsaK.getQ(), dsaK.getG()));
			case PublicKeyAlgorithmTags.ELGAMAL_ENCRYPT:
			case PublicKeyAlgorithmTags.ELGAMAL_GENERAL:
				ElGamalPublicBCPGKey elK = (ElGamalPublicBCPGKey) publicPk
						.getKey();

				return new ElGamalPublicKeyParameters(elK.getY(),
						new ElGamalParameters(elK.getP(), elK.getG()));
			default:
	...
	public AsymmetricKeyParameter getPrivateKey(PGPPrivateKey privKey)
		...
		try {
			...
			case PGPPublicKey.RSA_SIGN:
				...
				return new RSAPrivateCrtKeyParameters(rsaPriv.getModulus(),
			...
			case PGPPublicKey.DSA:
				...
				return new DSAPrivateKeyParameters(dsaPriv.getX(),
						new DSAParameters(dsaPub.getP(), dsaPub.getQ(),
								dsaPub.getG()));
			case PGPPublicKey.ELGAMAL_ENCRYPT:
			case PGPPublicKey.ELGAMAL_GENERAL:
				...
				return new ElGamalPrivateKeyParameters(elPriv.getX(),
						new ElGamalParameters(elPub.getP(), elPub.getG()));
			default:
				throw new PGPException(
						"unknown public key algorithm encountered");
			}
