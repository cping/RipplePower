...
/**
 * A BC lightweight method generator for supporting PBE based encryption
 * operations.
 */
public class BcPBEKeyEncryptionMethodGenerator extends
	...
	protected byte[] encryptSessionInfo(int encAlgorithm, byte[] key,
		...
		try {
			...
			BufferedBlockCipher cipher = BcUtil.createSymmetricKeyWrapper(true,
					engine, key, new byte[engine.getBlockSize()]);

			byte[] out = new byte[sessionInfo.length];

			int len = cipher.processBytes(sessionInfo, 0, sessionInfo.length,
					out, 0);
