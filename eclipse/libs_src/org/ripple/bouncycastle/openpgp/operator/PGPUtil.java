...
class PGPUtil implements HashAlgorithmTags {
	static byte[] makeKeyFromPassPhrase(PGPDigestCalculator digestCalculator,
			int algorithm, S2K s2k, char[] passPhrase) throws PGPException {
		String algName = null;
		int keySize = 0;
		...
		try {
			while (generatedBytes < keyBytes.length) {
				if (s2k != null) {
					...
					case S2K.SALTED_AND_ITERATED:
						...
						break;
					default:
						throw new PGPException("unknown S2K type: "
								+ s2k.getType());
					}
				} else {
					for (int i = 0; i != loopCount; i++) {
				...
				if (dig.length > (keyBytes.length - generatedBytes)) {
					System.arraycopy(dig, 0, keyBytes, generatedBytes,
							keyBytes.length - generatedBytes);
				} else {
					System.arraycopy(dig, 0, keyBytes, generatedBytes,
							dig.length);
				}
			...
			throw new PGPException("exception calculating digest: "
					+ e.getMessage(), e);
		}
	...
	public static byte[] makeKeyFromPassPhrase(
			PGPDigestCalculatorProvider digCalcProvider, int algorithm,
			S2K s2k, char[] passPhrase) throws PGPException {
		...
		return makeKeyFromPassPhrase(digestCalculator, algorithm, s2k,
				passPhrase);
	}
}
