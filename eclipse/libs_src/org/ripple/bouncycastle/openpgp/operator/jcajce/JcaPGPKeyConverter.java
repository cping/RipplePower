...
public class JcaPGPKeyConverter {
		...
		try {
			...
			case PublicKeyAlgorithmTags.RSA_SIGN:
				...
				RSAPublicKeySpec rsaSpec = new RSAPublicKeySpec(
						rsaK.getModulus(), rsaK.getPublicExponent());
			...
			case PublicKeyAlgorithmTags.DSA:
				...
				DSAPublicKeySpec dsaSpec = new DSAPublicKeySpec(dsaK.getY(),
			...
			case PublicKeyAlgorithmTags.ELGAMAL_GENERAL:
				ElGamalPublicBCPGKey elK = (ElGamalPublicBCPGKey) publicPk
						.getKey();
				ElGamalPublicKeySpec elSpec = new ElGamalPublicKeySpec(
						elK.getY(), new ElGamalParameterSpec(elK.getP(),
								elK.getG()));
	 ...
	 */
	public PGPPublicKey getPGPPublicKey(int algorithm, PublicKey pubKey,
		...
		if (pubKey instanceof RSAPublicKey) {
			...
			bcpgKey = new RSAPublicBCPGKey(rK.getModulus(),
					rK.getPublicExponent());
		} else if (pubKey instanceof DSAPublicKey) {
			DSAPublicKey dK = (DSAPublicKey) pubKey;
			DSAParams dP = dK.getParams();

			bcpgKey = new DSAPublicBCPGKey(dP.getP(), dP.getQ(), dP.getG(),
					dK.getY());
		...
		return new PGPPublicKey(new PublicKeyPacket(algorithm, time, bcpgKey),
		...
		try {
			...
			case PGPPublicKey.RSA_SIGN:
				...
				RSAPrivateCrtKeySpec rsaPrivSpec = new RSAPrivateCrtKeySpec(
						rsaPriv.getModulus(), rsaPub.getPublicExponent(),
						rsaPriv.getPrivateExponent(), rsaPriv.getPrimeP(),
						rsaPriv.getPrimeQ(), rsaPriv.getPrimeExponentP(),
			...
			case PGPPublicKey.DSA:
				...
				DSAPrivateKeySpec dsaPrivSpec = new DSAPrivateKeySpec(
						dsaPriv.getX(), dsaPub.getP(), dsaPub.getQ(),
						dsaPub.getG());
			...
			case PGPPublicKey.ELGAMAL_GENERAL:
				ElGamalPublicBCPGKey elPub = (ElGamalPublicBCPGKey) pubPk
						.getKey();
				ElGamalSecretBCPGKey elPriv = (ElGamalSecretBCPGKey) privPk;
				ElGamalPrivateKeySpec elSpec = new ElGamalPrivateKeySpec(
						elPriv.getX(), new ElGamalParameterSpec(elPub.getP(),
								elPub.getG()));
	 ...
	 * 
	 * @param pub
	 *            the corresponding PGPPublicKey to privKey.
	 * @param privKey
	...
	public PGPPrivateKey getPGPPrivateKey(PGPPublicKey pub, PrivateKey privKey)
		...
		case PGPPublicKey.RSA_GENERAL:
			...
			privPk = new RSASecretBCPGKey(rsK.getPrivateExponent(),
					rsK.getPrimeP(), rsK.getPrimeQ());
			break;
		case PGPPublicKey.DSA:
		...
		return new PGPPrivateKey(pub.getKeyID(), pub.getPublicKeyPacket(),
				privPk);
	}
}
