...
public class JcePBEDataDecryptorFactoryBuilder {
	 ...
	 *            a digest calculator provider to provide calculators to support
	 *            the key generation calculation required.
	 */
	...
	public PBEDataDecryptorFactory build(char[] passPhrase) {
		return new PBEDataDecryptorFactory(passPhrase, calculatorProvider) {
			public byte[] recoverSessionData(int keyAlgorithm, byte[] key,
					byte[] secKeyData) throws PGPException {
				try {
					if (secKeyData != null && secKeyData.length > 0) {
						String cipherName = PGPUtil
								.getSymmetricCipherName(keyAlgorithm);
						Cipher keyCipher = helper.createCipher(cipherName
								+ "/CFB/NoPadding");

						keyCipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(
								key, cipherName), new IvParameterSpec(
								new byte[keyCipher.getBlockSize()]));
					...
					throw new PGPException("Exception recovering session info",
							e);
				}
			}
			...
			public PGPDataDecryptor createDataDecryptor(
					...
					throws PGPException {
				return helper.createDataDecryptor(withIntegrityPacket,
						encAlgorithm, key);
			}
		};
	}
