...
public class JcePBESecretKeyEncryptorBuilder {
	...
	/**
	 * Create a SecretKeyEncryptorBuilder with the S2K count different to the
	 * default of 0x60.
	 * 
	 * @param encAlgorithm
	...
	/**
	 * Create a builder which will make encryptors using the passed in digest
	 * calculator. If a MD5 calculator is passed in the builder will assume the
	 * encryptors are for use with version 3 keys.
	 * 
	 * @param encAlgorithm
	 *            encryption algorithm to use.
	...
	/**
	 * Create an SecretKeyEncryptorBuilder with the S2k count different to the
	 * default of 0x60, and the S2K digest different from SHA-1.
	 * 
	 * @param encAlgorithm
	 *            encryption algorithm to use.
	 * @param s2kDigestCalculator
	...
	public JcePBESecretKeyEncryptorBuilder(int encAlgorithm,
		...
		if (s2kCount < 0 || s2kCount > 0xff) {
			throw new IllegalArgumentException(
					"s2KCount value outside of range 0 to 255.");
		}
	...
	public PBESecretKeyEncryptor build(char[] passPhrase) {
		...
		return new PBESecretKeyEncryptor(encAlgorithm, s2kDigestCalculator,
				s2kCount, random, passPhrase) {
			private Cipher c;
			private byte[] iv;

			public byte[] encryptKeyData(byte[] key, byte[] keyData,
					int keyOff, int keyLen) throws PGPException {
				try {
					c = helper.createCipher(PGPUtil
							.getSymmetricCipherName(this.encAlgorithm)
							+ "/CFB/NoPadding");

					c.init(Cipher.ENCRYPT_MODE,
							PGPUtil.makeSymmetricKey(this.encAlgorithm, key),
							this.random);
					...
					throw new PGPException("illegal block size: "
							+ e.getMessage(), e);
			...
			public byte[] encryptKeyData(byte[] key, byte[] iv, byte[] keyData,
					int keyOff, int keyLen) throws PGPException {
				try {
					c = helper.createCipher(PGPUtil
							.getSymmetricCipherName(this.encAlgorithm)
