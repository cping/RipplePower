...
class OperatorHelper {
	...
	MessageDigest createDigest(int algorithm) throws GeneralSecurityException,
			PGPException {
		MessageDigest dig;
	...
	PGPDataDecryptor createDataDecryptor(boolean withIntegrityPacket,
		...
		try {
			...
			final Cipher c = createStreamCipher(encAlgorithm,
					withIntegrityPacket);
	...
	private Signature createSignature(String cipherName) throws PGPException {
		try {
			...
			throw new PGPException(
					"cannot create signature: " + e.getMessage(), e);
		}
	}
	...
	public Signature createSignature(int keyAlgorithm, int hashAlgorithm)
		...
		return createSignature(PGPUtil.getDigestName(hashAlgorithm) + "with"
				+ encAlg);
	}
}
