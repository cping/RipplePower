...
public class JcePublicKeyDataDecryptorFactoryBuilder {
	...
	private OperatorHelper contentHelper = new OperatorHelper(
			new DefaultJcaJceHelper());
	...
	/**
	 * Set the provider object to use for creating cryptographic primitives in
	 * the resulting factory the builder produces.
	 * 
	 * @param provider
	...
	/**
	 * Set the provider name to use for creating cryptographic primitives in the
	 * resulting factory the builder produces.
	 * 
	 * @param providerName
	 *            the name of the provider to reference for cryptographic
	 *            primitives.
	 * @return the current builder.
	 */
	public JcePublicKeyDataDecryptorFactoryBuilder setProvider(
			String providerName) {
	...
	public JcePublicKeyDataDecryptorFactoryBuilder setContentProvider(
			Provider provider) {
		this.contentHelper = new OperatorHelper(new ProviderJcaJceHelper(
				provider));
	...
	public JcePublicKeyDataDecryptorFactoryBuilder setContentProvider(
			String providerName) {
		this.contentHelper = new OperatorHelper(new NamedJcaJceHelper(
				providerName));
	...
	public PublicKeyDataDecryptorFactory build(final PrivateKey privKey) {
		return new PublicKeyDataDecryptorFactory() {
			public byte[] recoverSessionData(int keyAlgorithm,
					BigInteger[] secKeyData) throws PGPException {
				return decryptSessionData(keyAlgorithm, privKey, secKeyData);
			}

			public PGPDataDecryptor createDataDecryptor(
					boolean withIntegrityPacket, int encAlgorithm, byte[] key)
					throws PGPException {
				return contentHelper.createDataDecryptor(withIntegrityPacket,
						encAlgorithm, key);
			}
		};
	}
	...
	public PublicKeyDataDecryptorFactory build(final PGPPrivateKey privKey) {
		return new PublicKeyDataDecryptorFactory() {
			public byte[] recoverSessionData(int keyAlgorithm,
					BigInteger[] secKeyData) throws PGPException {
				return decryptSessionData(keyAlgorithm,
						keyConverter.getPrivateKey(privKey), secKeyData);
			}
	...
	private byte[] decryptSessionData(int keyAlgorithm, PrivateKey privKey,
			BigInteger[] secKeyData) throws PGPException {
