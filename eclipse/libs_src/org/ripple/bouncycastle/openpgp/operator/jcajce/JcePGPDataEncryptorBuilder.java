...
public class JcePGPDataEncryptorBuilder implements PGPDataEncryptorBuilder {
	 ...
	 */
	public JcePGPDataEncryptorBuilder setWithIntegrityPacket(
		...
		MyPGPDataEncryptor(byte[] keyBytes) throws PGPException {
			...
			try {
				c.init(Cipher.ENCRYPT_MODE,
						PGPUtil.makeSymmetricKey(encAlgorithm, keyBytes),
						new IvParameterSpec(iv));
			} catch (InvalidKeyException e) {
				throw new PGPException("invalid key: " + e.getMessage(), e);
			} catch (InvalidAlgorithmParameterException e) {
				throw new PGPException("imvalid algorithm parameter: "
						+ e.getMessage(), e);
			}
		}
