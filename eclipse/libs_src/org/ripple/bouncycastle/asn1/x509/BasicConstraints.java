...
public class BasicConstraints extends ASN1Object {
	...
	public static BasicConstraints fromExtensions(Extensions extensions) {
		return BasicConstraints.getInstance(extensions
				.getExtensionParsedValue(Extension.basicConstraints));
	}
	...
	private BasicConstraints(ASN1Sequence seq) {
		if (seq.size() == 0) {
			...
			if (seq.getObjectAt(0) instanceof DERBoolean) {
				...
				this.cA = null;
				this.pathLenConstraint = ASN1Integer.getInstance(seq
						.getObjectAt(0));
			}
			if (seq.size() > 1) {
				if (this.cA != null) {
	...
	public ASN1Primitive toASN1Primitive() {
		...
		if (pathLenConstraint != null) // yes some people actually do this when
										// cA is false...
		{
	...
	public String toString() {
		...
		return "BasicConstraints: isCa(" + this.isCA()
				+ "), pathLenConstraint = " + pathLenConstraint.getValue();
	}
}
