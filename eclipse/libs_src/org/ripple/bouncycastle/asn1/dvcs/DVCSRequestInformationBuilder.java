...
public class DVCSRequestInformationBuilder {
	...
	public DVCSRequestInformation build() {
		...
		int[] tags = new int[] { TAG_REQUESTER, TAG_REQUEST_POLICY, TAG_DVCS,
				TAG_DATA_LOCATIONS, TAG_EXTENSIONS };
		ASN1Encodable[] taggedObjects = new ASN1Encodable[] { requester,
				requestPolicy, dvcs, dataLocations, extensions };
	...
	public void setVersion(int version) {
		if (initialInfo != null) {
			throw new IllegalStateException(
					"cannot change version in existing DVCSRequestInformation");
		}
	...
	public void setNonce(BigInteger nonce) {
		...
		if (initialInfo != null) {
			if (initialInfo.getNonce() == null) {
				...
				byte[] nonceBytes = new byte[initialBytes.length
						+ newBytes.length];

				System.arraycopy(initialBytes, 0, nonceBytes, 0,
						initialBytes.length);
				System.arraycopy(newBytes, 0, nonceBytes, initialBytes.length,
						newBytes.length);
