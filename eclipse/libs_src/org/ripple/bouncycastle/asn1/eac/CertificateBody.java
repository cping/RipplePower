...
public class CertificateBody extends ASN1Object {
	ASN1InputStream seq;
	private DERApplicationSpecific certificateProfileIdentifier;// version of
																// the
																// certificate
																// format. Must
																// be 0 (version
																// 1)
	private DERApplicationSpecific certificationAuthorityReference;// uniquely
																	// identifies
																	// the
																	// issuinng
																	// CA's
	...
	private DERApplicationSpecific certificateHolderReference;// associates the
																// public key
																// contained in
																// the
																// certificate
																// with a unique
																// name
	private CertificateHolderAuthorization certificateHolderAuthorization;// Encodes
																			// the
																			// role
																			// of
																			// the
																			...
																			// sensitive
																			// data
	private DERApplicationSpecific certificateEffectiveDate;// the date of the
															// certificate
															// generation
	private DERApplicationSpecific certificateExpirationDate;// the date after
																// wich the
																// certificate
																// expires
	private int certificateType = 0;// bit field of initialized data. This will
									// tell us if the data are valid.
	...
	private void setIso7816CertificateBody(DERApplicationSpecific appSpe)
		...
		if (appSpe.getApplicationTag() == EACTags.CERTIFICATE_CONTENT_TEMPLATE) {
			...
			throw new IOException(
					"Bad tag : not an iso7816 CERTIFICATE_CONTENT_TEMPLATE");
		}
		...
		while ((obj = aIS.readObject()) != null) {
			...
			if (obj instanceof DERApplicationSpecific) {
				...
				throw new IOException(
						"Not a valid iso7816 content : not a DERApplicationSpecific Object :"
			...
			case EACTags.CARDHOLDER_PUBLIC_KEY_TEMPLATE:
				setPublicKey(PublicKeyDataObject.getInstance(aSpe
			...
			default:
				certificateType = 0;
				throw new IOException(
						"Not a valid iso7816 DERApplicationSpecific tag "
								+ aSpe.getApplicationTag());
			}
		}
	}
	...
	public CertificateBody(DERApplicationSpecific certificateProfileIdentifier,
		...
		setCertificateHolderReference(new DERApplicationSpecific(
				EACTags.CARDHOLDER_NAME,
				certificateHolderReference.getEncoded()));
		setCertificateHolderAuthorization(certificateHolderAuthorization);
		try {
			setCertificateEffectiveDate(new DERApplicationSpecific(false,
					EACTags.APPLICATION_EFFECTIVE_DATE, new DEROctetString(
							certificateEffectiveDate.getEncoding())));
			setCertificateExpirationDate(new DERApplicationSpecific(false,
					EACTags.APPLICATION_EXPIRATION_DATE, new DEROctetString(
							certificateExpirationDate.getEncoding())));
		} catch (IOException e) {
			throw new IllegalArgumentException("unable to encode dates: "
					+ e.getMessage());
		}
	}
	...
	private ASN1Primitive profileToASN1Object() throws IOException {
		...
		v.add(new DERApplicationSpecific(false,
				EACTags.CARDHOLDER_PUBLIC_KEY_TEMPLATE, publicKey));
		...
		return new DERApplicationSpecific(EACTags.CERTIFICATE_CONTENT_TEMPLATE,
				v);
	}

	private void setCertificateProfileIdentifier(
		...
		if (certificateProfileIdentifier.getApplicationTag() == EACTags.INTERCHANGE_PROFILE) {
			...
			throw new IllegalArgumentException(
					"Not an Iso7816Tags.INTERCHANGE_PROFILE tag :"
							+ EACTags.encodeTag(certificateProfileIdentifier));
		}
	}

	private void setCertificateHolderReference(
			DERApplicationSpecific certificateHolderReference)
			throws IllegalArgumentException {
		if (certificateHolderReference.getApplicationTag() == EACTags.CARDHOLDER_NAME) {
			...
			throw new IllegalArgumentException(
					"Not an Iso7816Tags.CARDHOLDER_NAME tag");
		}
	}
	...
	private void setCertificationAuthorityReference(
		...
		if (certificationAuthorityReference.getApplicationTag() == EACTags.ISSUER_IDENTIFICATION_NUMBER) {
			...
			throw new IllegalArgumentException(
					"Not an Iso7816Tags.ISSUER_IDENTIFICATION_NUMBER tag");
		}
	}
	...
	/**
	 * create a "request" or "profile" type Iso7816CertificateBody according to
	 * the variables sets.
	 * 
	 * @return return the ASN1Primitive representing the "request" or "profile"
	 *         type certificate body.
	 * @throws IOException
	 *             if the DERApplicationSpecific cannot be created or if data
	 *             are missings to create a valid certificate.
	 */
	...
	/**
	 * gives the type of the certificate (value should be profileType or
	 * requestType if all data are set).
	 * 
	...
	private void setCertificateEffectiveDate(DERApplicationSpecific ced)
		...
		if (ced.getApplicationTag() == EACTags.APPLICATION_EFFECTIVE_DATE) {
			...
			throw new IllegalArgumentException(
					"Not an Iso7816Tags.APPLICATION_EFFECTIVE_DATE tag :"
							+ EACTags.encodeTag(ced));
		}
	}
	...
	private void setCertificateExpirationDate(DERApplicationSpecific ced)
		...
		if (ced.getApplicationTag() == EACTags.APPLICATION_EXPIRATION_DATE) {
			...
			throw new IllegalArgumentException(
					"Not an Iso7816Tags.APPLICATION_EXPIRATION_DATE tag");
		}
	}
	...
	public CertificateHolderReference getCertificateHolderReference() {
		return new CertificateHolderReference(
				certificateHolderReference.getContents());
	}

	/**
	 * CertificateProfileIdentifier : version of the certificate format. Must be
	 * 0 (version 1)
	 * 
	...
	public CertificationAuthorityReference getCertificationAuthorityReference()
		...
		if ((this.certificateType & CertificateBody.CAR) == CertificateBody.CAR) {
			return new CertificationAuthorityReference(
					certificationAuthorityReference.getContents());
		}
