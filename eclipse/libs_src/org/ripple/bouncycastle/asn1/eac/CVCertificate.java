...
public class CVCertificate extends ASN1Object {
	 ...
	 * 
	 * @param appSpe
	 *            is a DERApplicationSpecific object containing body and
	 *            signature.
	 * @throws IOException
				...
				if (tmpObj instanceof DERApplicationSpecific) {
					...
					default:
						throw new IOException(
								"Invalid tag, not an Iso7816CertificateStructure :"
										+ aSpe.getApplicationTag());
					}
				} else {
					throw new IOException(
							"Invalid Object, not an Iso7816CertificateStructure");
				}
			}
		} else {
			throw new IOException("not a CARDHOLDER_CERTIFICATE :"
					+ appSpe.getApplicationTag());
		}
	}
	...
	private void initFrom(ASN1InputStream aIS) throws IOException {
		...
		while ((obj = aIS.readObject()) != null) {
			if (obj instanceof DERApplicationSpecific) {
				...
				throw new IOException(
						"Invalid Input Stream for creating an Iso7816CertificateStructure");
			}
		}
	}
	...
	public ASN1ObjectIdentifier getHolderAuthorization() throws IOException {
		CertificateHolderAuthorization cha = certificateBody
	...
	/**
	 * return a bits field coded on one byte. For signification of the several
	 * bit see Iso7816CertificateHolderAuthorization
	 * 
	...
	public int getHolderAuthorizationRole() throws IOException {
		int rights = certificateBody.getCertificateHolderAuthorization()
				.getAccessRights();
		return rights & 0xC0;
	}
	 ...
	 *         certificate See Iso7816CertificateHolderAuthorization static int
	 *         for values
	 * @throws IOException
	 */
	public Flags getHolderAuthorizationRights() throws IOException {
		return new Flags(certificateBody.getCertificateHolderAuthorization()
				.getAccessRights() & 0x1F);
	}
}
