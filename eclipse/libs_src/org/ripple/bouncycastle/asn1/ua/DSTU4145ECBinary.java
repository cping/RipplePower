...
public class DSTU4145ECBinary extends ASN1Object {
	...
	public DSTU4145ECBinary(ECDomainParameters params) {
		...
		f = new DSTU4145BinaryField(curve.getM(), curve.getK1(), curve.getK2(),
				curve.getK3());
		a = new ASN1Integer(curve.getA().toBigInteger());
		X9IntegerConverter converter = new X9IntegerConverter();
		b = new DEROctetString(converter.integerToBytes(curve.getB()
				.toBigInteger(), converter.getByteLength(curve)));
	...
	private DSTU4145ECBinary(ASN1Sequence seq) {
		...
		if (seq.getObjectAt(index) instanceof ASN1TaggedObject) {
			ASN1TaggedObject taggedVersion = (ASN1TaggedObject) seq
					.getObjectAt(index);
			if (taggedVersion.isExplicit() && 0 == taggedVersion.getTagNo()) {
				version = ASN1Integer.getInstance(
						taggedVersion.getLoadedObject()).getValue();
				index++;
			} else {
	 ...
	 * BinaryField, a INTEGER (0..1), b OCTET STRING, n INTEGER, bp OCTET
	 * STRING}
	 */
