...
public abstract class DSTU4145PointEncoder {
	...
	private static ECFieldElement solveQuadradicEquation(ECFieldElement beta) {
		...
		ECFieldElement zeroElement = new ECFieldElement.F2m(b.getM(),
				b.getK1(), b.getK2(), b.getK3(), ECConstants.ZERO);
	...
	public static byte[] encodePoint(ECPoint Q) {
		 ...
		 * 
		 * if (bytes[0]==0x02) bytes[bytes.length-1]&=0xFE; else if
		 * (bytes[0]==0x02) bytes[bytes.length-1]|=0x01;
		 * 
		 * return Arrays.copyOfRange(bytes, 1, bytes.length);
		 */

		int byteCount = converter.getByteLength(Q.getX());
		byte[] bytes = converter.integerToBytes(Q.getX().toBigInteger(),
				byteCount);
	...
	public static ECPoint decodePoint(ECCurve curve, byte[] bytes) {
		/*
		 * byte[] bp_enc=new byte[bytes.length+1]; if
		 * (0==(bytes[bytes.length-1]&0x1)) bp_enc[0]=0x02; else bp_enc[0]=0x03;
		 * System.arraycopy(bytes, 0, bp_enc, 1, bytes.length); if
		 * (!trace(curve.fromBigInteger(new BigInteger(1,
		 * bytes))).equals(curve.getA().toBigInteger()))
		...
		if (!trace(curve.fromBigInteger(new BigInteger(1, bytes))).equals(
