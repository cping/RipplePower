...
public class McElieceKeyFactorySpi extends KeyFactorySpi {
	...
	public PublicKey generatePublic(KeySpec keySpec)
		...
		if (keySpec instanceof McEliecePublicKeySpec) {
			...
			try {
				pki = SubjectPublicKeyInfo.getInstance(ASN1Primitive
						.fromByteArray(encKey));
			} catch (IOException e) {
			...
			try {
				...
				String oidString = ((ASN1ObjectIdentifier) publicKey
						.getObjectAt(0)).toString();

				// decode <n>
				BigInteger bigN = ((ASN1Integer) publicKey.getObjectAt(1))
						.getValue();
				int n = bigN.intValue();

				// decode <t>
				BigInteger bigT = ((ASN1Integer) publicKey.getObjectAt(2))
						.getValue();
				int t = bigT.intValue();

				// decode <matrixG>
				byte[] matrixG = ((ASN1OctetString) publicKey.getObjectAt(3))
						.getOctets();

				return new BCMcEliecePublicKey(new McEliecePublicKeySpec(OID,
						t, n, matrixG));
	...
	public PrivateKey generatePrivate(KeySpec keySpec)
		...
		if (keySpec instanceof McEliecePrivateKeySpec) {
			...
			try {
				pki = PrivateKeyInfo.getInstance(ASN1Primitive
						.fromByteArray(encKey));
			} catch (IOException e) {
				throw new InvalidKeySpecException(
						"Unable to decode PKCS8EncodedKeySpec: " + e);
			}

			try {
				ASN1Primitive innerType = pki.parsePrivateKey()
						.toASN1Primitive();
				...
				// decode oidString (but we don't need it right now)
				String oidString = ((ASN1ObjectIdentifier) privKey
						.getObjectAt(0)).toString();

				// decode <n>
				BigInteger bigN = ((ASN1Integer) privKey.getObjectAt(1))
						.getValue();
				int n = bigN.intValue();

				// decode <k>
				BigInteger bigK = ((ASN1Integer) privKey.getObjectAt(2))
						.getValue();
				int k = bigK.intValue();
				...
				// decode <sInv>
				byte[] encSInv = ((ASN1OctetString) privKey.getObjectAt(5))
						.getOctets();
				// decode <p1>
				byte[] encP1 = ((ASN1OctetString) privKey.getObjectAt(6))
						.getOctets();
				// decode <p2>
				byte[] encP2 = ((ASN1OctetString) privKey.getObjectAt(7))
						.getOctets();

				// decode <h>
				byte[] encH = ((ASN1OctetString) privKey.getObjectAt(8))
						.getOctets();
				...
				for (int i = 0; i < qSeq.size(); i++) {
					encQInv[i] = ((ASN1OctetString) qSeq.getObjectAt(i))
							.getOctets();
				}

				return new BCMcEliecePrivateKey(new McEliecePrivateKeySpec(OID,
						n, k, encFieldPoly, encGoppaPoly, encSInv, encP1,
						encP2, encH, encQInv));
	...
	public KeySpec getKeySpec(Key key, Class keySpec)
		...
		if (key instanceof BCMcEliecePrivateKey) {
			if (PKCS8EncodedKeySpec.class.isAssignableFrom(keySpec)) {
				...
				return new McEliecePrivateKeySpec(OID, privKey.getN(),
						privKey.getK(), privKey.getField(),
						privKey.getGoppaPoly(), privKey.getSInv(),
						privKey.getP1(), privKey.getP2(), privKey.getH(),
						privKey.getQInv());
			}
			...
			if (X509EncodedKeySpec.class.isAssignableFrom(keySpec)) {
				...
				return new McEliecePublicKeySpec(OID, pubKey.getN(),
						pubKey.getT(), pubKey.getG());
			}
		} else {
	...
	public PublicKey generatePublic(SubjectPublicKeyInfo pki)
		...
		try {
			...
			return new BCMcEliecePublicKey(key.getOID().getId(), key.getN(),
					key.getT(), key.getG());
		} catch (IOException cce) {
			throw new InvalidKeySpecException(
					"Unable to decode X509EncodedKeySpec");
	...
	public PrivateKey generatePrivate(PrivateKeyInfo pki)
		...
		try {
			...
			return new BCMcEliecePrivateKey(key.getOID().getId(), key.getN(),
					key.getK(), key.getField(), key.getGoppaPoly(),
					key.getSInv(), key.getP1(), key.getP2(), key.getH(),
					key.getQInv());
		} catch (IOException cce) {
			throw new InvalidKeySpecException(
					"Unable to decode PKCS8EncodedKeySpec");
