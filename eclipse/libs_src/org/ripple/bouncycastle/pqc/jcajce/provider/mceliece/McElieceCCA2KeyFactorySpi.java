...
public class McElieceCCA2KeyFactorySpi extends KeyFactorySpi {
	...
	public PublicKey generatePublic(KeySpec keySpec)
		...
		if (keySpec instanceof McElieceCCA2PublicKeySpec) {
			...
			try {
				...
				return new BCMcElieceCCA2PublicKey(
						new McElieceCCA2PublicKeySpec(OID, n, t, matrixG));
		...
		if (keySpec instanceof McElieceCCA2PrivateKeySpec) {
			...
			try {
				...
				byte[] encGoppaPoly = ((ASN1OctetString) privKey.getObjectAt(4))
						.getOctets();
				// decode <p>
				byte[] encP = ((ASN1OctetString) privKey.getObjectAt(5))
						.getOctets();
				// decode <h>
				byte[] encH = ((ASN1OctetString) privKey.getObjectAt(6))
						.getOctets();
				// decode <qInv>
	...
	public KeySpec getKeySpec(Key key, Class keySpec)
		...
		if (key instanceof BCMcElieceCCA2PrivateKey) {
			if (PKCS8EncodedKeySpec.class.isAssignableFrom(keySpec)) {
				...
				return new McElieceCCA2PrivateKeySpec(OID, privKey.getN(),
						privKey.getK(), privKey.getField(),
						privKey.getGoppaPoly(), privKey.getP(), privKey.getH(),
						privKey.getQInv());
			}
			...
			if (X509EncodedKeySpec.class.isAssignableFrom(keySpec)) {
				...
				return new McElieceCCA2PublicKeySpec(OID, pubKey.getN(),
	 ...
	 * {@link BCMcElieceCCA2PrivateKey}, {@link BCMcElieceCCA2PublicKey}.
	 * 
	 * @param key
	 *            the key
	...
	public PublicKey generatePublic(SubjectPublicKeyInfo pki)
		...
		try {
			ASN1Primitive innerType = pki.parsePublicKey();
			McElieceCCA2PublicKey key = McElieceCCA2PublicKey
					.getInstance((ASN1Sequence) innerType);
			return new BCMcElieceCCA2PublicKey(key.getOID().getId(),
	...
	public PrivateKey generatePrivate(PrivateKeyInfo pki)
		...
		try {
			...
			McElieceCCA2PrivateKey key = McElieceCCA2PrivateKey
					.getInstance(innerType);
			return new BCMcElieceCCA2PrivateKey(key.getOID().getId(),
					key.getN(), key.getK(), key.getField(), key.getGoppaPoly(),
