...
public class NTRUSigner {
	...
	public void update(byte b) {
		if (hashAlg == null) {
			throw new IllegalStateException(
					"Call initSign or initVerify first!");
		}
	...
	private byte[] signHash(byte[] msgHash, NTRUSigningPrivateKeyParameters kp) {
		...
		do {
			...
			if (r > params.signFailTolerance) {
				throw new IllegalStateException(
						"Signing failed: too many retries (max="
								+ params.signFailTolerance + ")");
			}
	...
	private IntegerPolynomial sign(IntegerPolynomial i,
		...
		while (iLoop >= 1) {
			...
			IntegerPolynomial hi = (IntegerPolynomial) kPriv.getBasis(iLoop).h
					.clone();
			if (iLoop > 1) {
	...
	private boolean verifyHash(byte[] msgHash, byte[] sig,
			NTRUSigningPublicKeyParameters pub) {
		ByteBuffer sbuf = ByteBuffer.wrap(sig);
		byte[] rawSig = new byte[sig.length - 4];
		sbuf.get(rawSig);
		IntegerPolynomial s = IntegerPolynomial.fromBinary(rawSig, params.N,
				params.q);
		int r = sbuf.getInt();
		return verify(createMsgRep(msgHash, r), s, pub.h);
	}

	private boolean verify(IntegerPolynomial i, IntegerPolynomial s,
			IntegerPolynomial h) {
		int q = params.q;
		...
		t.sub(i);
		long centeredNormSq = (long) (s.centeredNormSq(q) + betaSq
				* t.centeredNormSq(q));
