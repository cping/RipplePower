...
public class ComputeInField {
	 ...
	 * 
	 * @param B
	 *            this matrix is the left part of the equation (B in the
	 *            equation above)
	 * @param b
	...
	public short[] solveEquation(short[][] B, short[] b) {
		try {
			...
			// In this column we store a free coefficient that should be later
			// subtracted from b
			...
			// the free coefficient, stored in the last column of A(
			// A[i][b.length]
	...
	public short[][] inverse(short[][] coef) {
		try {
			...
			// modify A to get only 1s on the diagonal: A[i][j]
			// =A[i][j]/A[i][i].
	 ...
	 *            solveEquation-function or by the inverse-function and
	 *            according to this creates matrices of different sizes.
	 * @throws RuntimeException
	...
	private void computeZerosUnder(boolean usedForInverse)
		...
		// the number of columns in the global A where the tmp results are
		// stored
		int length;
		short tmp = 0;

		// the function is used in inverse() - A should have 2 times more
		// columns than rows
		if (usedForInverse) {
			length = 2 * A.length;
		}
		...
		// elimination operations to modify A so that that it contains only 0s
		// under the diagonal
		for (int k = 0; k < A.length - 1; k++) { // the fixed row
			for (int i = k + 1; i < A.length; i++) { // rows
				...
				if (factor2 == 0) {
					throw new RuntimeException(
							"Matrix not invertible! We have to choose another one!");
				}
	 ...
	 * @return A = M1*M2
	 * @throws RuntimeException
	 *             in case the given matrices cannot be multiplied due to
	 ...
	 * @throws RuntimeException
	 *             in case the addition is impossible due to inconsistency in
	 *             the dimensions
	 */
	 ...
	 * @throws RuntimeException
	 *             in case the multiplication is impossible due to inconsistency
	 *             in the dimensions
	 */
