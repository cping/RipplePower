...
public class RainbowSigner implements MessageSigner {
	...
	private short[] initSign(Layer[] layer, short[] msg) {
		...
		tmpVec = cf.addVect(((RainbowPrivateKeyParameters) this.key).getB1(),
				msg);

		// Y_ = A1^{-1} * (Y - b1) :
		short[] Y_ = cf.multiplyMatrix(
				((RainbowPrivateKeyParameters) this.key).getInvA1(), tmpVec);
	...
	public byte[] generateSignature(byte[] message) {
		...
		x = new short[((RainbowPrivateKeyParameters) this.key).getInvA2().length]; // all
																					// variables
		...
		do {
			...
			try {
				...
				tmpVec = cf.addVect(
						((RainbowPrivateKeyParameters) this.key).getB2(), x);
				signature = cf.multiplyMatrix(
						((RainbowPrivateKeyParameters) this.key).getInvA2(),
						tmpVec);
	...
	private short[] verifySignatureIntern(short[] signature) {
		...
		short[][] coeff_quadratic = ((RainbowPublicKeyParameters) this.key)
				.getCoeffQuadratic();
		short[][] coeff_singular = ((RainbowPublicKeyParameters) this.key)
				.getCoeffSingular();
		short[] coeff_scalar = ((RainbowPublicKeyParameters) this.key)
				.getCoeffScalar();
