...
public class GMSSPrivateKeyParameters extends GMSSKeyParameters {
	...
	public GMSSPrivateKeyParameters(byte[][] currentSeed,
			...
			Vector[][] nextRetain, byte[][] nextRoot, byte[][] currentRootSig,
			GMSSParameters gmssParameterset, GMSSDigestProvider digestProvider) {
	...
	public GMSSPrivateKeyParameters(int[] index, byte[][] currentSeeds,
		...
		if (nextNextRoot == null) {
			...
			for (int i = 0; i < numLayer - 1; i++) {
				this.nextNextRoot[i] = new GMSSRootCalc(
						this.heightOfTrees[i + 1], this.K[i + 1],
						this.digestProvider);
			}
		} else {
		...
		if (numLayer > 1) {
			...
			if (nextNextLeaf == null) {
				...
				for (int i = 0; i < numLayer - 2; i++) {
					this.nextNextLeaf[i] = new GMSSLeaf(digestProvider.get(),
		...
		if (upperLeaf == null) {
			...
			for (int i = 0; i < numLayer - 1; i++) {
				this.upperLeaf[i] = new GMSSLeaf(digestProvider.get(),
						otsIndex[i], numLeafs[i + 1], this.currentSeeds[i]);
			}
		} else {
		...
		if (upperTreehashLeaf == null) {
			...
			for (int i = 0; i < numLayer - 1; i++) {
				this.upperTreehashLeaf[i] = new GMSSLeaf(digestProvider.get(),
						otsIndex[i], numLeafs[i + 1]);
			}
		} else {
		...
		if (nextRootSig == null) {
			...
			for (int i = 0; i < numLayer - 1; i++) {
				...
				this.nextRootSig[i] = new GMSSRootSig(digestProvider.get(),
						otsIndex[i], heightOfTrees[i + 1]);
	...
	private void nextTree(int layer) {
		...
		if (layer > 0) {
			...
			if (!lastTree) {
				...
				if (layer > 1) {
					nextNextLeaf[layer - 1 - 1] = nextNextLeaf[layer - 1 - 1]
							.nextLeaf();
				}
				...
				if (minTreehash[layer - 1] >= 0) {
					upperTreehashLeaf[layer - 1] = upperTreehashLeaf[layer - 1]
							.nextLeaf();
	 ...
	 * <code>layer</code>, and the LEAF++^1 for the next next tree in the layer
			...
			if (index[layer] % t == 1) {
				...
				if (this.minTreehash[layer - 1] >= 0) {
					...
					this.upperTreehashLeaf[layer - 1] = new GMSSLeaf(
							this.digestProvider.get(),
							this.otsIndex[layer - 1], t, seed);
					this.upperTreehashLeaf[layer - 1] = this.upperTreehashLeaf[layer - 1]
							.nextLeaf();
	...
	private void computeAuthPaths(int layer) {
		...
		if (Tau == 0) {
			...
			currentAuthPaths[layer][Tau] = new byte[messDigestTrees
					.getDigestSize()];
			...
			for (int i = 0; i < Tau; i++) {
				...
				if (i < H - K) {
					if (currentTreehash[layer][i].wasFinished()) {
						System.arraycopy(
								currentTreehash[layer][i].getFirstNode(), 0,
								currentAuthPaths[layer][i], 0, mdLength);
				...
				if (i < H - 1 && i >= H - K) {
					if (currentRetain[layer][i - (H - K)].size() > 0) {
						...
						System.arraycopy(
								...
								0, currentAuthPaths[layer][i], 0, mdLength);
