...
public final class GoppaCode {
	...
	public static class MatrixSet {
		 ...
		 *            generator matrix consisting of the specified columns is
		 *            the identity
		 */
	...
	public static GF2Matrix createCanonicalCheckMatrix(GF2mField field,
		...
		for (int i = 0; i < t; i++) {
			for (int j = 0; j < n; j++) {
				for (int k = 0; k <= i; k++) {
					hArray[i][j] = field.add(hArray[i][j],
							field.mult(yz[k][j], gp.getCoefficient(t + k - i)));
				}
			}
		}
	...
	/**
	 * Given a check matrix <tt>H</tt>, compute matrices <tt>S</tt>, <tt>M</tt>,
	 * and a random permutation <tt>P</tt> such that <tt>S*H*P = (Id|M)</tt>.
	 * Return <tt>S^-1</tt>, <tt>M</tt>, and <tt>P</tt> as {@link MaMaPe}. The
	 * matrix <tt>(Id | M)</tt> is called the systematic form of H.
	 * 
	 * @param h
	 *            the check matrix
	 * @param sr
	...
	public static GF2Vector syndromeDecode(GF2Vector syndVec, GF2mField field,
		...
		if (!syndVec.isZero()) {
			...
			PolynomialGF2mSmallM syndrome = new PolynomialGF2mSmallM(
					syndVec.toExtensionFieldVector(field));
