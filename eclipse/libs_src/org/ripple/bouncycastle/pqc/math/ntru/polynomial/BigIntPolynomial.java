...
public class BigIntPolynomial {
	 ...
	 * @return
	 */
	static BigIntPolynomial generateRandomSmall(int N, int numOnes,
			int numNegOnes) {
	...
	private BigIntPolynomial multRecursive(BigIntPolynomial poly2) {
		...
		if (n <= 1) {
			...
			BigIntPolynomial a2 = new BigIntPolynomial(Arrays.copyOfRange(a,
					n1, n));
			BigIntPolynomial b1 = new BigIntPolynomial(Arrays.copyOf(b, n1));
			BigIntPolynomial b2 = new BigIntPolynomial(Arrays.copyOfRange(b,
					n1, n));
	...
	/**
	 * Divides each coefficient by a <code>BigInteger</code> and rounds the
	 * result to the nearest whole number.<br/>
	...
	public void div(BigInteger divisor) {
		BigInteger d = divisor.add(Constants.BIGINT_ONE).divide(
				BigInteger.valueOf(2));
		for (int i = 0; i < coeffs.length; i++) {
			coeffs[i] = coeffs[i].compareTo(Constants.BIGINT_ZERO) > 0 ? coeffs[i]
					.add(d) : coeffs[i].add(d.negate());
			coeffs[i] = coeffs[i].divide(divisor);
		}
	}

	/**
	 * Divides each coefficient by a <code>BigDecimal</code> and rounds the
	 * result to <code>decimalPlaces</code> places.
	 * 
	 * @param divisor
	 *            the number to divide by
	 * @param decimalPlaces
	...
	public BigDecimalPolynomial div(BigDecimal divisor, int decimalPlaces) {
		...
		// factor = 1/divisor
		BigDecimal factor = Constants.BIGDEC_ONE.divide(divisor, coeffLength
				+ decimalPlaces + 1, BigDecimal.ROUND_HALF_EVEN);

		// multiply each coefficient by factor
		BigDecimalPolynomial p = new BigDecimalPolynomial(coeffs.length);
		for (int i = 0; i < coeffs.length; i++)
		// multiply, then truncate after decimalPlaces so subsequent operations
		// aren't slowed down
		{
			p.coeffs[i] = new BigDecimal(coeffs[i]).multiply(factor).setScale(
					decimalPlaces, BigDecimal.ROUND_HALF_EVEN);
		}
