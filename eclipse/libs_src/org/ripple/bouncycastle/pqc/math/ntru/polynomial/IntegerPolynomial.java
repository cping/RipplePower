...
public class IntegerPolynomial implements Polynomial {
	...
	/**
	 * Prime numbers &gt; 4500 for resultant computation. Starting them below
	 * ~4400 causes incorrect results occasionally. Fortunately, 4500 is about
	 * the optimum number for performance.<br/>
	 * This array contains enough prime numbers so primes never have to be
	 * computed on-line for any standard
	 * {@link org.ripple.bouncycastle.pqc.crypto.ntru.NTRUSigningParameters}.
	 */
	private static final int[] PRIMES = new int[] { 4507, 4513, 4517, 4519,
	...
	/**
	 * Constructs a new polynomial with <code>N</code> coefficients initialized
	 * to 0.
	 * 
	 * @param N
	 ...
	 * <code>BigIntPolynomial</code>. The two polynomials are independent of
	 * each other.
	 * 
	 * @param p
	...
	/**
	 * Converts a byte array produced by {@link #toBinary3Tight()} to a
	 * polynomial.
	 * 
	 * @param b
	 *            a byte array
	 ...
	 * <code>coeffs[2*i]</code> and <code>coeffs[2*i+1]</code> must not both
	 * equal -1 for any integer </code>i<code>,
	...
	/**
	 * Multiplies the polynomial with another, taking the values mod modulus and
	 * the indices mod N
	 */
	...
	private IntegerPolynomial multRecursive(IntegerPolynomial poly2) {
		...
		if (n <= 32) {
			...
			IntegerPolynomial a2 = new IntegerPolynomial(Arrays.copyOfRange(a,
					n1, n));
			IntegerPolynomial b1 = new IntegerPolynomial(Arrays.copyOf(b, n1));
			IntegerPolynomial b2 = new IntegerPolynomial(Arrays.copyOfRange(b,
					n1, n));
	...
	private IntegerPolynomial mod2ToModq(IntegerPolynomial Fq, int q) {
		if (Util.is64BitJVM() && q == 2048) {
			...
			while (v < q) {
				v *= 2;
				IntegerPolynomial temp = new IntegerPolynomial(Arrays.copyOf(
						Fq.coeffs, Fq.coeffs.length));
				temp.mult2(v);
	...
	/**
	 * Computes the inverse mod 3. Returns <code>null</code> if the polynomial
	 * is not invertible.
	 * 
	 * @return a new polynomial
	 */
	...
	/**
	 * Resultant of this polynomial with <code>x^n-1</code> using a
	 * probabilistic algorithm.
	 * <p/>
	 * Unlike EESS, this implementation does not compute all resultants modulo
	 * primes such that their product exceeds the maximum possible resultant,
	 * but rather stops when <code>NUM_EQUAL_RESULTANTS</code> consecutive
	 * modular resultants are equal.<br/>
	 * This means the return value may be incorrect. Experiments show this
	 * happens in about 1 out of 100 cases when <code>N=439</code> and
	 * <code>NUM_EQUAL_RESULTANTS=2</code>, so the likelyhood of leaving the
	...
	public Resultant resultant() {
		...
		// Compute resultants modulo prime numbers. Continue until
		...
		int numEqual = 1; // number of consecutive modular resultants equal to
							// each other
		Iterator<BigInteger> primes = BIGINT_PRIMES.iterator();
		while (true) {
			prime = primes.hasNext() ? primes.next() : prime
					.nextProbablePrime();
		...
		while (modResultants.size() > 1) {
			...
			ModularResultant modRes3 = ModularResultant.combineRho(modRes1,
					modRes2);
	...
	public Resultant resultantMultiThread() {
		...
		while (pProd.compareTo(max2) < 0) {
			...
			Future<ModularResultant> task = executor
					.submit(new ModResultantTask(prime.intValue()));
		...
		while (!resultantTasks.isEmpty()) {
			try {
				...
				Future<ModularResultant> newTask = executor
						.submit(new CombineTask(modRes1.get(), modRes2.get()));
	...
	public ModularResultant resultant(int p) {
		// Add a coefficient as the following operations involve polynomials of
		// degree deg(f)+1
		...
		return new ModularResultant(new BigIntPolynomial(v2),
	...
	/**
	 * Adds another polynomial which can have a different number of
	...
	/**
	 * Multiplies each coefficient by a 2 and applies a modulus. Does not return
	...
	/**
	 * Divides each coefficient by <code>k</code> and rounds to the nearest
	 * integer. Does not return a new polynomial but modifies this polynomial.
	 * 
	 * @param k
	 *            the divisor
	 */
	 ...
	 * 
	 * @return true iff all coefficients are equal to zero, except for the
	 *         lowest coefficient which must equal 1
	 */
	 ...
	 * 
	 * @return true iff all coefficients are equal to zero, except for the
	 *         lowest coefficient which must equal 1 or -1
	 */
	...
	/**
	 * Calls
	 * {@link ModularResultant#combineRho(ModularResultant, ModularResultant)
