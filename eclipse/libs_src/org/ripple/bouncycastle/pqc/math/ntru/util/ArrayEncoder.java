...
public class ArrayEncoder {
	 ...
	 * {@link http
	 * ://stackoverflow.com/questions/1562548/how-to-make-a-message-into
	 * -a-polynomial}
	 * <p/>
	 * Convert each three-bit quantity to two ternary coefficients as follows,
	 * and concatenate the resulting ternary quantities to obtain [the output].
	 * <p/>
	 * <code>
	 * {0, 0, 0} -> {0, 0}<br/>
	 * {0, 0, 1} -> {0, 1}<br/>
	 ...
	 * -a-polynomial}
	 * <p/>
	 * Convert each set of two ternary coefficients to three bits as follows,
	 * and concatenate the resulting bit quantities to obtain [the output]:
	 * <p/>
	 * <code>
	...
	/**
	 * Encodes an int array whose elements are between 0 and <code>q</code>, to
	 * a byte array leaving no gaps between bits.<br/>
	 ...
	 * <code>N</code> is the number of coefficients. <code>q</code> must be a
	...
	/**
	 * Decodes a <code>byte</code> array encoded with
	 * {@link #encodeMod3Sves(int[])} back to an <code>int</code> array with
	 * <code>N</code> coefficients between <code>-1</code> and <code>1</code>.<br/>
	...
	/**
	 * Encodes an <code>int</code> array whose elements are between
	 * <code>-1</code> and <code>1</code>, to a byte array.
	...
	public static byte[] encodeMod3Sves(int[] arr) {
		...
		int bitIndex = 0;
		int byteIndex = 0;
		for (int i = 0; i < arr.length / 2 * 2;) { // if length is an odd
													// number, throw away the
													// highest coeff
			int coeff1 = arr[i++] + 1;
			int coeff2 = arr[i++] + 1;
			...
			int[] bits = new int[] { BIT1_TABLE[bitTableIndex],
					BIT2_TABLE[bitTableIndex], BIT3_TABLE[bitTableIndex] };
			for (int j = 0; j < 3; j++) {
	...
	/**
	 * Converts a byte array produced by {@link #encodeMod3Tight(int[])} back to
