...
public class Util {
	 ...
	 */
	public static TernaryPolynomial generateRandomTernary(int N, int numOnes,
			int numNegOnes, boolean sparse, SecureRandom random) {
		if (sparse) {
			return SparseTernaryPolynomial.generateRandom(N, numOnes,
					numNegOnes, random);
		} else {
			return DenseTernaryPolynomial.generateRandom(N, numOnes,
					numNegOnes, random);
		}
	}

	/**
	 * Generates an array containing numOnes ints equal to 1, numNegOnes int
	 ...
	 */
	public static int[] generateRandomTernary(int N, int numOnes,
			int numNegOnes, SecureRandom random) {
	...
	public static boolean is64BitJVM() {
		if (!IS_64_BITNESS_KNOWN) {
			...
			IS_64_BIT_JVM = "amd64".equals(arch) || "x86_64".equals(arch)
					|| "ppc64".equals(arch) || "64".equals(sunModel);
	...
	/**
	 * Reads a given number of bytes from an <code>InputStream</code>. If there
	 * are not enough bytes in the stream, an <code>IOException</code> is
	 * thrown.
	 * 
	 * @param is
	 * @param length
