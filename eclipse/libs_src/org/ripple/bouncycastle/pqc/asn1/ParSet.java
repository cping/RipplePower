...
public class ParSet extends ASN1Object {
	...
	private ParSet(ASN1Sequence seq) {
		if (seq.size() != 4) {
			throw new IllegalArgumentException("sie of seqOfParams = "
					+ seq.size());
		}
		...
		if ((seqOfPSh.size() != t) || (seqOfPSw.size() != t)
				|| (seqOfPSK.size() != t)) {
		...
		for (int i = 0; i < t; i++) {
			h[i] = checkBigIntegerInIntRangeAndPositive((((ASN1Integer) seqOfPSh
					.getObjectAt(i))).getValue());
			w[i] = checkBigIntegerInIntRangeAndPositive((((ASN1Integer) seqOfPSw
					.getObjectAt(i))).getValue());
			k[i] = checkBigIntegerInIntRangeAndPositive((((ASN1Integer) seqOfPSK
					.getObjectAt(i))).getValue());
		}
	}
