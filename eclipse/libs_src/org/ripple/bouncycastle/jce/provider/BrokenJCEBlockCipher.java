...
public class BrokenJCEBlockCipher implements BrokenPBE {
	...
	protected BrokenJCEBlockCipher(BlockCipher engine, int pbeType,
			int pbeHash, int pbeKeySize, int pbeIvSize) {
		cipher = new PaddedBufferedBlockCipher(engine);
	...
	protected void engineSetMode(String mode) {
		...
		if (modeName.equals("ECB")) {
			...
			if (modeName.length() != 3) {
				...
				cipher = new PaddedBufferedBlockCipher(new OFBBlockCipher(
						cipher.getUnderlyingCipher(), wordSize));
			} else {
				cipher = new PaddedBufferedBlockCipher(
						new OFBBlockCipher(cipher.getUnderlyingCipher(),
			...
			if (modeName.length() != 3) {
				...
				cipher = new PaddedBufferedBlockCipher(new CFBBlockCipher(
						cipher.getUnderlyingCipher(), wordSize));
			} else {
				cipher = new PaddedBufferedBlockCipher(
						new CFBBlockCipher(cipher.getUnderlyingCipher(),
								8 * cipher.getBlockSize()));
			}
		} else {
	...
	protected void engineInit(int opmode, Key key,
		...
		if (key instanceof BCPBEKey) {
			param = BrokenPBE.Util.makePBEParameters((BCPBEKey) key, params,
					pbeType, pbeHash, cipher.getUnderlyingCipher()
							.getAlgorithmName(), pbeKeySize, pbeIvSize);
			...
			if (rc5Param.getWordSize() != 32) {
				throw new IllegalArgumentException(
						"can only accept RC5 word size 32 (at the moment...)");
			}
	...
	static public class OldPBEWithSHAAndDES3Key extends BrokenJCEBlockCipher {
		public OldPBEWithSHAAndDES3Key() {
			super(new CBCBlockCipher(new DESedeEngine()), OLD_PKCS12, SHA1,
					192, 64);
		}
	}
	...
	static public class OldPBEWithSHAAndTwofish extends BrokenJCEBlockCipher {
		public OldPBEWithSHAAndTwofish() {
			super(new CBCBlockCipher(new TwofishEngine()), OLD_PKCS12, SHA1,
					256, 128);
		}
	}
}
