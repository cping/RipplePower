...
public class X509CRLEntryObject extends X509CRLEntry {
	 ...
	 * is <code>false</code> {@link #getCertificateIssuer()} will always return
	 * <code>null</code>, <code>previousCertificateIssuer</code> is ignored. If
	 * this <code>isIndirect</code> is specified and this CRLEntry has no
	 ...
	 */
	public X509CRLEntryObject(TBSCertList.CRLEntry c, boolean isIndirect,
			X500Name previousCertificateIssuer) {
		this.c = c;
		this.certificateIssuer = loadCertificateIssuer(isIndirect,
	...
	private X500Name loadCertificateIssuer(boolean isIndirect,
			X500Name previousCertificateIssuer) {
		if (!isIndirect) {
			return null;
		}
		...
		try {
			GeneralName[] names = GeneralNames
					.getInstance(ext.getParsedValue()).getNames();
	...
	public String toString() {
		...
		buf.append("      userCertificate: ").append(this.getSerialNumber())
				.append(nl);
		buf.append("       revocationDate: ").append(this.getRevocationDate())
				.append(nl);
		buf.append("       certificateIssuer: ")
				.append(this.getCertificateIssuer()).append(nl);
					...
					if (ext.getExtnValue() != null) {
						...
						try {
							if (oid.equals(X509Extension.reasonCode)) {
								buf.append(
										CRLReason.getInstance(ASN1Enumerated
												.getInstance(dIn.readObject())))
										.append(nl);
							} else if (oid
									.equals(X509Extension.certificateIssuer)) {
								buf.append("Certificate issuer: ")
										.append(GeneralNames.getInstance(dIn
