...
public class JDKDSAPublicKey implements DSAPublicKey {
	...
	JDKDSAPublicKey(DSAPublicKeySpec spec) {
		this.y = spec.getY();
		this.dsaSpec = new DSAParameterSpec(spec.getP(), spec.getQ(),
				spec.getG());
	}
	...
	JDKDSAPublicKey(SubjectPublicKeyInfo info) {
		...
		if (isNotNull(info.getAlgorithm().getParameters())) {
			DSAParameter params = DSAParameter.getInstance(info.getAlgorithm()
					.getParameters());

			this.dsaSpec = new DSAParameterSpec(params.getP(), params.getQ(),
					params.getG());
		}
	}
	...
	public byte[] getEncoded() {
		try {
			if (dsaSpec == null) {
				return new SubjectPublicKeyInfo(new AlgorithmIdentifier(
						X9ObjectIdentifiers.id_dsa), new DERInteger(y))
						.getEncoded(ASN1Encoding.DER);
			}

			return new SubjectPublicKeyInfo(new AlgorithmIdentifier(
					X9ObjectIdentifiers.id_dsa, new DSAParameter(
							dsaSpec.getP(), dsaSpec.getQ(), dsaSpec.getG())),
					new DERInteger(y)).getEncoded(ASN1Encoding.DER);
		} catch (IOException e) {
			return null;
		}
	}
	...
	public String toString() {
		...
		buf.append("            y: ").append(this.getY().toString(16))
				.append(nl);
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		this.dsaSpec = new DSAParameterSpec((BigInteger) in.readObject(),
