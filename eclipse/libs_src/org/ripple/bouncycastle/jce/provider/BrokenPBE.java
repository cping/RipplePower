	...
	static class Util {
		...
		static private void setOddParity(byte[] bytes) {
			for (int i = 0; i < bytes.length; i++) {
				int b = bytes[i];
				bytes[i] = (byte) ((b & 0xfe) | (((b >> 1) ^ (b >> 2)
						^ (b >> 3) ^ (b >> 4) ^ (b >> 5) ^ (b >> 6) ^ (b >> 7)) ^ 0x01));
			}
		}
			...
			if (type == PKCS5S1) {
				...
				default:
					throw new IllegalStateException(
							"PKCS5 scheme 1 only supports only MD5 and SHA1.");
				}
				...
				switch (hash) {
				case MD5:
					generator = new OldPKCS12ParametersGenerator(
							new MD5Digest());
					break;
				case SHA1:
					generator = new OldPKCS12ParametersGenerator(
		 ...
		 * Cipher.
		 */
		static CipherParameters makePBEParameters(BCPBEKey pbeKey,
				AlgorithmParameterSpec spec, int type, int hash,
		 ...
		 * is greater.
		 */
		static CipherParameters makePBEMacParameters(BCPBEKey pbeKey,
				AlgorithmParameterSpec spec, int type, int hash, int keySize) {
			if ((spec == null) || !(spec instanceof PBEParameterSpec)) {
				throw new IllegalArgumentException(
						"Need a PBEParameter spec with a PBE key.");
			}
