...
public class BCDSTU4145PublicKey implements ECPublicKey,
		org.ripple.bouncycastle.jce.interfaces.ECPublicKey, ECPointEncoder {
	...
	public BCDSTU4145PublicKey(
		...
		if (spec.getParams() != null) // can be null if implictlyCa
		{
			...
			EllipticCurve ellipticCurve = EC5Util.convertCurve(curve, spec
					.getParams().getSeed());

			this.ecSpec = EC5Util.convertSpec(ellipticCurve, spec.getParams());
		} else {
			if (q.getCurve() == null) {
				org.ripple.bouncycastle.jce.spec.ECParameterSpec s = BouncyCastleProvider.CONFIGURATION
						.getEcImplicitlyCa();

				q = s.getCurve().createPoint(q.getX().toBigInteger(),
						q.getY().toBigInteger(), false);
			}
			this.ecSpec = null;
		}
	}
	...
	public BCDSTU4145PublicKey(String algorithm, ECPublicKeyParameters params,
		...
		if (spec == null) {
			...
			EllipticCurve ellipticCurve = EC5Util.convertCurve(spec.getCurve(),
					spec.getSeed());
	...
	private ECParameterSpec createSpec(EllipticCurve ellipticCurve,
		...
		return new ECParameterSpec(ellipticCurve, new ECPoint(dp.getG().getX()
				.toBigInteger(), dp.getG().getY().toBigInteger()), dp.getN(),
				dp.getH().intValue());
	}
	...
	private void populateFromPubKeyInfo(SubjectPublicKeyInfo info) {
		if (info.getAlgorithm().getAlgorithm()
				.equals(UAObjectIdentifiers.dstu4145be)
				|| info.getAlgorithm().getAlgorithm()
						.equals(UAObjectIdentifiers.dstu4145le)) {
			DERBitString bits = info.getPublicKeyData();
			ASN1OctetString key;
			this.algorithm = "DSTU4145";

			try {
				key = (ASN1OctetString) ASN1Primitive.fromByteArray(bits
						.getBytes());
			...
			dstuParams = DSTU4145Params.getInstance((ASN1Sequence) info
			...
			if (dstuParams.isNamedCurve()) {
				...
				spec = new ECNamedCurveParameterSpec(curveOid.getId(),
				...
				ECCurve curve = new ECCurve.F2m(field.getM(), field.getK1(),
						field.getK2(), field.getK3(), binary.getA(),
				...
				spec = new org.ripple.bouncycastle.jce.spec.ECParameterSpec(
						curve,
						DSTU4145PointEncoder.decodePoint(curve, g_bytes),
						binary.getN());
			}

			ECCurve curve = spec.getCurve();
			EllipticCurve ellipticCurve = EC5Util.convertCurve(curve,
					spec.getSeed());

			// this.q = curve.createPoint(new BigInteger(1, x), new
			// BigInteger(1, y), false);
			this.q = DSTU4145PointEncoder.decodePoint(curve, keyEnc);

			if (dstuParams.isNamedCurve()) {
				ecSpec = new ECNamedCurveSpec(dstuParams.getNamedCurve()
						.getId(), ellipticCurve, new ECPoint(spec.getG().getX()
						.toBigInteger(), spec.getG().getY().toBigInteger()),
						spec.getN(), spec.getH());
			} else {
				ecSpec = new ECParameterSpec(ellipticCurve, new ECPoint(spec
						.getG().getX().toBigInteger(), spec.getG().getY()
						.toBigInteger()), spec.getN(), spec.getH().intValue());
			}

		} else {
			X962Parameters params = new X962Parameters((ASN1Primitive) info
					.getAlgorithm().getParameters());
			ECCurve curve;
			...
			if (params.isNamedCurve()) {
				...
				ecSpec = new ECNamedCurveSpec(ECUtil.getCurveName(oid),
						ellipticCurve, new ECPoint(ecP.getG().getX()
								.toBigInteger(), ecP.getG().getY()
								.toBigInteger()), ecP.getN(), ecP.getH());
			} else if (params.isImplicitlyCA()) {
				ecSpec = null;
				curve = BouncyCastleProvider.CONFIGURATION.getEcImplicitlyCa()
						.getCurve();
			} else {
				X9ECParameters ecP = X9ECParameters.getInstance(params
						.getParameters());
				...
				this.ecSpec = new ECParameterSpec(ellipticCurve, new ECPoint(
						ecP.getG().getX().toBigInteger(), ecP.getG().getY()
								.toBigInteger()), ecP.getN(), ecP.getH()
						.intValue());
			}
			...
			if (data[0] == 0x04 && data[1] == data.length - 2
				...
				if (qLength >= data.length - 3) {
					try {
						key = (ASN1OctetString) ASN1Primitive
								.fromByteArray(data);
					} catch (IOException ex) {
						throw new IllegalArgumentException(
								"error recovering public key");
		...
		if (algorithm.equals("DSTU4145")) {
			if (dstuParams != null) {
				...
				if (ecSpec instanceof ECNamedCurveSpec) {
					params = new DSTU4145Params(new ASN1ObjectIdentifier(
					...
					X9ECParameters ecP = new X9ECParameters(curve,
							EC5Util.convertPoint(curve, ecSpec.getGenerator(),
									withCompression), ecSpec.getOrder(),
			...
			try {
				info = new SubjectPublicKeyInfo(new AlgorithmIdentifier(
						UAObjectIdentifiers.dstu4145be, params),
			...
			if (ecSpec instanceof ECNamedCurveSpec) {
				...
				if (curveOid == null) {
					curveOid = new ASN1ObjectIdentifier(
							((ECNamedCurveSpec) ecSpec).getName());
				}
			...
			ASN1OctetString p = (ASN1OctetString) new X9ECPoint(
					curve.createPoint(this.getQ().getX().toBigInteger(), this
							.getQ().getY().toBigInteger(), withCompression))
					.toASN1Primitive();
	...
	public org.ripple.bouncycastle.math.ec.ECPoint getQ() {
		if (ecSpec == null) {
			if (q instanceof org.ripple.bouncycastle.math.ec.ECPoint.Fp) {
				return new org.ripple.bouncycastle.math.ec.ECPoint.Fp(null,
						q.getX(), q.getY());
			} else {
				return new org.ripple.bouncycastle.math.ec.ECPoint.F2m(null,
						q.getX(), q.getY());
			}
		}
	...
	public String toString() {
		...
		buf.append("            X: ")
				.append(this.q.getX().toBigInteger().toString(16)).append(nl);
		buf.append("            Y: ")
				.append(this.q.getY().toBigInteger().toString(16)).append(nl);
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		populateFromPubKeyInfo(SubjectPublicKeyInfo.getInstance(ASN1Primitive
				.fromByteArray(enc)));
	}
