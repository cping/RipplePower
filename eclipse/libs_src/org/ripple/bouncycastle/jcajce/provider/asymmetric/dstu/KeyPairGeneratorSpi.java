...
public class KeyPairGeneratorSpi extends java.security.KeyPairGenerator {
	...
	public void initialize(AlgorithmParameterSpec params, SecureRandom random)
		...
		if (params instanceof ECParameterSpec) {
			...
			param = new ECKeyGenerationParameters(new ECDomainParameters(curve,
					g, p.getOrder(), BigInteger.valueOf(p.getCofactor())),
					random);
			...
			// ECGOST3410NamedCurves.getByName(curveName);
			ECDomainParameters ecP = DSTU4145NamedCurves
					.getByOID(new ASN1ObjectIdentifier(curveName));
			if (ecP == null) {
			...
			initialised = true;
				...
				&& BouncyCastleProvider.CONFIGURATION.getEcImplicitlyCa() != null) {
			ECParameterSpec p = BouncyCastleProvider.CONFIGURATION
					.getEcImplicitlyCa();
			this.ecParams = params;
	...
	public KeyPair generateKeyPair() {
		if (!initialised) {
			throw new IllegalStateException(
					"DSTU Key Pair Generator not initialised");
		}
		...
		if (ecParams instanceof ECParameterSpec) {
			...
			BCDSTU4145PublicKey pubKey = new BCDSTU4145PublicKey(algorithm,
					pub, p);
			return new KeyPair(pubKey, new BCDSTU4145PrivateKey(algorithm,
					priv, pubKey, p));
