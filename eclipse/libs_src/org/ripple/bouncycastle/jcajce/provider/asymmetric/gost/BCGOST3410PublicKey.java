...
public class BCGOST3410PublicKey implements GOST3410PublicKey {
	...
	BCGOST3410PublicKey(GOST3410PublicKeySpec spec) {
		...
		this.gost3410Spec = new GOST3410ParameterSpec(
				new GOST3410PublicKeyParameterSetSpec(spec.getP(), spec.getQ(),
						spec.getA()));
	}
	...
	BCGOST3410PublicKey(GOST3410PublicKeyParameters params,
	...
	BCGOST3410PublicKey(SubjectPublicKeyInfo info) {
		...
		try {
			...
			for (int i = 0; i != keyEnc.length; i++) {
				keyBytes[i] = keyEnc[keyEnc.length - 1 - i]; // was little
																// endian
			}
			...
			throw new IllegalArgumentException(
					"invalid info structure in GOST3410 public key");
		}
	...
	public byte[] getEncoded() {
		...
		for (int i = 0; i != keyBytes.length; i++) {
			keyBytes[i] = keyEnc[keyEnc.length - 1 - i]; // must be little
															// endian
		}
				...
				if (gost3410Spec.getEncryptionParamSetOID() != null) {
					info = new SubjectPublicKeyInfo(new AlgorithmIdentifier(
							...
							new GOST3410PublicKeyAlgParameters(
									new ASN1ObjectIdentifier(gost3410Spec
	...
	public boolean equals(Object o) {
		if (o instanceof BCGOST3410PublicKey) {
			...
			return this.y.equals(other.y)
					&& this.gost3410Spec.equals(other.gost3410Spec);
		}
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		if (publicKeyParamSetOID != null) {
			this.gost3410Spec = new GOST3410ParameterSpec(publicKeyParamSetOID,
					(String) in.readObject(), (String) in.readObject());
		} else {
			this.gost3410Spec = new GOST3410ParameterSpec(
					new GOST3410PublicKeyParameterSetSpec(
