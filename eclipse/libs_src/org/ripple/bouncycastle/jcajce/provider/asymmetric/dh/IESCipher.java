...
public class IESCipher extends CipherSpi {
	...
	public void engineInit(int opmode, Key key,
		...
		if (opmode == Cipher.ENCRYPT_MODE || opmode == Cipher.WRAP_MODE) {
			if (key instanceof DHPublicKey) {
				...
				this.otherKeyParameter = DHUtil
						.generatePrivateKeyParameter(ieKey.getPrivate());
			} else {
				throw new InvalidKeyException(
						"must be passed recipient's public DH key for encryption");
			}
			...
			if (key instanceof DHPrivateKey) {
				...
				this.key = DHUtil.generatePrivateKeyParameter(ieKey
						.getPrivate());
			} else {
				throw new InvalidKeyException(
						"must be passed recipient's private DH key for decryption");
			}
		} else {
	...
	public byte[] engineDoFinal(byte[] input, int inputOffset, int inputLen)
		...
		if (state == Cipher.ENCRYPT_MODE || state == Cipher.WRAP_MODE) {
			...
			EphemeralKeyPairGenerator kGen = new EphemeralKeyPairGenerator(gen,
					new KeyEncoder() {
						public byte[] getEncoded(
							...
							byte[] Vloc = new byte[(((DHKeyParameters) keyParameter)
									.getParameters().getP().bitLength() + 7) / 8];
							byte[] Vtmp = BigIntegers
									.asUnsignedByteArray(((DHPublicKeyParameters) keyParameter)
											.getY());

							if (Vtmp.length > Vloc.length) {
								throw new IllegalArgumentException(
										"Senders's public key longer than expected.");
							} else {
								System.arraycopy(Vtmp, 0, Vloc, Vloc.length
										- Vtmp.length, Vtmp.length);
							}
			...
			// Decrypt the buffer
			try {
				engine.init(key, params, new DHIESPublicKeyParser(
						((DHKeyParameters) key).getParameters()));
	...
	static public class IES extends IESCipher {
		public IES() {
			super(new IESEngine(new DHBasicAgreement(), new KDF2BytesGenerator(
