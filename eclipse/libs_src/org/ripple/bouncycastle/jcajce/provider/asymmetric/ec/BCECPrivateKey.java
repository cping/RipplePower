...
public class BCECPrivateKey implements ECPrivateKey,
		org.ripple.bouncycastle.jce.interfaces.ECPrivateKey,
	...
	public BCECPrivateKey(String algorithm,
		...
		if (spec.getParams() != null) // can be null if implicitlyCA
		{
			...
			ellipticCurve = EC5Util.convertCurve(curve, spec.getParams()
					.getSeed());
	...
	public BCECPrivateKey(String algorithm, ECPrivateKeySpec spec,
			ProviderConfiguration configuration) {
		this.algorithm = algorithm;
		this.d = spec.getS();
	...
	public BCECPrivateKey(String algorithm, ECPrivateKeyParameters params,
		...
		if (spec == null) {
			...
			this.ecSpec = new ECParameterSpec(ellipticCurve, new ECPoint(dp
					.getG().getX().toBigInteger(), dp.getG().getY()
					.toBigInteger()), dp.getN(), dp.getH().intValue());
		} else {
			this.ecSpec = spec;
		}
	...
	public BCECPrivateKey(String algorithm, ECPrivateKeyParameters params,
		...
		if (spec == null) {
			...
			this.ecSpec = new ECParameterSpec(ellipticCurve, new ECPoint(spec
	...
	BCECPrivateKey(String algorithm, PrivateKeyInfo info,
			ProviderConfiguration configuration) throws IOException {
		this.algorithm = algorithm;
		this.configuration = configuration;
	...
	private void populateFromPrivKeyInfo(PrivateKeyInfo info)
			throws IOException {
		X962Parameters params = X962Parameters.getInstance(info
				.getPrivateKeyAlgorithm().getParameters());

		if (params.isNamedCurve()) {
			ASN1ObjectIdentifier oid = ASN1ObjectIdentifier.getInstance(params
					.getParameters());
			...
			if (ecP == null) // GOST Curve
			{
				...
				EllipticCurve ellipticCurve = EC5Util.convertCurve(
						gParam.getCurve(), gParam.getSeed());

				ecSpec = new ECNamedCurveSpec(
						ECGOST3410NamedCurves.getName(oid), ellipticCurve,
						new ECPoint(gParam.getG().getX().toBigInteger(), gParam
								.getG().getY().toBigInteger()), gParam.getN(),
						gParam.getH());
			} else {
				EllipticCurve ellipticCurve = EC5Util.convertCurve(
						ecP.getCurve(), ecP.getSeed());
			...
			X9ECParameters ecP = X9ECParameters.getInstance(params
					.getParameters());
			EllipticCurve ellipticCurve = EC5Util.convertCurve(ecP.getCurve(),
					ecP.getSeed());

			this.ecSpec = new ECParameterSpec(ellipticCurve, new ECPoint(ecP
					.getG().getX().toBigInteger(), ecP.getG().getY()
					.toBigInteger()), ecP.getN(), ecP.getH().intValue());
		}
		...
		if (privKey instanceof DERInteger) {
			...
			org.ripple.bouncycastle.asn1.sec.ECPrivateKey ec = org.ripple.bouncycastle.asn1.sec.ECPrivateKey
					.getInstance(privKey);
	...
	public byte[] getEncoded() {
		...
		if (ecSpec instanceof ECNamedCurveSpec) {
			DERObjectIdentifier curveOid = ECUtil
					.getNamedCurveOid(((ECNamedCurveSpec) ecSpec).getName());
			if (curveOid == null) // guess it's the OID
			{
				curveOid = new DERObjectIdentifier(
			...
			X9ECParameters ecP = new X9ECParameters(curve,
					...
					BigInteger.valueOf(ecSpec.getCofactor()), ecSpec.getCurve()
							.getSeed());
		...
		if (publicKey != null) {
			keyStructure = new org.ripple.bouncycastle.asn1.sec.ECPrivateKey(
	...
	private DERBitString getPublicKeyDetails(BCECPublicKey pub) {
		try {
			SubjectPublicKeyInfo info = SubjectPublicKeyInfo
					.getInstance(ASN1Primitive.fromByteArray(pub.getEncoded()));
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		populateFromPrivKeyInfo(PrivateKeyInfo.getInstance(ASN1Primitive
				.fromByteArray(enc)));
