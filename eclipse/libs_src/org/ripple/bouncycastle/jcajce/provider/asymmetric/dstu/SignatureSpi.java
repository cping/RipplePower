...
public class SignatureSpi extends java.security.SignatureSpi implements
	...
	private static byte[] DEFAULT_SBOX = { 0xa, 0x9, 0xd, 0x6, 0xe, 0xb, 0x4,
			0x5, 0xf, 0x1, 0x3, 0xc, 0x7, 0x0, 0x8, 0x2, 0x8, 0x0, 0xc, 0x4,
			0x9, 0x6, 0x7, 0xb, 0x2, 0x3, 0x1, 0xf, 0x5, 0xe, 0xa, 0xd, 0xf,
			0x6, 0x5, 0x8, 0xe, 0xb, 0xa, 0x4, 0xc, 0x0, 0x3, 0x7, 0x2, 0x9,
			0x1, 0xd, 0x3, 0x8, 0xd, 0x9, 0x6, 0xb, 0xf, 0x0, 0x2, 0x5, 0xc,
			0xa, 0x4, 0xe, 0x1, 0x7, 0xf, 0x8, 0xe, 0x9, 0x7, 0x2, 0x0, 0xd,
			0xc, 0x6, 0x1, 0x5, 0xb, 0x4, 0x3, 0xa, 0x2, 0x8, 0x9, 0x7, 0x5,
			0xf, 0x0, 0xb, 0xc, 0x1, 0xd, 0xe, 0xa, 0x3, 0x6, 0x4, 0x3, 0x8,
			0xb, 0x5, 0x6, 0x4, 0xe, 0xa, 0x2, 0xc, 0x1, 0x7, 0x9, 0xf, 0xd,
			0x0, 0x1, 0x2, 0x3, 0xe, 0x6, 0xd, 0xb, 0x8, 0xf, 0xa, 0xc, 0x5,
			0x7, 0x9, 0x0, 0x4 };
	...
	protected void engineInitVerify(PublicKey publicKey)
		...
		digest = new GOST3411Digest(
				expandSbox(((BCDSTU4145PublicKey) publicKey).getSbox()));
	...
	protected byte[] engineSign() throws SignatureException {
		...
		try {
			...
			byte[] sigBytes = new byte[(r.length > s.length ? r.length * 2
					: s.length * 2)];
			System.arraycopy(s, 0, sigBytes, (sigBytes.length / 2) - s.length,
					s.length);
			System.arraycopy(r, 0, sigBytes, sigBytes.length - r.length,
					r.length);
	...
	protected boolean engineVerify(byte[] sigBytes) throws SignatureException {
		...
		try {
			byte[] bytes = ((ASN1OctetString) ASN1OctetString
					.fromByteArray(sigBytes)).getOctets();
