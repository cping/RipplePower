...
public class BcKeyStoreSpi extends KeyStoreSpi implements BCKeyStore {
		...
		try {
			...
			case KEY_PRIVATE:
				return KeyFactory.getInstance(algorithm,
						BouncyCastleProvider.PROVIDER_NAME).generatePrivate(
						spec);
			case KEY_PUBLIC:
				return KeyFactory.getInstance(algorithm,
						BouncyCastleProvider.PROVIDER_NAME)
	...
	public void engineLoad(InputStream stream, char[] password)
		...
		if (password != null && password.length != 0) {
			...
			if (version != 2) {
				macParams = pbeGen.generateDerivedMacParameters(hMac
						.getMacSize());
			} else {
				macParams = pbeGen.generateDerivedMacParameters(hMac
						.getMacSize() * 8);
	 ...
	 * stored encrypted on disk, so the password is mandatory, however if you
