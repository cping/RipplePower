...
public class PKCS12KeyStoreSpi extends KeyStoreSpi implements
	...
	public PKCS12KeyStoreSpi(Provider provider,
		...
		try {
			...
			throw new IllegalArgumentException("can't create cert factory - "
					+ e.toString());
		}
	}
	...
	private SubjectKeyIdentifier createSubjectKeyId(PublicKey pubKey) {
		try {
			SubjectPublicKeyInfo info = new SubjectPublicKeyInfo(
					(ASN1Sequence) ASN1Primitive.fromByteArray(pubKey
							.getEncoded()));
	...
	public Certificate engineGetCertificate(String alias) {
		if (alias == null) {
			throw new IllegalArgumentException(
					"null alias passed to getCertificate.");
		}
	...
	public Certificate[] engineGetCertificateChain(String alias) {
		...
		if (c != null) {
			...
			while (c != null) {
				...
				byte[] bytes = x509c
						.getExtensionValue(Extension.authorityKeyIdentifier
								.getId());
				if (bytes != null) {
					try {
						ASN1InputStream aIn = new ASN1InputStream(bytes);

						byte[] authBytes = ((ASN1OctetString) aIn.readObject())
								.getOctets();
						aIn = new ASN1InputStream(authBytes);

						AuthorityKeyIdentifier id = AuthorityKeyIdentifier
								.getInstance(aIn.readObject());
						if (id.getKeyIdentifier() != null) {
							nextC = (Certificate) chainCerts.get(new CertId(id
									.getKeyIdentifier()));
						}
				...
				if (nextC == null) {
					...
					if (!i.equals(s)) {
						...
						while (e.hasMoreElements()) {
							X509Certificate crt = (X509Certificate) chainCerts
									.get(e.nextElement());
	...
	public void engineSetCertificateEntry(String alias, Certificate cert)
		...
		if (keys.get(alias) != null) {
			throw new KeyStoreException("There is a key entry with the name "
					+ alias + ".");
		}
	...
	public void engineSetKeyEntry(String alias, Key key, char[] password,
		...
		if (!(key instanceof PrivateKey)) {
			throw new KeyStoreException(
					"PKCS12 does not support non-PrivateKeys");
		}
	...
	protected PrivateKey unwrapKey(AlgorithmIdentifier algId, byte[] data,
		...
		try {
			if (algorithm.on(PKCSObjectIdentifiers.pkcs_12PbeIds)) {
				...
				PBES2Parameters alg = PBES2Parameters.getInstance(algId
						.getParameters());
				PBKDF2Params func = PBKDF2Params.getInstance(alg
						.getKeyDerivationFunc().getParameters());

				SecretKeyFactory keyFact = SecretKeyFactory.getInstance(alg
						.getKeyDerivationFunc().getAlgorithm().getId(),
						bcProvider);

				SecretKey k = keyFact.generateSecret(new PBEKeySpec(password,
						func.getSalt(), func.getIterationCount().intValue(),
						SecretKeyUtil.getKeySize(alg.getEncryptionScheme()
								.getAlgorithm())));

				Cipher cipher = Cipher.getInstance(alg.getEncryptionScheme()
						.getAlgorithm().getId(), bcProvider);

				cipher.init(
						Cipher.UNWRAP_MODE,
						k,
						new IvParameterSpec(ASN1OctetString.getInstance(
		...
		throw new IOException(
				"exception unwrapping private key - cannot recognise: "
						+ algorithm);
	}

	protected byte[] wrapKey(String algorithm, Key key,
			PKCS12PBEParams pbeParams, char[] password) throws IOException {
		PBEKeySpec pbeSpec = new PBEKeySpec(password);
		byte[] out;
	...
	protected byte[] cryptData(boolean forEncryption,
			AlgorithmIdentifier algId, char[] password,
		...
		try {
			...
			Cipher cipher = Cipher.getInstance(algorithm, bcProvider);
			int mode = forEncryption ? Cipher.ENCRYPT_MODE
					: Cipher.DECRYPT_MODE;
	...
	public void engineLoad(InputStream stream, char[] password)
		...
		if (bag.getMacData() != null) // check the mac code
		{
			...
			try {
				byte[] res = calculatePbeMac(algId.getAlgorithm(), salt,
				...
				if (!Arrays.constantTimeAreEqual(res, dig)) {
					...
					res = calculatePbeMac(algId.getAlgorithm(), salt, itCount,
							password, true, data);
				...
				if (c[i].getContentType().equals(data)) {
					...
					for (int j = 0; j != seq.size(); j++) {
						...
						if (b.getBagId().equals(pkcs8ShroudedKeyBag)) {
							org.ripple.bouncycastle.asn1.pkcs.EncryptedPrivateKeyInfo eIn = org.ripple.bouncycastle.asn1.pkcs.EncryptedPrivateKeyInfo
					...
					for (int j = 0; j != seq.size(); j++) {
						...
						if (b.getBagId().equals(certBag)) {
							...
							while (e.hasMoreElements()) {
								...
								if (attrSet.size() > 0) {
									...
									if (existing != null) {
										...
										if (!existing.toASN1Primitive().equals(
												attr)) {
											throw new IOException(
													"attempt to add existing attribute with different value");
										}
							...
							org.ripple.bouncycastle.asn1.pkcs.PrivateKeyInfo kInfo = org.ripple.bouncycastle.asn1.pkcs.PrivateKeyInfo
									.getInstance(b.getBagValue());
					...
					System.out
							.println("extra " + c[i].getContentType().getId());
		...
		for (int i = 0; i != chain.size(); i++) {
			...
			if (b.getBagAttributes() != null) {
				...
				while (e.hasMoreElements()) {
					...
					ASN1ObjectIdentifier oid = (ASN1ObjectIdentifier) sq
							.getObjectAt(0);
					ASN1Primitive attr = (ASN1Primitive) ((ASN1Set) sq
							.getObjectAt(1)).getObjectAt(0);
			...
			if (unmarkedKey) {
				if (keyCerts.isEmpty()) {
					String name = new String(Hex.encode(createSubjectKeyId(
							cert.getPublicKey()).getKeyIdentifier()));
	...
	public void engineStore(LoadStoreParameter param) throws IOException,
		...
		if (param instanceof PKCS12StoreParameter) {
			...
			bcParam = new PKCS12StoreParameter(
	...
	private void doStore(OutputStream stream, char[] password,
			boolean useDEREncoding) throws IOException {
		if (password == null) {
			throw new NullPointerException(
					"No password supplied for PKCS#12 KeyStore.");
		}
		...
		while (ks.hasMoreElements()) {
			...
			byte[] kBytes = wrapKey(keyAlgorithm.getId(), privKey, kParams,
					password);
			AlgorithmIdentifier kAlgId = new AlgorithmIdentifier(keyAlgorithm,
					kParams.toASN1Primitive());
			org.ripple.bouncycastle.asn1.pkcs.EncryptedPrivateKeyInfo kInfo = new org.ripple.bouncycastle.asn1.pkcs.EncryptedPrivateKeyInfo(
					kAlgId, kBytes);
			...
			if (privKey instanceof PKCS12BagAttributeCarrier) {
				...
				//
				DERBMPString nm = (DERBMPString) bagAttrs
						.getBagAttribute(pkcs_9_at_friendlyName);
				if (nm == null || !nm.getString().equals(name)) {
					bagAttrs.setBagAttribute(pkcs_9_at_friendlyName,
							new DERBMPString(name));
				}
				...
				while (e.hasMoreElements()) {
					ASN1ObjectIdentifier oid = (ASN1ObjectIdentifier) e
							.nextElement();
			...
			SafeBag kBag = new SafeBag(pkcs8ShroudedKeyBag,
					kInfo.toASN1Primitive(), new DERSet(kName));
			keyS.add(kBag);
		}
		...
		AlgorithmIdentifier cAlgId = new AlgorithmIdentifier(certAlgorithm,
				cParams.toASN1Primitive());
		...
		while (cs.hasMoreElements()) {
			try {
				...
				if (cert instanceof PKCS12BagAttributeCarrier) {
					...
					if (bagAttrs.getBagAttribute(pkcs_9_at_localKeyId) == null) {
						bagAttrs.setBagAttribute(pkcs_9_at_localKeyId,
								createSubjectKeyId(cert.getPublicKey()));
					}
				...
				SafeBag sBag = new SafeBag(certBag, cBag.toASN1Primitive(),
						new DERSet(fName));
				...
				throw new IOException("Error encoding certificate: "
						+ e.toString());
			}
		}
		...
		byte[] certSeqEncoded = new DERSequence(certSeq)
				.getEncoded(ASN1Encoding.DER);
		byte[] certBytes = cryptData(true, cAlgId, password, false,
				certSeqEncoded);
		EncryptedData cInfo = new EncryptedData(data, cAlgId,
		...
		try {
			byte[] res = calculatePbeMac(id_SHA1, mSalt, itCount, password,
					false, data);

			AlgorithmIdentifier algId = new AlgorithmIdentifier(id_SHA1,
					DERNull.INSTANCE);
	...
	public static class BCPKCS12KeyStore extends PKCS12KeyStoreSpi {
		public BCPKCS12KeyStore() {
			super(bcProvider, pbeWithSHAAnd3_KeyTripleDES_CBC,
	...
	private static class IgnoresCaseHashtable {
		...
		public Object remove(String alias) {
			String k = (String) keys.remove(alias == null ? null : Strings
					.toLowerCase(alias));
			if (k == null) {
				return null;
			}
		...
		public Object get(String alias) {
			String k = (String) keys.get(alias == null ? null : Strings
					.toLowerCase(alias));
			if (k == null) {
				return null;
			}
