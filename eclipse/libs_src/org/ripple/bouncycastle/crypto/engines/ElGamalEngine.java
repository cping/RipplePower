...
public class ElGamalEngine implements AsymmetricBlockCipher {
	...
	public byte[] processBlock(byte[] in, int inOff, int inLen) {
		...
		int maxLength = forEncryption ? (bitSize - 1 + 7) / 8
				: getInputBlockSize();
		...
		if (key instanceof ElGamalPrivateKeyParameters) // decryption
		{
			...
			BigInteger m = gamma
					.modPow(p.subtract(ONE).subtract(priv.getX()), p)
					.multiply(phi).mod(p);
		...
		} else // encryption
		{
			...
			if (out1.length > output.length / 2) {
				...
				System.arraycopy(out1, 0, output, output.length / 2
						- out1.length, out1.length);
			}

			if (out2.length > output.length / 2) {
				System.arraycopy(out2, 1, output, output.length
						- (out2.length - 1), out2.length - 1);
			} else {
