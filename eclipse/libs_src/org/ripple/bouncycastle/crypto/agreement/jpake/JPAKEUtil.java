...
public class JPAKEUtil {
	 ...
	 */
	public static BigInteger calculateGA(BigInteger p, BigInteger gx1,
			BigInteger gx3, BigInteger gx4) {
	 ...
	 */
	public static BigInteger calculateX2s(BigInteger q, BigInteger x2,
			BigInteger s) {
	 ...
	 */
	public static BigInteger calculateA(BigInteger p, BigInteger q,
			BigInteger gA, BigInteger x2s) {
		// A = ga^(x*s)
		return gA.modPow(x2s, p);
	}

	/**
	 * Calculate a zero knowledge proof of x using Schnorr's signature. The
	 * returned array has two elements {g^v, r = v-x*h} for x.
	 */
	public static BigInteger[] calculateZeroKnowledgeProof(BigInteger p,
		...
		BigInteger h = calculateHashForZeroKnowledgeProof(g, gv, gx,
				participantId, digest); // h
	...
	private static BigInteger calculateHashForZeroKnowledgeProof(BigInteger g,
			BigInteger gr, BigInteger gx, String participantId, Digest digest) {
		digest.reset();
	...
	public static void validateGx4(BigInteger gx4) throws CryptoException {
		if (gx4.equals(ONE)) {
			throw new CryptoException(
					"g^x validation failed.  g^x should not be 1.");
		}
	}
	 ...
	 * <p/>
	 * <blockquote> Alice could simply check ga != 1 to ensure it is a
	 * generator. In fact, as we will explain in Section 3, (x1 + x3 + x4 ) is
	 * random over Zq even in the face of active attacks. Hence, the probability
	 * for ga = 1 is extremely small - on the order of 2^160 for 160-bit q.
	 * </blockquote>
	 * 
	 * @throws CryptoException
	 *             if ga is 1
	 ...
	 */
	public static void validateZeroKnowledgeProof(BigInteger p, BigInteger q,
			BigInteger g, BigInteger gx, BigInteger[] zeroKnowledgeProof,
		...
		/*
		 * Below, I took an straightforward way to compute g^r * g^x^h, which
		 * needs 2 exp. Using a simultaneous computation technique would only
		 * need 1 exp.
		 */
		g.modPow(r, p).multiply(gx.modPow(h, p)).mod(p).compareTo(gv) == 0)) // g^v=g^r
																				// *
																				// g^x^h
		{
	...
	public static BigInteger calculateKeyingMaterial(BigInteger p,
			BigInteger q, BigInteger gx4, BigInteger x2, BigInteger s,
			BigInteger B) {
		return gx4.modPow(x2.multiply(s).negate().mod(q), p).multiply(B)
				.modPow(x2, p);
	}
	...
	public static void validateParticipantIdsDiffer(String participantId1,
			String participantId2) throws CryptoException {
	...
	/**
	 * Validates that the given participant ids are equal. This is used to
	 * ensure that the payloads received from each round all come from the same
	 * participant.
	 * 
	 * @throws CryptoException
	 *             if the participantId strings are equal.
	...
	public static void validateParticipantIdsEqual(
			String expectedParticipantId, String actualParticipantId)
			throws CryptoException {
	 ...
	 * >NIST SP 800-56A Revision 1</a>, Section 8.2 Unilateral Key Confirmation
	...
	public static BigInteger calculateMacTag(String participantId,
			String partnerParticipantId, BigInteger gx1, BigInteger gx2,
			BigInteger gx3, BigInteger gx4, BigInteger keyingMaterial,
			Digest digest) {
		...
		/*
		 * MacData = "KC_1_U" || participantId_Alice || participantId_Bob || gx1
		 * || gx2 || gx3 || gx4.
		 */
	...
	/**
	 * Calculates the MacKey (i.e. the key to use when calculating the MagTag
	 ...
	 * </pre>
	 */
	private static byte[] calculateMacKey(BigInteger keyingMaterial,
			Digest digest) {
		digest.reset();

		updateDigest(digest, keyingMaterial);
		/*
		 * This constant is used to ensure that the macKey is NOT the same as
		 * the derived key.
		 */
	...
	public static void validateMacTag(String participantId,
			String partnerParticipantId, BigInteger gx1, BigInteger gx2,
			BigInteger gx3, BigInteger gx4, BigInteger keyingMaterial,
			Digest digest, BigInteger partnerMacTag) throws CryptoException {
		/*
		 ...
		 */
		BigInteger expectedMacTag = calculateMacTag(partnerParticipantId,
	...
	private static void updateDigestIncludingSize(Digest digest,
