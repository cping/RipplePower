...
/**
 * A CipherInputStream is composed of an InputStream and a BufferedBlockCipher
 * so that read() methods return data that are read in from the underlying
 * InputStream but have been additionally processed by the Cipher. The Cipher
 * must be fully initialized before being used by a CipherInputStream.
 * <p>
	...
	private int nextChunk() throws IOException {
		...
		if (available < 0) {
			...
			try {
				...
				throw new IOException("error processing stream: "
						+ e.toString());
			}
			...
			try {
				if (bufferedBlockCipher != null) {
					maxBuf = bufferedBlockCipher.processBytes(inBuf, 0,
							available, buf, 0);
				} else {
