...
public class TlsECCUtils {
	...
	public static byte[] createSupportedPointFormatsExtension(
		...
		} else if (!TlsProtocol.arrayContains(ecPointFormats,
	...
	public static short[] readSupportedPointFormatsExtension(
		...
		if (!TlsProtocol.arrayContains(ecPointFormats,
				ECPointFormat.uncompressed)) {
			/*
			 * RFC 4492 5.1. If the Supported Point Formats Extension is indeed
			 * sent, it MUST contain the value 0 (uncompressed) as one of the
		...
		try {
			...
			case ECCurveType.named_curve: {
				...
				if (!TlsProtocol.arrayContains(namedCurves, namedCurve)) {
					/*
					 * RFC 4492 4. [...] servers MUST NOT negotiate the use of
					 * an ECC cipher suite unless they can complete the
					 * handshake while respecting the choice of curves and
					 * compression techniques specified by the client.
					 */
					throw new TlsFatalAlert(AlertDescription.illegal_parameter);
				}
	...
	public static void writeExplicitECParameters(short[] ecPointFormats,
		...
		writeECFieldElement(curve.getFieldSize(), curve.getA().toBigInteger(),
				output);
		writeECFieldElement(curve.getFieldSize(), curve.getB().toBigInteger(),
				output);
