...
public class DTLSClientProtocol extends DTLSProtocol {
	...
	protected DTLSTransport clientHandshake(ClientHandshakeState state,
		...
		{
			// NOTE: After receiving a record from the server, we discover the
			// record layer version
			ProtocolVersion server_version = recordLayer
		...
		while (serverMessage.getType() == HandshakeType.hello_verify_request) {
			byte[] cookie = parseHelloVerifyRequest(state.clientContext,
		...
		byte[] clientVerifyData = TlsUtils.calculateVerifyData(
				state.clientContext, "client finished",
		...
		byte[] expectedServerVerifyData = TlsUtils.calculateVerifyData(
				state.clientContext, "server finished",
				handshake.getCurrentHash());
		serverMessage = handshake.receiveMessage();
		...
		// Cipher Suites (and SCSV)
		{
			...
			boolean noRenegExt = state.clientExtensions == null
					|| state.clientExtensions
							.get(TlsProtocol.EXT_RenegotiationInfo) == null;
			...
			if (noRenegExt) {
				TlsUtils.writeUint16(
						CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV, buf);
			}
		}
	...
	protected void processServerCertificate(ClientHandshakeState state,
			byte[] body) throws IOException {
		...
		if (serverExtensions != null) {
			...
			while (e.hasMoreElements()) {
				...
				if (!extType.equals(TlsProtocol.EXT_RenegotiationInfo)
						&& (state.clientExtensions == null || state.clientExtensions
								.get(extType) == null)) {
					/*
			...
			state.expectSessionTicket = serverExtensions
