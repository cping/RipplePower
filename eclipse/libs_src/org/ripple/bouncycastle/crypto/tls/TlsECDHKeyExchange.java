...
public class TlsECDHKeyExchange extends AbstractTlsKeyExchange {
	...
	public void generateClientKeyExchange(OutputStream output)
		...
		AsymmetricCipherKeyPair ecAgreeClientKeyPair = TlsECCUtils
				.generateECKeyPair(context.getSecureRandom(),
						ecAgreeServerPublicKey.getParameters());
		this.ecAgreeClientPrivateKey = (ECPrivateKeyParameters) ecAgreeClientKeyPair
				.getPrivate();
	...
	public void processClientKeyExchange(InputStream input) throws IOException {
		...
		if (ecAgreeClientPublicKey != null) {
			// For ecdsa_fixed_ecdh and rsa_fixed_ecdh, the key arrived in the
			// client certificate
			return;
		}
		...
		ECDomainParameters curve_params = this.ecAgreeServerPrivateKey
				.getParameters();

		this.ecAgreeClientPublicKey = TlsECCUtils
	...
	public byte[] generatePremasterSecret() throws IOException {
		if (agreementCredentials != null) {
			return agreementCredentials
					.generateAgreement(ecAgreeServerPublicKey);
		}

		if (ecAgreeServerPrivateKey != null) {
			return TlsECCUtils.calculateECDHBasicAgreement(
					ecAgreeClientPublicKey, ecAgreeServerPrivateKey);
		}

		if (ecAgreeClientPrivateKey != null) {
			return TlsECCUtils.calculateECDHBasicAgreement(
					ecAgreeServerPublicKey, ecAgreeClientPrivateKey);
		}
