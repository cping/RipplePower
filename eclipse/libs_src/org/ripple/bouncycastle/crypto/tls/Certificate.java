...
public class Certificate {
	...
	public Certificate(
			org.ripple.bouncycastle.asn1.x509.Certificate[] certificateList) {
		if (certificateList == null) {
			throw new IllegalArgumentException(
					"'certificateList' cannot be null");
		}
	...
	/**
	 * @return an array of {@link org.ripple.bouncycastle.asn1.x509.Certificate}
	...
	public org.ripple.bouncycastle.asn1.x509.Certificate getCertificateAt(
			int index) {
	...
	/**
	 * @return <code>true</code> if this certificate chain contains no
	 *         certificates, or <code>false</code> otherwise.
	 */
	...
	public static Certificate parse(InputStream input) throws IOException {
		...
		while (left > 0) {
			...
			tmp.addElement(org.ripple.bouncycastle.asn1.x509.Certificate
					.getInstance(asn1));
		}
		certs = new org.ripple.bouncycastle.asn1.x509.Certificate[tmp.size()];
		for (int i = 0; i < tmp.size(); i++) {
			certs[i] = (org.ripple.bouncycastle.asn1.x509.Certificate) tmp
					.elementAt(i);
		}
		return new Certificate(certs);
	}

	private org.ripple.bouncycastle.asn1.x509.Certificate[] clone(
			org.ripple.bouncycastle.asn1.x509.Certificate[] list) {
