...
public class TlsAEADCipher implements TlsCipher {
	...
	public TlsAEADCipher(TlsContext context, AEADBlockCipher clientWriteCipher,
			AEADBlockCipher serverWriteCipher, int cipherKeySize, int macSize)
			throws IOException {

		if (!ProtocolVersion.TLSv12.isEqualOrEarlierVersionOf(context
		...
		// NOTE: Valid for RFC 5288 ciphers but may need review for other AEAD
		// ciphers
		...
		KeyParameter server_write_key = new KeyParameter(key_block, offset,
				cipherKeySize);
		offset += cipherKeySize;
		byte[] client_write_IV = Arrays.copyOfRange(key_block, offset, offset
				+ fixed_iv_length);
		offset += fixed_iv_length;
		byte[] server_write_IV = Arrays.copyOfRange(key_block, offset, offset
				+ fixed_iv_length);
		offset += fixed_iv_length;
		...
		this.encryptCipher.init(true, new AEADParameters(encryptKey,
				8 * macSize, dummyNonce));
		this.decryptCipher.init(false, new AEADParameters(decryptKey,
				8 * macSize, dummyNonce));
	}

	public int getPlaintextLimit(int ciphertextLimit) {
		// TODO We ought to be able to ask the decryptCipher (independently of
		// it's current state!)
	...
	public byte[] encodePlaintext(long seqNo, short type, byte[] plaintext,
		...
		byte[] nonce = new byte[this.encryptImplicitNonce.length
				+ nonce_explicit_length];
		System.arraycopy(encryptImplicitNonce, 0, nonce, 0,
				encryptImplicitNonce.length);
		...
		System.arraycopy(nonce, encryptImplicitNonce.length, output, 0,
				nonce_explicit_length);
		int outputPos = nonce_explicit_length;

		encryptCipher.init(true, new AEADParameters(null, 8 * macSize, nonce,
				getAdditionalData(seqNo, type, plaintextLength)));

		outputPos += encryptCipher.processBytes(plaintext, plaintextOffset,
				plaintextLength, output, outputPos);
		try {
		...
		if (outputPos != output.length) {
			// NOTE: Existing AEAD cipher implementations all give exact output
			// lengths
			throw new TlsFatalAlert(AlertDescription.internal_error);
		}
	...
	public byte[] decodeCiphertext(long seqNo, short type, byte[] ciphertext,
		...
		byte[] nonce = new byte[this.decryptImplicitNonce.length
				+ nonce_explicit_length];
		System.arraycopy(decryptImplicitNonce, 0, nonce, 0,
				decryptImplicitNonce.length);
		System.arraycopy(ciphertext, offset, nonce,
				decryptImplicitNonce.length, nonce_explicit_length);
		...
		decryptCipher.init(false, new AEADParameters(null, 8 * macSize, nonce,
				getAdditionalData(seqNo, type, plaintextLength)));

		outputPos += decryptCipher.processBytes(ciphertext, ciphertextOffset,
				ciphertextLength, output, outputPos);
