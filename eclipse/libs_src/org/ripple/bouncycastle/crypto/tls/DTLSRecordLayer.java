...
class DTLSRecordLayer implements DatagramTransport {
	...
	DTLSRecordLayer(DatagramTransport transport, TlsContext context,
			TlsPeer peer, short contentType) {
	...
	void initPendingEpoch(TlsCipher pendingCipher) {
		...
		/*
		 * TODO "In order to ensure that any given sequence/epoch pair is
		 * unique, implementations MUST NOT allow the same epoch value to be
		 * reused within two times the TCP maximum segment lifetime."
		 */

		// TODO Check for overflow
		this.pendingEpoch = new DTLSEpoch(writeEpoch.getEpoch() + 1,
				pendingCipher);
	}
	...
	void handshakeSuccessful(DTLSHandshakeRetransmit retransmit) {
		...
		if (retransmit != null) {
			...
			this.retransmitExpiry = System.currentTimeMillis()
					+ RETRANSMIT_TIMEOUT;
		}
	...
	public int receive(byte[] buf, int off, int len, int waitMillis)
		...
		for (;;) {
			...
			int receiveLimit = Math.min(len, getReceiveLimit())
			...
			try {
				if (retransmit != null
						&& System.currentTimeMillis() > retransmitExpiry) {
					retransmit = null;
					retransmitEpoch = null;
				}

				int received = receiveRecord(record, 0, receiveLimit,
						waitMillis);
				if (received < 0) {
					return received;
				}
				...
				if (discoveredPeerVersion != null
						&& !discoveredPeerVersion.equals(version)) {
					continue;
				}

				byte[] plaintext = recordEpoch.getCipher().decodeCiphertext(
						getMacSequenceNumber(recordEpoch.getEpoch(), seq),
						type, record, RECORD_HEADER_LENGTH,
				...
				case ContentType.application_data: {
					if (inHandshake) {
						// TODO Consider buffering application data for new
						// epoch that arrives
				...
				case ContentType.change_cipher_spec: {
					// Implicitly receive change_cipher_spec and change to
					// pending cipher state
				...
				case ContentType.handshake: {
					if (!inHandshake) {
						if (retransmit != null) {
							retransmit.receivedHandshakeRecord(epoch,
				...
				/*
				 * NOTE: If we receive any non-handshake data in the new epoch
				 * implies the peer has received our final flight.
				 */
	...
	public void send(byte[] buf, int off, int len) throws IOException {
		...
		if (this.inHandshake || this.writeEpoch == this.retransmitEpoch) {
			...
			if (handshakeType == HandshakeType.finished) {
				...
				// Implicitly send change_cipher_spec and change to pending
				// cipher state

				// TODO Send change_cipher_spec and finished records in single
				// datagram?
	...
	private void closeTransport() {
		if (!closed) {
			/*
			 * RFC 5246 7.2.1. Unless some other fatal alert has been
			 ...
			 * the connection immediately, discarding any pending writes.
			 */
	...
	private void raiseAlert(short alertLevel, short alertDescription,
	...
	private int receiveRecord(byte[] buf, int off, int len, int waitMillis)
		...
		if (recordQueue.size() > 0) {
			...
			int received = Math.min(recordQueue.size(), RECORD_HEADER_LENGTH
					+ length);
		...
		if (received >= RECORD_HEADER_LENGTH) {
			...
			if (received > recordLength) {
				recordQueue.addData(buf, off + recordLength, received
						- recordLength);
	...
	private void sendRecord(short contentType, byte[] buf, int off, int len)
		...
		/*
		 * RFC 5264 6.2.1 Implementations MUST NOT send zero-length fragments of
		...
		byte[] ciphertext = writeEpoch.getCipher().encodePlaintext(
				getMacSequenceNumber(recordEpoch, recordSequenceNumber),
				contentType, buf, off, len);
		...
		System.arraycopy(ciphertext, 0, record, RECORD_HEADER_LENGTH,
				ciphertext.length);
