...
public class TlsClientProtocol extends TlsProtocol {
	...
	protected void handleChangeCipherSpecMessage() throws IOException {
		...
		case CS_CLIENT_FINISHED: {
			if (this.expectSessionTicket) {
				/*
				 * RFC 5077 3.3. This message MUST be sent if the server
				 * included a SessionTicket extension in the ServerHello.
				 */
	...
	protected void handleHandshakeMessage(short type, byte[] data)
		...
		case HandshakeType.server_hello_done:
			...
			case CS_CERTIFICATE_REQUEST:
				 ...
				 */
				recordStream.setPendingConnectionState(
						tlsClient.getCompression(), tlsClient.getCipher());
		...
		if (serverExtensions != null) {
			...
			while (e.hasMoreElements()) {
				...
				if (!extType.equals(EXT_RenegotiationInfo)
						&& (clientExtensions == null || clientExtensions
								.get(extType) == null)) {
					/*
					 * RFC 5246 7.4.1.4 An extension type MUST NOT appear in the
					 * ServerHello unless the same extension type appeared in
					 * the corresponding ClientHello. If a client receives an
	...
	protected void sendCertificateVerifyMessage(byte[] data) throws IOException {
		/*
		 * Send signature of handshake messages so far to prove we are the owner
		 * of the cert See RFC 2246 sections 4.7, 7.4.3 and 7.4.8
		 */
	...
	protected void sendClientHelloMessage() throws IOException {
		...
		recordStream.setWriteVersion(this.tlsClient
