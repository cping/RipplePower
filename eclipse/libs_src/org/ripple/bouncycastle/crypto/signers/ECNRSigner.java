...
public class ECNRSigner implements DSA {
	...
	public BigInteger[] generateSignature(byte[] digest) {
		...
		BigInteger n = ((ECPrivateKeyParameters) this.key).getParameters()
				.getN();
		...
		do // generate r
		{
			...
			keyGen.init(new ECKeyGenerationParameters(privKey.getParameters(),
					this.random));

			tempPair = keyGen.generateKeyPair();

			// BigInteger Vx = tempPair.getPublic().getW().getAffineX();
			ECPublicKeyParameters V = (ECPublicKeyParameters) tempPair
					.getPublic(); // get temp's public key
			BigInteger Vx = V.getQ().getX().toBigInteger(); // get the point's x
															// coordinate
		...
		BigInteger u = ((ECPrivateKeyParameters) tempPair.getPrivate()).getD(); // temp's
																				// private
																				// key
																				// value
