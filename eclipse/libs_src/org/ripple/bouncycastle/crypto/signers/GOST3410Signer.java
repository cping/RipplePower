...
public class GOST3410Signer implements DSA {
	...
	public void init(boolean forSigning, CipherParameters param) {
		if (forSigning) {
			if (param instanceof ParametersWithRandom) {
				...
				this.key = (GOST3410PrivateKeyParameters) rParam
						.getParameters();
			} else {
	...
	public BigInteger[] generateSignature(byte[] message) {
		...
		BigInteger r = params.getA().modPow(k, params.getP())
				.mod(params.getQ());
	...
	public boolean verifySignature(byte[] message, BigInteger r, BigInteger s) {
		...
		BigInteger v = m.modPow(params.getQ().subtract(new BigInteger("2")),
				params.getQ());

		BigInteger z1 = s.multiply(v).mod(params.getQ());
		BigInteger z2 = (params.getQ().subtract(r)).multiply(v).mod(
				params.getQ());

		z1 = params.getA().modPow(z1, params.getP());
		z2 = ((GOST3410PublicKeyParameters) key).getY().modPow(z2,
				params.getP());
