...
public class ISO9796d2PSSSigner implements SignerWithRecovery {
	 ...
	 */
	public ISO9796d2PSSSigner(AsymmetricBlockCipher cipher, Digest digest,
			int saltLength, boolean implicit) {
		this.cipher = cipher;
		this.digest = digest;
		...
		if (implicit) {
			...
			Integer trailerObj = (Integer) trailerMap.get(digest
					.getAlgorithmName());
	 ...
	 *             if wrong parameter type or a fixed salt is passed in which is
	 *             the wrong length.
	 */
	public void init(boolean forSigning, CipherParameters param) {
		...
		if (trailer == TRAILER_IMPLICIT) {
			mBuf = new byte[block.length - digest.getDigestSize()
					- lengthOfSalt - 1 - 1];
		} else {
			mBuf = new byte[block.length - digest.getDigestSize()
					- lengthOfSalt - 1 - 2];
	...
	public void updateWithRecoveredMessage(byte[] signature)
		...
		if (block.length < (keyBits + 7) / 8) {
			...
			System.arraycopy(block, 0, tmp, tmp.length - block.length,
					block.length);
			clearBlock(block);
			block = tmp;
		}
		...
		if (((block[block.length - 1] & 0xFF) ^ 0xBC) == 0) {
			...
			int sigTrail = ((block[block.length - 2] & 0xFF) << 8)
			...
			if (trailerObj != null) {
				if (sigTrail != trailerObj.intValue()) {
					throw new IllegalStateException(
							"signer initialised with wrong digest for trailer "
									+ sigTrail);
				}
			} else {
				throw new IllegalArgumentException(
						"unrecognised hash in signature");
			}
		...
		// remove the mask
		//
		byte[] dbMask = maskGeneratorFunction1(block, block.length - hLen
				- tLength, hLen, block.length - hLen - tLength);
	...
	public byte[] generateSignature() throws CryptoException {
		...
		int off = block.length - messageLength - salt.length - hLen - tLength
				- 1;
		...
		byte[] dbMask = maskGeneratorFunction1(hash, 0, hash.length,
