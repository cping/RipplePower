...
public class PSSSigner implements Signer {
	...
	public PSSSigner(AsymmetricBlockCipher cipher, Digest contentDigest,
			Digest mgfDigest, int sLen) {
	...
	public PSSSigner(AsymmetricBlockCipher cipher, Digest contentDigest,
			Digest mgfDigest, int sLen, byte trailer) {
		this.cipher = cipher;
	...
	private byte[] maskGeneratorFunction1(byte[] Z, int zOff, int zLen,
		...
		if ((counter * mgfhLen) < length) {
			...
			System.arraycopy(hashBuf, 0, mask, counter * mgfhLen, mask.length
					- (counter * mgfhLen));
		}
