...
public final class WhirlpoolDigest implements ExtendedDigest, Memoable {
	...
	private static final int REDUCTION_POLYNOMIAL = 0x011d; // 2^8 + 2^4 + 2^3 +
															// 2 + 1;

	private static final int[] SBOX = { ,
	...
	private long packIntoLong(int b7, int b6, int b5, int b4, int b3, int b2,
			int b1, int b0) {
		return ((long) b7 << 56) ^ ((long) b6 << 48) ^ ((long) b5 << 40)
				^ ((long) b4 << 32) ^ ((long) b3 << 24) ^ ((long) b2 << 16)
				^ ((long) b1 << 8) ^ b0;
	}

	/*
	 * int's are used to prevent sign extension. The values that are really
	 * being used are actually just 0..255
	 */
	...
	private void convertLongToByteArray(long inputLong, byte[] outputArray,
			int offSet) {
		for (int i = 0; i < 8; i++) {
	...
	/*
	 * increment() can be implemented in this way using 2 arrays or by having
	 * some temporary variables that are used to set the value provided by
	 * EIGHT[i] and carry within the loop.
	 * 
	...
	public void reset(Memoable other) {
		...
		System.arraycopy(originalDigest._bitCount, 0, _bitCount, 0,
				_bitCount.length);
