...
public class SHA3Digest implements ExtendedDigest {
	...
	private void init(int bitLength) {
		...
		default:
			throw new IllegalArgumentException(
					"bitLength must be one of 224, 256, 384, or 512.");
		}
	}
	...
	private void squeeze(byte[] output, int offset, long outputLength) {
		...
		while (i < outputLength) {
			if (bitsAvailableForSqueezing == 0) {
				...
				// displayIntermediateValues.displayBytes(1,
				// "Block available for squeezing", dataQueue,
				// bitsAvailableForSqueezing / 8);
	...
	private void keccakPermutation(byte[] state) {
		...
		// displayIntermediateValues.displayStateAsBytes(1,
		// "Input of permutation", longState);

		keccakPermutationOnWords(longState);

		// displayIntermediateValues.displayStateAsBytes(1,
		// "State after permutation", longState);

		fromWordsToBytes(state, longState);
	}

	private void keccakPermutationAfterXor(byte[] state, byte[] data,
