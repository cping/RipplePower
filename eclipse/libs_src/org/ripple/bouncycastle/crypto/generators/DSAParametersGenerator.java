...
public class DSAParametersGenerator {
	...
	public void init(DSAParameterGenerationParameters params) {
		// TODO Should we enforce the minimum 'certainty' values as per C.3
		// Table C.1?
		this.use186_3 = true;
		this.L = params.getL();
		this.N = params.getN();
	...
	public DSAParameters generateParameters() {
		return (use186_3) ? generateParameters_FIPS186_3()
		...
		for (;;) {
			...
			for (int counter = 0; counter < 4096; ++counter) {
				for (int k = 0; k < n; k++) {
					...
					System.arraycopy(part1, 0, w, w.length - (k + 1)
							* part1.length, part1.length);
				}

				inc(offset);
				hash(digest, offset, part1);
				System.arraycopy(part1, part1.length
						- ((w.length - (n) * part1.length)), w, 0, w.length - n
						* part1.length);
	...
	private static BigInteger calculateGenerator_FIPS186_2(BigInteger p,
			BigInteger q, SecureRandom r) {
	...
	private DSAParameters generateParameters_FIPS186_3() {
		...
		for (;;) {
			...
			BigInteger q = ONE.shiftLeft(N - 1).add(U).add(ONE)
					.subtract(U.mod(TWO));
			...
			for (int counter = 0; counter < counterLimit; ++counter) {
				...
				if (p.isProbablePrime(certainty)) {
					...
					BigInteger g = calculateGenerator_FIPS186_3_Unverifiable(p,
							q, random);
	...
	private static BigInteger calculateGenerator_FIPS186_3_Verifiable(Digest d,
			BigInteger p, BigInteger q, byte[] seed, int index) {
