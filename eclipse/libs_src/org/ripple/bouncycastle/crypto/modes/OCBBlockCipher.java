 ...
 * href="http://tools.ietf.org/html/draft-irtf-cfrg-ocb-00">The OCB
 * Authenticated-Encryption Algorithm</a>, licensed per:
 * <p/>
 * <blockquote> <a
 * href="http://www.cs.ucdavis.edu/~rogaway/ocb/license1.pdf">License for
...
public class OCBBlockCipher implements AEADBlockCipher {
	...
	public OCBBlockCipher(BlockCipher hashCipher, BlockCipher mainCipher) {
		...
		if (!hashCipher.getAlgorithmName()
				.equals(mainCipher.getAlgorithmName())) {
			throw new IllegalArgumentException(
	...
	public void init(boolean forEncryption, CipherParameters parameters)
		...
		this.mainBlock = new byte[forEncryption ? BLOCK_SIZE
				: (BLOCK_SIZE + macSize)];
		...
		if (N.length > 16 || (N.length == 16 && (N[0] & 0x80) != 0)) {
			/*
			 * NOTE: We don't just ignore bit 128 because it would hide from the
			 * caller the fact that two nonces differing only in bit 128 are not
			 * different.
			 */
			throw new IllegalArgumentException(
	...
	protected static byte[] OCB_double(byte[] block) {
		...
		/*
		 * NOTE: This construction is an attempt at a constant-time
		 * implementation.
		 */
