...
public class DualECSP800DRBG implements SP80090DRBG {
	...
	public DualECSP800DRBG(Digest digest, int securityStrength,
		...
		if (securityStrength <= 128) {
			...
			_P = new ECPoint.Fp(_curve, new ECFieldElement.Fp(_curve.getQ(),
					p256_Px), new ECFieldElement.Fp(_curve.getQ(), p256_Py));
			_Q = new ECPoint.Fp(_curve, new ECFieldElement.Fp(_curve.getQ(),
					p256_Qx), new ECFieldElement.Fp(_curve.getQ(), p256_Qy));
			...
			_P = new ECPoint.Fp(_curve, new ECFieldElement.Fp(_curve.getQ(),
					p384_Px), new ECFieldElement.Fp(_curve.getQ(), p384_Py));
			_Q = new ECPoint.Fp(_curve, new ECFieldElement.Fp(_curve.getQ(),
					p384_Qx), new ECFieldElement.Fp(_curve.getQ(), p384_Qy));
			...
			_P = new ECPoint.Fp(_curve, new ECFieldElement.Fp(_curve.getQ(),
					p521_Px), new ECFieldElement.Fp(_curve.getQ(), p521_Py));
			_Q = new ECPoint.Fp(_curve, new ECFieldElement.Fp(_curve.getQ(),
					p521_Qx), new ECFieldElement.Fp(_curve.getQ(), p521_Qy));
		} else {
			throw new IllegalArgumentException(
	...
	public int generate(byte[] output, byte[] additionalInput,
		...
		for (int i = 0; i < m; i++) {
			...
			byte[] r = _Q.multiply(new BigInteger(1, _s)).getX().toBigInteger()
					.toByteArray();

			if (r.length > _outlen) {
				System.arraycopy(r, r.length - _outlen, output, i * _outlen,
						_outlen);
			} else {
				System.arraycopy(r, 0, output, i * _outlen
						+ (_outlen - r.length), r.length);
			}
		...
		if (m * _outlen < output.length) {
			...
			System.arraycopy(r, 0, output, m * _outlen, output.length
					- (m * _outlen));
		}

		// Need to preserve length of S as unsigned int.
		_s = BigIntegers.asUnsignedByteArray(_sLength,
				_P.multiply(new BigInteger(1, _s)).getX().toBigInteger());
