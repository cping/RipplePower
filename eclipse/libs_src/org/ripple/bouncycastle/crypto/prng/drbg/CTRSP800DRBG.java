...
public class CTRSP800DRBG implements SP80090DRBG {
	 ...
	 */
	public CTRSP800DRBG(BlockCipher engine, int keySizeInBits,
	...
	private void CTR_DRBG_Update(byte[] seed, byte[] key, byte[] v) {
		...
		while (i * outLen < seed.length) {
			...
			int bytesToCopy = ((temp.length - i * outLen) > outLen) ? outLen
	...
	private void CTR_DRBG_Reseed_algorithm(EntropySource entropy,
			byte[] additionalInput) {
		byte[] seedMaterial = Arrays.concatenate(entropy.getEntropy(),
				additionalInput);
	...
	private static final byte[] K_BITS = Hex
	 ...
	 * outlen zeros. 2. n = len (data)/outlen. 3. Starting with the leftmost
	 * bits of data, split the data into n blocks of outlen bits each, forming
	 * block(1) to block(n). 4. For i = 1 to n do 4.1 input_block =
	 * chaining_value ^ block(i) . 4.2 chaining_value = Block_Encrypt (Key,
	 * input_block). 5. output_block = chaining_value. 6. Return output_block.
	 */
	...
	public int generate(byte[] output, byte[] additionalInput,
		...
		if (_isTDEA) {
			...
			if (Utils.isTooLarge(output, TDEA_MAX_BITS_REQUEST / 8)) {
				throw new IllegalArgumentException(
						"Number of bits per request limited to "
	...
	private boolean isTDEA(BlockCipher cipher) {
		return cipher.getAlgorithmName().equals("DESede")
				|| cipher.getAlgorithmName().equals("TDEA");
	}
