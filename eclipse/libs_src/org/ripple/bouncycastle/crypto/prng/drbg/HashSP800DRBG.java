...
public class HashSP800DRBG implements SP80090DRBG {
	 ...
	 */
	public HashSP800DRBG(Digest digest, int securityStrength,
		...
		_seedLength = ((Integer) seedlens.get(digest.getAlgorithmName()))
				.intValue();
	...
	public int generate(byte[] output, byte[] additionalInput,
		...
		if (additionalInput != null) {
			...
			// TODO: inOff / inLength
			System.arraycopy(additionalInput, 0, newInput, 1 + _V.length,
	...
	private void addTo(byte[] longer, byte[] shorter) {
		...
		{
			int res = (longer[longer.length - i] & 0xff)
					+ (shorter[shorter.length - i] & 0xff) + carry;
			carry = (res > 0xff) ? 1 : 0;
	...
	public void reseed(byte[] additionalInput) {
		...
		byte[] seedMaterial = Arrays.concatenate(ONE, _V, entropy,
				additionalInput);
		byte[] seed = Utils.hash_df(_digest, seedMaterial, _seedLength);
	...
	private byte[] hashgen(byte[] input, int lengthInBits) {
		...
		for (int i = 0; i <= m; i++) {
			...
			int bytesToCopy = ((W.length - i * dig.length) > dig.length) ? dig.length
