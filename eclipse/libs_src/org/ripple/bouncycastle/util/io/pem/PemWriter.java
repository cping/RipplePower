...
public class PemWriter extends BufferedWriter {
	...
	/**
	 * Return the number of bytes or characters required to contain the passed
	 * in object if it is PEM encoded.
	 * 
	 * @param obj
	...
	public int getOutputSize(PemObject obj) {
		...
		if (!obj.getHeaders().isEmpty()) {
			for (Iterator it = obj.getHeaders().iterator(); it.hasNext();) {
				...
				size += hdr.getName().length() + ": ".length()
						+ hdr.getValue().length() + nlLength;
			}
		...
		size += dataLen
				+ (((dataLen + LINE_LENGTH - 1) / LINE_LENGTH) * nlLength);
