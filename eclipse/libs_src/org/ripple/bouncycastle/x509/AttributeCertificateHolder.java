...
public class AttributeCertificateHolder implements CertSelector, Selector {
	...
	public AttributeCertificateHolder(X509Principal issuerName,
		...
		holder = new org.ripple.bouncycastle.asn1.x509.Holder(new IssuerSerial(
				GeneralNames.getInstance(new DERSequence(new GeneralName(
						issuerName))), new ASN1Integer(serialNumber)));
	}
	...
	public AttributeCertificateHolder(int digestedObjectType,
			String digestAlgorithm, String otherObjectTypeID,
			byte[] objectDigest) {
	 ...
	 * 
	 * @return The digest algorithm ID or <code>null</code> if no object digest
	 *         info is set.
	 */
	...
	private GeneralNames generateGeneralNames(X509Principal principal) {
		return GeneralNames.getInstance(new DERSequence(new GeneralName(
				principal)));
	}
			...
			if (gn.getTagNo() == GeneralName.directoryName) {
				try {
					if (new X509Principal(((ASN1Encodable) gn.getName())
							.toASN1Primitive().getEncoded()).equals(subject)) {
						return true;
					}
				} catch (IOException e) {
				}
	...
	public Object clone() {
		return new AttributeCertificateHolder(
				(ASN1Sequence) holder.toASN1Object());
	}
