...
public class X509AttributeCertStoreSelector implements Selector {
			...
			if (targetInfoExt != null) {
				...
				if (!targetNames.isEmpty()) {
					...
					for (int i = 0; i < targetss.length; i++) {
						...
						for (int j = 0; j < targets.length; j++) {
							if (targetNames.contains(GeneralName
									.getInstance(targets[j].getTargetName()))) {
								found = true;
								break;
							}
						}
				...
				if (!targetGroups.isEmpty()) {
					...
					for (int i = 0; i < targetss.length; i++) {
						...
						Target[] targets = t.getTargets();
						for (int j = 0; j < targets.length; j++) {
							if (targetGroups.contains(GeneralName
									.getInstance(targets[j].getTargetGroup()))) {
								found = true;
								break;
							}
						}
	...
	public void setAttributeCertificateValid(Date attributeCertificateValid) {
		if (attributeCertificateValid != null) {
			this.attributeCertificateValid = new Date(
					attributeCertificateValid.getTime());
		} else {
	 ...
	 * 
	 * @param name
	 *            a byte array containing the name in ASN.1 DER encoded form of
	 *            a GeneralName
	 * @throws IOException
	...
	public void addTargetName(byte[] name) throws IOException {
		addTargetName(GeneralName
				.getInstance(ASN1Primitive.fromByteArray(name)));
	}
	 ...
	 * 
	 * @param name
	 *            a byte array containing the group in ASN.1 DER encoded form of
	 *            a GeneralName
	 * @throws IOException
	 *             if a parsing error occurs.
	 */
	public void addTargetGroup(byte[] name) throws IOException {
		addTargetGroup(GeneralName.getInstance(ASN1Primitive
				.fromByteArray(name)));
	}
	...
	private Set extractGeneralNames(Collection names) throws IOException {
		...
		for (Iterator it = names.iterator(); it.hasNext();) {
			...
			if (o instanceof GeneralName) {
				...
				temp.add(GeneralName.getInstance(ASN1Primitive
						.fromByteArray((byte[]) o)));
			}
		}
		return temp;
	}
