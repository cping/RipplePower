...
public class AttributeCertificateIssuer implements CertSelector, Selector {
	...
	public AttributeCertificateIssuer(X509Principal principal) {
		form = new V2Form(GeneralNames.getInstance(new DERSequence(
				new GeneralName(principal))));
	}
	...
	private boolean matchesDN(X500Principal subject, GeneralNames targets) {
		...
		for (int i = 0; i != names.length; i++) {
			...
			if (gn.getTagNo() == GeneralName.directoryName) {
				try {
					if (new X500Principal(((ASN1Encodable) gn.getName())
	...
	public boolean match(Certificate cert) {
		...
		if (form instanceof V2Form) {
			...
			if (issuer.getBaseCertificateID() != null) {
				return issuer.getBaseCertificateID().getSerial().getValue()
						.equals(x509Cert.getSerialNumber())
						&& matchesDN(x509Cert.getIssuerX500Principal(), issuer
