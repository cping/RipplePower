...
public class AuthorityKeyIdentifierStructure extends AuthorityKeyIdentifier {
		...
		try {
			if (certificate.getVersion() != 3) {
				GeneralName genName = new GeneralName(
						PrincipalUtil.getIssuerX509Principal(certificate));
				SubjectPublicKeyInfo info = new SubjectPublicKeyInfo(
						(ASN1Sequence) new ASN1InputStream(certificate
								.getPublicKey().getEncoded()).readObject());

				return (ASN1Sequence) new AuthorityKeyIdentifier(info,
						new GeneralNames(genName),
						certificate.getSerialNumber()).toASN1Object();
			} else {
				GeneralName genName = new GeneralName(
						PrincipalUtil.getIssuerX509Principal(certificate));
				...
				if (ext != null) {
					ASN1OctetString str = (ASN1OctetString) X509ExtensionUtil
	...
	private static ASN1Sequence fromKey(PublicKey pubKey)
		...
		try {
			SubjectPublicKeyInfo info = new SubjectPublicKeyInfo(
					(ASN1Sequence) new ASN1InputStream(pubKey.getEncoded())
							.readObject());
