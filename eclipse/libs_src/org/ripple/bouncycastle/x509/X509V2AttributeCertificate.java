...
public class X509V2AttributeCertificate implements X509AttributeCertificate {
	...
	private static AttributeCertificate getObject(InputStream in)
			throws IOException {
		try {
			return AttributeCertificate.getInstance(new ASN1InputStream(in)
					.readObject());
		} catch (IOException e) {
			throw e;
		} catch (Exception e) {
			throw new IOException("exception decoding certificate structure: "
					+ e.toString());
		}
	}
	...
	X509V2AttributeCertificate(AttributeCertificate cert) throws IOException {
		...
		try {
			this.notAfter = cert.getAcinfo().getAttrCertValidityPeriod()
					.getNotAfterTime().getDate();
			this.notBefore = cert.getAcinfo().getAttrCertValidityPeriod()
					.getNotBeforeTime().getDate();
	...
	public AttributeCertificateHolder getHolder() {
		return new AttributeCertificateHolder((ASN1Sequence) cert.getAcinfo()
				.getHolder().toASN1Object());
	}
	...
	public final void verify(PublicKey key, String provider)
		...
		if (!cert.getSignatureAlgorithm().equals(
				cert.getAcinfo().getSignature())) {
			throw new CertificateException(
					"Signature algorithm in certificate info not same as outer certificate");
		}

		signature = Signature.getInstance(cert.getSignatureAlgorithm()
				.getObjectId().getId(), provider);
	...
	public byte[] getExtensionValue(String oid) {
		...
		if (extensions != null) {
			Extension ext = extensions.getExtension(new ASN1ObjectIdentifier(
					oid));
	...
	public X509Attribute[] getAttributes(String oid) {
		...
		for (int i = 0; i != seq.size(); i++) {
			X509Attribute attr = new X509Attribute(
					(ASN1Encodable) seq.getObjectAt(i));
