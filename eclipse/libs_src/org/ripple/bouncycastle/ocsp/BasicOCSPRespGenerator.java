...
public class BasicOCSPRespGenerator {
	...
	private class ResponseObject {
		...
		public ResponseObject(CertificateID certId,
		...
		public SingleResponse toResponse() throws Exception {
			return new SingleResponse(certId.toASN1Object(), certStatus,
					thisUpdate, nextUpdate, extensions);
		}
	}
	...
	/**
	 * construct with the responderID to be the SHA-1 keyHash of the passed in
	 * public key.
	 */
	...
	public void addResponse(CertificateID certID, CertificateStatus certStatus,
		...
		list.add(new ResponseObject(certID, certStatus, new Date(), null,
				singleExtensions));
	}
	 ...
	 */
	public void addResponse(CertificateID certID, CertificateStatus certStatus,
			Date nextUpdate, X509Extensions singleExtensions) {
		list.add(new ResponseObject(certID, certStatus, new Date(), nextUpdate,
				singleExtensions));
	}
	 ...
	 */
	public void addResponse(CertificateID certID, CertificateStatus certStatus,
			Date thisUpdate, Date nextUpdate, X509Extensions singleExtensions) {
		list.add(new ResponseObject(certID, certStatus, thisUpdate, nextUpdate,
				singleExtensions));
	}
	...
	private BasicOCSPResp generateResponse(String signatureName,
			PrivateKey key, X509Certificate[] chain, Date producedAt,
			String provider, SecureRandom random) throws OCSPException,
		...
		ResponseData tbsResp = new ResponseData(responderID.toASN1Object(),
				new DERGeneralizedTime(producedAt), new DERSequence(responses),
				responseExtensions);
			...
			try {
				for (int i = 0; i != chain.length; i++) {
					v.add(new X509CertificateStructure(
							(ASN1Sequence) ASN1Primitive.fromByteArray(chain[i]
									.getEncoded())));
				}
			} catch (IOException e) {
		...
		return new BasicOCSPResp(new BasicOCSPResponse(tbsResp, sigAlgId,
				bitSig, chainSeq));
	}

	public BasicOCSPResp generate(String signingAlgorithm, PrivateKey key,
			X509Certificate[] chain, Date thisUpdate, String provider)
			throws OCSPException, NoSuchProviderException,
			IllegalArgumentException {
		return generate(signingAlgorithm, key, chain, thisUpdate, provider,
				null);
	}

	public BasicOCSPResp generate(String signingAlgorithm, PrivateKey key,
			X509Certificate[] chain, Date producedAt, String provider,
		...
		return generateResponse(signingAlgorithm, key, chain, producedAt,
				provider, random);
	}
