...
public class CertificateID {
	 ...
	 *            serial number
	 * @param provider
	 *            provider to use for hashAlgorithm, null if the default one
	 *            should be used.
	 * 
	...
	public CertificateID(String hashAlgorithm, X509Certificate issuerCert,
			BigInteger number, String provider) throws OCSPException {
		AlgorithmIdentifier hashAlg = new AlgorithmIdentifier(
				new DERObjectIdentifier(hashAlgorithm), DERNull.INSTANCE);

		this.id = createCertID(hashAlg, issuerCert, new ASN1Integer(number),
				provider);
	}
	 ...
	 */
	public CertificateID(String hashAlgorithm, X509Certificate issuerCert,
			BigInteger number) throws OCSPException {
	...
	/**
	 * return the serial number for the certificate associated with this
	 * request.
	 */
	...
	public boolean matchesIssuer(X509Certificate issuerCert, String provider)
		...
		return createCertID(id.getHashAlgorithm(), issuerCert,
				id.getSerialNumber(), provider).equals(id);
	}
	...
	/**
	 * Create a new CertificateID for a new serial number derived from a
	 * previous one calculated for the same CA certificate.
	 * 
	 * @param original
	...
	public static CertificateID deriveCertificateID(CertificateID original,
		...
		return new CertificateID(new CertID(original.id.getHashAlgorithm(),
				original.id.getIssuerNameHash(),
				original.id.getIssuerKeyHash(),
				new ASN1Integer(newSerialNumber)));
	}

	private static CertID createCertID(AlgorithmIdentifier hashAlg,
			X509Certificate issuerCert, ASN1Integer serialNumber,
			String provider) throws OCSPException {
		try {
			MessageDigest digest = OCSPUtil.createDigestInstance(hashAlg
					.getAlgorithm().getId(), provider);

			X509Principal issuerName = PrincipalUtil
					.getSubjectX509Principal(issuerCert);
			...
			return new CertID(hashAlg, issuerNameHash, issuerKeyHash,
					serialNumber);
		} catch (Exception e) {
