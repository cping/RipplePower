...
public abstract class ECPoint {
	...
	public static class Fp extends ECPoint {
		...
		public byte[] getEncoded(boolean compressed) {
			...
			if (compressed) {
				...
				byte[] X = converter.integerToBytes(this.getX().toBigInteger(),
						qLength);
				...
				byte[] X = converter.integerToBytes(this.getX().toBigInteger(),
						qLength);
				byte[] Y = converter.integerToBytes(this.getY().toBigInteger(),
						qLength);
		...
		public ECPoint add(ECPoint b) {
			...
			ECFieldElement gamma = b.y.subtract(this.y).divide(
					b.x.subtract(this.x));
		...
		public ECPoint twice() {
			...
			ECFieldElement TWO = this.curve.fromBigInteger(BigInteger
					.valueOf(2));
			ECFieldElement THREE = this.curve.fromBigInteger(BigInteger
					.valueOf(3));
	...
	public static class F2m extends ECPoint {
		...
		public ECPoint.F2m addSimple(ECPoint.F2m b) {
			...
			ECFieldElement.F2m lambda = (ECFieldElement.F2m) (this.y.add(y2))
					.divide(this.x.add(x2));

			ECFieldElement.F2m x3 = (ECFieldElement.F2m) lambda.square()
					.add(lambda).add(this.x).add(x2).add(this.curve.getA());

			ECFieldElement.F2m y3 = (ECFieldElement.F2m) lambda
					.multiply(this.x.add(x3)).add(x3).add(this.y);
		...
		public ECPoint twice() {
			...
			ECFieldElement.F2m lambda = (ECFieldElement.F2m) this.x.add(this.y
					.divide(this.x));

			ECFieldElement.F2m x3 = (ECFieldElement.F2m) lambda.square()
					.add(lambda).add(this.curve.getA());
		...
		public ECPoint negate() {
			return new ECPoint.F2m(curve, this.getX(), this.getY().add(
					this.getX()), withCompression);
		}
