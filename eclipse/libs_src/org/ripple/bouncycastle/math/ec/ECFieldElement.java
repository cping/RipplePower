...
public abstract class ECFieldElement implements ECConstants {
	...
	public static class Fp extends ECFieldElement {
		...
		public ECFieldElement sqrt() {
			...
			if (q.testBit(1)) {
				// z = g^(u+1) + p, p = 4u + 3
				ECFieldElement z = new Fp(q, x.modPow(
						q.shiftRight(2).add(ECConstants.ONE), q));
			...
			// || !(r.multiply(r).subtract(fourX).modPow(legendreExponent,
			// q).equals(qMinusOne)));
			//
			// BigInteger n1 = qMinusOne.shiftRight(2);
			// //.divide(ECConstants.FOUR);
		...
		// private static BigInteger W(BigInteger n, BigInteger wOne, BigInteger
		// p)
		// {
		...
		// {
		// return r.multiply(r).multiply(x.modPow(q.subtract(ECConstants.TWO),
		// q)).subtract(ECConstants.TWO).mod(p);
		// }

		private static BigInteger[] lucasSequence(BigInteger p, BigInteger P,
	...
	// }
	//
	// return new F2m(this.m, this.k1, this.k2, this.k3,
	// this.x.xor(b.toBigInteger()));
	// }
	//
	...
	public static class F2m extends ECFieldElement {
		...
		public ECFieldElement invert() {
			...
			return new ECFieldElement.F2m(this.m, this.k1, this.k2, this.k3,
					g2z);
		}
		...
		/**
		 * @return the representation of the field
		 *         <code>F<sub>2<sup>m</sup></sub></code>, either of TPB
		 *         (trinomial basis representation) or PPB (pentanomial basis
		 *         representation).
		 */
		public int getRepresentation() {
			return this.representation;
		...
		public boolean equals(Object anObject) {
			...
			return ((this.m == b.m) && (this.k1 == b.k1) && (this.k2 == b.k2)
					&& (this.k3 == b.k3)
					&& (this.representation == b.representation) && (this.x
						.equals(b.x)));
		}
