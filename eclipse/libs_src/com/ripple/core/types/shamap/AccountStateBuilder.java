...
public class AccountStateBuilder {
	...
	public void onLedgerClose(long ledgerIndex, Hash256 accountHash,
			Hash256 parentHash) {
	...
	public void onTransaction(TransactionResult tr) {
		if (tr.meta.transactionIndex().longValue() != nextTransactionIndex)
			throw new AssertionError();
		if (tr.ledgerIndex.longValue() != targetLedgerIndex + 1)
			throw new AssertionError();
	...
	private ArrayList<AffectedNode> sortedAffectedNodes(TransactionResult tr) {
		ArrayList<AffectedNode> sorted = new ArrayList<AffectedNode>(
				makeCollection(tr.meta.affectedNodes()));
