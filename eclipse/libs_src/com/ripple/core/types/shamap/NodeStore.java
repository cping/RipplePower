...
public class NodeStore {
	/**
	 * In ripple, all data is stored in a simple binary key/value database. The
	 * keys are 256 bit binary strings and the values are binary strings of
	 * arbitrary length.
	 */
	...
	/**
	 * All data stored is keyed by the hash of it's contents. Ripple uses the
	 * first 256 bits of a sha512 as it's 33 percent faster than using sha256.
	 * 
	...
	/**
	 * @param hash
	 *            As ripple uses the `hash` of the contents as the NodeStore
	 *            key, `hash` is pervasively used in lieu of the term `key`.
	 */
	private void storeHashKeyedContent(Hash256 hash, byte[] content) {
		// Note: The real nodestore actually prepends some metadata, which
		// doesn't
	...
	/**
	 * The complement to `set` api, which together form a simple public
	 * interface.
	 */
	...
	/**
	 * The complement to `get` api, which together form a simple public
	 * interface.
	 */
