...
public class Offer extends ThreadedLedgerEntry {
	...
	public BigDecimal directoryAskQuality() {
		return Quality.fromBookDirectory(bookDirectory(), takerPays()
				.isNative(), takerGets().isNative());
	}
	...
	public STObject executed(STObject finalFields) {
		...
		executed.put(Amount.TakerPays, finalFields.get(Amount.TakerPays)
				.subtract(takerPays()));
		executed.put(Amount.TakerGets, finalFields.get(Amount.TakerGets)
				.subtract(takerGets()));
		return executed;
	}
	...
	public Amount takerGetsFunded() {
		return has(Field.taker_gets_funded) ? get(Amount.taker_gets_funded)
				: takerGets();
	}

	public Amount takerPaysFunded() {
		return has(Field.taker_pays_funded) ? get(Amount.taker_pays_funded)
				: takerPays();
	}
	...
	public static Comparator<Offer> qualityAscending = new Comparator<Offer>() {
		...
		public int compare(Offer lhs, Offer rhs) {
			return lhs.directoryAskQuality().compareTo(
					rhs.directoryAskQuality());
		}
	};
	...
	public Hash256[] directoryIndexes() {
		return new Hash256[] { bookNodeDirectoryIndex(),
