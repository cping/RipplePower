...
public class RippleState extends ThreadedLedgerEntry {
	...
	public boolean isFor(Issue issue) {
		return isFor(issue.issuer())
				&& balance().currency().equals(issue.currency());
	}
	...
	@Deprecated()
	// "not deprecated but needs fixing"
	public boolean authorizedBy(AccountID account) {
		UInt32 flags = flags();
		return flags == null
				|| flags.testBit(isHighAccount(account) ? LedgerFlag.HighAuth
						: LedgerFlag.LowAuth);
	}
	...
	public Hash256[] directoryIndexes() {
		return new Hash256[] { lowNodeOwnerDirectory(),
