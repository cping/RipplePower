...
public class TransactionResult implements Comparable<TransactionResult> {
	...
	public TransactionResult(long ledgerIndex, Hash256 hash, Transaction txn,
			...
			for (STObject node : affected) {
				if (node.has(STObject.CreatedNode)) {
					...
					if (STObject.ledgerEntryType(created) == LedgerEntryType.AccountRoot) {
						...
						if (destinationIndex.equals(created
								.get(Hash256.LedgerIndex))) {
							return destination;
						}
					}
				}
	...
	public static TransactionResult fromJSON(JSONObject json) {
		...
		String txKey = json.has("transaction") ? "transaction"
				: json.has("tx") ? "tx" : json.has("tx_blob") ? "tx_blob"
						: null;
		...
		try {
			...
			if (txKey == null) {
				...
				if (json.has("hash")) {
					txn.put(Hash256.hash,
							Hash256.fromHex(json.getString("hash")));
				} else if (binary) {
			...
			TransactionMeta meta = (TransactionMeta) parseObject(json, metaKey,
					binary);
			long ledger_index = json.optLong("ledger_index", 0);
			if (ledger_index == 0 && !binary) {
				ledger_index = json.getJSONObject(txKey)
						.getLong("ledger_index");
			}

			TransactionResult tr = new TransactionResult(ledger_index,
					txn.get(Hash256.hash), txn, meta);
	...
	private static STObject parseObject(JSONObject json, String key,
			boolean binary) throws JSONException {
		if (binary) {
	...
	public TransactionResult(JSONObject json, Source resultMessageSource) {
		...
		try {
			if (resultMessageSource == Source.transaction_subscription_notification) {
				...
				engineResult = EngineResult.valueOf(json
						.getString("engine_result"));
				validated = json.getBoolean("validated");
				ledgerHash = Hash256.translate.fromString(json
						.getString("ledger_hash"));
				...
				if (json.has("meta")) {
					meta = (TransactionMeta) STObject.fromJSONObject(json
							.getJSONObject("meta"));
				}
			} else if (resultMessageSource == Source.ledger_transactions_expanded_with_ledger_index_injected) {
				validated = true;
				meta = (TransactionMeta) STObject.translate.fromJSONObject(json
				...
				if (validated && !json.has("meta")) {
					throw new IllegalStateException(
							"It's validated, why doesn't it have meta??");
				}
				if (validated) {
					meta = (TransactionMeta) STObject.fromJSONObject(json
							.getJSONObject("meta"));
				...
				if (validated) {
					...
					|| resultMessageSource == Source.request_tx_binary) {
				validated = json.optBoolean("validated", false);
				if (validated && !json.has("meta")) {
					throw new IllegalStateException(
							"It's validated, why doesn't it have meta??");
				...
				if (validated) {
					/*
					 * { "ledger_index": 3378767, "meta": "201 ...", "tx_blob":
					 * "120 ...", "validated": true },
					 */
					...
					meta = (TransactionMeta) STObject.translate.fromHex(json
							.getString("meta"));
					this.txn = (Transaction) STObject.translate
							.fromBytes(decodedTx);

					if (account_tx) {
						hash = Index.transactionID(decodedTx);
					} else {
						hash = Hash256.translate
								.fromHex(json.getString("hash"));
					}
