...
public class PathSet extends ArrayList<PathSet.Path> implements SerializedType {
	...
	public static class Hop {
		...
		public static final int TYPE_ACCOUNT_CURRENCY_ISSUER = TYPE_CURRENCY
				| TYPE_ACCOUNT | TYPE_ISSUER;
		public static final int TYPE_ACCOUNT_CURRENCY = TYPE_CURRENCY
				| TYPE_ACCOUNT;
		...
		static public Hop fromJSONObject(JSONObject json) {
			...
			try {
				if (json.has("account")) {
					hop.account = AccountID.fromAddress(json
							.getString("account"));
				}
				if (json.has("issuer")) {
					hop.issuer = AccountID
							.fromAddress(json.getString("issuer"));
				}
				if (json.has("currency")) {
					hop.currency = Currency.fromString(json
							.getString("currency"));
				}
				if (json.has("type")) {
	...
	public static TypedFields.PathSetField pathsetField(final Field f) {
		return new TypedFields.PathSetField() {
			@Override
