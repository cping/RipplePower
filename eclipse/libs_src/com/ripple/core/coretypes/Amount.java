 ...
 */
public class Amount extends Number implements SerializedType,
		Comparable<Amount>

{

	private static BigDecimal TAKER_PAYS_FOR_THAT_DAMN_OFFER = new BigDecimal(
	...
	public static final MathContext MATH_CONTEXT = new MathContext(16,
	...
	public static final UInt64 BINARY_FLAG_IS_IOU = new UInt64(
			"8000000000000000", 16);
	public static final UInt64 BINARY_FLAG_IS_NON_NEGATIVE_NATIVE = new UInt64(
			"4000000000000000", 16);
	...
	public Amount(BigDecimal value, Currency currency, AccountID issuer,
			boolean isNative, boolean unbounded) {
	...
	Amount(BigDecimal newValue, Currency currency, AccountID issuer,
			boolean isNative) {
	...
	public boolean equals(Amount amt) {
		return equalValue(amt) && currency.equals(amt.currency)
	...
	public BigDecimal computeQuality(Amount toExchangeThisWith) {
		if (toExchangeThisWith.value.compareTo(BigDecimal.ZERO) > 0) {
			return value.divide(toExchangeThisWith.value,
					MathContext.DECIMAL128);
		} else {
			return BigDecimal.valueOf(0.0000000001);
		}
	}

	/**
	 * @return Amount The real native unit is a drop, one million of which are
	 *         an XRP. We want `one` unit at XRP scale (1e6 drops), or if it's
	 *         an IOU, just `one`.
	 */
	public Amount one() {
		if (isNative()) {
	...
	public void toBytesSink(BytesSink to) {
		...
		if (isNative()) {
			...
			if (isZero()) {
				...
				packed = man.or(new UInt64(512 + 0 + 97 + exponent)
						.shiftLeft(64 - 10));
			} else {
				packed = man.or(new UInt64(512 + 256 + 97 + exponent)
						.shiftLeft(64 - 10));
			}
	...
	public static class Translator extends TypeTranslator<Amount> {
		...
		public Amount fromJSONObject(JSONObject jsonObject) {
			try {
				...
				return new Amount(new BigDecimal(valueString), currencyString,
						issuerString);
			} catch (JSONException e) {
				throw new RuntimeException(e);
			}
	...
	public String iouTextFull() {
		return String.format("%s/%s/%s", valueText(), currencyString(),
				issuerString());
	}
	...
	private static PrecisionError getOutOfBoundsError(BigDecimal abs,
			String sized, BigDecimal bound) {
		return new PrecisionError(abs.toPlainString() + " is " + sized
				+ " than bound " + bound);
	}

	public static void checkXRPBounds(BigDecimal value) {
		// This is for that damn offer at index:
		// 6310D78E6AD408892743DD62455694162E758DA283D0E4A2CB3A3C173B7C794A
	...
	public static void checkDropsValueWhole(String drops) {
		...
		if (contains) {
			throw new RuntimeException(
					"Drops string contains floating point is decimal");
		}
	}
	...
	public static BigDecimal roundValue(BigDecimal value, boolean nativeSrc) {
		...
		return value.setScale(nativeSrc ? MAXIMUM_NATIVE_SCALE
				: MAXIMUM_IOU_PRECISION - i, MATH_CONTEXT.getRoundingMode());
	}
