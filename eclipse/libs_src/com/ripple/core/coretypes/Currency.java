...
public class Currency extends Hash160 {
	public static final Currency NEUTRAL = new Currency(
			BigInteger.ONE.toByteArray());
	public static final Currency XRP = new Currency(
	...
	public static class Demurrage {
		...
		static public BigDecimal applyRate(BigDecimal amount, BigDecimal rate,
				TimeUnit time, long units) {
			BigDecimal appliedRate = getSeconds(time, units).divide(rate,
					MathContext.DECIMAL64);
			BigDecimal factor = BigDecimal.valueOf(Math.exp(appliedRate
					.doubleValue()));
			return amount.multiply(factor, MathContext.DECIMAL64);
		}

		static public BigDecimal calculateRate(BigDecimal rate, TimeUnit time,
				long units) {
		...
		public Demurrage(byte[] bytes) {
			...
			isoCode = isoCodeFromBytesAndOffset(parser.read(3), 0);// The
																	// isoCode
	...
	public String humanCode() {
		if (type == Type.ISO) {
			...
			throw new IllegalStateException(
					"No human code for currency of type " + type);
		}
	}
		...
		if (obj instanceof Currency) {
			...
			if (type == Type.ISO && other.type == Type.ISO) {
				return (bytes[12] == otherBytes[12]
						&& bytes[13] == otherBytes[13] && bytes[14] == otherBytes[14]);
			}
		}
	...
	public static String getCurrencyCodeFromTLCBytes(byte[] bytes) {
		...
		for (i = 0; i < 20; i++) {
			zeroInNonCurrencyBytes = zeroInNonCurrencyBytes
					&& ((i == 12 || i == 13 || i == 14) || // currency bytes (0
															// or any other)
