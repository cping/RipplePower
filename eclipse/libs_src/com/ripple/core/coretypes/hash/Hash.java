...
abstract public class Hash<Subclass extends Hash> implements SerializedType,
		Comparable<Subclass> {
	...
	private byte[] normalizeAndCheckHash(byte[] bytes, int size) {
		...
		if (length > size) {
			...
			throw new RuntimeException("Hash length of " + length
					+ "  is too wide for " + simpleName);
		}
		if (length == size) {
			return bytes;
		} else {
	...
	static public abstract class HashTranslator<T extends Hash> extends
			TypeTranslator<T> {
