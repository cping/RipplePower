...
public class RippleDate extends Date {
	...
	private static void assertEquals(String s, String s1) {
		if (!s.equals(s1))
			throw new AssertionError(String.format("%s != %s", s, s1));
	}
	...
	/*
	 * @Override public Object toJSON() { return secondsSinceRippleEpoch(); }
	 * 
	 * @Override public byte[] toBytes() { return new
	 * UInt32(secondsSinceRippleEpoch()).toBytes(); }
	 * 
	 * @Override public String toHex() { return new
	 * UInt32(secondsSinceRippleEpoch()).toHex(); }
	 * 
	 * @Override public void toBytesSink(BytesSink to) { new
	 * UInt32(secondsSinceRippleEpoch()).toBytesSink(to); }
	 */
}
