...
public enum TransactionEngineResult {
	telLOCAL_ERROR(-399, "Local failure."), telBAD_DOMAIN(-398,
			...
			-296, "Malformed: Bad currency."), temBAD_FEE(-295,
			"Invalid fee, negative or not XRP."), temBAD_EXPIRATION(-294,
			"Malformed: Bad expiration."), temBAD_ISSUER(-293,
			...
			-289, "Malformed: Loop in path."), temBAD_PUBLISH(-288,
			"Malformed: Bad publish."), temBAD_TRANSFER_RATE(-287,
			"Malformed: Transfer rate must be >= 1.0"), temBAD_SEND_XRP_LIMIT(
			-286, "Malformed: Limit quality is not allowed for XRP to XRP."), temBAD_SEND_XRP_MAX(
			-285, "Malformed: Send max is not allowed for XRP to XRP."), temBAD_SEND_XRP_NO_DIRECT(
			-284, "Malformed: No Ripple direct is not allowed for XRP to XRP."), temBAD_SEND_XRP_PARTIAL(
			-283, "Malformed: Partial payment is not allowed for XRP to XRP."), temBAD_SEND_XRP_PATHS(
			-282, "Malformed: Paths are not allowed for XRP to XRP."), temBAD_SIGNATURE(
			-281, "Malformed: Bad signature."), temBAD_SRC_ACCOUNT(-280,
			"Malformed: Bad source account."), temBAD_SEQUENCE(-279,
			"Malformed: Sequence is not in the past."), temDST_IS_SRC(-278,
			"Destination may not be source."), temDST_NEEDED(-277,
			"Destination not specified."), temINVALID(-276,
			"The transaction is ill-formed."), temINVALID_FLAG(-275,
			"The transaction has an invalid flag."), temREDUNDANT(-274,
			"Sends same currency to self."), temREDUNDANT_SEND_MAX(-273,
			"Send max is redundant."), temRIPPLE_EMPTY(-272,
			"PathSet with no paths."), temUNCERTAIN(-271,
			"In process of determining result. Never returned."), temUNKNOWN(
			-270, "The transactions requires logic not implemented yet."), tefFAILURE(
			...
			"Transaction's public key is not authorized."), tefBAD_CLAIM_ID(
			-195, "Malformed: Bad claim id."), tefBAD_GEN_AUTH(-194,
			"Not authorized to claim generator."), tefBAD_LEDGER(-193,
			"Ledger in unexpected state."), tefCLAIMED(-192,
			"Can not claim a previously claimed account."), tefCREATED(-191,
			"Can't add an already created account."), tefDST_TAG_NEEDED(-190,
			"Destination tag required."), tefEXCEPTION(-189,
			"Unexpected program state."), tefGEN_IN_USE(-188,
			"Generator already in use."), tefINTERNAL(-187, "Internal error."), tefNO_AUTH_REQUIRED(
			-186, "Auth is not required."), tefPAST_SEQ(-185,
			"This sequence number has already past."), tefWRONG_PRIOR(-184,
			"tefWRONG_PRIOR"), tefMASTER_DISABLED(-183, "tefMASTER_DISABLED"),

	// TODO: check -182
	tefMAX_LEDGER(-182, "Ledger sequence too high."),

	terRETRY(-99, "Retry transaction."), terFUNDS_SPENT(-98,
			...
			"Process last."), tesSUCCESS(0, "The transaction was applied."), tecCLAIM(
			100, "Fee claimed. Sequence used. No action."), tecPATH_PARTIAL(
			101, "Path could not send full amount."), tecUNFUNDED_ADD(102,
			...
			"tecNO_REGULAR_KEY"), tecOWNERS(132, "tecOWNERS");
	...
	public static enum Class {
		telLOCAL_ERROR(-399), temMALFORMED(-299), tefFAILURE(-199), terRETRY(
				-99), tesSUCCESS(0), tecCLAIMED(100);
		...
		public static Class forResult(TransactionEngineResult result) {
			if (result.ord >= telLOCAL_ERROR.starts
					&& result.ord < temMALFORMED.starts) {
				return telLOCAL_ERROR;
			}
			if (result.ord >= temMALFORMED.starts
					&& result.ord < tefFAILURE.starts) {
				return temMALFORMED;
			}
			...
			if (result.ord >= tesSUCCESS.starts
					&& result.ord < tecCLAIMED.starts) {
				return tesSUCCESS;
			}
			return tecCLAIMED;
		}
