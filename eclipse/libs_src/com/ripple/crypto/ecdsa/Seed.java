...
public class Seed {
	 ...
	 * @param privateGen
	 *            secret point on the curve as BigInteger
	 * @return the corresponding public key is the public generator (aka public
	...
	public static BigInteger computePrivateGen(byte[] seedBytes) {
		...
		while (true) {
			privateGenBytes = new Sha512().add(seedBytes).add32(i++)
					.finish256();
	...
	public static BigInteger computeSecretKey(BigInteger privateGen,
			byte[] publicGenBytes, int accountNumber) {
		BigInteger secret;
		int i;
		...
		while (true) {
			byte[] secretBytes = new Sha512().add(publicGenBytes)
					.add32(accountNumber).add32(i++).finish256();
