...
public class JSONEncrypt {
	 ...
	 * https://github.com/AurionFinancial
	 * /AndroidWallet/blob/master/src/com/ripple/Blobvault.java
	 * 
	...
	public JSONObject encrypt(String key, JSONObject blob, String adata) {
		...
		byte[] iv = new byte[32], salt = new byte[8];
		...
		try {
			...
			AEADParameters ccm = new AEADParameters(keyParam, macSize(ts),
					nonce, adataBytes);
			...
			int res = aes
					.processBytes(plainBytes, 0, plainBytes.length, enc, 0);
	...
	public JSONObject decrypt(String key, String json)
	...
	public JSONObject decrypt(String key, JSONObject json)
			throws InvalidCipherTextException {
		try {
			...
			if (!json.getString("mode").equals("ccm")) {
				throw new RuntimeException(
						"Can only decrypt ccm mode encrypted data");
			}

			KeyParameter keyParam = this.createKey(key,
					Base64.decode(json.getString("salt")), json.getInt("iter"),
					json.getInt("ks"));

			AEADParameters ccm = new AEADParameters(keyParam,
					macSize(json.getInt("ts")), nonce, adataBytes);
			...
			int res = aes.processBytes(cipherText, 0, cipherText.length,
					plainBytes, 0);
