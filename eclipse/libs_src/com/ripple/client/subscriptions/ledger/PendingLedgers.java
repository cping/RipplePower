...
public class PendingLedgers {
	...
	private PendingLedger constructAndAddLedger(long ledger_index) {
		if (clearedLedgers.contains(ledger_index))
	...
	private void requestLedger(final long ledger_index,
			final boolean onlyHeader, final Callback<Response> cb) {
	...
	void checkHeader(final PendingLedger ledger) {
		...
		requestLedger(ledger_index, true, new Callback<Response>() {
			...
			public void called(Response response) {
				try {
					JSONObject ledgerJSON = response.result
							.getJSONObject("ledger");
					final String transaction_hash = ledgerJSON
							.getString("transaction_hash");
					boolean correctHash = ledger
							.transactionHashEquals(transaction_hash);
					if (correctHash) {
						clearLedger(ledger_index, "checkHeader");
					} else {
						LedgerSubscriber
								.log("Missing transactions, need to fillInLedger: "
										+ ledger);
						fillInLedger(ledger);
					}
		...
		requestLedger(ledger_index, false, new Callback<Response>() {
			...
			public void called(Response response) {
				try {
					JSONObject ledgerJSON = response.result
							.getJSONObject("ledger");
					JSONArray transactions = ledgerJSON
					...
					if (!correctHash)
						throw new IllegalStateException(
								"We don't handle invalid transactions yet");
	...
	/**
	 * We can't just look for gaps in the ledgers keys as they are popped off
	 * randomly as the ledgers clear.
	 */
