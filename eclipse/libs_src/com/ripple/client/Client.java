...
public class Client extends Publisher<Client.events> implements
	...
	public static void log(String fmt, Object... args) {
		ClientLogger.log(fmt, args);
	}

	public Request requestLedgerEntry(final Hash256 index, Number ledger_index,
			final Manager<LedgerEntry> cb) {
			...
			public void called(Response response) {
				try {
					if (response.succeeded) {
						STObject node = STObject.translate
								.fromHex(response.result
										.getString("node_binary"));
	...
	public Request requestLedger(Number ledger_index,
			final Manager<JSONObject> cb) {
	...
	public <T> Request makeManagedRequest(final Command cmd,
			final Manager<T> manager, final Request.Builder<T> builder) {
	...
	public Request requestAccountInfo(final AccountID addy,
			final Manager<AccountRoot> manager) {
		return makeManagedRequest(Command.account_info, manager,
				new Request.Builder<AccountRoot>() {
					@Override
					...
					public AccountRoot buildTypedResponse(Response response) {
						JSONObject root = response.result
								.optJSONObject("account_data");
						return (AccountRoot) STObject.fromJSONObject(root);
					}
				});
	}

	public Request requestAccountLines(final AccountID addy,
			final Manager<ArrayList<AccountLine>> manager) {
		return makeManagedRequest(Command.account_lines, manager,
				new Request.Builder<ArrayList<AccountLine>>() {
					@Override
					public void beforeRequest(Request request) {
						request.json("account", addy);
					}
					...
					@Override
					public ArrayList<AccountLine> buildTypedResponse(
							Response response) {
	...
	public static interface OnValidatedTransaction extends
	...
	public boolean runningOnClientThread() {
		return clientThread != null
				&& Thread.currentThread().getId() == clientThread.getId();
	}
	...
	public Request requestBookOffers(Number ledger_index, Issue get, Issue pay,
	...
	public Request requestTransaction(Hash256 hash,
			final Manager<TransactionResult> cb) {
		return requestTransaction(hash.toHex(), cb);
	}

	public Request requestTransaction(String hash,
			final Manager<TransactionResult> cb) {
	...
	private void requestAccountRoot(final AccountID id,
			final TrackedAccountRoot accountRoot, final int attempt) {
		...
		req.once(Request.OnResponse.class, new Request.OnResponse() {
			...
			public void called(Response response) {
				...
				try {
					if (response.succeeded) {
						accountRoot.setFromJSON(response.result
								.getJSONObject("node"));
					} else if (response.rpcerr == RPCErr.entryNotFound) {
						log("Unfunded account: {0}", response.message);
	...
	public void onMessageInClientThread(JSONObject msg) {
		...
		try {
			...
			log("Receive `{0}`: {1}", type, prettyJSON(msg));
	...
	public void setTransactionSubscriptionManager(
	...
	public void onTransactionResult(TransactionResult tr) {
		log("Transaction {0} is validated", tr.hash);
		...
		if (affected != null) {
			...
			for (Map.Entry<AccountID, STObject> entry : affected.entrySet()) {
				...
				if (account != null) {
					...
					account.getAccountRoot().updateFromTransaction(
							transactionHash, transactionLedgerIndex,
							rootUpdates);
				}
			}
		}
		...
		if (initator != null) {
			log("Found initiator {0}, notifying transactionManager", initator);
		} else {
	...
	void onResponse(JSONObject msg) {
		...
		if (request == null) {
			log("Response without a request: {0}", msg);
			return;
		}
	...
	private void doOnDisconnected() {
		ClientLogger.entering(getClass().getName(), "doOnDisconnected");
		connected = false;
		...
		if (!manuallyDisconnected) {
			...
			ClientLogger.fine("Currently disconnecting, so will not reconnect");
		}
