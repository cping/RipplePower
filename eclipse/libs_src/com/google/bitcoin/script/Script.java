 ...
 * href="https://en.bitcoin.it/wiki/Script">a small binary stack language</a> is
 * used to define programs that when evaluated return whether the transaction
...
public class Script {
	...
	// The program is a set of chunks where each element is either [opcode] or
	// [data, data, data ...]
	protected List<ScriptChunk> chunks;
	// Unfortunately, scripts are not ever re-serialized or canonicalized when
	// used in signature hashing. Thus we
	// must preserve the exact bytes that we read off the wire, along with the
	// parsed form.
	...
	Script(List<ScriptChunk> chunks) {
		this.chunks = Collections.unmodifiableList(new ArrayList<ScriptChunk>(
				chunks));
		creationTimeSeconds = Utils.currentTimeMillis() / 1000;
	}

	/**
	 * Construct a Script that copies and wraps the programBytes array. The
	 * array is parsed and checked for syntactic validity.
	 * 
	 * @param programBytes
	...
	public byte[] getProgram() {
		try {
			// Don't round-trip as Satoshi's code doesn't and it would introduce
			// a mismatch.
			if (program != null)
	...
	static {
		Script examplePayToAddress = ScriptBuilder
				.createOutputScript(new Address(MainNetParams.get(),
						new byte[20]));
		examplePayToAddress = new Script(examplePayToAddress.getProgram());
		INTERN_TABLE = new ScriptChunk[] { examplePayToAddress.chunks.get(0), // DUP
	...
	private void parse(byte[] program) throws ScriptException {
		...
		while (bis.available() > 0) {
			...
			if (opcode >= 0 && opcode < OP_PUSHDATA1) {
				...
				if (bis.available() < 4)
					throw new ScriptException("Unexpected end of script");
				dataToRead = ((long) bis.read()) | (((long) bis.read()) << 8)
						| (((long) bis.read()) << 16)
						| (((long) bis.read()) << 24);
			}
			...
			ScriptChunk chunk;
			if (dataToRead == -1) {
				chunk = new ScriptChunk(true, new byte[] { (byte) opcode },
				...
				checkState(dataToRead == 0
						|| bis.read(data, 0, (int) dataToRead) == dataToRead);
				chunk = new ScriptChunk(false, data, startLocationInProgram);
			}
			// Save some memory by eliminating redundant copies of the same
			// chunk objects. INTERN_TABLE can be null
	...
	/**
	 * Returns true if this script is of the form OP_HASH160 <scriptHash>
	 * OP_EQUAL, ie, payment to an address like
	 * 35b9vsyH1KoFT5a5KtrKusaCcPLkiSo1tU. This form was codified as part of
	 * BIP13 and BIP16, for pay to script hash type addresses.
	 */
	public boolean isSentToP2SH() {
		return chunks.size() == 3 && chunks.get(0).equalsOpCode(OP_HASH160)
	 ...
	 * derived from the public key. The concept of a "from address" isn't well
	 * defined in Bitcoin and you should not assume the sender of a transaction
	 * can actually receive coins on it. This method may be removed in future.
	 */
	@Deprecated
	...
	public static byte[] createMultiSigOutputScript(int threshold,
		...
		checkArgument(pubkeys.size() <= 16); // That's the max we can represent
												// with a single opcode.
		if (pubkeys.size() > 3) {
			log.warn(
					"Creating a multi-signature output that is non-standard: {} pubkeys, should be <= 3",
					pubkeys.size());
		}
		try {
	...
	public static byte[] createInputScript(byte[] signature, byte[] pubkey) {
		try {
			...
			ByteArrayOutputStream bits = new UnsafeByteArrayOutputStream(
	...
	/**
	 * Converts an opcode to its int representation (including OP_1NEGATE and
	 * OP_0/OP_FALSE)
	 * 
	 * @throws IllegalArgumentException
	 *             If the opcode is not an OP_N opcode
	 */
	...
	static int decodeFromOpN(int opcode) {
		checkArgument((opcode == OP_0 || opcode == OP_1NEGATE)
	...
	static int encodeToOpN(int value) {
		checkArgument(value >= -1 && value <= 16, "encodeToOpN called for "
				+ value + " which we cannot encode in an opcode.");
		if (value == 0)
			return OP_0;
		else if (value == -1)
			return OP_1NEGATE;
	 ...
	 * <a href="https://github.com/bitcoin/bips/blob/master/bip-0016.mediawiki">
	 * BIP 16</a>.
	 * </p>
	 * 
	 * <p>
	 * bitcoinj does not support creation of P2SH transactions today. The goal
	 * of P2SH is to allow short addresses even for complex scripts (eg,
	 ...
	 * output set (which improves performance of the Bitcoin system).
	 * </p>
	 */
	public boolean isPayToScriptHash() {
		// We have to check against the serialized form because BIP16 defines a
		// P2SH output using an exact byte
		// template, not the logical program structure. Thus you can have two
		// programs that look identical when
	...
	public boolean isSentToMultiSig() {
		...
		if (!chunk.isOpCode())
			return false;
		if (!(chunk.equalsOpCode(OP_CHECKMULTISIG) || chunk
				.equalsOpCode(OP_CHECKMULTISIGVERIFY)))
			return false;
		try {
			...
			if (decodeFromOpN(chunks.get(0).data[0]) < 1)
				return false;
	...
	private static void executeScript(Transaction txContainingThis, long index,
		...
		for (ScriptChunk chunk : script.chunks) {
			...
			if (!chunk.isOpCode()) {
				...
				case OP_16:
					stack.add(Utils.reverseBytes(Utils.encodeMPI(
							BigInteger.valueOf(decodeFromOpN(opcode)), false)));
					break;
				case OP_NOP:
					break;
				case OP_VERIFY:
				...
				case OP_CHECKSIG:
				case OP_CHECKSIGVERIFY:
					executeCheckSig(txContainingThis, (int) index, script,
							stack, lastCodeSepLocation, opcode);
					break;
				case OP_CHECKMULTISIG:
				case OP_CHECKMULTISIGVERIFY:
					opCount = executeMultiSig(txContainingThis, (int) index,
							script, stack, opCount, lastCodeSepLocation, opcode);
					break;
				case OP_NOP1:
				case OP_NOP2:
				case OP_NOP3:
				...
				default:
					throw new ScriptException("Script used a reserved opcode "
							+ opcode);
				}
			}
	...
	private static void executeCheckSig(Transaction txContainingThis,
			int index, Script script, LinkedList<byte[]> stack,
		...
		byte[] connectedScript = Arrays.copyOfRange(prog, lastCodeSepLocation,
				prog.length);
		...
		connectedScript = removeAllInstancesOf(connectedScript,
				outStream.toByteArray());

		// TODO: Use int for indexes everywhere, we can't have that many
		// inputs/outputs
		boolean sigValid = false;
		try {
			TransactionSignature sig = TransactionSignature.decodeFromBitcoin(
					sigBytes, false);
			Sha256Hash hash = txContainingThis.hashForSignature(index,
					connectedScript, (byte) sig.sighashFlags);
			sigValid = ECKey.verify(hash.getBytes(), sig, pubKey);
		} catch (Exception e1) {
			// There is (at least) one exception that could be hit here
			// (EOFException, if the sig is too short)
			// Because I can't verify there aren't more, we use a very generic
			// Exception catch
	...
	private static int executeMultiSig(Transaction txContainingThis, int index,
			Script script, LinkedList<byte[]> stack, int opCount,
			int lastCodeSepLocation, int opcode) throws ScriptException {
		if (stack.size() < 2)
			throw new ScriptException(
		...
		while (sigs.size() > 0) {
			...
			// We could reasonably move this out of the loop, but because
			// signature verification is significantly
			// more expensive than hashing, its not a big deal.
			try {
				TransactionSignature sig = TransactionSignature
						.decodeFromBitcoin(sigs.getFirst(), false);
				Sha256Hash hash = txContainingThis.hashForSignature(index,
						connectedScript, (byte) sig.sighashFlags);
				...
				// There is (at least) one exception that could be hit here
				// (EOFException, if the sig is too short)
				// Because I can't verify there aren't more, we use a very
	 ...
	 *            Accessing txContainingThis from another thread while this
	 *            method runs results in undefined behavior.
	...
	public void correctlySpends(Transaction txContainingThis,
			...
			throws ScriptException {
		// Clone the transaction because executing the script involves editing
		// it, and if we die, we'll leave
		// the tx half broken (also it's not so thread safe to work on it
		// directly.
		try {
			txContainingThis = new Transaction(txContainingThis.getParams(),
					txContainingThis.bitcoinSerialize());
		} catch (ProtocolException e) {
			throw new RuntimeException(e); // Should not happen unless we were
											// given a totally broken
											// transaction.
		}
		if (getProgram().length > 10000
				|| scriptPubKey.getProgram().length > 10000)
