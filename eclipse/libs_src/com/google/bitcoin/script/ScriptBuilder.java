 ...
 * need this as it's hidden behind convenience methods on
 * {@link com.google.bitcoin.core.Transaction}, but they are useful when working
 * with the protocol at a lower level.
 * </p>
 */
public class ScriptBuilder {
	...
	public ScriptBuilder smallNum(int num) {
		...
		chunks.add(new ScriptChunk(true, new byte[] { (byte) Script
				.encodeToOpN(num) }));
		return this;
	}
	...
	public static Script createOutputScript(Address to) {
		if (to.isP2SHAddress()) {
			...
			return new ScriptBuilder().op(OP_HASH160).data(to.getHash160())
					.op(OP_EQUAL).build();
		} else {
			// OP_DUP OP_HASH160 <pubKeyHash> OP_EQUALVERIFY OP_CHECKSIG
			return new ScriptBuilder().op(OP_DUP).op(OP_HASH160)
					.data(to.getHash160()).op(OP_EQUALVERIFY).op(OP_CHECKSIG)
					.build();
		}
	}
	...
	public static Script createOutputScript(ECKey key) {
		return new ScriptBuilder().data(key.getPubKey()).op(OP_CHECKSIG)
				.build();
	}
	...
	public static Script createInputScript(TransactionSignature signature,
		...
		return new ScriptBuilder().data(signature.encodeToBitcoin())
				.data(pubkeyBytes).build();
	}
	...
	public static Script createMultiSigInputScript(
	...
	public static Script createMultiSigInputScriptBytes(List<byte[]> signatures) {
		...
		ScriptBuilder builder = new ScriptBuilder();
		builder.smallNum(0); // Work around a bug in CHECKMULTISIG that is now a
	...
	public static Script createP2SHOutputScript(byte[] hash) {
		...
		return new ScriptBuilder().op(OP_HASH160).data(hash).op(OP_EQUAL)
				.build();
	}
}
