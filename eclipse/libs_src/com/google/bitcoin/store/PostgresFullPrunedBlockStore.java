...
/**
 * <p>
 * A full pruned block store using the Postgres database engine. As an added
 * bonus an address index is calculated, so you can use
 * {@link #calculateBalanceForAddress(com.google.bitcoin.core.Address)} to
 * quickly look up the quantity of bitcoins controlled by that address.
 * </p>
 */
public class PostgresFullPrunedBlockStore implements FullPrunedBlockStore {
	private static final Logger log = LoggerFactory
			.getLogger(PostgresFullPrunedBlockStore.class);
	...
	private static final String CREATE_HEADERS_TABLE = "CREATE TABLE headers ("
			+ "    hash bytea NOT NULL," + "    chainwork bytea NOT NULL,"
			+ "    height integer NOT NULL," + "    header bytea NOT NULL,"
			+ "    wasundoable boolean NOT NULL" + ");";

	private static final String CREATE_UNDOABLE_TABLE = "CREATE TABLE undoableblocks ("
			+ "    hash bytea NOT NULL,"
			+ "    height integer NOT NULL,"
			+ "    txoutchanges bytea," + "    transactions bytea" + ");";
	...
	public PostgresFullPrunedBlockStore(NetworkParameters params,
			int fullStoreDepth, String hostname, String dbName,
			String username, String password) throws BlockStoreException {
		this(params, "jdbc:postgresql://" + hostname + "/" + dbName,
				fullStoreDepth, username, password, null);
	}

	/**
	 * <p>
	 * Create a new PostgresFullPrunedBlockStore, storing the tables in the
	 * schema specified. You may want to specify a schema to avoid name
	 * collisions, or just to keep the database better organized. The schema is
	 * not required, and if one is not provided than the default schema for the
	 * username will be used. See <a
	 * href="http://www.postgres.org/docs/9.3/static/ddl-schemas.html">the
	 * postgres schema docs</a> for more on schemas.
	 * </p>
	 * 
	 * @param params
	 *            A copy of the NetworkParameters used.
	...
	public PostgresFullPrunedBlockStore(NetworkParameters params,
			int fullStoreDepth, String hostname, String dbName,
			String username, String password, @Nullable String schemaName)
			throws BlockStoreException {
		this(params, "jdbc:postgresql://" + hostname + "/" + dbName,
				fullStoreDepth, username, password, schemaName);
	}

	/**
	 * <p>
	 * Create a new PostgresFullPrunedBlockStore, using the full connection URL
	 * instead of a hostname and password, and optionally allowing a schema to
	 * be specified.
	 * </p>
	 * 
	 * <p>
	 * The connection URL will be passed to the database driver, and should look
	 * like "jdbc:postrgresql://host[:port]/databasename". You can use this to
	 * change the port, or specify additional parameters. See <a href=
	 * "http://jdbc.postgresql.org/documentation/head/connect.html#connection-parameters"
	 * > the PostgreSQL JDBC documentation</a> for more on the connection URL.
	 * </p>
	 * 
	 * <p>
	 * This constructor also accepts a schema name to use, which can be used to
	 * avoid name collisions, or to keep the database organized. If no schema is
	 * provided the default schema for the username will be used. See <a
	 * href="http://www.postgres.org/docs/9.3/static/ddl-schemas.html">the
	 * postgres schema docs</a> for more on schemas.
	 * </p>
	 * 
	 ...
	 */
	public PostgresFullPrunedBlockStore(NetworkParameters params,
			String connectionURL, int fullStoreDepth, String username,
			String password, @Nullable String schemaName)
			throws BlockStoreException {
		this.params = params;
	...
	private void initFromDatabase() throws SQLException, BlockStoreException {
		...
		if (!rs.next()) {
			throw new BlockStoreException(
					"corrupt Postgres block store - no chain head pointer");
		}
		...
		if (this.chainHeadBlock == null) {
			throw new BlockStoreException(
					"corrupt Postgres block store - head block not found");
		}
		...
		if (this.verifiedChainHeadBlock == null) {
			throw new BlockStoreException(
					"corrupt Postgres block store - verified head block not found");
		}
	}
	...
	public void put(StoredBlock storedBlock, StoredUndoableBlock undoableBlock)
		...
		try {
			if (log.isDebugEnabled())
				log.debug("Looking for undoable block with hash: "
						+ Utils.bytesToHexString(hashBytes));

			PreparedStatement findS = conn.get().prepareStatement(
			...
			if (rs.next()) {
				...
				if (log.isDebugEnabled())
					log.debug("Updating undoable block with hash: "
							+ Utils.bytesToHexString(hashBytes));
			...
			if (log.isDebugEnabled())
				log.debug("Inserting undoable block with hash: "
						+ Utils.bytesToHexString(hashBytes) + " at height "
						+ height);
	...
	private void removeUndoableBlocksWhereHeightIsLessThan(int height)
		...
		try {
			...
			if (log.isDebugEnabled())
				log.debug("Deleting undoable undoable block with height <= "
						+ height);
	...
	public StoredTransactionOutput getTransactionOutput(Sha256Hash hash,
		...
		try {
			...
			StoredTransactionOutput txout = new StoredTransactionOutput(hash,
					index, value, height, true, results.getBytes(3));
			return txout;
		} catch (SQLException ex) {
			throw new BlockStoreException(ex);
		} finally {
	...
	public void addUnspentTransactionOutput(StoredTransactionOutput out)
		...
		try {
			...
			// Unparseable, but this isn't an error - it's an output not
			// containing an address
			log.info("Could not parse script for output: "
					+ out.getHash().toString());
		}
		if (outputScript != null
				&& (outputScript.isSentToAddress()
						|| outputScript.isSentToRawPubKey() || outputScript
							.isPayToScriptHash())) {
			if (outputScript.isSentToAddress()) {
				Address targetAddr = new Address(params,
						outputScript.getPubKeyHash());
				dbAddress = targetAddr.toString();
				type = 1;
			} else if (outputScript.isSentToRawPubKey()) {
				/*
				 * Note we use the deprecated getFromAddress here. Coinbase
				 * outputs seem to have the target address in the pubkey of the
				 * script - perhaps we can rename this function?
				 */

				dbAddress = outputScript.getFromAddress(params).toString();
				type = 2;
			} else if (outputScript.isPayToScriptHash()) {
				dbAddress = Address.fromP2SHHash(params,
						outputScript.getPubKeyHash()).toString();
				type = 3;
			}
		}
	...
	public void abortDatabaseBatchWrite() throws BlockStoreException {
		...
		maybeConnect();
		if (log.isDebugEnabled())
			log.debug("Rollback database batch write with connection: "
					+ conn.get().toString());
	 ...
	 *         seen, or there are no outputs open for this address, the return
	 *         value is 0
	 * @throws BlockStoreException
	 */
	public BigInteger calculateBalanceForAddress(Address address)
		...
		try {
			...
			if (rs.next()) {
				...
				throw new BlockStoreException(
						"Failed to execute balance lookup");
			}
