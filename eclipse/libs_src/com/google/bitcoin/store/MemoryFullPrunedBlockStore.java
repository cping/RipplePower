...
/**
 * Used as a key for memory map (to avoid having to think about
 * NetworkParameters, which is required for {@link TransactionOutPoint}
 */
class StoredTransactionOutPoint implements Serializable {
	...
	public boolean equals(Object o) {
		...
		return ((StoredTransactionOutPoint) o).getIndex() == this.index
				&& Objects.equal(this.getHash(),
						((StoredTransactionOutPoint) o).getHash());
	}
}

/**
 * A HashMap<KeyType, ValueType> that is DB transaction-aware This class is not
 * thread-safe.
 */
class TransactionalHashMap<KeyType, ValueType> {
	...
	public ValueType get(KeyType key) {
		if (Boolean.TRUE.equals(inTransaction.get())) {
			...
			if (tempSetRemoved.get() != null
					&& tempSetRemoved.get().contains(key))
				return null;
		}
		return map.get(key);
	}
 ...
 * 
 * @param UniqueKeyType
 *            is a key that must be unique per object
 * @param MultiKeyType
 *            is a key that can have multiple values
 */
class TransactionalMultiKeyHashMap<UniqueKeyType, MultiKeyType, ValueType> {
	...
	public void put(UniqueKeyType uniqueKey, MultiKeyType multiKey,
			ValueType value) {
...
/**
 * Keeps {@link StoredBlock}s, {@link StoredUndoableBlock}s and
 * {@link StoredTransactionOutput}s in memory. Used primarily for unit testing.
 */
public class MemoryFullPrunedBlockStore implements FullPrunedBlockStore {
	 ...
	 * 
	 * @param params
	 *            The network parameters of this block store - used to get
	 *            genesis block
	 * @param fullStoreDepth
	 *            The depth of blocks to keep FullStoredBlocks instead of
	 *            StoredBlocks
	 */
	public MemoryFullPrunedBlockStore(NetworkParameters params,
			int fullStoreDepth) {
	...
	public synchronized void put(StoredBlock block) throws BlockStoreException {
		Preconditions.checkNotNull(blockMap,
				"MemoryFullPrunedBlockStore is closed");
	...
	public synchronized void put(StoredBlock storedBlock,
			StoredUndoableBlock undoableBlock) throws BlockStoreException {
		Preconditions.checkNotNull(blockMap,
				"MemoryFullPrunedBlockStore is closed");
	...
	public synchronized StoredBlock getOnceUndoableStoredBlock(Sha256Hash hash)
		...
		Preconditions.checkNotNull(blockMap,
				"MemoryFullPrunedBlockStore is closed");
		StoredBlockAndWasUndoableFlag storedBlock = blockMap.get(hash);
		return (storedBlock != null && storedBlock.wasUndoable) ? storedBlock.block
				: null;
	}
	...
	public synchronized StoredUndoableBlock getUndoBlock(Sha256Hash hash)
			throws BlockStoreException {
		Preconditions.checkNotNull(fullBlockMap,
				"MemoryFullPrunedBlockStore is closed");
	...
	public synchronized void setVerifiedChainHead(StoredBlock chainHead)
			throws BlockStoreException {
		Preconditions.checkNotNull(blockMap,
				"MemoryFullPrunedBlockStore is closed");
		...
		// Though the FullPrunedBlockStore allows for this, the current
		// AbstractBlockChain will not do it.
	...
	@Nullable
	public synchronized StoredTransactionOutput getTransactionOutput(
			Sha256Hash hash, long index) throws BlockStoreException {
		Preconditions.checkNotNull(transactionOutputMap,
				"MemoryFullPrunedBlockStore is closed");
		return transactionOutputMap.get(new StoredTransactionOutPoint(hash,
				index));
	}
	...
	public synchronized void removeUnspentTransactionOutput(
		...
		if (transactionOutputMap.remove(new StoredTransactionOutPoint(out)) == null)
			throw new BlockStoreException(
					"Tried to remove a StoredTransactionOutput from MemoryFullPrunedBlockStore that it didn't have!");
	}
	...
	public synchronized boolean hasUnspentOutputs(Sha256Hash hash,
			int numOutputs) throws BlockStoreException {
