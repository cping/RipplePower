 ...
 * ring buffer. With such a store, you may not be able to process very deep
 * re-orgs and could be disconnected from the chain (requiring a replay), but as
 * they are virtually unheard of this is not a significant risk.
 */
public class SPVBlockStore implements BlockStore {
	private static final Logger log = LoggerFactory
			.getLogger(SPVBlockStore.class);
	...
	// get() call is actually a full-blown JNI method under the hood, meaning
	// it's unbelievably slow. The caches
	...
	protected LinkedHashMap<Sha256Hash, StoredBlock> blockCache = new LinkedHashMap<Sha256Hash, StoredBlock>() {
		...
		protected boolean removeEldestEntry(
				Map.Entry<Sha256Hash, StoredBlock> entry) {
	...
	/**
	 * Creates and initializes an SPV block store. Will create the given file if
	...
	public SPVBlockStore(NetworkParameters params, File file)
		...
		try {
			...
			if (fileLock == null)
				throw new BlockStoreException(
						"Store file is already locked by another process");

			// Map it into memory read/write. The kernel will take care of
			// flushing writes to disk at the most
			// efficient times, which may mean that until the map is deallocated
			// the data on disk is randomly
			// inconsistent. However the only process accessing it is us, via
			// this mapping, so our own view will
			// always be correct. Once we establish the mmap the underlying file
			// and channel can go away. Note that
			// the details of mmapping vary between platforms.
			buffer = channel.map(FileChannel.MapMode.READ_WRITE, 0, fileSize);

			// Check or initialize the header bytes to ensure we don't try to
			...
			if (exists) {
				...
				if (!new String(header, "US-ASCII").equals(HEADER_MAGIC))
					throw new BlockStoreException("Header bytes do not equal "
							+ HEADER_MAGIC);
			} else {
				initNewStore(params);
			}
	...
	private void initNewStore(NetworkParameters params) throws Exception {
		...
		StoredBlock storedGenesis = new StoredBlock(genesis, genesis.getWork(),
				0);
		put(storedGenesis);
	...
	/**
	 * Returns the size in bytes of the file that is used to store the chain
	...
	public StoredBlock get(Sha256Hash hash) throws BlockStoreException {
		...
		try {
			...
			// Starting from the current tip of the ring work backwards until we
			// have either found the block or
			// wrapped around.
			int cursor = getRingCursor(buffer);
			...
			do {
				...
				// Cursor is now at the start of the next record to check, so
				// read the hash and compare it.
				buffer.position(cursor);
				buffer.get(scratch);
				if (Arrays.equals(scratch, targetHashBytes)) {
					// Found the target.
					StoredBlock storedBlock = StoredBlock.deserializeCompact(
							params, buffer);
	...
	public StoredBlock getChainHead() throws BlockStoreException {
		...
		try {
			if (lastChainHead == null) {
				...
				if (block == null)
					throw new BlockStoreException(
							"Corrupted block store: could not find chain head: "
									+ hash);
				lastChainHead = block;
			}
	...
	public void close() throws BlockStoreException {
		try {
			buffer.force();
			buffer = null; // Allow it to be GCd and the underlying file mapping
							// to go away.
			randomAccessFile.close();
		} catch (IOException e) {
			throw new BlockStoreException(e);
	...
	// File format:
	// 4 header bytes = "SPVB"
	// 4 cursor bytes, which indicate the offset from the first kb where the
	// next block header should be written.
