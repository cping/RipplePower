...
/**
 * Serialize and de-serialize a wallet to a byte stream containing a <a
 * href="http://code.google.com/apis/protocolbuffers/docs/overview.html"
 * >protocol buffer</a>. Protocol buffers are a data interchange format
 * developed by Google with an efficient binary representation, a type safe
 * specification language and compilers that generate code to work with those
 * data structures for many languages. Protocol buffers can have their format
 * evolved over time: conceptually they represent data using (tag, length,
 * value) tuples. The format is defined by the <tt>bitcoin.proto</tt> file in
 * the bitcoinj source distribution.
 * <p>
 * 
...
public class WalletProtobufSerializer {
	private static final Logger log = LoggerFactory
			.getLogger(WalletProtobufSerializer.class);
	...
	/**
	 * Formats the given wallet (transactions and keys) to the given output
	 * stream in protocol buffer format.
	 * <p>
	 * 
	...
	/**
	 * Returns the given wallet formatted as text. The text format is that used
	 * by protocol buffers and although it can also be parsed using
	 * {@link TextFormat#merge(CharSequence, com.google.protobuf.Message.Builder)}
	 * , it is designed more for debugging than storage. It is not well
	 * specified and wallets are largely binary data structures anyway,
	 * consisting as they do of keys (large random numbers) and
	 * {@link Transaction}s which also mostly contain keys and hashes.
	 */
	...
	public Protos.Wallet walletToProto(Wallet wallet) {
		...
		walletBuilder.setNetworkIdentifier(wallet.getNetworkParameters()
				.getId());
		...
		for (ECKey key : wallet.getKeys()) {
			Protos.Key.Builder keyBuilder = Protos.Key.newBuilder()
					.setCreationTimestamp(key.getCreationTimeSeconds() * 1000)
					// .setLabel() TODO
					.setType(Protos.Key.Type.ORIGINAL);
			if (key.getPrivKeyBytes() != null)
				keyBuilder.setPrivateKey(ByteString.copyFrom(key
						.getPrivKeyBytes()));
			...
			if (encryptedPrivateKey != null) {
				// Key is encrypted.
				Protos.EncryptedPrivateKey.Builder encryptedKeyBuilder = Protos.EncryptedPrivateKey
						.newBuilder()
				...
				if (key.getKeyCrypter() == null) {
					throw new IllegalStateException("The encrypted key "
							+ key.toString() + " has no KeyCrypter.");
				} else {
					// If it is a Scrypt + AES encrypted key, set the persisted
					// key type.
					if (key.getKeyCrypter().getUnderstoodEncryptionType() == Protos.Wallet.EncryptionType.ENCRYPTED_SCRYPT_AES) {
						keyBuilder
								.setType(Protos.Key.Type.ENCRYPTED_SCRYPT_AES);
					} else {
			...
			// We serialize the public key even if the private key is present
			// for speed reasons: we don't want to do
			// lots of slow EC math to load the wallet, we prefer to store the
			// redundant data instead. It matters more
			// on mobile platforms.
		...
		for (Script script : wallet.getWatchedScripts()) {
			Protos.Script protoScript = Protos.Script
					.newBuilder()
					.setProgram(ByteString.copyFrom(script.getProgram()))
					.setCreationTimestamp(
							script.getCreationTimeSeconds() * 1000).build();
		...
		if (lastSeenBlockHash != null) {
			walletBuilder
					.setLastSeenBlockHash(hashToByteString(lastSeenBlockHash));
			walletBuilder.setLastSeenBlockHeight(wallet
					.getLastBlockSeenHeight());
		}
		if (wallet.getLastBlockSeenTimeSecs() > 0)
			walletBuilder.setLastSeenBlockTimeSecs(wallet
		...
		if (keyCrypter == null) {
			...
			walletBuilder.setEncryptionType(keyCrypter
					.getUnderstoodEncryptionType());
			if (keyCrypter instanceof KeyCrypterScrypt) {
				KeyCrypterScrypt keyCrypterScrypt = (KeyCrypterScrypt) keyCrypter;
				walletBuilder.setEncryptionParameters(keyCrypterScrypt
						.getScryptParameters());
			} else {
	...
	private static void populateExtensions(Wallet wallet,
			Protos.Wallet.Builder walletBuilder) {
		for (WalletExtension extension : wallet.getExtensions().values()) {
			...
			proto.setData(ByteString.copyFrom(extension
	...
	private static Protos.Transaction makeTxProto(WalletTransaction wtx) {
		...
		for (TransactionInput input : tx.getInputs()) {
			Protos.TransactionInput.Builder inputBuilder = Protos.TransactionInput
					.newBuilder()
					.setScriptBytes(ByteString.copyFrom(input.getScriptBytes()))
					.setTransactionOutPointHash(
							hashToByteString(input.getOutpoint().getHash()))
		...
		for (TransactionOutput output : tx.getOutputs()) {
			Protos.TransactionOutput.Builder outputBuilder = Protos.TransactionOutput
					.newBuilder()
					.setScriptBytes(
							ByteString.copyFrom(output.getScriptBytes()))
					.setValue(output.getValue().longValue());
			final TransactionInput spentBy = output.getSpentBy();
			if (spentBy != null) {
				Sha256Hash spendingHash = spentBy.getParentTransaction()
						.getHash();
				int spentByTransactionIndex = spentBy.getParentTransaction()
						.getInputs().indexOf(spentBy);
				outputBuilder.setSpentByTransactionHash(
						hashToByteString(spendingHash))
		...
		final Map<Sha256Hash, Integer> appearsInHashes = tx
				.getAppearsInHashes();
		if (appearsInHashes != null) {
			for (Map.Entry<Sha256Hash, Integer> entry : appearsInHashes
					.entrySet()) {
		...
		if (tx.hasConfidence()) {
			...
			Protos.TransactionConfidence.Builder confidenceBuilder = Protos.TransactionConfidence
					.newBuilder();
		...
		default:
			throw new RuntimeException(
					"New tx purpose serialization not implemented.");
		}
	...
	private static void writeConfidence(Protos.Transaction.Builder txBuilder,
		...
		synchronized (confidence) {
			confidenceBuilder.setType(Protos.TransactionConfidence.Type
					.valueOf(confidence.getConfidenceType().getValue()));
			if (confidence.getConfidenceType() == ConfidenceType.BUILDING) {
				confidenceBuilder.setAppearedAtHeight(confidence
						.getAppearedAtChainHeight());
				confidenceBuilder.setDepth(confidence.getDepthInBlocks());
				if (confidence.getWorkDone() != null) {
					confidenceBuilder.setWorkDone(confidence.getWorkDone()
							.longValue());
				}
			}
			if (confidence.getConfidenceType() == ConfidenceType.DEAD) {
				...
				if (confidence.getOverridingTransaction() != null) {
					Sha256Hash overridingHash = confidence
							.getOverridingTransaction().getHash();
					confidenceBuilder
							.setOverridingTransaction(hashToByteString(overridingHash));
				}
			}
			...
			case SELF:
				confidenceBuilder
						.setSource(Protos.TransactionConfidence.Source.SOURCE_SELF);
				break;
			case NETWORK:
				confidenceBuilder
						.setSource(Protos.TransactionConfidence.Source.SOURCE_NETWORK);
				break;
			case UNKNOWN:
				// Fall through.
			default:
				confidenceBuilder
						.setSource(Protos.TransactionConfidence.Source.SOURCE_UNKNOWN);
				break;
			}
		}

		for (ListIterator<PeerAddress> it = confidence.getBroadcastBy(); it
				.hasNext();) {
			PeerAddress address = it.next();
			Protos.PeerAddress proto = Protos.PeerAddress
					.newBuilder()
					.setIpAddress(
							ByteString.copyFrom(address.getAddr().getAddress()))
	...
	/**
	 * <p>
	 * Parses a wallet from the given stream, using the provided Wallet instance
	 * to load data into. This is primarily used when you want to register
	 * extensions. Data in the proto will be added into the wallet where
	 * applicable and overwrite where not.
	 * </p>
	 * 
	 * <p>
		...
		try {
			...
			if (params == null)
				throw new UnreadableWalletException(
						"Unknown network parameters ID " + paramsID);
			Wallet wallet = new Wallet(params);
			readWallet(walletProto, wallet);
			return wallet;
		} catch (IOException e) {
			throw new UnreadableWalletException(
					"Could not parse input stream to protobuf", e);
		}
	}
			...
			if (extension == null) {
				if (extProto.getMandatory()) {
					if (requireMandatoryExtensions)
						throw new UnreadableWalletException(
								"Unknown mandatory extension in wallet: " + id);
					else
						log.error("Unknown extension in wallet {}, ignoring",
								id);
				}
			} else {
				log.info("Loading wallet extension {}", id);
				try {
					extension.deserializeWalletExtension(wallet, extProto
							.getData().toByteArray());
				} catch (Exception e) {
					if (extProto.getMandatory() && requireMandatoryExtensions)
						throw new UnreadableWalletException(
								"Could not parse mandatory extension in wallet: "
										+ id);
					else
						log.error(
								"Error whilst reading extension {}, ignoring: {}",
								id, e);
				}
			}
		}
	 ...
	 * normally want {@link Wallet#loadFromFile(java.io.File)} instead - this
	 * method is designed for low level work involving the wallet file format
	 * itself.
	 */
	...
	private WalletTransaction connectTransactionOutputs(
		...
		for (int i = 0; i < tx.getOutputs().size(); i++) {
			...
			if (transactionOutput.hasSpentByTransactionHash()) {
				...
				if (spendingTx == null) {
					throw new UnreadableWalletException(String.format(
							"Could not connect %s to %s", tx.getHashAsString(),
