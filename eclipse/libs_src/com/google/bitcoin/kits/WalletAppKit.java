...
public class WalletAppKit extends AbstractIdleService {
	...
	public WalletAppKit(NetworkParameters params, File directory,
			String filePrefix) {
	 ...
	 * here. For instance, a {@link DownloadListener} is a good choice.
	 */
	 ...
	 * {@link #startAndWait()} can potentially take a very long time. If false,
	 ...
	 * 
	 * @param userAgent
	 *            A short string that should be the name of your app, e.g.
	 *            "My Wallet"
	 * @param version
	...
	protected void startUp() throws Exception {
		...
		try {
			...
			if (!chainFileExists && checkpoints != null) {
				// Ugly hack! We have to create the wallet once here to learn
				// the earliest key time, and then throw it
				// away. The reason is that wallet extensions might need access
				// to peergroups/chains/etc so we have to
				// create the wallet later, but we need to know the time early
				// here before we create the BlockChain
				// object.
			...
			if (vWalletFile.exists()) {
				...
				addWalletExtensions(); // All extensions must be present before
										// we deserialize
				new WalletProtobufSerializer().readWallet(
						WalletProtobufSerializer.parseToProto(walletStream),
						vWallet);
			...
			if (blockingStartup) {
				...
				Futures.addCallback(vPeerGroup.start(),
