 ...
 * 
 * <p>
 * 1) Using a user password, create an AES key that can encrypt and decrypt your
 * private keys. To convert the password to the AES key, scrypt is used. This is
 * an algorithm resistant to brute force attacks. You can use the
 * ScryptParameters to tune how difficult you want this to be generation to be.
 * </p>
 * 
 * <p>
 * 2) Using the AES Key generated above, you then can encrypt and decrypt any
 * bytes using the AES symmetric cipher. Eight bytes of salt is used to prevent
 * dictionary attacks.
 * </p>
 */
public class KeyCrypterScrypt implements KeyCrypter, Serializable {
	private static final Logger log = LoggerFactory
			.getLogger(KeyCrypterScrypt.class);
	...
	/**
	 * The size of an AES block in bytes. This is also the length of the
	...
	public KeyCrypterScrypt() {
		Protos.ScryptParameters.Builder scryptParametersBuilder = Protos.ScryptParameters
				.newBuilder().setSalt(ByteString.copyFrom(randomSalt()));
		this.scryptParameters = scryptParametersBuilder.build();
	}

	/**
	 * Encryption/Decryption using custom number of iterations parameters and a
	 * random salt. A useful value for mobile devices is 512 (~500 ms).
	 * 
	 * @param iterations
	 ...
	 * 
	 * @param scryptParameters
	 *            ScryptParameters to use
	...
	public KeyCrypterScrypt(ScryptParameters scryptParameters) {
		...
		// (Some early MultiBit wallets has a missing salt so it is not a hard
		// fail).
	 ...
	 * Generate AES key.
	 * 
	 * This is a very slow operation compared to encrypt/ decrypt so it is
	 * normally worth caching the result.
	 * 
	 * @param password
	...
	public KeyParameter deriveKey(CharSequence password)
		...
		try {
			...
			byte[] keyBytes = SCrypt.scrypt(passwordBytes, salt,
					(int) scryptParameters.getN(), scryptParameters.getR(),
					scryptParameters.getP(), KEY_LENGTH);
			return new KeyParameter(keyBytes);
		} catch (Exception e) {
			throw new KeyCrypterException(
					"Could not generate key from password and salt.", e);
		} finally {
	...
	public byte[] decrypt(EncryptedPrivateKey privateKeyToDecode,
		...
		try {
			ParametersWithIV keyWithIv = new ParametersWithIV(new KeyParameter(
					aesKey.getKey()),
			...
			int length1 = cipher.processBytes(cipherBytes, 0,
					cipherBytes.length, outputBuffer, 0);
	 ...
	 * 
	 * Note: a String.getBytes() is not used to avoid creating a String of the
	 * password in the JVM.
	 */
	...
	public boolean equals(Object obj) {
		...
		return com.google.common.base.Objects.equal(this.scryptParameters,
				other.scryptParameters);
	}
}
