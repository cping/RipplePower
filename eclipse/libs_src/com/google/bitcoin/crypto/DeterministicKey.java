...
/**
 * A deterministic key is a node in a {@link DeterministicHierarchy}. As per <a
 * href="https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki">the BIP
 * 32 specification</a> it is a pair (key, chaincode). If you know its path in
 * the tree you can derive more keys from this.
 */
public class DeterministicKey implements Serializable {
	...
	DeterministicKey(ImmutableList<ChildNumber> childNumberPath,
			byte[] chainCode, @Nullable ECPoint publicAsPoint,
			@Nullable BigInteger privateKeyFieldElt,
			@Nullable DeterministicKey parent) {
		checkArgument(chainCode.length == 32);
		this.parent = parent;
		this.childNumberPath = childNumberPath;
		this.chainCode = Arrays.copyOf(chainCode, chainCode.length);
		this.publicAsPoint = publicAsPoint == null ? null : HDUtils
				.compressedCopy(publicAsPoint);
		this.privateAsFieldElement = privateKeyFieldElt;
	}

	/**
	 * Returns the path through some {@link DeterministicHierarchy} which
	 * reaches this keys position in the tree. A path can be written as 1/2/1
	 * which means the first child of the root, the second child of that node,
	 ...
	 * {@link DeterministicKey#getChildNumberPath()}
	 */
	public ChildNumber getChildNumber() {
		return getDepth() == 0 ? ChildNumber.ZERO : childNumberPath
				.get(childNumberPath.size() - 1);
	}
	...
	/**
	 * Returns the path of this key as a human readable string starting with M
	 * to indicate the master key.
	 */
	public String getPath() {
		return PATH_JOINER.join(Iterables.concat(Collections.singleton("M"),
				getChildNumberPath()));
	}
	...
	public byte[] getFingerprint() {
		// TODO: why is this different than armory's fingerprint? BIP 32:
	...
	public byte[] getPrivKeyBytes() {
		return privateAsFieldElement == null ? null : privateAsFieldElement
				.toByteArray();
	}
	...
	/**
	 * Returns the same key with the private part removed. May return the same
	 * instance.
	 */
	public DeterministicKey getPubOnly() {
		if (!hasPrivate())
			return this;
		final DeterministicKey parentPub = getParent() == null ? null
				: getParent().getPubOnly();
		return new DeterministicKey(getChildNumberPath(), getChainCode(),
				getPubPoint(), null, parentPub);
	}
	...
	public String toString() {
		return MessageFormat.format("ExtendedHierarchicKey[pub: {0}]",
				new String(Hex.encode(getPubKeyBytes())));
	}
}
