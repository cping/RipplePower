...
/**
 * Implementation of the (public derivation version) deterministic wallet child
...
public final class HDKeyDerivation {
	...
	private static final HMac MASTER_HMAC_SHA512 = HDUtils
			.createHmacSha512Digest("Bitcoin seed".getBytes());

	/**
	 * Generates a new deterministic key from the given seed, which can be any
	 * arbitrary byte array. However resist the temptation to use a string as
	 * the seed - any key derived from a password is likely to be weak and
	 * easily broken by attackers (this is not theoretical, people have had
	 * money stolen that way). This method checks that the given seed is at
	 * least 64 bits long.
	 * 
	...
	public static DeterministicKey createMasterPrivKeyFromBytes(
		...
		return new DeterministicKey(ImmutableList.<ChildNumber> of(),
				chainCode, null, privateKeyFieldElt, null);
	}

	public static DeterministicKey createMasterPubKeyFromBytes(
			byte[] pubKeyBytes, byte[] chainCode) {
		return new DeterministicKey(ImmutableList.<ChildNumber> of(),
				chainCode, ECKey.CURVE.getCurve().decodePoint(pubKeyBytes),
				null, null);
	}

	/**
	 * @param childNumber
	 *            the "extended" child number, ie. with the 0x80000000 bit
	 *            specifying private/public derivation.
	 */
	...
	/**
	 * @throws HDDerivationException
	 *             if private derivation is attempted for a public-only parent
	 *             key, or if the resulting derived key is invalid (eg. private
	 *             key == 0).
	 */
	public static DeterministicKey deriveChildKey(DeterministicKey parent,
		...
		return new DeterministicKey(
				...
				rawKey.chainCode,
				parent.hasPrivate() ? null : ECKey.CURVE.getCurve()
						.decodePoint(rawKey.keyBytes),
				parent.hasPrivate() ? new BigInteger(1, rawKey.keyBytes) : null,
				parent);
	}

	private static RawKeyBytes deriveChildKeyBytes(DeterministicKey parent,
			ChildNumber childNumber) throws HDDerivationException {
		...
		if (privAsFieldElement != null) {
			BigInteger ki = privAsFieldElement.add(ilInt).mod(
					ECKey.CURVE.getN());
			assertNonZero(ki,
					"Illegal derived key: derived private key equals 0.");
			keyBytes = ki.toByteArray();
		} else {
			checkArgument(!childNumber.isPrivateDerivation(),
					"Can't use private derivation with public keys only.");
			ECPoint Ki = ECKey.CURVE.getG().multiply(ilInt)
					.add(parent.getPubPoint());
