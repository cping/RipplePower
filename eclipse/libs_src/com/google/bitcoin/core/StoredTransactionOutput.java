...
/**
 * A StoredTransactionOutput message contains the information necessary to check
 * a spending transaction. It avoids having to store the entire
 * parentTransaction just to get the hash and index. Its only really useful for
 * MemoryFullPrunedBlockStore, and should probably be moved there
 */
public class StoredTransactionOutput implements Serializable {
	 ...
	 * that the redeemer is allowed to spend this output.
	 */
	...
	/**
	 * arbitrary value lower than -
	 * {@link NetworkParameters#spendableCoinbaseDepth} (not too low to get
	 * overflows when we do blockHeight - NONCOINBASE_HEIGHT, though)
	 */
	private static final int NONCOINBASE_HEIGHT = -200;
	/**
	 * The height of the creating block (for coinbases, NONCOINBASE_HEIGHT
	 * otherwise)
	 */
	private int height;
	 ...
	 * @param scriptBytes
	 */
	public StoredTransactionOutput(Sha256Hash hash, long index,
			BigInteger value, int height, boolean isCoinbase, byte[] scriptBytes) {
		this.hash = hash;
		this.index = index;
		this.value = value;
	...
	public StoredTransactionOutput(Sha256Hash hash, TransactionOutput out,
			int height, boolean isCoinbase) {
		this.hash = hash;
	...
	public StoredTransactionOutput(InputStream in) throws IOException {
		...
		int scriptBytesLength = ((in.read() & 0xFF) << 0)
				| ((in.read() & 0xFF) << 8) | ((in.read() & 0xFF) << 16)
				| ((in.read() & 0xFF) << 24);
		...
		height = ((in.read() & 0xFF) << 0) | ((in.read() & 0xFF) << 8)
	...
	/**
	 * Gets the height of the block that created this output (or -1 if this
	 * output was not created by a coinbase)
	 */
	...
	public String toString() {
		return String.format("Stored TxOut of %s (%s:%d)",
				Utils.bitcoinValueToFriendlyString(value), hash.toString(),
				index);
	}
	...
	public boolean equals(Object o) {
		...
		return ((StoredTransactionOutput) o).getIndex() == this.getIndex()
				&& ((StoredTransactionOutput) o).getHash().equals(
						this.getHash());
	}
