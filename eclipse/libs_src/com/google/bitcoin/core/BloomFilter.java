...
public class BloomFilter extends Message {
	...
	public BloomFilter(int elements, double falsePositiveRate, long randomNonce) {
		this(elements, falsePositiveRate, randomNonce,
	 ...
	 * BloomFilter to is controlled by the false positive rate.
	 * </p>
	 * 
	 * <p>
	 * For reference, as of block 187,000, the total number of addresses used in
	 * the chain was roughly 4.5 million.
	 * </p>
	 * 
	 * <p>
	 * Thus, if you use a false positive rate of 0.001 (0.1%), there will be, on
	 * average, 4,500 distinct public keys/addresses which will be thought to be
	 * yours by nodes which have your bloom filter, but which are not actually
	 * yours.
	 * </p>
	 * 
	 * <p>
	 * Keep in mind that a remote node can do a pretty good job estimating the
	 * order of magnitude of the false positive rate of a given filter you
	 * provide it when considering the anonymity of a given filter.
	 * </p>
	 * 
	 * <p>
	 ...
	 * downloading (default is {@link MemoryPool#MAX_SIZE}). See the comment in
	 * processBlock(FilteredBlock) for more information on this restriction.
	 * </p>
	 * 
	 * <p>
	...
	public BloomFilter(int elements, double falsePositiveRate,
			long randomNonce, BloomUpdate updateFlag) {
	...
	public double getFalsePositiveRate(int elements) {
		return Math.pow(
				1 - Math.pow(Math.E, -1.0 * (hashFuncs * elements)
						/ (data.length * 8)), hashFuncs);
	}

	@Override
	public String toString() {
		return "Bloom Filter of size " + data.length + " with " + hashFuncs
				+ " hash functions.";
	}
	...
	void parse() throws ProtocolException {
		...
		if (hashFuncs > MAX_HASH_FUNCS)
			throw new ProtocolException(
					"Bloom filter hash function count out of range");
		nTweak = readUint32();
	...
	/**
	 * Returns true if the given object matches the filter (either because it
	 * was inserted, or because we have a false-positive)
	 */
	 ...
	 * {@link com.google.bitcoin.core.BloomFilter#setMatchAll()} for when this
