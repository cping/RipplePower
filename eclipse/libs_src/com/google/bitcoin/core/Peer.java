 ...
 * {@link com.google.bitcoin.net.AbstractTimeoutHandler} and timeout is
 * automatically disabled (using
 * {@link com.google.bitcoin.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}
...
public class Peer extends PeerSocketHandler {
	...
	static class PeerListenerRegistration extends
			ListenerRegistration<PeerEventListener> {
		boolean callOnDisconnect = true;

		public PeerListenerRegistration(PeerEventListener listener,
				Executor executor) {
			super(listener, executor);
		}

		public PeerListenerRegistration(PeerEventListener listener,
				Executor executor, boolean callOnDisconnect) {
	...
	private static class GetDataRequest {
		...
		// If the peer does not support the notfound message, we'll use
		// ping/pong messages to simulate it. This is
		// a nasty hack that relies on the fact that bitcoin-qt is single
		// threaded and processes messages in order.
		// The nonce field records which pong should clear this request as
		// "not found".
		long nonce;
	}
	...
	private final SettableFuture<Peer> connectionOpenFuture = SettableFuture
			.create();
	...
	public Peer(NetworkParameters params, VersionMessage ver,
			@Nullable AbstractBlockChain chain, PeerAddress remoteAddress) {
		this(params, ver, remoteAddress, chain, null);
	}

	/**
	 * <p>
	 * Construct a peer that reads/writes from the given block chain and memory
	 * pool. Transactions stored in a memory pool will have their confidence
	 * levels updated when a peer announces it, to reflect the greater
	 ...
	 */
	public Peer(NetworkParameters params, VersionMessage ver,
			PeerAddress remoteAddress, @Nullable AbstractBlockChain chain,
			@Nullable MemoryPool mempool) {
	 ...
	 * </p>
	 */
	public Peer(NetworkParameters params, AbstractBlockChain blockChain,
			PeerAddress peerAddress, String thisSoftwareName,
			String thisSoftwareVersion) {
		this(params, new VersionMessage(params,
				blockChain.getBestChainHeight(), true), blockChain, peerAddress);
		this.versionMessage.appendToSubVer(thisSoftwareName,
				thisSoftwareVersion, null);
	}

	/**
	 * Registers the given object as an event listener that will be invoked on
	 * the user thread. Note that listeners added this way will <b>not</b>
	 * receive {@link PeerEventListener#getData(Peer, GetDataMessage)} or
	 * {@link PeerEventListener#onPreMessageReceived(Peer, Message)} calls
	 * because those require that the listener be added using
	 * {@link Threading#SAME_THREAD}, which requires the other addListener form.
	 */
	...
	/**
	 * Registers the given object as an event listener that will be invoked by
	 * the given executor. Note that listeners added using any other executor
	 * than {@link Threading#SAME_THREAD} will <b>not</b> receive
	 * {@link PeerEventListener#getData(Peer, GetDataMessage)} or
	 * {@link PeerEventListener#onPreMessageReceived(Peer, Message)} calls
	 * because this class is not willing to cross threads in order to get the
	 * results of those hook methods.
	 */
	...
	void addEventListenerWithoutOnDisconnect(PeerEventListener listener,
			Executor executor) {
		eventListeners.add(new PeerListenerRegistration(listener, executor,
				false));
	}
	...
	public void connectionOpened() {
		...
		log.info("Announcing to {} as: {}",
				address == null ? "Peer" : address.toSocketAddress(),
	...
	protected void processMessage(Message m) throws Exception {
		...
		if (m instanceof NotFoundMessage) {
			...
			if (vPeerVersionMessage.clientVersion < version) {
				log.warn(
						"Connected to a peer speaking protocol version {} but need {}, closing",
	...
	private void processVersionMessage(VersionMessage m)
		...
		log.info(
				"Connected to {}: version={}, subVer='{}', services=0x{}, time={}, blocks={}",
				peerAddress == null ? "Peer" : peerAddress.getAddr()
						.getHostAddress(), peerVersion,
				vPeerVersionMessage.subVer, vPeerVersionMessage.localServices,
	...
	private void processAlert(AlertMessage m) {
		try {
			if (m.isSignatureValid()) {
				log.info("Received alert from peer {}: {}", toString(),
						m.getStatusBar());
			} else {
				log.warn(
						"Received alert with invalid signature from peer {}: {}",
						toString(), m.getStatusBar());
			}
		} catch (Throwable t) {
			...
			log.error("Failed to check signature: bug in platform libraries?",
					t);
		}
	}
			...
			for (int i = 0; i < m.getBlockHeaders().size(); i++) {
				...
				if (!passedTime && !reachedTop) {
					...
					if (blockChain.add(header)) {
						...
						throw new ProtocolException(
								"Got unconnected header from peer: "
					...
					try {
						log.info(
								"Passed the fast catchup time, discarding {} headers and requesting full blocks",
	...
	private void processGetData(GetDataMessage getdata) {
		log.info("{}: Received getdata message: {}", getAddress(),
				getdata.toString());
		ArrayList<Message> items = new ArrayList<Message>();
		for (ListenerRegistration<PeerEventListener> registration : eventListeners) {
			if (registration.executor != Threading.SAME_THREAD)
				continue;
			List<Message> listenerItems = registration.listener.getData(this,
					getdata);
		...
		log.info("{}: Sending {} items gathered from listeners to peer",
				getAddress(), items.size());
			...
			for (final Wallet wallet : wallets) {
				try {
					if (wallet.isPendingTransactionRelevant(fTx)) {
						// This transaction seems interesting to us, so let's
						// download its dependencies. This has several
						// purposes: we can check that the sender isn't
						// attacking us by engaging in protocol abuse games,
						// like depending on a time-locked transaction that will
						// never confirm, or building huge chains
						// of unconfirmed transactions (again - so they don't
						// confirm and the money can be taken
						// back with a Finney attack). Knowing the dependencies
						// also lets us store them in a serialized
						// wallet so we always have enough data to re-announce
						// to the network and get the payment into
						// the chain, in case the sender goes away and the
						// network starts to forget.
						// TODO: Not all the above things are implemented.

						Futures.addCallback(downloadDependencies(fTx),
	 ...
	 * </p>
	 */
	public ListenableFuture<List<Transaction>> downloadDependencies(
			Transaction tx) {
		checkNotNull(memoryPool,
				"Must have a configured MemoryPool object to download dependencies.");
		TransactionConfidence.ConfidenceType txConfidence = tx.getConfidence()
				.getConfidenceType();
		Preconditions
				.checkArgument(txConfidence != TransactionConfidence.ConfidenceType.BUILDING);
		log.info("{}: Downloading dependencies of {}", getAddress(),
				tx.getHashAsString());
		...
		final ListenableFuture future = downloadDependenciesInternal(tx,
				new Object(), results);
		final SettableFuture<List<Transaction>> resultFuture = SettableFuture
				.create();
	...
	private ListenableFuture<Object> downloadDependenciesInternal(
			final Transaction tx, final Object marker,
		...
		//
		// Firstly find any that are already in the memory pool so if they
		// weren't garbage collected yet, they won't
		// be deleted. Use COW sets to make unit tests deterministic and because
		// they are small. It's slower for
		...
		try {
			...
			if (needToRequest.size() > 1)
				log.info("{}: Requesting {} transactions for dep resolution",
						getAddress(), needToRequest.size());
			...
			// The transactions we already grabbed out of the mempool must still
			// be considered by the code below.
			for (Transaction dep : dependencies) {
				futures.add(Futures.immediateFuture(dep));
			}
			ListenableFuture<List<Transaction>> successful = Futures
					.successfulAsList(futures);
			Futures.addCallback(successful,
					new FutureCallback<List<Transaction>>() {
						public void onSuccess(List<Transaction> transactions) {
							// Once all transactions either were received, or we
							// know there are no more to come ...
							// Note that transactions will contain "null" for
							// any positions that weren't successful.
							List<ListenableFuture<Object>> childFutures = Lists
									.newLinkedList();
							for (Transaction tx : transactions) {
								if (tx == null)
									continue;
								log.info("{}: Downloaded dependency of {}: {}",
										getAddress(), rootTxHash,
										tx.getHashAsString());
								results.add(tx);
								// Now recurse into the dependencies of this
								// transaction too.
								childFutures.add(downloadDependenciesInternal(
										tx, marker, results));
							}
							if (childFutures.size() == 0) {
								// Short-circuit: we're at the bottom of this
								// part of the tree.
								resultFuture.set(marker);
							} else {
								// There are some children to download. Wait
								// until it's done (and their children and their
								// children...) to inform the caller that we're
								// finished.
								Futures.addCallback(
										Futures.successfulAsList(childFutures),
			...
			sendMessage(getdata);
			if (!isNotFoundMessageSupported()) {
				// If the peer isn't new enough to support the notfound message,
				// we use a nasty hack instead and
				// assume if we send a ping message after the getdata message,
				// it'll be processed after all answers
				// from getdata are done, so we can watch for the pong message
				// as a substitute.
				log.info("{}: Dep resolution waiting for a pong with nonce {}",
						this, nonce);
				ping(nonce).addListener(new Runnable() {
					public void run() {
						// The pong came back so clear out any transactions we
						// requested but didn't get.
						for (GetDataRequest req : getDataFutures) {
							if (req.nonce == nonce) {
								log.info("{}: Bottomed out dep tree at {}",
										this, req.hash);
								req.future.cancel(true);
								getDataFutures.remove(req);
							}
			...
			log.error("{}: Couldn't send getdata in downloadDependencies({})",
					this, tx.getHash());
	...
	private void processBlock(Block m) {
		if (log.isDebugEnabled()) {
			log.debug("{}: Received broadcast block {}", getAddress(),
					m.getHashAsString());
		}
		...
		if (!vDownloadData) {
			log.debug("{}: Received block we did not ask for: {}",
					getAddress(), m.getHashAsString());
			return;
		}
			...
			if (blockChain.add(m)) {
				...
				try {
					if (downloadBlockBodies) {
						final Block orphanRoot = checkNotNull(blockChain
								.getOrphanRoot(m.getHash()));
	...
	private void endFilteredBlock(FilteredBlock m) {
		if (log.isDebugEnabled()) {
			log.debug("{}: Received broadcast filtered block {}", getAddress(),
					m.getHash().toString());
		}
		if (!vDownloadData) {
			log.debug("{}: Received block we did not ask for: {}",
					getAddress(), m.getHash().toString());
			return;
		}
		if (blockChain == null) {
	...
	private void invokeOnBlocksDownloaded(final Block m) {
		...
		for (final ListenerRegistration<PeerEventListener> registration : eventListeners) {
			registration.executor.execute(new Runnable() {
				@Override
				public void run() {
					registration.listener.onBlocksDownloaded(Peer.this, m,
							blocksLeft);
				}
			});
		}
	...
	private void processInv(InventoryMessage inv) {
		...
		while (it.hasNext()) {
			...
			if (memoryPool == null) {
				if (downloadData) {
					// If there's no memory pool only download transactions if
					// we're configured to.
					getdata.addItem(item);
				}
			} else {
				// Only download the transaction if we are the first peer that
				// saw it be advertised. Other peers will also
				// see it be advertised in inv packets asynchronously, they
				// co-ordinate via the memory pool. We could
				// potentially download transactions faster by always asking
				// every peer for a tx when advertised, as remote
				// peers run at different speeds. However to conserve bandwidth
				// on mobile devices we try to only download a
				// transaction once. This means we can miss broadcasts if the
				// peer disconnects between sending us an inv and
				// sending us the transaction: currently we'll never try to
				// re-fetch after a timeout.
				...
				for (InventoryItem item : blocks) {
					if (blockChain.isOrphan(item.hash) && downloadBlockBodies) {
						...
						if (!pendingBlockDownloads.contains(item.hash)) {
							if (vPeerVersionMessage.isBloomFilteringSupported()
									&& useFilteredBlocks) {
								getdata.addItem(new InventoryItem(
										InventoryItem.Type.FilteredBlock,
										item.hash));
	 ...
	 * need.
	 */
	public ListenableFuture<Transaction> getPeerMempoolTransaction(
			Sha256Hash hash) {
		// This does not need to be locked.
	...
	public void setDownloadParameters(long secondsSinceEpoch,
		...
		try {
			...
			if (secondsSinceEpoch == 0) {
				...
				if (fastCatchupTimeSecs > blockChain.getChainHead().getHeader()
						.getTimeSeconds()) {
	...
	private void blockChainDownloadLocked(Sha256Hash toHash) {
		...
		// 50 block headers. If there is a re-org deeper than that, we'll end up
		...
		if (Objects.equal(lastGetBlocksBegin, chainHeadHash)
				&& Objects.equal(lastGetBlocksEnd, toHash)) {
			log.info(
					"blockChainDownloadLocked({}): ignoring duplicated request",
					toHash.toString());
			return;
		}
		log.debug("{}: blockChainDownloadLocked({}) current head = {}",
				toString(), toHash.toString(), chainHead.getHeader()
						.getHashAsString());
		...
		if (downloadBlockBodies) {
			GetBlocksMessage message = new GetBlocksMessage(params,
					blockLocator, toHash);
			sendMessage(message);
		} else {
			// Downloading headers for a while instead of full blocks.
			GetHeadersMessage message = new GetHeadersMessage(params,
					blockLocator, toHash);
			sendMessage(message);
		}
	}
		...
		if (blocksLeft >= 0) {
			for (final ListenerRegistration<PeerEventListener> registration : eventListeners) {
				registration.executor.execute(new Runnable() {
					@Override
					public void run() {
						registration.listener.onChainDownloadStarted(Peer.this,
								blocksLeft);
					}
				});
			}
	...
	private void addPingTimeData(long sample) {
		...
		try {
			if (lastPingTimes == null) {
				...
				System.arraycopy(lastPingTimes, 1, lastPingTimes, 0,
						lastPingTimes.length - 1);
	 ...
	 * {@link com.google.bitcoin.core.Peer#getLastPingTime()} is updated.
	 * 
	 * @throws ProtocolException
	...
	protected ListenableFuture<Long> ping(long nonce) throws ProtocolException {
		...
		if (!ver.isPingPongSupported())
			throw new ProtocolException(
					"Peer version is too low for measurable pings: " + ver);
	 ...
	 * {@link com.google.bitcoin.core.Peer#ping()} has never been called or we
	 * did not hear back the "pong" message yet, returns {@link Long#MAX_VALUE}.
	 */
	...
	public int getPeerBlockHeightDifference() {
		...
		checkState(params.allowEmptyPeerChain() || chainHeight > 0,
				"Connected to peer with zero/negative chain height",
				chainHeight);
	...
	public boolean setMinProtocolVersion(int minProtocolVersion) {
		...
		if (getVersionMessage().clientVersion < minProtocolVersion) {
			log.warn("{}: Disconnecting due to new min protocol version {}",
	...
	public void setBloomFilter(BloomFilter filter) {
		...
		log.info("{}: Sending Bloom filter{}", this,
				shouldQueryMemPool ? " and querying mempool" : "");
		sendMessage(filter);
