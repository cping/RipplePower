...
public class TransactionInput extends ChildMessage implements Serializable {
	...
	// Allows for altering transactions after they were broadcast. Tx
	// replacement is currently disabled in the C++
	// client so this is always the UINT_MAX.
	// TODO: Document this in more detail and build features that use it.
	private long sequence;
	// Data needed to connect to the output of the transaction we're gathering
	// coins from.
	private TransactionOutPoint outpoint;
	// The "script bytes" might not actually be a script. In coinbase
	// transactions where new coins are minted there
	// is no input transaction, so instead the scriptBytes contains some extra
	// stuff (like a rollover nonce) that we
	// don't care about much. The bytes are turned into a Script object (cached
	// below) on demand via a getter.
	private byte[] scriptBytes;
	// The Script object obtained from parsing scriptBytes. Only filled in on
	// demand and if the transaction is not
	// coinbase.
	...
	public TransactionInput(NetworkParameters params,
		...
		this.outpoint = new TransactionOutPoint(params, NO_SEQUENCE,
				(Transaction) null);
		this.sequence = NO_SEQUENCE;
		this.parentTransaction = parentTransaction;
		length = 40 + (scriptBytes == null ? 1 : VarInt
				.sizeOf(scriptBytes.length) + scriptBytes.length);
	}

	public TransactionInput(NetworkParameters params,
			@Nullable Transaction parentTransaction, byte[] scriptBytes,
	...
	TransactionInput(NetworkParameters params, Transaction parentTransaction,
		...
		outpoint = new TransactionOutPoint(params, outputIndex,
				output.parentTransaction);
	 ...
	 *            serialized again in the future. as the length will be provided
	 *            as part of the header. If unknown then set to
	 *            Message.UNKNOWN_LENGTH
	 * @throws ProtocolException
	 */
	public TransactionInput(NetworkParameters params,
			Transaction parentTransaction, byte[] msg, int offset,
	...
	void parse() throws ProtocolException {
		outpoint = new TransactionOutPoint(params, bytes, cursor, this,
				parseLazy, parseRetain);
	...
	public boolean isCoinBase() {
		...
		return outpoint.getHash().equals(Sha256Hash.ZERO_HASH)
				&& (outpoint.getIndex() & 0xFFFFFFFFL) == 0xFFFFFFFFL; // -1 but
																		// all
																		// is
																		// serialized
																		// to
	 ...
	 * the scriptSig. The concept of a "from address" is not well defined in
	 * Bitcoin and you should not assume that senders of a transaction can
	 * actually receive coins on the same address they used to sign (e.g. this
	 * is not true for shared wallets).
	 */
	@Deprecated
	 ...
	 * this feature to build contract protocols. Note that as of 2012 the tx
	 * replacement feature is disabled so sequence numbers are unusable.
	 */
	 ...
	 *         structure. This contains the data needed to connect to the output
	 *         of the transaction we're gathering coins from.
	 */
	...
	void setScriptBytes(byte[] scriptBytes) {
		...
		int newLength = 40 + (scriptBytes == null ? 1 : VarInt
				.sizeOf(scriptBytes.length) + scriptBytes.length);
	 ...
	 * 
	 * @param transactions
	 *            Map of txhash->transaction.
	 * @param mode
	 *            Whether to abort if there's a pre-existing connection or not.
	 * @return NO_SUCH_TX if the prevtx wasn't found, ALREADY_SPENT if there was
	 *         a conflict, SUCCESS if not.
	 */
	public ConnectionResult connect(Map<Sha256Hash, Transaction> transactions,
			ConnectMode mode) {
		Transaction tx = transactions.get(outpoint.getHash());
		if (tx == null) {
	...
	public ConnectionResult connect(Transaction transaction, ConnectMode mode) {
		...
		checkElementIndex((int) outpoint.getIndex(), transaction.getOutputs()
				.size(), "Corrupt transaction");
		TransactionOutput out = transaction
				.getOutput((int) outpoint.getIndex());
	...
	/**
	 * For a connected transaction, runs the script against the connected pubkey
	 * and verifies they are correct.
	 * 
	 * @throws ScriptException
	 *             if the script did not verify.
	 ...
	 * 
	 * @param output
	 *            the output that this input is supposed to spend.
	 * @throws ScriptException
	 *             If the script doesn't verify.
	 * @throws VerificationException
	 *             If the outpoint doesn't match the given output.
	 */
	public void verify(TransactionOutput output) throws VerificationException {
		...
		getScriptSig()
				.correctlySpends(parentTransaction, myIndex, pubKey, true);
	}
