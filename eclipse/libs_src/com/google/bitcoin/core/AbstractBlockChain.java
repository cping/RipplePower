 ...
 * together, and knows how to verify that the chain follows the rules of the
 * {@link NetworkParameters} for this chain.
 * </p>
 * 
 * <p>
 * It can be connected to a {@link Wallet}, and also {@link BlockChainListener}s
 * that can receive transactions and notifications of re-organizations.
 * </p>
 * 
 * <p>
 * An AbstractBlockChain implementation must be connected to a
 * {@link BlockStore} implementation. The chain object by itself doesn't store
 ...
 * block chains need fully validating stores. In the lightweight SPV mode, a
 * {@link com.google.bitcoin.store.SPVBlockStore} is the right choice.
 * </p>
 * 
 * <p>
 * This class implements an abstract class which makes it simple to create a
 * BlockChain that does/doesn't do full verification. It verifies headers and is
 * implements most of what is required to implement SPV mode, but also provides
 * callback hooks which can be used to do full verification.
 * </p>
 * 
 * <p>
 * There are two subclasses of AbstractBlockChain that are useful:
 * {@link BlockChain}, which is the simplest class and implements <i>simplified
 ...
 * {@link com.google.bitcoin.store.H2FullPrunedBlockStore} implements full
 * verification, which is equivalent to the original Satoshi client. To learn
 * more about the alternative security models, please consult the articles on
 * the website.
 * </p>
 * 
 * <b>Theory</b>
 ...
 * block within a few seconds of each other, or it can happen when the chain is
 * under attack.
 * </p>
 * 
 * <p>
 ...
 * then we say this is a full chain. If you cannot reach the genesis block we
 * say it is an orphan chain. Orphan chains can occur when blocks are solved and
 * received during the initial block chain download, or if we connect to a peer
 * that doesn't send us blocks in order.
 * </p>
 * 
 * <p>
 * A reorganize occurs when the blocks that make up the best known chain
 * changes. Note that simply adding a new block to the top of the best chain
 ...
 * mean the chain representing the largest amount of work done.
 * </p>
 * 
 * <p>
...
public abstract class AbstractBlockChain {
	private static final Logger log = LoggerFactory
			.getLogger(AbstractBlockChain.class);
	...
	// objects.
	// The chainHead field is read/written synchronized with this object rather
	// than BlockChain. However writing is
	...
	class OrphanBlock {
		...
		OrphanBlock(Block block, @Nullable List<Sha256Hash> filteredTxHashes,
			...
			final boolean filtered = filteredTxHashes != null
					&& filteredTxn != null;
			Preconditions
	...
	public AbstractBlockChain(NetworkParameters params,
			List<BlockChainListener> listeners, BlockStore blockStore)
		...
		for (BlockChainListener l : listeners)
			addListener(l, Threading.SAME_THREAD);
	}

	/**
	 * Add a wallet to the BlockChain. Note that the wallet will be unaffected
	 * by any blocks received while it was not part of this BlockChain. This
	 * method is useful if the wallet has just been created, and its keys have
	 * never been in use, or if the wallet has been loaded along with the
	...
	public void addWallet(Wallet wallet) {
		...
		if (wallet.getLastBlockSeenHeight() != getBestChainHeight()) {
			log.warn("Wallet/chain height mismatch: {} vs {}",
					wallet.getLastBlockSeenHeight(), getBestChainHeight());
			log.warn("Hashes: {} vs {}", wallet.getLastBlockSeenHash(),
					getChainHead().getHeader().getHash());
		}
	}
	...
	public void addListener(BlockChainListener listener, Executor executor) {
		listeners.add(new ListenerRegistration<BlockChainListener>(listener,
				executor));
	}
	...
	/**
	 * Returns the {@link BlockStore} the chain was constructed with. You can
	 * use this to iterate over the chain.
	 */
	...
	/**
	 * Called before setting chain head in memory. Should write the new head to
	 * block store and then commit any database transactions that were started
	 * by disconnectTransactions/connectTransactions.
	 */
	 ...
	 * disconnectTransaction/connectTransactions, but will not be calling
	 * preSetChainHead as a block failed verification. Can be used to abort
	 * database transactions that were started by
	...
	public boolean add(Block block) throws VerificationException,
		...
		try {
			...
			// TODO: Figure out a better way to propagate this exception to the
			// user.
	...
	public boolean add(FilteredBlock block) throws VerificationException,
		...
		try {
			...
			// chain and thus haven't witnessed any
			// of the transactions.
			return add(block.getBlockHeader(), true,
	...
	protected abstract TransactionOutputChanges connectTransactions(int height,
			Block block) throws VerificationException, BlockStoreException;

	/**
	 * Load newBlock from BlockStore and connect its transactions, returning
	 * changes to the set of unspent transactions. If an error is encountered in
	 * a transaction, no changes should be made to the underlying BlockStore.
	 * Only called if(shouldVerifyTransactions())
	 * 
	 * @throws PrunedException
	 *             if newBlock does not exist as a {@link StoredUndoableBlock}
	 *             in the block store.
	 * @throws VerificationException
	 *             if an attempt was made to spend an already-spent output, or
	 *             if a transaction incorrectly solved an output script.
	 * @throws BlockStoreException
	 *             if the block store had an underlying error or newBlock does
	 *             not exist in the block store at all.
	 * @return The full set of all changes made to the set of open transaction
	 *         outputs.
	 */
	protected abstract TransactionOutputChanges connectTransactions(
	...
	private boolean add(Block block, boolean tryConnecting,
		...
		lock.lock();
		try {
			// TODO: Use read/write locks to ensure that during chain download
			// properties are still low latency.
			...
			// Quick check for duplicates to avoid an expensive check further
			// down (in findSplit). This can happen a lot
			// when connecting orphan transactions due to the dumb brute force
			// algorithm we use.
			...
			// blocks), verify that block has transactions
			if (shouldVerifyTransactions() && block.transactions == null)
				throw new VerificationException(
						"Got a block header while running in full-block mode");

			// Check for already-seen block, but only for full pruned mode,
			// where the DB is
			// more likely able to handle these queries quickly.
			if (shouldVerifyTransactions()
					&& blockStore.get(block.getHash()) != null) {
				return true;
			}

			// Does this block contain any transactions we might care about?
			// Check this up front before verifying the
			// blocks validity so we can skip the merkle root verification if
			// the contents aren't interesting. This saves
			// a lot of time for big blocks.
			boolean contentsImportant = shouldVerifyTransactions();
			if (block.transactions != null) {
				contentsImportant = contentsImportant
						|| containsRelevantTransactions(block);
			}

			// Prove the block is internally valid: hash is lower than target,
			// etc. This only checks the block contents
			// if there is a tx sending or receiving coins using an address in
			// one of our wallets. And those transactions
			// are only lightly verified: presence in a valid connecting block
			// is taken as proof of validity. See the
			...
			StoredBlock storedPrev = getStoredBlockInCurrentScope(block
					.getPrevBlockHash());

			if (storedPrev == null) {
				// We can't find the previous block. Probably we are still in
				// the process of downloading the chain and a
				// block was solved whilst we were doing it. We put it to one
				// side and try to connect it later when we
				// have more blocks.
				checkState(tryConnecting, "bug in tryConnectingOrphans");
				log.warn("Block does not connect: {} prev {}",
						block.getHashAsString(), block.getPrevBlockHash());
				orphanBlocks.put(block.getHash(), new OrphanBlock(block,
						filteredTxHashList, filteredTxn));
				return false;
			} else {
	...
	private void connectBlock(final Block block, StoredBlock storedPrev,
		...
		if (!params.passesCheckpoint(storedPrev.getHeight() + 1,
				block.getHash()))
			throw new VerificationException(
					"Block failed checkpoint lockin at "
							+ (storedPrev.getHeight() + 1));
		if (shouldVerifyTransactions()) {
			checkNotNull(block.transactions);
			for (Transaction tx : block.transactions)
				if (!tx.isFinal(storedPrev.getHeight() + 1,
		...
		if (storedPrev.equals(head)) {
			if (filtered && filteredTxn.size() > 0) {
				log.debug(
						...
						block.getHashAsString(), filteredTxHashList.size(),
						filteredTxn.size());
			...
			if (shouldVerifyTransactions())
				txOutChanges = connectTransactions(storedPrev.getHeight() + 1,
						block);
			StoredBlock newStoredBlock = addToBlockStore(storedPrev,
					block.transactions == null ? block : block.cloneAsHeader(),
					txOutChanges);
			setChainHead(newStoredBlock);
			log.debug("Chain is now {} blocks high, running listeners",
					newStoredBlock.getHeight());
			informListenersForNewBlock(block, NewBlockType.BEST_CHAIN,
					filteredTxHashList, filteredTxn, newStoredBlock);
		} else {
			// This block connects to somewhere other than the top of the best
			// known chain. We treat these differently.
			//
			// Note that we send the transactions to the wallet FIRST, even if
			// we're about to re-organize this block
			// to become the new best chain head. This simplifies handling of
			// the re-org in the Wallet class.
			...
			if (haveNewBestChain) {
				...
				if (splitPoint != null && splitPoint.equals(newBlock)) {
					// newStoredBlock is a part of the same chain, there's no
					// fork. This happens when we receive a block
					// that we already saw and linked into the chain previously,
					// which isn't the chain head.
					// Re-processing it is confusing for the wallet so just
					// skip.
					log.warn(
							"Saw duplicated block in main chain at height {}: {}",
							newBlock.getHeight(), newBlock.getHeader()
									.getHash());
					return;
				}
				if (splitPoint == null) {
					// This should absolutely never happen
					// (lets not write the full block to disk to keep any bugs
					// which allow this to happen
					// from writing unreasonable amounts of data to disk)
					throw new VerificationException(
							"Block forks the chain but splitPoint is null");
				} else {
					// We aren't actually spending any transactions (yet)
					// because we are on a fork
					addToBlockStore(storedPrev, block);
					int splitPointHeight = splitPoint.getHeight();
					String splitPointHash = splitPoint.getHeader()
							.getHashAsString();
					log.info(
							"Block forks the chain at height {}/block {}, but it did not cause a reorganize:\n{}",
							splitPointHeight, splitPointHash, newBlock
									.getHeader().getHashAsString());
				}
			}

			// We may not have any transactions if we received only a header,
			// which can happen during fast catchup.
			// If we do, send them to the wallet but state that they are on a
			// side chain so it knows not to try and
			// spend them until they become activated.
			if (block.transactions != null || filtered) {
				informListenersForNewBlock(block, NewBlockType.SIDE_CHAIN,
						filteredTxHashList, filteredTxn, newBlock);
			}
	...
	private void informListenersForNewBlock(final Block block,
			final NewBlockType newBlockType,
			@Nullable final List<Sha256Hash> filteredTxHashList,
			@Nullable final Map<Sha256Hash, Transaction> filteredTxn,
			final StoredBlock newStoredBlock) throws VerificationException {
		// Notify the listeners of the new block, so the depth and workDone of
		// stored transactions can be updated
		// (in the case of the listener being a wallet). Wallets need to know
		// how deep each transaction is so
		// coinbases aren't used before maturity.
		boolean first = true;
		Set<Transaction> falsePositives = Sets.newHashSet();
		if (filteredTxn != null)
			falsePositives.addAll(filteredTxn.values());
		for (final ListenerRegistration<BlockChainListener> registration : listeners) {
			if (registration.executor == Threading.SAME_THREAD) {
				informListenerForNewTransactions(block, newBlockType,
						filteredTxHashList, filteredTxn, newStoredBlock, first,
						registration.listener, falsePositives);
				if (newBlockType == NewBlockType.BEST_CHAIN)
					registration.listener.notifyNewBestBlock(newStoredBlock);
			} else {
				// Listener wants to be run on some other thread, so marshal it
				// across here.
				...
				registration.executor.execute(new Runnable() {
					...
					public void run() {
						try {
							// We can't do false-positive handling when
							// executing on another thread
							Set<Transaction> ignoredFalsePositives = Sets
									.newHashSet();
							informListenerForNewTransactions(block,
									newBlockType, filteredTxHashList,
									filteredTxn, newStoredBlock, notFirst,
									registration.listener,
									ignoredFalsePositives);
							if (newBlockType == NewBlockType.BEST_CHAIN)
								registration.listener
										.notifyNewBestBlock(newStoredBlock);
						} catch (VerificationException e) {
							log.error("Block chain listener threw exception: ",
									e);
							// Don't attempt to relay this back to the original
							// peer thread if this was an async
							// listener invocation.
							// TODO: Make exception reporting a global feature
							// and use it here.
						}
					}
				});
	...
	private static void informListenerForNewTransactions(Block block,
			NewBlockType newBlockType,
			@Nullable List<Sha256Hash> filteredTxHashList,
			@Nullable Map<Sha256Hash, Transaction> filteredTxn,
			StoredBlock newStoredBlock, boolean first,
			BlockChainListener listener, Set<Transaction> falsePositives)
			throws VerificationException {
		if (block.transactions != null) {
			// If this is not the first wallet, ask for the transactions to be
			// duplicated before being given
			// to the wallet when relevant. This ensures that if we have two
			// connected wallets and a tx that
			// is relevant to both of them, they don't end up accidentally
			// sharing the same object (which can
			// result in temporary in-memory corruption during re-orgs). See bug
			// 257. We only duplicate in
			// the case of multiple wallets to avoid an unnecessary efficiency
			// hit in the common case.
			sendTransactionsToListener(newStoredBlock, newBlockType, listener,
					0, block.transactions, !first, falsePositives);
		} else if (filteredTxHashList != null) {
			checkNotNull(filteredTxn);
			// We must send transactions to listeners in the order they appeared
			// in the block - thus we iterate over the
			// set of hashes and call sendTransactionsToListener with individual
			// txn when they have not already been
			// seen in loose broadcasts - otherwise notifyTransactionIsInBlock
			// on the hash.
			int relativityOffset = 0;
			for (Sha256Hash hash : filteredTxHashList) {
				Transaction tx = filteredTxn.get(hash);
				if (tx != null)
					sendTransactionsToListener(newStoredBlock, newBlockType,
							listener, relativityOffset, Arrays.asList(tx),
							!first, falsePositives);
				else
					listener.notifyTransactionIsInBlock(hash, newStoredBlock,
							newBlockType, relativityOffset);
				relativityOffset++;
			}
		}
	}
	 ...
	 */
	private static long getMedianTimestampOfRecentBlocks(
			StoredBlock storedBlock, BlockStore store)
			throws BlockStoreException {
		long[] timestamps = new long[11];
		int unused = 9;
		timestamps[10] = storedBlock.getHeader().getTimeSeconds();
		while (unused >= 0
				&& (storedBlock = storedBlock.getPrev(store)) != null)
	 ...
	 * 
	 * if (shouldVerifyTransactions) Either newChainHead needs to be in the
	 * block store as a FullStoredBlock, or (block != null && block.transactions
	 * != null)
	 */
	private void handleNewBestChain(StoredBlock storedPrev,
			StoredBlock newChainHead, Block block, boolean expensiveChecks)
			throws BlockStoreException, VerificationException, PrunedException {
		checkState(lock.isHeldByCurrentThread());
		...
		log.info("New chain head: {}", newChainHead.getHeader()
				.getHashAsString());
		log.info("Split at block: {}", splitPoint.getHeader().getHashAsString());
		// Then build a list of all blocks in the old part of the chain and the
		// new part.
		final LinkedList<StoredBlock> oldBlocks = getPartialChain(head,
				splitPoint, blockStore);
		final LinkedList<StoredBlock> newBlocks = getPartialChain(newChainHead,
				splitPoint, blockStore);
		// Disconnect each transaction in the previous main chain that is no
		// longer in the new main chain
		...
		if (shouldVerifyTransactions()) {
			for (StoredBlock oldBlock : oldBlocks) {
				try {
					...
					// We threw away the data we need to re-org this deep! We
					// need to go back to a peer with full
					// block contents and ask them for the relevant data then
					// rebuild the indexs. Or we could just
					// give up and ask the human operator to help get us unstuck
					// (eg, rescan from the genesis block).
					// TODO: Retry adding this block when we get a block with
					// hash e.getHash()
					throw e;
				}
			}
			...
			for (Iterator<StoredBlock> it = newBlocks.descendingIterator(); it
					.hasNext();) {
				cursor = it.next();
				if (expensiveChecks
						&& cursor.getHeader().getTimeSeconds() <= getMedianTimestampOfRecentBlocks(
								cursor.getPrev(blockStore), blockStore))
					throw new VerificationException(
				...
				if (cursor != newChainHead || block == null)
					...
					txOutChanges = connectTransactions(
							newChainHead.getHeight(), block);
				storedNewHead = addToBlockStore(storedNewHead,
						cursor.getHeader(), txOutChanges);
			}
		} else {
			// (Finally) write block to block store
			storedNewHead = addToBlockStore(storedPrev,
					newChainHead.getHeader());
		}
		// Now inform the listeners. This is necessary so the set of currently
		// active transactions (that we can spend)
		// can be updated to take into account the re-organize. We might also
		// have received new coins we didn't have
		// before and our previous spends might have been undone.
		for (final ListenerRegistration<BlockChainListener> registration : listeners) {
			if (registration.executor == Threading.SAME_THREAD) {
				// Short circuit the executor so we can propagate any
				// exceptions.
				// TODO: Do we really need to do this or should it be
				// irrelevant?
				registration.listener.reorganize(splitPoint, oldBlocks,
						newBlocks);
			} else {
				registration.executor.execute(new Runnable() {
					@Override
					public void run() {
						try {
							registration.listener.reorganize(splitPoint,
									oldBlocks, newBlocks);
						} catch (VerificationException e) {
							log.error(
	 ...
	 */
	private static LinkedList<StoredBlock> getPartialChain(StoredBlock higher,
		...
		while (true) {
			results.add(cursor);
			cursor = checkNotNull(cursor.getPrev(store),
	 ...
	 * diverge. Returns null if no split point was found (ie they are not part
	 * of the same chain). Returns newChainHead or chainHead if they don't
	 * actually diverge but are part of the same chain.
	 */
	private static StoredBlock findSplit(StoredBlock newChainHead,
			StoredBlock oldChainHead, BlockStore store)
			throws BlockStoreException {
	...
	private static void sendTransactionsToListener(StoredBlock block,
			NewBlockType blockType, BlockChainListener listener,
			int relativityOffset, List<Transaction> transactions,
			boolean clone, Set<Transaction> falsePositives)
			throws VerificationException {
		for (Transaction tx : transactions) {
			try {
				...
				// We don't want scripts we don't understand to break the block
				// chain so just note that this tx was
	 ...
	 */
	private void tryConnectingOrphans() throws VerificationException,
			BlockStoreException, PrunedException {
		checkState(lock.isHeldByCurrentThread());
		...
		do {
			...
			while (iter.hasNext()) {
				...
				StoredBlock prev = getStoredBlockInCurrentScope(orphanBlock.block
						.getPrevBlockHash());
				if (prev == null) {
				...
				add(orphanBlock.block, false, orphanBlock.filteredTxHashes,
						orphanBlock.filteredTxn);
				iter.remove();
				blocksConnectedThisRound++;
			}
			if (blocksConnectedThisRound > 0) {
				log.info("Connected {} orphan blocks.",
						blocksConnectedThisRound);
			}
	...
	private void checkDifficultyTransitions(StoredBlock storedPrev,
			Block nextBlock) throws BlockStoreException, VerificationException {
		checkState(lock.isHeldByCurrentThread());
		...
		if ((storedPrev.getHeight() + 1) % params.getInterval() != 0) {
			...
			// TODO: Refactor this hack after 0.5 is released and we stop
			// supporting deserialization compatibility.
			// This should be a method of the NetworkParameters, which should in
			// turn be using singletons and a subclass
			// for each network type. Then each network can define its own
			// difficulty transition rules.
			if (params.getId().equals(NetworkParameters.ID_TESTNET)
					&& nextBlock.getTime().after(testnetDiffDate)) {
			...
			if (nextBlock.getDifficultyTarget() != prev.getDifficultyTarget())
				throw new VerificationException(
						"Unexpected change in difficulty at height "
								...
								+ ": "
								+ Long.toHexString(nextBlock
										.getDifficultyTarget()) + " vs "
								+ Long.toHexString(prev.getDifficultyTarget()));
			return;
		}

		// We need to find a block far back in the chain. It's OK that this is
		// expensive because it only occurs every
		...
		int timespan = (int) (prev.getTimeSeconds() - blockIntervalAgo
				.getTimeSeconds());
		...
		BigInteger newDifficulty = Utils.decodeCompactBits(prev
				.getDifficultyTarget());
		...
		if (newDifficulty.compareTo(receivedDifficulty) != 0)
			throw new VerificationException(
					"Network provided difficulty bits do not match what was calculated: "
							+ receivedDifficulty.toString(16) + " vs "
							+ newDifficulty.toString(16));
	}

	private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev,
			Block next) throws VerificationException, BlockStoreException {
		checkState(lock.isHeldByCurrentThread());
		// After 15th February 2012 the rules on the testnet change to avoid
		// people running up the difficulty
		// and then leaving, making it too hard to mine a block. On
		// non-difficulty transition points, easy
		// blocks are allowed if there has been a span of 20 minutes without
		// one.
		final long timeDelta = next.getTimeSeconds() - prev.getTimeSeconds();
		// There is an integer underflow bug in bitcoin-qt that means mindiff
		// blocks are accepted when time
		// goes backwards.
		if (timeDelta >= 0 && timeDelta <= NetworkParameters.TARGET_SPACING * 2) {
			// Walk backwards until we find a block that doesn't have the
			// easiest proof of work, then check
			...
			while (!cursor.getHeader().equals(params.getGenesisBlock())
					...
					&& cursor.getHeader().getDifficultyTargetAsInteger()
							.equals(params.getProofOfWorkLimit()))
				cursor = cursor.getPrev(blockStore);
			BigInteger cursorDifficulty = cursor.getHeader()
					.getDifficultyTargetAsInteger();
			BigInteger newDifficulty = next.getDifficultyTargetAsInteger();
			if (!cursorDifficulty.equals(newDifficulty))
				throw new VerificationException(
						"Testnet block transition that is not allowed: "
								+ Long.toHexString(cursor.getHeader()
										.getDifficultyTarget()) + " vs "
	...
	/**
	 * Returns true if any connected wallet considers any transaction in the
	 * block to be relevant.
	 */
	 ...
	 * can't find its parent, therefore it's an orphan). Typically this occurs
	 * when we are downloading the chain and didn't reach the head yet, and/or
	 * if a block is solved whilst we are downloading. It's possible that we see
	 * a small amount of orphan blocks which chain together, this method tries
	 * walking backwards through the known orphan blocks to find the
	 * bottom-most.
	 * 
	 * @return from or one of froms parents, or null if "from" does not identify
	 *         an orphan block
	 */
	@Nullable
	 ...
	 * perfect 10 minute average for each block. This is useful for turning
	 * transaction lock times into human readable times. Note that a height in
	 * the past will still be estimated, even though the time of solving is
	 * actually known (we won't scan backwards through the chain to obtain the
	 * right answer).
	 */
	...
	/**
	 * Returns a future that completes when the block chain has reached the
	 * given height. Yields the {@link StoredBlock} of the block that reaches
	 * that height first. The future completes on a peer thread.
	 */
	...
	protected void trackFilteredTransactions(int count) {
		...
		falsePositiveTrend = FP_ESTIMATOR_BETA * count
				* (falsePositiveRate - previousFalsePositiveRate) + betaDecay
				* falsePositiveTrend;
	...
	void trackFalsePositives(int count) {
		...
		if (count > 0)
			log.debug("{} false positives, current rate = {} trend = {}",
					count, falsePositiveRate, falsePositiveTrend);
	}
