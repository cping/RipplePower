 ...
 * features. Read <a
 * href="http://code.google.com/p/bitcoinj/wiki/WorkingWithTransactions"
 * >"Working with transactions"</a> in the documentation to learn more about how
 * to use this class.
 * </p>
 * 
 * <p>
...
public class Transaction extends ChildMessage implements Serializable {
	...
	/**
	 * If fee is lower than this value (in satoshis), a default reference client
	 * will treat it as if there were no fee. Currently this is 10000 satoshis.
	 */
	public static final BigInteger REFERENCE_DEFAULT_MIN_TX_FEE = BigInteger
			.valueOf(10000);
	 ...
	 * formula used in {@link TransactionOutput#getMinNonDustValue(BigInteger)}.
	 * Currently it's 5460 satoshis.
	 */
	public static final BigInteger MIN_NONDUST_OUTPUT = BigInteger
			.valueOf(5460);
	...
	public enum Purpose {
		...
		// In future: de/refragmentation, privacy boosting/mixing,
		// child-pays-for-parent fees, etc.
	}
	 ...
	 * @throws ProtocolException
	 */
	public Transaction(NetworkParameters params, byte[] msg, int offset,
	 ...
	 * Length of a transaction is fixed.
	 */
	public Transaction(NetworkParameters params, byte[] msg,
			@Nullable Message parent, boolean parseLazy, boolean parseRetain,
			int length) throws ProtocolException {
	...
	public Map<Sha256Hash, Integer> getAppearsInHashes() {
		return appearsInHashes != null ? ImmutableMap.copyOf(appearsInHashes)
				: null;
	}
	...
	/**
	 * <p>
	 * Puts the given block in the internal set of blocks in which this
	 * transaction appears. This is used by the wallet to ensure transactions
	 * that appear on side chains are recorded properly even though the block
	 * stores do not save the transaction data at all.
	 * </p>
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * Sets updatedAt to be the earliest valid block time where this tx was
	 * seen.
	 * </p>
	 * 
	 * @param block
	 ...
	 *            A number that disambiguates the order of transactions within a
	 *            block.
	 */
	public void setBlockAppearance(StoredBlock block, boolean bestChain,
		...
		if (bestChain
				&& (updatedAt == null || updatedAt.getTime() == 0 || updatedAt
						.getTime() > blockTime)) {
	...
	public BigInteger getValueSentFromMe(Wallet wallet) throws ScriptException {
		...
		for (TransactionInput input : inputs) {
			...
			TransactionOutput connected = input
					.getConnectedOutput(wallet.unspent);
			if (connected == null)
	...
	/**
	 * Returns the difference of {@link Transaction#getValueSentFromMe(Wallet)}
	 * and {@link Transaction#getValueSentToMe(Wallet)}.
	 */
	...
	void parse() throws ProtocolException {
		...
		for (long i = 0; i < numInputs; i++) {
			TransactionInput input = new TransactionInput(params, this, bytes,
					cursor, parseLazy, parseRetain);
			inputs.add(input);
			long scriptLen = readVarInt(TransactionOutPoint.MESSAGE_LENGTH);
			optimalEncodingMessageSize += TransactionOutPoint.MESSAGE_LENGTH
					+ VarInt.sizeOf(scriptLen) + scriptLen + 4;
			cursor += scriptLen + 4;
		}
		// Now the outputs
		...
		for (long i = 0; i < numOutputs; i++) {
			TransactionOutput output = new TransactionOutput(params, this,
					bytes, cursor, parseLazy, parseRetain);
			outputs.add(output);
			long scriptLen = readVarInt(8);
			optimalEncodingMessageSize += 8 + VarInt.sizeOf(scriptLen)
					+ scriptLen;
			cursor += scriptLen;
		}
		lockTime = readUint32();
	...
	public boolean isMature() {
		...
		return getConfidence().getDepthInBlocks() >= params
	...
	public String toString(@Nullable AbstractBlockChain chain) {
		...
		s.append(String
				.format("  %s: %s%n", getHashAsString(), getConfidence()));
		if (isTimeLocked()) {
			String time;
			if (lockTime < LOCKTIME_THRESHOLD) {
				...
				if (chain != null) {
					time = time
							+ " (estimated to be reached at "
							+ chain.estimateBlockTime((int) lockTime)
									.toString() + ")";
				}
			} else {
		...
		if (isCoinBase()) {
			...
			s.append("     == COINBASE TXN (scriptSig " + script
		...
		for (TransactionOutput out : outputs) {
			...
			try {
				...
				if (out.getSpentBy() != null) {
					s.append(" by ");
					s.append(out.getSpentBy().getParentTransaction()
							.getHashAsString());
				}
			} catch (Exception e) {
				s.append("[exception: ").append(e.getMessage()).append("]");
	 ...
	 * inputs off. Otherwise it won't be accepted by the network. Returns the
	 * newly created input.
	 */
	...
	/**
	 * Adds an input directly, with no checking that it's valid. Returns the new
	 * input.
	 */
	...
	public TransactionInput addSignedInput(TransactionOutPoint prevOut,
			Script scriptPubKey, ECKey sigKey, SigHash sigHash,
			boolean anyoneCanPay) throws ScriptException {
		TransactionInput input = new TransactionInput(params, this,
				new byte[] {}, prevOut);
		addInput(input);
		Sha256Hash hash = hashForSignature(inputs.size() - 1, scriptPubKey,
				sigHash, anyoneCanPay);
		ECKey.ECDSASignature ecSig = sigKey.sign(hash);
		TransactionSignature txSig = new TransactionSignature(ecSig, sigHash,
				anyoneCanPay);
	...
	/**
	 * Same as
	 * {@link #addSignedInput(TransactionOutPoint, com.google.bitcoin.script.Script, ECKey, com.google.bitcoin.core.Transaction.SigHash, boolean)}
	 * but defaults to {@link SigHash#ALL} and "false" for the anyoneCanPay
	 * flag. This is normally what you want.
	 */
	public TransactionInput addSignedInput(TransactionOutPoint prevOut,
			Script scriptPubKey, ECKey sigKey) throws ScriptException {
	...
	public TransactionOutput addOutput(BigInteger value, Script script) {
		return addOutput(new TransactionOutput(params, this, value,
				script.getProgram()));
	}
	 ...
	 *            A wallet is required to fetch the keys needed for signing.
	 * @param aesKey
	 *            The AES key to use to decrypt the key before signing. Null if
	 *            no decryption is required.
	 */
	public synchronized void signInputs(SigHash hashType, Wallet wallet,
			@Nullable KeyParameter aesKey) throws ScriptException {
		...
		TransactionSignature[] signatures = new TransactionSignature[inputs
				.size()];
		ECKey[] signingKeys = new ECKey[inputs.size()];
		for (int i = 0; i < inputs.size(); i++) {
			TransactionInput input = inputs.get(i);
			// We don't have the connected output, we assume it was signed
			// already and move on
			if (input.getOutpoint().getConnectedOutput() == null) {
				log.warn(
						"Missing connected output, assuming input {} is already signed.",
						i);
				continue;
			}
			try {
			...
			byte[] connectedPubKeyScript = input.getOutpoint()
					.getConnectedPubKeyScript();
			if (key.hasPrivKey() || key.isEncrypted()) {
				signatures[i] = calculateSignature(i, key, aesKey,
						connectedPubKeyScript, hashType, anyoneCanPay);
			} else {
				// Create a dummy signature to ensure the transaction is of the
				// correct size when we try to ensure
				// the right fee-per-kb is attached. If the wallet doesn't have
				// the privkey, the user is assumed to
				// be doing something special and that they will replace the
				// dummy signature with a real one later.
		...
		for (int i = 0; i < inputs.size(); i++) {
			if (signatures[i] == null)
				continue;
			TransactionInput input = inputs.get(i);
			final TransactionOutput connectedOutput = input.getOutpoint()
					.getConnectedOutput();
			checkNotNull(connectedOutput); // Quiet static analysis: is never
											// null here but cannot be
											// statically proven
			Script scriptPubKey = connectedOutput.getScriptPubKey();
			if (scriptPubKey.isSentToAddress()) {
				input.setScriptSig(ScriptBuilder.createInputScript(
						signatures[i], signingKeys[i]));
			} else if (scriptPubKey.isSentToRawPubKey()) {
				input.setScriptSig(ScriptBuilder
						.createInputScript(signatures[i]));
			} else {
				// Should be unreachable - if we don't recognize the type of
				// script we're trying to sign for, we should
				// have failed above when fetching the key to sign with.
				throw new RuntimeException("Do not understand script type: "
						+ scriptPubKey);
			}
		}
	 ...
	 * {@link Transaction#hashForSignature(int, byte[], com.google.bitcoin.core.Transaction.SigHash, boolean)}
	 * followed by
	 * {@link ECKey#sign(Sha256Hash, org.spongycastle.crypto.params.KeyParameter)}
	 * and then returning a new {@link TransactionSignature}.
	 * 
	 * @param inputIndex
	 ...
	 *         sighash components.
	 */
	public synchronized TransactionSignature calculateSignature(int inputIndex,
			ECKey key, @Nullable KeyParameter aesKey,
			byte[] connectedPubKeyScript, SigHash hashType, boolean anyoneCanPay) {
		Sha256Hash hash = hashForSignature(inputIndex, connectedPubKeyScript,
				hashType, anyoneCanPay);
		return new TransactionSignature(key.sign(hash, aesKey), hashType,
				anyoneCanPay);
	}
	 ...
	 */
	public synchronized TransactionSignature calculateSignature(int inputIndex,
			ECKey key, Script connectedPubKeyScript, SigHash hashType,
			boolean anyoneCanPay) {
		Sha256Hash hash = hashForSignature(inputIndex,
				connectedPubKeyScript.getProgram(), hashType, anyoneCanPay);
	 ...
	 *            should be false.
	 */
	public synchronized Sha256Hash hashForSignature(int inputIndex,
			byte[] connectedScript, SigHash type, boolean anyoneCanPay) {
	 ...
	 */
	public synchronized Sha256Hash hashForSignature(int inputIndex,
			Script connectedScript, SigHash type, boolean anyoneCanPay) {
		int sigHash = TransactionSignature.calcSigHashValue(type, anyoneCanPay);
		return hashForSignature(inputIndex, connectedScript.getProgram(),
				(byte) sigHash);
	}
	 ...
	 */
	public synchronized Sha256Hash hashForSignature(int inputIndex,
			byte[] connectedScript, byte sigHashType) {
		...
		try {
			// Store all the input scripts and clear them in preparation for
			// signing. If we're signing a fresh
			// transaction that step isn't very helpful, but it doesn't add much
			// cost relative to the actual
			// EC math so we'll do it anyway.
			//
			// Also store the input sequence numbers in case we are clearing
			// them with SigHash.NONE/SINGLE
			...
			// reference clients bugs. OP_CODESEPARATOR
			...
			// the step has no obvious purpose as
			...
			if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
				...
				if (inputIndex >= this.outputs.size()) {
					...
					// Bitcoin implementation. The reference client also
					// contains a bug in handling this case:
					...
					//
					// TODO: Only allow this to happen if we are checking a
					// signature, not signing a transactions
					for (int i = 0; i < inputs.size(); i++) {
						inputs.get(i).setScriptBytes(inputScripts[i]);
						inputs.get(i)
								.setSequenceNumber(inputSequenceNumbers[i]);
					}
					this.outputs = outputs;
					// Satoshis bug is that SignatureHash was supposed to return
					// a hash and on this codepath it
				...
				this.outputs = new ArrayList<TransactionOutput>(
						this.outputs.subList(0, inputIndex + 1));
				for (int i = 0; i < inputIndex; i++)
					this.outputs.set(i, new TransactionOutput(params, this,
							NEGATIVE_ONE, new byte[] {}));
				// The signature isn't broken by new versions of the transaction
				// issued by other parties.
				for (int i = 0; i < inputs.size(); i++)
					if (i != inputIndex)
	...
	public void setLockTime(long lockTime) {
		unCache();
		// TODO: Consider checking that at least one input has a non-final
		// sequence number.
	...
	public synchronized boolean hasConfidence() {
		return confidence != null
				&& confidence.getConfidenceType() != TransactionConfidence.ConfidenceType.UNKNOWN;
	}
	 ...
	 * such as whether the inputs are already spent.
	 * 
	...
	public void verify() throws VerificationException {
		...
		if (inputs.size() == 0 || outputs.size() == 0)
			throw new VerificationException(
					"Transaction had no inputs or no outputs.");
		if (this.getMessageSize() > Block.MAX_BLOCK_SIZE)
			throw new VerificationException(
					"Transaction larger than MAX_BLOCK_SIZE");
		...
		if (valueOut.compareTo(params.MAX_MONEY) > 0)
			throw new VerificationException(
					"Total transaction output value greater than possible");

		if (isCoinBase()) {
			if (inputs.get(0).getScriptBytes().length < 2
					|| inputs.get(0).getScriptBytes().length > 100)
				throw new VerificationException(
			...
			for (TransactionInput input : inputs)
				if (input.isCoinBase())
					throw new VerificationException(
							"Coinbase input as input in non-coinbase transaction");
		}
	}
	 ...
	 * useful in certain types of <a
	 * href="http://en.bitcoin.it/wiki/Contracts">contracts</a>, such as
	 * micropayment channels.
	 * </p>
	 * 
	 * <p>
	 * Note that currently the replacement feature is disabled in the Satoshi
	 * client and will need to be re-activated before this functionality is
	 * useful.
	 * </p>
	 */
	...
	/**
	 * Parses the string either as a whole number of blocks, or if it contains
	 * slashes as a YYYY/MM/DD format date and returns the lock time in wire
	 * format.
	 */
