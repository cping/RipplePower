...
/**
 * <p>
 * Methods to serialize and de-serialize messages to the Bitcoin network format
 * as defined in <a href="https://en.bitcoin.it/wiki/Protocol_specification">the
 * protocol specification</a>.
 * </p>
 * 
 * <p>
 * To be able to serialize and deserialize new Message subclasses the following
 * criteria needs to be met.
 * </p>
 * 
 * <ul>
 * <li>The proper Class instance needs to be mapped to its message name in the
 * names variable below</li>
 * <li>There needs to be a constructor matching: NetworkParameters params,
 * byte[] payload</li>
...
public class BitcoinSerializer {
	private static final Logger log = LoggerFactory
			.getLogger(BitcoinSerializer.class);
	 ...
	 * 
	 * @param params
	 *            networkParams used to create Messages instances and termining
	 *            packetMagic
	 */
	 ...
	 *            packetMagic
	 * @param parseLazy
	 *            deserialize messages in lazy mode.
	 * @param parseRetain
	 *            retain the backing byte array of a message for fast
	 *            reserialization.
	 */
	public BitcoinSerializer(NetworkParameters params, boolean parseLazy,
			boolean parseRetain) {
		this.params = params;
	...
	public void serialize(String name, byte[] message, OutputStream out)
		...
		// The header array is initialized to zero by Java so we don't have to
		// worry about
		...
		if (log.isDebugEnabled())
			log.debug("Sending {} message: {}", name, bytesToHexString(header)
	...
	public void serialize(Message message, OutputStream out) throws IOException {
		...
		if (name == null) {
			throw new Error(
					"BitcoinSerializer doesn't currently know how to serialize "
							+ message.getClass());
		}
	...
	public Message deserialize(ByteBuffer in) throws ProtocolException,
		...
		// - Payload data
		//
		// The checksum is the first 4 bytes of a SHA256 hash of the message
		// payload. It isn't
		// present for all messages, notably, the first one on a connection.
		//
		// Satoshi's implementation ignores garbage before the magic header
		// bytes. We have to do the same because
	...
	/**
	 * Deserializes only the header in case packet meta data is needed before
	 * decoding the payload. This method assumes you have already called
	 * seekPastMagicBytes()
	 */
	...
	/**
	 * Deserialize payload only. You must provide a header, typically obtained
	 * by calling {@link BitcoinSerializer#deserializeHeader}.
	 */
	public Message deserializePayload(BitcoinPacketHeader header, ByteBuffer in)
		...
		if (header.checksum[0] != hash[0] || header.checksum[1] != hash[1]
			...
			throw new ProtocolException("Checksum failed to verify, actual "
					+ bytesToHexString(hash) + " vs "
					+ bytesToHexString(header.checksum));
		}

		if (log.isDebugEnabled()) {
			log.debug("Received {} byte '{}' message: {}", header.size,
					header.command, Utils.bytesToHexString(payloadBytes));
		}
		...
		try {
			...
			throw new ProtocolException("Error deserializing message "
					+ Utils.bytesToHexString(payloadBytes) + "\n", e);
		}
	}

	private Message makeMessage(String command, int length,
			byte[] payloadBytes, byte[] hash, byte[] checksum)
			throws ProtocolException {
		// We use an if ladder rather than reflection because reflection is very
		// slow on Android.
		Message message;
		if (command.equals("version")) {
			return new VersionMessage(params, payloadBytes);
		} else if (command.equals("inv")) {
			message = new InventoryMessage(params, payloadBytes, parseLazy,
					parseRetain, length);
		} else if (command.equals("block")) {
			message = new Block(params, payloadBytes, parseLazy, parseRetain,
					length);
		} else if (command.equals("merkleblock")) {
			message = new FilteredBlock(params, payloadBytes);
		} else if (command.equals("getdata")) {
			message = new GetDataMessage(params, payloadBytes, parseLazy,
					parseRetain, length);
			...
			Transaction tx = new Transaction(params, payloadBytes, null,
					parseLazy, parseRetain, length);
			if (hash != null)
				tx.setHash(new Sha256Hash(Utils.reverseBytes(hash)));
			message = tx;
		} else if (command.equals("addr")) {
			message = new AddressMessage(params, payloadBytes, parseLazy,
					parseRetain, length);
			...
			log.warn("No support for deserializing message with name {}",
					command);
	...
	public static class BitcoinPacketHeader {
		...
		public BitcoinPacketHeader(ByteBuffer in) throws ProtocolException,
			...
			// The command is a NULL terminated string, unless the command fills
			// all twelve bytes
