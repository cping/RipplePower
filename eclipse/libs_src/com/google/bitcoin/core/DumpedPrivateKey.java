...
public class DumpedPrivateKey extends VersionedChecksummedBytes {
	...
	private static byte[] encode(byte[] keyBytes, boolean compressed) {
		Preconditions.checkArgument(keyBytes.length == 32,
	 ...
	 * 
	 * @param params
	 *            The expected network parameters of the key. If you don't care,
	 *            provide null.
	 * @param encoded
	...
	public DumpedPrivateKey(NetworkParameters params, String encoded)
		...
		if (bytes.length == 33 && bytes[32] == 1) {
			compressed = true;
			bytes = Arrays.copyOf(bytes, 32); // Chop off the additional marker
												// byte.
		} else if (bytes.length == 32) {
			compressed = false;
		} else {
			throw new AddressFormatException(
					"Wrong number of bytes for a private key, not 32 or 33");
		}
	}
	...
	public boolean equals(Object other) {
		// This odd construction is to avoid anti-symmetry of equality: where
		// a.equals(b) != b.equals(a).
		boolean result = false;
		if (other instanceof VersionedChecksummedBytes) {
			result = Arrays.equals(bytes,
					((VersionedChecksummedBytes) other).bytes);
		}
		if (other instanceof DumpedPrivateKey) {
			DumpedPrivateKey o = (DumpedPrivateKey) other;
			result = Arrays.equals(bytes, o.bytes) && version == o.version
