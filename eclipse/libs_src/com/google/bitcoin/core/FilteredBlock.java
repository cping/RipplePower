...
/**
 * <p>
 * A FilteredBlock is used to relay a block with its transactions filtered using
 * a {@link BloomFilter}. It consists of the block header and a
 * {@link PartialMerkleTree} which contains the transactions which matched the
 * filter.
 * </p>
 */
public class FilteredBlock extends Message {
	...
	// These were relayed as a part of the filteredblock getdata, ie likely
	// weren't previously received as loose transactions
	...
	/**
	 * Gets a list of leaf hashes which are contained in the partial merkle tree
	 * in this filtered block
	 * 
	 * @throws ProtocolException
	 *             If the partial merkle block is invalid or the merkle root of
	 *             the partial merkle block doesnt match the block header
	 */
	public List<Sha256Hash> getTransactionHashes() throws VerificationException {
		...
		if (header.getMerkleRoot().equals(
				merkleTree.getTxnHashAndMerkleRoot(hashesMatched))) {
	...
	/**
	 * Gets the set of transactions which were provided using
	 * provideTransaction() which match in getTransactionHashes()
	 */
