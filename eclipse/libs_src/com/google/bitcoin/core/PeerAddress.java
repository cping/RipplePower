...
public class PeerAddress extends ChildMessage {
	 ...
	 */
	public PeerAddress(NetworkParameters params, byte[] payload, int offset,
	 ...
	 */
	public PeerAddress(NetworkParameters params, byte[] msg, int offset,
			int protocolVersion, Message parent, boolean parseLazy,
			boolean parseRetain) throws ProtocolException {
		super(params, msg, offset, protocolVersion, parent, parseLazy,
				parseRetain, UNKNOWN_LENGTH);
		// Message length is calculated in parseLite which is guaranteed to be
		// called before it is ever read.
		// Even though message length is static for a PeerAddress it is safer to
		// leave it there
	...
	/**
	 * Constructs a peer address from the given IP address and port. Protocol
	...
	/**
	 * Constructs a peer address from the given IP address. Port and protocol
	 * version are default for the prodnet.
	 */
	...
	protected void bitcoinSerializeToStream(OutputStream stream)
		...
		if (protocolVersion >= 31402) {
			// TODO this appears to be dynamic because the client only ever
			// sends out it's own address
			// so assumes itself to be up. For a fuller implementation this
			// needs to be dynamic only if
		...
		// Java does not provide any utility to map an IPv4 address into IPv6
		// space, so we have to do it by hand.
		...
		// And write out the port. Unlike the rest of the protocol, address and
	...
	public int getMessageSize() {
		// The 4 byte difference is the uint32 timestamp that was introduced in
		// version 31402
		length = protocolVersion > 31402 ? MESSAGE_SIZE : MESSAGE_SIZE - 4;
		return length;
	}
	...
	public boolean equals(Object o) {
		...
		return other.addr.equals(addr) && other.port == port
				&& other.services.equals(services) && other.time == time;
		// FIXME including services and time could cause same peer to be added
		// multiple times in collections
	}
