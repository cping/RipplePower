 ...
 * 
 * <p>
 * To learn more about this class, read <b><a
 * href="http://code.google.com/p/bitcoinj/wiki/WorkingWithTheWallet"> working
 * with the wallet.</a></b>
 * </p>
 * 
 * <p>
 * To fill up a Wallet with transactions, you need to use it in combination with
 * a {@link BlockChain} and various other objects, see the <a
 * href="http://code.google.com/p/bitcoinj/wiki/GettingStarted">Getting
 * started</a> tutorial on the website to learn more about how to set everything
 * up.
 * </p>
 * 
 * <p>
 * Wallets can be serialized using either Java serialization - this is not
 * compatible across versions of bitcoinj, or protocol buffer serialization. You
 * need to save the wallet whenever it changes, there is an auto-save feature
 * that simplifies this for you although you're still responsible for manually
 * triggering a save when your app is about to quit because the auto-save
 * feature waits a moment before actually committing to disk to avoid IO
 * thrashing when the wallet is changing very fast (eg due to a block chain
 * sync). See
...
public class Wallet implements Serializable, BlockChainListener,
	...
	// If a double-spend appears in the pending state as well, currently we just
	// ignore the second
	...
	// of pending. Note that the Satoshi
	// client has no notion of dead-ness: the assumption is that double spends
	...
	// A list of public/private EC keys owned by this user. Access it using
	// addKey[s], hasKey[s] and findPubKeyFromHash.
	...
	// Whether or not to ignore nLockTime > 0 transactions that are received to
	// the mempool.
	...
	// The keyCrypter for the wallet. This specifies the algorithm used for
	// encrypting and decrypting the private keys.
	...
	// Object that performs risk analysis of received pending transactions. We
	// might reject transactions that seem like
	...
	/**
	 * Create a wallet with a keyCrypter to use in encrypting and decrypting
	 * keys.
	 */
	...
	private void createTransientState() {
		...
		txConfidenceListener = new TransactionConfidence.Listener() {
			...
			public void onConfidenceChanged(Transaction tx,
				...
				// This will run on the user code thread so we shouldn't do
				// anything too complicated here.
				// We only want to queue a wallet changed event and auto-save if
				// the number of peers announcing
				// the transaction has changed, as that confidence change is
				// made by the networking code which
				// doesn't necessarily know at that point which wallets contain
				// which transactions, so it's up
				// to us to listen for that. Other types of confidence changes
				// (type, etc) are triggered by us,
				// so we'll queue up a wallet change event in other parts of the
				// code.
	...
	/**
	 * Removes the given key from the keychain. Be very careful with this -
	...
	public void saveToFile(File temp, File destFile) throws IOException {
		...
		try {
			...
			if (Utils.isWindows()) {
				...
				throw new IOException("Failed to rename " + temp + " to "
						+ canonical);
	...
	/**
	 * <p>
	 * Whether or not the wallet will ignore received pending transactions that
	 * fail the selected {@link RiskAnalysis}. By default, if a transaction is
	 * considered risky then it won't enter the wallet and won't trigger any
	 * event listeners. If you set this property to true, then all transactions
	 * will be allowed in regardless of risk. Currently, the
	 * {@link DefaultRiskAnalysis} checks for non-finality of transactions. You
	 * should not encounter these outside of special protocols.
	 * </p>
	 * 
	 * <p>
	...
	public WalletFiles autosaveToFile(File f, long delayTime,
			TimeUnit timeUnit, @Nullable WalletFiles.Listener eventListener) {
		lock.lock();
		try {
	...
	public boolean isConsistent() {
		...
		try {
			...
			int size2 = unspent.size() + spent.size() + pending.size()
					+ dead.size();
			if (size1 != size2) {
			...
			for (Transaction tx : unspent.values()) {
				if (!tx.isConsistent(this, false)) {
					success = false;
					log.error("Inconsistent unspent tx {}",
							tx.getHashAsString());
				}
			}
	...
	/**
	 * Called by the {@link BlockChain} when we receive a new filtered block
	 * that contains a transactions previously received by a call to @{link
	 * receivePending}.
	 * <p>
	 * 
	...
	public void notifyTransactionIsInBlock(Sha256Hash txHash,
			StoredBlock block, BlockChain.NewBlockType blockType,
			int relativityOffset) throws VerificationException {
		lock.lock();
		try {
			Transaction tx = transactions.get(txHash);
			if (tx == null) {
				log.error("TX {} not found despite being sent to wallet",
						txHash);
				return;
			}
		...
		if (blockType == AbstractBlockChain.NewBlockType.BEST_CHAIN) {
			// If some keys are considered to be bad, possibly move money
			// assigned to them now.
			// This has to run outside the wallet lock as it may trigger
			// broadcasting of new transactions.
			maybeRotateKeys();
		}
	}
	 ...
	 * <p>
	 * This is the same as
	 * {@link Wallet#receivePending(Transaction, java.util.List)} but allows you
	 * to override the {@link Wallet#isPendingTransactionRelevant(Transaction)}
	 * sanity-check to keep track of transactions that are not spendable or
	 * spend our coins. This can be useful when you want to keep track of
	 * transaction confidence on arbitrary transactions. Note that transactions
	 * added in this way will still be relayed to peers and appear in
	 * transaction lists like any other pending transaction (even when not
	 * relevant).
	 * </p>
	 */
	public void receivePending(Transaction tx,
			@Nullable List<Transaction> dependencies, boolean overrideIsRelevant)
			throws VerificationException {
		// Can run in a peer thread. This method will only be called if a prior
		// call to isPendingTransactionRelevant
		// returned true, so we already know by this point that it sends coins
		// to or from our wallet, or is a double
		...
		try {
			...
			if (!containingPools.equals(EnumSet.noneOf(Pool.class))) {
				log.debug("Received tx we already saw in a block or created ourselves: "
						+ tx.getHashAsString());
				return;
			}
			// Repeat the check of relevancy here, even though the caller may
			// have already done so - this is to avoid
			// race conditions where receivePending may be being called in
			// parallel.
			if (!overrideIsRelevant && !isPendingTransactionRelevant(tx))
				return;
			if (isTransactionRisky(tx, dependencies)
			...
			if (log.isInfoEnabled()) {
				log.info(String.format(
						"Received a pending transaction %s that spends %s BTC from our own wallet,"
			...
			if (tx.getConfidence().getSource()
					.equals(TransactionConfidence.Source.UNKNOWN)) {
				log.warn("Wallet received transaction with an unknown source. Consider tagging it!");
			}
			// If this tx spends any of our unspent outputs, mark them as spent
			// now, then add to the pending pool. This
		...
		// maybeRotateKeys() will ignore pending transactions so we don't bother
		// calling it here (see the comments
	 ...
	 * independent of the value of {@link #doesAcceptRiskyTransactions()}. Risky
	...
	public boolean isTransactionRisky(Transaction tx,
			@Nullable List<Transaction> dependencies) {
		lock.lock();
		try {
	 ...
	 * {@link Wallet#isPendingTransactionRelevant(Transaction)} should have been
	 * called to decide whether the wallet cares about the transaction - if it
	 * does, then this method expects the transaction and any dependencies it
	 * has which are still in the memory pool.
	 * </p>
	 */
	public void receivePending(Transaction tx,
			@Nullable List<Transaction> dependencies)
	 ...
	 */
	private boolean checkForDoubleSpendAgainstPending(Transaction tx,
			boolean takeAction) {
		...
		for (Transaction p : pending.values()) {
			for (TransactionInput input : p.getInputs()) {
				...
				if (outpoints.contains(outpoint)) {
					...
					if (!doubleSpentTxns.isEmpty()
							&& doubleSpentTxns.getLast() == p)
						continue;
	...
	private void receive(Transaction tx, StoredBlock block,
			BlockChain.NewBlockType blockType, int relativityOffset)
			throws VerificationException {
		...
		log.info("Received tx{} for {} BTC: {} [{}] in block {}",
				sideChain ? " on a side chain" : "",
				bitcoinValueToFriendlyString(valueDifference), tx
						.getHashAsString(), relativityOffset,
		...
		if (bestChain) {
			if (wasPending) {
				// Was pending and is now confirmed. Disconnect the outputs in
				// case we spent any already: they will be
			...
			// Transactions that appear in a side chain will have that
			// appearance recorded below - we assume that
			// some miners are also trying to include the transaction into the
			// current best chain too, so let's treat
			// it as pending, except we don't need to do any risk analysis on
			// it.
			if (wasPending) {
				// Just put it back in without touching the connections or
				// confidence.
				addWalletTransaction(Pool.PENDING, tx);
				log.info("  ->pending");
			} else {
				// Ignore the case where a tx appears on a side chain at the
				// same time as the best chain (this is
				...
				if (!unspent.containsKey(hash) && !spent.containsKey(hash)) {
					...
					// Committing it updates the spent flags and inserts into
					// the pool as well.
					commitTx(tx);
				}
			}
		...
		if (block != null) {
			...
			// after a re-org. This also tells the tx
			// confidence object about the block and sets its work done/depth
			// appropriately.
			tx.setBlockAppearance(block, bestChain, relativityOffset);
			if (bestChain) {
				// Don't notify this tx of work done in notifyNewBestBlock which
				// will be called immediately after
				// this method has been called by BlockChain for all relevant
				// transactions. Otherwise we'd double
				// count.
		...
		if (bestChain) {
			// notifyNewBestBlock will be invoked next and will then call
			// maybeQueueOnWalletChanged for us.
			confidenceChanged.put(tx,
		...
		if (!insideReorg && bestChain) {
			...
			log.info("Balance is now: "
					+ bitcoinValueToFriendlyString(newBalance));
			if (!wasPending) {
				int diff = valueDifference.signum();
				// We pick one callback based on the value difference, though a
				// tx can of course both send and receive
	...
	/**
	 * <p>
	 * Called by the {@link BlockChain} when a new block on the best chain is
	 * seen, AFTER relevant wallet transactions are extracted and sent to us
	 * UNLESS the new block caused a re-org, in which case this will not be
	 * called (the
	 * {@link Wallet#reorganize(StoredBlock, java.util.List, java.util.List)}
	 * method will call this one in that case).
	 * </p>
	 * <p/>
	 * <p>
	 * Used to update confidence data in each transaction and last seen block
	 * hash. Triggers auto saving. Invokes the onWalletChanged event listener if
	 * there were any affected transactions.
	 * </p>
	 */
		...
		try {
			...
			for (Transaction tx : transactions) {
				if (ignoreNextNewBlock.contains(tx.getHash())) {
					// tx was already processed in receive() due to it appearing
					// in this block, so we don't want to
					// notify the tx confidence of work done twice, it'd result
					// in miscounting.
			...
			informConfidenceListenersIfNotReorganizing();
			maybeQueueOnWalletChanged();
			// Coalesce writes to avoid throttling on disk access when catching
			// up with the chain.
			saveLater();
		} finally {
			lock.unlock();
	...
	private void processTxFromBestChain(Transaction tx, boolean forceAddToPool)
		...
		// tests, if keys are moved between wallets, if we're catching up to the
		// chain given only a set of keys,
		// or if a dead coinbase transaction has moved back onto the main chain.
		boolean isDeadCoinbase = tx.isCoinBase()
				&& dead.containsKey(tx.getHash());
		if (isDeadCoinbase) {
			// There is a dead coinbase tx being received on the best chain. A
			// coinbase tx is made dead when it moves
			// to a side chain but it can be switched back on a reorg and
			// resurrected back to spent or unspent.
			// So take it out of the dead pool. Note that we don't resurrect
			// dependent transactions here, even though
			// we could. Bitcoin Core nodes on the network have deleted the
			// dependent transactions from their mempools
			// entirely by this point. We could and maybe should rebroadcast
			// them so the network remembers and tries
			// to confirm them again. But this is a deeply unusual edge case
			// that due to the maturity rule should never
			// happen in practice, thus for simplicities sake we ignore it here.
			log.info("  coinbase tx <-dead: confidence {}",
					tx.getHashAsString(), tx.getConfidence()
							.getConfidenceType().name());
			dead.remove(tx.getHash());
		}

		// Update tx and other unspent/pending transactions by connecting
		// inputs/outputs.
		updateForSpends(tx, true);

		// Now make sure it ends up in the right pool. Also, handle the case
		// where this TX is double-spending
		// against our pending transactions. Note that a tx may double spend our
		// pending transactions and also send
		// us money/spend our money.
		boolean hasOutputsToMe = tx.getValueSentToMe(this, true).signum() > 0;
		if (hasOutputsToMe) {
			// Needs to go into either unspent or spent (if the outputs were
			// already spent by a pending tx).
			...
			// Didn't send us any money, but did spend some. Keep it around for
			// record keeping purposes.
			log.info("  tx {} ->spent", tx.getHashAsString());
			addWalletTransaction(Pool.SPENT, tx);
		} else if (forceAddToPool) {
			// Was manually added to pending, so we should keep it to notify the
			// user of confidence information
	...
	private void updateForSpends(Transaction tx, boolean fromChain)
		...
		for (TransactionInput input : tx.getInputs()) {
			...
			if (result == TransactionInput.ConnectionResult.NO_SUCH_TX) {
				...
				result = input.connect(spent,
						TransactionInput.ConnectMode.ABORT_ON_CONFLICT);
				if (result == TransactionInput.ConnectionResult.NO_SUCH_TX) {
					// Not found in the unspent and spent maps. Try again with
					// the pending map.
					result = input.connect(pending,
							TransactionInput.ConnectMode.ABORT_ON_CONFLICT);
					if (result == TransactionInput.ConnectionResult.NO_SUCH_TX) {
			...
			if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
				if (fromChain) {
					// Double spend from chain: this will be handled later by
					// checkForDoubleSpendAgainstPending.
					...
					log.warn("  offending input is input {}", tx.getInputs()
							.indexOf(input));
					log.warn("{}: {}", tx.getHash(),
							new String(Hex.encode(tx.unsafeBitcoinSerialize())));
					Transaction other = input.getConnectedOutput().getSpentBy()
							.getParentTransaction();
					log.warn("{}: {}", other.getHash(),
							new String(Hex.encode(tx.unsafeBitcoinSerialize())));
				}
			...
			for (Transaction pendingTx : pending.values()) {
				for (TransactionInput input : pendingTx.getInputs()) {
					TransactionInput.ConnectionResult result = input.connect(
							tx, TransactionInput.ConnectMode.ABORT_ON_CONFLICT);
					...
					if (result == TransactionInput.ConnectionResult.SUCCESS) {
						log.info("Connected pending tx input {}:{}", pendingTx
								.getHashAsString(), pendingTx.getInputs()
								.indexOf(input));
					}
				}
				// If the transactions outputs are now all spent, it will be
				// moved into the spent pool by the
	...
	private void killTx(@Nullable Transaction overridingTx,
			List<Transaction> killedTx) {
		...
		while (!work.isEmpty()) {
			...
			log.warn(
					"TX {} killed{}",
					tx.getHashAsString(),
					overridingTx != null ? "by "
							+ overridingTx.getHashAsString() : "");
			...
			for (TransactionOutput deadOutput : tx.getOutputs()) {
				...
				if (connected == null)
					continue;
				final Transaction parentTransaction = connected
						.getParentTransaction();
				log.info("This death invalidated dependent tx {}",
						parentTransaction.getHash());
	...
	private void maybeMovePool(Transaction tx, String context) {
		...
		if (tx.isEveryOwnedOutputSpent(this)) {
			...
			if (unspent.remove(tx.getHash()) != null) {
				if (log.isInfoEnabled()) {
					log.info("  {} {} <-unspent ->spent", tx.getHashAsString(),
							context);
				}
			...
			if (spent.remove(tx.getHash()) != null) {
				if (log.isInfoEnabled()) {
					log.info("  {} {} <-spent ->unspent", tx.getHashAsString(),
							context);
				}
	...
	public void addEventListener(WalletEventListener listener, Executor executor) {
		eventListeners.add(new ListenerRegistration<WalletEventListener>(
				listener, executor));
	}
	...
	public boolean maybeCommitTx(Transaction tx) throws VerificationException {
		...
		try {
			...
			// Mark the outputs we're spending as spent so we won't try and use
			// them in future creations. This will also
			// move any transactions that are now fully spent to the spent map
			// so we can skip them when creating future
			// spends.
			updateForSpends(tx, false);
			// Add to the pending pool. It'll be moved out once we receive this
			// transaction on the best chain.
			// This also registers txConfidenceListener so wallet listeners get
			// informed.
			...
			try {
				BigInteger valueSentFromMe = tx.getValueSentFromMe(this);
				BigInteger valueSentToMe = tx.getValueSentToMe(this);
				BigInteger newBalance = balance.add(valueSentToMe).subtract(
						valueSentFromMe);
	...
	public List<Transaction> getRecentTransactions(int numTransactions,
		...
		try {
			...
			int size = getPoolSize(Pool.UNSPENT) + getPoolSize(Pool.SPENT)
					+ getPoolSize(Pool.PENDING);
			if (numTransactions > size || numTransactions == 0) {
				numTransactions = size;
			}
			ArrayList<Transaction> all = new ArrayList<Transaction>(
					getTransactions(includeDead));
			// Order by date.
			Collections.sort(all,
					Collections.reverseOrder(new Comparator<Transaction>() {
						public int compare(Transaction t1, Transaction t2) {
							return t1.getUpdateTime().compareTo(
									t2.getUpdateTime());
						}
					}));
	...
	public void cleanup() {
		...
		try {
			boolean dirty = false;
			for (Iterator<Transaction> i = pending.values().iterator(); i
					.hasNext();) {
				Transaction tx = i.next();
				if (isTransactionRisky(tx, null) && !acceptRiskyTransactions) {
					log.debug(
							"Found risky transaction {} in wallet during cleanup.",
							tx.getHashAsString());
					if (!tx.isAnyOutputSpent()) {
						...
						dirty = true;
						log.info(
								"Removed transaction {} from pending pool during cleanup.",
								tx.getHashAsString());
					} else {
						log.info(
	...
	/**
	 * A SendRequest gives the wallet information about precisely how to send
	 * money to a recipient or set of recipients. Static methods are provided to
	 * help you create SendRequests and there are a few helper methods on the
	 * wallet that just simplify the most common use cases. You may wish to
	 * customize a SendRequest if you want to attach a fee or modify the change
	 * address.
	 */
	public static class SendRequest {
		/**
		 * <p>
		 * A transaction, probably incomplete, that describes the outline of
		 * what you want to do. This typically will mean it has some outputs to
		 * the intended destinations, but no inputs or change address (and
		 * therefore no fees) - the wallet will calculate all that for you and
		 * update tx later.
		 * </p>
		 * 
		 * <p>
		 * Be careful when adding outputs that you check the min output value (
		 * {@link TransactionOutput#getMinNonDustValue(BigInteger)}) to avoid
		 * the whole transaction being rejected because one output is dust.
		 * </p>
		 * 
		 * <p>
		 * If there are already inputs to the transaction, make sure their out
		 * point has a connected output, otherwise their value will be added to
		 * fee. Also ensure they are either signed or are spendable by a wallet
		 * key, otherwise the behavior of
		 * {@link Wallet#completeTx(Wallet.SendRequest)} is undefined (likely
		 * RuntimeException).
		 * </p>
		 */
		public Transaction tx;
		...
		/**
		 * When emptyWallet is set, all coins selected by the coin selector are
		 * sent to the first output in tx (its value is ignored and set to
		 * {@link com.google.bitcoin.core.Wallet#getBalance()} - the fees
		 * required for the transaction). Any additional outputs are removed.
		 */
		 ...
		 * depends on who sent you money), and the value being sent somewhere
		 * else. The change address should be selected from this wallet,
		 * normally. <b>If null this will be chosen for you.</b>
		 */
		public Address changeAddress = null;

		/**
		 * <p>
		 * A transaction can have a fee attached, which is defined as the
		 * difference between the input values and output values. Any value
		 * taken in that is not provided to an output can be claimed by a miner.
		 * This is how mining is incentivized in later years of the Bitcoin
		 * system when inflation drops. It also provides a way for people to
		 * prioritize their transactions over others and is used as a way to
		 * make denial of service attacks expensive.
		 * </p>
		 * 
		 * <p>
		 * This is a constant fee (in satoshis) which will be added to the
		 * transaction. It is recommended that it be at least
		 * {@link Transaction#REFERENCE_DEFAULT_MIN_TX_FEE} if it is set, as
		 * default reference clients will otherwise simply treat the transaction
		 * as if there were no fee at all.
		 * </p>
		 * 
		 * <p>
		 * Once
		 * {@link Wallet#completeTx(com.google.bitcoin.core.Wallet.SendRequest)}
		 * is called, this is set to the value of the fee that was added.
		 * </p>
		 * 
		 * <p>
		 * You might also consider adding a {@link SendRequest#feePerKb} to set
		 * the fee per kb of transaction size (rounded down to the nearest kb)
		 * as that is how transactions are sorted when added to a block by
		 * miners.
		 * </p>
		 */
		 ...
		 * transaction for each kilobyte in size including the first. This is
		 * useful as as miners usually sort pending transactions by their fee
		 * per unit size when choosing which transactions to add to a block.
		 * Note that, to keep this equivalent to the reference client
		 * definition, a kilobyte is defined as 1000 bytes, not 1024.
		 * </p>
		 * 
		 * <p>
		 * You might also consider using a {@link SendRequest#fee} to set the
		 * fee added for the first kb of size.
		 * </p>
		 */
		public BigInteger feePerKb = DEFAULT_FEE_PER_KB;

		/**
		 * If you want to modify the default fee for your entire app without
		 * having to change each SendRequest you make, you can do it here. This
		...
		/**
		 * <p>
		 * Requires that there be enough fee for a default reference client to
		 * at least relay the transaction. (ie ensure the transaction will not
		 * be outright rejected by the network). Defaults to true, you should
		 * only set this to false if you know what you're doing.
		 * </p>
		 * 
		 * <p>
		 * Note that this does not enforce certain fee rules that only apply to
		 * transactions which are larger than 26,000 bytes. If you get a
		 * transaction which is that large, you should set a fee and feePerKb of
		 * at least {@link Transaction#REFERENCE_DEFAULT_MIN_TX_FEE}.
		 * </p>
		 */
		public boolean ensureMinRequiredFee = true;

		/**
		 * The AES key to use to decrypt the private keys before signing. If
		 * null then no decryption will be performed and if decryption is
		 * required an exception will be thrown. You can get this from a
		 * password by doing wallet.getKeyCrypter().deriveKey(password).
		 */
		public KeyParameter aesKey = null;

		/**
		 * If not null, the {@link com.google.bitcoin.wallet.CoinSelector} to
		 * use instead of the wallets default. Coin selectors are responsible
		 * for choosing which transaction outputs (coins) in a wallet to use
		 * given the desired send value amount.
		 */
		public CoinSelector coinSelector = null;

		// Tracks if this has been passed to wallet.completeTx already: just a
		// safety check.
		 ...
		 * 
		 * <p>
		 * Be very careful when value is smaller than
		 * {@link Transaction#MIN_NONDUST_OUTPUT} as the transaction will likely
		 * be rejected by the network in this case.
		 * </p>
		 */
		 ...
		 * {@link TransactionOutput#getMinNonDustValue(BigInteger)} afterwards
		 * or you risk having the transaction rejected by the network. Note that
		 * using {@link SendRequest#to(Address, java.math.BigInteger)} will
		 * result in a smaller output, and thus the ability to use a smaller
		 * output value without rejection.
		 * </p>
		 */
		public static SendRequest to(NetworkParameters params,
				ECKey destination, BigInteger value) {
			SendRequest req = new SendRequest();
	 ...
	 * The change is sent to {@link Wallet#getChangeAddress()}, so you must have
	 * added at least one key.
	 * </p>
	 * 
	 * <p>
	 * If you just want to send money quickly, you probably want
	 * {@link Wallet#sendCoins(TransactionBroadcaster, Address, java.math.BigInteger)}
	 * instead. That will create the sending transaction, commit to the wallet
	 * and broadcast it to the network all in one go. This method is lower level
	 * and lets you see the proposed transaction before anything is done with
	 * it.
	 * </p>
	 * 
	 * <p>
	 * This is a helper method that is equivalent to using
	 * {@link Wallet.SendRequest#to(Address, java.math.BigInteger)} followed by
	 ...
	 * 
	 * <p>
	 * You MUST ensure that nanocoins is larger than
	 ...
	 *         coins. coins as spent until commitTx is called on the result.
	 ...
	 * announced to the network. The given {@link SendRequest} is completed
	 * first using {@link Wallet#completeTx(Wallet.SendRequest)} to make it
	 * valid.
	 * 
	 ...
	 * is returned to {@link Wallet#getChangeAddress()}. Note that a fee may be
	 * automatically added if one may be required for the transaction to be
	 * confirmed.
	 * </p>
	 * 
	 * <p>
	 ...
	 * spend those same coins again.
	 * </p>
	 * 
	 * <p>
	 * You MUST ensure that value is smaller than
	 * {@link Transaction#MIN_NONDUST_OUTPUT} or the transaction will almost
	 * certainly be rejected by the network as dust.
	 * </p>
	 * 
	 * @param broadcaster
	 *            a {@link TransactionBroadcaster} to use to send the
	 *            transactions out.
	 * @param to
	 *            Which address to send coins to.
	 * @param value
	 *            How much value to send. You can use Utils.toNanoCoins() to
	 *            calculate this.
	 ...
	 *             balance.
	 */
	public SendResult sendCoins(TransactionBroadcaster broadcaster, Address to,
	 ...
	 * @param request
	 *            the SendRequest that describes what to do, get one using
	...
	public SendResult sendCoins(TransactionBroadcaster broadcaster,
			SendRequest request) throws InsufficientMoneyException {
		// Should not be locked here, as we're going to call into the
		// broadcaster and that might want to hold its
		// own lock. sendCoinsOffline handles everything that needs to be
		// locked.
		checkState(!lock.isHeldByCurrentThread());

		// Commit the TX to the wallet immediately so the spent coins won't be
		// reused.
		// TODO: We should probably allow the request to specify tx commit only
		// after the network has accepted it.
		Transaction tx = sendCoinsOffline(request);
		SendResult result = new SendResult();
		result.tx = tx;
		// The tx has been committed to the pending pool by this point (via
		// sendCoinsOffline -> commitTx), so it has
		// a txConfidenceListener registered. Once the tx is broadcast the peers
		// will update the memory pool with the
		// count of seen peers, the memory pool will update the transaction
		// confidence object, that will invoke the
		// txConfidenceListener which will in turn invoke the wallets event
		// listener onTransactionConfidenceChanged
		// method.
	 ...
	 *             if no transaction broadcaster has been configured.
	 * @throws InsufficientMoneyException
	 *             if the request could not be completed due to not enough
	 *             balance.
	 */
	 ...
	 * time. Note that a fee may be automatically added if one may be required
	 ...
	 * method, as is the fee parameter.
	 * 
	 * @param req
	 ...
	 *             if you try and complete the same SendRequest twice, or if the
	 *             given send request cannot be completed without violating the
	 *             protocol rules.
	 */
		...
		try {
			...
			for (TransactionInput input : req.tx.getInputs())
				if (input.getConnectedOutput() != null)
					totalInput = totalInput.add(input.getConnectedOutput()
							.getValue());
				else
					log.warn("SendRequest transaction already has inputs but we don't know how much they are worth - they will be added to fee.");
			value = value.subtract(totalInput);

			List<TransactionInput> originalInputs = new ArrayList<TransactionInput>(
					req.tx.getInputs());

			// We need to know if we need to add an additional fee because one
			// of our values are smaller than 0.01 BTC
			...
			if (req.ensureMinRequiredFee && !req.emptyWallet) { // min fee
				...
				for (TransactionOutput output : req.tx.getOutputs())
					if (output.getValue().compareTo(Utils.CENT) < 0) {
						if (output.getValue().compareTo(
								output.getMinNonDustValue()) < 0)
			...
			//
			// Note that this code is poorly optimized: the spend candidates
			// only alter when transactions in the wallet
			// change - it could be pre-calculated and held in RAM, and this is
			// probably an optimization worth doing.
			// Note that output.isMine(this) needs to test the keychain which is
			// currently an array, so it's
			// O(candidate outputs ^ keychain.size())! There's lots of low
			// hanging fruit here.
			...
			if (!req.emptyWallet) {
				...
				feeCalculation = new FeeCalculation(req, value, originalInputs,
						needAtLeastReferenceFee, candidates);
				...
				CoinSelector selector = req.coinSelector == null ? coinSelector
						: req.coinSelector;
				bestCoinSelection = selector.select(
						NetworkParameters.MAX_MONEY, candidates);
			...
			if (req.ensureMinRequiredFee && req.emptyWallet) {
				final BigInteger baseFee = req.fee == null ? BigInteger.ZERO
						: req.fee;
				final BigInteger feePerKb = req.feePerKb == null ? BigInteger.ZERO
						: req.feePerKb;
				Transaction tx = req.tx;
				if (!adjustOutputDownwardsForFee(tx, bestCoinSelection,
						baseFee, feePerKb))
			...
			if (bestChangeOutput != null) {
				...
				log.info("  with {} coins change",
						bitcoinValueToFriendlyString(bestChangeOutput
								.getValue()));
			}
			final BigInteger calculatedFee = totalInput.subtract(totalOutput);
			if (calculatedFee.signum() > 0) {
				log.info("  with a fee of {}",
						bitcoinValueToFriendlyString(calculatedFee));
			}
	 ...
	 */
	private boolean adjustOutputDownwardsForFee(Transaction tx,
			CoinSelection coinSelection, BigInteger baseFee, BigInteger feePerKb) {
		TransactionOutput output = tx.getOutput(0);
		// Check if we need additional fee due to the transaction's size
		int size = tx.bitcoinSerialize().length;
		size += estimateBytesForSigning(coinSelection);
		BigInteger fee = baseFee.add(BigInteger.valueOf((size / 1000) + 1)
				.multiply(feePerKb));
		...
		if (output.getValue().compareTo(Utils.CENT) < 0
			...
			output.setValue(output.getValue().subtract(
					Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.subtract(fee)));
		return output.getMinNonDustValue().compareTo(output.getValue()) <= 0;
	}

	/**
	 * Returns a list of all possible outputs we could possibly spend,
	 * potentially even including immature coinbases (which the protocol may
	 * forbid us from spending). In other words, return all outputs that this
	 * wallet holds keys for and which are not already marked as spent.
	 */
	public LinkedList<TransactionOutput> calculateAllSpendCandidates(
			boolean excludeImmatureCoinbases) {
		lock.lock();
		try {
			LinkedList<TransactionOutput> candidates = Lists.newLinkedList();
			for (Transaction tx : Iterables.concat(unspent.values(),
					pending.values())) {
	...
	public Address getChangeAddress() {
		lock.lock();
		try {
			// For now let's just pick the first key in our keychain. In future
			// we might want to do something else to
			// give the user better privacy here, eg in incognito mode.
			checkState(keychain.size() > 0,
					"Can't send value without an address to use for receiving change");
	 ...
	 * keys (that would result in coin loss). If
	 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.bitcoin.wallet.WalletFiles.Listener)}
	 * has been called, triggers an auto save bypassing the normal coalescing
	 * delay and event handlers. If the key already exists in the wallet, does
	...
	/**
	 * Adds the given keys to the wallet. There is currently no way to delete
	 * keys (that would result in coin loss). If
	 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.bitcoin.wallet.WalletFiles.Listener)}
	 * has been called, triggers an auto save bypassing the normal coalescing
	...
	public int addKeys(final List<ECKey> keys) {
		...
		try {
			int added = 0;
			// TODO: Consider making keys a sorted list or hashset so membership
			// testing is faster.
			for (final ECKey key : keys) {
				if (keychain.contains(key))
					continue;

				// If the key has a keyCrypter that does not match the Wallet's
				// then a KeyCrypterException is thrown.
				// This is done because only one keyCrypter is persisted per
				// Wallet and hence all the keys must be homogenous.
				if (isEncrypted()
						&& (!key.isEncrypted() || !keyCrypter.equals(key
								.getKeyCrypter()))) {
					...
					throw new KeyCrypterException(
							"Cannot add key because it's encrypted and this wallet is not.");
				}
				keychain.add(key);
				added++;
			}
			queueOnKeysAdded(keys);
			// Force an auto-save immediately rather than queueing one, as keys
	...
	/**
	 * Returns true if this wallet is watching transactions for outputs with the
	 * script.
	 */
	...
	public enum BalanceType {
		...
		/**
		 * Balance that can be safely used to create new spends. This is
		 * whatever the default coin selector would make available, which by
		 * default means transaction outputs with at least 1 confirmation and
		 * pending transactions created by our own wallet which have been
	 ...
	 * selector. Just asks it to select as many coins as possible and returns
	 * the total.
	 */
	public BigInteger getBalance(CoinSelector selector) {
		...
		try {
			...
			LinkedList<TransactionOutput> candidates = calculateAllSpendCandidates(true);
			CoinSelection selection = selector.select(
					NetworkParameters.MAX_MONEY, candidates);
	...
	/**
	 * Returns the available balance, including any unspent balance at watched
	 * addresses
	 */
	...
	/**
	 * Returns the balance that would be considered spendable by the given coin
	 * selector, including any unspent balance at watched addresses.
	 */
	...
	/**
	 * Formats the wallet as a human readable piece of text. Intended for
	 * debugging, the format is not meant to be stable or human readable.
	 * 
	 * @param includePrivateKeys
	 *            Whether raw private key data should be included.
	 * @param includeTransactions
	 *            Whether to print transaction data.
	 * @param includeExtensions
	 *            Whether to print extension data.
	 * @param chain
	 *            If set, will be used to estimate lock times for block
	 *            timelocked transactions.
	 */
	public String toString(boolean includePrivateKeys,
			boolean includeTransactions, boolean includeExtensions,
		...
		try {
			...
			builder.append(String.format(
					"Wallet containing %s BTC (available: %s BTC) in:%n",
					bitcoinValueToPlainString(estimatedBalance),
					bitcoinValueToPlainString(availableBalance)));
			builder.append(String.format("  %d pending transactions%n",
					pending.size()));
			builder.append(String.format("  %d unspent transactions%n",
					unspent.size()));
			builder.append(String.format("  %d spent transactions%n",
					spent.size()));
			builder.append(String.format("  %d dead transactions%n",
					dead.size()));
			...
			builder.append(String.format("Last seen best block: %d (%s): %s%n",
					getLastBlockSeenHeight(), lastBlockSeenTimeStr,
					getLastBlockSeenHash()));
			if (this.keyCrypter != null) {
				builder.append(String.format("Encryption: %s%n",
						keyCrypter.toString()));
			}
			// Do the keys.
			builder.append("\nKeys:\n");
			for (ECKey key : keychain) {
				...
				builder.append(" ");
				builder.append(includePrivateKeys ? key.toStringWithPrivate()
						: key.toString());
				builder.append("\n");
			}
	...
	private void toStringHelper(StringBuilder builder,
			Map<Sha256Hash, Transaction> transactionMap,
			@Nullable AbstractBlockChain chain,
			@Nullable Comparator<Transaction> sortOrder) {
		checkState(lock.isHeldByCurrentThread());
		...
		for (Transaction tx : txns) {
			try {
				...
				builder.append(Utils.bitcoinValueToFriendlyString(tx
						.getValueSentFromMe(this)));
				builder.append(" and receives ");
				builder.append(Utils.bitcoinValueToFriendlyString(tx
						.getValueSentToMe(this)));
	 ...
	 * work done) has changed. This can cause the number of confirmations of a
	 * transaction to go higher, lower, drop to zero and can even result in a
	 * transaction going dead (will never confirm) due to a double spend.
	 * </p>
	 * 
	 * <p>
	...
	public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks,
		...
		try {
			...
			// ChainSplitTest.java
			//
			// receive() has been called on the block that is triggering the
			// re-org before this is called, with type
			// of SIDE_CHAIN.
			//
			// Note that this code assumes blocks are not invalid - if blocks
			// contain duplicated transactions,
			// transactions that double spend etc then we can calculate the
			// incorrect result. This could open up
			// obscure DoS attacks if someone successfully mines a throwaway
			// invalid block and feeds it to us, just
			// to try and corrupt the internal data structures. We should try
			// harder to avoid this but it's tricky
			// because there are so many ways the block can be invalid.

			// Avoid spuriously informing the user of wallet/tx confidence
			// changes whilst we're re-organizing.
			...
			// Map block hash to transactions that appear in it. We ensure that
			// the map values are sorted according
			// to their relative position within those blocks.
			ArrayListMultimap<Sha256Hash, TxOffsetPair> mapBlockTx = ArrayListMultimap
					.create();
			for (Transaction tx : getTransactions(true)) {
				Map<Sha256Hash, Integer> appearsIn = tx.getAppearsInHashes();
				if (appearsIn == null)
					continue; // Pending.
				for (Map.Entry<Sha256Hash, Integer> block : appearsIn
						.entrySet())
					mapBlockTx.put(block.getKey(),
							new TxOffsetPair(tx, block.getValue()));
			}
			for (Sha256Hash blockHash : mapBlockTx.keySet())
				Collections.sort(mapBlockTx.get(blockHash));

			List<Sha256Hash> oldBlockHashes = new ArrayList<Sha256Hash>(
					oldBlocks.size());
			...
			// For each block in the old chain, disconnect the transactions in
			// reverse order.
			...
			for (Sha256Hash blockHash : oldBlockHashes) {
				for (TxOffsetPair pair : mapBlockTx.get(blockHash)) {
					...
					if (tx.isCoinBase()) {
						// All the transactions that we have in our wallet which
						// spent this coinbase are now invalid
						// and will never confirm. Hopefully this should never
						// happen - that's the point of the maturity
						// rule that forbids spending of coinbase transactions
						// for 100 blocks.
						//
						// This could be recursive, although of course because
						// we don't have the full transaction
						// graph we can never reliably kill all transactions we
						// might have that were rooted in
						// this coinbase tx. Some can just go pending forever,
						// like the Satoshi client. However we
						// can do our best.
						log.warn("Coinbase killed by re-org: {}",
								tx.getHashAsString());
			...
			// Put all the disconnected transactions back into the pending pool
			// and re-connect them.
			for (Transaction tx : oldChainTxns) {
				// Coinbase transactions on the old part of the chain are dead
				// for good and won't come back unless
				// there's another re-org.
				if (tx.isCoinBase())
					continue;
				log.info("  ->pending {}", tx.getHash());
				tx.getConfidence().setConfidenceType(ConfidenceType.PENDING); // Wipe
																				// height/depth/work
																				// data.
				confidenceChanged.put(tx,
						TransactionConfidence.Listener.ChangeReason.TYPE);
			...
			// Note that dead transactions stay dead. Consider a chain that
			// Finney attacks T1 and replaces it with
			// T2, so we move T1 into the dead pool. If there's now a re-org to
			// a chain that doesn't include T2, it
			// doesn't matter - the miners deleted T1 from their mempool, will
			// resurrect T2 and put that into the
			// mempool and so T1 is still seen as a losing double spend.

			// The old blocks have contributed to the depth and work done for
			// all the transactions in the
			// wallet that are in blocks up to and including the chain split
			// block.
			// The total depth and work done is calculated here and then
			// subtracted from the appropriate transactions.
			int depthToSubtract = oldBlocks.size();
			BigInteger workDoneToSubtract = BigInteger.ZERO;
			for (StoredBlock b : oldBlocks) {
				workDoneToSubtract = workDoneToSubtract.add(b.getHeader()
						.getWork());
			}
			log.info("depthToSubtract = " + depthToSubtract
					+ ", workDoneToSubtract = " + workDoneToSubtract);
			// Remove depthToSubtract and workDoneToSubtract from all
			// transactions in the wallet except for pending.
			subtractDepthAndWorkDone(depthToSubtract, workDoneToSubtract,
					spent.values());
			subtractDepthAndWorkDone(depthToSubtract, workDoneToSubtract,
					unspent.values());
			subtractDepthAndWorkDone(depthToSubtract, workDoneToSubtract,
			...
			// The effective last seen block is now the split point so set the
			// lastSeenBlockHash.
			setLastBlockSeenHash(splitPoint.getHeader().getHash());

			// For each block in the new chain, work forwards calling receive()
			// and notifyNewBestBlock().
			// This will pull them back out of the pending pool, or if the tx
			// didn't appear in the old chain and
			// does appear in the new chain, will treat it as such and possibly
			// kill pending transactions that
			// conflict.
			for (StoredBlock block : newBlocks) {
				log.info("Replaying block {}", block.getHeader()
						.getHashAsString());
				for (TxOffsetPair pair : mapBlockTx.get(block.getHeader()
						.getHash())) {
					log.info("  tx {}", pair.tx.getHash());
					try {
						receive(pair.tx, block,
								BlockChain.NewBlockType.BEST_CHAIN, pair.offset);
					} catch (ScriptException e) {
			...
			log.info("post-reorg balance is {}",
					Utils.bitcoinValueToFriendlyString(balance));
	 ...
	 */
	private void subtractDepthAndWorkDone(int depthToSubtract,
			BigInteger workDoneToSubtract, Collection<Transaction> transactions) {
		for (Transaction tx : transactions) {
				...
				tx.getConfidence()
						.setDepthInBlocks(
								tx.getConfidence().getDepthInBlocks()
										- depthToSubtract);
				tx.getConfidence().setWorkDone(
						tx.getConfidence().getWorkDone()
								.subtract(workDoneToSubtract));
				confidenceChanged.put(tx,
						TransactionConfidence.Listener.ChangeReason.DEPTH);
			}
		}
	...
	public long getEarliestKeyCreationTime() {
		...
		try {
			...
			for (ECKey key : keychain)
				earliestTime = Math.min(key.getCreationTimeSeconds(),
						earliestTime);
			for (Script script : watchedScripts)
				earliestTime = Math.min(script.getCreationTimeSeconds(),
						earliestTime);
	 ...
	 * {@link Wallet#encrypt(com.google.bitcoin.crypto.KeyCrypter, org.spongycastle.crypto.params.KeyParameter)}
	 * which uses the default Scrypt key derivation algorithm and parameters,
	 * derives a key from the given password and returns the created key.
	 */
	...
	public void encrypt(KeyCrypter keyCrypter, KeyParameter aesKey) {
		...
		try {
			...
			checkState(getEncryptionType() == EncryptionType.UNENCRYPTED,
			...
			for (ECKey key : keychain) {
				if (key.isEncrypted()) {
					...
					// Check that the encrypted key can be successfully
					// decrypted.
					// This is done as it is a critical failure if the private
					// key cannot be decrypted successfully
					// (all bitcoin controlled by that private key is lost
					// forever).
					// For a correctly constructed keyCrypter the encryption
					// should always be reversible so it is just being as
					// cautious as possible.
					if (!ECKey.encryptionIsReversible(key, encryptedKey,
							keyCrypter, aesKey)) {
						// Abort encryption
						throw new KeyCrypterException(
								"The key "
			...
			// Now ready to use the encrypted keychain so go through the old
			// keychain clearing all the unencrypted private keys.
	...
	public void decrypt(KeyParameter aesKey) {
		lock.lock();
		try {
			// Check the wallet is already encrypted - you cannot decrypt an
			// unencrypted wallet.
			checkState(getEncryptionType() != EncryptionType.UNENCRYPTED,
					"Wallet is already decrypted");
			// Check that the wallet keyCrypter is non-null.
			// This is set either at construction (if an encrypted wallet is
			// created) or by wallet encryption.
			checkNotNull(keyCrypter);
	 ...
	 * 
	 * @param keyCrypter
	 *            The keyCrypter to use in encrypting the new key
	 * @param aesKey
	 *            The AES key to use to encrypt the new key
	 * @return ECKey the new, encrypted ECKey
	 */
	public ECKey addNewEncryptedKey(KeyCrypter keyCrypter, KeyParameter aesKey) {
		ECKey newKey = (new ECKey()).encrypt(checkNotNull(keyCrypter),
				checkNotNull(aesKey));
		addKey(newKey);
		return newKey;
	}
	 ...
	 * <p>
	 * Convenience wrapper around
	 * {@link Wallet#addNewEncryptedKey(com.google.bitcoin.crypto.KeyCrypter, org.spongycastle.crypto.params.KeyParameter)}
	 * which just derives the key afresh and uses the pre-set keycrypter. The
	 * wallet must have been encrypted using one of the encrypt methods
	 * previously.
	 * </p>
	 * 
	 * <p>
	 * Note that key derivation is deliberately very slow! So if you plan to add
	 * multiple keys, it can be faster to use the other method instead and
	 * re-use the {@link KeyParameter} object instead.
	 * </p>
	 */
	public ECKey addNewEncryptedKey(CharSequence password) {
		...
		try {
			checkNotNull(keyCrypter,
					"Wallet is not encrypted, you must call encrypt() first.");
			return addNewEncryptedKey(keyCrypter,
	...
	public boolean checkPassword(CharSequence password) {
		...
		try {
			return keyCrypter != null
					&& checkAESKey(keyCrypter.deriveKey(checkNotNull(password)));
		} finally {
			lock.unlock();
		}
	}
	...
	public boolean checkAESKey(KeyParameter aesKey) {
		...
		try {
			...
			if (firstEncryptedECKey == null)
				return false;
			String originalAddress = firstEncryptedECKey.toAddress(
					getNetworkParameters()).toString();
			if (firstEncryptedECKey.isEncrypted()
					&& firstEncryptedECKey.getEncryptedPrivateKey() != null) {
				try {
					ECKey rebornKey = firstEncryptedECKey.decrypt(keyCrypter,
							aesKey);

					// Check that the decrypted private key's address is correct
					// ie it decrypted accurately.
					String rebornAddress = rebornKey.toAddress(
							getNetworkParameters()).toString();
	...
	/**
	 * Get the wallet's KeyCrypter. (Used in encrypting/ decrypting an ECKey).
	 */
	...
	/**
	 * Sets the wallet's KeyCrypter. Note that this does not encrypt the wallet,
	 * and should only be used if the keyCrypter can not be included in the
	 * constructor during initial wallet loading. Note that if the keyCrypter
	 * was not properly set during wallet load,
	 * {@link Wallet#getEncryptionType()} and {@link Wallet#isEncrypted()} will
		...
		for (Transaction tx : getTransactions(false)) {
			for (TransactionOutput out : tx.getOutputs()) {
				try {
					...
					throw new RuntimeException(e); // If it is ours, we parsed
													// the script correctly, so
													// this shouldn't happen
				}
			}
		}

		// Some scripts may have more than one bloom element. That should
		// normally be okay,
	...
	/**
	 * If we are watching any scripts, the bloom filter must update on peers
	 * whenever an output is identified. This is because we don't necessarily
	 * have the associated pubkey, so we can't watch for it on spending
	 * transactions.
	 */
	@Override
	...
	/**
	 * Gets a bloom filter that contains all of the public keys from this
	 * wallet, and which will provide the given false-positive rate. See the
	 * docs for {@link BloomFilter} for a brief explanation of anonymity when
	 * using filters.
	 */
	...
	/**
	 * Gets a bloom filter that contains all of the public keys from this
	 * wallet, and which will provide the given false-positive rate if it has
	 * size elements. Keep in mind that you will get 2 elements in the bloom
	 * filter for each key in the wallet.
	 * 
	 * This is used to generate a BloomFilter which can be #{link
	 * BloomFilter.merge}d with another. It could also be used if you have a
	 * specific target for the filter's size.
	 * 
	 * See the docs for {@link BloomFilter(int, double)} for a brief explanation
	 * of anonymity when using bloom filters.
	 */
	@Override
		...
		try {
			...
			for (Script script : watchedScripts) {
				for (ScriptChunk chunk : script.getChunks()) {
					...
					// If any long constants become popular in scripts, we will
					// need logic
					// here to exclude them.
					if (!chunk.isOpCode()
							&& chunk.data.length >= MINIMUM_BLOOM_DATA_LENGTH) {
	 ...
	 * {@link Wallet.SendRequest#coinSelector}.
	 */
	...
	@GuardedBy("lock")
	private List<BalanceFutureRequest> balanceFutureRequests = Lists
			.newLinkedList();
	 ...
	 * large enough balance the future is returned in a pre-completed state.
	 * Note that this method is not blocking, if you want to actually wait
	 * immediately, you have to call .get() on the result.
	 * </p>
	 * 
	 * <p>
	 ...
	 * {@link com.google.bitcoin.utils.Threading#waitForUserCode()} to block
	 * until the future had a chance to be updated.
	 * </p>
	 */
	public ListenableFuture<BigInteger> getBalanceFuture(
			final BigInteger value, final BalanceType type) {
		lock.lock();
		try {
			...
			if (current.compareTo(value) >= 0) {
				...
				// Will be checked later in checkBalanceFutures. We don't just
				// add an event listener for ourselves
				// here so that running getBalanceFuture().get() in the user
				// code thread works - generally we must
				// avoid giving the user back futures that require the user code
				// thread to be free.
	...
	private void checkBalanceFuturesLocked(@Nullable BigInteger avail) {
		...
		final ListIterator<BalanceFutureRequest> it = balanceFutureRequests
				.listIterator();
		while (it.hasNext()) {
		...
		try {
			if (extensions.containsKey(id))
				throw new IllegalStateException(
						"Cannot add two extensions with the same ID: " + id);
	 ...
	 * one with the same id alreadypresent.
	 */
	...
	// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	// Boilerplate for running event listeners - dispatches events onto the user
	// code thread (where we don't do
		...
		for (final ListenerRegistration<WalletEventListener> registration : eventListeners) {
			if (registration.executor == Threading.SAME_THREAD) {
				...
				registration.executor.execute(new Runnable() {
					@Override
					public void run() {
						registration.listener.onTransactionConfidenceChanged(
								Wallet.this, tx);
					}
				});
			}
	...
	private void queueOnCoinsReceived(final Transaction tx,
			final BigInteger balance, final BigInteger newBalance) {
		checkState(lock.isHeldByCurrentThread());
		for (final ListenerRegistration<WalletEventListener> registration : eventListeners) {
			registration.executor.execute(new Runnable() {
				@Override
				public void run() {
					registration.listener.onCoinsReceived(Wallet.this, tx,
							balance, newBalance);
				}
			});
		}
	...
	private void queueOnCoinsSent(final Transaction tx,
		...
		for (final ListenerRegistration<WalletEventListener> registration : eventListeners) {
			registration.executor.execute(new Runnable() {
				@Override
				public void run() {
					registration.listener.onCoinsSent(Wallet.this, tx,
	...
	private class FeeCalculation {
		...
		public FeeCalculation(SendRequest req, BigInteger value,
			...
			// add inputs as we go up the list (keeping track of minimum inputs
			// for each category). At the end, we pick
			// the best input set as the one which generates the lowest total
			// fee.
			...
			// but only if the act of adding inputs and
			// change resulted in the size crossing a 1000 byte boundary.
			...
			while (true) {
				...
				if (lastCalculatedSize > 0) {
					// If the size is exactly 1000 bytes then we'll over-pay,
					// but this should be rare.
					fees = fees.add(BigInteger.valueOf(
							(lastCalculatedSize / 1000) + 1).multiply(
							req.feePerKb));
				} else {
					fees = fees.add(req.feePerKb); // First time around the
													// loop.
				}
				...
				if (additionalValueForNextCategory != null)
					valueNeeded = valueNeeded
							.add(additionalValueForNextCategory);
				...
				if (req.ensureMinRequiredFee
					...
					// If the change is smaller than the fee we want to add,
				...
				if (change.signum() > 0) {
					...
					// send. Just like in real life then,
					...
					// getChangeAddress() as a default.
					Address changeAddress = req.changeAddress;
					if (changeAddress == null)
						changeAddress = getChangeAddress();
					changeOutput = new TransactionOutput(params, req.tx,
							change, changeAddress);
					// If the change output would result in this transaction
					// being rejected as dust, just drop the change and make it
					// a fee
					if (req.ensureMinRequiredFee
							&& Transaction.MIN_NONDUST_OUTPUT.compareTo(change) >= 0) {
						...
						size += changeOutput.bitcoinSerialize().length
								+ VarInt.sizeOf(req.tx.getOutputs().size())
				...
				if (isCategory3) {
					...
					checkState(additionalValueForNextCategory
							.equals(Utils.CENT));
					...
					// out of the loop because we can't do better
				...
				if (additionalValueForNextCategory != null) {
					if (additionalValueSelected != null)
						checkState(additionalValueForNextCategory
								.compareTo(additionalValueSelected) > 0);
					continue;
				}
				break;
			}
			...
			if (selection3 == null && selection2 == null && selection1 == null) {
				...
				log.warn(
						"Insufficient value in wallet for send: needed {} more",
			...
			if (selection1 != null) {
				if (selection1Change != null)
					lowestFee = selection1.valueGathered
							.subtract(selection1Change.getValue());
				else
					lowestFee = selection1.valueGathered;
			...
			if (selection2 != null) {
				BigInteger fee = selection2.valueGathered
						.subtract(checkNotNull(selection2Change).getValue());
			...
			if (selection3 != null) {
				if (lowestFee == null
						|| selection3.valueGathered.compareTo(lowestFee) < 0) {
		...
		private void resetTxInputs(SendRequest req,
				List<TransactionInput> originalInputs) {
			req.tx.clearInputs();
		...
		for (TransactionOutput output : selection.gathered) {
			try {
				if (output.getScriptPubKey().isSentToAddress()) {
					// Send-to-address spends usually take maximum pubkey.length
					// (as it may be compressed or not) + 75 bytes
					final ECKey key = findKeyFromPubHash(output
							.getScriptPubKey().getPubKeyHash());
					size += checkNotNull(key,
							"Coin selection includes unspendable outputs")
							.getPubKey().length + 75;
				} else if (output.getScriptPubKey().isSentToRawPubKey())
					size += 74; // Send-to-pubkey spends usually take maximum 74
								// bytes to spend
				else
					throw new IllegalStateException(
							"Unknown output type returned in coin selection");
			} catch (ScriptException e) {
	 ...
	 * specified then the wallet won't attempt to broadcast transactions itself.
	 * </p>
	 * 
	 * <p>
	 ...
	 * wallet may choose to optimise itself to reduce fees or improve privacy.
	 * </p>
	 */
	public void setTransactionBroadcaster(
			@Nullable com.google.bitcoin.core.TransactionBroadcaster broadcaster) {
		...
		// Now use it to upload any pending transactions we have that are marked
		// as not being seen by any peers yet.
		// Don't hold the wallet lock whilst doing this, so if the broadcaster
		// accesses the wallet at some point there
		// is no inversion.
		for (Transaction tx : toBroadcast) {
			checkState(tx.getConfidence().getConfidenceType() == ConfidenceType.PENDING);
			// Re-broadcast even if it's marked as already seen for two reasons
			// 1) Old wallets may have transactions marked as broadcast by 1
			// peer when in reality the network
	 ...
	 * come in after calling this method will be automatically respent
	 * immediately. The rotation time is persisted to the wallet. You can stop
	 * key rotation by calling this method again with zero as the argument, or
	 * by using {@link #setKeyRotationEnabled(boolean)}.
	 * </p>
	 * 
	 * <p>
	 * Note that this method won't do anything unless you call
	 * {@link #setKeyRotationEnabled(boolean)} first.
	 * </p>
	 */
	 ...
	 * Activating it can trigger tx sends.
	 */
	...
	private void maybeRotateKeys() {
		...
		do {
			tx = rekeyOneBatch(keyRotationTimestamp, broadcaster);
				...
				&& tx.getInputs().size() == KeyTimeCoinSelector.MAX_SIMULTANEOUS_INPUTS);
	}

	@Nullable
	private Transaction rekeyOneBatch(long keyRotationTimestamp,
			final TransactionBroadcaster broadcaster) {
		final Transaction rekeyTx;

		lock.lock();
		try {
			// Firstly, see if we have any keys that are beyond the rotation
			// time, and any before.
			ECKey safeKey = null;
			...
			CoinSelector selector = new KeyTimeCoinSelector(this,
					keyRotationTimestamp, true);
			CoinSelection toMove = selector.select(BigInteger.ZERO,
					calculateAllSpendCandidates(true));
			...
			rekeyTx.getConfidence()
					.setSource(TransactionConfidence.Source.SELF);
		...
		// We must broadcast the tx in a separate thread to avoid inverting any
		// locks. Otherwise we may be running
		// with the blockchain lock held (whilst receiving a block) and thus
		// re-entering the peerGroup would invert
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					Futures.addCallback(
							broadcaster.broadcastTransaction(rekeyTx),
							new FutureCallback<Transaction>() {
								@Override
								public void onSuccess(Transaction transaction) {
									log.info(
											"Successfully broadcast key rotation tx: {}",
											transaction);
								}
					...
					log.error(
							"Failed to broadcast rekey tx, will try again later",
							e);
				}
			}
		}.start();
