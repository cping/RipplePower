...
public abstract class Message implements Serializable {
	...
	// The offset is how many bytes into the provided byte array this message
	// starts at.
	protected transient int offset;
	// The cursor keeps track of where we are in the byte array as we parse it.
	// Note that it's relative to the start of the array NOT the start of the
	// message.
	...
	Message(NetworkParameters params, byte[] msg, int offset,
	 ...
	 */
	Message(NetworkParameters params, byte[] msg, int offset,
			int protocolVersion, final boolean parseLazy,
	...
	Message(NetworkParameters params, byte[] msg, int offset)
			throws ProtocolException {
		this(params, msg, offset, NetworkParameters.PROTOCOL_VERSION, false,
				false, UNKNOWN_LENGTH);
	}

	Message(NetworkParameters params, byte[] msg, int offset,
			final boolean parseLazy, final boolean parseRetain, int length)
			throws ProtocolException {
		this(params, msg, offset, NetworkParameters.PROTOCOL_VERSION,
				parseLazy, parseRetain, length);
	}

	// These methods handle the serialization/deserialization using the custom
	// Bitcoin protocol.
	// It's somewhat painful to work with in Java, so some of these objects
	// support a second
	// serialization mechanism - the standard Java serialization system. This is
	// used when things
	// are serialized to the wallet.
	abstract void parse() throws ProtocolException;

	/**
	 * Perform the most minimal parse possible to calculate the length of the
	 * message. This is only required for subclasses of ChildClass as root level
	 * messages will have their length passed into the constructor.
	 * <p/>
	 * Implementations should adhere to the following contract: If parseLazy =
	 * true the 'length' field must be set before returning. If parseLazy =
	 * false the length field must be set either within the parseLite() method
	 * OR the parse() method. The overriding requirement is that length must be
	 * set to non UNKNOWN_MESSAGE value by the time the constructor exits.
	 * 
	 * @return
	 * @throws ProtocolException
	 */
	 ...
	 * This ensures any cached byte array is removed after performing a lazy
	 * parse if necessary to ensure the object is fully populated.
	 * <p/>
	 * Child messages of this object(e.g. Transactions belonging to a Block)
	 * will not have their internal byte caches invalidated unless they are also
	 * modified internally.
	 */
	...
	protected void adjustLength(int newArraySize, int adjustment) {
		if (length == UNKNOWN_LENGTH)
			return;
		// Our own length is now unknown if we have an unknown length
		// adjustment.
		...
		if (newArraySize == 1)
			length++; // The assumption here is we never call adjustLength with
						// the same arraySize as before.
		else if (newArraySize != 0)
			length += VarInt.sizeOf(newArraySize)
					- VarInt.sizeOf(newArraySize - 1);
	}
	...
	void setChecksum(byte[] checksum) {
		if (checksum.length != 4)
			throw new IllegalArgumentException(
					"Checksum length must be 4 bytes, actual length: "
							+ checksum.length);
	 ...
	 * {@link Message#unsafeBitcoinSerialize()}, which is safe to mutate. If you
	 * need extra performance and can guarantee you won't write to the array,
	 * you can use the unsafe version.
	 * 
	 ...
	 * protocol. <br/>
	 * This method may return the original byte array used to construct this
	 ...
	 * </ol>
	 * 
	 * If condition 3 is not met then an copy of the relevant portion of the
	 * array will be returned. Otherwise a full serialize will occur. For this
	 * reason you should only use this API if you can guarantee you will treat
	 * the resulting array as read only.
	 * 
	...
	public byte[] unsafeBitcoinSerialize() {
		...
		if (bytes != null) {
			if (offset == 0 && length == bytes.length) {
				// Cached byte array is the entire message with no extras so we
				// can return as is and avoid an array
				// copy.
				return bytes;
			}
		...
		if (parseRetain) {
			...
			// If there happens to be a call to this method we gain an
			// opportunity to recache
			// the byte array and in this case it contains no bytes from parent
			// messages.
			// This give a dual benefit. Releasing references to the larger byte
			// array so that it
			// it is more likely to be GC'd. And preventing double
			// serializations. E.g. calculating
			// merkle root calls this method. It is will frequently happen prior
			// to serializing the block
			// which means another call to bitcoinSerialize is coming. If we
			// didn't recache then internal
			// serialization would occur a 2nd time and every subsequent time
		...
		// Record length. If this Message wasn't parsed from a byte stream it
		// won't have length field
		// set (except for static length message types). Setting it makes future
		// streaming more efficient
		// because we can preallocate the ByteArrayOutputStream buffer and avoid
		// resizing.
	...
	/**
	 * Serialize this message to the provided OutputStream using the bitcoin
	 * wire format.
	 * 
	 * @param stream
	 * @throws IOException
	...
	/**
	 * This method is a NOP for all classes except Block and Transaction. It is
	 * only declared in Message so BitcoinSerializer can avoid 2 instanceof
	 * checks + a casting.
	 */
	...
	/**
	 * This should be overridden to extract correct message size in the case of
	 * lazy parsing. Until this method is implemented in a subclass of
	 * ChildMessage lazy parsing may have no effect.
	 * 
	 * This default implementation is a safe fall back that will ensure it
	 * returns a correct value by parsing the message.
	 */
	public int getMessageSize() {
		...
		if (length == UNKNOWN_LENGTH)
			checkState(false,
					"Length field has not been set in %s after full parse.",
