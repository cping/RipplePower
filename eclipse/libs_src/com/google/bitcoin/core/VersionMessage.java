 ...
 * nodes to see what they are running. If blank, it means the Satoshi client.
 * <p>
 * 
...
public class VersionMessage extends Message {
	...
	public VersionMessage(NetworkParameters params, int newBestHeight,
		...
		// Note that the official client doesn't do anything with these, and
		// finding out your own external IP address
		// is kind of tricky anyway, so we just put nonsense here for now.
		try {
			// We hard-code the IPv4 localhost address here rather than use
			// InetAddress.getLocalHost() because some
			// mobile phones have broken localhost DNS entries, also, this is
			// faster.
			final byte[] localhost = { 127, 0, 0, 1 };
			myAddr = new PeerAddress(InetAddress.getByAddress(localhost),
					params.getPort(), 0);
	...
	public boolean equals(Object o) {
		...
		return other.bestHeight == bestHeight
				...
				&& other.localServices == localServices && other.time == time
				&& other.subVer.equals(subVer) && other.myAddr.equals(myAddr)
				&& other.theirAddr.equals(theirAddr)
				&& other.relayTxesBeforeFilter == relayTxesBeforeFilter;
	}

	/**
	 * VersionMessage does not handle cached byte array so should not have a
	 * cached checksum.
	 */
	@Override
	byte[] getChecksum() {
	...
	@Override
	public int hashCode() {
		return (int) bestHeight ^ clientVersion ^ (int) localServices
				^ (int) time ^ subVer.hashCode() ^ myAddr.hashCode()
	...
	public String toString() {
		...
		sb.append("delay tx relay: ").append(relayTxesBeforeFilter)
				.append("\n");
		return sb.toString();
	}
	 ...
	 * might look like "/BitCoinJ:0.4-SNAPSHOT/MultiBit:1.2/" where libraries
	 * come further to the left.
	 * <p>
	 * 
	 ...
	 */
	public void appendToSubVer(String name, String version,
			@Nullable String comments) {
		checkSubVerComponent(name);
		checkSubVerComponent(version);
		if (comments != null) {
			checkSubVerComponent(comments);
			subVer = subVer.concat(String.format("%s:%s(%s)/", name, version,
					comments));
		} else {
