...
public class ECKey implements Serializable {
	...
	static {
		// All clients must agree on the curve to use by agreement. Bitcoin uses
		// secp256k1.
	 ...
	 */
	public ECKey(@Nullable EncryptedPrivateKey encryptedPrivateKey,
			@Nullable byte[] pubKey, KeyCrypter keyCrypter) {
	...
	public ECKey(@Nullable BigInteger privKey, @Nullable byte[] pubKey,
		...
		if (privKey == null && pubKey == null)
			throw new IllegalArgumentException(
					"ECKey requires at least private or public key");
		this.priv = privKey;
		this.pub = null;
		if (pubKey == null) {
	...
	public static class ECDSASignature {
		...
		public static ECDSASignature decodeFromDER(byte[] bytes) {
			try {
				...
				return new ECDSASignature(r.getPositiveValue(),
						s.getPositiveValue());
			} catch (IOException e) {
	 ...
	 * you want {@link com.google.bitcoin.core.ECKey.ECDSASignature#toASN1()}
	 ...
	 * In the Bitcoin protocol, they are usually encoded using DER format, so
	 * you want
	 * {@link com.google.bitcoin.core.ECKey.ECDSASignature#encodeToDER()}
	 * instead. However sometimes the independent components can be useful, for
	 * instance, if you're doing to do further EC maths on them.
	 * 
	 * @param aesKey
	...
	public ECDSASignature sign(Sha256Hash input, @Nullable KeyParameter aesKey)
		...
		if (isEncrypted()) {
			...
			if (aesKey == null) {
				throw new KeyCrypterException(
						"This ECKey is encrypted but no decryption key has been supplied.");
			}

			if (keyCrypter == null) {
				throw new KeyCrypterException(
						"There is no KeyCrypter to decrypt the private key for signing.");
			}

			privateKeyForSigning = new BigInteger(1, keyCrypter.decrypt(
					encryptedPrivateKey, aesKey));
			// Check encryption was correct.
			if (!Arrays.equals(pub,
					publicKeyFromPrivate(privateKeyForSigning, isCompressed())))
		...
		ECPrivateKeyParameters privKey = new ECPrivateKeyParameters(
				privateKeyForSigning, CURVE);
		signer.init(true, privKey);
		BigInteger[] components = signer.generateSignature(input.getBytes());
		final ECDSASignature signature = new ECDSASignature(components[0],
				components[1]);
		...
		try {
			...
			checkArgument(
					((DERInteger) seq.getObjectAt(0)).getValue().equals(
							BigInteger.ONE), "Input is of wrong version");
			Object obj = seq.getObjectAt(1);
			byte[] privbits = ((ASN1OctetString) obj).getOctets();
			decoder.close();
			BigInteger privkey = new BigInteger(1, privbits);
			byte[] pubbits = ((DERBitString) ((ASN1TaggedObject) seq
					.getObjectAt(3)).getObject()).getBytes();
	...
	public static ECKey signedMessageToKey(String message,
		...
		if (signatureEncoded.length < 65)
			throw new SignatureException(
					"Signature truncated, expected 65 bytes and got "
	...
	public static ECKey recoverFromSignature(int recId, ECDSASignature sig,
		...
		ECPoint.Fp q = (ECPoint.Fp) ECAlgorithms.sumOfTwoMultiplies(
	...
	private static ECPoint decompressKey(BigInteger xBN, boolean yBit) {
		...
		byte[] compEnc = x9.integerToBytes(xBN,
				1 + x9.getByteLength(CURVE.getCurve()));
	 ...
	 * {@link com.google.bitcoin.core.DumpedPrivateKey#toString()} method to get
	 * the string.
	 * 
	 * @param params
	...
	public void setCreationTimeSeconds(long newCreationTimeSeconds) {
		if (newCreationTimeSeconds < 0)
			throw new IllegalArgumentException(
					"Cannot set creation time to negative value: "
	 ...
	 * can definitely be decrypted successfully. See
	...
	public static boolean encryptionIsReversible(ECKey originalKey,
		...
		String genericErrorText = "The check that encryption could be reversed failed for key "
		...
		try {
			...
			if (rebornUnencryptedKey == null) {
				log.error(genericErrorText
						+ "The test decrypted key was missing.");
				return false;
			}
			...
			if (originalPrivateKeyBytes != null) {
				if (rebornUnencryptedKey.getPrivKeyBytes() == null) {
					...
					if (originalPrivateKeyBytes.length != rebornUnencryptedKey
						...
						log.error(genericErrorText
								+ "The test decrypted private key was a different length to the original.");
						return false;
					} else {
						for (int i = 0; i < originalPrivateKeyBytes.length; i++) {
							if (originalPrivateKeyBytes[i] != rebornUnencryptedKey
								...
								log.error(genericErrorText
										+ "Byte "
										+ i
										+ " of the private key did not match the original.");
								return false;
							}
						}
					}
