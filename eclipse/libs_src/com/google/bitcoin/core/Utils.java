...
public class Utils {
	...
	public static BigInteger toNanoCoins(int coins, int cents) {
		...
		checkArgument(coins < NetworkParameters.MAX_MONEY.divide(Utils.COIN)
				.longValue());
	 ...
	 *             if you try to specify fractional nanocoins, or nanocoins out
	 *             of range.
	 */
	public static BigInteger toNanoCoins(String coins) {
		BigInteger bigint = new BigDecimal(coins).movePointRight(8)
				.toBigIntegerExact();
		if (bigint.signum() < 0)
			throw new ArithmeticException("Negative coins specified");
		if (bigint.compareTo(NetworkParameters.MAX_MONEY) > 0)
			throw new ArithmeticException(
					"Amount larger than the total quantity of Bitcoins possible specified.");
		return bigint;
	}
	...
	public static void uint64ToByteStreamLE(BigInteger val, OutputStream stream)
		...
		if (bytes.length > 8) {
			throw new RuntimeException(
					"Input too large to encode into a uint64");
		}
	...
	public static byte[] reverseDwordBytes(byte[] bytes, int trimLength) {
		...
		byte[] rev = new byte[trimLength >= 0 && bytes.length > trimLength ? trimLength
				: bytes.length];
	...
	public static String bitcoinValueToPlainString(BigInteger value) {
		...
		BigDecimal valueInBTC = new BigDecimal(value).divide(new BigDecimal(
				Utils.COIN));
	...
	/**
	 * Returns the current time in seconds since the epoch, or a mocked out
	 * equivalent.
	 */
