...
/**
 * Handles high-level message (de)serialization for peers, acting as the bridge
 * between the {@link com.google.bitcoin.net} classes and {@link Peer}.
 */
public abstract class PeerSocketHandler extends AbstractTimeoutHandler
		implements StreamParser {
	private static final Logger log = LoggerFactory
			.getLogger(PeerSocketHandler.class);
	...
	// writeTarget will be thread-safe, and may call into PeerGroup, which calls
	// us, so we should call it unlocked
	@VisibleForTesting
	MessageWriteTarget writeTarget = null;

	// The ByteBuffers passed to us from the writeTarget are static in size, and
	// usually smaller than some messages we
	// will receive. For SPV clients, this should be rare (ie we're mostly
	// dealing with small transactions), but for
	// messages which are larger than the read buffer, we have to keep a
	// temporary buffer with its bytes.
	...
	/**
	 * Sends the given message to the peer. Due to the asynchronousness of
	 * network programming, there is no guarantee the peer will have received
	 * it. Throws NotYetConnectedException if we are not yet connected to the
	 * remote peer. TODO: Maybe use something other than the unchecked
	...
	/**
	 * Closes the connection to the peer if one exists, or immediately closes
	 * the connection as soon as it opens
	 */
	public void close() {
		lock.lock();
		try {
	...
	public int receiveBytes(ByteBuffer buff) {
		...
		try {
			// Repeatedly try to deserialize messages until we hit a
			// BufferUnderflowException
			for (int i = 0; true; i++) {
				// If we are in the middle of reading a message, try to fill
				// that one first, before we expect another
				if (largeReadBuffer != null) {
					...
					if (largeReadBufferPos == largeReadBuffer.length) {
						...
						processMessage(serializer.deserializePayload(header,
								ByteBuffer.wrap(largeReadBuffer)));
						largeReadBuffer = null;
						header = null;
					} else
						// ...or just returning if we don't have enough bytes
						// yet
				...
				try {
					...
					// If we went through the whole buffer without a full
					// message, we need to use the largeReadBuffer
					if (i == 0 && buff.limit() == buff.capacity()) {
						// ...so reposition the buffer to 0 and read the next
						// message header
						buff.position(0);
						try {
							serializer.seekPastMagicBytes(buff);
							header = serializer.deserializeHeader(buff);
							// Initialize the largeReadBuffer with the next
							// message's size and fill it with any bytes
							// left in buff
							largeReadBuffer = new byte[header.size];
							largeReadBufferPos = buff.remaining();
							buff.get(largeReadBuffer, 0, largeReadBufferPos);
						} catch (BufferUnderflowException e1) {
							// If we went through a whole buffer's worth of
							// bytes without getting a header, give up
							// In cases where the buff is just really small, we
							// could create a second largeReadBuffer
							// that we use to deserialize the magic+header, but
							// that is rather complicated when the buff
							// should probably be at least that big anyway (for
							// efficiency)
							throw new ProtocolException(
									"No magic bytes+header after reading "
											+ buff.capacity() + " bytes");
						}
					} else {
						// Reposition the buffer to its original position, which
						// saves us from skipping messages by
						// seeking past part of the magic bytes before all of
						// them are in the buffer
			...
			exceptionCaught(e);
			return -1; // Returning -1 also throws an IllegalStateException
						// upstream and kills the connection
		}
	}

	/**
	 * Sets the {@link MessageWriteTarget} used to write messages to the peer.
	 * This should almost never be called, it is called automatically by
	 * {@link com.google.bitcoin.net.NioClient} or
	 * {@link com.google.bitcoin.net.NioClientManager} once the socket finishes
	 * initialization.
	 */
	@Override
