...
/**
 * A protocol message that contains a repeated series of block headers, sent in
 * response to the "getheaders" command. This is useful when you want to
 * traverse the chain but know you don't care about the block contents, for
 * example, because you have a freshly created wallet with no keys.
 */
public class HeadersMessage extends Message {
	private static final Logger log = LoggerFactory
			.getLogger(HeadersMessage.class);
	...
	protected void parseLite() throws ProtocolException {
		if (length == UNKNOWN_LENGTH) {
			...
			// Each header has 80 bytes and one more byte for transactions
			// number which is 00.
	...
	void parse() throws ProtocolException {
		...
		for (int i = 0; i < numHeaders; ++i) {
			// Read 80 bytes of the header and one more byte for the transaction
			// list, which is always a 00 because the
			// transaction list is empty.
			byte[] blockHeader = readBytes(81);
			if (blockHeader[80] != 0)
				throw new ProtocolException(
						"Block header does not end with a null byte");
			Block newBlockHeader = new Block(this.params, blockHeader, true,
					true, 81);
