...
public class Address extends VersionedChecksummedBytes {
	 ...
	 * 
	 * <pre>
	 * new Address(NetworkParameters.prodNet(), NetworkParameters.getAddressHeader(),
	...
	public Address(NetworkParameters params, int version, byte[] hash160)
		...
		checkNotNull(params);
		checkArgument(hash160.length == 20,
				"Addresses are 160-bit hashes, so you must provide 20 bytes");
		if (!isAcceptableVersion(params, version))
			throw new WrongNetworkException(version,
					params.getAcceptableAddressCodes());
	}
	...
	/*
	 * Returns true if this address is a Pay-To-Script-Hash (P2SH) address. See
	 * also https://github.com/bitcoin/bips/blob/master/bip-0013.mediawiki:
	...
	public NetworkParameters getParameters() {
		...
		// networks.
		NetworkParameters[] networks = { TestNet3Params.get(),
				MainNetParams.get() };
	 ...
	 * decide if it is compatible with the current wallet.
	 * 
	 ...
	 */
	private static boolean isAcceptableVersion(NetworkParameters params,
			int version) {
