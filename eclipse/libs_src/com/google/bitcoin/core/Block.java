...
public class Block extends Message {
	...
	public BigInteger getBlockInflation(int height) {
		return Utils.toNanoCoins(50, 0).shiftRight(
				height / params.getSubsidyDecreaseBlockCount());
	}
		...
		if (h.compareTo(target) > 0) {
			...
			if (throwException)
				throw new VerificationException("Hash is higher than target: "
						+ getHashAsString() + " vs " + target.toString(16));
			else
				return false;
		}
		return true;
	...
	public Block createNextBlock(@Nullable Address to,
		...
		return createNextBlock(to, prevOut, Utils.currentTimeMillis() / 1000,
				EMPTY_BYTES, Utils.toNanoCoins(50, 0));
	}

	@VisibleForTesting
	public Block createNextBlock(@Nullable Address to, BigInteger value) {
		return createNextBlock(to, null, Utils.currentTimeMillis() / 1000,
				EMPTY_BYTES, value);
	}
	...
	public Block createNextBlockWithCoinbase(byte[] pubKey,
			BigInteger coinbaseValue) {
		return createNextBlock(null, null, Utils.currentTimeMillis() / 1000,
