...
public class TransactionOutput extends ChildMessage implements Serializable {
	private static final Logger log = LoggerFactory
			.getLogger(TransactionOutput.class);
	private static final long serialVersionUID = -590332479859256824L;

	// A transaction output has some value and a script used for authenticating
	// that the redeemer is allowed to spend
	// this output.
	...
	// These fields are Java serialized but not Bitcoin serialized. They are
	// used for tracking purposes in our wallet
	...
	public TransactionOutput(NetworkParameters params,
			@Nullable Transaction parent, byte[] payload, int offset)
	 ...
	 * @throws ProtocolException
	 */
	public TransactionOutput(NetworkParameters params, Transaction parent,
			byte[] msg, int offset, boolean parseLazy, boolean parseRetain)
			throws ProtocolException {
	 ...
	 * {@link Utils#toNanoCoins(int, int)}. Typically you would use
	 * {@link Transaction#addOutput(java.math.BigInteger, Address)} instead of
	 * creating a TransactionOutput directly.
	 */
	public TransactionOutput(NetworkParameters params, Transaction parent,
			BigInteger value, Address to) {
		this(params, parent, value, ScriptBuilder.createOutputScript(to)
				.getProgram());
	}
	 ...
	 * something like {@link Utils#toNanoCoins(int, int)}. Typically you would
	 * use {@link Transaction#addOutput(java.math.BigInteger, ECKey)} instead of
	 * creating an output directly.
	 */
	public TransactionOutput(NetworkParameters params, Transaction parent,
			BigInteger value, ECKey to) {
		this(params, parent, value, ScriptBuilder.createOutputScript(to)
				.getProgram());
	}

	public TransactionOutput(NetworkParameters params, Transaction parent,
			BigInteger value, byte[] scriptBytes) {
		super(params);
		...
		// here.
		checkArgument(value.signum() >= 0 || value.equals(Utils.NEGATIVE_ONE),
				"Negative values not allowed");
		checkArgument(value.compareTo(NetworkParameters.MAX_MONEY) < 0,
	...
	public Script getScriptPubKey() throws ScriptException {
		// Quick hack to try and reduce memory consumption on Androids.
		// SoftReference is the same as WeakReference
		// on Dalvik (by design), so this arrangement just means that we can
		// avoid the cost of re-parsing the script
		// bytes if getScriptPubKey is called multiple times in quick succession
	...
	protected void parseLite() throws ProtocolException {
		// TODO: There is no reason to use BigInteger for values, they are
		// always smaller than 21 million * COIN
		// The only reason to use BigInteger would be to properly read values
		// from the reference implementation, however
		// the reference implementation uses signed 64-bit integers for its
		// values as well (though it probably shouldn't)
	...
	protected void bitcoinSerializeToStream(OutputStream stream)
		...
		// TODO: Move script serialization into the Script class, where it
		// belongs.
	 ...
	 * by a reference client (and thus relayed). See: CTxOut::IsDust() in the
	 * reference client. The assumption is that any output that would consume
	 * more than a third of its value in fees is not something the Bitcoin
	 * system wants to deal with right now, so we call them "dust outputs" and
	 * they're made non standard. The choice of one third is somewhat arbitrary
	 * and may change in future.
	 * </p>
	 * 
	 * <p>
	 * You probably should use
	 * {@link com.google.bitcoin.core.TransactionOutput#getMinNonDustValue()}
	 * which uses a safe fee-per-kb by default.
	 * </p>
	 * 
	 * @param feePerKbRequired
	 *            The fee required per kilobyte. Note that this is the same as
	 *            the reference client's -minrelaytxfee * 3 If you want a safe
	 *            default, use {@link Transaction#REFERENCE_DEFAULT_MIN_TX_FEE}
	 *            *3
	 */
	public BigInteger getMinNonDustValue(BigInteger feePerKbRequired) {
		...
		// unfortunately, we must follow the reference
		// clients wrongness in order to ensure we're considered standard. A
		// better formula would either estimate the
		// size of data needed to satisfy all different script types, or just
		// hard code 33 below.
		final BigInteger size = BigInteger
				.valueOf(this.bitcoinSerialize().length + 148);
		BigInteger[] nonDustAndRemainder = feePerKbRequired.multiply(size)
				.divideAndRemainder(BigInteger.valueOf(1000));
	 ...
	 * normal pay to address outputs, this is 5460 satoshis, the same as
	...
	public String toString() {
		try {
			return "TxOut of " + Utils.bitcoinValueToFriendlyString(value)
					+ " to "
	...
	/**
	 * Returns the transaction that owns this output, or throws
	 * NullPointerException if unowned.
	 */
	public Transaction getParentTransaction() {
		return checkNotNull(parentTransaction,
				"Free-standing TransactionOutput");
	}
