...
/**
 * Represents an "addr" message on the P2P network, which contains broadcast IP
 * addresses of other peers. This is one of the ways peers can find each other
 * without using the DNS or IRC discovery mechansisms. However storing and using
 * addr messages is not presently implemented.
 */
public class AddressMessage extends Message {
	 ...
	 * @throws ProtocolException
	 */
	AddressMessage(NetworkParameters params, byte[] payload, boolean parseLazy,
	...
	void parse() throws ProtocolException {
		...
		for (int i = 0; i < numAddresses; i++) {
			PeerAddress addr = new PeerAddress(params, bytes, cursor,
					protocolVersion, this, parseLazy, parseRetain);
			addresses.add(addr);
	...
	public int getMessageSize() {
		...
		if (addresses != null) {
			...
			// The 4 byte difference is the uint32 timestamp that was introduced
			// in version 31402
			length += addresses.size()
					* (protocolVersion > 31402 ? PeerAddress.MESSAGE_SIZE
							: PeerAddress.MESSAGE_SIZE - 4);
		}
		return length;
	}

	/**
	 * AddressMessage cannot cache checksum in non-retain mode due to dynamic
	 * time being used.
	 */
	@Override
	...
	/**
	 * @return An unmodifiableList view of the backing List of addresses.
	 *         Addresses contained within the list may be safely modified.
	 */
