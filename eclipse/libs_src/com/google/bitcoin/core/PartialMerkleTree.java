...
/**
 * <p>
 * A data structure that contains proofs of block inclusion for one or more
 ...
 * we recurse into both (or the only) child branch. During decoding, the same
 ...
 * size, <tt>SIZE <= 10 + ceil(32.25*N)</tt> where N represents the number of
...
public class PartialMerkleTree extends Message {
	...
	public PartialMerkleTree(NetworkParameters params, byte[] payloadBytes,
	...
	private Sha256Hash recursiveExtractHashes(int height, int pos,
			ValuesUsed used, List<Sha256Hash> matchedHashes)
			throws VerificationException {
		if (used.bitsUsed >= matchedChildBits.length * 8) {
			// overflowed the bits array - failure
			throw new VerificationException(
					"CPartialMerkleTree overflowed its bits array");
		}
		boolean parentOfMatch = Utils.checkBitLE(matchedChildBits,
				used.bitsUsed++);
		if (height == 0 || !parentOfMatch) {
			...
			if (used.hashesUsed >= hashes.size()) {
				...
				throw new VerificationException(
						"CPartialMerkleTree overflowed its hash array");
			}
			if (height == 0 && parentOfMatch) // in case of height 0, we have a
												// matched txid
			...
			// hashes
			byte[] left = recursiveExtractHashes(height - 1, pos * 2, used,
					matchedHashes).getBytes(), right;
			if (pos * 2 + 1 < getTreeWidth(height - 1))
				right = recursiveExtractHashes(height - 1, pos * 2 + 1, used,
						matchedHashes).getBytes();
			else
				right = left;
			// and combine them before returning
			return new Sha256Hash(Utils.reverseBytes(Utils
					.doubleDigestTwoBuffers(Utils.reverseBytes(left), 0, 32,
	...
	public Sha256Hash getTxnHashAndMerkleRoot(List<Sha256Hash> matchedHashes)
		...
		if (transactionCount > Block.MAX_BLOCK_SIZE / 60) // 60 is the lower
															// bound for the
															// size of a
															// serialized
															// CTransaction
		...
		Sha256Hash merkleRoot = recursiveExtractHashes(height, 0, used,
				matchedHashes);
