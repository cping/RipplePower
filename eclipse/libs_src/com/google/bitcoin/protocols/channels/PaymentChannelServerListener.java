...
public class PaymentChannelServerListener {
	...
	// The event handler factory which creates new ServerConnectionEventHandler
	// per connection
	...
	public static interface HandlerFactory {
		 ...
		 * connection-specific listener. If null is returned, the connection is
		 * immediately closed.
		 */
		@Nullable
	...
	private class ServerHandler {
		public ServerHandler(final SocketAddress address,
				final int timeoutSeconds) {
			paymentChannelManager = new PaymentChannelServer(broadcaster,
					wallet, minAcceptedChannelSize,
			...
			protobufHandlerListener = new ProtobufParser.Listener<Protos.TwoWayChannelMessage>() {
				...
				public synchronized void connectionClosed(ProtobufParser handler) {
					...
					if (closeReason != null)
						...
						eventHandler
								.channelClosed(PaymentChannelCloseException.CloseReason.CONNECTION_CLOSED);
					eventHandler.setConnectionChannel(null);
				}

				@Override
				public synchronized void connectionOpen(ProtobufParser handler) {
					ServerConnectionEventHandler eventHandler = eventHandlerFactory
		...
		// The payment channel server which does the actual payment channel
		// handling
	 ...
	 * 
	 * @throws Exception
	 *             If binding to the given port fails (eg SocketException:
	...
	public void bindAndStart(int port) throws Exception {
		server = new NioServer(new StreamParserFactory() {
			...
			public ProtobufParser getNewParser(InetAddress inetAddress, int port) {
				return new ServerHandler(new InetSocketAddress(inetAddress,
						port), timeoutSeconds).socketProtobufHandler;
			}
	 ...
	 *            The read timeout between messages. This should accommodate
	 *            latency and client ECDSA signature operations.
	 * @param minAcceptedChannelSize
	 *            The minimum amount of coins clients must lock in to create a
	 *            channel. Clients which are unwilling or unable to lock in at
	 *            least this value will immediately disconnect. For this reason,
	 *            a fairly conservative value (in terms of average value spent
	 *            on a channel) should generally be chosen.
	 * @param eventHandlerFactory
	 *            A factory which generates event handlers which are created for
	 *            each new connection
	 */
	public PaymentChannelServerListener(TransactionBroadcaster broadcaster,
			Wallet wallet, final int timeoutSeconds,
	 ...
	 * 
	 * <p>
	 * Note that this does <i>not</i> settle the actual payment channels (and
	 * broadcast payment transactions), which must be done using the
	 * {@link StoredPaymentChannelServerStates} which manages the states for the
	 * associated wallet.
	 * </p>
	 */
	public void close() {
