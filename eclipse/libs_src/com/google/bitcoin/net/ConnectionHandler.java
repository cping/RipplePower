...
/**
 * A simple NIO MessageWriteTarget which handles all the business logic of a
 * connection (reading+writing bytes). Used only by the NioClient and NioServer
 * classes
 */
class ConnectionHandler implements MessageWriteTarget {
	...
	private static final int OUTBOUND_BUFFER_BYTE_COUNT = Message.MAX_SIZE + 24; // 24
																					// byte
																					// message
																					// header

	// We lock when touching local flags and when writing data, but NEVER when
	// calling any methods which leave this
	...
	public ConnectionHandler(StreamParserFactory parserFactory, SelectionKey key)
			throws IOException {
		this(parserFactory.getNewParser(((SocketChannel) key.channel())
	...
	private ConnectionHandler(@Nullable StreamParser parser, SelectionKey key) {
		...
		this.parser = parser;
		readBuff = ByteBuffer.allocateDirect(Math.min(
				Math.max(parser.getMaxMessageSize(), BUFFER_SIZE_LOWER_BOUND),
				BUFFER_SIZE_UPPER_BOUND));
		parser.setWriteTarget(this); // May callback into us (eg
										// closeConnection() now)
	...
	public ConnectionHandler(StreamParser parser, SelectionKey key,
		...
		// closeConnection() may have already happened, in which case we
		// shouldn't add ourselves to the connectedHandlers set
		lock.lock();
	...
	private void setWriteOps() {
		// Make sure we are registered to get updated when writing is available
		// again
	...
	// Tries to write any outstanding write bytes, runs in any thread (possibly
	// unlocked)
	private void tryWriteBytes() throws IOException {
		lock.lock();
		try {
			// Iterate through the outbound ByteBuff queue, pushing as much as
			// possible into the OS' network buffer.
			...
			if (bytesToWrite.isEmpty())
				key.interestOps(key.interestOps() & ~SelectionKey.OP_WRITE);
			// Don't bother waking up the selector here, since we're just
			// removing an op, not adding
		} finally {
			lock.unlock();
		}
	}
	...
	public void writeBytes(byte[] message) throws IOException {
		lock.lock();
		try {
			// Network buffers are not unlimited (and are often smaller than
			// some messages we may wish to send), and
			// thus we have to buffer outbound messages sometimes. To do this,
			// we use a queue of ByteBuffers and just
			// append to it when we want to send a message. We then let
			// tryWriteBytes() either send the message or
			// register our SelectionKey to wakeup when we have free outbound
			// buffer space available.

			if (bytesToWriteRemaining + message.length > OUTBOUND_BUFFER_BYTE_COUNT)
				throw new IOException("Outbound buffer overflowed");
			// Just dump the message onto the write buffer and call
			// tryWriteBytes
			// TODO: Kill the needless message duplication when the write
			// completes right away
			bytesToWrite.offer(ByteBuffer.wrap(Arrays.copyOf(message,
					message.length)));
	...
	private void connectionClosed() {
		...
		if (callClosed) {
			checkState(connectedHandlers == null
	...
	// Runs unlocked as the caller is single-threaded (or if not, should enforce
	// that handleKey is only called
	...
	public static void handleKey(SelectionKey key) {
		...
		try {
			if (handler == null)
				return;
			if (!key.isValid()) {
				handler.closeConnection(); // Key has been cancelled, make sure
			...
			if (key.isReadable()) {
				...
				// and setting position to 0
				handler.readBuff.flip();
				// Use parser.receiveBytes's return value as a check that it
				// stopped reading at the right location
				int bytesConsumed = checkNotNull(handler.parser).receiveBytes(
						handler.readBuff);
				checkState(handler.readBuff.position() == bytesConsumed);
				// Now drop the bytes which were read by compacting readBuff
				// (resetting limit and keeping relative
				// position)
			...
			// This can happen eg if the channel closes while the thread is
			// about to get killed
			// (ClosedByInterruptException), or if handler.parser.receiveBytes
			// throws something
			log.error("Error handling SelectionKey: {}", Throwables
