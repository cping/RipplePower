...
/**
 * A class which manages a set of client connections. Uses Java NIO to select
 * network events and processes them in a single network processing thread.
 */
public class NioClientManager extends AbstractExecutionThreadService implements
	...
	// SocketChannels and StreamParsers of newly-created connections which
	// should be registered with OP_CONNECT
	...
	// Added to/removed from by the individual ConnectionHandler's, thus must by
	...
	private void handleKey(SelectionKey key) throws IOException {
		// We could have a !isValid() key here if the connection is already
		// closed at this point
		if (key.isValid() && key.isConnectable()) { // ie a client connection
													// which has finished the
			...
			ConnectionHandler handler = new ConnectionHandler(parser, key,
					connectedHandlers);
			try {
				if (sc.finishConnect()) {
					log.info("Successfully connected to {}", sc.socket()
							.getRemoteSocketAddress());
					key.interestOps(SelectionKey.OP_READ).attach(handler);
					handler.parser.connectionOpened();
				} else {
					log.error("Failed to connect to {}", sc.socket()
							.getRemoteSocketAddress());
					handler.closeConnection(); // Failed to connect for some
												// reason
				}
			} catch (Exception e) {
				// If e is a CancelledKeyException, there is a race to get to
				// interestOps after finishConnect() which
				// may cause this. Otherwise it may be any arbitrary kind of
				// connection failure.
				// Calling sc.socket().getRemoteSocketAddress() here throws an
				// exception, so we can only log the error itself
				log.error("Failed to connect with exception: {}", Throwables
		...
		try {
			...
			while (isRunning()) {
				...
				while ((conn = newConnectionChannels.poll()) != null) {
					try {
						SelectionKey key = conn.sc.register(selector,
			...
			// Go through and close everything, without letting IOExceptions get
			// in our way
			for (SelectionKey key : selector.keys()) {
				...
				if (key.attachment() instanceof ConnectionHandler)
					ConnectionHandler.handleKey(key); // Close connection if
														// relevant
			}
			try {
				selector.close();
	...
	public void openConnection(SocketAddress serverAddress, StreamParser parser) {
		...
		try {
			...
			throw new RuntimeException(e); // This should only happen if we are,
											// eg, out of system resources
		}
	}
	...
	public void closeConnections(int n) {
		while (n-- > 0) {
			...
			if (handler != null)
				handler.closeConnection(); // Removes handler from
											// connectedHandlers before
											// returning
		}
	}
}
