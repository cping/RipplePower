 ...
 * {@link DnsDiscovery#getPeers(long, java.util.concurrent.TimeUnit)} will
 * return up to 30 random peers from the set of those returned within the
 * timeout period. If you want more peers to connect to, you need to discover
 * them via other means (like addr broadcasts).
 * </p>
 */
public class DnsDiscovery implements PeerDiscovery {
	private static final Logger log = LoggerFactory
			.getLogger(DnsDiscovery.class);
	 ...
	 * 
	 * @param hostNames
	 *            Host names to be examined for seed addresses.
	 * @param netParams
	 *            Network parameters to be used for port information.
	 */
	...
	public InetSocketAddress[] getPeers(long timeoutValue, TimeUnit timeoutUnit)
		...
		// Java doesn't have an async DNS API so we have to do all lookups in a
		// thread pool, as sometimes seeds go
		// hard down and it takes ages to give up and move on.
		ExecutorService threadPool = Executors
				.newFixedThreadPool(hostNames.length);
		try {
			...
			final List<Future<InetAddress[]>> futures = threadPool.invokeAll(
					tasks, timeoutValue, timeoutUnit);
			...
			for (int i = 0; i < futures.size(); i++) {
				...
				try {
					...
					log.error("Failed to look up DNS seeds from {}: {}",
							hostNames[i], e.getMessage());
					continue;
				}
			...
			if (addrs.size() == 0)
				throw new PeerDiscoveryException(
						"Unable to find any peers via DNS");
