...
/**
 * <p>
 * Creates a simple connection to a server using a {@link StreamParser} to
 * process data.
 * </p>
 * 
 * <p>
...
public class BlockingClient implements MessageWriteTarget {
	private static final org.slf4j.Logger log = LoggerFactory
			.getLogger(BlockingClient.class);
	 ...
	 * {@link StreamParser} to decode the data. The given parser <b>MUST</b> be
	 * unique to this object. This does not block while waiting for the
	 * connection to open, but will call either the
	 * {@link StreamParser#connectionOpened()} or
	 * {@link StreamParser#connectionClosed()} callback on the created network
	 ...
	 */
	public BlockingClient(final SocketAddress serverAddress,
			final StreamParser parser, final int connectTimeoutMillis,
		...
		dbuf = ByteBuffer.allocateDirect(Math.min(
			...
			public void run() {
				...
				try {
					...
					while (true) {
						...
						checkState(dbuf.remaining() > 0
								&& dbuf.remaining() <= readBuff.length);
						int read = stream.read(
								readBuff,
								0,
								Math.max(
										1,
										Math.min(dbuf.remaining(),
												stream.available())));
						if (read == -1)
							return;
						dbuf.put(readBuff, 0, read);
						// "flip" the buffer - setting the limit to the current
						// position and setting position to 0
						dbuf.flip();
						// Use parser.receiveBytes's return value as a
						// double-check that it stopped reading at the right
						// location
						int bytesConsumed = parser.receiveBytes(dbuf);
						checkState(dbuf.position() == bytesConsumed);
						// Now drop the bytes which were read by compacting dbuf
					...
					if (!vCloseRequested)
						log.error("Error trying to open/read from connection: "
								+ serverAddress, e);
				} finally {
					try {
						socket.close();
	 ...
	 * {@link StreamParser#connectionClosed()} event on the network-handling
