...
/**
 * This class implements a {@link com.google.bitcoin.wallet.CoinSelector} which
 * attempts to get the highest priority possible. This means that the
 * transaction is the most likely to get confirmed. Note that this means we may
 * end up "spending" more priority than would be required to get the transaction
 * we are creating confirmed.
 */
public class DefaultCoinSelector implements CoinSelector {
	...
	static void sortOutputs(ArrayList<TransactionOutput> outputs) {
		Collections.sort(outputs, new Comparator<TransactionOutput>() {
			public int compare(TransactionOutput a, TransactionOutput b) {
				...
				BigInteger aCoinDepth = aValue.multiply(BigInteger
						.valueOf(depth1));
				BigInteger bCoinDepth = bValue.multiply(BigInteger
						.valueOf(depth2));
	...
	public static boolean isSelectable(Transaction tx) {
		...
		return type.equals(TransactionConfidence.ConfidenceType.BUILDING)
				...
				// In regtest mode we expect to have only one peer, so we won't
				// see transactions propagate.
				// TODO: The value 1 below dates from a time when transactions
				// we broadcast *to* were counted, set to 0
				(confidence.numBroadcastPeers() > 1 || tx.getParams() == RegTestParams
						.get());
	}
}
