...
/**
 * <p>
 * The default risk analysis. Currently, it only is concerned with whether a
 * tx/dependency is non-final or not, and whether a tx/dependency violates the
 * dust rules. Outside of specialised protocols you should not encounter
 * non-final transactions.
 * </p>
 */
public class DefaultRiskAnalysis implements RiskAnalysis {
	private static final Logger log = LoggerFactory
			.getLogger(DefaultRiskAnalysis.class);
	...
	private DefaultRiskAnalysis(Wallet wallet, Transaction tx,
	...
	private Result analyzeIsFinal() {
		...
		// If the transaction has a lock time specified in blocks, we consider
		// that if the tx would become final in the
	...
	/**
	 * <p>
	 * Checks if a transaction is considered "standard" by the reference
	 * client's IsStandardTx and AreInputsStandard functions.
	 * </p>
	 * 
	 * <p>
	 * Note that this method currently only implements a minimum of checks. More
	 * to be added later.
	 * </p>
	 */
	public static RuleViolation isStandard(Transaction tx) {
		...
		if (tx.getVersion() > 1 || tx.getVersion() < 1) {
			log.warn(
					"TX considered non-standard due to unknown version number {}",
					tx.getVersion());
		...
		for (int i = 0; i < outputs.size(); i++) {
			...
			if (output.getMinNonDustValue().compareTo(output.getValue()) > 0) {
				log.warn(
						"TX considered non-standard due to output {} being dusty",
						i);
	...
	private Result analyzeIsStandard() {
		// The IsStandard rules don't apply on testnet, because they're just a
		// safety mechanism and we don't want to
		// crush innovation with valueless test coins.
		if (!wallet.getNetworkParameters().getId()
	...
	public String toString() {
		if (!analyzed)
			...
			return "Risky due to non-standard tx "
					+ nonStandard.getHashAsString();
		else
			return "Non-risky";
	}
	...
	public static class Analyzer implements RiskAnalysis.Analyzer {
		@Override
		public DefaultRiskAnalysis create(Wallet wallet, Transaction tx,
				List<Transaction> dependencies) {
