 ...
 * @see <a
 *      href="https://github.com/bitcoin/bips/blob/master/bip-0021.mediawiki">BIP
 *      0021</a>
 */
public class BitcoinURI {
	...
	public BitcoinURI(@Nullable NetworkParameters params, String input)
			throws BitcoinURIParseException {
		checkNotNull(input);
		log.debug("Attempting to parse '{}' for {}", input,
				params == null ? "any" : params.getId());
		...
		if (addressSplitTokens.length == 0)
			throw new BitcoinURIParseException(
					"No data found after the bitcoin: prefix");
		...
		if (addressSplitTokens.length == 1) {
			...
			if (addressSplitTokens.length == 2) {
				...
				throw new BitcoinURIParseException(
						"Too many question marks in URI '" + uri + "'");
			}
		}
		...
		if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
			throw new BitcoinURIParseException(
					"No address and no r= parameter found");
		}
	}
	...
	private void parseParameters(@Nullable NetworkParameters params,
			String addressToken, String[] nameValuePairTokens)
			throws BitcoinURIParseException {
		...
		for (String nameValuePairToken : nameValuePairTokens) {
			...
			if (tokens.length != 2 || "".equals(tokens[0])) {
				throw new BitcoinURIParseException(
						"Malformed Bitcoin URI - cannot parse name value pair '"
								+ nameValuePairToken + "'");
			}
			...
			if (FIELD_AMOUNT.equals(nameToken)) {
				...
				try {
					...
					throw new OptionalFieldValidationException(String.format(
							"'%s' is not a valid amount", valueToken), e);
				} catch (ArithmeticException e) {
					throw new OptionalFieldValidationException(String.format(
							"'%s' has too many decimal places", valueToken), e);
				}
			} else {
				if (nameToken.startsWith("req-")) {
					...
					throw new RequiredFieldValidationException(
							"'"
									+ nameToken
									+ "' is required but not known, this URI is not valid");
				} else {
					// Known fields and unknown parameters that are optional.
					try {
						putWithValidation(nameToken,
								URLDecoder.decode(valueToken, "UTF-8"));
	...
	private void putWithValidation(String key, Object value)
		...
		if (parameterMap.containsKey(key)) {
			throw new BitcoinURIParseException(String.format(
					"'%s' is duplicated, URI is invalid", key));
		} else {
	...
	public static String convertToBitcoinURI(Address address,
			BigInteger amount, String label, String message) {
	...
	public static String convertToBitcoinURI(String address,
			@Nullable BigInteger amount, @Nullable String label,
