 ...
 * library.
 * 
 * Given a library path and name this loader looks for a native library with
...
public class JarLibraryLoader implements LibraryLoader {
	...
	public JarLibraryLoader() {
		this(JarLibraryLoader.class.getProtectionDomain().getCodeSource(),
				"lib");
	}
	 ...
	 * 
	 * @param codeSource
	 *            Code source containing shared libraries.
	 * @param libraryPath
	 *            Path prefix of shared libraries.
	 */
	...
	public boolean load(String name, boolean verify) {
		...
		try {
			...
			JarFile jar = new JarFile(codeSource.getLocation().getPath(),
					verify);
			try {
