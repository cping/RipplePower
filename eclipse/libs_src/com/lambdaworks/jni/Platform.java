 ...
 * and normalizing the <code>os.arch</code> and <code>os.name</code> system
 * properties.
 * 
 * @author Will Glozer
 */
public class Platform {
	public enum Arch {
		x86("x86|i386"), x86_64("x86_64|amd64");

		Pattern pattern;

		Arch(String pattern) {
			this.pattern = Pattern.compile("\\A" + pattern + "\\Z",
					CASE_INSENSITIVE);
		}
	}
	...
	public static Platform detect() throws UnsupportedPlatformException {
		...
		String msg = String
				.format("Unsupported platform %s %s", osArch, osName);
