...
/**
 * The <code>MP4Atom</code> object represents the smallest information block of
 * the MP4 file. It could contain other atoms as children.
 */
public class MP4Atom {

	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4AudioSampleEntryAtomType = MP4Atom
			.typeToInt("mp4a");
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4ChunkLargeOffsetAtomType = MP4Atom
			.typeToInt("co64");
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4ChunkOffsetAtomType = MP4Atom.typeToInt("stco");
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4DataInformationAtomType = MP4Atom
			.typeToInt("dinf");
	/** Constanta, the type of the MP4 Atom. */
	...
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4MediaInformationAtomType = MP4Atom
			.typeToInt("minf");
	/** Constanta, the type of the MP4 Atom. */
	...
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4SampleDescriptionAtomType = MP4Atom
			.typeToInt("stsd");
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4SampleSizeAtomType = MP4Atom.typeToInt("stsz");
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4CompactSampleSizeAtomType = MP4Atom
			.typeToInt("stz2");
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4SampleTableAtomType = MP4Atom.typeToInt("stbl");
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4SampleToChunkAtomType = MP4Atom
			.typeToInt("stsc");
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4SoundMediaHeaderAtomType = MP4Atom
			.typeToInt("smhd");
	/** Constanta, the type of the MP4 Atom. */
	...
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4VideoMediaHeaderAtomType = MP4Atom
			.typeToInt("vmhd");
	/** Constanta, the type of the MP4 Atom. */
	public final static int MP4VisualSampleEntryAtomType = MP4Atom
			.typeToInt("mp4v");
	...
	public static MP4Atom createAtom(DataStream bitstream) throws IOException {
		...
		if ((type == MP4MediaAtomType) || (type == MP4DataInformationAtomType)
				|| (type == MP4MovieAtomType)
				|| (type == MP4MediaInformationAtomType)
				|| (type == MP4SampleTableAtomType)
				|| (type == MP4TrackAtomType)) {
			...
			/*
			 * } else if(type == MP4SoundMediaHeaderAtomType){ readed =
			 * atom.create_sound_media_header_atom(bitstream); } else if(type ==
			 * MP4TrackHeaderAtomType){ readed =
			 * atom.create_track_header_atom(bitstream); } else if(type ==
			 * MP4VideoMediaHeaderAtomType){ readed =
			 * atom.create_video_media_header_atom(bitstream);
			 */
		...
		// System.out.println("Atom: type = " + intToType(type) + " size = " +
		// size);
	...
	/**
	 * Lookups for a child atom with the specified <code>type</code>, skips the
	 * <code>number</code> children with the same type before finding a result.
	 * 
	 * @param type
	 *            the type of the atom.
	 * @param number
	 ...
	 * @return the number of bytes which was being loaded.
	 */
	public long create_audio_sample_entry_atom(DataStream bitstream)
			throws IOException {
	 ...
	 * @return the number of bytes which was being loaded.
	 */
	public long create_chunk_large_offset_atom(DataStream bitstream)
			throws IOException {
	 ...
	 * @return the number of bytes which was being loaded.
	 */
	public long create_chunk_offset_atom(DataStream bitstream)
			throws IOException {
		create_full_atom(bitstream);
		entryCount = (int) bitstream.readBytes(4);
		readed += 4;
	 ...
	 * @return the number of bytes which was being loaded.
	 */
	public long create_media_header_atom(DataStream bitstream)
			throws IOException {
		create_full_atom(bitstream);
		if (version == 1) {
	 ...
	 * @return the number of bytes which was being loaded.
	 */
	public long create_movie_header_atom(DataStream bitstream)
			throws IOException {
		create_full_atom(bitstream);
		if (version == 1) {
			creationTime = createDate(bitstream.readBytes(8));
	 ...
	 * @return the number of bytes which was being loaded.
	 */
	public long create_sample_description_atom(DataStream bitstream)
			throws IOException {
		create_full_atom(bitstream);
		entryCount = (int) bitstream.readBytes(4);
		readed += 4;
	 ...
	 * @return the number of bytes which was being loaded.
	 */
	public long create_sample_size_atom(DataStream bitstream)
			throws IOException {
		create_full_atom(bitstream);
	 ...
	 * @return the number of bytes which was being loaded.
	 */
	public long create_compact_sample_size_atom(DataStream stream)
			throws IOException {
	...
	public class Record {
		...
		public Record(int firstChunk, int samplesPerChunk,
	 ...
	 * @return the number of bytes which was being loaded.
	 */
	public long create_sample_to_chunk_atom(DataStream bitstream)
			throws IOException {
		create_full_atom(bitstream);
		entryCount = (int) bitstream.readBytes(4);
		readed += 4;
		for (int i = 0; i < entryCount; i++) {
			...
			records.addElement(new Record(firstChunk, samplesPerChunk,
	 ...
	 * 
	 * @return the number of bytes which was being loaded. / public long
	 * create_sound_media_header_atom(DataStream bitstream) throws IOException {
	 * create_full_atom(bitstream); balance = (int)bitstream.readBytes(2);
	 * bitstream.skipBytes(2); readed += 4; return readed; }
	 * 
	 * protected long trackId; protected int qt_trackWidth; protected int
	 * qt_trackHeight;
	 * 
	 ...
	 * 
	 * @return the number of bytes which was being loaded. / public long
	 * create_track_header_atom(DataStream bitstream) throws IOException {
	 * create_full_atom(bitstream); if(version == 1) { creationTime =
	 * createDate(bitstream.readBytes(8)); modificationTime =
	 * createDate(bitstream.readBytes(8)); trackId = bitstream.readBytes(4);
	 * bitstream.skipBytes(4); duration = bitstream.readBytes(8); readed += 32;
	 * } else { creationTime = createDate(bitstream.readBytes(4));
	 * modificationTime = createDate(bitstream.readBytes(4)); trackId =
	 * bitstream.readBytes(4); bitstream.skipBytes(4); duration =
	 * bitstream.readBytes(4); readed += 20; } bitstream.skipBytes(8); int
	 * qt_layer = (int)bitstream.readBytes(2); int qt_alternateGroup =
	 * (int)bitstream.readBytes(2); int qt_volume = (int)bitstream.readBytes(2);
	 * bitstream.skipBytes(2); long qt_matrixA = bitstream.readBytes(4); long
	 * qt_matrixB = bitstream.readBytes(4); long qt_matrixU =
	 * bitstream.readBytes(4); long qt_matrixC = bitstream.readBytes(4); long
	 * qt_matrixD = bitstream.readBytes(4); long qt_matrixV =
	 * bitstream.readBytes(4); long qt_matrixX = bitstream.readBytes(4); long
	 * qt_matrixY = bitstream.readBytes(4); long qt_matrixW =
	 * bitstream.readBytes(4); qt_trackWidth = (int)bitstream.readBytes(4);
	 * qt_trackHeight = (int)bitstream.readBytes(4); readed += 60; return
	 * readed; }
	 * 
	 * protected int graphicsMode; protected int opColorRed; protected int
	 * opColorGreen; protected int opColorBlue;
	 * 
	 ...
	 * 
	 * @return the number of bytes which was being loaded. / public long
	 * create_video_media_header_atom(DataStream bitstream) throws IOException {
	 * create_full_atom(bitstream); if((size - readed) == 8) { graphicsMode =
	 * (int)bitstream.readBytes(2); opColorRed = (int)bitstream.readBytes(2);
	 * opColorGreen = (int)bitstream.readBytes(2); opColorBlue =
	 * (int)bitstream.readBytes(2); readed += 8; } return readed; }
	 */
	protected int width;
	 ...
	 * @return the number of bytes which was being loaded.
	 */
	public long create_visual_sample_entry_atom(DataStream bitstream)
			throws IOException {
	...
	/**
	 * Converts the time in seconds since midnight 1 Jan 1904 to the
	 * <code>Date</code>.
	 * 
	 * @param movieTime
	...
	public static int typeToInt(String type) {
		int result = ((int) type.charAt(0) << 24)
				+ ((int) type.charAt(1) << 16) + ((int) type.charAt(2) << 8)
				+ ((int) type.charAt(3));
		return result;
	}
