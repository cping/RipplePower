...
public class NativeArray extends IdScriptableObject implements List {
		...
		try {
			if (!iterating) {
				...
				for (i = 0; i < length; i++) {
					...
					if (elem == NOT_FOUND
							|| (skipUndefinedAndNull && (elem == null || elem == Undefined.instance))) {
						haslast = false;
						continue;
					}
					haslast = true;
	...
	private Object indexOfHelper(Context cx, Scriptable thisObj, Object[] args,
		...
		if (isLast) {
			 ...
			 * start searching backwards. Defaults to the array's length, i.e.
			 * the whole array will be searched. If the index is greater than or
			 * equal to the length of the array, the whole array will be
			 * searched. If negative, it is taken as the offset from the end of
			 * the array. Note that even when the index is negative, the array
			...
			/*
			 * From http://developer.mozilla.org/en/docs/Core_JavaScript_1
			 * .5_Reference:Objects:Array:indexOf The index at which to begin
			 * the search. Defaults to 0, i.e. the whole array will be searched.
			 ...
			 * it is taken as the offset from the end of the array. Note that
			 * even when the index is negative, the array is still searched from
