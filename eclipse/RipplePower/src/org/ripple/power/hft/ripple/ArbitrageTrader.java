...
public class ArbitrageTrader extends TraderBase {
	...
	public ArbitrageTrader(RippleBackendsAPI api, RippleSeedAddress seed, BOT_SET set, BotLog log) {
	...
	protected void check() {
		...
		Market baseMarket = _rippleApi.getSynXRPMarketDepth(null, _pay, query_limit);

		if (null == baseMarket || null == baseMarket.Asks || null == baseMarket.Bids) {
			return;
		}

		Market arbMarket = _rippleApi.getSynXRPMarketDepth(null, _get, query_limit);
		...
		log("Balances: %s %s; %s %s; %s XRP", baseBalance, _baseCurrency, arbBalance, _arbCurrency, xrpBalance);
		...
		if (baseBalance >= 0.1) {
			if (baseRatio > _parity * _arbFactor) {
				if (baseRatio > _parity * 1.1 || baseRatio < _parity * 0.9) {
					log("BASIC ratio has suspicious value %s. Let's leave it be", baseRatio);
					return;
				}

				log("Chance to buy cheap %s (BASIC ratio %s > %s)", _arbCurrency, baseRatio, _parity * _arbFactor);
				...
				if (baseVolume < MIN_TRADE_VOLUME || arbVolume < MIN_TRADE_VOLUME) {
					log("Insufficient volume: %s XRP for %s; %s XRP for %s", baseVolume, _baseCurrency, arbVolume,
							_arbCurrency);
				} else {
					// Try to buy XRP for BASIC
					double amount = Math.min(baseVolume, arbVolume);
					long orderId = _rippleApi.placeSynXRPBuyOrder(lowestBaseAskPrice + 0.00001, amount, _pay);
					log("Tried to buy %s XRP for %s %s each. OrderID=%s", amount, lowestBaseAskPrice, _baseCurrency,
							orderId);
					...
					if (null != orderInfo && orderInfo.Closed) {
						...
						log("Buy XRP orderID=%s filled OK, bought %s XRP", orderId, amount);
						amount -= MIN_ORDER_AMOUNT;
						long arbBuyOrderId = _rippleApi.placeSynXRPSellOrder(highestArbBidPrice * 0.9, amount, _get);
						log("Tried to sell %s XRP for %s %s each. OrderID=%s", amount, highestArbBidPrice, _arbCurrency,
								arbBuyOrderId);
						Offer arbBuyOrderInfo = _rippleApi.getSynOrderInfo(arbBuyOrderId);
						if (null != arbBuyOrderInfo && arbBuyOrderInfo.Closed) {
							log("Buy %s orderID=%s filled OK", _arbCurrency, arbBuyOrderId);
							log("%s -> %s ARBITRAGE SUCCEEDED!", _baseCurrency, _arbCurrency);
						} else {
							log("OrderID=%s (sell %s XRP for %s %s each) remains dangling. Forgetting it...",
									arbBuyOrderId, arbBuyOrderInfo.getAmountXrp(), arbBuyOrderInfo.getPrice(),
									_arbCurrency);
						}
					} else {
						log("OrderID=%s (buy %s XRP for %s %s each) remains dangling. Trying to cancel...", orderId,
								orderInfo.getAmountXrp(), orderInfo.getPrice(), _baseCurrency);
		...
		if (arbBalance >= 0.1) {
			if (arbRatio < _parity) {
				if (arbRatio > _parity * 1.1 || arbRatio < _parity * 0.9) {
					log("ARB ratio has suspicious value %s. Let's leave it be", arbRatio);
					return;
				}

				log("Chance to sell %s for %s (ARB ratio %s < {3:0.00000})", _arbCurrency, _baseCurrency, arbRatio,
						_parity);
				...
				if (arbVolume < MIN_TRADE_VOLUME || baseVolume < MIN_TRADE_VOLUME) {
					log("Insufficient volume: %s XRP for %s; %s XRP for %s", arbVolume, _arbCurrency, baseVolume,
							_baseCurrency);
				} else {
					// Try to buy XRP for ARB
					double amount = Math.min(baseVolume, arbVolume);
					long orderId = _rippleApi.placeSynXRPBuyOrder(lowestArbAskPrice + 0.00001, amount, _get);
					log("Tried to buy %s XRP for %s %s each. OrderID=%s", amount, lowestArbAskPrice, _arbCurrency,
							orderId);
					Offer orderInfo = _rippleApi.getSynOrderInfo(orderId);
					...
					if (null != orderInfo && orderInfo.Closed) {
						...
						long baseBuyOrderId = _rippleApi.placeSynXRPSellOrder(highestBaseBidPrice * 0.9, amount, _pay);
						log("Tried to sell %s XRP for %s %s each. OrderID=%s", amount, highestBaseBidPrice,
								_baseCurrency, baseBuyOrderId);
						Offer baseBuyOrderInfo = _rippleApi.getSynOrderInfo(baseBuyOrderId);
						if (null != baseBuyOrderInfo && baseBuyOrderInfo.Closed) {
							log("Buy %s orderID=%s filled OK", _baseCurrency, baseBuyOrderId);
							log("%s -> %s ARBITRAGE SUCCEEDED!", _arbCurrency, _baseCurrency);
						} else {
							log("OrderID=%s (sell %s XRP for %s %s each) remains dangling. Forgetting it...",
									baseBuyOrderId, baseBuyOrderInfo.getAmountXrp(), baseBuyOrderInfo.getPrice(),
									_baseCurrency);
						}
					} else {
						log("OrderID=%s (buy %s XRP for %s %s each) remains dangling. Trying to cancel...", orderId,
								orderInfo.getAmountXrp(), orderInfo.getPrice(), _arbCurrency);
						if (_rippleApi.cancelSynOrder(orderId)) {
							log("...success");
						} else {
							log("...failed");
		...
		if (_lastValidXrpBalance > 0.0 && xrpBalance - 2.0 > _lastValidXrpBalance) {
			...
			log("Balance {0:0.000} XRP is too high. Must convert %s to fiat.", xrpBalance, amount);
			if (baseRatio > _parity * _arbFactor) {
				log("Converting to %s", _arbCurrency);
				orderId = _rippleApi.placeSynXRPSellOrder(highestArbBidPrice * 0.9, amount, _get);
			} else if (arbRatio < _parity) {
				log("Converting to %s", _baseCurrency);
				orderId = _rippleApi.placeSynXRPSellOrder(highestBaseBidPrice * 0.9, amount, _pay);
			} else {
