...
public class CrazyBuyerTrap extends TraderBase {
	...
	public CrazyBuyerTrap(RippleBackendsAPI api, RippleSeedAddress seed, BOT_SET set, BotLog log) {
	...
	protected void check() {
		...
		if ((trend == null) || (trend == RippleBOTLoader.Trend.DOWN) || (trend == RippleBOTLoader.Trend.UNKOWN)) {
			...
			if (_sellOrderId != -1) {
				...
				if (!sellOrder.Closed) {
					if (Extensions.eq(sellOrder.getAmountXrp(), _sellOrderAmount)) {
						log("SELL order ID=%s untouched (amount=%s XRP, price=%s %s)", _sellOrderId, _sellOrderAmount,
						...
						if (newAmount > _sellOrderAmount || !Extensions.eq(_sellOrderPrice, price)) {
							this._sellOrderId = _rippleApi.updateSynXRPSellOrder(_sellOrderId, price, newAmount, _get);
						...
						log("SELL order ID=%s partially filled at price=%s %s. Remaining amount=%s XRP;", _sellOrderId,
								_executedSellPrice, sellOrder.getCurrency(), sellOrder.getAmountXrp());
					...
					if (Extensions.eq(balance, _xrpBalance, 0.1d)) {
						...
						log("SELL order ID=%s (amount=%s XRP) was closed at price=%s %s", _sellOrderId,
								_sellOrderAmount, _executedSellPrice, _currencyCode);
						_sellOrderId = -1;
						_sellOrderAmount = 0;
					}
					...
					if (!buyOrder.Closed) {
						...
						if (!Extensions.eq(buyOrder.getAmountXrp(), _buyOrderAmount)) {
							log("BUY order ID=%s partially filled at price=%s %s. Remaining amount=%s XRP;",
									_buyOrderId, buyOrder.getPrice(), buyOrder.getCurrency(), buyOrder.getAmountXrp());
							...
							log("SELL dumped some XRP. Increasing BUY amount to %s XRP", newAmount);
						...
						if (Extensions.eq(balance, _xrpBalance, 0.1)) {
							...
							if (-1 != _buyOrderId) {
								log("Successfully created BUY order with ID=%s; amount=%s XRP; price=%s %s",
										_buyOrderId, _buyOrderAmount, _buyOrderPrice, _currencyCode);
							}
						} else {
							log("BUY order ID=%s (amount=%s XRP) was closed at price=%s %s", _buyOrderId,
	...
	private double suggestSellPrice(Market market) {
		...
		for (Ask ask : market.Asks) {
			if (sum + _operativeAmount > _volumeWall && ask.getPrice() - _minDifference > highestBid) {
				double sellPrice = MathUtils.round(ask.getPrice() - increment, decPlaces);

				if (-1 != _sellOrderId && sellPrice > market.Asks.get(0).getPrice()
		...
		double price = market.Asks.get(market.Asks.size() - 1).getPrice() - increment;
		if (_sellOrderId != -1 && Math.abs(price - _sellOrderPrice) < _minPriceUpdate) {
			return _sellOrderPrice;
		}
		...
		for (Bid bid : market.Bids) {
			...
			if (bid.getPrice() < _executedSellPrice - _minDifference) {
				return Extensions.eq(bid.getPrice(), _buyOrderPrice, increment) ? _buyOrderPrice
						: MathUtils.round(bid.getPrice() + increment, decPlaces);
			}
		}
