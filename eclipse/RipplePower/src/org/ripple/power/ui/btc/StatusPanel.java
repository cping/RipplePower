...
public class StatusPanel extends JPanel implements AlertListener, ChainListener, ConnectionListener {
	...
	private static final Class<?>[] blockColumnClasses = { Date.class, Integer.class, String.class, Integer.class,
			String.class };

	private static final String[] blockColumnNames = { "Date", "Height", "Block", "Version", "Status" };

	private static final int[] blockColumnTypes = { AddressTable.DATE, AddressTable.INTEGER, AddressTable.HASH,
			AddressTable.INTEGER, AddressTable.STATUS };
	...
	private static final Class<?>[] alertColumnClasses = { Integer.class, Date.class, String.class, String.class };

	private static final String[] alertColumnNames = { "ID", "Expires", "Status", "Message" };

	private static final int[] alertColumnTypes = { AddressTable.INTEGER, AddressTable.DATE, AddressTable.STATUS,
			AddressTable.MESSAGE };
	...
	private static final Class<?>[] connectionColumnClasses = { Date.class, String.class, Integer.class, String.class,
			String.class };

	private static final String[] connectionColumnNames = { "Connected", "Address", "Version", "Subversion",
			"Services" };

	private static final int[] connectionColumnTypes = { AddressTable.DATE, AddressTable.ADDRESS, AddressTable.INTEGER,
			AddressTable.SERVICES, AddressTable.SERVICES };
	...
	public StatusPanel() {
		...
		try {
			...
			alertTable.setPreferredScrollableViewportSize(new Dimension(
					alertTable.getPreferredScrollableViewportSize().width, alertTable.getRowHeight() * 3));
	...
	private void updateStatus() {
		...
		chainHeadField.setText(String.format("<html><b>Chain head: %s</b></html>", chainHead.toString()));
		int chainHeight = BTCLoader.blockStore.getChainHeight();
		chainHeightField.setText(String.format("<html><b>Chain height: %d</b></html>", chainHeight));
		...
		networkDifficultyField.setText(String.format("<html><b>Network difficulty: %s</b></html>", displayDifficulty));
		peerConnectionsField
				.setText(String.format("<html><b>Peer connections: %d</b></html>", connectionTable.getRowCount()));
		LSystem.invokeLater(new Runnable() {
			...
			synchronized (blockMap) {
				...
				if (blockStatus == null) {
					blockStatus = new BlockStatus(blockHash, storedBlock.getBlock().getTimeStamp(),
							storedBlock.getHeight(), storedBlock.getBlock().getVersion(), storedBlock.isOnChain(),
