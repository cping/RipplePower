...
public abstract class LGraphicsMath {
	...
	public final static Point getBoundingPointAtAngle(int boundingX, int boundingY, int boundingWidth,
			int boundingHeight, int angle) {
		if (angle >= 315 || angle <= 45) {
			return new Point(boundingX + boundingWidth, boundingY + (boundingHeight * (65536 - toShift(angle)) >>> 17));
		} else if (angle > 45 && angle < 135) {
			return new Point(boundingX + (boundingWidth * (65536 + toShift(angle)) >>> 17), boundingY);
		} else if (angle >= 135 && angle <= 225) {
			return new Point(boundingX, boundingY + (boundingHeight * (65536 + toShift(angle)) >>> 17));
		} else {
			return new Point(boundingX + (boundingWidth * (65536 - toShift(angle)) >>> 17), boundingY + boundingHeight);
		}
	}

	public final static RectBox getBoundingBox(int xpoints[], int ypoints[], int npoints) {
		...
		return new RectBox(boundsMinX, boundsMinY, boundsMaxX - boundsMinX, boundsMaxY - boundsMinY);
	}
	...
	public final static boolean contains(int xPoints[], int yPoints[], int nPoints, RectBox bounds, int x, int y) {
		if ((bounds != null && bounds.inside(x, y))
				|| (bounds == null && getBoundingBox(xPoints, yPoints, nPoints).inside(x, y))) {
			int hits = 0;
			int ySave = 0;
			int i = 0;
			...
			for (int n = 0; n < nPoints; n++) {
				...
				if (dy != 0) {
					...
					if (ry * dy >= 0 && (ry <= dy && ry >= 0 || ry >= dy && ry <= 0) && round(dx * ry, dy) >= rx) {
						hits++;
					}
				}
				i = j;
