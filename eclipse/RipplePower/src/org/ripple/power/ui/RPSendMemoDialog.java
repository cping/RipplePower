...
public class RPSendMemoDialog extends ABaseDialog implements WindowListener {
	/**
	 * 
	 */
	...
	public static void showDialog(String name, Window parent, String address, String message) {
	...
	public static void showDialog(String name, Window parent, String address, String message, WalletItem item) {
		try {
			RPSendMemoDialog dialog = new RPSendMemoDialog(name, parent, address, message, item);
			dialog.pack();
	...
	public RPSendMemoDialog(String text, Window parent, String address, String message, WalletItem item) {
		super(parent,
				text + (item == null ? (address == null ? "" : "(" + address + ")") : "(" + item.getPublicKey() + ")"),
	...
	private void initComponents() {
		...
		_exitButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				SwingUtils.close(RPSendMemoDialog.this);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (AccountFind.isRippleAddress(address)) {
					...
					if (!StringUtils.isNumber(fee)) {
						RPToast.makeText(RPSendMemoDialog.this, UIMessage.errFee, Style.ERROR).display();
						return;
					}
					...
					if (StringUtils.isNumber(amountValue.valueText())) {
						...
						if (password == null) {
							encode = new RippleMemoEncode(RippleMemoEncode.Mode.BASE64, "MESSAGE", context, password);
						} else {
							encode = new RippleMemoEncode(RippleMemoEncode.Mode.ENCODE, "MESSAGE", context, password);
						}
						final WaitDialog wait = WaitDialog.showDialog(RPSendMemoDialog.this);
						_waitDialogs.add(wait);
						Payment.send(_item.getSeed(), amount, address, encode, fee, new Rollback() {

							@Override
							public void success(JSONObject res) {
								RPJSonLog.get().println(res);
								wait.closeDialog();
								RPToast.makeText(RPSendMemoDialog.this, UIMessage.completed, Style.SUCCESS).display();
							}
						...
						RPToast.makeText(RPSendMemoDialog.this, UIMessage.errMoney, Style.ERROR).display();
					}
				} else {
					RPToast.makeText(RPSendMemoDialog.this, UIMessage.errAddress, Style.ERROR).display();
				}
					...
					find.message(address, password, min, max, limit, new Updateable() {
						...
						public void action(Object o) {
							...
							if (o != null && o instanceof RippleMemoDecodes) {
								...
								if (decodes.size() > 0) {
									_messageList.setModel(new javax.swing.AbstractListModel<Object>() {
										private static final long serialVersionUID = 1L;
