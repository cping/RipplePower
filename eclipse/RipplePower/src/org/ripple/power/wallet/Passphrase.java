...
public class Passphrase {
	...
	public static byte[] encrypt(Passphrase passphrase, byte[] unencryptedData) throws Exception {
	...
	private static IvParameterSpec ivParameterSpec16(byte[] salt, int level) throws Exception {
	...
	private static List<Cipher> cipherList(byte[] passphraseBytes, int mode) throws NoSuchAlgorithmException,
		...
		aes1.init(mode, keyGenAES(passphraseBytes, "cping@passsaltyN3SS&Wha9dste", 18913),
				ivParameterSpec16(passphraseBytes, 1));
		aes2.init(mode, keyGenAES(passphraseBytes, "cping!wordsaltyN74G@337q0877", 23944),
				ivParameterSpec16(passphraseBytes, 2));
		aes3.init(mode, keyGenAES(passphraseBytes, "cping#privatesaltyN9A9!14Ra1", 19781),
				ivParameterSpec16(passphraseBytes, 3));
		return Arrays.asList(aes1, aes2, aes3);
	}

	private static SecretKey keyGenAES(byte[] passphraseBytes, String saltString, int iterations) throws Exception {
		SecretKey key = null;
		try {
			...
			PBEKeySpec pbeKeySpec = new PBEKeySpec(ByteUtils.toHexString(passphraseBytes).toCharArray(), salt,
					iterations, AES_KEY_LENGTH);
	...
	public final void setPassphrase(String passphrase) throws Exception {
		...
		try {
			this.passphraseHash = Helper.hash512(passphrase, STATIC_SALT, PASSWORD_ITERATIONS);
		} catch (Exception ex) {
			clear();
			throw ex;
		}
	...
	public static String encodeToHex(String context, String password) throws Exception {
		Passphrase passphrase = new Passphrase(password);
		byte[] buffer = Passphrase.encrypt(passphrase, context.getBytes(LSystem.encoding));
		return CoinUtils.toHex(buffer);
	}

	public static String decodeToHex(String hash, String password) throws Exception {
		Passphrase passphrase = new Passphrase(password);
