...
public class SerializedBuffer {
	...
	/**
	 * Create a new serialized buffer. The buffer start is set to the beginning
	 * of the byte array and the buffer limit is set to the end of the byte
	 * array.
	 *
	 * @param buffer
	 *            Backing byte array
	...
	/**
	 * Create a new serialized buffer. The buffer start is set to the byte array
	 * offset and the buffer limit is set to the buffer start plus the array
	 * length.
	 *
	 * @param buffer
	 *            Backing byte array
	...
	/**
	 * Creates a new serialized buffer from an input ByteBuffer. The buffer
	 * start is set to 0 and the buffer limit is set to the ByteBuffer limit.
	 *
	 * @param byteBuffer
	 *            Input ByteBuffer
	 */
	...
	/**
	 * Return the number of available bytes (buffer limit - current buffer
	 * position)
	 *
	...
	/**
	 * Start a new segment at the specified buffer position relative to the
	...
	/**
	 * Return the byte array for the serialized buffer. A new array will be
	 * allocated if the used portion of the buffer is not the same size as the
	 * backing array.
	 *
	 * @return Byte array
	 */
	...
	/**
	 * Return a ByteBuffer for the serialized buffer. The ByteBuffer will use
	 * the little-endian format.
	 *
	 * @return ByteBuffer
	 */
	...
	public int getInt() throws EOFException {
		if (bufferLimit - offset < 4)
			throw new EOFException("Byte array underrun");
		return ((int) bytes[offset++] & 255) | (((int) bytes[offset++] & 255) << 8)
				| (((int) bytes[offset++] & 255) << 16) | (((int) bytes[offset++] & 255) << 24);
	}
	...
	public long getLong() throws EOFException {
		if (bufferLimit - offset < 8)
			throw new EOFException("Byte array underrun");
		return ((long) bytes[offset++] & 255) | (((long) bytes[offset++] & 255) << 8)
				| (((long) bytes[offset++] & 255) << 16) | (((long) bytes[offset++] & 255) << 24)
				| (((long) bytes[offset++] & 255) << 32) | (((long) bytes[offset++] & 255) << 40)
				| (((long) bytes[offset++] & 255) << 48) | (((long) bytes[offset++] & 255) << 56);
	}
	...
	/**
	 * Return bytes in a new buffer. The byte array length is a variable-length
	 * integer preceding the bytes.
	 *
	 * @return Byte array
	 * @throws EOFException
	 *             Byte array underrun
	...
	/**
	 * Return bytes in the supplied array (the array length is the number of
	 * bytes to read)
	 *
	 * @param buffer
	 *            Buffer
	...
	/**
	 * Return bytes in the supplied array starting at the specified offset and
	 ...
	 *
	 * @param byteList
	 *            List of byte serializable elements
	 * @return This buffer
	 */
