...
/**
 * A VarInt is an unsigned variable-length encoded integer using the bitcoin
 * encoding (called the 'compact size' in the reference client). It consists of
 * a marker byte and zero or more data bytes as follows:
 * 
 * <pre>
 *      Value           Marker      Data
 *      ======          ======      ====
...
public final class VarInt {
	...
	public VarInt(InputStream in) throws EOFException, IOException {
		...
		if (first < 253) {
			...
			value = Helper.readUint32LE(buf, 0);
			encodedSize = 5;
		} else {
			// 64 bits.
			byte[] buf = new byte[8];
			...
			value = Helper.readUint64LE(buf, 0);
			encodedSize = 9;
		}
	}
	...
	public static byte[] encode(long value) {
		...
		if ((value & 0xFFFFFFFF00000000L) != 0) {
			...
			bytes = new byte[9];
			bytes[0] = (byte) 255;
			Helper.uint64ToByteArrayLE(value, bytes, 1);
		} else if ((value & 0x00000000FFFF0000L) != 0) {
			// 1 marker + 4 data bytes
			bytes = new byte[5];
			bytes[0] = (byte) 254;
			Helper.uint32ToByteArrayLE(value, bytes, 1);
		} else if (value >= 253L) {
			// 1 marker + 2 data bytes
