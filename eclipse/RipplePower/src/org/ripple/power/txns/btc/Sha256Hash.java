...
/**
 * A Sha256Hash wraps a byte[] so that equals and hashCode work correctly,
 * allowing it to be used as a key in a map.
 */
public class Sha256Hash {
	...
	/**
	 * Generates the hash code for this object. We use the last 4 bytes of the
	 * value to form the hash because the first 4 bytes often contain zero
	 * values in the Bitcoin protocol.
	 *
	 * @return Hash code
	 */
	@Override
	public int hashCode() {
		return (((int) bytes[28] & 0xff) << 24) | (((int) bytes[29] & 0xff) << 16) | (((int) bytes[30] & 0xff) << 8)
				| ((int) bytes[31] & 0xff);
	}
