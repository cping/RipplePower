...
/**
 * <p>
 * A Merkle branch is a data structure that contains proofs of block inclusion
 * for one or more transactions in an efficient manner.
 * </p>
 *
 * <p>
 * The encoding works as follows: we traverse the tree in depth-first order,
 * storing a bit for each traversed node, signifying whether the node is the
 * parent of at least one matched leaf txid (or a matched txid itself). In case
 * we are at the leaf level, or this bit is 0, its Merkle node hash is stored,
 * and its children are not explored further. Otherwise, no hash is stored, but
 * we recurse into both (or the only) child branches. During decoding, the same
 * depth-first traversal is performed, consuming bits and hashes as they were
 * written during encoding.
 * </p>
 *
 * <p>
 * The serialization is fixed and provides a hard guarantee about the encoded
 * size, <tt>SIZE LE 10 + ceil(32.25*N)</tt> where N represents the number of
 * leaf nodes of the partial tree. N itself is bounded by:
 * </p>
 *
 * <p>
...
public class MerkleBranch implements ByteSerializable {
	...
	/**
	 * Creates a new Merkle branch for the supplied transactions. The
	 * transaction list must be sorted by transaction index. The Merkle tree
	 * starts with the leaf nodes and ends with the root node.
	 *
	 * @param txCount
	 *            Total number of transactions in the block
	 * @param txList
	 *            Matched transaction index list
	 * @param merkleTree
	 *            Merkle tree
	 */
	public MerkleBranch(int txCount, List<Integer> txList, List<byte[]> merkleTree) {
		...
		// node in the tree has two descendants (the last node in a level may
		// have
		// just one descendant)
		//
		int height = 0;
		...
		//
		// Allocate the hash list and flag array (we will adjust the flag array
		// size
		// when we are done)
		//
		...
		//
		// Create the paths to reach each matched transaction. Each tree level
		// has
		// an array of node indexes representing the next node for each
		// transaction.
		...
		// down to the leaf transactions. Remember that the tree is upside down
		// with
		...
		//
		// We have another special case if the block contains just the coinbase
		// transaction
		// and this is also the matched transaction. In this case, we will
		// create
		// a Merkle branch with the coinbase transaction set in nodeFlags. Note
		// that
		// the merkle root is the same as the coinbase transaction hash in this
		// case.
		//
		if (txList.isEmpty()) {
			bitsUsed++;
	...
	public SerializedBuffer getBytes(SerializedBuffer outBuffer) {
		outBuffer.putInt(txCount).putVarInt(nodeHashes.size());
		for (byte[] hash : nodeHashes)
			outBuffer.putBytes(Helper.reverseBytes(hash));
		outBuffer.putVarInt(nodeFlags.length).putBytes(nodeFlags);
		return outBuffer;
	}
	...
	/**
	 * Traverse the Merkle branch down to the leaf transaction. Return the leaf
	 * transaction hashes and the calculated Merkle root hash. The matchedHash
	 * list will be cleared before starting.
	 *
	 * @param matchedHashes
	 *            Return the hashes of the matched leaf transactions
	 * @return Merkle root
	 * @throws VerificationException
	 *             Malformed Merkle branch
	 */
	public Sha256Hash calculateMerkleRoot(List<Sha256Hash> matchedHashes) throws VerificationException {
		...
		if (hashesUsed != nodeHashes.size())
			throw new VerificationException(
					String.format("Merkle branch used %d of %d hashes", hashesUsed, nodeHashes.size()),
					RejectMessage.REJECT_INVALID);
		return new Sha256Hash(merkleRoot);
	}

	/**
	 * Recursively build the Merkle branch setting the node flags to indicate
	 * the branches we have taken. For each node, follow the left branch as long
	 * as it leads to the next leaf transaction. Then switch to the right branch
	 * to continue. At each node junction, set the appropriate node flag to
	 * indicate which branch we took. For each branch that we don't take, add
	 * the hash for that path to the node hash list.
	 *
	 * @param height
	 *            Current height
	 * @param levelPos
	 *            Current position within level
	 * @param nodePath
	 *            Transaction paths
	 * @param treePos
	...
	private void buildBranch(int height, int levelPos, int[][] nodePath, int treePos, List<byte[]> merkleTree) {
		...
		//
		// hashesUsed tracks our position within nodePath and bitsUsed tracks
		// our
		...
		if (height > 0) {
			...
			if (nextNode == nextLevelPos) {
				...
				//
				// We have found the current transaction, so update nextNode and
				// see if
				// we should now follow the right branch. If not, add the hash
				// for the
				...
				//
				// We need to follow the right branch, so add the hash for the
				// left node
				// and then recurse
				//
				...
				throw new IllegalStateException(
						String.format("Next node %d at height %d is not valid", nextNode, height));
			}
		} else {
			//
			// We are at the leaf nodes, so our choice is simple: left node or
			// right node
			//
			if (nextNode == nextLevelPos) {
				//
	...
	/**
	 * Recursively traverse the tree nodes as dictated by the node flags,
	 * getting missing hashes from the node hash list for branches not on our
	 * path. Matching leaf hashes are added to matchesHashes.
	 *
	 * @param height
	 *            Current height
	 * @param pos
	 *            Current position within the level
	 * @param matchedHashes
	 *            Matched hashes from the leaf node
	 * @return Node hash
	 * @throws VerificationException
	 */
	private byte[] parseBranch(int height, int pos, List<Sha256Hash> matchedHashes) throws VerificationException {
		if (bitsUsed >= nodeFlags.length * 8)
			throw new VerificationException("Merkle branch overflowed the bits array", RejectMessage.REJECT_INVALID);
		...
		if (height == 0 || !parentOfMatch) {
			...
			// and do not descend
			// to the next level. If we have a match at height 0, it is a
		...
		//
		return Helper.reverseBytes(
				Helper.doubleDigestTwoBuffers(Helper.reverseBytes(left), 0, 32, Helper.reverseBytes(right), 0, 32));
	}

	/**
	 * Calculate the Merkle tree width for a given height based upon the total
	 * transaction count
	 *
	 * @param height
	 *            Tree height
