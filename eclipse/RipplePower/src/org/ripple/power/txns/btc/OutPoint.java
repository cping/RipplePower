...
/**
 * An outpoint describes the transaction output that is connected to a
 * transaction input. It consists of the hash for the transaction containing the
 * output and the index of the output within the transaction.
 */
public class OutPoint implements ByteSerializable {
	 ...
	 *
	 * @param txHash
	 *            Hash for the transaction output
	 * @param outputIndex
	...
	public SerializedBuffer getBytes(SerializedBuffer outBuffer) {
		outBuffer.putBytes(Helper.reverseBytes(txHash.getBytes())).putInt(outputIndex);
		return outBuffer;
	}
	...
	public boolean equals(Object obj) {
		return (obj != null && (obj instanceof OutPoint) && txHash.equals(((OutPoint) obj).txHash)
				&& outputIndex == ((OutPoint) obj).outputIndex);
	}
}
