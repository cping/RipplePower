...
/**
 * BlockHeader contains the block header. It is used to validate transactions
 * sent to us and to determine if a transaction has become unconfirmed because
 * the block containing it is no longer on the block chain.
 *
 * <p>
...
public class BlockHeader implements ByteSerializable {
	 ...
	 *            Previous block hash
	 * @param blockTime
	 *            Time block was mined (seconds since Unix epoch)
	 ...
	 *            Block nonce
	 */
	public BlockHeader(int version, Sha256Hash blockHash, Sha256Hash prevHash, long blockTime, long targetDifficulty,
			Sha256Hash merkleRoot, int nonce) {
	 ...
	 */
	public BlockHeader(int version, Sha256Hash blockHash, Sha256Hash prevHash, long blockTime, long targetDifficulty,
			Sha256Hash merkleRoot, int nonce, List<Sha256Hash> matches) {
		this.version = version;
		this.blockHash = blockHash;
		this.prevHash = prevHash;
		this.blockTime = blockTime;
	 ...
	 *            Serialized data
	 * @param doVerify
	 *            TRUE to verify the block header structure
	 * @throws EOFException
	 *             Serialized data is too short
	 ...
	 *             Block verification failed
	 */
	public BlockHeader(SerializedBuffer inBuffer, boolean doVerify) throws EOFException, VerificationException {
		...
		if (doVerify) {
			...
			if (target.signum() <= 0 || target.compareTo(NetParams.PROOF_OF_WORK_LIMIT) > 0)
				throw new VerificationException("Target difficulty is not valid", RejectMessage.REJECT_INVALID,
						blockHash);
	...
	public SerializedBuffer getBytes(SerializedBuffer outBuffer) {
		outBuffer.putInt(version).putBytes(Helper.reverseBytes(prevHash.getBytes()))
				.putBytes(Helper.reverseBytes(merkleRoot.getBytes())).putUnsignedInt(blockTime)
				.putUnsignedInt(targetDifficulty).putInt(nonce);
		return outBuffer;
	}
	...
	/**
	 * Returns the list of matched transactions or null if there are no matched
	 * transactions
	 *
	 ...
	 *
	 * @param matches
	 *            List of matched transactions or null if there are no matched
	 *            transactions
	 */
