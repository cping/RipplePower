...
public class ECKey {
	...
	/**
	 * Creates an ECKey with a new public/private key pair. Point compression is
	 * used so the resulting public key will be 33 bytes (32 bytes for the
	 * x-coordinate and 1 byte to represent the y-coordinate sign)
	 */
	public ECKey() {
	...
	/**
	 * Creates an ECKey public/private key pair using the supplied private key.
	 * The 'compressed' parameter determines the type of public key created.
	 * 
	 * @param privKey
	 *            Private key
	 * @param compressed
	 *            TRUE to create a compressed public key
	 */
	...
	/**
	 * Creates an ECKey with the supplied public/private key pair. The private
	 * key may be null if you only want to use this ECKey to verify signatures.
	 * The public key will be generated from the private key if it is not
	 * provided (the 'compressed' parameter determines the type of public key
	 * created)
	 *
	 * @param pubKey
	 *            Public key or null
	...
	public static boolean isSignatureCanonical(byte[] encodedSig) {
		//
		// DER-encoding requires that there is only one representation for a
		// given
		// encoding. This means that no pad bytes are inserted for numeric
		// values.
		//
		// An ASN.1 sequence is identified by 0x30 and each primitive by a type
		// field.
		// An integer is identified as 0x02. Each field type is followed by a
		// field length.
		// For valid R and S values, the length is a single byte since R and S
		// are both
		// 32-byte or 33-byte values (a leading zero byte is added to ensure a
		// positive
		// value if the sign bit would otherwise bet set).
		//
		// Bitcoin appends that hash type to the end of the DER-encoded
		// signature. We require
		// this to be a single byte for a canonical signature.
		//
		// The length is encoded in the lower 7 bits for lengths between 0 and
		// 127 and the upper bit is 0.
		// Longer length have the upper bit set to 1 and the lower 7 bits
		// contain the number of bytes
		// in the length.
		//
	...
	/**
	 * Returns the encoded private key in the format used by the Bitcoin
	 * reference client
	 *
	...
	public boolean verifySignature(byte[] contents, byte[] signature) throws ECException {
		...
		// Get the double SHA-256 hash of the signed contents
		//
		// A null contents will result in a hash with the first byte set to 1
		// and
		// all other bytes set to 0. This is needed to handle a bug in the
		// reference
		// client where it doesn't check for an error when serializing a
		// transaction
		...
		try {
			...
			ECPublicKeyParameters params = new ECPublicKeyParameters(ecParams.getCurve().decodePoint(pubKey), ecParams);
	...
	public String signMessage(String message) throws ECException {
		...
		try {
			...
			//
			// The message signature consists of a header byte followed by the R
			// and S values
			//
	...
	public static boolean verifyMessage(String address, String message, String encodedSignature)
		...
		//
		// The signature is correct if the recovered public key hash matches the
		// supplied hash
		//
	 ...
	 * @param e
	 *            The double SHA-256 hash of the original message
	 * @param compressed
	 *            Whether or not the original public key was compressed
	...
	private static ECKey recoverFromSignature(int recID, ECDSASignature sig, BigInteger e, boolean compressed) {
		...
		//
		// Convert the integer x to an octet string X of length mlen using the
		// conversion routine
		// specified in Section 2.3.7, where mlen = ⌈(log2 p)/8⌉ or mlen =
		// ⌈m/8⌉.
		// Convert the octet string (16 set binary digits)||X to an elliptic
		...
		if (!R.multiply(n).isInfinity())
			return null;
		//
		// For k from 1 to 2 do the following. (loop is outside this function
		// via iterating recId)
		...
		// addition (the EC group operator).
		//
	...
	/**
	 * Decompress a compressed public key (x coordinate and low-bit of
	 * y-coordinate).
	 *
	 * @param xBN
	 *            X-coordinate
	 ...
	 *            Private key
	 * @param compressed
	 *            TRUE to generate a compressed public key
	 * @return Public key
	 */
