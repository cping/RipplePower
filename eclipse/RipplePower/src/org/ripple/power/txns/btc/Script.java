 ...
 * provided by the transaction output.
 */
public class Script {
	 ...
	 *
	 * For canonical scripts, each push-data operation must use the shortest
	 * opcode possible. Numeric values between 0 and 16 must use OP_n opcodes.
	 *
	 * @param scriptBytes
	 *            Script bytes
		...
		if (scriptBytes.length > 0) {
			if (scriptBytes[0] == (byte) ScriptOpCodes.OP_RETURN) {
				...
				if (scriptBytes.length == 25 && scriptBytes[1] == (byte) ScriptOpCodes.OP_HASH160
						&& scriptBytes[2] == 20 && scriptBytes[23] == (byte) ScriptOpCodes.OP_EQUALVERIFY
	 ...
	 *            Script program
	 * @param startOffset
	 *            Offset to byte following the opcode
	...
	public static int[] getDataLength(int opcode, byte[] scriptBytes, int startOffset) throws EOFException {
		...
		if (opcode < ScriptOpCodes.OP_PUSHDATA1) {
			...
			if (offset > scriptBytes.length - 4)
				throw new EOFException("End-of-data while processing script");
			dataToRead = ((int) scriptBytes[offset] & 0xff) | (((int) scriptBytes[offset + 1] & 0xff) << 8)
					| (((int) scriptBytes[offset + 2] & 0xff) << 16) | (((int) scriptBytes[offset + 3] & 0xff) << 24);
			offset += 4;
		} else {
			dataToRead = 0;
		}
