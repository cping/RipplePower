...
public class NetworkMessageListener extends AbstractMessageListener {
	...
	public void sendInventory(Message msg, List<InventoryItem> invList) {
		...
		//
		// If this is a request restart, we need to skip over the items that
		// have already
		// been processed as indicated by the restart index contained in the
		// message. Otherwise,
		...
		for (int i = restart; i < invList.size(); i++) {
			...
			// We will
			// restart at this point after the current batch has been sent to
			// the peer.
			//
			if (blocksSent == 25) {
			...
			case InventoryItem.INV_FILTERED_BLOCK:
				...
				if (filter != null) {
					//
					// Get the block from the database and locate any matching
					// transactions.
					// Send the Merkle block followed by the matching
					// transactions (if any)
					//
					try {
						Block block = BTCLoader.blockStore.getBlock(item.getHash());
		...
		//
		// Create a 'notfound' response if we didn't find all of the requested
		// items
		//
		if (!notFound.isEmpty()) {
	...
	public void requestInventory(Message msg, List<InventoryItem> invList) {
		...
		for (InventoryItem item : invList) {
			...
			case InventoryItem.INV_TX:
				//
				// Ignore large transaction broadcasts to avoid running out of
				// storage
				//
				if (txRequests >= 50) {
				...
				if (!newTx)
					continue;
				//
				// Ignore transactions if we are down-level since they will be
				// orphaned
				...
				if (BTCLoader.blockStore.getChainHeight() < BTCLoader.networkChainHeight - 5)
					continue;
				//
				// Request the transaction if it is not in the memory pool and
				// has not
				// been requested. We add the request at the front of the queue
				// so it
				...
				try {
					...
					// Unable to check database - wait for another inventory
					// broadcast
				}
				break;
			case InventoryItem.INV_BLOCK:
				//
				// Request the block if it is not in the database and has not
				// been requested.
				// Block requests are added to the end of the queue so that we
				// don't hold
	...
	public void requestMemoryPool(Message msg) {
		//
		// Get the list of transaction identifiers in the memory pool. We will
		// send a maximum
	...
	public void processAddresses(Message msg, List<PeerAddress> addresses) {
		...
		//
		// Add new addresses to the peer address list and update the timestamp
		// and services
		// for existing entries. We will not include peers that provide no
		// services or peers
		// that that are too old. The peer list is sorted by timestamp from
		// newest to oldest.
		// Existing entries are updated in-place.
		//
		for (PeerAddress addr : addresses) {
			if (addr.getServices() != 0 && addr.getTimeStamp() > oldestTime && !addr.getAddress().isAnyLocalAddress()
					&& !addr.getAddress().isLoopbackAddress() && addr.getPort() > 0 && addr.getPort() < 65536
	...
	public void processBlock(Message msg, Block block) {
		//
		// Indicate the block is being processed so the block request won't be
		// rebroadcast
		//
		synchronized (BTCLoader.pendingRequests) {
	...
	public void processGetBlocks(Message msg, int version, List<Sha256Hash> blockList, Sha256Hash stopBlock) {
		...
		try {
			...
			//
			// We go back to the genesis block if none of the supplied locators
	...
	public void processGetHeaders(Message msg, int version, List<Sha256Hash> blockList, Sha256Hash stopBlock) {
		...
		try {
			...
			// Can't access the database, so just ignore the 'getheaders'
			// request
		}
	}
	...
	public void processTransaction(Message msg, Transaction tx) {
		...
		//
		// Ignore the transaction if we have already seen it. Otherwise, add it
		// to
		...
		try {
			...
			//
			if (tx.isCoinBase())
				throw new VerificationException("Coinbase transaction cannot be relayed", RejectMessage.REJECT_INVALID,
						txHash);
			//
		...
		//
		// Increment the banscore for the peer if this is an invalid and
	...
	public void processVersion(Message msg, PeerAddress localAddress) {
		...
		BTCLoader.info(String.format(
				"Peer %s: Protocol level %d, Services %d, Agent %s, Height %d, " + "Relay blocks %s, Relay tx %s",
				peer.getAddress(), peer.getVersion(), peer.getServices(), peer.getUserAgent(), peer.getHeight(),
				peer.shouldRelayBlocks() ? "Yes" : "No", peer.shouldRelayTx() ? "Yes" : "No"));
		Message ackMsg = VersionAckMessage.buildVersionAckMessage(peer);
		BTCLoader.networkHandler.sendMessage(ackMsg);
		//
		// Set our local address from the Version message if it hasn't been set
		// yet
		//
	...
	/**
	 * Sends a 'merkleblock' message followed by 'tx' messages for the matched
	 * transactions
	 *
	 * @param peer
	 *            Destination peer
	...
	private boolean validateTx(Transaction tx) throws EOFException, VerificationException {
		...
		//
		// The transaction must be final. If the transaction lock time is
		// specified as a block height,
		// the block height must not be greater than the current chain height+1.
		// The reference code
		// does not perform a check if a timestamp is used instead of a block
		// height.
		//
		...
		for (TransactionOutput output : outputs) {
			// Dust transactions are not relayed - a dust transaction is one
			// where the minimum
			// relay fee is greater than 1/3 of the output value, assuming a
			// single 148-byte input
			// to spend the output
			BigInteger chkValue = output.getValue().multiply(BigInteger.valueOf(1000))
					.divide(BigInteger.valueOf(3 * (output.getScriptBytes().length + 9 + 148)));
			if (chkValue.compareTo(BTCLoader.MIN_TX_RELAY_FEE) < 0)
				throw new VerificationException("Dust transactions are not relayed", RejectMessage.REJECT_DUST,
						tx.getHash());
			// Non-standard payment types are not relayed
			int paymentType = Script.getPaymentType(output.getScriptBytes());
			if (paymentType != ScriptOpCodes.PAY_TO_PUBKEY_HASH && paymentType != ScriptOpCodes.PAY_TO_PUBKEY
					&& paymentType != ScriptOpCodes.PAY_TO_SCRIPT_HASH && paymentType != ScriptOpCodes.PAY_TO_MULTISIG
			...
			// Add the output value to the total output value for the
			// transaction
		...
		for (TransactionInput input : inputs) {
			...
			if (spendHash == null) {
				// Connected output is not in the recently spent list, check the
				// memory pool
				...
				if (outTx != null) {
					// Transaction is in the memory pool, get the connected
					// output
					...
					if (output == null)
						throw new VerificationException(
								String.format("Transaction references non-existent output\n  Tx %s", txHash),
			...
			case ScriptOpCodes.PAY_TO_PUBKEY_HASH:
				// First data element is signature, second data element is
				// public key
				if (dataList.isEmpty() || !ECKey.isSignatureCanonical(dataList.get(0)))
					canonicalType = 1;
		...
		//
		// Ignore a duplicate transaction (race condition among message handler
		// threads)
		//
		if (duplicateTx)
			return false;
		...
		if (txLength > BTCLoader.MAX_FREE_TX_SIZE) {
			...
			if (totalFee.compareTo(minFee) < 0) {
				throw new VerificationException("Insufficient transaction fee", RejectMessage.REJECT_INSUFFICIENT_FEE,
						txHash);
			}
		}
		//
		// Store the transaction in the memory pool (maximum size we will store
		// is 50KB)
		//
		if (txLength <= 50 * 1024) {
			StoredTransaction storedTx = new StoredTransaction(tx);
	...
	private void broadcastTx(Transaction tx) throws EOFException {
		...
		for (BloomFilter filter : filters) {
			...
			//
			// Check the transaction against the filter and send an 'inv'
			// message if it is a match
			//
