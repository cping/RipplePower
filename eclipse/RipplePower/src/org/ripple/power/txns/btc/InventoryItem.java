...
/**
 * InventoryItem represents an inventory item (block or transaction). Inventory
 * items are used in messages that announce the availability of an item or
...
public class InventoryItem implements ByteSerializable {
	 ...
	 *
	 * @param type
	 *            Inventory item type (INV_BLOCK, INV_FILTERED_BLOCK, INV_TX)
	 * @param hash
	 *            Inventory item hash
	 */
	...
	public SerializedBuffer getBytes(SerializedBuffer outBuffer) {
		outBuffer.putInt(type).putBytes(Helper.reverseBytes(hash.getBytes()));
		return outBuffer;
	}
