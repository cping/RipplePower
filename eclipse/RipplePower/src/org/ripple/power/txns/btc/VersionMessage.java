...
/**
 * <p>
 * The 'version' message is exchanged when two nodes connect. It identifies the
 * services provided by the nodes and the latest block each has seen. A node
...
public class VersionMessage {
	 ...
	 *            The remote peer
	 * @param localAddress
	 *            Local listen address or null if not accepting inbound
	 *            connections
	 * @param chainHeight
	...
	public static Message buildVersionMessage(Peer peer, PeerAddress localAddress, int chainHeight) {
		...
		msgBuffer.putInt(NetParams.PROTOCOL_VERSION).putLong(NetParams.SUPPORTED_SERVICES)
		...
		msgBuffer.putUnsignedByte(peerAddress.getPort() >>> 8).putUnsignedByte(peerAddress.getPort());
		//
		...
		if (localAddress != null) {
			...
			if (srcAddress.length == 16) {
				...
				msgBuffer.putBytes(PeerAddress.IPV6_PREFIX).putBytes(srcAddress);
			}
			msgBuffer.putUnsignedByte(localAddress.getPort() >>> 8).putUnsignedByte(localAddress.getPort());
		} else {
			msgBuffer.skip(16 + 2);
		}
		//
	 ...
	 *            Message listener
	 * @throws EOFException
	 *             End-of-data processing message data
	...
	public static void processVersionMessage(Message msg, SerializedBuffer inBuffer, MessageListener msgListener)
		...
		PeerAddress localAddress = new PeerAddress(addr,
				(inBuffer.getUnsignedByte() << 8) | inBuffer.getUnsignedByte());
		//
		// Get the user agent
		//
		inBuffer.skip(26 + 8);
		...
		//
		// Get the chain height and transaction relay flag (the transaction
		// relay flag is
		// not included in earlier protocol versions and defaults to TRUE).
		// Always relay
		// block notifications.
		//
