...
/**
 * <p>
 * An 'addr' message is sent to inform a node about peers on the network.
 * </p>
 *
 * <p>
 * Address Message
...
public class AddressMessage {
	 ...
	 *
	 * We will include the first 1000 peers as well as our own external listen
	 * address
	 *
	 * @param peer
	 ...
	 *            Peer address list
	 * @param localAddress
	 *            Local address or null if not accepting inbound connections
	 * @return 'addr' message
	 */
	public static Message buildAddressMessage(Peer peer, List<PeerAddress> addressList, PeerAddress localAddress) {
		//
		// Create an address list containing the first 1000 peers plus our own
		// local address.
		// Static addresses are not included in the list nor are peers that
		// provide no services.
		//
		...
		msgBuffer.putVarInt(addresses.size()).putBytes(addresses);
		//
		// Build the message
		//
	 ...
	 *
	 * Build the address list and then notify the processAddress() message
	 * listener.
	 *
	 * @param msg
	 *            Message
	 ...
	 *             Message contains more than 1000 entries
	 */
