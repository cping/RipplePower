...
public class Base58 {
	...
	public static String encode(byte[] bytes) {
		...
		//
		// Count the number of leading zeroes (we will need to prefix the
		// encoded result
	 ...
	 *             Invalid Base-58 encoded string
	 */
	public static byte[] decode(String string) throws AddressFormatException {
		...
		while (zeroCount < input.length && input[zeroCount] == 0)
			zeroCount++;
		//
		// Convert from Base58 encoding starting with the first non-zero
		// character
		//
	 ...
	 *
	 * @param string
	 *            Base-58 encoded checksummed string
	 * @return Decoded value
	 * @throws AddressFormatException
	 *             The string is not valid or the checksum is incorrect
	 */
	public static byte[] decodeChecked(String string) throws AddressFormatException {
		...
		byte[] hash = Arrays.copyOfRange(Helper.doubleDigest(bytes), 0, 4);
	...
	/**
	 * Divide the current number by 256 and return the remainder. The input
	 * array is updated for the next round.
	 *
	 * @param number
	 *            Number array
	 * @param offset
