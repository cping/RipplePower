...
public class LoadBlockChain {
	 ...
	 *            Reference client application directory path
	 * @param startBlock
	 *            Starting block file number
	 * @param stopBlock
	 *            Stop block file number
	 */
	public static void load(String blockChainPath, int startBlock, int stopBlock) {
		...
		try {
			//
			// Get the block chain file list from the 'blocks' subdirectory
			// sorted by ordinal value
			// (blk00000.dat, blk00001.dat, blk00002.dat, etc). We will stop
			// when we reach a
			// non-existent file.
			//
			...
			for (int i = startBlock; i <= stopBlock; i++) {
				File file = new File(
						String.format("%s%sblocks%sblk%05d.dat.gz", blockChainPath, LSystem.FS, LSystem.FS, i));
				if (!file.exists()) {
					break;
				}
				fileList.add(file);
			...
			if (fileList.isEmpty()) {
				for (int i = startBlock; i <= stopBlock; i++) {
					File file = new File(
							String.format("%s%sblocks%sblk%05d.dat", blockChainPath, LSystem.FS, LSystem.FS, i));
					if (!file.exists())
						break;
					fileList.add(file);
				}
			...
			//
			// The blocks in the file are separated by 4 bytes containing the
			// network-specific packet magic
			// value. The next 4 bytes contain the block length in little-endian
			// format.
			//
			...
			for (File inFile : fileList) {
				...
				while (!stopLoad) {
					//
					// Get the magic number and the block length from the input
					// stream.
					// Stop when we reach the end of the file or the magic
					// number is zero.
					//
					...
					if (count != (int) length) {
						BTCLoader.error(
								String.format("Block truncated: Needed %d bytes, Read %d bytes", (int) length, count));
						throw new IOException("Incorrect file format");
					}
					//
					// Create a new block from the serialized byte stream. Since
					// we are
					// reading from a trusted input source, we won't waste time
					// verifying blocks
					// (this also avoids a lot of problems with bad blocks that
					// made it into
					// the block chain before the rules were tightened)
					//
					Block block = new Block(blockBuffer, 0, count, false);
					//
					// Add the block to the block store and update the block
					// chain. Stop
					// loading blocks if we get 25 consecutive blocks being held
					// (something
	 ...
	 *
	 * We need to issue multiple read() requests when using GZIPInputStream
	 * because a read() will return when the end of the internal buffer is
	 * reached. A subsequent read() will then refill the internal buffer and
	 * continue processing the input stream.
	 *
	 * @param in
	 *            Input stream
	 * @param buffer
