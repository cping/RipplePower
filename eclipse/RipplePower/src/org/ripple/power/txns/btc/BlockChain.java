...
public class BlockChain {
	 ...
	 *
	 * @param verifyBlocks
	 *            TRUE if new blocks should be verified
	 */
	 ...
	 * @param block
	 *            The block to add
	 * @return List of blocks that have been added to the chain. The first
	 *         element in the list is the junction block and will not contain
	 *         any block data. The list will be null if no blocks have been
	 *         added to the chain.
	 * @throws BlockStoreException
	 *             Unable to store the block in the database
	 */
	public List<StoredBlock> storeBlock(Block block) throws BlockStoreException {
		//
		// Store the block in the database with hold status until we have
		// verified the block
		//
	...
	/**
	 * Updates the block chain to reflect a new or updated block. The block must
	 * be in the block store and must be on hold if this is a new block that
	 * hasn't been verified yet.
	 *
	 * @param storedBlock
	 *            The new or updated stored block
	 * @return List of blocks that have been added to the chain. The first
	 *         element in the list is the junction block and will not contain
	 *         any block data. The list will be null if no blocks have been
	 *         added to the chain.
	 * @throws BlockStoreException
	 *             Unable to update the block chain in the database
	 */
	public List<StoredBlock> updateBlockChain(StoredBlock storedBlock) throws BlockStoreException {
		...
		//
		// Locate the chain containing this block and map the transactions in
		// the chain.
		// We will need this information when validating transactions since
		// these transactions
		// are not in the database yet.
		//
		// A BlockNotFoundException is thrown if a block in the chain is not in
		// the database.
		// This can happen if we receive blocks out-of-order. In this case, we
		// need to place
		// the new block on hold until we receive another block in the chain. We
		// will add
		// the missing block to the list of blocks to be fetched from a peer.
		//
		// A ChainTooLongException is thrown if the block chain exceeds 144
		// blocks. This is
		// done to avoid running out of storage as the unresolved chain
		// increases in size.
		// The exception contains the hash for the restart block. We will
		// recursively call
		...
		if (onHold)
			return null;
		//
		// The block version must be 2 (or greater) if the chain height is
		// 250,000 or greater
		//
		...
		//
		// Check for any held blocks in the chain. If we find one, attempt to
		// verify it.
		// If the verification fails, we will need to wait until another block
		// is received
			...
			if (chainBlock != null) {
				...
				if (chainStoredBlock.isOnHold()) {
					...
					BTCLoader.info(String
							.format(String.format("Held block released\n  Block %s", chainBlock.getHashAsString())));
					for (ChainListener listener : listeners) {
		...
		//
		// Release the block and update the chain work and block height values
		// in the database
		//
		...
		//
		// Make this block the new chain head if it is a better chain than the
		// current chain.
		...
		if (storedBlock.getChainWork().compareTo(BTCLoader.blockStore.getChainWork()) > 0) {
			try {
				...
				//
				// Delete spent transaction outputs if we are caught up with the
				// network
				//
				...
				chainList = null;
				BTCLoader
						.info(String.format("Block being held due to verification failure\n  Block %s", exc.getHash()));
			}
		}
		return chainList;
	}
	 ...
	 * @param junctionHeight
	 *            Height of the junction block
	 * @param txMap
	 *            Transaction map
	 * @param outputMap
	 ...
	 */
	private boolean verifyBlock(StoredBlock storedBlock, int junctionHeight, Map<Sha256Hash, Transaction> txMap,
			Map<Sha256Hash, List<StoredOutput>> outputMap) throws BlockStoreException {
		Block block = storedBlock.getBlock();
		...
		for (Transaction tx : txList) {
			//
			// The input script for the coinbase transaction must contain the
			// chain height
			...
			for (TransactionInput input : inputs) {
					...
					if (outTx == null) {
						...
						if (outputs == null) {
							BTCLoader.error(String.format(
									"Transaction input specifies unavailable transaction\n"
											+ "  Transaction %s\n  Transaction input %d\n  Connected output %s",
				...
				//
				// Add the input amount to the running total for the
				// transaction.
				// Verify the input signature against the connected output. We
				// allow a double-spend
				// if the spending block is above the junction block since that
				// spending block will
				...
				if (txValid) {
					...
					if (!foundOutput) {
						...
						BTCLoader.error(String.format(
								"Transaction input specifies non-existent output\n"
										+ "  Transaction %s\n  Transaction input %d\n"
						...
						if (output.isSpent() && output.getHeight() != 0 && output.getHeight() <= junctionHeight) {
							...
							BTCLoader.error(String.format(
									"Transaction input specifies spent output\n"
							...
							if (output.isCoinBase()) {
								...
								if (txDepth < BTCLoader.COINBASE_MATURITY) {
									BTCLoader.error(String.format(
											"Transaction input specifies immature coinbase output\n"
													+ "  Transaction %s\n  Transaction input %d\n"
													+ "  Connected output %s\n  Connected output index %d",
											tx.getHash(), input.getIndex(), opHash, opIndex));
									txValid = false;
							...
							if (!txValid) {
								BTCLoader.error(String.format(
										"Transaction failed signature verification\n"
												+ "  Transaction %s\n  Transaction input %d\n"
												+ "  Outpoint %s\n  Outpoint index %d",
										tx.getHash(), input.getIndex(), op.getHash(), op.getIndex()));
							}
				...
				//
				// Stop processing transaction inputs if this input failed to
				// verify
				//
				if (!txValid) {
					break;
			...
			//
			// Get the amount for each output and subtract it from the
			// transaction total
			//
			if (txValid) {
				...
				if (txAmount.compareTo(BigInteger.ZERO) < 0) {
					BTCLoader.error(
							String.format("Transaction inputs less than transaction outputs\n  Tx %s", tx.getHash()));
					txValid = false;
				} else {
			...
			//
			// Stop processing the block transactions if we already have a
			// failed transaction
			//
			if (!txValid)
				break;
		...
		//
		// The coinbase amount must not exceed the block reward plus the
		// transaction fees for the block.
		// The block reward starts at 50 BTC and is cut in half every 210,000
		// blocks.
		//
		if (txValid) {
