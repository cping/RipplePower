...
public class TransactionEntry {
	 ...
	 *            Script bytes
	 * @param timeSpent
	 *            Time when all outputs were spent
	 * @param blockHeight
	 *            Height of block spending this output
	 * @param isCoinBase
	 *            TRUE if this is a coinbase transaction
	 */
	public TransactionEntry(Sha256Hash blockHash, BigInteger value, byte[] scriptBytes, long timeSpent, int blockHeight,
			boolean isCoinBase) {
	 ...
	 * @throws EOFException
	 *             End-of-data processing serialized data
	 */
	...
	public byte[] getBytes() {
		...
		outBuffer.putBytes(blockHash.getBytes()).putVarLong(timeSpent).putVarInt(blockHeight)
				.putVarInt(valueData.length).putBytes(valueData).putVarInt(scriptBytes.length).putBytes(scriptBytes)
	 ...
	 *
	 * @param timeSpent
	 *            Time spent or zero if all outputs have not been spent
	 */
