...
/**
 * <p>
 * A transaction output has the following format:
 * </p>
 * 
 * <pre>
...
public class TransactionOutput implements ByteSerializable {
	...
	private final byte[] unspendableScript = new byte[] { (byte) 0x76, (byte) 0xa9, (byte) 0x14, (byte) 0x81,
			(byte) 0x88, (byte) 0x95, (byte) 0xf3, (byte) 0xdc, (byte) 0x2c, (byte) 0x17, (byte) 0x86, (byte) 0x29,
			(byte) 0xd3, (byte) 0xd2, (byte) 0xd8, (byte) 0xfa, (byte) 0x3e, (byte) 0xc4, (byte) 0xa3, (byte) 0xf8,
			(byte) 0x17, (byte) 0x98, (byte) 0x21, (byte) 0x88, (byte) 0xac };
	...
	/**
	 * Creates a transaction output for the specified amount using the supplied
	 * script
	 *
	 * @param txIndex
	 *            Transaction output index
	...
	public SerializedBuffer getBytes(SerializedBuffer outBuffer) {
		outBuffer.putLong(value.longValue()).putVarInt(scriptBytes.length).putBytes(scriptBytes);
		return outBuffer;
	}
	 ...
	 *
	 * Proof-of-burn transactions are sent to
	 * '1CounterpartyXXXXXXXXXXXXXXXUWLpVr'. This address has no private key and
	 * thus can never be spent. So we will mark it as unspendable.
	 *
	...
	public void serializeForSignature(int index, int hashType, SerializedBuffer outBuffer) {
		if (hashType == ScriptOpCodes.SIGHASH_SINGLE && index != txIndex) {
			//
			// For SIGHASH_SINGLE, we have a zero-length script and a value of
			// -1
			//
			outBuffer.putLong(-1L).putVarInt(0);
		} else {
			//
			// Encode normally
			//
