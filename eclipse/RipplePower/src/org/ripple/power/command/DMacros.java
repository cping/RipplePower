...
public class DMacros extends Conversion implements Serializable {
	 ...
	 * @param fileName
	 */
	public DMacros(String fileName) {
	 ...
	 * @param resource
	 */
	public DMacros(String fileName, String[] res) {
		this.tmpName = fileName;
		createCache(false);
	...
	public void formatCommand(String fileName) {
		formatCommand(fileName, DMacros.includeFile(fileName));
	}

	public void formatCommand(InputStream in) {
		formatCommand("temp" + in.hashCode(), DMacros.includeFile(in));
	}
	...
	public static List getNameTags(String messages, String startString, String endString) {
		return DMacros.getNameTags(messages.toCharArray(), startString.toCharArray(), endString.toCharArray());
	}
	...
	private void setupRandom(String cmd) {
		...
		if (cmd.indexOf(RAND_TAG) != -1) {
			randTags = DMacros.getNameTags(cmd, RAND_TAG + BRACKET_LEFT_TAG, BRACKET_RIGHT_TAG);
			if (randTags != null) {
	...
	public synchronized String doExecute() {
		...
		try {
			...
			// 结束脚本中代码段标记
			if (cmd.equals(END_TAG) || cmd.endsWith(END_TAG)) {
				functioning = false;
				return executeCommand;
			}
			...
			if (((!esleflag && !ifing) || (esleflag && ifing)) && cmd.startsWith(CALL_TAG) && !isCall) {
				...
				if (temps.size() == 2) {
					...
					if (funs != null) {
						innerCommand = new DMacros(scriptName + FLAG + functionName, funs);
			...
			if (executeCommand != null) {
				printTags = DMacros.getNameTags(executeCommand, PRINT_TAG + BRACKET_LEFT_TAG, BRACKET_RIGHT_TAG);
				if (printTags != null) {
	...
	private final boolean includeCommand(String cmd) {
		...
		if (fileName.length() > 0) {
			innerCommand = new DMacros(fileName);
			isInnerCommand = true;
			return true;
		}
		return false;
	 ...
	 * @return
	 */
	public static List<String> commandSplit(final String src, final boolean useNumber) {
		if (src.length() == 0) {
