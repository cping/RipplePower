...
getTextKey:		function(){
					if(rippleUtils.getExplorer() == "firefox"){
						return "textContent";
					}
					return "innerText";
				},
...
currentEvent:	function()
				{
					...
					func= rippleUtils.currentEvent.caller;
					while(func!=null){
...
mergeList : function (target, source, isAll) {
			for (var p in source) {
...
var rippleset =  window.rippleset || {

	showConfig : {
		imagePrefix	: "",
		cssPrefix		: "",
		staticPrefix	: "",
		cssUrl:	"resource/ripple.css"
	},

   listConfig:{
	   			'ripple'	:   {icon: "resource/ripple.png", name:"Ripple", desc: "瑞波网络讨赏",url:"", className:""},
				'alipay'	: 	{icon: "resource/alipay.png", name:"支付宝", desc: "支付宝讨赏", url:"",className: ""},
	...
	imageUrl:	function(url){
					return rippleUtils.staticUrl(rippleset.showConfig.staticPrefix || rippleset.showConfig.imagePrefix, url);
				},
	cssUrl:	function(url){
					return rippleUtils.staticUrl(rippleset.showConfig.staticPrefix || rippleset.showConfig.cssPrefix, url);
				},
	buttonImageUrl: function(){
						var url = rippleConfig.buttonImage;
						if(url == "" || url == null || url.length == 0){
						    return rippleset.imageUrl("resource/donate.png");
						}else{
							return rippleset.imageUrl(url);
						}
				},

	generateMyConfig:	function(){
							rippleset.showConfig = rippleUtils.mergeList(rippleset.showConfig, rippleConfig);
							var list = [];
							var num = 0;
							var hasOn = false;
							for(var key in rippleConfig.list){
								if(rippleset.listConfig.hasOwnProperty(key)){
									var one = rippleUtils.mergeList(rippleset.listConfig[key], rippleConfig.list[key], true);
								}else{
									var one = rippleConfig.list[key];
								}
								if(one.className == "ripple-on"){
									hasOn = true;
								}
								list.push(one);
								num += 1;
							...
							if(hasOn == false){
								list[0].className = "ripple-on";
							}

							rippleset.showConfig.list = list;
						},
								...
								for(var i= 0; i < 5; i++){
									...
									(function(){
										...
										var a_params = {
												href		:"javascript:;", 
												onmouseover : function(){
															rippleset.leftMouseover(this, tmp_one);
															}
												};
											a_params[rippleUtils.getTextKey()] = one.name;
										if(i==4){
											a_params.className = "fifth";
										}
										a_el = rippleUtils.createElement(a_params, 'a', li_el);
									})();
									...
									rippleUtils.createElement({className: "png", src: rippleset.imageUrl(one.icon), alt:one.name}, 'img', a_el);
								}
							},
	generateDonateDetail:function(){
	 ...
	 leftMouseover	:	function(element, data){
							rippleset.currentLi.className = "";
							element.parentNode.className = "ripple-on";
							rippleset.currentLi = element.parentNode;
							rippleset.currentData = data;
							rippleset.generateDonateDetail();
	},
	importCss:	function(){
						rippleUtils.createElement({type: "text/css",rel: "stylesheet", href: rippleset.cssUrl(rippleset.showConfig.cssUrl)}, "link");
				},
	loading:		function(){
						rippleset.generateMyConfig();
						rippleset.importCss();
						rippleset.generateDonate();
				}
};
if(!rippleConfig.runOnLoad){
	rippleset.loading();
}else{
	rippleUtils.addLoadEvent(rippleset.loading);
}
